"""
 @file
 @brief Build server used to generate daily builds of libsmartedit-audio, libsmartedit, and smartedit-qt
...
WINDOWS_STORE_MSIX_NAME = "SmartEditStudios.SmartEditforWindows"
WINDOWS_STORE_MSIX_PUBLISHER = "CN=5FE34B8B-A62B-4594-911F-0D6CFC87D00F"
WINDOWS_STORE_MSIX_PUBLISHER_DISPLAY_NAME = "SmartEdit Studios"
...
def zulip_upload_log(zulip_token, log, title, comment=None):
    ...
    zulip_auth = HTTPBasicAuth('builder-bot@smartedit.zulipchat.com', zulip_token)
    filename = "%s-build-server.txt" % platform.system()

    # Upload file to Zulip
    zulip_url = 'https://smartedit.zulipchat.com/api/v1/user_uploads'
    zulip_upload_url = ''
    ...
    # SEND MESSAGE
    zulip_url = 'https://smartedit.zulipchat.com/api/v1/messages'
    zulip_data = {
        "type": "stream",
        "to": "build-server",
        "subject": topic,
...
def get_expected_msix_version():
    """Return the artifact SmartEdit version in the four-part MSIX format."""
    source_version = version_info.get("smartedit-qt", {}).get("VERSION")
    match = re.match(r"^(\d+)\.(\d+)\.(\d+)(?:\.(\d+))?(?:[-+].*)?$", source_version or "")
    if not match:
        raise RuntimeError("Invalid or missing smartedit-qt artifact version: %s" % source_version)
    return ".".join((match.group(1), match.group(2), match.group(3), match.group(4) or "0"))


def replace_msix_visual_assets(package_root):
    """Replace every Packaging Tool logo variant using the canonical SmartEdit logo."""
    from PIL import Image

    package_assets_dir = os.path.join(package_root, "Assets")
    source_logo_path = os.path.join(PATH, "xdg", "icon", "512", "smartedit-qt.png")
    asset_specs = {
        "StoreLogo": (50, 50),
        "SMARTEDITQT-Square44x44Logo": (44, 44),
        "SMARTEDITQT-Square71x71Logo": (71, 71),
        "SMARTEDITQT-Square150x150Logo": (150, 150),
        "SMARTEDITQT-Square310x310Logo": (310, 310),
        "SMARTEDITQT-Wide310x150Logo": (310, 150),
    }
    if not os.path.isfile(source_logo_path):
        error("Canonical SmartEdit logo not found: %s" % source_logo_path)
        return False
...
def prepare_windows_store_msix(msix_path):
    ...
    expected_application = {
        "application_id": "SMARTEDITQT",
        "executable": "smartedit-qt.exe",
    ...
    try:
        ...
        application.setAttribute("Executable", "smartedit-qt.exe")
    ...
    for key, expected_value in expected_identity.items():
        if final_metadata[key] != expected_value:
            ...
            return False
    if final_metadata["executable"] != "smartedit-qt.exe":
        error("Repacked MSIX does not use the SmartEdit GUI executable")
        return False
...
def sign_windows_installer(installer_path, debug=False):
    ...
    metadata = {
        ...
        "CorrelationId": os.getenv("AZURE_CODESIGN_CORRELATION_ID", "smartedit-gitlab")
    }
...
def main():
    ...
    try:
        ...
        for repo_name in ["libsmartedit-audio", "libsmartedit", "smartedit-qt"]:
        ...
        # Get GIT description of smartedit-qt-git branch (i.e. v2.0.6-18-ga01a98c)
        smartedit_qt_git_desc = parse_build_name(version_info, git_branch_name)
        needs_upload = True
        ...
        output("git description of smartedit-qt-git: %s" % smartedit_qt_git_desc)

        # Detect version number from git description
        version = re.search('v(.+?)($|-)', smartedit_qt_git_desc).groups()[0]

        # Determine the name and path of the final installer
        app_name = smartedit_qt_git_desc
        app_upload_bucket = ""
        if platform.system() == "Linux":
            app_name += "-x86_64.AppImage"
            app_upload_bucket = "releases.smartedit.org/linux"
        elif platform.system() == "Darwin":
            app_name += "-x86_64.dmg"
            app_upload_bucket = "releases.smartedit.org/mac"
        elif platform.system() == "Windows" and not windows_32bit:
            app_name += "-x86_64.exe"
            app_upload_bucket = "releases.smartedit.org/windows"
        ...
        if platform.system() == "Linux":
            ...
            app_dir_path = os.path.join(PATH, "build", "SmartEdit.AppDir")
            ...
            icons = [
                ("scalable", os.path.join(PATH, "xdg", "smartedit-qt.svg")),
                ("64x64", os.path.join(PATH, "xdg", "icon", "64", "smartedit-qt.png")),
                ("128x128", os.path.join(PATH, "xdg", "icon", "128", "smartedit-qt.png")),
                ("256x256", os.path.join(PATH, "xdg", "icon", "256", "smartedit-qt.png")),
                ("512x512", os.path.join(PATH, "xdg", "icon", "512", "smartedit-qt.png")),
                ]
            ...
            shutil.copyfile(icons[0][1], os.path.join(app_dir_path, "smartedit-qt.svg"))

            dest = os.path.join(app_dir_path, "usr", "share", "pixmaps")
            os.makedirs(dest, exist_ok=True)

            # Copy pixmaps (as a 64x64 PNG & SVG)
            shutil.copyfile(icons[0][1], os.path.join(dest, "smartedit-qt.svg"))
            shutil.copyfile(icons[1][1], os.path.join(dest, "smartedit-qt.png"))

            # Install MIME handler
            dest = os.path.join(app_dir_path, "usr", "share", "mime", "packages")
            os.makedirs(dest, exist_ok=True)
            shutil.copyfile(os.path.join(PATH, "xdg", "org.smartedit.SmartEdit.xml"),
                            os.path.join(dest, "org.smartedit.SmartEdit.xml"))

            # Install AppStream XML metadata
            dest = os.path.join(app_dir_path, "usr", "share", "metainfo")
            os.makedirs(dest, exist_ok=True)
            shutil.copyfile(os.path.join(PATH, "xdg", "org.smartedit.SmartEdit.appdata.xml"),
                            os.path.join(dest, "org.smartedit.SmartEdit.appdata.xml"))
            ...
            desk_in = os.path.join(PATH, "xdg", "org.smartedit.SmartEdit.desktop")
            desk_out = os.path.join(app_dir_path, "org.smartedit.SmartEdit.desktop")
            with open(desk_in, "r") as inf, open(desk_out, "w") as outf:
                for line in inf:
                    if line.startswith("Exec="):
                        outf.write("Exec=smartedit-qt-launch %F\n")
                    else:
                        outf.write(line)
            # Copy modified .desktop file to usr/share/applciations
            dest = os.path.join(app_dir_path, "usr", "share", "applications")
            os.makedirs(dest, exist_ok=True)
            shutil.copyfile(os.path.join(app_dir_path, "org.smartedit.SmartEdit.desktop"),
                            os.path.join(dest, "org.smartedit.SmartEdit.desktop"))

            # Rename executable launcher script
            launcher_path = os.path.join(app_dir_path, "usr", "bin", "smartedit-qt-launch")
            ...
            # Create AppImage (SmartEdit-%s-x86_64.AppImage)
        ...
        if platform.system() == "Darwin":
            # Create DMG (SmartEdit-%s-x86_64.DMG)
            app_image_success = False
        ...
        if platform.system() == "Windows":
            ...
            if windows_mode != "sign-upload-only":
                ...
                # Remove a redundant smartedit_qt module folder (duplicates lots of files)
                duplicate_smartedit_qt_path = os.path.join(python_dir, 'smartedit_qt')
                if os.path.exists(duplicate_smartedit_qt_path):
                    shutil.rmtree(duplicate_smartedit_qt_path, True)
                ...
                launcher_exe = os.path.join(exe_dir, "smartedit-qt.exe")
                ...
                # Create Installer (SmartEdit-%s-x86_64.exe)
                inno_success = True
                inno_command = " ".join([
                    'iscc.exe',
                    '/Q',
                ...
                inno_output_exe = os.path.join(PATH, "installer", "Output", "SmartEdit.exe")
        ...
        if needs_upload and os.path.exists(app_build_path):
            ...
            torrent_command = " ".join([
                ...
                '-c "SmartEdit Video Editor %s"' % version,
                '-w "%s"' % download_url,
