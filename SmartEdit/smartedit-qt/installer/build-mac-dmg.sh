...
OS_APP_NAME="SmartEdit Video Editor.app"
OS_DMG_NAME="SmartEdit-$VERSION.dmg"
...
mv "$OS_PATH/MacOS/smartedit-qt.hqx" "$OS_PATH/Resources/smartedit-qt.hqx"; ln -s "../Resources/smartedit-qt.hqx" "$OS_PATH/MacOS/smartedit-qt.hqx";
...
find "build" \( -iname '*.dylib' -o -iname '*.so' \) -exec codesign -s "Developer ID Application: SmartEdit Studios, LLC (C94ZNQ5JF3)" --timestamp=http://timestamp.apple.com/ts01 --entitlements "installer/smartedit.entitlements" --force "{}" \;

echo "Code Sign App Bundle (deep)"
codesign -s "Developer ID Application: SmartEdit Studios, LLC (C94ZNQ5JF3)" --force --deep --entitlements "installer/smartedit.entitlements" --options runtime --timestamp=http://timestamp.apple.com/ts01 "build/$OS_APP_NAME"
...
echo "Code Sign DMG"
codesign -s "Developer ID Application: SmartEdit Studios, LLC (C94ZNQ5JF3)" --force --entitlements "installer/smartedit.entitlements" --timestamp=http://timestamp.apple.com/ts01 "build/$OS_DMG_NAME"

echo "Notarize DMG file (submit to Apple)"
# No errors uploading '/Users/jonathan/builds/7d5103a1/0/SmartEdit/smartedit-qt/build/test.zip'.
# RequestUUID = cc285719-823f-4f0b-8e71-2df4bbbdaf72
notarize_output=$(xcrun notarytool submit --apple-id "jonathan@smartedit.org" --password "$MAC_NOTARIZE_PASSWORD" --team-id "C94ZNQ5JF3" --wait "build/$OS_DMG_NAME")
