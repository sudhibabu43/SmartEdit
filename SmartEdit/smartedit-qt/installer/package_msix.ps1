...
function Get-MsixVersion {
    ...
    $versionFile = Join-Path $ArtifactRoot "build\install-x64\share\smartedit-qt.env"
    if (-not (Test-Path -Path $versionFile -PathType Leaf)) {
        throw "SmartEdit version metadata not found: $versionFile"
    }
    ...
    if ($version -notmatch '^(\d+)\.(\d+)\.(\d+)(?:\.([0-9]+))?(?:[-+].*)?$') {
        throw "Unsupported SmartEdit version for MSIX: $version"
    }
        ...
        $capturedInstallers = @(
            foreach ($entry in $archive.Entries) {
                ...
                if ($normalizedName -like "VFS/AppVPackageDrive/*/$sourceInstallerName" -or
                    $normalizedName -like "VFS/AppVPackageDrive/*/SmartEdit-*-x86_64.exe") {
                    $entry.FullName
                }
            }
        )
...
$installerMatches = @(Get-ChildItem -Path "build" -Filter "SmartEdit-*-x86_64.exe" -File)
Assert-SingleArtifact -Artifacts $installerMatches -Description "build\SmartEdit-*-x86_64.exe installer"
...
$templatePath = Join-Path $PSScriptRoot "smartedit-msix-template.xml"
...
$sourceInstallerDir = Join-Path ([System.IO.Path]::GetTempPath()) "SmartEdit-MSIX-InstallerSource"
...
$workingTemplatePath = Join-Path $outputDir "SmartEdit_template.generated.xml"
...
$generatedPackagePath = Join-Path $sourceInstallerDir "SmartEdit.generated.msix"
$generatedTemplateOutputPath = Join-Path $outputDir "SmartEdit_template.output.xml"
...
$msixPackageName = "SmartEditStudios.SmartEditforWindows"
Set-TemplateAttribute -TemplatePath $workingTemplatePath -ElementName "PackageInformation" `
...
$publisherDisplayName = "SmartEdit Studios"
Set-TemplateAttribute -TemplatePath $workingTemplatePath -ElementName "PackageInformation" `
