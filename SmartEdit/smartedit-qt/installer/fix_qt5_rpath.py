"""Methods to fix Mac OS X @rpath and /usr/local dependencies, and analyze the
   app bundle for SmartEdit to find all external dependencies"""
import os
import subprocess
...
def fix_rpath(PATH):
    ...
    duplicate_path = os.path.join(PATH, "lib", "smartedit_qt")
...
def print_min_versions(PATH):
    """Print ALL MINIMUM and SDK VERSIONS for files in SmartEdit build folder.
...
if __name__ == "__main__":
    ...
    PATH = "/Users/jonathanthomas/apps/smartedit-qt/build/exe.macosx-10.15-x86_64-3.7"
    fix_rpath(PATH)
