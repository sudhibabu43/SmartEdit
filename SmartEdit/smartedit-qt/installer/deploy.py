"""
 @file
 @brief Script used to deploy and publish smartedit-qt
...
RELEASE_NAME_REGEX = re.compile(r'^SmartEdit-v.*?(-.*?)-x86[_64]*')
DOWNLOAD_PAGE_URLS = re.compile(r'"(//github.com/SmartEdit/smartedit-qt/releases/download/v.*?/SmartEdit-.*?)"',
                                re.MULTILINE)
...
def main():
    ...
    repo_names = ["smartedit-qt", "libsmartedit", "libsmartedit-audio"]
    ...
    try:
        ...
        if len(sys.argv) >= 4:
            ...
            for repo_name in repo_names:
                repos[repo_name] = gh.repository("SmartEdit", repo_name)
        ...
        # Get version info
        smartedit_qt_version = version_info.get('smartedit-qt', {}).get('VERSION', 'N/A')
        release_git_sha = version_info.get('smartedit-qt', {}).get('CI_COMMIT_SHA', 'N/A')
        ...
        if not is_publish:
            ...
            # NOTE: ONLY for `smartedit-qt` repo
            # github_release_url = github_release.html_url
            github_release = releases.get('smartedit-qt')
                ...
                if os.path.exists(artifact_path) and artifact_ext in ['.exe', '.dmg', '.AppImage']:
                    ...
                    if artifact_ext in ['.exe', '.dmg', '.AppImage']:
                        ...
                        with open(sha256sum_instructions_path, "w", encoding="UTF-8") as f:
                            f.write('Run this command in your terminal in the directory SmartEdit was downloaded to verify '
                        ...
                        torrent_command = " ".join([
                            ...
                            '-c "SmartEdit Video Editor %s"' % github_release.tag_name,
                            '-w "%s"' % download_url,
                            '-o "%s"' % torrent_name,
                            '"%s"' % artifact_name,
                            ])
            ...
            auth = HTTPBasicAuth(os.getenv('SMARTEDIT_ORG_USER'), os.getenv('SMARTEDIT_ORG_PASS'))
            r = post(
                "https://www.smartedit.org/api/release/submit/",
                auth=auth,
                data={
                    "version": smartedit_qt_version,
                    "changelog": log_title + combined_log_markdown,
                    "sha": release_git_sha
                    })
            if not r.ok:
                raise Exception(
                    "HTTP post to smartedit.org/api/release/submit/ failed: %s (user: %s): %s" % (
                        r.status_code,
                        os.getenv('SMARTEDIT_ORG_USER'),
                        r.content.decode('UTF-8'))
                )
        else:
            # Publish the release (make new version visible on smartedit.org, and make blog post visible)
            auth = HTTPBasicAuth(os.getenv('SMARTEDIT_ORG_USER'), os.getenv('SMARTEDIT_ORG_PASS'))
            r = post(
                "https://www.smartedit.org/api/release/publish/",
                auth=auth,
                data={"version": smartedit_qt_version})
            if not r.ok:
                raise Exception(
                    "HTTP post to smartedit.org/api/release/publish/ failed: %s (user: %s): %s" % (
                        r.status_code,
                        os.getenv('SMARTEDIT_ORG_USER'),
                        r.content.decode('UTF-8')))
            ...
            for repo_name in repo_names:
                ...
                if github_release:
                    ...
                    raise Exception("Cannot publish missing GitHub release: %s, version: %s" % (repo_name, smartedit_qt_version))

            # Verify download links on smartedit.org are correct (and include the new release version)
            r = get("https://www.smartedit.org/download/")
            if r.ok:
                # Find all release download URLs on smartedit.org
                ...
                for url in urls:
                    ...
                    if smartedit_qt_version not in url:
                        raise Exception(
                            "Validation of URL FAILED. Missing version %s: %s, %s, %s" % (
                                smartedit_qt_version, url, r.status_code, r.reason)
                        )
            else:
                raise Exception(
                    "Failed to GET smartedit.org/download for URL validation: %s" % r.status_code)
