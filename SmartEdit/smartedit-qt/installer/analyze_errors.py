...
c_error_regex = re.compile(r"(smartedit::\w*?::.*?)\(")
c_mangled_regex = re.compile(r"ZN(\w*)\s")
smartedit_version_regex = re.compile(r"\((.*)\)")

# Message folder with exported archived error messages
messages_folder = "/home/jonathan/Downloads/SmartEdit Project Slack export Jul 9 2018 - Aug 8 2018/python-exceptions"
            ...
            with open(message_path, "r") as f:
                for message in json.load(f):
                    for file_details in message.get("files", []):
                        ...
                        # Parse smartedit version (if any)
                        version = "Unknown"
                        m = smartedit_version_regex.search(file_title)
                        if m and m.groups():
                            version = m.groups()[0]
    ...
    if version.startswith(version_starts_with):
        with open(attachment_path, "r", encoding="utf-8") as f:
            ...
            for mangled_line in c_mangled_regex.findall(attachment_data):
                ...
                if not stack_line.startswith("smartedit::"):
                    # Skip non-smartedit methods
                    continue
