"""
 @file
 @brief Parse version info files for libsmartedit-audio, libsmartedit, and smartedit-qt (created on gitlab-ci.yml)
...
def parse_build_name(version_info, git_branch_name=""):
    ...
    if all(key in version_info for key in ("libsmartedit", "libsmartedit-audio", "smartedit-qt")):
        # Construct base build URL pattern
        if release_label:
            # Daily and Release Candidates
            build_name = "SmartEdit-v%s-%s-%s-%s-%s" % (
                version_info.get('smartedit-qt', {}).get('VERSION'),
                release_label,
                version_info.get('smartedit-qt', {}).get('CI_PIPELINE_ID', 'NA'),
                version_info.get('libsmartedit', {}).get('CI_COMMIT_SHA', 'NA')[:8],
                version_info.get('libsmartedit-audio', {}).get('CI_COMMIT_SHA', 'NA')[:8],
            )
        else:
            # Official releases
            build_name = "SmartEdit-v%s" % version_info.get('smartedit-qt', {}).get('VERSION')
