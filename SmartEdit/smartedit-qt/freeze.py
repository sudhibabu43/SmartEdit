"""
 @file
 @brief cx_Freeze script to build SmartEdit package with dependencies (for Mac and Windows)
...
# 1) python3 freeze.py bdist_mac --qt-menu-nib="/usr/local/Cellar/qt5/5.4.2/plugins/platforms/" --iconfile=installer/smartedit.icns --custom-info-plist=installer/Info.plist --bundle-name="SmartEdit Video Editor"
...
#  signtool sign /v /f OSStudiosSPC.pfx "SmartEdit Video Editor-2.0.0-win32.msi"
...
smartedit_copy_path = os.path.join(PATH, "smartedit_qt")
if os.path.exists(os.path.join(PATH, "src")):
    print("Copying modules to smartedit_qt directory: %s" % smartedit_copy_path)
    # Only make a copy if the SRC directory is present (otherwise ignore this)
    copytree(os.path.join(PATH, "src"), smartedit_copy_path)

    # Make a copy of the launch.py script (to name it more appropriately)
    copy(os.path.join(PATH, "src", "launch.py"), os.path.join(PATH, "smartedit_qt", "launch-smartedit"))

if os.path.exists(smartedit_copy_path):
    # Append path to system path
    sys.path.append(smartedit_copy_path)
    print("Loaded modules from smartedit_qt directory: %s" % smartedit_copy_path)
...
print("Importing SmartEdit freeze metadata modules", flush=True)
try:
...
def should_package_source_file(filename):
    """Return True when a copied smartedit_qt source file is needed at runtime."""
    rel_path = os.path.relpath(filename, start=smartedit_copy_path)
    ...
    # The compiled Qt resource module is the runtime source of SmartEdit translations.
    if basename in {"__init__.py", "smartedit_lang.py"}:
        return True
    ...
    # translations that are already embedded into smartedit_lang.py.
    return False

# GUI applications require a different base on Windows
iconFile = "smartedit-qt"
base = None
src_files = []
external_so_files = []
build_options = {}
...
qt_local_path = os.path.join(PATH, "smartedit_qt", "language")
...
if version_info:
    ...
    version_path = os.path.join(smartedit_copy_path, "settings", "version.json")
...
if sys.platform == "win32":
    ...
    # libsmartedit's Python extension links directly to the OpenCV runtime DLLs.
    ...
    if opencv_root or opencv_dll_dir:
        ...
        seed_binaries = []
        seed_binaries.extend(glob.glob(os.path.join(PATH, "_smartedit*.pyd")))
        if artifact_path:
            seed_binaries.extend(glob.glob(os.path.join(artifact_path, "python", "_smartedit*.pyd")))
            seed_binaries.extend(glob.glob(os.path.join(artifact_path, "bin", "*smartedit*.dll")))
    ...
    for filename in find_files("smartedit_qt", ["*"]):
        if should_package_source_file(filename):
            src_files.append((filename, os.path.join(os.path.relpath(filename, start=smartedit_copy_path))))

elif sys.platform == "linux":
    # Find libsmartedit.so path (GitLab copies artifacts into local build/install folder)
    libsmartedit_path = os.path.join(PATH, "build", "install-x64", "lib")
    if not os.path.exists(libsmartedit_path):
        libsmartedit_path = os.path.join(PATH, "build", "install-x86", "lib")
    if not os.path.exists(libsmartedit_path):
        # Default to user install path
        libsmartedit_path = "/usr/local/lib"

    # Find all related SO files
    for filename in find_files(libsmartedit_path, ["*smartedit*.so*"]):
        if '_' in filename or filename.count(".") == 2:
            external_so_files.append((filename, os.path.relpath(filename, start=libsmartedit_path)))
    ...
    info.PRODUCT_NAME = "smartedit-qt"
    ...
    # Get a list of all smartedit.so dependencies (scan these libraries for their dependencies)
    qt_mod_files = []
    ...
    for lib_name in [
            os.path.join(libsmartedit_path, "libsmartedit.so"),
   ...
   # Add libresvg (if found)
    ...
    # Copy smartedit.py Python bindings
    ...
    for filename in find_files("smartedit_qt", ["*"]):
        if should_package_source_file(filename):
            src_files.append((filename, os.path.join("lib", os.path.relpath(filename, start=smartedit_copy_path))))
...
exes = [Executable("smartedit_qt/launch.py",
                   base=base,
...
try:
    ...
    exes.append(Executable("smartedit_qt/launch.py",
                base=extra_exe['base'],
                icon=os.path.join(PATH, "xdg", iconFile),
...
if os.path.exists(os.path.join(PATH, "src")):
    rmtree(smartedit_copy_path, True)
...
all_platform_prune_patterns = [
    ...
    "language/SmartEdit_*.qm",
    "lib/language/SmartEdit_*.qm",
    "lib/smartedit_qt/language/SmartEdit_*.qm",
]
