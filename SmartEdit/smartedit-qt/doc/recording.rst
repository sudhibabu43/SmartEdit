...
SmartEdit can record a microphone, the desktop, and a webcam directly into the current project. You can enable one
...
.. image:: images/recording-view-default.jpg
   :alt: SmartEdit Recording View with the microphone source selected by default
...
8. Wait briefly while SmartEdit finishes the files. They are imported into :guilabel:`Project Files` and, unless
   :guilabel:`No Track` is selected, placed on the timeline.

Quick Audio Commentary
----------------------

To begin commentary at an existing clip, right-click the clip and choose :guilabel:`Audio → Record`. SmartEdit moves
...
device, operating-system capture service, or libsmartedit feature was not detected.
...
During recording, SmartEdit draws a temporary clip and live microphone waveform on the timeline. The completed files
...
All enabled sources start at the same timeline position. SmartEdit stacks them in a useful compositing order:
...
If there are not enough tracks below the selected track, SmartEdit creates or selects additional tracks as needed.
...
Preview resolution changes only the temporary SmartEdit preview. It does not reduce the resolution or quality of the
...
Aim for a healthy meter response without clipping. A quiet waveform should remain visually quiet; SmartEdit no longer
...
.. table:: Screen options
   ...
   Size                Selected bounds  Sets capture width and height. SmartEdit uses encoder-safe even dimensions.
   ...
   Hide SmartEdit       No               Temporarily hides SmartEdit during selection and recording when enabled.
   ==================  ===============  ========================================================================
...
If the control is disabled, the active platform or libsmartedit build does not provide system-audio capture. SmartEdit
...
.. image:: images/recording-view-all-sources.jpg
   :alt: SmartEdit Recording View with microphone, screen, and webcam sources selected
...
SmartEdit uses its user-data location. Filenames contain the source name and a timestamp; another numeric suffix avoids
...
rename recorded files outside SmartEdit while the project is using them.
...
the requested source, then restart SmartEdit if the operating system requires it. A disabled source card or option is
