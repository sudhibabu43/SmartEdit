...
A transition is used to gradually fade (also called a **cross dissolve** or **crossfade**) or wipe between two clip images. In SmartEdit,
...
original clip disappearing abruptly (or popping out of existence), since SmartEdit's transition system expects the 2nd
...
Cutting & Slicing
-----------------
SmartEdit has many easy ways to adjust the start and end trim positions of a transition (otherwise known as cutting or trimming). The most common
...
revealing hidden elements or transitioning between different visual states. In SmartEdit, you can convert a
...
Custom Transition
-----------------
Any greyscale image can be used as a transition (or mask), by adding it to your ``~/.smartedit_qt/transitions/`` folder. Just
be sure to name your file something that is easily recognizable, and restart SmartEdit. Your custom transition/mask will now show
...
.. table::
   ...
   Mask: Time Mode             Enum        Controls how SmartEdit maps transition time to an animated mask source.
...
ensuring its uniqueness. This is a Read-only property, and assigned by SmartEdit when a transition is created.
