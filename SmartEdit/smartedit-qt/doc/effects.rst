...
Effects are used in SmartEdit to enhance or modify the audio or video of a clip. They can modify pixels and audio data,
...
- When looping, set start/end mask trim carefully. If not adjusted, SmartEdit
...
List of Effects
---------------
SmartEdit Video Editor has a total of 43 built-in video and audio effects: 34 video effects and 9 audio effects.
...
Effect Properties
-----------------
Below is a list of **common** effect properties, shared by all effects in SmartEdit. To view an effect's properties,
...
.. table::
   ...
   Mask: Time Mode         Enum        Controls how SmartEdit maps effect time to an animated mask source.
...
*NOTE: Most effects in SmartEdit default the effect duration to the clip duration, and hide this property from the editor.*
...
*NOTE: Most effects in SmartEdit default this property to match the clip, and hide this property from the editor.*

ID
""
The :guilabel:`ID` property holds a randomly generated GUID (Globally Unique Identifier) assigned to each effect,
ensuring its uniqueness. This is a Read-only property, and assigned by SmartEdit when an effect is created.
...
sequence of events, and this is the default behavior of effects in SmartEdit. However, you can optionally override this
...
effect can even animate the text fading in/out, and supports any font, size, color, and margin. SmartEdit also has an
...
and press the plus icon again. SmartEdit inserts the current playhead timestamp as the cue's end time.
...
photography blogs or marketplaces, such as https://freshluts.com/. SmartEdit includes a selection of popular LUTs
...
used for most HD and online video today. By shipping with **Rec 709** LUTs, SmartEdit makes it simple to apply a grade
...
The following **Rec 709** LUT files are included in SmartEdit, organized into the following categories:
...
primary method for cropping a Clip in SmartEdit. The ``left``, ``right``, ``top``, and ``bottom`` key-frames can
...
neighboring frames and only blends when the image appears stable enough. When SmartEdit detects a seek, scrub, frame
...
rectangular tracking box, you mark the subject with points or rectangles and SmartEdit follows its visible outline
...
Object Mask runs locally in SmartEdit and does not require a ComfyUI server. SmartEdit uses an
`EfficientSAM model <https://github.com/SmartEdit/smartedit-onnx/tree/main/efficient-sam>`_ to turn your prompts into a
mask on selected frames, then uses
`Cutie models <https://github.com/SmartEdit/smartedit-onnx/tree/main/cutie>`_ to propagate that mask through the video.
These OpenCV-friendly model packages are maintained in the
`SmartEdit ONNX repository <https://github.com/SmartEdit/smartedit-onnx>`_. The model files are downloaded the first time
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^
Object Mask is the quickest built-in workflow when you need a reusable mask inside an SmartEdit project. The similar
...
.. table::
   ...
   protobuf_data_path          ``(string)`` Internal path to the processed mask data. Normally managed by SmartEdit.
   ==========================  ================================================================================

Object Detector
"""""""""""""""
The Object Detector effect automatically finds and follows known classes of objects throughout a video. Depending
on the selected YOLO model, these can include people, vehicles, animals, and many common objects. SmartEdit stores each
...
4. Click :guilabel:`Process Effect`. SmartEdit analyzes the clip and creates the tracked-object data. Processing can
...
over time. Select the Tracker effect, then use the video preview transform handles to move or resize the box; SmartEdit
