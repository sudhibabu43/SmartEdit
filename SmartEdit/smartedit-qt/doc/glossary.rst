...
contacting support@smartedit.org.
...
Gravity:
   Gravity in SmartEdit is a property of each clip that sets the clip's initial position on the screen.
Green screen
...
~~~
Ken Burns Effect:
   A pan-and-zoom animation technique applied to still images or video clips to create the illusion of camera movement. Named after documentary filmmaker Ken Burns, who popularized the approach. In SmartEdit, use :guilabel:`Right-Click → Motion → Camera` presets for one-click Ken Burns animations.
Key:
...
Lower Third:
   A text or graphic overlay positioned in the lower third of the screen, commonly used to display a speaker's name, title, or other identifying information. Lower thirds are a staple of news, documentaries, and corporate video. In SmartEdit, create them using the Title Editor and place the title clip on a track above your video.
LUT (Look-Up Table):
   A file that maps one set of colors to another, used to apply a specific color grade or look to a video clip in one step. LUTs can replicate film stock emulations, broadcast standards, or custom styles. SmartEdit's Color Grade effect supports LUT files directly.
...
Project:
   A project is all the files, transitions, effects, and animations that you make or use within SmartEdit.
