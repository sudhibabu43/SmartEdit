...
SmartEdit has been designed specifically with animation in mind. The powerful curve-based animation framework can
handle most jobs with ease, and is flexible enough to create just about any animation. Key frames specify
values at certain points on a clip, and SmartEdit does the hard work of interpolating the in-between values.
...
Key Frames
----------
To create a key frame in SmartEdit, simply position the play-head (i.e. playback position) at any point over a clip,
...
- The :guilabel:`Timing` tool lets you drag a clip’s edges to speed it up or slow it down. SmartEdit adds the needed Time keyframes
...
SmartEdit writes the :guilabel:`Time` curve for you, and you can edit those keyframes just like any other.
See more: :ref:`clip_time_repeat_ref`.

Bézier Presets
--------------
When using a Bézier curve for animation, SmartEdit includes more than 20 curve presets (which affect the shape
...
that brings still images or video clips to life with slow, deliberate camera movement. In SmartEdit, use
...
drag and drop one of them into SmartEdit, and you will be prompted to import the entire sequence. SmartEdit will playback
...
**NOTE:** Be sure your image sequence starts at 0 or 1, or you will likely receive an error when importing it into SmartEdit.
For example, if your sequence starts at cat222.png, or has missing images in the sequence, SmartEdit will
