...
The Preferences window contains many important settings and configuration options for SmartEdit. They can be
found in the top menu under :guilabel:`Edit→Preferences`. Many settings will require SmartEdit to be restarted after your
changes are applied.
...
The General tab of the Preferences window allows you to modify the settings that apply to SmartEdit as a whole.
...
.. list-table::
   ...
   * - Language
     - Default
     - Choose your preferred language for SmartEdit menus and windows
   * - Default Theme
     - Cosmic Dusk
     - Choose your theme for SmartEdit
...
Themes
""""""
SmartEdit comes with 3 standard themes, which change the look and feel of the program.
...
Restoring Defaults
""""""""""""""""""
In SmartEdit, each preferences category (or tab) in the Preferences window has a **Restore Defaults** button that allows
...
.. table::
   ...
   Concurrent Optimize Jobs          2                   Number of video files SmartEdit can optimize at the same time
...
Autosave is a feature in SmartEdit which automatically saves the current changes to your project after
...
reduce the risk of data loss. The recovery folder is located at ``~/.smartedit_qt/recovery/`` or
``C:\Users\USERNAME\.smartedit_qt\recovery``.
...
.. table::
   ...
   Cache Max Frames:                 This is an override on the total allowed frames that can be cached by our caching thread. It defaults to 900 frames, but even if you give a huge amount of RAM to SmartEdit's cache size, this will override the max # of frames cached. The reason is... sometimes when the preview window is very small, and the cache size is set very high, SmartEdit might calculate that we can cache 30,000 frames, or something silly which will take a huge amount of CPU, lagging the system. This setting is designed to clamp the upper limit of the cache to something reasonable... even on systems that give SmartEdit huge amounts of RAM to work with.
   ================================  ==================

SmartEdit's default cache settings now target a balanced preview experience:
...
Please keep in mind that GPU hardware acceleration is experimental at the moment. SmartEdit supports both decoding and
encoding acceleration. For more information take a look at our `Github HW-ACCEL Doc <https://github.com/SmartEdit/libsmartedit/blob/develop/doc/HW-ACCEL.md>`_.
...
If SmartEdit cannot reach this server, AI context menu options are hidden.
...
To reset **all** preferences to their default values, please delete the ``smartedit.settings`` file and
re-launch SmartEdit. The settings file can be located at this path: ``~/.smartedit_qt/smartedit.settings`` or
``C:\Users\USERNAME\.smartedit_qt\smartedit.settings``. When SmartEdit is re-launched, it will create the
missing ``smartedit.settings`` file with default values.

Optionally, you can delete the entire ``.smartedit_qt/`` folder and re-launch SmartEdit. However, please make a
...
Deleting the ``.smartedit_qt/`` folder is the quickest method to restore SmartEdit preferences and settings
to their Default values (i.e. also called a `clean install`). When SmartEdit is re-launched, it will create
any missing folders (i.e. ``.smartedit_qt/``) and settings files. See our
`step-by-step guide <https://github.com/SmartEdit/smartedit-qt/wiki/Clean-Installation-of-SmartEdit>`_ for more
information about **clean installs** of SmartEdit.
