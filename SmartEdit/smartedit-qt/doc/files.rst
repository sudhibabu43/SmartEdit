...
into SmartEdit. Most media file types are recognized, such as videos, images, and audio files.
...
Import Files
------------
There are many different ways to import media files into an SmartEdit project. When a file is imported successfully,
it will be automatically selected and scrolled into view (in the **Project Files** panel). Also, if the **Project Files** panel
is not currently visible, SmartEdit will automatically display the panel.
...
--------------------
You can paste files and clipboard media straight into SmartEdit. If you copied one or more files in your
file manager, switch to SmartEdit and press :kbd:`Ctrl-V` in the **Project Files** panel to add them, just like importing.
...
press :kbd:`Ctrl-V` in SmartEdit to create a file for that clipboard content. SmartEdit saves a copy in a temporary folder:
``.smartedit_qt/clipboard/``. The new file is then added to your project and appears in **Project Files**.
...
.. list-table::
   ...
   * - Action
     - Description
   * - Optimize Video
     - Create an optimized preview file for each selected video file. SmartEdit stores these files in the project's ``optimized`` assets folder.
   * - Link to Existing...
...
When you use :guilabel:`Link to Existing...`, SmartEdit tries to match each selected source clip to an existing optimized
...
or ``_1080p``, and camera/editor-style variants such as ``_p``. It also supports names that add the SmartEdit file ID
...
collisions, append the SmartEdit file ID after the proxy suffix, for example ``clip001_proxy_F1.mp4``. Avoid unrelated
suffixes such as ``_final`` or ``_reviewcopy``, because SmartEdit does not treat those as safe proxy naming patterns.
...
Export Files
------------
If you want your split file segments available outside of your SmartEdit project, or want to copy all your video assets to one place,
...
---------------------
SmartEdit creates and uses a few **temporary working folders** while you edit. These live under
``.smartedit_qt/`` in your user profile and hold project-specific files that SmartEdit generates for you:

* ``.smartedit_qt/blender/`` - Blender animation renders created by the Animated Title dialog
* ``.smartedit_qt/title/`` - SVG title files created by the Title dialog
* ``.smartedit_qt/thumbnail/`` - Thumbnails generated for Project Files and Timeline
* ``.smartedit_qt/clipboard/`` - Media created from clipboard pastes (images, audio, or video that must be saved to disk first)
* ``.smartedit_qt/optimized/`` - Optimized preview files created for smoother editing
* ``.smartedit_qt/protobuf_data`` - Tracking and object detection data

When you choose **File→Save As**, SmartEdit copies these folders into your project directory, inside a single folder named
...
When you create and save a project in SmartEdit, any files imported into the software (such as videos, audio, and images) 
...
cause a :guilabel:`Missing File` message, in which SmartEdit prompts you to locate the folder of the missing file(s). Depending on the 
number of files you added to your project, SmartEdit could prompt you many times to find the missing files.

When SmartEdit saves a project, all file paths are converted to **relative** file paths. As long as you keep all assets organized in
...
see `The Case of “Missing Files”! <https://github.com/SmartEdit/smartedit-qt/wiki/Missing-File-Prompt>`_
