...
In SmartEdit, when you add project files (videos, images, and audio) to the timeline, they appear as **clips**
...
Trimming & Slicing
------------------
SmartEdit provides multiple ways to adjust the start and end trim positions of a clip (also known as trimming). The
...
To **slice** a clip into smaller sections, SmartEdit offers several options, including dividing or slicing a clip
...
Here is a list of all methods for cutting and/or trimming clips in SmartEdit:
...
Selections
----------
Selecting clips and transitions on the timeline is an essential part of editing in SmartEdit. Multiple selection methods
are available to streamline your workflow, enabling efficient editing of clips and transitions.

Here is a list of all methods for selecting clips in SmartEdit:
...
Context Menu
------------
SmartEdit has tons of great preset animations and clip properties, such as fading, sliding, zooming, 
...
- **Speed ramp** — use the :guilabel:`Timing` tool to drag clip edges and create a natural-feeling speed change; SmartEdit scales all keyframes automatically.
...
time curve by hand. SmartEdit writes the needed :guilabel:`Time` keyframes for you (you can edit them later).
...
- When you use Repeat, SmartEdit will also **repeat other keyframes** found inside the trimmed section (location, scale, effects, etc.) into each pass so your animations stay in sync.
...
SmartEdit tabifies :guilabel:`Audio Levels` with them.
...
origin values one property at a time. SmartEdit shows the Transform Tool
...
approximate levels of different clips. New waveform data preserves the source's **absolute peak level**. SmartEdit does
not enlarge every quiet file until its highest peak fills the clip, so a quiet recording remains visibly quieter than
a loud recording.

Pure linear amplitude is accurate but makes ordinary speech difficult to see. SmartEdit therefore applies a fixed,
...
SmartEdit samples both **peak** and **RMS** envelopes at regular time intervals. The lighter outer envelope shows brief
...
Projects saved by older SmartEdit versions can contain independently peak-normalized waveform data. These waveforms
have no format marker, so SmartEdit deliberately displays them with the original legacy scaling. They remain familiar
...
ensuring its uniqueness. This is a Read-only property, and assigned by SmartEdit when a clip is created.
...
To avoid distortion, SmartEdit might need to reduce the volume levels in overlapping clips. The following audio mixing methods are available:
...
respectively. The range for these curves is 0 to 1. See :ref:`clip_transform_ref`. SmartEdit limits the max scale values based
...
angles in degrees, respectively. See :ref:`clip_transform_ref`. SmartEdit limits the max shear values based
on the file type and the project size, to prevent crashes and performance issues.
...
The same adjustments can be made interactively with the :guilabel:`Timing` toolbar button by dragging a clip's edges; SmartEdit
...
and the end of your clip sets a time value of ``1`` (`frame 1`), SmartEdit will play this clip backwards, starting
