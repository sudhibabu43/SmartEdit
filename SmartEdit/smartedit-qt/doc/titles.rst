...
Adding text overlays, lower thirds, and titles is an important aspect of video editing, and SmartEdit comes with an easy-to-use Title Editor. Use
the Title menu (located in the main menu of SmartEdit) to launch the Title Editor. You can also use the keyboard shortcut
:kbd:`Ctrl+T`.

Titles are simply vector image files with transparent backgrounds (``*.svg``). SmartEdit comes with many easy-to-use
templates, but you can also create your own or import new templates into SmartEdit. These templates allow you to quickly
...
SmartEdit includes a variety of vector title templates that can be used to enhance your video projects.
...
.. list-table::
   ...
   * - SmartEdit
     - A title featuring SmartEdit branding.
   * - Oval 1
...
^^^^^^^^^^^^^^^^^^^^^^
SmartEdit can use any vector SVG image file as a custom title template in the :guilabel:`Title Editor` dialog. Just add an SVG image file to your
``~/.smartedit_qt/title_templates/`` folder, and it will appear the next time you launch the :guilabel:`Title Editor` dialog. You can
...
in the main menu of SmartEdit) to launch the Animated Title editor. You can also use the keyboard shortcut **Ctrl+B**.
Note: Blender must be installed and configured before this feature will work in SmartEdit. See :ref:`blender_install_ref`.
...
SmartEdit includes a variety of 3D animated templates that can be used to add dynamic and engaging elements to your
...
can import text into SmartEdit, you must first export the text from these programs into a compatible image format that
...
it is **not** always 100% compatible with SmartEdit. Thus, we recommend using ``PNG`` format, which is a great web-based
image format that can include a transparent background and alpha channel. A transparent background and alpha channel
are needed for SmartEdit to allow the text to not cover up videos and images on the timeline below them.

For information on importing animated sequences into SmartEdit, please see :ref:`animation_image_seq_ref`.

Installing Inkscape
-------------------

The :guilabel:`Advanced Editor` feature in the :guilabel:`Title Editor` dialog requires the latest version of
Inkscape (https://inkscape.org/release/) be installed and the SmartEdit **Preferences** updated with the 
...
The :guilabel:`Animated Title` feature in SmartEdit requires the latest version of
Blender (https://www.blender.org/download/) be installed and the SmartEdit **Preferences** updated 
...
not compatible with SmartEdit Video Editor.

For a detailed guide on how to install these dependencies, see 
`Blender & Inkscape Guide <https://github.com/SmartEdit/smartedit-qt/wiki/Blender-and-Inkscape-Guide>`_.
