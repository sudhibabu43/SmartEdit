...
#   Name                Description
==  ==================  ============
1   Title Bar           The title bar of SmartEdit displays the current profile
...
To create a custom profile, right-click on any profile in SmartEdit and choose :guilabel:`Duplicate` to open up the Profile Editor.
...
The custom profiles are saved in the ``~/.smartedit_qt/profiles/`` or ``C:\Users\USERNAME\.smartedit_qt\profiles`` folder.
...
.. table::
   ...
   2   **Description**      A text description for your custom profile, which is displayed in SmartEdit.
...
Converting Profiles
-------------------
When switching profiles (or exporting to a different profile), SmartEdit will do it's best to convert all clip,
...
timeline, SmartEdit will match the `position` and `start` trim as closely as possible, and any tiny gaps (1-3 frames big)
...
Custom Profile
--------------
Although SmartEdit has more than 400 profiles (:ref:`profile_list_ref`) included by default, you can also create
your own custom profiles. Create a new text file in the ``~/.smartedit_qt/profiles/`` or
``C:\Users\USERNAME\.smartedit_qt\profiles`` folder.
...
.. table::
   ...
   ======================  ============
   description             The friendly name of the profile (this is what SmartEdit displays in the user interface)
...
Once you restart SmartEdit, you will see your custom profile appear in the list of Profiles.
...
SmartEdit includes many **export presets**, which combine our list of common profiles and their associated video export settings (``video codec``,
``audio codec``, ``audio channels``, ``audio sample rate``, etc...), which target specific output formats, websites, and devices. The **default
export preset** used by SmartEdit is ``MP4 (h.264 + AAC)``, see :ref:`profiles_mp4_h264_ref`.
...
This is the default export preset used by SmartEdit. This format is compatible
...
SmartEdit includes over 400 built-in video profiles. These profiles match the most common
