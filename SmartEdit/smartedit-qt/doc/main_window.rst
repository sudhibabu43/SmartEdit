...
SmartEdit Video Editor has one main window which contains most of the information, buttons,
...
For step-by-step instructions on the basic usage of SmartEdit, be sure to read the
:ref:`quick_tutorial_ref`.

Built-in Tutorial
-----------------
When you first launch SmartEdit, you will be presented with a friendly built-in tutorial. It will demonstrate and explain
...
SmartEdit uses tracks to layer videos and images. The top most track is the top
...
concept. SmartEdit will stack the layers and mix each one together, just like a
...
Keyboard Shortcuts
------------------
Here is a list of the default keyboard shortcuts supported by SmartEdit. You can
configure these shortcuts in the Preferences window, which is opened by selecting
:guilabel:`Edit→Preferences` from the SmartEdit menu bar.
(On macOS, choose :guilabel:`SmartEdit Video Editor→Preferences`.)
...
Menu
----
The following menu options are available on the main window on SmartEdit. Most of these options can be accessed
...
About SmartEdit
--------------
Use :guilabel:`Help→About SmartEdit` to check your SmartEdit version. Click
...
.. code-block:: text
   ...
   **SmartEdit Version Info**
   Version: 3.5.1-dev | libsmartedit: 0.7.0
   OS: Ubuntu 24.04
...
The SmartEdit main window is composed of multiple **docks**. These **docks** are arranged and snapped together
into a grouping that we call a **View**. SmartEdit includes :guilabel:`Simple View`, :guilabel:`Color View`,
...
Recording View
^^^^^^^^^^^^^^
This view is focused on creating new recordings inside SmartEdit. It shows :guilabel:`Project Files`,
...
sources. When screen and webcam are recorded together, SmartEdit can place the webcam as a corner picture-in-picture
...
Docks
^^^^^
Each widget on the SmartEdit main window is contained in a **dock**. These docks can be dragged and snapped around the
main window, and even grouped together (into tabs). SmartEdit will always save your main window dock layout when you
exit the program. Re-launching SmartEdit will restore your custom dock layout automatically. Scope docks are grouped
