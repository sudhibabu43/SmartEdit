...
Video editing projects (including tracks, clips, and keyframes) can be **imported** and **exported** from SmartEdit
...
your edits to SmartEdit (or vice versa).

EDL (Edit Decision Lists)
-------------------------
The following features are supported when importing and exporting an EDL file with SmartEdit.
...
.. table::
   ...
   Tape Name             Only **AX** and **BL** tape names are currently supported in SmartEdit
...
SmartEdit follows the CMX 3600 layout for event lines and uses comment lines (`* ...`) to carry keyframes.
...
.. code-block:: python
   ...
   :caption: Example EDL format supported by SmartEdit:
...
--------------------------
The following features are supported when importing and exporting an XML file with SmartEdit. This XML format
...
SmartEdit uses the legacy Final Cut Pro XML Interchange Format (**xmeml**) from Final Cut Pro 7. Our exporter writes
