...
SmartEdit can connect to a local `ComfyUI <https://github.com/comfyanonymous/ComfyUI>`_ server and run AI workflows from the
...
.. warning::
   AI features in SmartEdit are **experimental** and require a **high-end workstation**.
...
Use this quick setup path before trying any AI workflow in SmartEdit:
...
5. Click :guilabel:`Check` to confirm SmartEdit can reach the server.

For a full step-by-step server setup guide, see the SmartEdit wiki:
`ComfyUI: Advanced AI Setup Guide <https://github.com/SmartEdit/smartedit-qt/wiki/ComfyUI:-Advanced-AI-Setup-Guide>`_.
...
- `SmartEdit-ComfyUI <https://github.com/SmartEdit/SmartEdit-ComfyUI>`_
...
What Users See in SmartEdit
--------------------------

When ComfyUI is available, SmartEdit shows AI tools in context menus:
...
queue badges. Outputs are saved under ``.smartedit_qt/comfyui-output/``.

Starting a **new project** or opening an **existing project** clears the
temporary ``.smartedit_qt`` AI working folders so you begin with a clean slate.
...
If ComfyUI is unavailable, SmartEdit disables the AI menus. Configure the server URL
...
SmartEdit reads built-in templates from ``comfyui/``. It also loads custom
user templates from ``~/.smartedit_qt/comfyui/``.
...
3. Copy that JSON file into ``~/.smartedit_qt/comfyui/``.
4. Restart SmartEdit, or reopen the project if needed.

SmartEdit will automatically load the workflow and show it in the appropriate AI
menu. When you trigger it from SmartEdit, the selected source file is passed into
the workflow, and the output from the workflow's final output node is imported
back into :guilabel:`Project Files`.

SmartEdit also writes a ``.smartedit_qt/comfyui/debug.json`` payload for advanced
...
SmartEdit sends your positive and negative markers as seed coordinates to the
...
- Outputs are written under ``.smartedit_qt/comfyui-output/``.
...
- Details: Uses ``comfyui/audio-noise-reduce.json`` with ``SmartEditDeepFilterNetDenoiseAudio``.
...
- Details: Uses ``comfyui/audio-noise-remove.json`` with ``SmartEditDeepFilterNetDenoiseAudio``.
...
- Details: Uses ``comfyui/audio-clarity-speech.json`` with ``SmartEditLavaSRSpeechClarity``.
