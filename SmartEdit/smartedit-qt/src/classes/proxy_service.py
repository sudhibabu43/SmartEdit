...
class ProxyService(QObject):
     ...
     def apply_runtime_updates_for_file(self, file_id):
         """Push runtime-only clip/effect updates to libsmartedit for one file's proxy state."""
     ...
     def _reader_json_for_path(self, file_path, file_id):
         clip = smartedit.Clip(str(file_path))
         try:
             clip.Open()
     ...
     def _build_proxy_reader(self, file_id, file_data):
         ...
         try:
             ...
             try:
                 ...
                 writer = smartedit.FFmpegWriter(output_path)
                 writer.SetVideoOptions(
                     True,
                     "libx264",
                     smartedit.Fraction(int(fps.get("num", 30)), int(fps.get("den", 1))),
                     target_width,
                     target_height,
                     smartedit.Fraction(int(pixel_ratio.get("num", 1)), int(pixel_ratio.get("den", 1))),
                     False,
                     False,
                     28,
                 )
                 ...
                 if source_reader.get("has_audio"):
                     channel_layout = smartedit.LAYOUT_STEREO if int(source_reader.get("channels", 2) or 2) > 1 else smartedit.LAYOUT_MONO
                     writer.SetAudioOptions(
                         True,
                         "aac",
                         48000,
                         2 if channel_layout == smartedit.LAYOUT_STEREO else 1,
                         channel_layout,
                         128000,
                     )
                     writer.PrepareStreams()
     ...
     def _create_optimize_timeline(self, width, height):
         ...
         timeline = smartedit.Timeline(
             width,
             height,
             smartedit.Fraction(30, 1),
             48000,
             2,
             smartedit.LAYOUT_STEREO,
         )
