...
INTERPOLATION_EXPORT_MAP = {
    smartedit.LINEAR: "linear",
    smartedit.BEZIER: "bezier",
    smartedit.CONSTANT: "constant"
}
...
def _normalized_to_center_pixels(x_norm, y_norm, frame_width, frame_height):
    """Map normalized SmartEdit coords (-1..1, origin center) to pixel center values."""
    try:
        w = float(frame_width)
...
def _gravity_offset(gravity, frame_w, frame_h, scaled_w, scaled_h):
    ...
    x = 0.0
    y = 0.0
    if gravity == smartedit.GRAVITY_TOP:
...
def _merge_uniform_scale(scale_x_points, scale_y_points):
    ...
    def _add(points):
        for point in points:
            ...
            interp = point.get("interpolation", smartedit.LINEAR)
            if t in merged:
        ...
        merged_points.append(
            {
                ...
                "interpolation": merged[t].get("interp", smartedit.LINEAR),
            }
        )
...
def createEffect(xmldoc, name, node, points, scale, max_frames=None, param_name=None):
    ...
    for point in points:
        ...
        interpolation = point.get('interpolation', smartedit.LINEAR)
