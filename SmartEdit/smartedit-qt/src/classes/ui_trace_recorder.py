...
class UiTraceRecorder(QObject):
    ...
    def __init__(self, window):
        ...
        self._include_load = _env_true("SMARTEDIT_UI_TRACE_INCLUDE_LOAD", default=False)
        self._include_ignored = _env_true("SMARTEDIT_UI_TRACE_INCLUDE_IGNORED", default=False)
        self._trace_actions = _env_true("SMARTEDIT_UI_TRACE_ACTIONS", default=True)
        self._trace_dialogs = _env_true("SMARTEDIT_UI_TRACE_DIALOGS", default=True)
        self._trace_docks = _env_true("SMARTEDIT_UI_TRACE_DOCKS", default=True)
        self._trace_cache = _env_true("SMARTEDIT_UI_TRACE_CACHE", default=False)
        self._cache_interval_ms = max(100, int(_env_float("SMARTEDIT_UI_TRACE_CACHE_INTERVAL", 0.5) * 1000))
        self.enabled = False
    ...
    def _resolve_paths(self):
        updates_path = os.getenv("SMARTEDIT_UI_TRACE_UPDATES", "").strip()
        selections_path = os.getenv("SMARTEDIT_UI_TRACE_SELECTIONS", "").strip()
        events_path = os.getenv("SMARTEDIT_UI_TRACE_EVENTS", "").strip()

        trace_dir = os.getenv("SMARTEDIT_UI_TRACE_DIR", "").strip()
        trace_enabled = _env_true("SMARTEDIT_UI_TRACE", default=False)
