...
def center(window):
    ...
    try:
        app = get_app()
        if app and getattr(app, "window", None) and app.window != window:
            frameGm = window.frameGeometry()
            centerPoint = app.window.frameGeometry().center()
            frameGm.moveCenter(centerPoint)
            window.move(frameGm.topLeft())
        elif hasattr(window, "screen") and window.screen():
            frameGm = window.frameGeometry()
            centerPoint = window.screen().geometry().center()
            frameGm.moveCenter(centerPoint)
            window.move(frameGm.topLeft())
    except Exception:
        pass
