...
class GenerationService(QObject):
    ...
    def _prepare_generation_source_path(self, source_file, template_id):
        ...
        tmp_fd, tmp_png = tempfile.mkstemp(prefix="smartedit-comfy-", suffix=".png")
        os.close(tmp_fd)
        try:
    ...
    def _apply_dynamic_sam2_meta_batch(self, workflow, source_file, template_id=None):
        ...
        for node in workflow.values():
            ...
            if class_type == "smarteditsam2videosegmentationchunked":
        ...
        for node in workflow.values():
            if not isinstance(node, dict):
                continue
            class_type = str(node.get("class_type", "")).strip().lower()
            if class_type == "smarteditimagehighlightmasked":
                estimated_bpp = max(estimated_bpp, float(self.SAM2_ESTIMATED_BYTES_PER_PIXEL_HIGHLIGHT))
            elif class_type == "smarteditimageblurmasked":
        ...
        for node in workflow.values():
            ...
            if not isinstance(inputs, dict):
                continue
            if class_type == "smarteditsam2videosegmentationchunked" and "chunk_size_frames" in inputs:
    ...
    def _prepare_template_workflow(
        ...
        def _is_placeholder_value(path_text):
            ...
            return path_text in ("__smartedit_input__", "{{smartedit_input}}", "$smartedit_input")

        def _is_reference_image_placeholder_value(path_text):
            path_text = str(path_text or "").strip().lower()
            return path_text in (
                "__smartedit_reference_image__",
                "{{smartedit_reference_image}}",
                "$smartedit_reference_image",
            )

        def _is_prompt_placeholder_value(text_value):
            text_value = str(text_value or "").strip().lower()
            return text_value in ("__smartedit_prompt__", "{{smartedit_prompt}}", "$smartedit_prompt")

        def _is_lyrics_placeholder_value(text_value):
            text_value = str(text_value or "").strip().lower()
            return text_value in ("__smartedit_lyrics__", "{{smartedit_lyrics}}", "$smartedit_lyrics")
        ...
        def _replace_prompt_placeholders(text_value):
            ...
            return (
                text_value
                .replace("__smartedit_prompt__", prompt_text)
                .replace("{{smartedit_prompt}}", prompt_text)
                .replace("$smartedit_prompt", prompt_text)
            )
        ...
        def _replace_lyrics_placeholders(text_value):
            ...
            return (
                text_value
                .replace("__smartedit_lyrics__", music_lyrics_text)
                .replace("{{smartedit_lyrics}}", music_lyrics_text)
                .replace("$smartedit_lyrics", music_lyrics_text)
            )
        ...
        for node_id, node in workflow.items():
            ...
            if class_flat in ("smarteditimagehighlightmasked",) or class_type.strip() == "SmartEditImageHighlightMasked":
    ...
    def _enqueue_generation_for_file(self, source_file, payload):
        ...
        try:
            ...
            return False, "SmartEdit could not convert this image into PNG for ComfyUI.\n\n{}".format(ex)
    ...
    def action_generate_trigger(self, checked=True, source_file=None, template_id=None, open_dialog=True):
        ...
        if not self.is_comfy_available(force=True):
            ...
            msg.setText(
                "SmartEdit could not connect to ComfyUI at:\n{}\n\n"
        ...
        for target_file in selected_files or [None]:
            ...
            if not ok:
                ...
                elif error_text.startswith("SmartEdit could not convert this image into PNG for ComfyUI."):
