...
class ObjectMaskPreviewWorker(QObject):
    ...
    def run(self):
        try:
            mask_frame = smartedit.ClipProcessingJobs.PreviewObjectMask(self.config_json, self.frame)
            if not mask_frame:
...
class SelectRegion(QDialog):
    ...
    def __init__(self, file=None, clip=None, selection_mode="rect", parent=None, object_mask_preview_context=None):
        ...
        if clip:
            ...
            self.clip = smartedit.Clip(source_path)
    ...
    def _build_annotation_toolbar(self):
        ...
        if not self._mask_preview_supported():
            ...
            self.chkMaskPreview.setToolTip(_("Preview requires an updated libsmartedit Python module."))
    ...
    def _mask_preview_supported(self):
        try:
            return hasattr(smartedit.ClipProcessingJobs, "PreviewObjectMask")
        except Exception:
            return False
