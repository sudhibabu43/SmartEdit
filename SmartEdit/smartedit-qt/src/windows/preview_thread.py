...
class PlayerWorker(QObject):
    ...
    def Init(self, parent, timeline, videoPreview):
        ...
        if hasattr(self, "frame_ready") and getattr(self, "videoPreview", None):
            self.frame_ready.connect(self.videoPreview.present)
        self.clip_path = None
    ...
    def CheckAudioDevice(self):
        ...
        if detected_sample_rate and not math.isnan(detected_sample_rate) and detected_sample_rate > 0.0:
            ...
            if detected_sample_rate_int != settings_sample_rate:
                log.warning("Your sample rate (%d) does not match SmartEdit (%d). "
        ...
        if s.get("playback-audio-device") != audio_device_value:
            log.warning("Your active audio device (%s) does not match SmartEdit (%s). "
    ...
    def initPlayer(self):
        ...
        try:
            if hasattr(self.player, "SetFrameCallback"):
                import ctypes
                FRAME_CB_TYPE = ctypes.CFUNCTYPE(
                    None, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p
                )

                def _on_frame(width, height, bytes_per_line, pixels_ptr, user_data):
                    if not pixels_ptr or width <= 0 or height <= 0:
                        return
                    try:
                        raw_bytes = ctypes.string_at(pixels_ptr, bytes_per_line * height)
                        img = QImage(raw_bytes, width, height, bytes_per_line, QImage.Format.Format_RGBA8888_Premultiplied).copy()
                        self.frame_ready.emit(img)
                    except Exception:
                        pass

                self._c_frame_callback = FRAME_CB_TYPE(_on_frame)
                cb_addr = ctypes.cast(self._c_frame_callback, ctypes.c_void_p).value
                ...
                log.info("Qt binding is %s (Qt5 libsmartedit); skipping direct C++ pointer wrapping", QT_API)
        except Exception as ex:
            log.warning("initPlayer could not bind preview widget: %s", ex)
    ...
    def run_scope_analysis(self, frame_number, need_waveform, need_histogram, need_vectorscope, need_audio, scope_region, waveform_render, vectorscope_render):
        ...
        Channel key contract: libsmartedit frames are stored as
        ...
        try:
            ...
            if need_vectorscope:
                is_playing = False
                try:
                    is_playing = self.player.Mode() == smartedit.PLAYBACK_PLAY
                except Exception:
                    is_playing = self.current_mode == smartedit.PLAYBACK_PLAY
                if is_playing:
    ...
    def LoadFilePreview(self, path=None, stretch=False):
        ...
        if not path:
            ...
            try:
                ...
                if stretch:
                    new_clip.scale = smartedit.SCALE_STRETCH
