...
def _keyframe_value(frame_number=1.0, value=0.0, interpolation=smartedit.CONSTANT):
    return {
        "Points": [{
        ...
        for point in data["Points"]:
            try:
                normalized_point = {
                    ...
                    "interpolation": int(point.get("interpolation", smartedit.LINEAR)),
                }
                ...
                if "handle_type" in point:
                    normalized_point["handle_type"] = int(point.get("handle_type", smartedit.AUTO))
...
def _evaluate_keyframe(data, frame_number, default_value=0.0):
    ...
    for index in range(1, len(points)):
        ...
        if abs(frame_value - right_x) < 0.00001:
            return right_y
        interpolation = int(right.get("interpolation", smartedit.BEZIER))
        if interpolation == smartedit.CONSTANT or abs(right_x - left_x) < 0.00001:
            return left_y
...
def _set_keyframe_value(data, frame_number, value, interpolation=smartedit.BEZIER):
...
def _set_color_value(data, frame_number, color, interpolation=smartedit.BEZIER):
...
def _default_curve_node(node_id, x_value, y_value, frame_number=1):
    return {
        "id": int(node_id),
        "x": _keyframe_value(frame_number=frame_number, value=x_value, interpolation=smartedit.LINEAR),
        "y": _keyframe_value(frame_number=frame_number, value=y_value, interpolation=smartedit.LINEAR),
        "left_handle_x": _keyframe_value(frame_number=frame_number, value=0.5, interpolation=smartedit.LINEAR),
        "left_handle_y": _keyframe_value(frame_number=frame_number, value=1.0, interpolation=smartedit.LINEAR),
        "right_handle_x": _keyframe_value(frame_number=frame_number, value=0.5, interpolation=smartedit.LINEAR),
        "right_handle_y": _keyframe_value(frame_number=frame_number, value=0.0, interpolation=smartedit.LINEAR),
...
def normalize_curve_data(data):
    ...
    for index, node in enumerate(data.get("nodes") or []):
        ...
        nodes.append({
            ...
            "interpolation": int(node.get("interpolation", smartedit.LINEAR)),
            "handle_type": int(node.get("handle_type", smartedit.AUTO)),
        })
    if len(nodes) < 2:
...
def _draw_keyframe_marker(painter, center, interpolation, size, fill_color, border_color):
    ...
    if interpolation == smartedit.LINEAR:
        painter.drawRect(rect)
    elif interpolation == smartedit.CONSTANT:
        path = QPainterPath()
...
class CurvePreviewWidget(QWidget):
    ...
    def __init__(self, curve_data=None, frame_number=1, parent=None):
        ...
        self._curve_icons = {
            smartedit.BEZIER: _keyframe_marker_icon(smartedit.BEZIER),
            smartedit.LINEAR: _keyframe_marker_icon(smartedit.LINEAR),
            smartedit.CONSTANT: _keyframe_marker_icon(smartedit.CONSTANT),
        }
    ...
    def _curve_icon(self, interpolation):
        return self._curve_icons.get(interpolation) or self._curve_icons[smartedit.BEZIER]
    ...
    def _handle_point(self, node_id, side):
        ...
        if side == "left" and right["interpolation"] != smartedit.BEZIER:
            return None
        if side == "right" and right["interpolation"] != smartedit.BEZIER:
            return None
    ...
    def _apply_selected_bezier_preset(self, preset):
        ...
        self.dragStarted.emit()
        node["interpolation"] = int(smartedit.BEZIER)
    ...
    def _show_node_menu(self, global_pos):
        ...
        populate_keyframe_context_menu(
            ...
            linear_callback=lambda: self._set_selected_interpolation(smartedit.LINEAR),
            constant_callback=lambda: self._set_selected_interpolation(smartedit.CONSTANT),
    ...
    def paintEvent(self, event):
        ...
        for node in nodes:
            if node["interpolation"] == smartedit.BEZIER:
...
class PropertySlider(QWidget):
    ...
    def __init__(self, min_val=0.0, max_val=1.0, value=0.0, decimals=2, parent=None):
        ...
        self._interpolation = smartedit.LINEAR
        self._curve_pixmaps = {
    ...
    def set_keyframe_status(self, points=1, interpolation=smartedit.LINEAR):
    ...
    def paintEvent(self, event):
        ...
        if self._keyframe_points > 1:
            painter.drawPixmap(
                ...
                int(rect.y() + 5.0),
                self._curve_pixmaps.get(self._interpolation, self._curve_pixmaps[smartedit.LINEAR]))
...
class WheelRow(QWidget):
        ...
        def _update(kf_data):
            ...
            for point in points:
                if _matches(point, previous_frame):
                    changed = True
                    if int(point.get("interpolation", smartedit.LINEAR)) == smartedit.BEZIER and preset:
                ...
                if _matches(point, target_frame):
                    ...
                    if interpolation == smartedit.BEZIER and preset:
    ...
    def _show_slider_menu(self, key, pos, source_widget):
        ...
        populate_keyframe_context_menu(
            menu,
            bezier_callback=lambda preset: self._set_slider_interpolation(key, smartedit.BEZIER, preset),
            linear_callback=lambda: self._set_slider_interpolation(key, smartedit.LINEAR),
            constant_callback=lambda: self._set_slider_interpolation(key, smartedit.CONSTANT),
            bezier_icon=None,
            linear_icon=None,
            constant_icon=None,
        )
    ...
    def _show_wheel_menu(self, pos, source_widget=None):
        ...
        populate_keyframe_context_menu(
            menu,
            bezier_callback=lambda preset: self._set_wheel_interpolation(smartedit.BEZIER, preset),
            linear_callback=lambda: self._set_wheel_interpolation(smartedit.LINEAR),
            constant_callback=lambda: self._set_wheel_interpolation(smartedit.CONSTANT),
            bezier_icon=None,
            linear_icon=None,
            constant_icon=None,
        )
