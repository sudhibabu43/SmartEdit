...
class VideoWidget(QWidget, updates.UpdateInterface):
    ...
    def _build_clip_transform(self, x, y, sw, sh, props):
        """Return (QTransform, unpacked props, originScreenPt) for a clip/effect box.
           ...
           matching libsmartedit's Clip::get_transform so shear/scale share the same pivot and basis.
        """
        ...
        for point in points:
            ...
            if co.get("X") == frame_number:
                ...
                point.update({
                    ...
                    "interpolation": smartedit.BEZIER,
                })
    ...
    def _location_offset(location, anchored_position, canvas_size, clip_size):
        """Match libsmartedit normalized location semantics for one axis."""
    ...
    def _clip_location_geometry(self, base_width, base_height, clip, raw_properties, viewport_rect):
        ...
        anchored_x = 0.0
        anchored_y = 0.0
        if gravity == smartedit.GRAVITY_TOP_LEFT:
            anchored_x = layout_x
            anchored_y = layout_y
        elif gravity == smartedit.GRAVITY_TOP:
            anchored_x = layout_x + ((layout_width - scaled_width) / 2.0)
            anchored_y = layout_y
        elif gravity == smartedit.GRAVITY_TOP_RIGHT:
            anchored_x = layout_x + layout_width - scaled_width
            anchored_y = layout_y
        elif gravity == smartedit.GRAVITY_LEFT:
            anchored_x = layout_x
            anchored_y = layout_y + ((layout_height - scaled_height) / 2.0)
        elif gravity == smartedit.GRAVITY_CENTER:
            anchored_x = layout_x + ((layout_width - scaled_width) / 2.0)
            anchored_y = layout_y + ((layout_height - scaled_height) / 2.0)
        elif gravity == smartedit.GRAVITY_RIGHT:
            anchored_x = layout_x + layout_width - scaled_width
            anchored_y = layout_y + ((layout_height - scaled_height) / 2.0)
        elif gravity == smartedit.GRAVITY_BOTTOM_LEFT:
            anchored_x = layout_x
            anchored_y = layout_y + layout_height - scaled_height
        elif gravity == smartedit.GRAVITY_BOTTOM:
            anchored_x = layout_x + ((layout_width - scaled_width) / 2.0)
            anchored_y = layout_y + layout_height - scaled_height
        elif gravity == smartedit.GRAVITY_BOTTOM_RIGHT:
            anchored_x = layout_x + layout_width - scaled_width
            anchored_y = layout_y + layout_height - scaled_height
    ...
    def _clip_display_rect(self, base_width, base_height, clip, raw_properties, viewport_rect):
        ...
        if clip.data['scale'] == smartedit.SCALE_CROP:
