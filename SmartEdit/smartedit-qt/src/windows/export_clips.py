...
def nameOfExport(file_obj) -> str:
    if isClip(file_obj):
        backup_name = os.path.splitext(file_obj.data.get("path", "smartedit_clip"))[0]
...
def setupWriter(clip, writer):
    ...
    frames_per_second = smartedit.Fraction(fps.get("num",1), fps.get("den",1))
...
class clipExportWindow(QDialog):
    ...
    def _exportClip(self, clip, export_path: str, frames_written: int, total_frames: int):
        ...
        w = smartedit.FFmpegWriter(export_path)
        try:
            setupWriter(clip, w)
        except Exception as ex:
        ...
        try:
            ...
            clip_reader = smartedit.Clip(clip.data.get("path"))
            clip_reader.Open()
