...
Please check the preferences in SmartEdit and be sure the Blender executable is correct.
...
{}""").format(info.BLENDER_MIN_VERSION,
    ...
    def Render(self, frame=None):
        ...
        self.background.setObjectName("smartedit_renderer")
...
class Worker(QObject):
    ...
    def Render(self):
        ...
        if not os.path.exists(self.blend_file_path):
            log.warning("Blender template file not found: %s", self.blend_file_path)
            self.blender_error_with_data.emit(_("Blender template file (.blend) is not found or has been removed."))
            self.finished.emit()
            return
