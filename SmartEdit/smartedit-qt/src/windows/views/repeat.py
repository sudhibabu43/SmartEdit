...
def _normalize_points(points):
    ...
    for p in pts:
        ...
        y = p["co"].get("Y")
        out.append({"co": {"X": x, "Y": y}, "interpolation": p.get("interpolation", smartedit.LINEAR)})
    return out
    ...
    for k in range(passes):
        ...
        for idx, p in enumerate(pts_iter):
            ...
            if dir_local > 0:
                ...
                nx = base + (dur - nx_off)
            interp = p.get("interpolation", smartedit.LINEAR)
            if pattern == "loop" and k > 0 and idx == 0:
                interp = smartedit.CONSTANT
            new_points.append({"co": {"X": nx, "Y": y}, "interpolation": interp})
        base += dur
        if k < passes - 1 and delay_frames:
            last_y = new_points[-1]["co"].get("Y")
            new_points.append({"co": {"X": base, "Y": last_y}, "interpolation": smartedit.LINEAR})
            new_points.append({"co": {"X": base + delay_frames, "Y": last_y}, "interpolation": smartedit.LINEAR})
            base += delay_frames
        if pattern == "pingpong":
            dir_local *= -1
...
def apply_repeat(clip, pattern, start_dir, passes, delay_frames, ramp, fps_float):
    ...
    if isinstance(orig_time, list) and len(orig_time) >= 2:
        ...
        base_time = [
            {"co": {"X": 1, "Y": target_start_y}, "interpolation": smartedit.LINEAR},
            {"co": {"X": base_frames, "Y": target_end_y}, "interpolation": smartedit.LINEAR},
        ]
