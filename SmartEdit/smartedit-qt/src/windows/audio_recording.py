...
def frame_to_qimage(frame):
    """Copy a libsmartedit RGBA frame into a Qt image for preview signals."""
...
def screen_capture_backend():
    """Return the libsmartedit screen backend best suited for the current session."""
    auto_backend = getattr(smartedit, "SCREEN_CAPTURE_AUTO", None)
    default_backend = getattr(getattr(smartedit, "ScreenCaptureReader", None), "DefaultBackend", None)
    ...
    if session == "wayland" and hasattr(smartedit, "SCREEN_CAPTURE_WAYLAND"):
        return smartedit.SCREEN_CAPTURE_WAYLAND
    if sys.platform.startswith("win") and hasattr(smartedit, "SCREEN_CAPTURE_WINDOWS_GDI"):
        return smartedit.SCREEN_CAPTURE_WINDOWS_GDI
    if sys.platform == "darwin" and hasattr(smartedit, "SCREEN_CAPTURE_MAC_AVFOUNDATION"):
        return smartedit.SCREEN_CAPTURE_MAC_AVFOUNDATION
    if hasattr(smartedit, "SCREEN_CAPTURE_X11"):
        return smartedit.SCREEN_CAPTURE_X11
    return auto_backend


def screen_capture_backend_supported(backend=None):
    """Return whether libsmartedit exposes and supports the selected screen backend."""
    if not all(hasattr(smartedit, name) for name in ("ScreenCaptureReader", "ScreenCaptureSettings")):
        return False

    selected_backend = screen_capture_backend() if backend is None else backend
    if selected_backend == getattr(smartedit, "SCREEN_CAPTURE_AUTO", object()):
        return False
    is_supported = getattr(smartedit.ScreenCaptureReader, "IsBackendSupported", None)
    ...
    if sys.platform.startswith("win"):
        return selected_backend == getattr(smartedit, "SCREEN_CAPTURE_WINDOWS_GDI", object())
    if sys.platform == "darwin":
        return selected_backend == getattr(smartedit, "SCREEN_CAPTURE_MAC_AVFOUNDATION", object())
    return selected_backend == getattr(smartedit, "SCREEN_CAPTURE_X11", object())


def screen_capture_backend_is_wayland(backend=None):
    selected_backend = screen_capture_backend() if backend is None else backend
    return selected_backend == getattr(smartedit, "SCREEN_CAPTURE_WAYLAND", object())
...
def camera_capture_backend():
    default_backend = getattr(getattr(smartedit, "CameraCaptureReader", None), "DefaultBackend", None)
    if callable(default_backend):
        try:
            return default_backend()
        except Exception:
            log.debug("Unable to query default camera capture backend", exc_info=True)
    if sys.platform.startswith("win") and hasattr(smartedit, "CAMERA_CAPTURE_WINDOWS_DSHOW"):
        return smartedit.CAMERA_CAPTURE_WINDOWS_DSHOW
    if sys.platform == "darwin" and hasattr(smartedit, "CAMERA_CAPTURE_MAC_AVFOUNDATION"):
        return smartedit.CAMERA_CAPTURE_MAC_AVFOUNDATION
    if hasattr(smartedit, "CAMERA_CAPTURE_V4L2"):
        return smartedit.CAMERA_CAPTURE_V4L2
    return getattr(smartedit, "CAMERA_CAPTURE_AUTO", None)
...
def camera_capture_backend_is_windows(backend=None):
    ...
    return selected_backend == getattr(smartedit, "CAMERA_CAPTURE_WINDOWS_DSHOW", object())


def camera_capture_backend_is_mac(backend=None):
    selected_backend = camera_capture_backend() if backend is None else backend
    return selected_backend == getattr(smartedit, "CAMERA_CAPTURE_MAC_AVFOUNDATION", object())
...
class LiveVideoRecordingJob(QObject):
    """Small background writer for live libsmartedit video capture readers."""
    ...
    def start_sync(self):
        ...
        self._open()
        self._thread = threading.Thread(target=self._run, name="SmartEditLiveVideoRecording", daemon=True)
        self._thread.start()

    def start_async(self):
        if self._thread and self._thread.is_alive():
            return
        self._thread = threading.Thread(target=self._open_and_run, name="SmartEditLiveVideoRecording", daemon=True)
        self._thread.start()
    ...
    def _open(self):
        ...
        self._writer = smartedit.FFmpegWriter(self.path)
        ...
        if bool(getattr(reader_info, "has_audio", False)):
            ...
            channel_layout = (
                smartedit.LAYOUT_MONO if channels == 1 else smartedit.LAYOUT_STEREO
            )
...
class WebcamPreviewJob(QObject):
    ...
    def start(self):
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._run, name="SmartEditWebcamPreview", daemon=True)
        self._thread.start()
    ...
    def _run(self):
        try:
            ...
            self._reader = smartedit.CameraCaptureReader(settings)
            self._reader.Open()
            frame_number = 1
            last_emit = 0.0
...
class AudioRecordingDockContent(QWidget):
    ...
    def __init__(self, window, parent=None):
        ...
        self._hiding_smartedit_window = False
        self._hide_smartedit_user_set = False
        ...
        self.hide_smartedit_combo = QComboBox(self.screen_section)
        self.hide_smartedit_combo.addItem(_("Yes"), True)
        self.hide_smartedit_combo.addItem(_("No"), False)
        self.hide_smartedit_combo.setCurrentIndex(self.hide_smartedit_combo.findData(False))
        self.hide_smartedit_combo.setToolTip(_("Temporarily hide SmartEdit while selecting or recording a window or region."))
        ...
        self.screen_hide_label = QLabel(_("Hide SmartEdit:"), self.screen_section)
        ...
        self.screen_section.advanced_layout.addWidget(self.hide_smartedit_combo, 6, 1)
        ...
        self.hide_smartedit_combo.currentIndexChanged.connect(self._hide_smartedit_changed)
    ...
    def _backend_available(self):
        audio_available = all(hasattr(smartedit, name) for name in (
            "AudioRecorder",
            "AudioRecorderSettings",
        ))
        video_available = all(hasattr(smartedit, name) for name in (
            "ScreenCaptureReader",
    ...
    def _system_audio_available(self):
        if not hasattr(smartedit, "ScreenCaptureReader"):
            return False
        is_supported = getattr(smartedit.ScreenCaptureReader, "IsSystemAudioSupported", None)
    ...
    def _camera_backend_available(self):
        if not all(hasattr(smartedit, name) for name in ("CameraCaptureReader", "CameraCaptureSettings")):
            return False
        backend = camera_capture_backend()
        is_supported = getattr(smartedit.CameraCaptureReader, "IsBackendSupported", None)
        ...
        return (
            backend == getattr(smartedit, "CAMERA_CAPTURE_V4L2", object())
            and sys.platform.startswith("linux")
        ) or (
            backend == getattr(smartedit, "CAMERA_CAPTURE_WINDOWS_DSHOW", object())
            and sys.platform.startswith("win")
        ) or (
            backend == getattr(smartedit, "CAMERA_CAPTURE_MAC_AVFOUNDATION", object())
    ...
    def _sync_source_availability(self):
        _ = get_app()._tr
        audio_available = all(hasattr(smartedit, name) for name in ("AudioRecorder", "AudioRecorderSettings"))
        ...
        screen_tip = "" if screen_available else _("Screen recording is not available for this platform or libsmartedit build.")
        ...
        if not camera_backend_available:
            camera_tip = _("Webcam recording is not available for this platform or libsmartedit build.")
    ...
    def _sync_screen_backend_ui(self):
        ...
        self.hide_smartedit_combo.setVisible(not wayland)
        ...
        if not system_audio_available:
            ...
            unavailable_tip = _("System audio recording is not available for this platform or libsmartedit build.")
    ...
    def _select_full_screen(self):
        ...
        self._screen_window_id = ""
        self._set_hide_smartedit_default(False)
    ...
    def _select_window(self):
        ...
        QApplication.processEvents()
        self._set_hide_smartedit_default(False)
        hidden_state = self._hide_smartedit_for_picker()
        try:
            result = pick_screen_window()
        finally:
            self._restore_smartedit_window(hidden_state)
        if result:
    ...
    def _hide_smartedit_changed(self):
        self._hide_smartedit_user_set = True

    def _set_hide_smartedit_default(self, checked):
        if self._hide_smartedit_user_set:
            return
        self.hide_smartedit_combo.blockSignals(True)
        index = self.hide_smartedit_combo.findData(bool(checked))
        if index >= 0:
            self.hide_smartedit_combo.setCurrentIndex(index)
        self.hide_smartedit_combo.blockSignals(False)

    def _hide_smartedit_enabled(self):
        if screen_capture_backend_is_wayland():
            return False
        return bool(self.hide_smartedit_combo.currentData())

    def _hide_smartedit_for_picker(self):
        if not self._hide_smartedit_enabled():
            return None
        return self._hide_smartedit_window(delay_ms=0)

    def _hide_smartedit_for_recording(self):
        if (
            self._recording_hidden_window_state is not None
            or not self.screen_card.isChecked()
            or not self._hide_smartedit_enabled()
        ):
            return
        self._recording_hidden_window_state = self._hide_smartedit_window(delay_ms=50)

    def _restore_smartedit_after_recording(self):
        state = self._recording_hidden_window_state
        self._recording_hidden_window_state = None
        self._restore_smartedit_window(state)

    def _hide_smartedit_window(self, delay_ms=75):
        ...
        self._hiding_smartedit_window = True
        try:
            self.window.hide()
            QApplication.processEvents()
            if delay_ms:
            ...
            log.debug("Unable to hide SmartEdit window", exc_info=True)
        finally:
            self._hiding_smartedit_window = False
        return None

    def _restore_smartedit_window(self, state):
        ...
        try:
            ...
            log.debug("Unable to restore SmartEdit window", exc_info=True)
    ...
    def refresh_devices(self):
        ...
        try:
            audio_devices = smartedit.AudioDevices()
    ...
    def _supported_audio_channels(self):
        ...
        for channels in (1, 2):
            try:
                ...
                settings.channel_layout = smartedit.LAYOUT_MONO if channels == 1 else smartedit.LAYOUT_STEREO
                settings.path = os.path.join(info.USER_PATH, "recording-channel-probe.wav")
                recorder = smartedit.AudioRecorder(settings)
                recorder.Open()
                recorder.Close()
    ...
    def refresh_cameras(self):
        ...
        if camera_capture_backend_is_windows() or camera_capture_backend_is_mac():
            try:
                get_devices = getattr(smartedit.CameraCaptureReader, "GetDeviceNames", None)
    ...
    def start_recording(self):
        ...
        try:
            ...
            self._hide_smartedit_for_recording()
            ...
            self._restore_smartedit_after_recording()
            self._set_record_button_idle()
    ...
    def _recording_start_validation_error(self):
        ...
        if self.screen_card.isChecked() and not self._screen_backend_available():
            return _("Screen recording is not available for this platform or libsmartedit build.")
        if self.camera_card.isChecked():
            if not self._camera_backend_available():
                return _("Webcam recording is not available for this platform or libsmartedit build.")
    ...
    def _build_recorder_settings(self, recording=False):
        settings = smartedit.AudioRecorderSettings()
        ...
        settings.channel_layout = smartedit.LAYOUT_MONO if self._channels == 1 else smartedit.LAYOUT_STEREO
    ...
    def _build_video_jobs(self):
        jobs = []
        screen_fps = smartedit.Fraction(int(self.video_fps_combo.currentData() or 30), 1)
        if self.screen_card.isChecked():
            ...
            settings = smartedit.ScreenCaptureSettings()
            ...
            jobs.append(LiveVideoRecordingJob(
                smartedit.ScreenCaptureReader(settings),
                path,
                settings.width,
                settings.height,
                screen_fps,
        ...
        if self.camera_card.isChecked():
            camera_fps = smartedit.Fraction(int(self.camera_fps_combo.currentData() or 30), 1)
            ...
            job = LiveVideoRecordingJob(
                smartedit.CameraCaptureReader(settings),
                path,
                settings.width,
                settings.height,
                camera_fps,
    ...
    def _apply_recording_clip_defaults(self, clip_data):
        clip_data["scale"] = smartedit.SCALE_FIT
    ...
    def _apply_webcam_clip_layout(self, clip_data):
        ...
        gravity = {
            "top-left": smartedit.GRAVITY_TOP_LEFT,
            "top-right": smartedit.GRAVITY_TOP_RIGHT,
            "bottom-left": smartedit.GRAVITY_BOTTOM_LEFT,
            "bottom-right": smartedit.GRAVITY_BOTTOM_RIGHT,
            "left": smartedit.GRAVITY_LEFT,
            "right": smartedit.GRAVITY_RIGHT,
            "center": smartedit.GRAVITY_CENTER,
            "full": smartedit.GRAVITY_CENTER,
        }.get(layout, smartedit.GRAVITY_BOTTOM_RIGHT)
    ...
    def _keyframe_point(self, value):
        return json.loads(smartedit.Point(1, float(value), smartedit.BEZIER).Json())
    ...
    def _playback_active(self):
        try:
            ...
            return player.Mode() == smartedit.PLAYBACK_PLAY and player.Speed() != 0
        except Exception:
            return False
    ...
    def hideEvent(self, event):
        if self._hiding_smartedit_window:
