"""
Centralized Qt binding loader for SmartEdit.

Selects an available binding (PyQt6/PySide6/PyQt5) using the
`SMARTEDIT_QT_API` env var (`auto` default, otherwise one of
...
if _is_android_runtime():
    ...
    if _smartedit_shiboken_ext is not None:
        logger.debug("qt_api: smartedit_shiboken_ext loaded successfully")
    else:
        logger.warning("qt_api: smartedit_shiboken_ext failed to load: %r", _shiboken_ext_load_error)
...
def wrapinstance(ptr, base_type):
    ...
    if _smartedit_shiboken_ext is not None:
        return _smartedit_shiboken_ext.wrap_instance_u64(ptr, base_type)
...
class _AndroidFilePicker:
    ...
    def _resolve_uri(self, resolver, uri_str, cache_dir, OpenableColumns, File, FileOutputStream):
        ...
        6. Stream-copy into app cache (media files that must have a real path
           for libsmartedit, or cloud files like Google Drive).
        """
        autoclass = self._autoclass
...
def read_file_text(path) -> str:
    ...
    if is_content_uri(path):
        text = read_from_content_uri(path)
...
def _select_binding() -> str:
    ...
    requested = os.environ.get("SMARTEDIT_QT_API", "auto")
    ...
    _FAILED_IMPORT = ImportError(
        ...
        + ", ".join(errors)
        + ". Set SMARTEDIT_QT_API to force a specific binding."
    )
    raise _FAILED_IMPORT
