...
class RecordingPreviewTests(unittest.TestCase):
    ...
    def test_screen_capture_backend_uses_libsmartedit_default_backend(self):
        helper = self.audio_recording_module
        ...
        fake_smartedit = types.SimpleNamespace(
        ...
        with patch.object(helper.sys, "platform", "linux"), patch.object(helper, "smartedit", fake_smartedit):
    ...
    def test_windows_screen_capture_backend_uses_gdi_when_supported(self):
        ...
        with patch.object(helper.sys, "platform", "win32"), patch.object(helper, "smartedit", fake_smartedit):
    ...
    def test_live_video_writer_enables_audio_for_screen_reader_audio(self):
        ...
        with patch.object(helper.smartedit, "FFmpegWriter", FakeWriter):
            job._open()

        self.assertEqual(
            FakeWriter.instance.audio_options,
            (True, "aac", 48000, 2, helper.smartedit.LAYOUT_STEREO, 192000),
        )
    ...
    def test_restore_hidden_smartedit_maximized_window_keeps_maximized_state(self):
        helper = self.audio_recording_module
        ...
        helper.AudioRecordingDockContent._restore_smartedit_window(dock, state)
    ...
    def test_restore_hidden_smartedit_normal_window_restores_geometry(self):
        helper = self.audio_recording_module
    ...
    def test_webcam_corner_layout_uses_native_margin_and_corner_radius(self):
        ...
        self.assertEqual(clip_data["scale"], helper.smartedit.SCALE_FIT)
    ...
    def test_webcam_full_layout_uses_best_fit_not_stretch(self):
        ...
        dock._webcam_layout_scale = lambda: helper.AudioRecordingDockContent._webcam_layout_scale(dock)
        dock._webcam_corner_radius = lambda: helper.AudioRecordingDockContent._webcam_corner_radius(dock)
        clip_data = {"scale": helper.smartedit.SCALE_STRETCH}

        helper.AudioRecordingDockContent._apply_webcam_clip_layout(dock, clip_data)

        self.assertEqual(clip_data["gravity"], helper.smartedit.GRAVITY_CENTER)
        self.assertEqual(clip_data["scale"], helper.smartedit.SCALE_FIT)
    ...
    def test_windows_window_picker_excludes_overlay_not_smartedit_window(self):
        helper = self.recording_widgets_module
        ...
        class FakeUser32:
            ...
            def GetWindowThreadProcessId(self, *_args):
                raise AssertionError("The picker must not reject all SmartEdit windows")
        ...
        with patch.object(helper, "_windows_types", return_value=fake_types), \
                ...
                patch.object(helper, "_windows_window_title", side_effect=lambda hwnd: "SmartEdit" if hwnd == 22 else "Other"), \
        ...
        self.assertEqual(picked[2], "SmartEdit")
    ...
    def test_select_region_limits_windows_picker_to_selected_screen(self):
        ...
        dock = types.SimpleNamespace(
            ...
            _set_hide_smartedit_default=MagicMock(),
            _hide_smartedit_for_picker=MagicMock(return_value=None),
            _restore_smartedit_window=MagicMock(),
    ...
    def test_windows_screen_job_uses_selected_monitor_bounds(self):
        ...
        for x, width, all_screens in (
            ...
            with patch.object(helper, "screen_capture_backend", return_value=7), \
                    ...
                    patch.object(helper.smartedit, "ScreenCaptureSettings", FakeSettings), \
                    patch.object(helper.smartedit, "ScreenCaptureReader", FakeReader):
