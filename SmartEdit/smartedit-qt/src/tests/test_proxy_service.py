...
class ProxyServiceTests(unittest.TestCase):
     ...
     def test_build_proxy_reader_opens_and_closes_source_clip(self):
         ...
         with patch("classes.proxy_service.absolute_media_path", return_value="/media/source.mp4"), \
              ...
              patch("classes.proxy_service.smartedit.Clip", return_value=clip_obj), \
              patch("classes.proxy_service.smartedit.Timeline", side_effect=fake_timeline), \
              patch("classes.proxy_service.smartedit.FFmpegWriter", return_value=writer_obj), \
              patch("classes.proxy_service.smartedit.Fraction", side_effect=lambda num, den: (num, den)), \
     ...
     def test_proxy_root_ignores_backup_project_path(self):
         self.app.project = types.SimpleNamespace(current_filepath="/home/test/.smartedit_qt/backup.osp")

         with patch("classes.proxy_service.info.BACKUP_FILE", "/home/test/.smartedit_qt/backup.osp"), \
              patch("classes.proxy_service.info.RECOVERY_PATH", "/home/test/.smartedit_qt/recovery"), \
              patch("classes.proxy_service.info.PROXY_PATH", "/home/test/.smartedit_qt/optimized"):
             proxy_root = self.service._proxy_root()

         self.assertEqual(proxy_root, "/home/test/.smartedit_qt/optimized")
