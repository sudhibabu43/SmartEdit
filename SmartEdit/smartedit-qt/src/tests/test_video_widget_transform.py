...
def clip_with(scale_mode, gravity=smartedit.GRAVITY_CENTER):
...
class VideoWidgetTransformTests(unittest.TestCase):
    ...
    def test_square_clip_location_y_endpoints_are_offscreen_for_fit_and_crop(self):
        for scale_mode in (smartedit.SCALE_FIT, smartedit.SCALE_CROP):
    ...
    def test_non_crop_locations_remain_canvas_relative(self):
        for scale_mode in (
            smartedit.SCALE_FIT,
            smartedit.SCALE_STRETCH,
            smartedit.SCALE_NONE,
        ):
            with self.subTest(scale_mode=scale_mode):
    ...
    def test_clip_margin_offsets_edge_gravity_handle_rect(self):
        ...
        with patch("windows.video_widget.get_app", return_value=app):
            rect = VideoWidget._clip_display_rect(
                ...
                40,
                30,
                clip_with(smartedit.SCALE_NONE, smartedit.GRAVITY_BOTTOM_RIGHT),
    ...
    def test_clip_margin_reduces_fit_layout_handle_rect(self):
        rect = VideoWidget._clip_display_rect(
            ...
            160,
            90,
            clip_with(smartedit.SCALE_FIT, smartedit.GRAVITY_CENTER),
            props(margin=0.1),
            self.viewport,
        )
    ...
    def test_yolo5_file_sha256(self):
        with tempfile.NamedTemporaryFile(delete=False) as test_file:
            test_file.write(b"smartedit-yolov5")
    ...
    def test_yolo5_validation_uses_libsmartedit(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".onnx") as test_model, \
                ...
                patch(
                    "windows.process_effect.smartedit.ClipProcessingJobs.ValidateONNXModel",
                    return_value="",
                ) as validate:
    ...
    def test_yolo5_validation_reports_libsmartedit_error(self):
        process = ProcessEffect.__new__(ProcessEffect)
        process.onnx_validation_cache = {}
    ...
    def test_object_mask_initializer_defaults_to_efficientsam_downloads(self):
        ...
        self.assertTrue(efficient_sam["value"].endswith(
            ".smartedit_qt/yolo/efficient-sam-tiny-1024/image_segmentation_efficientsam_ti_2025april.onnx"))
        self.assertTrue(cutie_key["value"].endswith(".smartedit_qt/yolo/cutie-medium/cutie-encode-key-640x368.onnx"))
        self.assertTrue(cutie_value["value"].endswith(".smartedit_qt/yolo/cutie-medium/cutie-encode-value-640x368.onnx"))
        self.assertTrue(cutie_readout["value"].endswith(
            ".smartedit_qt/yolo/cutie-medium/cutie-memory-readout-floatmask-valid-640x368-m6-topk30-opencv.onnx"))
        self.assertTrue(cutie_decode["value"].endswith(".smartedit_qt/yolo/cutie-medium/cutie-decode-640x368.onnx"))
    ...
    def test_location_offset_inverse_round_trips_drag_motion(self):
        ...
        source_w, source_h, scaled_w, scaled_h, anchor_x, anchor_y, layout_x, layout_y, layout_w, layout_h = (
            VideoWidget._clip_location_geometry(
                ...
                40,
                40,
                clip_with(smartedit.SCALE_CROP),
                props(),
                self.viewport,
            )
        )
    ...
    def test_scale_none_uses_project_to_viewport_pixel_ratio(self):
        ...
        with patch("windows.video_widget.get_app", return_value=fake_app):
            center = self.rect_for(smartedit.SCALE_NONE)
            ...
            top = self.rect_for(smartedit.SCALE_NONE, location_y=-1.0)
            bottom = self.rect_for(smartedit.SCALE_NONE, location_y=1.0)
            self.assertLessEqual(top.y() + top.height(), 0.0)
            self.assertGreaterEqual(bottom.y(), self.viewport.height())
