...
class DialogPreviewResizeTests(unittest.TestCase):
    ...
    def test_cutting_apply_dynamic_preview_max_size_resumes_playback_when_was_playing(self):
        ...
        player = types.SimpleNamespace(
            Mode=lambda: smartedit.PLAYBACK_PLAY,
            Speed=lambda: 1.0,
        )
        fake = types.SimpleNamespace(
            initialized=True,
    ...
    def test_cutting_build_preview_timeline_uses_source_dimensions(self):
        ...
        with patch("windows.cutting.smartedit.Timeline", FakeTimeline), \
             patch("windows.cutting.smartedit.Clip", FakeClip), \
             patch("windows.cutting.smartedit.Fraction", DummyFraction), \
             patch("windows.cutting.smartedit.FRAME_DISPLAY_CLIP", 7), \
