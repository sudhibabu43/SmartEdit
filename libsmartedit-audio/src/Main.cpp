 ...
 *
 * Welcome to the SmartEdit Audio Library C++ API.
 * This library is used by libsmartedit to enable audio features, which powers
 * the <a href="http://www.smartedit.org">SmartEdit Video Editor</a>
 * application.
 */
...
public:
  ...
  void createAudioDeviceTypes(
      juce::OwnedArray<juce::AudioIODeviceType> &list) override {
...
void display_device_probe(TestAudioDeviceManager &deviceManager) {
  ...
  if (curType == nullptr || curDevice == nullptr) {
    ...
    std::cout << "Sample rate: " << curDevice->getCurrentSampleRate()
              << " Hz\n";
    std::cout << "Block size: " << curDevice->getCurrentBufferSizeSamples()
              << " samples\n";
    std::cout << "Bit depth: " << curDevice->getCurrentBitDepth() << '\n';
  }
}
...
int main(int argc, char *argv[]) {
  ...
  auto error = deviceManager.initialise(0,       // numInputChannelsNeeded
  ...
  std::cout << "\nYou should hear five test tones, precisely " << pause
            << " second" << (pause > 1 ? "s" : "") << " apart.\n";
