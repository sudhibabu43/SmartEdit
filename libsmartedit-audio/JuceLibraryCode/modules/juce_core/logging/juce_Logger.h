...
/**
    ...
    A subclass of Logger can be created and passed into the
   Logger::setCurrentLogger method and this will then be used by all calls to
   writeToLog.

    The logger class also contains methods for writing messages to the
...
public:
  ...
  /** Sets the current logging class to use.
      ...
      Note that the object passed in will not be owned or deleted by the logger,
     so the caller must make sure that it is not deleted while still being used.
  ...
  */
  ...
  /** Writes a message to the standard error stream.
      ...
      juce_PlatformDefs.h (which will avoid calling the method in non-debug
     builds).
  */
