...
Logger::~Logger() {
  ...
  // Always call Logger::setCurrentLogger (nullptr) before deleting the active
  // logger.
...
void JUCE_API JUCE_CALLTYPE logAssertion(const char *const filename,
                                         const int lineNum) noexcept {
  String m("JUCE Assertion failure in ");
  m << File::createFileWithoutCheckingPath(CharPointer_UTF8(filename))
           .getFileName()
    << ':' << lineNum;
