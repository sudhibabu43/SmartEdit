...
#define JUCE_JS_OPERATORS(X)                                                   \
  X(semicolon, ";")                                                            \
  X(dot, ".") X(comma, ",") X(openParen, "(") X(closeParen, ")") X(            \
      openBrace, "{") X(closeBrace, "}") X(openBracket, "[")                   \
      X(closeBracket, "]") X(colon, ":") X(question, "?") X(typeEquals, "===") \
          X(equals, "==") X(assign, "=") X(typeNotEquals, "!==")               \
              X(notEquals, "!=") X(logicalNot, "!") X(plusEquals, "+=") X(     \
                  plusplus, "++") X(plus, "+") X(minusEquals, "-=")            \
                  X(minusminus, "--") X(minus, "-") X(timesEquals, "*=") X(    \
                      times, "*") X(divideEquals,                              \
                                    "/=") X(divide, "/") X(moduloEquals, "%=") \
                      X(modulo, "%") X(xorEquals, "^=") X(bitwiseXor, "^")     \
                          X(andEquals, "&=") X(logicalAnd,                     \
                                               "&&") X(bitwiseAnd, "&")        \
                              X(orEquals, "|=") X(logicalOr, "||") X(          \
                                  bitwiseOr, "|") X(leftShiftEquals, "<<=")    \
                                  X(lessThanOrEqual, "<=") X(leftShift, "<<")  \
                                      X(lessThan, "<") X(rightShiftUnsigned,   \
                                                         ">>>")                \
                                          X(rightShiftEquals, ">>=")           \
                                              X(rightShift, ">>")              \
...
#define JUCE_JS_KEYWORDS(X)                                                    \
  X(var, "var")                                                                \
  X(if_, "if") X(else_, "else") X(do_, "do") X(null_, "null")                  \
      X(while_, "while") X(for_, "for") X(break_, "break")                     \
          X(continue_, "continue") X(undefined, "undefined")                   \
              X(function, "function") X(return_, "return") X(true_, "true")    \
                  X(false_, "false") X(new_, "new") X(typeof_, "typeof")
...
struct JavascriptEngine::RootObject final : public DynamicObject {
  ...
  static bool areTypeEqual(const var &a, const var &b) {
    return a.hasSameTypeAs(b) && isFunction(a) == isFunction(b) &&
           (((a.isUndefined() || a.isVoid()) &&
             (b.isUndefined() || b.isVoid())) ||
            a == b);
  }
  ...
  static var *getPropertyPointer(DynamicObject &o,
                                 const Identifier &i) noexcept {
  ...
  struct CodeLocation {
    ...
    void throwError(const String &message) const {
      ...
      for (auto i = program.getCharPointer(); i < location && !i.isEmpty();
           ++i) {
        ++col;
        if (*i == '\n') {
          col = 1;
      ...
      throw "Line " + String(line) + ", column " + String(col) + " : " +
          message;
    }
  ...
  //==============================================================================
  struct Scope {
    Scope(const Scope *p, ReferenceCountedObjectPtr<RootObject> rt,
          DynamicObject::Ptr scp) noexcept
        : parent(p), root(std::move(rt)), scope(std::move(scp)) {}

    const Scope *const parent;
    ReferenceCountedObjectPtr<RootObject> root;
    DynamicObject::Ptr scope;

    var findFunctionCall(const CodeLocation &location, const var &targetObject,
      ...
      if (auto *o = targetObject.getDynamicObject()) {
        ...
        for (auto *p =
                 o->getProperty(getPrototypeIdentifier()).getDynamicObject();
             p != nullptr;
        ...
        // if there's a class with an overridden DynamicObject::hasMethod, this
        // avoids an error
      ...
      if (targetObject.isString())
        if (auto *m = findRootClassProperty(StringClass::getClassName(),
                                            functionName))
          return *m;

      if (targetObject.isArray())
        if (auto *m =
                findRootClassProperty(ArrayClass::getClassName(), functionName))
          return *m;

      if (auto *m =
              findRootClassProperty(ObjectClass::getClassName(), functionName))
        return *m;
    ...
    var *findRootClassProperty(const Identifier &className,
    ...
    bool findAndInvokeMethod(const Identifier &function,
      ...
      for (int i = 0; i < props.size(); ++i)
        if (auto *o = props.getValueAt(i).getDynamicObject())
          if (Scope(this, *root, *o)
                  .findAndInvokeMethod(function, args, result))
            return true;

      return false;
    }

    bool invokeMethod(const var &m, const var::NativeFunctionArgs &args,
                      var &result) const {
      if (isFunction(m)) {
        auto *target = args.thisObject.getDynamicObject();
    ...
    void checkTimeOut(const CodeLocation &location) const {
      if (Time::getCurrentTime() > root->timeout)
        location.throwError(root->timeout == Time() ? "Interrupted"
                                                    : "Execution timed-out");
    }
  ...
  struct IfStatement final : public Statement {
    ...
    ResultCode perform(const Scope &s, var *returnedValue) const override {
      return (condition->getResult(s) ? trueBranch : falseBranch)
          ->perform(s, returnedValue);
    }
  ...
  struct DotOperator final : public Expression {
    DotOperator(const CodeLocation &l, ExpPtr &p, const Identifier &c) noexcept
        : Expression(l), parent(p.release()), child(c) {}
  ...
  struct ArraySubscript final : public Expression {
    ...
    var getResult(const Scope &s) const override {
      auto arrayVar =
          object->getResult(s); // must stay alive for the scope of this method
  ...
  struct BinaryOperatorBase : public Expression {
    BinaryOperatorBase(const CodeLocation &l, ExpPtr &a, ExpPtr &b,
                       TokenType op) noexcept
  ...
  struct BinaryOperator : public BinaryOperatorBase {
    BinaryOperator(const CodeLocation &l, ExpPtr &a, ExpPtr &b,
                   TokenType op) noexcept
    ...
    var getResult(const Scope &s) const override {
      ...
      if (isNumericOrUndefined(a) && isNumericOrUndefined(b))
        return (a.isDouble() || b.isDouble()) ? getWithDoubles(a, b)
                                              : getWithInts(a, b);
    ...
    var throwError(const char *typeName) const {
      location.throwError(getTokenName(operation) + " is not allowed on the " +
                          typeName + " type");
      return {};
    }
  };
  ...
  struct DivideOp final : public BinaryOperator {
    ...
    var getWithDoubles(double a, double b) const override {
      return exactlyEqual(b, 0.0) ? std::numeric_limits<double>::infinity()
                                  : a / b;
    }
    var getWithInts(int64 a, int64 b) const override {
      return b != 0 ? var((double)a / (double)b)
  ...
  struct Assignment final : public Expression {
    Assignment(const CodeLocation &l, ExpPtr &dest, ExpPtr &source) noexcept
        : Expression(l), target(dest.release()), newValue(source.release()) {}
  ...
  struct SelfAssignment : public Expression {
    SelfAssignment(const CodeLocation &l, Expression *dest,
                   Expression *source) noexcept
  ...
  struct PostAssignment final : public SelfAssignment {
    PostAssignment(const CodeLocation &l, Expression *dest,
                   Expression *source) noexcept
  ...
  struct FunctionCall : public Expression {
    ...
    var getResult(const Scope &s) const override {
      if (auto *dot = dynamic_cast<DotOperator *>(object.get())) {
        ...
        return invokeFunction(
            s, s.findFunctionCall(location, thisObject, dot->child),
            thisObject);
      }
    ...
    var invokeFunction(const Scope &s, const var &function,
                       const var &thisObject) const {
      s.checkTimeOut(location);
      Array<var> argVars;
      ...
      const var::NativeFunctionArgs args(thisObject, argVars.begin(),
                                         argVars.size());
      ...
      if (auto *dot = dynamic_cast<DotOperator *>(object.get()))
        if (auto *o = thisObject.getDynamicObject())
          if (o->hasMethod(dot->child)) // allow an overridden
                                        // DynamicObject::invokeMethod to accept
                                        // a method call.
  ...
  struct ObjectDeclaration final : public Expression {
    ...
    var getResult(const Scope &s) const override {
      ...
      for (int i = 0; i < names.size(); ++i)
        newObject->setProperty(names.getUnchecked(i),
                               initialisers.getUnchecked(i)->getResult(s));
    ...
    var invoke(const Scope &s, const var::NativeFunctionArgs &args) const {
      ...
      for (int i = 0; i < parameters.size(); ++i)
        functionRoot->setProperty(parameters.getReference(i),
                                  i < args.numArguments ? args.arguments[i]
                                                        : var::undefined());
  ...
  struct TokenIterator {
    TokenIterator(const String &code)
        : location(code), p(code.getCharPointer()) {
      skip();
    }
    ...
    void match(TokenType expected) {
      if (currentType != expected)
        location.throwError("Found " + getTokenName(currentType) +
...
#define JUCE_JS_COMPARE_KEYWORD(name, str)                                     \
  if (len == sizeof(str) - 1 && matchToken(TokenTypes::name, len))             \
    return TokenTypes::name;
      ...
      if (p.isDigit()) {
        if (parseHexLiteral() || parseFloatLiteral() || parseOctalLiteral() ||
            parseDecimalLiteral())
...
#define JUCE_JS_COMPARE_OPERATOR(name, str)                                    \
  if (matchToken(TokenTypes::name, sizeof(str) - 1))                           \
    return TokenTypes::name;
      ...
      if (!p.isEmpty())
        location.throwError("Unexpected character '" +
                            String::charToString(*p) + "' in source");
  ...
  private:
    ...
    Expression *parseInPlaceOpExpression(ExpPtr &lhs) {
      ExpPtr rhs(parseExpression());
      Expression *bareLHS =
          lhs.get(); // careful - bare pointer is deliberately aliased
      return new SelfAssignment(location, bareLHS,
    ...
    Statement *parseStatement() {
      ...
      if (matchesAny(TokenTypes::identifier, TokenTypes::literal,
                     TokenTypes::minus))
        return matchEndOfStatement(parseExpression());

      throwError("Found " + getTokenName(currentType) +
    ...
    Statement *parseIf() {
      ...
      s->falseBranch.reset(matchIf(TokenTypes::else_)
                               ? parseStatement()
    ...
    Statement *parseVar() {
      ...
      s->initialiser.reset(matchIf(TokenTypes::assign)
                               ? parseExpression()
    ...
    Statement *parseFunction() {
      ...
      ExpPtr nm(new UnqualifiedName(location, name)),
          value(new LiteralValue(location, fn));
    ...
    Expression *parseSuffixes(Expression *e) {
      ...
      if (currentType == TokenTypes::openParen)
        return parseSuffixes(
            parseFunctionCall(new FunctionCall(location), input));
    ...
    template <typename OpType> Expression *parsePreIncDec() {
      Expression *e =
          parseFactor(); // careful - bare pointer is deliberately aliased
      ExpPtr lhs(e), one(new LiteralValue(location, (int)1));
      return new SelfAssignment(location, e, new OpType(location, lhs, one));
    }

    template <typename OpType> Expression *parsePostIncDec(ExpPtr &lhs) {
      Expression *e =
          lhs.release(); // careful - bare pointer is deliberately aliased
  ...
  struct ArrayClass final : public DynamicObject {
    ...
    static var splice(Args a) {
      if (auto *array = a.thisObject.getArray()) {
        ...
        const int num = a.numArguments > 1
                            ? jlimit(0, arraySize - start, getInt(a, 1))
                            : arraySize - start;
    ...
    static var indexOf(Args a) {
      if (auto *array = a.thisObject.getArray()) {
        ...
        for (int i = (a.numArguments > 1 ? getInt(a, 1) : 0); i < array->size();
             ++i)
  ...
  struct StringClass final : public DynamicObject {
    ...
    static var fromCharCode(Args a) {
      return String::charToString(static_cast<juce_wchar>(getInt(a, 0)));
    }
  ...
  struct MathClass final : public DynamicObject {
    MathClass() {
      ...
      setProperty("LOG2E",
                  std::log(MathConstants<double>::euler) / std::log(2.0));
      setProperty("LOG10E",
                  std::log(MathConstants<double>::euler) / std::log(10.0));
    }
    ...
    static var Math_abs(Args a) {
      return isInt(a, 0) ? var(std::abs(getInt(a, 0)))
                         : var(std::abs(getDouble(a, 0)));
    }
    static var Math_round(Args a) {
      return isInt(a, 0) ? var(roundToInt(getInt(a, 0)))
    ...
    static var Math_range(Args a) {
      return isInt(a, 0) ? var(jlimit(getInt(a, 1), getInt(a, 2), getInt(a, 0)))
                         : var(jlimit(getDouble(a, 1), getDouble(a, 2),
                                      getDouble(a, 0)));
    }
    ...
    // We can't use the std namespace equivalents of these functions without
    // breaking compatibility with older versions of OS X.
    ...
    template <typename Type> static Type sign(Type n) noexcept {
  ...
  struct IntegerClass final : public DynamicObject {
    ...
    static var parseInt(Args a) {
      ...
      return s[0] == '0' ? (s[1] == 'x' ? s.substring(2).getHexValue64()
                                        : getOctalValue(s))
                         : s.getLargeIntValue();
    }
  };
...
JavascriptEngine::JavascriptEngine()
    : maximumExecutionTime(15.0), root(new RootObject()) {
  registerNativeObject(RootObject::ObjectClass ::getClassName(),
                       new RootObject::ObjectClass());
  registerNativeObject(RootObject::ArrayClass ::getClassName(),
                       new RootObject::ArrayClass());
  registerNativeObject(RootObject::StringClass ::getClassName(),
                       new RootObject::StringClass());
  registerNativeObject(RootObject::MathClass ::getClassName(),
                       new RootObject::MathClass());
  registerNativeObject(RootObject::JSONClass ::getClassName(),
                       new RootObject::JSONClass());
  registerNativeObject(RootObject::IntegerClass ::getClassName(),
...
void JavascriptEngine::registerNativeObject(const Identifier &name,
...
var JavascriptEngine::callFunction(const Identifier &function,
  ...
  try {
    ...
    if (result != nullptr)
      *result = Result::ok();
    RootObject::Scope({}, *root, *root)
        .findAndInvokeMethod(function, args, returnVal);
  } catch (String &error) {
    if (result != nullptr)
      *result = Result::fail(error);
  }
...
const NamedValueSet &
JavascriptEngine::getRootObjectProperties() const noexcept {
