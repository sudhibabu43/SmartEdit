...
struct JSONParser {
  JSONParser(String::CharPointerType text)
      : startLocation(text), currentLocation(text) {}
  ...
  [[noreturn]] void throwError(juce::String message,
      ...
      if (c == '\\') {
        ...
        case 'u': {
          ...
          for (int i = 4; --i >= 0;) {
            ...
            if (digitValue < 0)
              throwError("Syntax error in unicode escape sequence",
                         errorLocation);
  ...
  var parseNumber(bool isNegative) {
    ...
    for (;;) {
      ...
      if (CharacterFunctions::isWhitespace(c) || c == ',' || c == '}' ||
          c == ']' || c == 0) {
  ...
  static void writeArray(OutputStream &out, const Array<var> &array,
    ...
    if (!array.isEmpty()) {
      ...
      for (int i = 0; i < array.size(); ++i) {
        ...
        JSON::writeToStream(
            out, array.getReference(i),
            format.withIndentLevel(format.getIndentLevel() + indentSize));
...
void JSON::writeToStream(OutputStream &out, const var &v,
...
var JSON::parse(const File &file) { return parse(file.loadFileAsString()); }
...
String JSON::toString(const var &data, const bool allOnOneLine,
  ...
  return toString(data, FormatOptions{}
                            .withSpacing(allOnOneLine ? Spacing::singleLine
                                                      : Spacing::multiLine)
                            .withMaxDecimalPlaces(maximumDecimalPlaces));
}

void JSON::writeToStream(OutputStream &output, const var &data,
                         const bool allOnOneLine, int maximumDecimalPlaces) {
  writeToStream(
      output, data,
      FormatOptions{}
          .withSpacing(allOnOneLine ? Spacing::singleLine : Spacing::multiLine)
          .withMaxDecimalPlaces(maximumDecimalPlaces));
}
...
public:
  JSONTests() : UnitTest("JSON", UnitTestCategories::json) {}
  ...
  static var createRandomVar(Random &r, int depth) {
    ...
    case 7: {
      ...
      for (int i = r.nextInt(30); --i >= 0;)
        o->setProperty(createRandomIdentifier(r),
                       createRandomVar(r, depth + 1));
