...
String SystemStats::getJUCEVersion() {
  // Some basic tests, to keep an eye on things and make sure these types work
  // ok on all platforms. Let me know if any of these assertions fail on your
  // system!
  ...
  return "JUCE v" JUCE_STRINGIFY(JUCE_MAJOR_VERSION) "." JUCE_STRINGIFY(
...
struct JuceVersionPrinter {
  JuceVersionPrinter() { DBG(SystemStats::getJUCEVersion()); }
};
...
StringArray SystemStats::getDeviceIdentifiers() {
  for (const auto flag :
       {MachineIdFlags::fileSystemId, MachineIdFlags::macAddresses})
...
int SystemStats::getNumCpus() noexcept {
  return getCPUInformation().numLogicalCPUs;
}
int SystemStats::getNumPhysicalCpus() noexcept {
...
bool SystemStats::hasAVX512F() noexcept {
  return getCPUInformation().hasAVX512F;
}
bool SystemStats::hasAVX512BW() noexcept {
  return getCPUInformation().hasAVX512BW;
}
bool SystemStats::hasAVX512CD() noexcept {
  return getCPUInformation().hasAVX512CD;
}
bool SystemStats::hasAVX512DQ() noexcept {
  return getCPUInformation().hasAVX512DQ;
}
bool SystemStats::hasAVX512ER() noexcept {
  return getCPUInformation().hasAVX512ER;
}
bool SystemStats::hasAVX512IFMA() noexcept {
  return getCPUInformation().hasAVX512IFMA;
}
bool SystemStats::hasAVX512PF() noexcept {
  return getCPUInformation().hasAVX512PF;
}
bool SystemStats::hasAVX512VBMI() noexcept {
  return getCPUInformation().hasAVX512VBMI;
}
bool SystemStats::hasAVX512VL() noexcept {
  return getCPUInformation().hasAVX512VL;
}
bool SystemStats::hasAVX512VPOPCNTDQ() noexcept {
...
String SystemStats::getStackBacktrace() {
  ...
  void *stack[128];
  int frames =
      (int)CaptureStackBackTrace(0, numElementsInArray(stack), stack, nullptr);
...
bool SystemStats::isRunningInAppExtensionSandbox() noexcept {
  ...
  static bool isRunningInAppSandbox = [&] {
    File bundle = File::getSpecialLocation(File::invokedExecutableFile)
                      .getParentDirectory();
