...
static String normaliseSeparators(const String &path) {
  ...
  auto uncPath =
      ...
      !normalisedPath.fromFirstOccurrenceOf(doubleSeparator, false, false)
...
String File::parseAbsolutePath(const String &p) {
  ...
  // Windows..
  auto path =
      normaliseSeparators(removeEllipsis(p.replaceCharacter('/', '\\')));
  ...
  if (path.startsWithChar(getSeparatorChar())) {
    if (path[1] != getSeparatorChar()) {
      /*  When you supply a raw string to the File object constructor, it must
         be an absolute path. If you're trying to parse a string that may be
         either a relative path or an absolute path, you MUST provide a context
         against which the partial path can be evaluated - you can do this by
         simply using File::getChildFile() instead of the File constructor. E.g.
         saying "File::getCurrentWorkingDirectory().getChildFile
         (myUnknownPath)" would return an absolute path if that's what was
         supplied, or would evaluate a partial path relative to the CWD.
      */
      jassertfalse;

      path =
          File::getCurrentWorkingDirectory().getFullPathName().substring(0, 2) +
          path;
    }
  } else if (!path.containsChar(':')) {
    /*  When you supply a raw string to the File object constructor, it must be
       an absolute path. If you're trying to parse a string that may be either a
       relative path or an absolute path, you MUST provide a context against
       which the partial path can be evaluated - you can do this by simply using
       File::getChildFile() instead of the File constructor. E.g. saying
        "File::getCurrentWorkingDirectory().getChildFile (myUnknownPath)" would
       return an absolute path if that's what was supplied, or would evaluate a
       partial path relative to the CWD.
    */
    jassertfalse;
  ...
  // Yes, I know it's legal for a unix pathname to contain a backslash, but this
  // assertion is here to catch anyone who's trying to run code that was written
  // on Windows with hard-coded path names. If that's why you've ended up here,
  // use File::getChildFile() to build your paths instead.
  jassert(
      (!p.containsChar('\\')) ||
  ...
  if (path.startsWithChar('~')) {
    if (path[1] == getSeparatorChar() || path[1] == 0) {
      ...
      auto userName =
          path.substring(1).upToFirstOccurrenceOf("/", false, false);
  ...
  while (path.endsWithChar(getSeparatorChar()) &&
         path != getSeparatorString()) // careful not to turn a single "/" into
                                       // an empty string.
...
bool File::isNonEmptyDirectory() const {
  ...
  return RangedDirectoryIterator(*this, false, "*", findFilesAndDirectories) !=
...
String File::descriptionOfSizeInBytes(const int64 bytes) {
  ...
  return (divisor > 0 ? String((double)bytes / divisor, 1) : String(bytes)) +
         suffix;
}
...
Result File::createDirectory() const {
  ...
  if (r.wasOk())
    r = createDirectoryInternal(
        fullPath.trimCharactersAtEnd(getSeparatorString()));
...
//==============================================================================
Array<File> File::findChildFiles(int whatToLookFor, bool searchRecursively,
...
int File::findChildFiles(Array<File> &results, int whatToLookFor,
                         bool searchRecursively, const String &wildcard,
                         FollowSymlinks followSymlinks) const {
  int total = 0;

  for (const auto &di : RangedDirectoryIterator(
...
int File::getNumberOfChildFiles(const int whatToLookFor,
  ...
  return std::accumulate(
      RangedDirectoryIterator(*this, false, wildCardPattern, whatToLookFor),
...
bool File::containsSubDirectories() const {
  ...
  return RangedDirectoryIterator(*this, false, "*", findDirectories) !=
         RangedDirectoryIterator();
}
  ...
  if (f.exists()) {
    ...
    if (prefix.trim().endsWithChar(')')) {
      ...
      if (openBracks > 0 && closeBracks > openBracks &&
          prefix.substring(openBracks + 1, closeBracks)
    ...
    do {
      ...
      if (putNumbersInBrackets) {
        ...
        if (CharacterFunctions::isDigit(prefix.getLastCharacter()))
          newName << '_'; // pad with an underscore if the name already ends in
                          // a digit
...
File File::getNonexistentSibling(const bool putNumbersInBrackets) const {
  ...
  return getParentDirectory().getNonexistentChildFile(
      getFileNameWithoutExtension(), getFileExtension(), putNumbersInBrackets);
}
...
bool File::hasFileExtension(StringRef possibleSuffix) const {
  if (possibleSuffix.isEmpty())
    return fullPath.lastIndexOfChar('.') <=
           fullPath.lastIndexOfChar(getSeparatorChar());

  auto semicolon = possibleSuffix.text.indexOf((juce_wchar)';');

  if (semicolon >= 0)
    return hasFileExtension(
               String(possibleSuffix.text).substring(0, semicolon).trimEnd()) ||
           hasFileExtension(
               (possibleSuffix.text + (semicolon + 1)).findEndOfWhitespace());
...
File::createOutputStream(size_t bufferSize) const {
...
bool File::replaceWithText(const String &textToWrite, bool asUnicode,
                           bool writeHeaderBytes, const char *lineFeed) const {
  TemporaryFile tempFile(*this, TemporaryFile::useHiddenFile);
  tempFile.getFile().appendText(textToWrite, asUnicode, writeHeaderBytes,
                                lineFeed);
  return tempFile.overwriteTargetFileWithTemporary();
}
...
String File::getRelativePathFrom(const File &dir) const {
  ...
  auto dirPath = addTrailingSeparator(
      dir.existsAsFile() ? dir.getParentDirectory().getFullPathName()
                         : dir.fullPath);
...

  {
    ...
    for (int i = 0;;) {
      ...
      if (c1 != c2
#else
      if ((c1 != c2 && CharacterFunctions::toLowerCase(c1) !=
  ...
  if (commonBitLength == 0 ||
      (commonBitLength == 1 && thisPath[1] == getSeparatorChar()))
    return fullPath;
...
File File::createTempFile(StringRef fileNameEnding) {
  auto tempFile =
      getSpecialLocation(tempDirectory)
          .getChildFile("temp_" + String::toHexString(
...
bool File::createSymbolicLink(const File &linkFileToCreate,
  ...
  if (symlink(nativePathOfTarget.toRawUTF8(),
              linkFileToCreate.getFullPathName().toRawUTF8()) == -1) {
    jassertfalse;
    return false;
  }
  ...
  return CreateSymbolicLink(
             linkFileToCreate.getFullPathName().toWideCharPointer(),
...
bool File::createSymbolicLink(const File &linkFileToCreate,
  ...
  return createSymbolicLink(linkFileToCreate, getFullPathName(),
                            overwriteExisting);
}
...
MemoryMappedFile::MemoryMappedFile(const File &file,
                                   MemoryMappedFile::AccessMode mode,
                                   bool exclusive)
...
public:
  FileTests() : UnitTest("Files", UnitTestCategories::files) {}
  ...
  void runTest() override {
    ...
    expect(File::getSpecialLocation(File::userApplicationDataDirectory)
               .isDirectory());
...

    {
      ...
      // (on windows, some of the drives may not contain media, so as long as at
      // least one is ok..)
    ...
    expect(temp.findChildFiles(File::findFilesAndDirectories, false, "*")
               .contains(demoFolder));
    expect(temp.findChildFiles(File::findDirectories, true, "*.folder")
               .contains(demoFolder));
    ...
    expect(home.getChildFile("././xyz") == home.getChildFile("xyz"));
    expect(home.getChildFile("../xyz") ==
           home.getParentDirectory().getChildFile("xyz"));
    expect(home.getChildFile(".././xyz") ==
           home.getParentDirectory().getChildFile("xyz"));
    expect(home.getChildFile(".././xyz/./abc") ==
           home.getParentDirectory().getChildFile("xyz/abc"));
    expect(home.getChildFile("./../xyz") ==
           home.getParentDirectory().getChildFile("xyz"));
    expect(home.getChildFile("a1/a2/a3/./../../a4") ==
           home.getChildFile("a1/a4"));
    ...
    expect(std::abs((int)(tempFile.getLastModificationTime().toMilliseconds() -
    ...
    expectEquals(tempFile.getRelativePathFrom(demoFolder.getParentDirectory()),
                 demoFolder.getFileName() + File::getSeparatorString() +
                     tempFile.getFileName());
    expectEquals(demoFolder.getParentDirectory().getRelativePathFrom(tempFile),
                 ".." + File::getSeparatorString() + ".." +
                     File::getSeparatorString() +
                     demoFolder.getParentDirectory().getFileName());

    expect(demoFolder.getNumberOfChildFiles(File::findFiles) == 1);
    expect(demoFolder.getNumberOfChildFiles(File::findFilesAndDirectories) ==
           1);
    expect(demoFolder.getNumberOfChildFiles(File::findDirectories) == 0);
    demoFolder.getNonexistentChildFile("tempFolder", "", false)
        .createDirectory();
...

    {
      ...
      expectEquals(url.getChildURL("x").toString(false),
                   String("https://audio.dev/foo/bar/x"));
      expectEquals(url.getParentURL().toString(false),
                   String("https://audio.dev/foo"));
      expectEquals(url.getParentURL().getParentURL().toString(false),
                   String("https://audio.dev/"));
      expectEquals(
          url.getParentURL().getParentURL().getParentURL().toString(false),
          String("https://audio.dev/"));
      expectEquals(url.getParentURL().getChildURL("x").toString(false),
                   String("https://audio.dev/foo/x"));
      expectEquals(url.getParentURL()
                       .getParentURL()
                       .getParentURL()
                       .getChildURL("x")
                       .toString(false),
                   String("https://audio.dev/x"));
    }
