...
*/
inline const DirectoryEntry &operator*(const DirectoryEntry &e) noexcept {
  return e;
}
...
/**
    ...
    for (DirectoryEntry entry : RangedDirectoryIterator (File
   ("/path/to/folder"), isRecursive)) if (entry.isHidden())
...
public:
  ...
  /** Creates a RangedDirectoryIterator for a given directory.
      ...
      @param isRecursive      whether all the subdirectories should also be
     searched
      @param wildCard         the file pattern to match. This may contain
     multiple patterns separated by a semi-colon or comma, e.g. "*.jpg;*.png"
      @param whatToLookFor    a value from the File::TypesOfFileToFind enum,
     specifying whether to look for files, directories, or both.
...
*/
inline RangedDirectoryIterator begin(const RangedDirectoryIterator &it) {
  return it;
}
...
/** Returns a default-constructed sentinel value.
    Provided for range-for compatibility.
*/
inline RangedDirectoryIterator end(const RangedDirectoryIterator &) {
  return {};
}
