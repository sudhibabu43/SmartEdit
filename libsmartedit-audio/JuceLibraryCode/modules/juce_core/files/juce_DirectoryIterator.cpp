...
bool DirectoryIterator::fileMatches(const StringArray &wildcards,
...
bool DirectoryIterator::next(bool *isDirResult, bool *isHiddenResult,
  ...
  for (;;) {
    ...
    if (subIterator != nullptr) {
      if (subIterator->next(isDirResult, isHiddenResult, fileSize, modTime,
    ...
    while (fileFinder.next(filename, &isDirectory,
      ...
      if (!filename.containsOnly(".")) {
          ...
          const auto mayRecurseIntoPossibleSymlink = [this, &fullPath] {
            return followSymlinks == File::FollowSymlinks::yes ||
                   ...
                   (followSymlinks == File::FollowSymlinks::noCycles &&
                    knownPaths->find(fullPath.getLinkedTarget()) ==
                        knownPaths->end());
          };

          if (isRecursive && mayRecurseIntoPossibleHiddenDir() &&
              mayRecurseIntoPossibleSymlink())
            subIterator.reset(
                new DirectoryIterator(fullPath, true, wildCard, whatToLookFor,
                                      followSymlinks, knownPaths));
        ...
        // if we're not relying on the OS iterator to do the wildcard match, do
        // it now..
...
const File &DirectoryIterator::getFile() const {
  ...
  // You need to call DirectoryIterator::next() before asking it for the file
  // that it found!
...
float DirectoryIterator::getEstimatedProgress() const {
  if (totalNumFiles < 0)
    totalNumFiles =
        File(path).getNumberOfChildFiles(File::findFilesAndDirectories);

  if (totalNumFiles <= 0)
    return 0.0f;

  auto detailedIndex = (subIterator != nullptr)
