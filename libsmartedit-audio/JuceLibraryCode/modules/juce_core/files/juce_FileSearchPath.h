...
public:
  ...
  */
  ...
  /** Returns the unaltered text of the folder at the specified index.
      ...
      Unlike operator[], this function returns the exact text that was entered.
     It does not attempt to convert the path into an absolute path.

      This may be useful if the directory string is expected to understand
     environment variables or other placeholders that the File constructor
  ...
  */
  ...
  /** Adds a new directory to the search path.
      ...
      The new directory is added to the end of the list if the insertIndex
     parameter is less than zero, otherwise it is inserted at the given index.
  */
  ...
  */
  ...
  /** Removes any directories that are actually subdirectories of one of the
     other directories in the search path.

      If the search is intended to be recursive, there's no point having nested
     folders in the search path, because they'll just get searched twice and
     you'll get duplicate results.

      e.g. if the path is "c:\abc\de;c:\abc", this method will simplify it to
     "c:\abc"
  */
  ...
  /** Searches the path for a wildcard.
      ...
      This will search all the directories in the search path in order and
     return an array of the files that were found.

      @param whatToLookFor            a value from the File::TypesOfFileToFind
     enum, specifying whether to return files, directories, or both.
      @param searchRecursively        whether to recursively search the
     subdirectories too
  ...
  */
  ...
  /** Finds out whether a file is inside one of the path's directories.
      ...
      @param checkRecursively if true, then this will return true if the file is
     inside a subfolder of one of the path's directories (at any depth). If
     false it will only return true if the file is actually a direct child of
