...
FileSearchPath::FileSearchPath(const String &path) { init(path); }
...
int FileSearchPath::getNumPaths() const { return directories.size(); }
...
String FileSearchPath::toString() const { return toStringWithSeparator(";"); }
...
void FileSearchPath::remove(int index) { directories.remove(index); }
...
void FileSearchPath::removeRedundantPaths() {
  ...
  for (const auto &directory : directories) {
    const auto checkedIsChildOf = [&](const auto &a, const auto &b) {
      return File::isAbsolutePath(a) && File::isAbsolutePath(b) &&
             File(a).isAChildOf(b);
    };
    ...
    if (std::find_if(reduced.begin(), reduced.end(), fContainsDirectory) !=
        reduced.end())
      continue;
    ...
    reduced.erase(
        std::remove_if(reduced.begin(), reduced.end(), directoryContainsF),
        reduced.end());
...
Array<File> FileSearchPath::findChildFiles(int whatToLookFor, bool recurse,
...
public:
  ...
  void runTest() override {
    beginTest("removeRedundantPaths");
    {
      ...
      {
        FileSearchPath fsp{prefix + "/a/b/c/d;" + prefix + "/a/b/c/e;" +
                           prefix + "/a/b/c"};
      ...
      {
        FileSearchPath fsp{prefix + "/a/b/c;" + prefix + "/a/b/c/d;" + prefix +
                           "/a/b/c/e"};
        fsp.removeRedundantPaths();
        expectEquals(fsp.toString(), prefix + "/a/b/c");
      }
      ...
      {
        FileSearchPath fsp{prefix + "/a/b/c/d;" + prefix + "/a/b/c;" + prefix +
                           "/a/b/c/e"};
        fsp.removeRedundantPaths();
        expectEquals(fsp.toString(), prefix + "/a/b/c");
      }
      ...
      {
        FileSearchPath fsp{"%FOO%;" + prefix + "/a/b/c;%FOO%;" + prefix +
                           "/a/b/c/d"};
        fsp.removeRedundantPaths();
