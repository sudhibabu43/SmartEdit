...
struct Reservoir {
  ...
  */
  template <typename Index, typename GetBufferedRange,
            typename ReadFromReservoir, typename FillReservoir>
  static Range<Index> doBufferedRead(Range<Index> rangeToRead,
    ...
    while (!rangeToRead.isEmpty()) {
      ...
      if (bufferedRange.contains(rangeToRead.getStart())) {
        const auto rangeToReadInBuffer =
            rangeToRead.getIntersectionWith(getBufferedRange());
