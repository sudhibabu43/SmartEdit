...
/**
    ...
    @tags{Core}
*/
template <typename ObjectType> struct ContainerDeletePolicy {
  static void destroy(ObjectType *object) {
    // If the line below triggers a compiler error, it means that you are using
    // an incomplete type for ObjectType (for example, a type that is declared
    // but not defined). This is a problem because then the following delete is
    // undefined behaviour. The purpose of the sizeof is to capture this
    // situation. If this was caused by a OwnedArray of a forward-declared type,
    // move the implementation of all methods trying to use the OwnedArray (e.g.
    // the destructor of the class owning it) into cpp files where they can see
    // to the definition of ObjectType. This should fix the error.
