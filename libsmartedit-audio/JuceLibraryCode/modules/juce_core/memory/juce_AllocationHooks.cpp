...
void notifyAllocationHooksForThread() {
  getAllocationHooksForThread().listenerList.call(
      [](AllocationHooks::Listener &l) { l.newOrDeleteCalled(); });
}
...
UnitTestAllocationChecker::~UnitTestAllocationChecker() noexcept {
  ...
  unitTest.expectEquals((int)calls, 0,
                        "new or delete was incorrectly called while allocation "
                        "checker was active");
}
