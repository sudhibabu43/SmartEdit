...
*/
template <class ObjectType> class [[deprecated]] ScopedPointer {
public:
  //==============================================================================
  ...
  void reset(ObjectType *newObject) {
    if (object != newObject) {
      ...
      // You're trying to reset this ScopedPointer to itself! This will work
      // here as ScopedPointer does an equality check but be aware that
      // std::unique_ptr won't do this and you could end up with some nasty,
      // subtle bugs!
      jassert(newObject == nullptr);
    }
  }

  void reset(ScopedPointer &newObject) { reset(newObject.release()); }
...
private:
  ...
  const ScopedPointer *getAddress() const noexcept {
    return this;
  } // Used internally to avoid the & operator
...
bool operator==(ObjectType1 *pointer1,
...
template <typename ObjectType1, typename ObjectType2>
bool operator==(const ScopedPointer<ObjectType1> &pointer1,
                ObjectType2 *pointer2) noexcept {
...
template <class ObjectType>
bool operator==(const ScopedPointer<ObjectType> &pointer,
...
//==============================================================================
// NB: This is just here to prevent any silly attempts to call deleteAndZero()
// on a ScopedPointer.
template <typename Type> void deleteAndZero(ScopedPointer<Type> &) {
