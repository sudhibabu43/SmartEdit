...
/**
    ...
    The SharedResourcePointer offers a pattern that differs from using a
   singleton or static instance of an object, because it uses reference-counting
   to make sure that the underlying shared object is automatically
   created/destroyed according to the number of SharedResourcePointer objects
   that exist. When the last one is deleted, the underlying object is also
   immediately destroyed. This allows you to use scoping to manage the lifetime
   of a shared resource.
    ...
    struct MySharedData
    {
        // There's no need to ever create an instance of this class directly
   yourself,
    ...
    struct DataUserClass
    {
        DataUserClass()
        {
            ...
            // shared common instance of MySharedData referenced by their
   sharedData
            // member variables.
        ...
        // is guaranteed to be available for as long as the DataUserClass
   object. SharedResourcePointer<MySharedData> sharedData;
    };
    ...
    @tags{Core}
*/
template <typename SharedObjectType> class SharedResourcePointer {
public:
  /** Creates an instance of the shared object.
      ...
      using. Otherwise, if this is the first SharedResourcePointer to be
     created, then a shared object will be created automatically.
  */
...
#ifndef DOXYGEN
  [[deprecated(
      "If you are relying on this function please inform the JUCE team as we "
      "are planing on removing this in a subsequent release")]] int
  getReferenceCount() const noexcept {
    return (int)sharedObject.use_count();
  }
#endif

  /** Returns the SharedResourcePointer if one already exists, or a null
   * optional otherwise. */
...
private:
  explicit SharedResourcePointer(
      std::shared_ptr<SharedObjectType> &&other) noexcept
