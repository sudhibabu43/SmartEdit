...
template <typename T> struct DiffTypeHelper {
  using Type = T;
};
template <typename T> struct DiffTypeHelper<T *> {
...
/**
    ...
    @tags{Core}
*/
template <typename Type> struct Atomic final {
  ...
  /** Atomically compares this value with a target value, and if it is equal,
     sets this to be equal to a new value.
      ...
      @returns true if the comparison was true and the value was replaced; false
     if the comparison failed and the value was left unchanged.
  ...
  */
  ...
  [[deprecated(
      ...
      "std::atomic. Use compareAndSetBool instead.")]] Type
      compareAndSetValue(Type, Type) noexcept;
#endif
};
