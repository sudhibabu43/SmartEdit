...
public:
  ...
  */
  template <typename Type> char &operator[](const Type offset) noexcept {
    return data[offset];
  }
  ...
  /** Resizes the memory block.
     ...
     either be cleared, or left uninitialised.

      @param newSize                      the new desired size for the block
      @param initialiseNewSpaceToZero     if the block gets enlarged, this
     determines whether to clear the new section or just leave it uninitialised
      @see ensureSize
  */
  ...
  /** Increases the block's size only if it's smaller than a given size.
     ...
     this size, no action will be taken; otherwise it will be increased to this
     size
      @param initialiseNewSpaceToZero     if the block gets enlarged, this
     determines whether to clear the new section or just leave it uninitialised
      @see setSize
  ...
  */
  ...
  /** Resizes this block to the given size and fills its contents from the
     supplied buffer. The data pointer must not be null.
  */
  void replaceAll(const void *data, size_t numBytes);

  /** Inserts some data into the block.
      The dataToInsert pointer must not be null. This block's size will be
     increased accordingly. If the insert position lies outside the valid range
     of the block, it will be clipped to within the range before being used.
  */
  void insert(const void *dataToInsert, size_t numBytesToInsert,
              size_t insertPosition);

  /** Chops out a section  of the block.

      This will remove a section of the memory block and close the gap around
     it, shifting any subsequent data downwards and reducing the size of the
     block.
  ...
  */
  ...
  /** Copies data into this MemoryBlock from a memory address.
     ...
     size of the memory block, it will be clipped so not to do anything nasty)
  */
  void copyFrom(const void *srcData, int destinationOffset,
  ...
  /** Copies data from this MemoryBlock to a memory address.
     ...
     limits of the memory block, zeros will be used for that portion of the
     data)
  */
  ...
  */
  ...
  //==============================================================================
  /** Attempts to parse the contents of the block as a zero-terminated UTF8
   * string. */
  String toString() const;

  //==============================================================================
  /** Parses a string of hexadecimal numbers and writes this data into the
     memory block.

      The block will be resized to the number of valid bytes read from the
     string. Non-hex characters in the string will be ignored.
  ...
  */
  ...
  //==============================================================================
  /** Sets a number of bits in the memory block, treating it as a long binary
   * sequence. */
  void setBitRange(size_t bitRangeStart, size_t numBits,
                   int binaryNumberToApply) noexcept;

  /** Reads a number of bits from the memory block, treating it as one long
   * binary sequence */
  int getBitRange(size_t bitRangeStart, size_t numBitsToRead) const noexcept;

  //==============================================================================
  /** Returns a string of characters in a JUCE-specific text encoding that
     represents the binary contents of this block.

      This uses a JUCE-specific (i.e. not standard!) 64-bit encoding system to
     convert binary data into a string of ASCII characters for purposes like
     storage in XML. Note that this proprietary format is mainly kept here for
     backwards-compatibility, and you may prefer to use the Base64::toBase64()
     method if you want to use the standard base-64 encoding.

      @see fromBase64Encoding, Base64::toBase64, Base64::convertToBase64
  */
  String toBase64Encoding() const;

  /** Takes a string created by MemoryBlock::toBase64Encoding() and extracts the
     original data.
      ...
      Note that these methods use a JUCE-specific (i.e. not standard!) 64-bit
     encoding system. You may prefer to use the Base64::convertFromBase64()
     method if you want to use the standard base-64 encoding.
  ...
  */
  ...
  //==============================================================================
#ifndef DOXYGEN
  [[deprecated("Use the replaceAll method instead, which will also replace the "
               "data when numBytes == 0.")]] void
  replaceWith(const void *srcData, size_t numBytes) {
    if (numBytes > 0)
