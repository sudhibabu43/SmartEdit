...
public:
   ...
   * big-endian */
  template <typename Type> static Type swapIfBigEndian(Type value) noexcept {
  ...
  /** Swaps the byte order of a signed or unsigned integer if the CPU is
   * little-endian */
  template <typename Type> static Type swapIfLittleEndian(Type value) noexcept {
#if JUCE_LITTLE_ENDIAN
    return swap(value);
#else
    return value;
   ...
   * significance. */
  constexpr static uint32 makeInt(uint8 leastSig, uint8 byte1, uint8 byte2,
                                  uint8 mostSig) noexcept;

  /** Constructs a 64-bit integer from its constituent bytes, in order of
   * significance. */
  constexpr static uint64 makeInt(uint8 leastSig, uint8 byte1, uint8 byte2,
                                  uint8 byte3, uint8 byte4, uint8 byte5,
                                  uint8 byte6, uint8 mostSig) noexcept;
...
//==============================================================================
constexpr inline uint16 ByteOrder::swap(uint16 v) noexcept {
  return static_cast<uint16>((v << 8) | (v >> 8));
}
constexpr inline int16 ByteOrder::swap(int16 v) noexcept {
  return static_cast<int16>(swap(static_cast<uint16>(v)));
}
inline int32 ByteOrder::swap(int32 v) noexcept {
  return static_cast<int32>(swap(static_cast<uint32>(v)));
}
inline int64 ByteOrder::swap(int64 v) noexcept {
...
constexpr inline uint16 ByteOrder::makeInt(uint8 b0, uint8 b1) noexcept {
  return static_cast<uint16>(static_cast<uint16>(b0) |
                             (static_cast<uint16>(b1) << 8));
}

constexpr inline uint32 ByteOrder::makeInt(uint8 b0, uint8 b1, uint8 b2,
                                           uint8 b3) noexcept {
  return static_cast<uint32>(b0) | (static_cast<uint32>(b1) << 8) |
         (static_cast<uint32>(b2) << 16) | (static_cast<uint32>(b3) << 24);
}

constexpr inline uint64 ByteOrder::makeInt(uint8 b0, uint8 b1, uint8 b2,
                                           uint8 b3, uint8 b4, uint8 b5,
                                           uint8 b6, uint8 b7) noexcept {
  return static_cast<uint64>(b0) | (static_cast<uint64>(b1) << 8) |
         (static_cast<uint64>(b2) << 16) | (static_cast<uint64>(b3) << 24) |
         (static_cast<uint64>(b4) << 32) | (static_cast<uint64>(b5) << 40) |
         (static_cast<uint64>(b6) << 48) | (static_cast<uint64>(b7) << 56);
}

constexpr inline uint16
ByteOrder::littleEndianShort(const void *bytes) noexcept {
...
ByteOrder::littleEndianInt64(const void *bytes) noexcept {
  return makeInt(static_cast<const uint8 *>(bytes)[0],
                 static_cast<const uint8 *>(bytes)[1],
                 static_cast<const uint8 *>(bytes)[2],
                 static_cast<const uint8 *>(bytes)[3],
...
constexpr inline uint16 ByteOrder::bigEndianShort(const void *bytes) noexcept {
  return makeInt(static_cast<const uint8 *>(bytes)[1],
...
ByteOrder::littleEndian24Bit(const void *bytes) noexcept {
...
inline void ByteOrder::littleEndian24BitToChars(int32 value,
                                                void *destBytes) noexcept {
...
inline void ByteOrder::bigEndian24BitToChars(int32 value,
                                             void *destBytes) noexcept {
