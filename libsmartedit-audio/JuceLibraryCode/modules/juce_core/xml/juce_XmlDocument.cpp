...
std::unique_ptr<XmlElement> parseXMLIfTagMatches(const File &file,
                                                 StringRef requiredTag) {
...
static bool isIdentifierChar(juce_wchar c) noexcept {
  ...
  return ((int)c < (int)numElementsInArray(legalChars) * 32)
             ? ((legalChars[c >> 5] & (uint32)(1 << (c & 31))) != 0)
...
findEndOfToken(String::CharPointerType p) noexcept {
...
XmlDocument::getDocumentElement(const bool onlyReadOuterDocumentElement) {
    ...
    if (in != nullptr) {
      ...
      if (data.getDataSize() > 2) {
        ...
        if (CharPointer_UTF16::isByteOrderMarkBigEndian(text) ||
          ...
          // parse the input buffer directly to avoid copying it all to a
          // string..
          return parseDocumentElement(String::CharPointerType(text),
  ...
  return parseDocumentElement(originalText.getCharPointer(),
                              onlyReadOuterDocumentElement);
}
...
String XmlDocument::getFileContents(const String &filename) const {
  if (inputSource != nullptr) {
    std::unique_ptr<InputStream> in(
        inputSource->createInputStreamFor(filename.trim().unquoted()));
...
XmlDocument::parseDocumentElement(String::CharPointerType textToParse,
  ...
  if (textToParse.isEmpty()) {
    ...
    lastError.clear();
    std::unique_ptr<XmlElement> result(
        readNextElement(!onlyReadOuterDocumentElement));
...
bool XmlDocument::parseHeader() {
  ...
  if (CharacterFunctions::compareUpTo(input, CharPointer_ASCII("<?xml"), 5) ==
      0) {
    ...
    /* If you load an XML document with a non-UTF encoding type, it may have
       been loaded wrongly.. Since all the files are read via the normal juce
       file streams, they're treated as UTF-8, so by the time it gets to the
       parser, the encoding will have been lost. Best plan is to stick to utf-8
       or if you have specific files to read, use your own code to convert them
       to a unicode String, and pass that to the XML parser.
    */
...
bool XmlDocument::parseDTD() {
  if (CharacterFunctions::compareUpTo(input, CharPointer_ASCII("<!DOCTYPE"),
                                      9) == 0) {
    input += 9;
    auto dtdStart = input;
...
XmlElement *XmlDocument::readNextElement(const bool alsoParseSubElements) {
  ...
  if (*input == '<') {
    ...
    input = endOfToken;
    LinkedListPointer<XmlElement::XmlAttributeNode>::Appender attributeAppender(
        node->attributes);
    ...
    for (;;) {
      ...
      if (XmlIdentifierChars::isIdentifierChar(c)) {
        ...
        if (!outOfData)
          setLastError("illegal character found in " + node->getTagName() +
                           ": '" + c + "'",
                       false);
      }
...
void XmlDocument::readChildElements(XmlElement &parent) {
  LinkedListPointer<XmlElement>::Appender childAppender(
  ...
  for (;;) {
    ...
    if (*input == '<') {
      ...
      if (c1 == '!' && CharacterFunctions::compareUpTo(
                           input + 2, CharPointer_ASCII("[CDATA["), 7) == 0) {
        input += 9;
        ...
        for (;;) {
          ...
          if (c0 == ']' && input[1] == ']' && input[2] == '>') {
            childAppender.append(
                XmlElement::createTextElement(String(inputStart, input)));
            input += 3;
            break;
          }
    ...
    } else // must be a character block
    {
      ...
      for (;;) {
        ...
        if (c == '&') {
          ...
          if (entity.startsWithChar('<') && entity[1] != 0) {
            ...
            contentShouldBeUsed =
                contentShouldBeUsed || entity.containsNonWhitespaceChars();
          }
        } else {
          for (;; ++input) {
            auto nextChar = *input;
            ...
            contentShouldBeUsed = contentShouldBeUsed ||
      ...
      if (contentShouldBeUsed)
        childAppender.append(
            XmlElement::createTextElement(textElementContent.toUTF8()));
    }
  }
}
...
String XmlDocument::expandEntity(const String &ent) {
  ...
  if (ent[0] == '#') {
    ...
    if (char1 == 'x' || char1 == 'X')
      return String::charToString(
          static_cast<juce_wchar>(ent.substring(2).getHexValue32()));

    if (char1 >= '0' && char1 <= '9')
      return String::charToString(
          static_cast<juce_wchar>(ent.substring(1).getIntValue()));
    ...
    if (dtdText.isNotEmpty()) {
      ...
      if (tokenisedDTD[tokenisedDTD.size() - 2].equalsIgnoreCase("system") &&
        ...
        if (openBracket > 0) {
          ...
          if (closeBracket > openBracket)
            tokenisedDTD.addTokens(
                dtdText.substring(openBracket + 1, closeBracket), true);
        }
      }
      ...
      for (int i = tokenisedDTD.size(); --i >= 0;) {
        if (tokenisedDTD[i].startsWithChar('%') &&
          ...
          auto parsed = getParameterEntity(
              tokenisedDTD[i].substring(1, tokenisedDTD[i].length() - 1));
          StringArray newToks;
  ...
  for (int i = 0; i < tokenisedDTD.size(); ++i) {
    if (tokenisedDTD[i] == entity) {
      if (tokenisedDTD[i - 1].equalsIgnoreCase("<!entity")) {
        auto ent =
            tokenisedDTD[i + 1].trimCharactersAtEnd(">").trim().unquoted();
        ...
        while (ampersand >= 0) {
          ...
          ent = ent.substring(0, ampersand) + resolved +
...
String XmlDocument::getParameterEntity(const String &entity) {
  for (int i = 0; i < tokenisedDTD.size(); ++i) {
    if (tokenisedDTD[i] == entity && tokenisedDTD[i - 1] == "%" &&
