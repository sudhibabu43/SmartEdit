...
/**
    ...
    @tags{Core}
*/
template <typename FloatType> class StatisticsAccumulator {
public:
  //==============================================================================
  ...
  */
  FloatType getVariance() const noexcept {
    return count > 0
               ? (sumSquares - sum * sum / (FloatType)count) / (FloatType)count
               : FloatType();
  }
...
private:
  ...
  struct KahanSum {
    ...
    void JUCE_NO_ASSOCIATIVE_MATH_OPTIMISATIONS
    operator+=(FloatType value) noexcept {
