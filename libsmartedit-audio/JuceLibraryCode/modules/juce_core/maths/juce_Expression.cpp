...
public:
  ...
  virtual ReferenceCountedObjectPtr<Term> resolve(const Scope &,
  ...
  virtual ReferenceCountedObjectPtr<Term>
  createTermToEvaluateInput(const Scope &, const Term * /*inputTerm*/,
  ...
  virtual String getName() const {
    jassertfalse; // You shouldn't call this for an expression that's not
                  // actually a function!
    return {};
  }

  virtual void renameSymbol(const Symbol &oldSymbol, const String &newName,
                            const Scope &scope, int recursionDepth) {
  ...
  virtual void visitAllSymbols(SymbolVisitor &visitor, const Scope &scope,
                               int recursionDepth) {
    for (int i = getNumInputs(); --i >= 0;)
...
struct Expression::Helpers {
  ...
  public:
    ...
    TermPtr resolve(const Scope &scope, int recursionDepth) override {
      return *new Constant(
          performFunction(left->resolve(scope, recursionDepth)->toDouble(),
  ...
  protected:
    ...
    TermPtr createDestinationTerm(const Scope &scope, const Term *input,
                                  double overallTarget,
                                  Term *topLevelTerm) const {
      ...
      if (auto dest = findDestinationFor(topLevelTerm, this))
        return dest->createTermToEvaluateInput(scope, this, overallTarget,
                                               topLevelTerm);
  ...
  public:
    ...
    TermPtr resolve(const Scope &scope, int recursionDepth) override {
      ...
      return scope.getSymbolValue(symbol).term->resolve(scope,
                                                        recursionDepth + 1);
    }
    ...
    void visitAllSymbols(SymbolVisitor &visitor, const Scope &scope,
                         int recursionDepth) override {
      checkRecursionDepth(recursionDepth);
      visitor.useSymbol(Symbol(scope.getScopeUID(), symbol));
      scope.getSymbolValue(symbol).term->visitAllSymbols(visitor, scope,
                                                         recursionDepth + 1);
    }

    void renameSymbol(const Symbol &oldSymbol, const String &newName,
                      const Scope &scope, int /*recursionDepth*/) override {
      if (oldSymbol.symbolName == symbol &&
          scope.getScopeUID() == oldSymbol.scopeUID)
        symbol = newName;
    }
  ...
  public:
    ...
    Term *getInput(int i) const override {
    ...
    TermPtr resolve(const Scope &scope, int recursionDepth) override {
      ...
      if (numParams > 0) {
        ...
        for (int i = 0; i < numParams; ++i)
          params[i] = parameters.getReference(i)
                          .term->resolve(scope, recursionDepth + 1)
                          ->toDouble();
  ...
  public:
    ...
    void renameSymbol(const Symbol &oldSymbol, const String &newName,
      ...
      SymbolRenamingVisitor visitor(right, oldSymbol, newName,
                                    recursionDepth + 1);
  ...
  private:
    ...
    public:
      SymbolVisitingVisitor(const TermPtr &t, SymbolVisitor &v,
                            const int recursion)
    ...
    public:
      SymbolRenamingVisitor(const TermPtr &t, const Expression::Symbol &symbol_,
                            const String &newName_, const int recursionCount_)
          : input(t), symbol(symbol_), newName(newName_),
  ...
  public:
    explicit Negate(const TermPtr &t) : input(t) { jassert(t != nullptr); }
    ...
    TermPtr resolve(const Scope &scope, int recursionDepth) override {
      return *new Constant(-input->resolve(scope, recursionDepth)->toDouble(),
                           false);
    }
    ...
    TermPtr createTermToEvaluateInput(const Scope &scope,
                                      [[maybe_unused]] const Term *t,
                                      double overallTarget,
                                      Term *topLevelTerm) const override {
      jassert(t == input);

      const Term *const dest = findDestinationFor(topLevelTerm, this);

      return *new Negate(dest == nullptr
                             ? TermPtr(*new Constant(overallTarget, false))
                             : dest->createTermToEvaluateInput(
                                   scope, this, overallTarget, topLevelTerm));
    }
  ...
  public:
    ...
    double performFunction(double lhs, double rhs) const override {
      return lhs + rhs;
    }
    ...
    TermPtr createTermToEvaluateInput(const Scope &scope, const Term *input,
                                      double overallTarget,
                                      Term *topLevelTerm) const override {
      if (auto newDest =
              createDestinationTerm(scope, input, overallTarget, topLevelTerm))
  ...
  public:
    ...
    TermPtr createTermToEvaluateInput(const Scope &scope, const Term *input,
                                      double overallTarget,
                                      Term *topLevelTerm) const override {
      if (auto newDest = createDestinationTerm(scope, input, overallTarget,
                                               topLevelTerm)) {
        if (input == left)
  ...
  //==============================================================================
  static Term *findDestinationFor(Term *const topLevel,
  ...
  static Constant *findTermToAdjust(Term *const term,
  ...
  private:
    ...
    Term *readNumber() noexcept {
      ...
      if (isDecimalDigit(*t) || (*t == '.' && isDecimalDigit(t[1])))
        return new Constant(CharacterFunctions::readDoubleValue(text),
                            isResolutionTarget);
    ...
    TermPtr readSymbolOrFunction() {
      ...
      if (readIdentifier(identifier)) {
        if (readOperator("(")) // method call...
        {
          ...
          std::unique_ptr<Term> func(
              f); // (can't use std::unique_ptr<Function> in MSVC)
...
//==============================================================================
Expression::Expression() : term(new Expression::Helpers::Constant(0, false)) {}

Expression::~Expression() {}

Expression::Expression(Term *t) : term(t) { jassert(term != nullptr); }

Expression::Expression(const double constant)
    : term(new Expression::Helpers::Constant(constant, false)) {}

Expression::Expression(const Expression &other) : term(other.term) {}
...
double Expression::evaluate() const { return evaluate(Expression::Scope()); }
...
Expression::adjustedToGiveNewResult(const double targetValue,
                                    const Expression::Scope &scope) const {
  ...
  if (termToAdjust == nullptr) {
    newTerm.reset(
        new Helpers::Add(*newTerm.release(), *new Helpers::Constant(0, false)));
    termToAdjust = Helpers::findTermToAdjust(newTerm.get(), false);
  }

  jassert(termToAdjust != nullptr);

  if (const Term *parent =
          Helpers::findDestinationFor(newTerm.get(), termToAdjust)) {
    if (Helpers::TermPtr reverseTerm = parent->createTermToEvaluateInput(
            scope, termToAdjust, targetValue, newTerm.get()))
...
Expression Expression::withRenamedSymbol(const Expression::Symbol &oldSymbol,
                                         const String &newName,
                                         const Scope &scope) const {
  jassert(newName.toLowerCase().containsOnly(
...
bool Expression::referencesSymbol(const Expression::Symbol &symbolToCheck,
                                  const Scope &scope) const {
...
String Expression::Scope::getScopeUID() const { return {}; }
