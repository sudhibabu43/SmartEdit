...
template <typename T> class ApproximatelyEqualTests final : public UnitTest {
public:
  ApproximatelyEqualTests()
      : UnitTest{getTemplatedMathsFunctionUnitTestName<T>("approximatelyEqual"),
                 UnitTestCategories::maths} {}
  ...
  void runTest() final {
    ...
    beginTest("Comparing normal values greater than the minimum to zero, "
              "returns true");
    {
    ...
    beginTest("Tolerances scale with the values being compared");
    {
      ...
      expect(!approximatelyEqual((T)100.01, (T)100.011));

      expect(!approximatelyEqual((T)123'000, (T)121'000,
                                 relativeTolerance((T)1e-2)));
      expect(approximatelyEqual((T)123'000, (T)122'000,
                                relativeTolerance((T)1e-2)));
      expect(approximatelyEqual((T)123'000, (T)123'000,
                                relativeTolerance((T)1e-2)));
      expect(approximatelyEqual((T)123'000, (T)124'000,
                                relativeTolerance((T)1e-2)));
      expect(!approximatelyEqual((T)123'000, (T)125'000,
                                 relativeTolerance((T)1e-2)));
      ...
      expect(
          !approximatelyEqual((T)0.123, (T)0.121, relativeTolerance((T)1e-2)));
      expect(
          approximatelyEqual((T)0.123, (T)0.122, relativeTolerance((T)1e-2)));
      expect(
          approximatelyEqual((T)0.123, (T)0.123, relativeTolerance((T)1e-2)));
      expect(
          approximatelyEqual((T)0.123, (T)0.124, relativeTolerance((T)1e-2)));
      expect(
          !approximatelyEqual((T)0.123, (T)0.125, relativeTolerance((T)1e-2)));

      expect(!approximatelyEqual((T)0.000123, (T)0.000121,
                                 relativeTolerance((T)1e-2)));
      expect(approximatelyEqual((T)0.000123, (T)0.000122,
                                relativeTolerance((T)1e-2)));
      expect(approximatelyEqual((T)0.000123, (T)0.000123,
                                relativeTolerance((T)1e-2)));
      expect(approximatelyEqual((T)0.000123, (T)0.000124,
                                relativeTolerance((T)1e-2)));
      expect(!approximatelyEqual((T)0.000123, (T)0.000125,
                                 relativeTolerance((T)1e-2)));
    }
    ...
    beginTest("Can set an absolute tolerance");
    {
      constexpr std::array<T, 7> negativePowersOfTwo{
          (T)0.5 /* 2^-1 */,    (T)0.25 /* 2^-2 */,    (T)0.125 /* 2^-3 */,
          (T)0.0625 /* 2^-4 */, (T)0.03125 /* 2^-5 */, (T)0.015625 /* 2^-6 */,
          (T)0.0078125 /* 2^-7 */
      };
    ...
    {
      expect(
          !approximatelyEqual(oneThird, (T)0.34, relativeTolerance((T)1e-2)));
      expect(
          approximatelyEqual(oneThird, (T)0.334, relativeTolerance((T)1e-2)));

      expect(
          !approximatelyEqual(oneThird, (T)0.334, relativeTolerance((T)1e-3)));
      expect(
          approximatelyEqual(oneThird, (T)0.3334, relativeTolerance((T)1e-3)));

      expect(
          !approximatelyEqual(oneThird, (T)0.3334, relativeTolerance((T)1e-4)));
      expect(
          approximatelyEqual(oneThird, (T)0.33334, relativeTolerance((T)1e-4)));

      expect(!approximatelyEqual(oneThird, (T)0.33334,
                                 relativeTolerance((T)1e-5)));
      expect(approximatelyEqual(oneThird, (T)0.333334,
                                relativeTolerance((T)1e-5)));

      expect(!approximatelyEqual(oneThird, (T)0.333334,
                                 relativeTolerance((T)1e-6)));
      expect(approximatelyEqual(oneThird, (T)0.3333334,
                                relativeTolerance((T)1e-6)));

      expect(!approximatelyEqual(oneThird, (T)0.3333334,
                                 relativeTolerance((T)1e-7)));
      expect(approximatelyEqual(oneThird, (T)0.33333334,
                                relativeTolerance((T)1e-7)));

      expect(approximatelyEqual((T)1e6, (T)1e6 + (T)1,
                                relativeTolerance((T)1e-6)));
      expect(!approximatelyEqual((T)1e6, (T)1e6 + (T)1,
                                 relativeTolerance((T)1e-7)));

      expect(approximatelyEqual((T)-1e-6, (T)-1.0000009e-6,
                                relativeTolerance((T)1e-6)));
      expect(!approximatelyEqual((T)-1e-6, (T)-1.0000009e-6,
                                 relativeTolerance((T)1e-7)));
    ...
    {
      expect(
          approximatelyEqual((T)9, (T)10, absoluteTolerance((T)10.0 * (T)0.1)));
      expect(
          !approximatelyEqual((T)9, (T)10, absoluteTolerance((T)9.0 * (T)0.1)));
...
template <> class ApproximatelyEqualTests<int> final : public UnitTest {
public:
  ApproximatelyEqualTests()
      : UnitTest{
            getTemplatedMathsFunctionUnitTestName<int>("approximatelyEqual"),
            UnitTestCategories::maths} {}
...
template <typename T> class IsFiniteTests final : public UnitTest {
public:
  IsFiniteTests()
      : UnitTest{getTemplatedMathsFunctionUnitTestName<T>("juce_isfinite"),
                 UnitTestCategories::maths} {}
...
template <typename T> class NextFloatTests final : public UnitTest {
public:
  NextFloatTests()
      : UnitTest{getTemplatedMathsFunctionUnitTestName<T>("nextFloat"),
                 UnitTestCategories::maths} {}
