...
BigInteger::BigInteger(const uint32 value)
    : allocatedSize(numPreallocatedInts), highestBit(31) {
...
BigInteger::BigInteger(const BigInteger &other)
    : allocatedSize(other.allocatedSize), highestBit(other.getHighestBit()),
...
BigInteger::BigInteger(BigInteger &&other) noexcept
    : heapAllocation(std::move(other.heapAllocation)),
      allocatedSize(other.allocatedSize), highestBit(other.highestBit),
      negative(other.negative) {
...
uint32 *BigInteger::ensureSize(const size_t numVals) {
  if (numVals > allocatedSize) {
    ...
    if (heapAllocation == nullptr) {
      ...
      memcpy(heapAllocation, preallocated,
...
uint32 BigInteger::getBitRangeAsInt(const int startBit,
                                    int numBits) const noexcept {
  if (numBits > 32) {
...
BigInteger &BigInteger::setBitRangeAsInt(const int startBit, int numBits,
                                         uint32 valueToSet) {
  if (numBits > 32) {
    jassertfalse;
    numBits = 32;
...
BigInteger &BigInteger::setRange(int startBit, int numBits,
                                 const bool shouldBeSet) {
  while (--numBits >= 0)
...
BigInteger &BigInteger::operator-=(const BigInteger &other) {
  ...
  for (size_t i = 0; i < numInts; ++i) {
    ...
    if (values[i] >= amountToSubtract) {
      ...
      const int64 n =
          ((int64)values[i] + (((int64)1) << 32)) - amountToSubtract;
      values[i] = (uint32)n;
      amountToSubtract = 1;
    }
  }
...
BigInteger &BigInteger::operator*=(const BigInteger &other) {
  ...
  for (int i = 0; i <= t; ++i) {
    ...
    for (int j = 0; j <= n; ++j) {
      auto uv = (uint64)totalValues[i + j] +
                (uint64)values[j] * (uint64)mValues[i] + (uint64)c;
...
void BigInteger::divideBy(const BigInteger &divisor, BigInteger &remainder) {
  ...
  jassert(this !=
          &remainder); // (can't handle passing itself in to get the remainder)
...
String BigInteger::toString(const int base,
                            const int minimumNumCharacters) const {
  String s;
  auto v = *this;
...
//==============================================================================
void writeLittleEndianBitsInBuffer(void *buffer, uint32 startBit,
  ...
  if (const uint32 offset = (startBit & 7)) {
    ...
    if (bitsInByte >= numBits) {
      *data = (uint8)((current & ~(((1u << numBits) - 1u) << offset)) |
                      (value << offset));
      return;
    }

    *data++ = current ^ (uint8)(((value << offset) ^ current) &
                                (((1u << bitsInByte) - 1u) << offset));
    numBits -= bitsInByte;
    value >>= bitsInByte;
  }
...
uint32 readLittleEndianBitsInBuffer(const void *buffer, uint32 startBit,
...
public:
  BigIntegerTests() : UnitTest("BigInteger", UnitTestCategories::maths) {}
  ...
  void runTest() override {
    {
      ...
      for (int j = 10000; --j >= 0;) {
        ...
        // TODO: should add tests for other ops (although they also get pretty
        // well tested in the RSA unit test)
