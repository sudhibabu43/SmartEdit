...
/**
    ...
    @tags{Core}
*/
template <typename ValueType> class NormalisableRange {
public:
  ...
  /** Creates a NormalisableRange with a given range, continuous interval, but a
   * dummy skew-factor. */
  ...
  /** Creates a NormalisableRange with a given range and interval, but a dummy
   * skew-factor. */
  NormalisableRange(ValueType rangeStart, ValueType rangeEnd,
  ...
  /** A function object which can remap a value in some way based on the start
   * and end of a range. */
  using ValueRemapFunction = std::function<ValueType(
      ValueType rangeStart, ValueType rangeEnd, ValueType valueToRemap)>;

  /** Creates a NormalisableRange with a given range and an injective mapping
     function.

      @param rangeStart           The minimum value in the range.
      @param rangeEnd             The maximum value in the range.
      @param convertFrom0To1Func  A function which uses the current start and
     end of this NormalisableRange and produces a mapped value from a normalised
     value.
      @param convertTo0To1Func    A function which uses the current start and
     end of this NormalisableRange and produces a normalised value from a mapped
     value.
      @param snapToLegalValueFunc A function which uses the current start and
     end of this NormalisableRange to take a mapped value and snap it to the
     nearest legal value.
  */
  NormalisableRange(ValueType rangeStart, ValueType rangeEnd,
  ...
  */
  ValueType convertTo0to1(ValueType v) const noexcept {
    ...
    auto distanceFromMiddle =
        static_cast<ValueType>(2) * proportion - static_cast<ValueType>(1);

    return (static_cast<ValueType>(1) +
            std::pow(std::abs(distanceFromMiddle), skew) *
                (distanceFromMiddle < ValueType()
  ...
  */
  ValueType convertFrom0to1(ValueType proportion) const noexcept {
    ...
    if (!exactlyEqual(skew, static_cast<ValueType>(1)) &&
        !exactlyEqual(distanceFromMiddle, static_cast<ValueType>(0)))
      distanceFromMiddle =
          std::exp(std::log(std::abs(distanceFromMiddle)) / skew) *
          (distanceFromMiddle < ValueType() ? static_cast<ValueType>(-1)
                                            : static_cast<ValueType>(1));

    return start + (end - start) / static_cast<ValueType>(2) *
                       (static_cast<ValueType>(1) + distanceFromMiddle);
  }

  /** Takes a non-normalised value and snaps it based on either the interval
     property of this NormalisableRange or the lambda function supplied to the
     constructor.
  */
  ValueType snapToLegalValue(ValueType v) const noexcept {
    ...
    if (interval > ValueType())
      v = start + interval * std::floor((v - start) / interval +
  ...
  /** Given a value which is between the start and end points, this sets the
     skew such that convertFrom0to1 (0.5) will return this value.

      If you have used lambda functions for convertFrom0to1Func and
     convertFrom0to1Func in the constructor of this class then the skew value is
     ignored.

      @param centrePointValue  this must be greater than the start of the range
  ...
  */
  ...
  /** The snapping interval that should be used (for a non-normalised value).
     Use 0 for a continuous range.
      ...
      If you have used a lambda function for snapToLegalValueFunction in the
     constructor of this class then the interval is ignored.
  */
  ValueType interval = 0;

  /** An optional skew factor that alters the way values are distribute across
     the range.

      The skew factor lets you skew the mapping logarithmically so that larger
     or smaller values are given a larger proportion of the available space.

      A factor of 1.0 has no skewing effect at all. If the factor is < 1.0, the
     lower end of the range will fill more of the slider's length; if the factor
     is > 1.0, the upper end of the range will be expanded.
  ...
  */
  ...
  /** If true, the skew factor applies from the middle of the slider to each of
   * its ends. */
...
private:
  ...
  static ValueType clampTo0To1(ValueType value) {
    ...
    // If you hit this assertion then either your normalisation function is not
    // working correctly or your input is out of the expected bounds.
    jassert(exactlyEqual(clampedValue, value));

    return clampedValue;
  }

  ValueRemapFunction convertFrom0To1Function, convertTo0To1Function,
