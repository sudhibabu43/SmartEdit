...
//==============================================================================
/**
    A simple class to generate hash functions for some primitive types, intended
   for use with the HashMap class.
    @see HashMap
...
/**
    ...
    You can also specify a class to supply a hash function that converts a key
   value into an hashed integer. This class must have the form:
    ...
    Like the Array class, the key and value types are expected to be
   copy-by-value types, so if you define them to be pointer types, this class
   won't delete the objects that they point to.
    ...
    @tparam HashFunctionType The class of hash function, which must be
   copy-constructible.
...
public:
  ...
  /** Creates an empty hash-map.
      ...
      @param numberOfSlots Specifies the number of hash entries the map will
     use. This will be the "upperLimit" parameter that is passed to your
     generateHash() function. The number of hash slots will grow automatically
     if necessary, or it can be remapped manually using remapTable().
      @param hashFunction An instance of HashFunctionType, which will be copied
     and stored to use with the HashMap. This parameter can be omitted if
     HashFunctionType has a default constructor.
  */
  ...
  /** Removes all values from the map.
      Note that this will clear the content, but won't affect the number of
     slots (see remapTable and getNumSlots).
  */
  void clear() {
  ...
  inline int size() const noexcept { return totalNumItems; }

  /** Returns the value corresponding to a given key.
      If the map doesn't contain the key, a default instance of the value type
     is returned.
  ...
  */
  ...
  /** Returns a reference to the value corresponding to a given key.
      If the map doesn't contain the key, a default instance of the value type
     is added to the map and a reference to this is returned.
      @param keyToLookFor    the key of the item being requested
  */
  ...
  bool containsValue(ValueTypeParameter valueToLookFor) const {
    ...
    for (auto i = getNumSlots(); --i >= 0;)
      for (auto *entry = hashSlots.getUnchecked(i); entry != nullptr;
  ...
  */
  ...
  /** Remaps the hash-map to use a different number of slots for its hash
     function. Each slot corresponds to a single hash-code, and each one can
  ...
  */
  ...
  /** Returns the number of slots which are available for hashing.
      Each slot corresponds to a single hash-code, and each one can contain
     multiple items.
      @see getNumSlots()
  */
  inline int getNumSlots() const noexcept { return hashSlots.size(); }
  ...
  void swapWith(OtherHashMapType &otherHashMap) noexcept {
    ...
    const typename OtherHashMapType::ScopedLockType lock2(
        otherHashMap.getLock());
...
public:
  ...
  /** Iterates over the items in a HashMap.
      ...
      The order in which items are iterated bears no resemblance to the order in
     which they were originally added!

      Obviously as soon as you call any non-const methods on the original
     hash-map, any iterators that were created beforehand will cease to be
  ...
  */
  struct Iterator {
    ...
    /** Moves to the next item, if one is available.
        When this returns true, you can get the item's key and value using
       getKey() and getValue(). If it returns false, the iteration has finished
       and you should stop.
    */
    bool next() noexcept {
      if (entry != nullptr)
    ...
    */
    KeyType getKey() const { return entry != nullptr ? entry->key : KeyType(); }
...
private:
  ...
  int generateHashFor(KeyTypeParameter key, int numSlots) const {
    ...
    jassert(isPositiveAndBelow(
        hash,
        numSlots)); // your hash function is generating out-of-range numbers!
    return hash;
  }

  static HashEntry *getEntry(HashEntry *firstEntry,
                             KeyType keyToLookFor) noexcept {
  ...
  inline HashEntry *getSlot(KeyType key) const noexcept {
