...
public:
  ...
  void runTest() override {
    ...
    beginTest("Constructing from a moved optional calls appropriate member "
              "functions");
    {
      auto ptr = makePtr();
    ...
    beginTest("Strong exception safety is maintained when forwarding over "
              "empty object");
    {
      bool threw = false;
      ...
      expect(threw);
      expect(!a.hasValue()); // If construction failed, this object should still
                             // be well-formed but empty
    }

    beginTest("Weak exception safety is maintained when forwarding over "
              "populated object");
    {
      bool threw = false;
      ...
      expect(threw);
      expect(a.hasValue());
      expect(a->value == -100); // If we assign to an extant object, it's up to
                                // that object to provide an exception guarantee
    }
    ...
    beginTest("Exception safety of contained type is maintained when copying "
              "over populated object");
    {
      bool threw = false;
      Optional<ThrowOnCopy> a = ThrowOnCopy();
      a->value = 5;
    ...
    beginTest("An exception thrown during emplace leaves the optional without "
              "a value");
    {
    ...
    beginTest("An exception thrown during a swap leaves both objects in the "
