...
static bool
operator==(const ArrayBaseTestsHelpers::TriviallyCopyableType &tct,
           const ArrayBaseTestsHelpers::NonTriviallyCopyableType &ntct) {
  return tct.getValue() == ntct.getValue();
}

static bool
operator==(const ArrayBaseTestsHelpers::NonTriviallyCopyableType &ntct,
...
public:
  ArrayBaseTests() : UnitTest("ArrayBase", UnitTestCategories::containers) {}
  ...
  void runTest() override {
    ...
    beginTest("shrink capacity");
    {
      ...
      int numElements = 45;
      addData(referenceContainer, copyableContainer, noncopyableContainer,
              numElements);
    ...
    beginTest("add multiple");
    {
      ...
      copyableContainer.add(CopyableType(-4), CopyableType(-5),
                            CopyableType(-6));
      noncopyableContainer.add(NoncopyableType(-4), NoncopyableType(-5),
                               NoncopyableType(-6));
    ...
    beginTest("add array from a pointer");
    {
      ...
      noncopyableContainer.addArray(noncopyableData.data(),
    ...
    beginTest("insert array");
    {
      ...
      copyableContainer.insertArray(0, copyableData.data(),
                                    (int)copyableData.size());
      noncopyableContainer.insertArray(0, noncopyableData.data(),
                                       (int)noncopyableData.size());
      ...
      for (auto it = copyableData.end(); it != copyableData.begin(); --it)
        referenceContainer.insert(referenceContainer.begin() + insertPos,
                                  CopyableType(*(it - 1)));

      copyableContainer.insertArray(insertPos, copyableData.data(),
                                    (int)copyableData.size());
      noncopyableContainer.insertArray(insertPos, noncopyableData.data(),
                                       (int)noncopyableData.size());
...
private:
  ...
  template <typename A, typename B>
  void checkEqual(ArrayBase<A, DummyCriticalSection> &a, std::vector<B> &b) {
    expectEquals((int)a.size(), (int)b.size());
  ...
  void checkEqual(ArrayBase<A, DummyCriticalSection> &a,
                  ArrayBase<B, DummyCriticalSection> &b, std::vector<C> &c) {
    checkEqual(a, b);
    checkEqual(a, c);
    checkEqual(b, c);
  }
