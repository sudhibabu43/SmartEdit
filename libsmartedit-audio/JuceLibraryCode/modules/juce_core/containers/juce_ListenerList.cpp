...
public:
  ...
  public:
    ...
    void removeListener(int i) {
      listenerList.remove(listeners[(size_t)i].get());
    }
    ...
    bool wereAllNonRemovedListenersCalled(int numCalls) const {
      return std::all_of(std::begin(listeners), std::end(listeners),
                         [&](auto &listener) {
                           return (!listenerList.contains(listener.get())) ||
                                  listener->getNumCalls() == numCalls;
                         });
    }
  ...
  //==============================================================================
  ListenerListTests()
      : UnitTest("ListenerList", UnitTestCategories::containers) {}

  void runTest() override {
    // This is a test that the pre-iterator adjustment implementation should
    // pass too
    beginTest("All non-removed listeners should be called - removing an "
    ...
    beginTest("All non-removed listeners should be called - removing a yet "
              "uncalled listener");
    {
      TestObject test;
    ...
    // This test case demonstrates why we have to call --it.index instead of
    // it.next()
    beginTest("All non-removed listeners should be called - one callback "
    ...
    beginTest("All non-removed listeners should be called - removing listeners "
              "randomly");
    {
      ...
      for (auto run = 0; run < 10; ++run) {
        ...
        const auto listenersThatRemoveListeners = chooseUnique(
            random, numListeners, random.nextInt({0, numListeners / 2}));
        ...
        for (auto i : listenersThatRemoveListeners) {
          ...
          removals[i] =
              chooseUnique(random, numListeners,
    ...
    beginTest("All non-removed listeners should be called - add listener "
              "during iteration");
    {
      TestObject test;
      ...
      // Only the Listeners added before the test can be expected to have been
      // called
      bool success = true;
      ...
      // Listeners added during the iteration must not be called in that
      // iteration
    ...
    beginTest("All non-removed listeners should be called - nested "
              "ListenerList::call()");
    {
      TestObject test;
    ...
    beginTest("All non-removed listeners should be called - random "
              "ListenerList::call()");
    {
      ...
      for (int run = 0; run < 10; ++run) {
        ...
        auto listenersToRemove =
            chooseUnique(random, numListeners, numListeners / 2);

        for (int i = 0; i < numListeners; ++i) {
          // Capturing numListeners is a warning on MacOS, not capturing it is
          // an error on Windows
          test.addListener([&] {
            const auto callLevel = test.getCallLevel();
    ...
    beginTest("Using a critical section");
    {
      ...
      auto listeners = std::make_unique<juce::ListenerList<
          Listener, Array<Listener *, TestCriticalSection>>>();
    ...
    beginTest("Adding a listener during a callback when one has already been "
              "removed");
    {
      struct Listener {};
