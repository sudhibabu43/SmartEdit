...
template <typename Fn> void move(void *from, void *to) {
...
template <typename Fn> void clear(void *s) {
  // I know this looks insane, for some reason MSVC 14 sometimes thinks fn is
  // unreferenced
...
template <size_t len, typename T> class FixedSizeFunction;
...
/**
    ...
    This in turn means that creating a FixedSizeFunction instance will never
   allocate, making FixedSizeFunctions suitable for use in realtime contexts.
...
private:
  ...
  template <typename Item> using Decay = std::decay_t<Item>;

  template <typename Item, typename Fn = Decay<Item>>
  using IntIfValidConversion =
      std::enable_if_t<sizeof(Fn) <= len && alignof(Fn) <= alignof(Storage) &&
...
public:
  ...
  /** Create an empty function. */
  FixedSizeFunction(std::nullptr_t) noexcept : FixedSizeFunction() {}

  FixedSizeFunction(const FixedSizeFunction &) = delete;

  /** Forwards the passed Callable into the internal storage buffer. */
  template <typename Callable, typename Fn = Decay<Callable>,
  ...
  FixedSizeFunction(Callable &&callable) {
    ...
    static_assert(alignof(Fn) <= alignof(Storage),
                  "FixedSizeFunction cannot accommodate the requested "
                  "alignment requirements");

    static constexpr auto vtableForCallable =
        detail::makeVtable<Fn, Ret, Args...>();
  ...
  /** Move constructor. */
  FixedSizeFunction(FixedSizeFunction &&other) noexcept : vtable(other.vtable) {
    move(std::move(other));
  }
  ...
  template <size_t otherLen, std::enable_if_t<(otherLen < len), int> = 0>
  FixedSizeFunction &
  operator=(FixedSizeFunction<otherLen, Ret(Args...)> &&other) noexcept {
  ...
  /** If this instance is currently storing a callable object, calls that
     object, otherwise throws `std::bad_function_call`.
  */
...
private:
  template <size_t, typename> friend class FixedSizeFunction;
