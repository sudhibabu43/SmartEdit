...
PropertySet::PropertySet(bool ignoreCaseOfKeyNames)
    : properties(ignoreCaseOfKeyNames), fallbackProperties(nullptr),
...
String PropertySet::getValue(StringRef keyName,
  ...
  return fallbackProperties != nullptr
             ? fallbackProperties->getValue(keyName, defaultValue)
             : defaultValue;
}

int PropertySet::getIntValue(StringRef keyName,
...
double PropertySet::getDoubleValue(StringRef keyName,
...
bool PropertySet::getBoolValue(StringRef keyName,
...
void PropertySet::setFallbackPropertySet(
    PropertySet *fallbackProperties_) noexcept {
  const ScopedLock sl(lock);
  fallbackProperties = fallbackProperties_;
}

std::unique_ptr<XmlElement>
PropertySet::createXml(const String &nodeName) const {
...
void PropertySet::restoreFromXml(const XmlElement &xml) {
  ...
  for (auto *e : xml.getChildWithTagNameIterator("VALUE")) {
    if (e->hasAttribute("name") && e->hasAttribute("val")) {
