...
public:
  ...
  template <typename Head, typename... Tail,
            std::enable_if_t<!IsOptional<std::decay_t<Head>>::value, int> = 0>
  Optional(Head &&head, Tail &&...tail) noexcept(
      std::is_nothrow_constructible_v<std::optional<Value>, Head, Tail...>)
      : opt(std::forward<Head>(head), std::forward<Tail>(tail)...) {}

  template <typename Other>
  Optional(const Optional<Other> &other) noexcept(
      std::is_nothrow_constructible_v<std::optional<Value>,
  ...
  template <typename Other,
            std::enable_if_t<!IsOptional<std::decay_t<Other>>::value, int> = 0>
  Optional &operator=(Other &&other) noexcept(
      std::is_nothrow_assignable_v<std::optional<Value>, Other>) {
    opt = std::forward<Other>(other);
    return *this;
  }

  template <typename Other>
  Optional &operator=(const Optional<Other> &other) noexcept(
      std::is_nothrow_assignable_v<std::optional<Value>,
                                   const std::optional<Other> &>) {
    opt = other.opt;
    return *this;
  }
  ...
  template <typename... Other> auto &emplace(Other &&...other) {
  ...
  void swap(Optional &other) noexcept(
      std::is_nothrow_swappable_v<std::optional<Value>>) {
    opt.swap(other.opt);
  }
  ...
  template <typename U> decltype(auto) orFallback(U &&fallback) const & {
    return opt.value_or(std::forward<U>(fallback));
  }

  template <typename U> decltype(auto) orFallback(U &&fallback) & {
    return opt.value_or(std::forward<U>(fallback));
  }
...
private:
  template <typename Other> friend class Optional;
