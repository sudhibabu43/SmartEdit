...
NamedValueSet::NamedValue::NamedValue(NamedValue &&other) noexcept
    : NamedValue(std::move(other.name), std::move(other.value)) {}
...
NamedValueSet::NamedValue::operator=(NamedValue &&other) noexcept {
  name = std::move(other.name);
  value = std::move(other.value);
  return *this;
}

bool NamedValueSet::NamedValue::operator==(
    const NamedValue &other) const noexcept {
...
void NamedValueSet::clear() { values.clear(); }
...
void NamedValueSet::setFromXmlAttributes(const XmlElement &xml) {
  ...
  for (auto *att = xml.attributes.get(); att != nullptr;
...
void NamedValueSet::copyToXmlAttributes(XmlElement &xml) const {
  for (auto &i : values) {
    if (auto *mb = i.value.getBinaryData()) {
      ...
      xml.setAttribute(i.name.toString(), i.value.toString());
    }
  }
}
