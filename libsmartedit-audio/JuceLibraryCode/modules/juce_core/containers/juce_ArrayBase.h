...
private:
  ...
  using AllowConversion = std::enable_if_t<
      !std::is_same_v<std::tuple<ElementType, TypeOfCriticalSectionToUse>,
...
public:
  ...
  ArrayBase(ArrayBase &&other) noexcept
      : elements(std::move(other.elements)), numAllocated(other.numAllocated),
  ...
  /** Converting move constructor.
      ...
      If you see a compile error here, it's probably because you're attempting a
     conversion that HeapBlock won't allow.
  */
  template <class OtherElementType, class OtherCriticalSection,
  ...
  */
  template <class OtherElementType, class OtherCriticalSection,
            typename = AllowConversion<OtherElementType, OtherCriticalSection>>
  ArrayBase &operator=(
      ArrayBase<OtherElementType, OtherCriticalSection> &&other) noexcept {
    // No need to worry about assignment to *this, because 'other' must be of a
    // different type.
  ...
  void add(ElementType &&firstNewElement, OtherElements &&...otherElements) {
    addImpl(std::move(firstNewElement),
  ...
  addArray(const OtherArrayType &arrayToAddFrom, int startIndex,
    ...
    if (numElementsToAdd < 0 ||
        startIndex + numElementsToAdd > (int)arrayToAddFrom.size())
...
private:
...
#else
  static constexpr auto isTriviallyCopyable =
  ...
  void addArrayInternal(const Type *otherElements, int numElements) {
    if constexpr (isTriviallyCopyable && std::is_same_v<Type, ElementType>) {
      if (numElements > 0)
        memcpy(elements + numUsed, otherElements,
  ...
  void createInsertSpaceInternal(int indexToInsertAt, int numElements) {
    if constexpr (isTriviallyCopyable) {
      ...
      memmove(start + numElements, start,
  ...
  //==============================================================================
  template <typename... Elements> void addImpl(Elements &&...toAdd) {
  ...
  void checkSourceIsNotAMember([[maybe_unused]] const ElementType &element) {
    // when you pass a reference to an existing element into a method like add()
    // which may need to reallocate the array to make more space, the incoming
    // reference may be deleted indirectly during the reallocation operation! To
    // work around this, make a local copy of the item you're trying to add (and
    // maybe use std::move to move it into the add() method to avoid any extra
    // overhead)
    jassert(std::addressof(element) < begin() ||
            end() <= std::addressof(element));
  }
