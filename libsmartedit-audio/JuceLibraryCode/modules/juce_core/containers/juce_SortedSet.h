...
/**
    ...
    kind of primitive type is used must support the ==, <, >, <= and >=
   operators to determine the order), and searching the set for known values is
   very fast because it uses a binary-chop method.

    Note that if you're using a class or struct as the element type, it must be
    capable of being copied or moved with a straightforward memcpy, rather than
    needing construction and destruction code.

    To make all the set's methods thread-safe, pass in "CriticalSection" as the
   templated TypeOfCriticalSectionToUse parameter, instead of the default
...
public:
  ...
  */
  ...
  /** Removes all elements from the set without freeing the array's allocated
     storage.
      @see clear
  */
  void clearQuick() noexcept { data.clearQuick(); }
  ...
  */
  ...
  /** Returns one of the elements in the set, without checking the index passed
     in. Unlike the operator[] method, this will try to return an element
     without checking that the index is within the bounds of the set, so should
     only be used when you're confident that it will always be a valid index.
  ...
  */
  ...
  /** Returns a direct reference to one of the elements in the set, without
  ...
  */
  inline ElementType getFirst() const noexcept { return data.getFirst(); }

  /** Returns the last element in the set, or 0 if the set is empty.
      @see operator[], getUnchecked, getFirst
  */
  inline ElementType getLast() const noexcept { return data.getLast(); }
  ...
  /** Returns a pointer to the first element in the set.
     ...
     mechanisms.
  */
  inline const ElementType *begin() const noexcept { return data.begin(); }

  /** Returns a pointer to the element which follows the last element in the
     set. This method is provided for compatibility with standard C++ iteration
     mechanisms.
  */
  inline const ElementType *end() const noexcept { return data.end(); }
  ...
  */
  ...
  /** Adds a new element to the set, (as long as it's not already in there).
      ...
      Note that if a matching element already exists, the new value will be
     assigned to the existing one using operator=, so that if there are any
     differences between the objects which were not recognised by the object's
     operator==, then the set will always contain a copy of the most recently
     added one.

      @param newElement   the new object to add to the set
      @returns            true if the value was added, or false if it already
     existed
  ...
  */
  bool add(const ElementType &newElement) noexcept {
    ...
    while (s < e) {
      ...
      if (newElement == elem) {
        elem = newElement; // force an update in case operator== permits
                           // differences.
        return false;
      }
  ...
  */
  ...
  /** Adds elements from another set to this one.
      ...
      @param startIndex           the first element of the other set to start
     copying from
      @param numElementsToAdd     how many elements to add from the other set.
     If this value is negative or greater than the number of available elements,
                                  all available elements will be copied.
      @see add
  */
  template <class OtherSetType>
  void addSet(const OtherSetType &setToAddFrom, int startIndex = 0,
    ...
    if (this != &setToAddFrom) {
      ...
      if (numElementsToAdd < 0 ||
          startIndex + numElementsToAdd > setToAddFrom.size())
  ...
  */
  template <class OtherSetType> void swapWith(OtherSetType &otherSet) noexcept {
  ...
  */
  void minimiseStorageOverheads() noexcept { data.minimiseStorageOverheads(); }
