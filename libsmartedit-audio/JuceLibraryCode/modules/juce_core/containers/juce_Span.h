...
namespace detail {
//==============================================================================
template <typename, typename = void> constexpr auto hasToAddress = false;

template <typename T>
constexpr auto hasToAddress<
    T, Void<decltype(std::pointer_traits<T>::to_address(std::declval<T>()))>> =
    true;

template <typename, typename = void> constexpr auto hasDataAndSize = false;

template <typename T>
constexpr auto hasDataAndSize<T, Void<decltype(std::data(std::declval<T>())),
                                      decltype(std::size(std::declval<T>()))>> =
    true;

template <size_t Extent> struct NumBase {
...
template <> struct NumBase<dynamicExtent> {
  constexpr NumBase() = default;

  constexpr explicit NumBase(size_t arg) : num(arg) {}
...
template <typename T> constexpr T *toAddress(T *p) { return p; }

template <typename It> constexpr auto toAddress(const It &it) {
...
public:
  ...
  template <size_t e = extent,
            std::enable_if_t<e == 0 || e == dynamicExtent, int> = 0>
  constexpr Span() {}

  template <typename It>
  constexpr Span(It it, size_t end) : Base(end), ptr(detail::toAddress(it)) {}

  template <typename Range,
            std::enable_if_t<detail::hasDataAndSize<Range>, int> = 0>
...
Span(T, End) -> Span<
    std::remove_pointer_t<decltype(detail::toAddress(std::declval<T>()))>>;
