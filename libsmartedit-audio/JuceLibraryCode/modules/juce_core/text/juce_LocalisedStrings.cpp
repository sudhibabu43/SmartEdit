...
// before the currentMappings object, we can force the static
// order-of-destruction to delete the currentMappings object first, which avoids
// a bogus leak warning. (Oddly, just creating a LocalisedStrings on the stack
// doesn't work in gcc, it has to be created with 'new' for this to work..)
struct LeakAvoidanceTrick {
  LeakAvoidanceTrick() {
    const std::unique_ptr<LocalisedStrings> dummy(
        new LocalisedStrings(String(), false));
  }
};
  ...
  for (auto &l : lines) {
    ...
    if (line.startsWithChar('"')) {
      ...
      if (originalText.isNotEmpty()) {
        ...
        auto newText =
            unescapeString(line.substring(openingQuote + 1, closeQuote));
...
void LocalisedStrings::setFallback(LocalisedStrings *f) { fallback.reset(f); }
