...
StringArray::StringArray(
    const std::initializer_list<const char *> &stringList) {
...
bool StringArray::addIfNotAlreadyThere(const String &newString,
                                       bool ignoreCase) {
...
void StringArray::addArray(const StringArray &otherArray, int startIndex,
...
int StringArray::indexOf(StringRef stringToLookFor, bool ignoreCase,
                         int i) const {
  if (i < 0)
    i = 0;
...
//==============================================================================
String StringArray::joinIntoString(StringRef separator, int start,
                                   int numberToJoin) const {
  ...
  auto separatorBytes =
      separator.text.sizeInBytes() - sizeof(String::CharPointerType::CharType);
...
int StringArray::addTokens(StringRef text, StringRef breakCharacters,
                           StringRef quoteCharacters) {
  int num = 0;

  if (text.isNotEmpty()) {
    for (auto t = text.text;;) {
      auto tokenEnd = CharacterFunctions::findEndOfToken(
          t, breakCharacters.text, quoteCharacters.text);
...
void StringArray::appendNumbersToDuplicates(bool ignoreCase,
  ...
  for (int i = 0; i < size() - 1; ++i) {
    ...
    if (nextIndex >= 0) {
      ...
      while (nextIndex >= 0) {
        set(nextIndex, (*this)[nextIndex] + String(preNumberString) +
                           String(++number) + String(postNumberString));
