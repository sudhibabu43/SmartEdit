...
struct TextDiffHelpers {
  ...
  static void addInsertion(TextDiff &td, String::CharPointerType text,
  ...
  static void diffSkippingCommonStart(TextDiff &td, StringRegion a,
                                      StringRegion b) {
    for (;;) {
      auto ca = *a.text;
      auto cb = *b.text;
  ...
  static void diffRecursively(TextDiff &td, StringRegion a, StringRegion b) {
    ...
    auto len = findLongestCommonSubstring(a.text, a.length, indexA, b.text,
                                          b.length, indexB);
    ...
    if (len >= minLengthToMatch) {
      ...
      diffRecursively(
          td,
          StringRegion(a.text + (indexA + len), a.start + indexA + len,
                       a.length - indexA - len),
          StringRegion(b.text + (indexB + len), b.start + indexB + len,
                       b.length - indexB - len));
    } else {
      if (a.length > 0)
  ...
  static int findLongestCommonSubstring(String::CharPointerType a,
                                        const int lenA, int &indexInA,
                                        String::CharPointerType b,
                                        const int lenB,
                                        int &indexInB) noexcept {
    if (lenA == 0 || lenB == 0)
      return 0;
    ...
    if (scratchSpace < 4096) {
      ...
      return findLongestCommonSubstring(a, lenA, indexInA, b, lenB, indexInB,
  ...
  static int findCommonSuffix(String::CharPointerType a, int lenA,
                              int &indexInA, String::CharPointerType b,
                              int lenB, int &indexInB) noexcept {
    int length = 0;
    a += lenA - 1;
    b += lenB - 1;
...
public:
  DiffTests() : UnitTest("TextDiff class", UnitTestCategories::text) {}
