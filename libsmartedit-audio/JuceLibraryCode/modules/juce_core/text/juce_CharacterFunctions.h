...
#else
/** This macro will be set to 1 if the compiler's native wchar_t is an 8-bit
 * type. */
#define JUCE_NATIVE_WCHAR_IS_UTF8 0
/** This macro will be set to 1 if the compiler's native wchar_t is a 16-bit
 * type. */
#define JUCE_NATIVE_WCHAR_IS_UTF16 0
/** This macro will be set to 1 if the compiler's native wchar_t is a 32-bit
 * type. */
...
/** The 'T' macro is an alternative for using the "L" prefix in front of a
   string literal.

    This macro is deprecated, but available for compatibility with old code if
   you set JUCE_DEFINE_T_MACRO = 1. The fastest, most portable and best way to
   write your string literals is as standard char strings, using escaped utf-8
   character sequences for extended characters, rather than trying to store them
...
/**
    ...
    Most of these methods are designed for internal use by the String and
   CharPointer classes, but some of them may be useful to call directly.
...
public:
  ...
  /** Checks whether a character is a printable character, i.e. alphabetic,
     numeric, a punctuation character or a space.
  */
  ...
  */
  ...
  /** Returns 0 to 16 for '0' to 'F", or -1 for characters that aren't a legal
   * hex digit. */
  static int getHexDigitValue(juce_wchar digit) noexcept;

  /** Converts a byte of Windows 1252 codepage to unicode. */
  static juce_wchar
  getUnicodeCharFromWindows1252Codepage(uint8 windows1252Char) noexcept;
  ...
  */
  ...
  static double readDoubleValue(CharPointerType &text) noexcept {
...
#if !JUCE_MINGW
    constexpr const int maxSignificantDigits =
        17 + 1; // An additional digit for rounding
...
#if JUCE_MINGW
    // MinGW does not have access to the locale functions required for strtold,
    // so we parse the doubles ourselves. There are some edge cases where the
    // least significant digit will be wrong!
    ...
    if (c == 'e' || c == 'E') {
      ...
      *writePtr++ = 'e';
      ...
      exponent =
          extraExponent + (parsedExponentIsPositive ? exponent : -exponent);
  ...
  template <typename ResultType> struct HexParser {
    ...
    static ResultType parse(CharPointerType t) noexcept {
      ...
      while (!t.isEmpty()) {
        ...
        if (hexValue >= 0)
          result = static_cast<ResultType>(result << 4) |
                   static_cast<ResultType>(hexValue);
      }
  ...
  //==============================================================================
  /** Counts the number of characters in a given string, stopping if the count
     exceeds a specified limit. */
  template <typename CharPointerType>
  static size_t lengthUpTo(CharPointerType text,
  ...
  template <typename CharPointerType>
  static size_t lengthUpTo(CharPointerType start,
  ...
  static void copyAll(DestCharPointerType &dest,
                      SrcCharPointerType src) noexcept {
  ...
  template <typename DestCharPointerType, typename SrcCharPointerType>
  static size_t copyWithDestByteLimit(DestCharPointerType &dest,
    ...
    maxBytes -= (ssize_t)sizeof(
        typename DestCharPointerType::CharType); // (allow for a terminating
                                                 // null)
  ...
  template <typename DestCharPointerType, typename SrcCharPointerType>
  static void copyWithCharLimit(DestCharPointerType &dest,
                                SrcCharPointerType src, int maxChars) noexcept {
    while (--maxChars > 0) {
      auto c = src.getAndAdvance();
  ...
  /** Compares two null-terminated character strings, up to a given number of
   * characters. */
  template <typename CharPointerType1, typename CharPointerType2>
  static int compareUpTo(CharPointerType1 s1, CharPointerType2 s2,
  ...
  /** Compares two null-terminated character strings, using a case-independant
   * match. */
  template <typename CharPointerType1, typename CharPointerType2>
  static int compareIgnoreCase(CharPointerType1 s1,
                               CharPointerType2 s2) noexcept {
    for (;;) {
      auto c1 = s1.getAndAdvance();
  ...
  /** Compares two null-terminated character strings, using a case-independent
   * match. */
  template <typename CharPointerType1, typename CharPointerType2>
  ...
  */
  template <typename CharPointerType1, typename CharPointerType2>
  static int indexOfIgnoreCase(CharPointerType1 haystack,
  ...
  */
  template <typename Type> static Type findEndOfWhitespace(Type text) noexcept {
  ...
  */
  ...
  static Type findEndOfToken(Type text, BreakType breakCharacters,
