...
public:
  ...
  juce_wchar operator*() const noexcept {
    ...
    if (n >= 0xd800 && n <= 0xdfff && ((uint32)(uint16)data[1]) >= 0xdc00)
      n = 0x10000 +
          (((n - 0xd800) << 10) | (((uint32)(uint16)data[1]) - 0xdc00));
  ...
  juce_wchar getAndAdvance() noexcept {
    ...
    if (n >= 0xd800 && n <= 0xdfff && ((uint32)(uint16)*data) >= 0xdc00)
      n = 0x10000 +
          ((((n - 0xd800) << 10) | (((uint32)(uint16)*data++) - 0xdc00)));
  ...
  /** Returns the character at a given character index from the start of the
   * string. */
  ...
  /** Returns a pointer which is moved forwards from this one by the specified
  ...
  /** Returns a pointer which is moved backwards from this one by the specified
   * number of characters. */
  CharPointer_UTF16 operator-(int numToSkip) const noexcept {
    auto p(*this);
    p += -numToSkip;
  ...
  /** Writes a unicode character to this string, and advances this pointer to
  ...
  /** Writes a null character to this string (leaving the pointer's position
   * unchanged). */
  ...
  /** Returns the number of characters in this string, or the given value,
   * whichever is lower. */
  size_t lengthUpTo(size_t maxCharsToCount) const noexcept {
    return CharacterFunctions::lengthUpTo(*this, maxCharsToCount);
  }

  /** Returns the number of characters in this string, or up to the given end
   * pointer, whichever is lower. */
  ...
  */
   ...
   */
  template <typename CharPointer> void writeAll(CharPointer src) noexcept {
  ...
  /** Copies a source string to this pointer, advancing this pointer as it goes.
      The maxDestBytes parameter specifies the maximum number of bytes that can
     be written to the destination buffer before stopping.
  */
  ...
  /** Copies a source string to this pointer, advancing this pointer as it goes.
      The maxChars parameter specifies the maximum number of characters that can
     be written to the destination buffer before stopping (including the
     terminating null).
  */
  template <typename CharPointer>
  ...
  /** Compares this string with another one, up to a specified number of
   * characters. */
  template <typename CharPointer>
  ...
  int compareIgnoreCaseUpTo(CharPointer_UTF16 other,
                            int maxChars) const noexcept {
  ...
  /** Returns the character index of a unicode character, or -1 if it isn't
   * found. */
  ...
  /** Returns true if the given unicode character can be represented in this
   * encoding. */
  ...
  /** Atomically swaps this pointer for a new value, returning the previous
   * value. */
  CharPointer_UTF16 atomicSwap(CharPointer_UTF16 newValue) {
    return CharPointer_UTF16(
        reinterpret_cast<Atomic<CharType *> &>(data).exchange(newValue.data));
  }
  ...
  /** Returns true if the first pair of bytes in this pointer are the UTF16
     byte-order mark (big endian). The pointer must not be null, and must
  ...
  */
  ...
  /** Returns true if the first pair of bytes in this pointer are the UTF16
     byte-order mark (little endian). The pointer must not be null, and must
     contain at least two valid bytes.
  */
  static bool
  isByteOrderMarkLittleEndian(const void *possibleByteOrder) noexcept {
    JUCE_BEGIN_IGNORE_WARNINGS_MSVC(28182)
    jassert(possibleByteOrder != nullptr);
    auto c = static_cast<const uint8 *>(possibleByteOrder);
