...
#error                                                                         \
    "JUCE_STRINGS_ARE_UNICODE is deprecated! All strings are now unicode by default."
#endif
...
static CharPointer_wchar_t castToCharPointer_wchar_t(const void *t) noexcept {
  return CharPointer_wchar_t(
      static_cast<const CharPointer_wchar_t::CharType *>(t));
}
...
public:
  ...
  template <class CharPointer>
  static CharPointerType createFromCharPointer(const CharPointer start,
  ...
  static CharPointerType createFromCharPointer(const CharPointerType start,
    ...
    auto numBytes =
        (size_t)(reinterpret_cast<const char *>(end.getAddress()) -
  ...
  static CharPointerType createFromFixedLength(const char *const src,
                                               const size_t numChars) {
    auto dest = createUninitialisedBytes(numChars * sizeof(CharType) +
                                         sizeof(CharType));
    CharPointerType(dest).writeWithCharLimit(CharPointer_UTF8(src),
                                             (int)(numChars + 1));
    return dest;
  }
  ...
  //==============================================================================
  static CharPointerType makeUniqueWithByteSize(const CharPointerType text,
                                                size_t numBytes) {
    auto *b = bufferFromText(text);
    ...
    auto newText =
        createUninitialisedBytes(jmax(b->allocatedNumBytes, numBytes));
...
private:
  ...
  static StringHolder *bufferFromText(const CharPointerType charPtr) noexcept {
    return unalignedPointerCast<StringHolder *>(
        unalignedPointerCast<char *>(charPtr.getAddress()) -
        offsetof(StringHolder, text));
  }
...
void String::preallocateBytes(const size_t numBytesNeeded) {
  text = StringHolderUtils::makeUniqueWithByteSize(
...
String::String(const char *const t)
  ...
  /*  If you get an assertion here, then you're trying to create a string from
     8-bit data that contains values greater than 127. These can NOT be
     correctly converted to unicode because there's no way for the String class
     to know what encoding was used to create them. The source data could be
     UTF-8, ASCII or one of many local code-pages.

      To get around this problem, you must be more explicit when you pass an
     ambiguous 8-bit string to the String class - so for example if your source
     data is actually UTF-8, you'd call String (CharPointer_UTF8 ("my utf8
     string..")), and it would be able to correctly convert the multi-byte
     characters to unicode. It's *highly* recommended that you use UTF-8 with
     escape characters in your source code to represent extended characters,
      because there's no other way to represent these strings in a way that
     isn't dependent on the compiler, source code editor and platform.

      Note that the Projucer has a handy string literal generator utility that
     will convert any unicode string to a valid C++ string literal, creating
     ascii escape sequences that will work in any compiler.
  */
  jassert(t == nullptr ||
          CharPointer_ASCII::isValidString(t, std::numeric_limits<int>::max()));
}
...
String::String(const wchar_t *const t)
    : text(StringHolderUtils::createFromCharPointer(
          castToCharPointer_wchar_t(t))) {}
...
String::String(const wchar_t *t, size_t maxChars)
    : text(StringHolderUtils::createFromCharPointer(
          castToCharPointer_wchar_t(t), maxChars)) {}
...
String String::charToString(juce_wchar character) {
  String result(
      PreallocationBytes(CharPointerType::getBytesRequiredFor(character)));
...
template <typename Type> static char *printDigits(char *t, Type v) noexcept {
  *--t = 0;
...
static char *numberToString(char *t, int64 n) noexcept {
  ...
  // -std::numeric_limits<int64>::min(), which has undefined behaviour
...
static String::CharPointerType createFromInteger(IntegerType number) {
  ...
  return StringHolderUtils::createFromFixedLength(start,
                                                  (size_t)(end - start - 1));
}

static String::CharPointerType createFromDouble(double number,
                                                int numberOfDecimalPlaces,
                                                bool useScientificNotation) {
  char buffer[charsNeededForDouble];
  size_t len;
  auto start = doubleToString(buffer, number, numberOfDecimalPlaces,
                              useScientificNotation, len);
...
String::String(float number)
    : text(NumberToStringConverters::createFromDouble((double)number, 0,
                                                      false)) {}
String::String(double number)
    : text(NumberToStringConverters::createFromDouble(number, 0, false)) {}
String::String(float number, int numberOfDecimalPlaces,
               bool useScientificNotation)
    : text(NumberToStringConverters::createFromDouble(
          (double)number, numberOfDecimalPlaces, useScientificNotation)) {}
String::String(double number, int numberOfDecimalPlaces,
               bool useScientificNotation)
    : text(NumberToStringConverters::createFromDouble(
          number, numberOfDecimalPlaces, useScientificNotation)) {}
...
static size_t findByteOffsetOfEnd(String::CharPointerType text) noexcept {
  return (size_t)(((char *)text.findTerminatingNull().getAddress()) -
...
juce_wchar String::operator[](int index) const noexcept {
  jassert(index == 0 ||
          (index > 0 && index <= (int)text.lengthUpTo((size_t)index + 1)));
  return text[index];
}

template <typename Type> struct HashGenerator {
...
//==============================================================================
JUCE_API bool JUCE_CALLTYPE operator==(const String &s1,
...
bool String::equalsIgnoreCase(const wchar_t *const t) const noexcept {
  return t != nullptr
             ? text.compareIgnoreCase(castToCharPointer_wchar_t(t)) == 0
             : isEmpty();
}
...
static int stringCompareRight(String::CharPointerType s1,
                              String::CharPointerType s2) noexcept {
  for (int bias = 0;;) {
...
static int stringCompareLeft(String::CharPointerType s1,
                             String::CharPointerType s2) noexcept {
  for (;;) {
    auto c1 = s1.getAndAdvance();
    bool isDigit1 = CharacterFunctions::isDigit(c1);
...
static int naturalStringCompare(String::CharPointerType s1,
...
void String::appendCharPointer(const CharPointerType startOfTextToAppend,
  ...
  auto extraBytesNeeded = getAddressDifference(
      endOfTextToAppend.getAddress(), startOfTextToAppend.getAddress());
  ...
  if (extraBytesNeeded > 0) {
    ...
    memcpy(newStringStart, startOfTextToAppend.getAddress(),
           (size_t)extraBytesNeeded);
    CharPointerType(addBytesToPointer(newStringStart, extraBytesNeeded))
        .writeNull();
  }
}
...
String &String::operator+=(const char *t) {
  appendCharPointer(CharPointer_UTF8(
      t)); // (using UTF8 here triggers a faster code-path than ascii)
  return *this;
}
...
inline String &operationAddAssign(String &str, const T number) {
  ...
  str.appendCharPointer(String::CharPointerType(start),
...
#else
  // (This avoids using toUTF8() to prevent the memory bloat that it would leave
  // behind if lots of large, persistent strings were to be written to streams).
...
int String::indexOfIgnoreCase(StringRef other) const noexcept {
  return other.isEmpty()
             ? 0
             : CharacterFunctions::indexOfIgnoreCase(text, other.text);
}
...
bool String::containsWholeWordIgnoreCase(
    StringRef wordToLookFor) const noexcept {
  return indexOfWholeWordIgnoreCase(wordToLookFor) >= 0;
}

//==============================================================================
template <typename CharPointer> struct WildCardMatcher {
  static bool matches(CharPointer wildcard, CharPointer test,
    ...
    for (;;) {
      ...
      if (wc == '*')
        return wildcard.isEmpty() ||
               matchesAnywhere(wildcard, test, ignoreCase);
  ...
  static bool characterMatches(const juce_wchar wc, const juce_wchar tc,
                               const bool ignoreCase) noexcept {
    return (wc == tc) || (wc == '?' && tc != 0) ||
           (ignoreCase && CharacterFunctions::toLowerCase(wc) ==
...
bool String::matchesWildcard(StringRef wildcard,
                             const bool ignoreCase) const noexcept {
  return WildCardMatcher<CharPointerType>::matches(wildcard.text, text,
                                                   ignoreCase);
}

//==============================================================================
String String::repeatedString(StringRef stringToRepeat,
...
String String::paddedLeft(const juce_wchar padCharacter,
  ...
  auto currentByteSize =
      (size_t)(((char *)end.getAddress()) - (char *)text.getAddress());
...
String String::paddedRight(const juce_wchar padCharacter,
                           int minimumLength) const {
  jassert(padCharacter != 0);
...
//==============================================================================
String String::replaceSection(int index, int numCharsToReplace,
  ...
  auto initialBytes =
      (size_t)(((char *)insertPoint.getAddress()) - (char *)text.getAddress());
  auto newStringBytes = findByteOffsetOfEnd(stringToInsert);
  auto remainderBytes =
      (size_t)(((char *)startOfRemainder.findTerminatingNull().getAddress()) -
  ...
  dest += remainderBytes;
  CharPointerType(unalignedPointerCast<CharPointerType::CharType *>(dest))
      .writeNull();
...
struct StringCreationHelper {
  ...
  void write(juce_wchar c) {
    ...
    if (bytesWritten > allocatedBytes) {
      ...
      result.preallocateBytes(allocatedBytes);
      dest = addBytesToPointer(result.getCharPointer().getAddress(),
                               (int)destOffset);
    }
...
String String::replaceCharacter(const juce_wchar charToReplace,
                                const juce_wchar charToInsert) const {
...
String String::fromFirstOccurrenceOf(StringRef sub, bool includeSubString,
...
String String::fromLastOccurrenceOf(StringRef sub, bool includeSubString,
                                    bool ignoreCase) const {
...
String String::upToFirstOccurrenceOf(StringRef sub, bool includeSubString,
                                     bool ignoreCase) const {
  auto i = ignoreCase ? indexOfIgnoreCase(sub) : indexOf(sub);
  if (i < 0)
    return *this;
...
String String::upToLastOccurrenceOf(StringRef sub, bool includeSubString,
                                    bool ignoreCase) const {
  auto i = ignoreCase ? lastIndexOfIgnoreCase(sub) : lastIndexOf(sub);
  if (i < 0)
    return *this;
...
String String::formattedRaw(const char *pf, ...) {
  ...
  for (;;) {
    ...
    if (num == 0 ||
        bufferSize > 65536) // the upper limit is a sanity check to avoid
                            // situations where vprintf repeatedly
      break; // returns -1 because of an error rather than because it needs more
             // space.
  }
...
template <typename Type> static String hexToString(Type v) {
...
String String::toHexString(const void *const d, const int size,
                           const int groupSize) {
  if (size <= 0)
    return {};
  ...
  String s(
      PreallocationBytes((size_t)numChars * sizeof(CharPointerType::CharType)));
...
static String getStringFromWindows1252Codepage(const char *data, size_t num) {
  ...
  for (size_t i = 0; i < num; ++i)
    unicode[i] = CharacterFunctions::getUnicodeCharFromWindows1252Codepage(
        (uint8)data[i]);
...
String String::createStringFromData(const void *const unknownData, int size) {
  ...
  if (CharPointer_UTF8::isValidString(start, size))
    return String(CharPointer_UTF8(start), CharPointer_UTF8(start + size));
...
struct StringEncodingConverter {
  static CharPointerType_Dest convert(const String &s) {
    ...
    if (source.isEmpty())
      return CharPointerType_Dest(
          reinterpret_cast<const DestChar *>(&emptyChar));
    ...
    auto endOffset = (text.sizeInBytes() + 3) &
                     ~3u; // the new string must be word-aligned or many Windows
...
#if JUCE_DEBUG // (This just avoids spurious warnings from valgrind about the
               // uninitialised bytes at the end of the buffer..)
    auto bytesToClear = (size_t)jmin((int)extraBytesNeeded, 4);
    zeromem(addBytesToPointer(newSpace, extraBytesNeeded - bytesToClear),
            bytesToClear);
#endif
...
template <> struct StringEncodingConverter<CharPointer_UTF8, CharPointer_UTF8> {
  static CharPointer_UTF8 convert(const String &source) noexcept {
    return CharPointer_UTF8(unalignedPointerCast<CharPointer_UTF8::CharType *>(
...
struct StringEncodingConverter<CharPointer_UTF16, CharPointer_UTF16> {
  static CharPointer_UTF16 convert(const String &source) noexcept {
    return CharPointer_UTF16(
        unalignedPointerCast<CharPointer_UTF16::CharType *>(
            source.getCharPointer().getAddress()));
  }
};
...
struct StringEncodingConverter<CharPointer_UTF32, CharPointer_UTF32> {
  static CharPointer_UTF32 convert(const String &source) noexcept {
    return CharPointer_UTF32(
        unalignedPointerCast<CharPointer_UTF32::CharType *>(
            source.getCharPointer().getAddress()));
  }
};

CharPointer_UTF8 String::toUTF8() const {
  return StringEncodingConverter<CharPointerType, CharPointer_UTF8>::convert(
      *this);
}
CharPointer_UTF16 String::toUTF16() const {
  return StringEncodingConverter<CharPointerType, CharPointer_UTF16>::convert(
      *this);
}
CharPointer_UTF32 String::toUTF32() const {
  return StringEncodingConverter<CharPointerType, CharPointer_UTF32>::convert(
      *this);
}
...
const wchar_t *String::toWideCharPointer() const {
  return StringEncodingConverter<CharPointerType, CharPointer_wchar_t>::convert(
             *this)
      .getAddress();
}
...
struct StringCopier {
  static size_t
  copyToBuffer(const CharPointerType_Src source,
    ...
    return CharPointerType_Dest(buffer).writeWithDestByteLimit(
        source, maxBufferSizeBytes);
  }
};

size_t String::copyToUTF8(CharPointer_UTF8::CharType *const buffer,
                          size_t maxBufferSizeBytes) const noexcept {
  return StringCopier<CharPointerType, CharPointer_UTF8>::copyToBuffer(
      text, buffer, maxBufferSizeBytes);
}

size_t String::copyToUTF16(CharPointer_UTF16::CharType *const buffer,
                           size_t maxBufferSizeBytes) const noexcept {
  return StringCopier<CharPointerType, CharPointer_UTF16>::copyToBuffer(
      text, buffer, maxBufferSizeBytes);
}

size_t String::copyToUTF32(CharPointer_UTF32::CharType *const buffer,
                           size_t maxBufferSizeBytes) const noexcept {
  return StringCopier<CharPointerType, CharPointer_UTF32>::copyToBuffer(
      text, buffer, maxBufferSizeBytes);
}
...
StringRef::StringRef() noexcept
    : text(unalignedPointerCast<const String::CharPointerType::CharType *>(
          "\0\0\0")) {}
...
#endif
{
  ...
  /*  If you get an assertion here, then you're trying to create a string from
     ...
     UTF-8, ASCII or one of many local code-pages.
      ...
      To get around this problem, you must be more explicit when you pass an
     ambiguous 8-bit string to the StringRef class - so for example if your
     source data is actually UTF-8, you'd call StringRef (CharPointer_UTF8 ("my
     utf8 string..")), and it would be able to correctly convert the multi-byte
     characters to unicode. It's *highly* recommended that you use UTF-8 with
     escape characters in your source code to represent extended characters,
      because there's no other way to represent these strings in a way that
     isn't dependent on the compiler, source code editor and platform.
  */
  jassert(CharPointer_ASCII::isValidString(stringLiteral,
...
static String reduceLengthOfFloatString(const String &input) {
  ...
  if ((trimStart != trimEnd && currentChar == '.') ||
...
public:
  StringTests() : UnitTest("String class", UnitTestCategories::text) {}

  template <class CharPointerType> struct TestUTFConversion {
    static void test(UnitTest &test, Random &r) {
      ...
      test.expect(CharPointerType::isValidString(
          buffer, (int)strlen((const char *)buffer)));
    }
  };
  ...
  void runTest() override {
...

    {
      ...
      expect(s1.startsWith("ab") && s1.startsWith("abcd") &&
             !s1.startsWith("abcde"));
      ...
      expect(s1.startsWith(String()) && s1.endsWith(String()) &&
             s1.contains(String()));
      ...
      expect(String("abc foo bar").containsWholeWord("abc") &&
             String("abc foo bar").containsWholeWord("abc"));
    }

    {
      ...
      expect(String::charToString(s.getLastCharacter()) ==
             s.getLastCharacters(1));
      ...
      expect(String(std::numeric_limits<int>::max()).getIntValue() ==
             std::numeric_limits<int>::max());
      expect(String(std::numeric_limits<int>::min()).getIntValue() ==
             std::numeric_limits<int>::min());
      expect(String(std::numeric_limits<int64>::max()).getLargeIntValue() ==
             std::numeric_limits<int64>::max());
      expect(String(std::numeric_limits<int64>::min()).getLargeIntValue() ==
      ...
      expect(
          String::toHexString((int64)0x1234abcd).equalsIgnoreCase("1234abcd"));
      ...
      expect(
          String::toHexString((int64)-1).equalsIgnoreCase("ffffffffffffffff"));

      unsigned char data[] = {1, 2, 3, 4, 0xa, 0xb, 0xc, 0xd};
      expect(
          String::toHexString(data, 8, 0).equalsIgnoreCase("010203040a0b0c0d"));
      expect(String::toHexString(data, 8, 1)
                 .equalsIgnoreCase("01 02 03 04 0a 0b 0c 0d"));
      expect(String::toHexString(data, 8, 2)
                 .equalsIgnoreCase("0102 0304 0a0b 0c0d"));
      ...
      expectEquals(String(1e-34, 36),
                   String("0.000000000000000000000000000000000100"));
      ...
      expectEquals(s5.fromFirstOccurrenceOf("xword2", true, false),
                   s5.substring(100));
      expectEquals(s5.fromFirstOccurrenceOf(String(L"word2"), true, false),
                   s5.substring(5));
      expectEquals(s5.fromFirstOccurrenceOf("Word2", true, true),
                   s5.substring(5));
      expectEquals(s5.fromFirstOccurrenceOf("word2", false, false),
                   s5.getLastCharacters(6));
      expectEquals(s5.fromFirstOccurrenceOf("Word2", false, true),
                   s5.getLastCharacters(6));

      expectEquals(s5.fromLastOccurrenceOf(String(), true, false), s5);
      expectEquals(s5.fromLastOccurrenceOf("wordx", true, false), s5);
      expectEquals(s5.fromLastOccurrenceOf("word", true, false),
                   s5.getLastCharacters(5));
      expectEquals(s5.fromLastOccurrenceOf("worD", true, true),
                   s5.getLastCharacters(5));
      expectEquals(s5.fromLastOccurrenceOf("word", false, false),
                   s5.getLastCharacters(1));
      expectEquals(s5.fromLastOccurrenceOf("worD", false, true),
                   s5.getLastCharacters(1));

      expect(s5.upToFirstOccurrenceOf(String(), true, false).isEmpty());
      expectEquals(s5.upToFirstOccurrenceOf("word4", true, false), s5);
      expectEquals(s5.upToFirstOccurrenceOf("word2", true, false),
                   s5.substring(0, 10));
      expectEquals(s5.upToFirstOccurrenceOf("Word2", true, true),
                   s5.substring(0, 10));
      expectEquals(s5.upToFirstOccurrenceOf("word2", false, false),
                   s5.substring(0, 5));
      expectEquals(s5.upToFirstOccurrenceOf("Word2", false, true),
                   s5.substring(0, 5));

      expectEquals(s5.upToLastOccurrenceOf(String(), true, false), s5);
      expectEquals(s5.upToLastOccurrenceOf("zword", true, false), s5);
      expectEquals(s5.upToLastOccurrenceOf("word", true, false),
                   s5.dropLastCharacters(1));
      expectEquals(
          s5.dropLastCharacters(1).upToLastOccurrenceOf("word", true, false),
          s5.dropLastCharacters(1));
      expectEquals(s5.upToLastOccurrenceOf("Word", true, true),
                   s5.dropLastCharacters(1));
      expectEquals(s5.upToLastOccurrenceOf("word", false, false),
                   s5.dropLastCharacters(5));
      expectEquals(s5.upToLastOccurrenceOf("Word", false, true),
                   s5.dropLastCharacters(5));

      expectEquals(s5.replace("word", "xyz", false), String("xyz xyz2 xyz3"));
      expect(s5.replace("Word", "xyz", true) == "xyz xyz2 xyz3");
      expect(s5.dropLastCharacters(1).replace("Word", String("xyz"), true) ==
             L"xyz xyz2 xyz");
      ...
      expectEquals(s5.replaceCharacter('w', L'x').replaceCharacter('x', 'w'),
                   s5);
      expect(s5.replaceCharacters("wo", "xy") != s5);
      expectEquals(
          s5.replaceCharacters("wo", "xy").replaceCharacters("xy", "wo"), s5);
      ...
      expectEquals(s5.initialSectionNotContaining(String("xyz ")),
                   String("word"));
      ...
      expectEquals(s6.trimStart().trimStart().trimEnd().trimEnd(),
...

    {
      ...
      expectEquals(String::toDecimalStringWithSignificantFigures(13, 1),
                   String("10"));
      expectEquals(String::toDecimalStringWithSignificantFigures(13, 2),
                   String("13"));
      expectEquals(String::toDecimalStringWithSignificantFigures(13, 3),
                   String("13.0"));
      expectEquals(String::toDecimalStringWithSignificantFigures(13, 4),
                   String("13.00"));

      expectEquals(String::toDecimalStringWithSignificantFigures(19368, 1),
                   String("20000"));
      expectEquals(String::toDecimalStringWithSignificantFigures(19348, 3),
                   String("19300"));

      expectEquals(String::toDecimalStringWithSignificantFigures(-5, 1),
                   String("-5"));
      expectEquals(String::toDecimalStringWithSignificantFigures(-5, 3),
                   String("-5.00"));

      // Zero

      expectEquals(String::toDecimalStringWithSignificantFigures(0, 1),
                   String("0"));
      expectEquals(String::toDecimalStringWithSignificantFigures(0, 2),
                   String("0.0"));
      expectEquals(String::toDecimalStringWithSignificantFigures(0, 3),
                   String("0.00"));

      // Floating point

      expectEquals(String::toDecimalStringWithSignificantFigures(19.0, 1),
                   String("20"));
      expectEquals(String::toDecimalStringWithSignificantFigures(19.0, 2),
                   String("19"));
      expectEquals(String::toDecimalStringWithSignificantFigures(19.0, 3),
                   String("19.0"));
      expectEquals(String::toDecimalStringWithSignificantFigures(19.0, 4),
                   String("19.00"));

      expectEquals(String::toDecimalStringWithSignificantFigures(-5.45, 1),
                   String("-5"));
      expectEquals(String::toDecimalStringWithSignificantFigures(-5.45, 3),
                   String("-5.45"));

      expectEquals(String::toDecimalStringWithSignificantFigures(12345.6789, 9),
                   String("12345.6789"));
      expectEquals(String::toDecimalStringWithSignificantFigures(12345.6789, 8),
                   String("12345.679"));
      expectEquals(String::toDecimalStringWithSignificantFigures(12345.6789, 5),
                   String("12346"));

      expectEquals(String::toDecimalStringWithSignificantFigures(0.00028647, 6),
                   String("0.000286470"));
      expectEquals(String::toDecimalStringWithSignificantFigures(0.0028647, 6),
                   String("0.00286470"));
      expectEquals(String::toDecimalStringWithSignificantFigures(2.8647, 6),
                   String("2.86470"));

      expectEquals(
          String::toDecimalStringWithSignificantFigures(-0.0000000000019, 1),
          String("-0.000000000002"));
    }

    {
...

      {
        ...
        for (auto &test : tests)
          expectEquals(reduceLengthOfFloatString(String(test.first, 15, true)),
                       test.second);
      }
    }
