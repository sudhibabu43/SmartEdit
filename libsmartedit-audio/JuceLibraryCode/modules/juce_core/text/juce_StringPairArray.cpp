...
bool StringPairArray::operator==(const StringPairArray &other) const {
  ...
  for (int i = 0; i < num; ++i) {
    if (keys[i] == other.keys[i]) // optimise for the case where the keys are in
                                  // the same order
    {
      if (values[i] != other.values[i])
        return false;
    } else {
      // if we encounter keys that are in a different order, search remaining
      // items by brute force..
      for (int j = i; j < num; ++j) {
...
bool StringPairArray::getIgnoresCase() const noexcept { return ignoreCase; }
...
template <typename Map> void StringPairArray::addMapImpl(const Map &toAdd) {
...
void StringPairArray::addUnorderedMap(
    const std::unordered_map<String, String> &toAdd) {
  addMapImpl(toAdd);
}
...
static String operator""_S(const char *chars, size_t) { return String{chars}; }
