...
public:
  ...
  */
  ...
  void write(const juce_wchar charToWrite) noexcept {
    ...
    if (c >= 0x80) {
      ...
      *data++ = (CharType)((uint32)(0xff << (7 - numExtraBytes)) |
                           (c >> (numExtraBytes * 6)));
  ...
  */
  template <typename CharPointer>
  size_t writeWithDestByteLimit(const CharPointer src,
                                const size_t maxDestBytes) noexcept {
  ...
  */
   ...
   * characters. */
  template <typename CharPointer>
  int compareIgnoreCaseUpTo(const CharPointer other,
                            const int maxChars) const noexcept {
  ...
  /** Returns the character index of a unicode character, or -1 if it isn't
   * found. */
  int indexOf(const juce_wchar charToFind,
  ...
  bool isWhitespace() const noexcept {
    return CharacterFunctions::isWhitespace((juce_wchar) * (*this));
  }
  ...
  /** Returns true if the first three bytes in this pointer are the UTF8
     byte-order mark (BOM). The pointer must not be null, and must point to at
     least 3 valid bytes.
  */
