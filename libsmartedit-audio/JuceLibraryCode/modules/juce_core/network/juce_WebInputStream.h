...
public:
  /** Creates a new WebInputStream which can be used to read from a URL.
      ...
      @param url      the URL that should be retrieved. This parameter may also
     contain POST data and/or parameters.
      @param addParametersToRequestBody  specifies whether any URL parameters
     that have been set will be transferred via the request body data or added
                      to the URL address. This will also determine whether a
     POST or GET command will be used if a custom command is not set.
  */
  ...
  /** Add extra headers to the HTTP request.
      ...
      @param extraHeaders   this string is appended onto the headers that are
     used for the request. It must therefore be a valid set of HTML header
  ...
  */
  ...
  /** Override the HTTP command that is sent.
      ...
      @param customRequestCommand  this string is the custom HTTP request
  ...
  */
  ...
  /** Specify the connection time-out.
      ...
      @param timeoutInMs  the number of milliseconds to wait until the
     connection request is aborted.
  */
  ...
  /** Specify the number of redirects to be followed.
      ...
      @param numRedirects  specifies the number of redirects that will be
     followed before returning a response (ignored for Android which follows up
     to 5 redirects)
  */
  ...
  /** Used to receive callbacks for POST data send progress.
      ...
      method will be called periodically with updates on POST data upload
     progress.
  */
  ...
  public:
    ...
    /** This method will be called periodically with updates on POST data upload
       progress.
    ...
    */
    virtual bool postDataSendProgress(WebInputStream &request, int bytesSent,
                                      int totalBytes);
  };
  ...
  /** Wait until the first byte is ready for reading.
      ...
      and block until the status code and all response headers have been
     received or an error has occurred.

      Note that most methods will call connect() internally if they are called
     without an established connection. Therefore, it is not necessary to
     explicitly call connect unless you would like to use a custom listener.

      After a successful call to connect(), getResponseHeaders(),
     getTotalLength() and getStatusCode() will all be non-blocking.
  ...
  */
  ...
  /** Will cancel a blocking read and prevent any subsequent connection
   * attempts. */
  void cancel();
  ...
  */
  ...
  /** Reads some data from the stream into a memory buffer.
     ...
     the memory block passed in is big enough to contain this many bytes. This
     value must not be negative.

      @returns    the actual number of bytes that were read, which may be less
     than maxBytesToRead if the stream is exhausted before it gets that far
  */
