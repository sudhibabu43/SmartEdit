...
public:
  ...
  /** Creates a URL from a string.
      ...
      This will parse any embedded parameters after a '?' character and store
     them in the list (see getParameterNames etc). If you don't want this to
     happen, you can use createWithoutParsing().
  */
  URL(const String &url);

  /** Creates URL referring to a local file on your disk using the file://
   * scheme. */
  explicit URL(File localFile);

  /** Compares two URLs.

      All aspects of the URLs must be identical for them to match, including any
     parameters, upload files, etc.
  */
  ...
  /** Returns a string version of the URL.
      ...
      @param includeGetParameters  if this is true and any parameters have been
     set with the withParameter() method, then the string will have these
     appended on the end and URL-encoded.
  ...
  */
  ...
  /** Returns the file path of the local file to which this URL refers to.
      ...
      If the URL does not represent a local file URL (i.e. the URL's scheme is
     not 'file') then this method will assert.

      This method also supports converting Android's content:// URLs to local
     file paths.
  ...
  */
  ...
  /** Returns the file name.
      ...
      For all but Android's content:// scheme, it will simply return the last
     segment of the URL, e.g. for "http://www.xyz.com/foo/bar.txt", this will
     return "bar.txt".
  ...
  */
  ...
  /** Returns a new URL that refers to a sub-path relative to this one.
      ...
      Note that there's no way for this method to know whether the original URL
     is a file or directory, so it's up to you to make sure it's a directory. It
     also won't attempt to be smart about the content of the childPath string,
     so if this string is an absolute URL, it'll still just get bolted onto the
     end of the path.
  ...
  */
  ...
  /** Returns a copy of this URL, with a GET or POST parameter added to the end.
      ...
      e.g. calling "withParameter ("amount", "some fish") for the url
     "www.fish.com" would produce a new url whose `toString (true)` method would
     return "www.fish.com?amount=some+fish".
  ...
  */
  ...
  /** Returns a copy of this URL, with a set of GET or POST parameters added.
      ...
      This is a convenience method, equivalent to calling withParameter for each
     value.

      @see withParameter
  */
  [[nodiscard]] URL
  withParameters(const StringPairArray &parametersToAdd) const;
  ...
  /** Returns a copy of this URL, with a file-upload type parameter added to it.
      ...
      Note that the filename is stored, but the file itself won't actually be
     read until this URL is later used to create a network input stream. If you
     want to upload data from memory, use withDataToUpload().
  ...
  */
  ...
  /** Returns an array of the names of all the URL's parameters.
      ...
      e.g. for the url "www.fish.com?type=haddock&amount=some+fish", this array
     would contain two items: "type" and "amount".

      You can call getParameterValues() to get the corresponding value of each
      parameter. Note that the list can contain multiple parameters with the
     same name.
  ...
  */
  ...
  /** Returns an array of the values of all the URL's parameters.
      ...
      e.g. for the url "www.fish.com?type=haddock&amount=some+fish", this array
     would contain two items: "haddock" and "some fish".

      The values returned will have been cleaned up to remove any escape
     characters.
  ...
  */
  ...
  /** Class used to create a set of options to pass to the createInputStream()
     method.
      ...
      if (auto inputStream = URL ("http://www.xyz.com/foobar")
                               .createInputStream (URL::InputStreamOptions
     (URL::ParameterHandling::inAddress) .withConnectionTimeoutMs (1000)
  ...
  */
  ...
  public:
    /** Constructor.
        ...
        If parameterHandling is ParameterHandling::inPostData, any URL
       parameters that have been set will be transferred via the request body
       data. Otherwise the parameters will be added to the URL address.
    */
    ...
    /** A callback function to keep track of the operation's progress.
        ...
        This can be useful for lengthy POST operations, so that you can provide
       user feedback.
    */
    [[nodiscard]] InputStreamOptions withProgressCallback(
        std::function<bool(int bytesSent, int totalBytes)> progressCallback)
        const;

    /** A string that will be appended onto the headers that are used for the
       request.

        It must be a valid set of HTML header directives, separated by newlines.
    */
    [[nodiscard]] InputStreamOptions
    withExtraHeaders(const String &extraHeaders) const;

    /** Specifies a timeout for the request in milliseconds.

        If 0, this will use whatever default setting the OS chooses. If a
       negative number, it will be infinite.
    */
    [[nodiscard]] InputStreamOptions
    withConnectionTimeoutMs(int connectionTimeoutMs) const;

    /** If this is non-null, all the (key, value) pairs received as headers
        in the response will be stored in this array.
    */
    [[nodiscard]] InputStreamOptions
    withResponseHeaders(StringPairArray *responseHeaders) const;
    ...
    */
    ...
    /** Specifies the number of redirects that will be followed before returning
       a response.

        N.B. This will be ignored on Android which follows up to 5 redirects.
    */
    [[nodiscard]] InputStreamOptions
    withNumRedirectsToFollow(int numRedirectsToFollow) const;

    /** Specifies which HTTP request command to use.

        If this is not set, then the command will be POST if parameterHandling
       is set to ParameterHandling::inPostData or if any POST data has been
       specified via withPOSTData(), withFileToUpload(), or withDataToUpload().
       Otherwise it will be GET.
    */
    [[nodiscard]] InputStreamOptions
    withHttpRequestCmd(const String &httpRequestCmd) const;
  ...
  /** Attempts to open a stream that can read from this URL.
      ...
      Note that this method will block until the first byte of data has been
     received or an error has occurred.

      Note that on some platforms (Android, for example) it's not permitted to
     do any network action from the message thread, so you must only call it
     from a background thread.

      Unless the URL represents a local file, this method returns an instance of
     a WebInputStream. You can use dynamic_cast to cast the return value to a
     WebInputStream which allows you more fine-grained control of the transfer
     process.

      If the URL represents a local file, then this method simply returns a
     FileInputStream.

      @param options           a set of options that will be used when opening
     the stream.

      @returns                 a valid input stream, or nullptr if there was an
     error trying to open it.
  */
  std::unique_ptr<InputStream>
  createInputStream(const InputStreamOptions &options) const;
  ...
  */
  ...
  struct JUCE_API DownloadTaskListener {
    ...
    */
    virtual void progress(DownloadTask *task, int64 bytesDownloaded,
                          int64 totalLength);
  };
  ...
  */
  ...
  public:
    ...
    [[nodiscard]] auto withExtraHeaders(String value) const {
      return with(&DownloadTaskOptions::extraHeaders, std::move(value));
    }

    /** On iOS, specifies the container where the downloaded file will be
       stored.
    ...
    */
    [[nodiscard]] auto withSharedContainer(String value) const {
      return with(&DownloadTaskOptions::sharedContainer, std::move(value));
    }

    /** Specifies an observer for the download task. */
    [[nodiscard]] auto withListener(DownloadTaskListener *value) const {
      return with(&DownloadTaskOptions::listener, std::move(value));
    }

    /** Specifies whether a post command should be used. */
    [[nodiscard]] auto withUsePost(bool value) const {
      return with(&DownloadTaskOptions::usePost, value);
    }

  private:
    template <typename Member, typename Value>
    [[nodiscard]] DownloadTaskOptions with(Member &&member,
                                           Value &&value) const {
      auto copy = *this;
  ...
  /** Represents a download task.
      ...
      Returned by downloadToFile() to allow querying and controlling the
     download task.
  */
  ...
  public:
    ...
    */
    ...
    /** Returns the target file location that was provided in
     * URL::downloadToFile. */
  ...
  private:
    friend class URL;
    static std::unique_ptr<DownloadTask>
    createFallbackDownloader(const URL &, const File &,
  ...
  /** This function is replaced by a new overload accepting a
   * DownloadTaskOptions argument. */
  [[deprecated(
      "Use the overload with a DownloadTaskOptions argument instead")]] std::
      unique_ptr<DownloadTask>
      downloadToFile(const File &targetLocation, String extraHeaders = String(),
                     DownloadTaskListener *listener = nullptr,
  ...
  /** Download the URL to a file.
      ...
      Using this method to download files on mobile is less flexible but more
     reliable than using createInputStream or WebInputStreams as it will attempt
     to download the file using a native OS background network task. Such tasks
     automatically deal with network re-connections and continuing your download
     while your app is suspended.
  */
  std::unique_ptr<DownloadTask>
  downloadToFile(const File &targetLocation,
                 const DownloadTaskOptions &options);

  //==============================================================================
  /** Tries to download the entire contents of this URL into a binary data
     block.

      If it succeeds, this will return true and append the data it read onto the
  ...
  */
  ...
  /** Tries to download the entire contents of this URL as a string.
      ...
      If it fails, this will return an empty string, otherwise it will return
     the contents of the downloaded file. If you need to distinguish between a
     read operation that fails and one that returns an empty string, you'll need
     to use a different method, such as readEntireBinaryStream().
  ...
  */
  ...
  /** Tries to download the entire contents of this URL and parse it as XML.
      ...
      If it fails, or if the text that it reads can't be parsed as XML, this
     will return nullptr.
  ...
  */
  ...
  readEntireXmlStream(bool usePostCommand = false) const;
  ...
  /** Adds escape sequences to a string to encode any characters that aren't
      ...
      @param isParameter               if true then the string is going to be
                                       ...
                                       legal in a URL.
      @param roundBracketsAreLegal     technically round brackets are ok in
     URLs, however, some servers (like AWS) also want round brackets to be
     escaped.
  ...
  */
  ...
  /** Returns a URL without attempting to remove any embedded parameters from
     the string.

      This may be necessary if you need to create a request that involves both
     POST parameters and parameters which are embedded in the URL address
     itself.
  */
  ...
  //==============================================================================
#ifndef DOXYGEN
  using OpenStreamProgressCallback = bool(void *context, int bytesSent,
                                          int totalBytes);
  ...
  */
  [[deprecated(
      "New code should use the method which takes an InputStreamOptions "
      "argument instead.")]] std::unique_ptr<InputStream>
  createInputStream(bool doPostLikeRequest,
                    OpenStreamProgressCallback *progressCallback = nullptr,
                    void *progressCallbackContext = nullptr,
                    String extraHeaders = {}, int connectionTimeOutMs = 0,
                    StringPairArray *responseHeaders = nullptr,
                    int *statusCode = nullptr, int numRedirectsToFollow = 5,
...
private:
  ...
  struct Upload : public ReferenceCountedObject {
    Upload(const String &, const String &, const String &, const File &,
           MemoryBlock *);
