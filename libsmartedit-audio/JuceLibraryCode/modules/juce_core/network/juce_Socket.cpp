...
static bool setOption(SocketHandle handle, int mode, int property,
                      Type value) noexcept {
  return setsockopt(handle, mode, property,
...
static std::optional<int> getBufferSize(SocketHandle handle, int property) {
  ...
  if (getsockopt(handle, SOL_SOCKET, property,
                 reinterpret_cast<char *>(&result), &outParamSize) != 0 ||
...
static bool resetSocketOptions(SocketHandle handle, bool isDatagram,
                               bool allowBroadcast,
  ...
  const auto receiveBufferSize = options.getReceiveBufferSize().value_or(
      getCurrentBufferSizeWithMinimum(SO_RCVBUF));
  const auto sendBufferSize = options.getSendBufferSize().value_or(
      getCurrentBufferSizeWithMinimum(SO_SNDBUF));

  return handle != invalidSocket &&
         setOption(handle, SO_RCVBUF, receiveBufferSize) &&
         setOption(handle, SO_SNDBUF, sendBufferSize) &&
         (isDatagram
              ? ((!allowBroadcast) || setOption(handle, SO_BROADCAST, (int)1))
...
static bool bindSocket(SocketHandle handle, int port,
                       const String &address) noexcept {
  if (handle == invalidSocket || !isValidPortNumber(port))
    return false;

  struct sockaddr_in addr;
  zerostruct(addr); // (can't use "= { 0 }" on this object because it's
                    // typedef'ed as a C struct)
...
static bool setSocketBlockingState(SocketHandle handle,
...
static int readSocket(SocketHandle handle, void *destBuffer, int maxBytesToRead,
                      std::atomic<bool> &connected,
                      bool blockUntilSpecifiedAmountHasArrived,
                      CriticalSection &readLock, String *senderIP = nullptr,
...

    {
      ...
      if (lock.isLocked()) {
        if (senderIP == nullptr || senderPort == nullptr) {
          ...
          bytesThisTime = ::recvfrom(handle, buffer, numToRead, 0,
...
static int waitForReadiness(std::atomic<int> &handle, CriticalSection &readLock,
  ...
  if (select((int)h + 1, prset, pwset, nullptr, timeoutp) < 0 ||
      hasErrorOccurred())
    return -1;
...
static addrinfo *getAddressInfo(bool isDatagram, const String &hostName,
                                int portNumber) {
  ...
  if (getaddrinfo(hostName.toRawUTF8(), String(portNumber).toRawUTF8(), &hints,
                  &info) == 0)
    return info;
...
static bool connectSocket(std::atomic<int> &handle, CriticalSection &readLock,
                          const String &hostName, int portNumber,
  ...
  if (auto *info = getAddressInfo(false, hostName, portNumber)) {
    for (auto *i = info; i != nullptr; i = i->ai_next) {
      ...
      if (newHandle != invalidSocket) {
        ...
        auto result =
            ::connect(newHandle, i->ai_addr, (socklen_t)i->ai_addrlen);
        success = (result >= 0);
        ...
        if (!success) {
...
#endif
          {
            ...
            if (waitForReadiness(cvHandle, readLock, false, timeOutMillisecs) ==
                1)
              success = true;
          }
        }
...
//==============================================================================
StreamingSocket::StreamingSocket() { SocketHelpers::initSockets(); }

StreamingSocket::StreamingSocket(const String &host, int portNum, int h,
                                 const SocketOptions &optionsIn)
    : options(optionsIn), hostName(host), portNumber(portNum), handle(h),
      connected(true) {
...
StreamingSocket::~StreamingSocket() { close(); }

//==============================================================================
int StreamingSocket::read(void *destBuffer, int maxBytesToRead,
                          bool shouldBlock) {
  return (connected && !isListener)
             ? SocketHelpers::readSocket((SocketHandle)handle.load(),
                                         destBuffer, maxBytesToRead, connected,
                                         shouldBlock, readLock)
             : -1;
}
...
int StreamingSocket::write(const void *sourceBuffer, int numBytesToWrite) {
  ...
  return (int)::send((SocketHandle)handle.load(), (const char *)sourceBuffer,
                     (juce_recvsend_size_t)numBytesToWrite, 0);
}

//==============================================================================
int StreamingSocket::waitUntilReady(bool readyForReading, int timeoutMsecs) {
  return connected ? SocketHelpers::waitForReadiness(
                         handle, readLock, readyForReading, timeoutMsecs)
                   : -1;
}
...
bool StreamingSocket::connect(const String &remoteHostName,
                              int remotePortNumber, int timeOutMillisecs) {
  ...
  connected =
      SocketHelpers::connectSocket(handle, readLock, remoteHostName,
                                   remotePortNumber, timeOutMillisecs, options);

  if (!connected)
    return false;

  if (!SocketHelpers::resetSocketOptions((SocketHandle)handle.load(), false,
                                         false, options)) {
    close();
    return false;
  }
...
void StreamingSocket::close() {
  if (handle >= 0)
    SocketHelpers::closeSocket(handle, readLock, isListener, portNumber,
                               connected);
...
//==============================================================================
bool StreamingSocket::createListener(int newPortNumber,
...
#if !JUCE_WINDOWS // on windows, adding this option produces behaviour different
                  // to posix
  SocketHelpers::makeReusable(handle);
#endif

  if (SocketHelpers::bindSocket((SocketHandle)handle.load(), portNumber,
                                localHostName) &&
...
StreamingSocket *StreamingSocket::waitForNextConnection() const {
  ...
  if (connected && isListener) {
    ...
    auto newSocket = (int)accept((SocketHandle)handle.load(),
                                 (struct sockaddr *)&address, &len);

    if (newSocket >= 0 && connected)
      return new StreamingSocket(
          inet_ntoa(((struct sockaddr_in *)&address)->sin_addr), portNumber,
          newSocket, options);
  }
...
bool StreamingSocket::isLocal() const noexcept {
  ...
  IPAddress currentIP(
      SocketHelpers::getConnectedAddress((SocketHandle)handle.load()));
...
DatagramSocket::DatagramSocket(bool canBroadcast,
  ...
  if (handle >= 0) {
    SocketHelpers::resetSocketOptions((SocketHandle)handle.load(), true,
...
bool DatagramSocket::bindToPort(int port) { return bindToPort(port, String()); }
...
int DatagramSocket::getBoundPort() const noexcept {
  return (handle >= 0 && isBound)
             ? SocketHelpers::getBoundPort((SocketHandle)handle.load())
             : -1;
}
...
int DatagramSocket::waitUntilReady(bool readyForReading, int timeoutMsecs) {
  ...
  return SocketHelpers::waitForReadiness(handle, readLock, readyForReading,
                                         timeoutMsecs);
}

int DatagramSocket::read(void *destBuffer, int maxBytesToRead,
                         bool shouldBlock) {
  if (handle < 0 || !isBound)
    return -1;

  std::atomic<bool> connected{true};
  return SocketHelpers::readSocket((SocketHandle)handle.load(), destBuffer,
                                   maxBytesToRead, connected, shouldBlock,
                                   readLock);
}

int DatagramSocket::read(void *destBuffer, int maxBytesToRead, bool shouldBlock,
                         String &senderIPAddress, int &senderPort) {
  if (handle < 0 || !isBound)
    return -1;
  ...
  return SocketHelpers::readSocket((SocketHandle)handle.load(), destBuffer,
                                   maxBytesToRead, connected, shouldBlock,
                                   readLock, &senderIPAddress, &senderPort);
}
...
int DatagramSocket::write(const String &remoteHostname, int remotePortNumber,
  ...
  struct addrinfo *&info =
      reinterpret_cast<struct addrinfo *&>(lastServerAddress);

  // getaddrinfo can be quite slow so cache the result of the address lookup
  if (info == nullptr || remoteHostname != lastServerHost ||
      remotePortNumber != lastServerPort) {
    if (info != nullptr)
      freeaddrinfo(info);

    if ((info = SocketHelpers::getAddressInfo(true, remoteHostname,
  ...
  return (int)::sendto((SocketHandle)handle.load(), (const char *)sourceBuffer,
                       (juce_recvsend_size_t)numBytesToWrite, 0, info->ai_addr,
...
bool DatagramSocket::joinMulticast(const String &multicastIPAddress) {
  ...
  return SocketHelpers::multicast(handle, multicastIPAddress, lastBindAddress,
                                  true);
}
...
bool DatagramSocket::setMulticastLoopbackEnabled(bool enable) {
  ...
  return SocketHelpers::setOption<bool>((SocketHandle)handle.load(), IPPROTO_IP,
                                        IP_MULTICAST_LOOP, enable);
}

bool DatagramSocket::setEnablePortReuse([[maybe_unused]] bool enabled) {
#if !JUCE_ANDROID
  if (handle >= 0)
    return SocketHelpers::setOption(
        (SocketHandle)handle.load(),
#if JUCE_WINDOWS || JUCE_LINUX || JUCE_BSD
        SO_REUSEADDR, // port re-use is implied by addr re-use on these
                      // platforms
#else
        SO_REUSEPORT,
#endif
...
struct SocketTests final : public UnitTest {
  SocketTests() : UnitTest("Sockets", UnitTestCategories::networking) {}
  ...
  void runTest() override {
    ...
    beginTest("StreamingSocket");
    {
      ...
      expect(static_cast<SocketHandle>(socketServer.getRawSocketHandle()) ==
             invalidSocket);
      ...
      expect(static_cast<SocketHandle>(socket.getRawSocketHandle()) !=
             invalidSocket);
