...
public:
  /** The provided size will be used to configure the socket's SO_RCVBUF
     property. Increasing the buffer size can reduce the number of lost packets
     with the DatagramSocket class, if the socket is to receive packets in large
     bursts.

      If this property is not specified, the system default value will be used,
     but a minimum of 65536 will be ensured.
  */
  [[nodiscard]] SocketOptions withReceiveBufferSize(int size) const {
    return withMember(*this, &SocketOptions::receiveBufferSize, size);
  }

  /** The provided size will be used to configure the socket's SO_SNDBUF
     property.
...
/**
    ...
    This allows low-level use of sockets; for an easier-to-use messaging layer
   on top of sockets, you could also try the InterprocessConnection class.
...
public:
  ...
  /** Creates an uninitialised socket.
      ...
      To connect it, use the connect() method, after which you can read() or
     write() to it.

      To wait for other sockets to connect to this one, the createListener()
     method enters "listener" mode, and can be used to spawn new sockets for
     each connection that comes along.
  */
  StreamingSocket();

  /** Creates an uninitialised socket and allows specifying options related to
     the configuration of the underlying socket.
  ...
  */
  ...
  /** Binds the socket to the specified local port.
      ...
      @returns  true on success; false may indicate that another socket is
     already bound on the same port
  */
  bool bindToPort(int localPortNumber);

  /** Binds the socket to the specified local port and local address.

      If localAddress is not an empty string then the socket will be bound to
     localAddress as well. This is useful if you would like to bind your socket
     to a specific network adapter. Note that localAddress must be an IP address
     assigned to one of your network address otherwise this function will fail.
  ...
  */
  ...
  /** Returns the local port number to which this socket is currently bound.
      ...
      This is useful if you need to know to which port the OS has actually bound
     your socket when calling the constructor or bindToPort with zero as the
  ...
  */
  ...
  /** Tries to connect the socket to hostname:port.
      ...
      If timeOutMillisecs is 0, then this method will block until the operating
     system rejects the connection (which could take a long time).

      @returns  true if it succeeds, false if otherwise
      @see isConnected
  */
  bool connect(const String &remoteHostname, int remotePortNumber,
  ...
  /** True if the socket is connected to this machine rather than over the
   * network. */
  ...
  */
  ...
  /** Reads bytes from the socket.
      ...
      If blockUntilSpecifiedAmountHasArrived is true, the method will block
     until maxBytesToRead bytes have been read, (or until an error occurs). If
     this flag is false, the method will return as much data as is currently
  ...
  */
  ...
  /** Writes bytes to the socket from a buffer.
      ...
      Note that this method will block unless you have checked the socket is
     ready for writing before calling it (see the waitUntilReady() method).
  ...
  */
  ...
  /** Puts this socket into "listener" mode.
      ...
      When in this mode, your thread can call waitForNextConnection()
     repeatedly, which will spawn new sockets for each new connection, so that
     these can be handled in parallel by other threads.
  ...
  */
  ...
  /** When in "listener" mode, this waits for a connection and spawns it as a
     new socket.
...
public:
  ...
  /** Creates a datagram socket and allows specifying options related to the
      ...
      You first need to bind this socket to a port with bindToPort if you intend
     to read from this socket.

      If enableBroadcasting is true, the socket will be allowed to send
     broadcast messages (may require extra privileges on linux)
  */
  ...
  */
  ...
  /** Creates a datagram socket.
     ...
     to read from this socket.
      ...
      This constructor creates a socket that does not allow sending broadcast
     messages.
  */
  ...
  /** Binds the socket to the specified local port.
      ...
      The localPortNumber is the port on which to bind this socket. If this
     value is 0, the port number is assigned by the operating system.
  ...
  */
  ...
  /** Returns the local port number to which this socket is currently bound.
      ...
      This is useful if you need to know to which port the OS has actually bound
     your socket when bindToPort was called with zero.

      @returns  -1 if the socket didn't bind to any port yet or an error
     occurred
  */
  int getBoundPort() const noexcept;
  ...
  */
  ...
  /** Reads bytes from the socket and return the IP address of the sender.
     ...
     available without blocking.
      ...
      @returns  the number of bytes read, or -1 if there was an error. On a
     successful result, the senderIPAddress value will be set to the IP of the
     sender
      @see waitUntilReady
  */
  int read(void *destBuffer, int maxBytesToRead,
           bool blockUntilSpecifiedAmountHasArrived, String &senderIPAddress,
  ...
  */
  ...
  /** Closes the underlying socket object.
      ...
      Closes the underlying socket object and aborts any read or write
     operations. Note that all other methods will return an error after this
     call and the object cannot be re-used.

      This method is useful if another thread is blocking in a read/write call
     and you would like to abort the read/write thread. Simply deleting the
     socket object without calling shutdown may cause a race-condition where the
     read/write returns just before the socket is deleted and the subsequent
     read/write would try to read from an invalid pointer. By calling shutdown
     first, the socket object remains valid but all current and subsequent calls
     to read/write will return immediately.
  */
  void shutdown();
