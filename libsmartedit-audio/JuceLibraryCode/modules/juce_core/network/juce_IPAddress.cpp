...
/** Union used to split a 16-bit unsigned integer into 2 8-bit unsigned integers
 * or vice-versa */
...
IPAddress::IPAddress(uint16 a1, uint16 a2, uint16 a3, uint16 a4, uint16 a5,
                     uint16 a6, uint16 a7, uint16 a8) noexcept
    : isIPv6(true)
...
static String removePort(const String &adr) {
  if (adr.containsAnyOf("[]"))
    return adr.fromFirstOccurrenceOf("[", false, true)
...
IPAddress::IPAddress(const String &adr) {
  ...
  if (!isIPv6) {
    ...
    if (tokens.contains({})) // if :: shorthand has been used
    {
      ...
      // mapped IPv4 address will be treated as a single token, so pad the end
      // of the StringArray
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      IPAddressByteUnion temp;
      temp.combined = CharacterFunctions::HexParser<uint16>::parse(
          tokens[i].getCharPointer());
...
IPAddress IPAddress::broadcast() noexcept {
  return IPAddress(255, 255, 255, 255);
}
IPAddress IPAddress::local(bool IPv6) noexcept {
...
String IPAddress::getFormattedAddress(const String &unformattedAddress) {
  jassert(unformattedAddress.contains(":") &&
          !unformattedAddress.contains(
              "::")); // needs to be an unformatted IPv6 address!

  auto portString = unformattedAddress.fromFirstOccurrenceOf("]", false, true);
  auto addressString =
      unformattedAddress.dropLastCharacters(portString.length())
          .removeCharacters("[]");
  ...
  for (int i = 0; i < tokens.size(); ++i) {
    ...
    if (t.getHexValue32() == 0x0000) {
      ...
      if (t.length() > 1)
        addressString =
            addressString.replace(String::repeatedString("0", t.length()), "0");

      if (isFirst && numZerosTemp != 0 && numZeros > numZerosTemp)
        isFirst = false;
    } else {
      addressString =
          addressString.replace(t, t.trimCharactersAtStart("0").toLowerCase());
  ...
  if (numZeros > 1) {
    if (numZeros == tokens.size()) {
      ...
      auto zeroString = isFirst
                            ? "0" + String::repeatedString(":0", numZeros - 1)
...
IPAddress
IPAddress::convertIPv4MappedAddressToIPv4(const IPAddress &mappedAddress) {
...
IPAddress
IPAddress::convertIPv4AddressToIPv4Mapped(const IPAddress &addressToMap) {
  ...
  return {0x0,
          ...
          0x0,
          0xffff,
          static_cast<uint16>((addressToMap.address[0] << 8) |
                              addressToMap.address[1]),
          static_cast<uint16>((addressToMap.address[2] << 8) |
                              addressToMap.address[3])};
}
...
struct IPAddressTests final : public UnitTest {
  IPAddressTests() : UnitTest("IPAddress", UnitTestCategories::networking) {}
