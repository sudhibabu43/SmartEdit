...
//==============================================================================
static int64 findCentralDirectoryFileHeader(InputStream &input,
                                            int &numEntries) {
...
struct ZipFile::ZipInputStream final : public InputStream {
  ...
  int64 getTotalLength() override { return zipEntryHolder.compressedSize; }
...
ZipFile::ZipFile(InputStream &stream) : inputStream(&stream) { init(); }
...
ZipFile::~ZipFile() { entries.clear(); }

#if JUCE_DEBUG
ZipFile::OpenStreamCounter::~OpenStreamCounter() {
  /* If you hit this assertion, it means you've created a stream to read one of
     the items in the zipfile, but you've forgotten to delete that stream object
     before deleting the file.. Streams can't be kept open after the file is
     deleted because they need to share the input stream that is managed by the
     ZipFile object.
  */
...
int ZipFile::getIndexOfFileName(const String &fileName,
...
const ZipFile::ZipEntry *ZipFile::getEntry(const String &fileName,
                                           bool ignoreCase) const noexcept {
...
InputStream *ZipFile::createStreamForEntry(const int index) {
  ...
  if (auto *zei = entries[index]) {
    ...
    if (zei->isCompressed) {
      stream = new GZIPDecompressorInputStream(
          stream, true, GZIPDecompressorInputStream::deflateFormat,
...
void ZipFile::sortEntriesByFilename() {
  std::sort(entries.begin(), entries.end(),
            [](const ZipEntryHolder *e1, const ZipEntryHolder *e2) {
...
void ZipFile::init() {
  ...
  if (in != nullptr) {
    ...
    if (centralDirectoryPos >= 0 &&
        centralDirectoryPos < in->getTotalLength()) {
...
Result ZipFile::uncompressEntry(int index, const File &targetDirectory,
                                bool shouldOverwriteFiles) {
  return uncompressEntry(index, targetDirectory,
                         shouldOverwriteFiles ? OverwriteFiles::yes
                                              : OverwriteFiles::no,
                         FollowSymlinks::no);
}
...
Result ZipFile::uncompressEntry(int index, const File &targetDirectory,
  ...
  if (!targetFile.isAChildOf(targetDirectory))
    return Result::fail("Entry " + entryPath +
  ...
  if (targetFile.exists()) {
    ...
    if (!targetFile.deleteFile())
      return Result::fail("Failed to write to target file: " +
                          targetFile.getFullPathName());
  }

  if (followSymlinks == FollowSymlinks::no &&
      hasSymbolicPart(targetDirectory, targetFile.getParentDirectory()))
    return Result::fail(
        "Parent directory leads through symlink for target file: " +
        targetFile.getFullPathName());

  if (!targetFile.getParentDirectory().createDirectory())
    return Result::fail("Failed to create target folder: " +
                        targetFile.getParentDirectory().getFullPathName());

  if (zei->entry.isSymbolicLink) {
    String originalFilePath(in->readEntireStreamAsString().replaceCharacter(
        L'/', File::getSeparatorChar()));

    if (!File::createSymbolicLink(targetFile, originalFilePath, true))
      return Result::fail("Failed to create symbolic link: " +
                          originalFilePath);
  } else {
...
struct ZipFile::Builder::Item {
  Item(const File &f, InputStream *s, int compression, const String &storedPath,
       Time time)
  ...
  bool writeData(OutputStream &target, const int64 overallStartPosition) {
    ...
    if (symbolicLink) {
      auto relativePath = file.getNativeLinkedTarget().replaceCharacter(
          File::getSeparatorChar(), L'/');

      uncompressedSize = relativePath.length();

      checksum = zlibNamespace::crc32(0, (uint8_t *)relativePath.toRawUTF8(),
                                      (unsigned int)uncompressedSize);
      compressedData << relativePath;
    } else if (compressionLevel > 0) {
      GZIPCompressorOutputStream compressor(
          compressedData, compressionLevel,
  ...
  bool writeDirectoryEntry(OutputStream &target) {
    ...
    target.writeInt(
        (int)(symbolicLink ? 0xA1ED0000 : 0)); // external attributes
...
private:
  ...
  static void writeTimeAndDate(OutputStream &target, Time t) {
    ...
    target.writeShort((short)(t.getDayOfMonth() + ((t.getMonth() + 1) << 5) +
                              ((t.getYear() - 1980) << 9)));
  }
  ...
  bool writeSource(OutputStream &target) {
    ...
    while (!stream->isExhausted()) {
      ...
      checksum =
          zlibNamespace::crc32(checksum, buffer, (unsigned int)bytesRead);
  ...
  void writeFlagsAndSizes(OutputStream &target) const {
    ...
    target.writeShort(
        (short)(1 << 11)); // this flag indicates UTF-8 filename encoding
    target.writeShort((!symbolicLink && compressionLevel > 0)
                          ? (short)8
                          : (short)0); // symlink target path is not compressed
...
void ZipFile::Builder::addFile(const File &file, int compression,
                               const String &path) {
...
void ZipFile::Builder::addEntry(InputStream *stream, int compression,
                                const String &path, Time time) {
...
struct ZIPTests final : public UnitTest {
  ZIPTests() : UnitTest("ZIP", UnitTestCategories::compression) {}
  ...
  static MemoryBlock createZipMemoryBlock(const StringArray &entryNames) {
    ...
    for (auto &entryName : entryNames) {
      ...
      mo << entryName;
      mo.flush();
      builder.addEntry(new MemoryInputStream(block, false), 9, entryName,
                       Time::getCurrentTime());
    }
    ...
    for (int i = 0; i < zip.getNumEntries(); ++i) {
      ...
      if (caseIt != testCases.end()) {
        expect(result.wasOk() == caseIt->second,
               zip.getEntry(i)->filename + " was unexpectedly " +
                   (result.wasOk() ? "OK" : "not OK"));
      } else {
        expect(false);
      }
    }
