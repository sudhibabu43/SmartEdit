...
public:
  GZIPCompressorHelper(int compressionLevel, int windowBits)
      : compLevel((compressionLevel < 0 || compressionLevel > 9)
                      ? -1
                      : compressionLevel) {
    ...
    streamIsValid = (deflateInit2(&stream, compLevel, Z_DEFLATED,
                                  windowBits != 0 ? windowBits : MAX_WBITS, 8,
                                  strategy) == Z_OK);
  }
...
private:
  ...
  bool doNextBlock(const uint8 *&data, size_t &dataSize, OutputStream &out,
                   const int flushMode) {
    using namespace zlibNamespace;
...
//==============================================================================
GZIPCompressorOutputStream::GZIPCompressorOutputStream(OutputStream &s,
                                                       int compressionLevel,
                                                       int windowBits)
    : GZIPCompressorOutputStream(&s, compressionLevel, false, windowBits) {}

GZIPCompressorOutputStream::GZIPCompressorOutputStream(OutputStream *out,
                                                       int compressionLevel,
                                                       bool deleteDestStream,
                                                       int windowBits)
...
GZIPCompressorOutputStream::~GZIPCompressorOutputStream() { flush(); }
...
bool GZIPCompressorOutputStream::write(const void *destBuffer, size_t howMany) {
  ...
  return helper->write(static_cast<const uint8 *>(destBuffer), howMany,
                       *destStream);
}
...
struct GZIPTests final : public UnitTest {
  GZIPTests() : UnitTest("GZIP", UnitTestCategories::compression) {}
  ...
  void runTest() override {
    ...
    for (int i = 100; --i >= 0;) {
      ...
      {
        MemoryInputStream compressedInput(compressed.getData(),
                                          compressed.getDataSize(), false);
      ...
      if (original.getDataSize() == uncompressed.getDataSize())
        expect(memcmp(uncompressed.getData(), original.getData(),
