...
JUCE_BEGIN_IGNORE_WARNINGS_GCC_LIKE(
    "-Wconversion", "-Wsign-conversion", "-Wshadow", "-Wdeprecated-register",
    "-Wswitch-enum", "-Wswitch-default", "-Wredundant-decls",
    "-Wimplicit-fallthrough", "-Wzero-as-null-pointer-constant", "-Wcomma")
...
//==============================================================================
// internal helper object that holds the zlib structures so they don't have to
...
public:
  ...
  bool finished = true, needsDictionary = false, error = true,
       streamIsValid = false;
...
GZIPDecompressorInputStream::GZIPDecompressorInputStream(
    InputStream *source, bool deleteSourceWhenDestroyed, Format f,
    int64 uncompressedLength)
    : sourceStream(source, deleteSourceWhenDestroyed),
      uncompressedStreamLength(uncompressedLength), format(f),
...
GZIPDecompressorInputStream::GZIPDecompressorInputStream(InputStream &source)
    : sourceStream(&source, false), uncompressedStreamLength(-1),
      format(zlibFormat), originalSourcePos(source.getPosition()),
      buffer((size_t)GZIPDecompressHelper::gzipDecompBufferSize),
    ...
    while (!helper->error) {
      ...
      if (n == 0) {
        ...
        if (helper->needsInput()) {
          activeBufferSize = sourceStream->read(
              buffer, (int)GZIPDecompressHelper::gzipDecompBufferSize);
...
int64 GZIPDecompressorInputStream::getPosition() { return currentPos; }
...
struct GZIPDecompressorInputStreamTests final : public UnitTest {
  GZIPDecompressorInputStreamTests()
      : UnitTest("GZIPDecompressorInputStreamTests",
                 UnitTestCategories::streams) {}
  ...
  void runTest() override {
    ...
    GZIPDecompressorInputStream stream(&mi, false,
