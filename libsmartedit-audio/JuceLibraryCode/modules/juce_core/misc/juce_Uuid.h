...
/**
    ...
    The class includes methods for saving the ID as a string or as raw binary
   data.
...
public:
  ...
  /** Returns a stringified version of this UUID.
      ...
      A Uuid object can later be reconstructed from this string using operator=
     or the constructor that takes a string parameter.

      @returns a 32 character hex string.
  */
  String toString() const;

  /** Returns a stringified version of this UUID, separating it into sections
     with dashes.
  ...
  */
  ...
  /** Copies from a stringified UUID.
      The string passed in should be one that was created with the toString()
     method.
  */
...
template <> struct hash<juce::Uuid> {
  size_t operator()(const juce::Uuid &u) const noexcept {
    return (size_t)u.hash();
  }
};
} // namespace std
#endif
