...
static File resolveFilenameForOption(const ArgumentList &args, StringRef option,
                                     const String &filename) {
  if (filename.isEmpty()) {
    args.failIfOptionIsMissing(option);
    ConsoleApplication::fail("Expected a filename after the " + option +
                             " option");
  }
...
bool ArgumentList::Argument::operator!=(StringRef s) const {
  return !operator==(s);
}
...
ArgumentList::Argument ArgumentList::operator[](int index) const {
...
int ArgumentList::indexOfOption(StringRef option) const {
  jassert(option == String(option).trim()); // passing non-trimmed strings will
                                            // always fail to find a match!
...
String ArgumentList::getValueForOption(StringRef option) const {
  jassert(isOptionFormat(
      option)); // the thing you're searching for must be an option
    ...
    if (arg == option) {
      if (arg.isShortOption()) {
        if (i < arguments.size() - 1 &&
            !arguments.getReference(i + 1).isOption())
...
ConsoleApplication::findCommand(const ArgumentList &args,
...
int ConsoleApplication::findAndRunCommand(const ArgumentList &args,
                                          bool optionMustBeFirstArg) const {
...
void ConsoleApplication::addHelpCommand(String arg, String helpMessage,
...
const std::vector<ConsoleApplication::Command> &
ConsoleApplication::getCommands() const {
  return commands;
}

static String getExeNameAndArgs(const ArgumentList &args,
...
static void printCommandDescription(const ArgumentList &args,
                                    const ConsoleApplication::Command &command,
...
void ConsoleApplication::printCommandList(const ArgumentList &args) const {
  ...
  for (auto &c : commands)
    descriptionIndent =
        std::max(descriptionIndent, getExeNameAndArgs(args, c).length());
...
void ConsoleApplication::printCommandDetails(const ArgumentList &args,
