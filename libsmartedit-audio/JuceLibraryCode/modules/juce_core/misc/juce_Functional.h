...
template <typename T>
constexpr auto equalityComparableToNullptr<
    T, Void<decltype(std::declval<T>() != nullptr)>> = true;
} // namespace detail
#endif
...
/** Some helper methods for checking a callable object before invoking with
    ...
    If the object provides a comparison operator for nullptr it will check
   before calling. For other objects it will just invoke the function call
   operator.
...
struct NullCheckedInvocation {
  ...
  template <typename... Args> static void invoke(std::nullptr_t, Args &&...) {}
};

/** Can be used to disable template constructors that would otherwise cause
   ambiguity with compiler-generated copy and move constructors.

    Adapted from
   https://ericniebler.com/2013/08/07/universal-references-and-the-copy-constructo/
*/
template <typename A, typename B>
using DisableIfSameOrDerived =
    std::enable_if_t<!std::is_base_of_v<A, std::remove_reference_t<B>>>;

/** Copies an object, sets one of the copy's members to the specified value, and
 * then returns the copy. */
template <typename Object, typename OtherObject, typename Member,
          typename Other>
[[nodiscard]] Object withMember(Object copy, Member OtherObject::*member,
                                Other &&value) {
...
template <typename Functor> static constexpr auto toFnPtr(Functor functor) {
