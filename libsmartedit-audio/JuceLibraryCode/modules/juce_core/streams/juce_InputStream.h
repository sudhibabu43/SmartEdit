...
/** The base class for streams that read data.
   ...
   override some or all of the virtual functions to implement their behaviour.
...
public:
  ...
  */
  ...
  /** Reads some data from the stream into a memory buffer.
     ...
     as the InputStream base class implements the other read methods in terms of
     this one (although it's often more efficient for subclasses to implement
     them directly).
  ...
  */
  ...
  /** Reads two bytes from the stream as a little-endian 16-bit value.
     ...
     (byte2 << 8)). If the stream is exhausted partway through reading the
     bytes, this will return zero.
  ...
  */
  ...
  /** Reads two bytes from the stream as a little-endian 16-bit value.
     ...
     (byte1 << 8)). If the stream is exhausted partway through reading the
     bytes, this will return zero.
  ...
  */
  ...
  /** Reads four bytes as a 32-bit floating point value.
     ...
     little-endian int. If the stream is exhausted partway through reading the
     bytes, this will return zero.
  ...
  */
  ...
  /** Reads four bytes as a 32-bit floating point value.
      The raw 32-bit encoding of the float is read from the stream as a
     big-endian int. If the stream is exhausted partway through reading the
     bytes, this will return zero.
  ...
  */
  ...
  /** Reads eight bytes as a 64-bit floating point value.
     ...
     little-endian int64. If the stream is exhausted partway through reading the
     bytes, this will return zero.
  ...
  */
  ...
  /** Reads eight bytes as a 64-bit floating point value.
      The raw 64-bit encoding of the double is read from the stream as a
     big-endian int64. If the stream is exhausted partway through reading the
     bytes, this will return zero.
  ...
  */
  ...
  /** Reads an encoded 32-bit number from the stream using a space-saving
     compressed format. For small values, this is more space-efficient than
     using readInt() and OutputStream::writeInt() The format used is: number of
     significant bytes + up to 4 bytes in little-endian order.
      @see OutputStream::writeCompressedInt()
  */
  virtual int readCompressedInt();

  //==============================================================================
  /** Reads a UTF-8 string from the stream, up to the next linefeed or carriage
     return.

      This will read up to the next "\n" or "\r\n" or end-of-stream.

      After this call, the stream's position will be left pointing to the next
     character following the line-feed, but the linefeeds aren't included in the
  ...
  */
  ...
  /** Tries to read the whole stream and turn it into a string.
      ...
      It can read from UTF-8 data, or UTF-16 if it detects suitable
     header-bytes.
  */
  ...
  /** Reads from the stream and appends the data to a MemoryBlock.
     ...
     to the number of bytes that will be read - if it's negative, data will be
