...
public:
  ...
  /** Returns a new InputStream to read this item.
      ...
      @returns            an inputstream that the caller will delete, or nullptr
     if the document can't be opened.
  */
  ...
  /** @internal
      ...
      @param relatedItemPath  the relative pathname of the resource that is
     required
      @returns            an input stream if relatedItemPath was empty,
     otherwise nullptr.
  */
  InputStream *createInputStreamFor(const String &relatedItemPath) override {
    return relatedItemPath.isEmpty() ? document.createInputStream().release()
                                     : nullptr;
  }
