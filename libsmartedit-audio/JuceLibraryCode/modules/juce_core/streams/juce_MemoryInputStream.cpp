...
MemoryInputStream::MemoryInputStream(const void *sourceData,
                                     size_t sourceDataSize, bool keepCopy)
    : data(sourceData), dataSize(sourceDataSize) {
  if (keepCopy) {
...
MemoryInputStream::MemoryInputStream(const MemoryBlock &sourceData,
                                     bool keepCopy)
    : data(sourceData.getData()), dataSize(sourceData.getSize()) {
  if (keepCopy) {
...
public:
  MemoryStreamTests()
      : UnitTest("MemoryInputStream & MemoryOutputStream",
