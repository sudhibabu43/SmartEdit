...
SubregionStream::SubregionStream(InputStream *sourceStream, int64 start,
                                 int64 length, bool deleteSourceWhenDestroyed)
    : source(sourceStream, deleteSourceWhenDestroyed),
      startPositionInSourceStream(start), lengthOfSourceStream(length) {
...
bool SubregionStream::setPosition(int64 newPosition) {
  return source->setPosition(
      jmax((int64)0, newPosition + startPositionInSourceStream));
}
...
int SubregionStream::read(void *destBuffer, int maxBytesToRead) {
  ...
  maxBytesToRead =
      (int)jmin((int64)maxBytesToRead, lengthOfSourceStream - getPosition());
...
struct SubregionInputStreamTests final : public UnitTest {
  ...
  void runTest() override {
    ...
    const MemoryBlock memoryBlockToCheck(data.begin() + (size_t)offset,
