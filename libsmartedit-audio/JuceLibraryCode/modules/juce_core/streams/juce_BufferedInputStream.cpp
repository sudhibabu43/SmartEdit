...
static int calcBufferStreamBufferSize(int requestedSize,
                                      InputStream *source) noexcept {
...
char BufferedInputStream::peekByte() {
  ...
  return position < lastReadPos
             ? buffer[(int)(position - bufferedRange.getStart())]
             : 0;
}

int64 BufferedInputStream::getTotalLength() { return source->getTotalLength(); }

int64 BufferedInputStream::getPosition() { return position; }
...
bool BufferedInputStream::ensureBuffered() {
  ...
  if (position < bufferedRange.getStart() || position >= bufferEndOverlap) {
    ...
    if (position < lastReadPos && position >= bufferEndOverlap &&
        position >= bufferedRange.getStart()) {
      auto bytesToKeep = (int)(lastReadPos - position);
      memmove(buffer, buffer + (int)(position - bufferedRange.getStart()),
              (size_t)bytesToKeep);

      bytesRead =
          source->read(buffer + bytesToKeep, (int)(bufferLength - bytesToKeep));
...
int BufferedInputStream::read(void *destBuffer, const int maxBytesToRead) {
  ...
  const auto readFromReservoir =
      [this, &destBuffer, &initialPosition](const Range<int64> rangeToRead) {
        memcpy(static_cast<char *>(destBuffer) +
                   (rangeToRead.getStart() - initialPosition),
  ...
  const auto remaining = Reservoir::doBufferedRead(
      Range<int64>(position, position + maxBytesToRead), getBufferedRange,
      readFromReservoir, fillReservoir);
...
String BufferedInputStream::readString() {
  if (position >= bufferedRange.getStart() && position < lastReadPos) {
...
struct BufferedInputStreamTests final : public UnitTest {
  ...
  static void allCombinationsImpl(Fn &&fn, Values &&values, Range &&range,
                                  Ranges &&...ranges) {
    for (auto &item : range)
      allCombinationsImpl(fn, std::tuple_cat(values, std::tie(item)),
                          ranges...);
  }
  ...
  void runTest() override {
    ...
    const auto runTest = [this](const MemoryBlock &data, const int readSize,
                                const bool peek) {
      ...
      while (numBytesRead < data.getSize()) {
        ...
        expect(std::equal(
            readBuffer.begin() + startingPos, readBuffer.begin() + numBytesRead,
            data.begin() + startingPos, data.begin() + numBytesRead));
        expectEquals(stream.getPosition(), (int64)numBytesRead);
        expectEquals(stream.getNumBytesRemaining(),
                     (int64)(data.getSize() - numBytesRead));
