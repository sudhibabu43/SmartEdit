...
public:
  ...
  /** Creates an empty memory stream, ready to be written into.
      @param initialSize  the initial amount of capacity to allocate for writing
     into
  */
  ...
  /** Creates a memory stream for writing into into a pre-existing MemoryBlock
     ...
     data that has been written to the stream, because the MemoryOutputStream
     keeps some spare capacity at its end. To trim the block's size down to fit
     the actual data, call flush(), or delete the MemoryOutputStream.
     ...
     the block will be kept, and new data will be appended to it. If false, the
     block will be cleared before use
  */
  ...
  /** Creates a MemoryOutputStream that will write into a user-supplied,
     fixed-size block of memory. When using this mode, the stream will write
     directly into this memory area until it's full, at which point write
  ...
  */
  ...
  /** Increases the internal storage capacity to be able to contain at least the
     specified amount of data without needing to be resized.
  */
  ...
  /** Returns a String created from the (UTF8) data that has been written to the
   * stream. */
  String toUTF8() const;
  ...
  */
  ...
  /** If the stream is writing to a user-supplied MemoryBlock, this will trim
     any excess capacity off the block, so that its length matches the amount of
     actual data that has been written so far.
  */
  void flush() override;
