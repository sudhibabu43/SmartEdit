...
struct ThreadPool::ThreadPoolThread final : public Thread {
  ThreadPoolThread(ThreadPool &p, const Options &options)
      : Thread{options.threadName, options.threadStackSizeBytes}, pool{p} {}
...
ThreadPoolJob::~ThreadPoolJob() {
  // you mustn't delete a job while it's still in a pool! Use
  // ThreadPool::removeJob() to remove it first!
  jassert(pool == nullptr || !pool->contains(this));
}

String ThreadPoolJob::getJobName() const { return jobName; }

void ThreadPoolJob::setJobName(const String &newName) { jobName = newName; }
...
ThreadPoolJob *ThreadPoolJob::getCurrentThreadPoolJob() {
  if (auto *t = dynamic_cast<ThreadPool::ThreadPoolThread *>(
...
ThreadPool::ThreadPool(int numberOfThreads, size_t threadStackSizeBytes,
...
void ThreadPool::addJob(std::function<void()> jobToRun) {
  struct LambdaJobWrapper final : public ThreadPoolJob {
    LambdaJobWrapper(std::function<void()> j)
        : ThreadPoolJob("lambda"), job(std::move(j)) {}
...
int ThreadPool::getNumThreads() const noexcept { return threads.size(); }
...
bool ThreadPool::waitForJobToFinish(const ThreadPoolJob *job,
  ...
  if (job != nullptr) {
    ...
    while (contains(job)) {
      if (timeOutMs >= 0 &&
          Time::getMillisecondCounter() >= start + (uint32)timeOutMs)
        return false;
...
bool ThreadPool::removeJob(ThreadPoolJob *job, bool interruptIfRunning,
                           int timeOutMs) {
  bool dontWait = true;
...

  {
...

    {
      ...
      for (int i = jobs.size(); --i >= 0;) {
        ...
        if (selectedJobsToRemove == nullptr ||
            selectedJobsToRemove->isJobSuitable(job)) {
          if (job->isActive) {
...
void ThreadPool::addToDeleteList(OwnedArray<ThreadPoolJob> &deletionList,
                                 ThreadPoolJob *job) const {
  job->shouldStop = true;
  job->pool = nullptr;
