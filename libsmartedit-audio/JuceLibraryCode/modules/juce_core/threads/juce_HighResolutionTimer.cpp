...
public:
  explicit Impl(HighResolutionTimer &o) : owner{o} {}
  ...
  bool isTimerRunning() const { return getIntervalMs() > 0; }
...
void HighResolutionTimer::stopTimer() { impl->startTimer(0); }
...
public:
  ...
  void runTest() override {
    ...
    beginTest("Calling stopTimer on a timer, waits for any timer callbacks to "
              "finish");
    {
    ...
    beginTest("Calling stopTimer on a timer, waits for any timer callbacks to "
              "finish, even if the timer callback calls stopTimer first");
    {
    ...
    beginTest("Adjusting a timer period from outside the timer callback "
    ...
    beginTest("Calls to `startTimer` and `getTimerInterval` succeed while a "
              "callback is blocked");
    {
  ...
  public:
    explicit Timer(std::function<void()> fn) : callback(std::move(fn)) {}
