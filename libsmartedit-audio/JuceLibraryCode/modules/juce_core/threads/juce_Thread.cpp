...
Thread::~Thread() {
  ...
  /* If your thread class's destructor has been called without first stopping
     the thread, that means that this partially destructed object is still
     performing some work - and that's probably a Bad Thing!

     To avoid this type of nastiness, always make sure you call stopThread()
     before or during your subclass's destructor.
  */
...
//==============================================================================
// Use a ref-counted object to hold this shared data, so that it can outlive its
// static shared pointer when threads are still running during static shutdown.
...
static char currentThreadHolderLock[sizeof(
    SpinLock)]; // (statically initialised to zeros).
...
static CurrentThreadHolder::Ptr getCurrentThreadHolder() {
  ...
  SpinLock::ScopedLockType lock(
      *castToSpinLockWithoutAliasingWarning(currentThreadHolderLock));
...
void Thread::threadEntryPoint() {
  ...
  // This 'startSuspensionEvent' protects 'threadId' which is initialised after
  // the platform's native 'CreateThread' method. This ensures it has been
  // initialised correctly before it reaches this point.
...
bool Thread::startThread() { return startThread(Priority::normal); }
...
bool Thread::isThreadRunning() const { return threadHandle != nullptr; }

Thread *JUCE_CALLTYPE Thread::getCurrentThread() {
  return getCurrentThreadHolder()->value.get();
}

Thread::ThreadID Thread::getThreadId() const noexcept { return threadId; }
...
bool Thread::stopThread(const int timeOutMilliseconds) {
  ...
  if (isThreadRunning()) {
    ...
    if (isThreadRunning()) {
      ...
      // locks and events left in silly states when a thread is killed by
      // force..
      jassertfalse;
...
void Thread::addListener(Listener *listener) { listeners.add(listener); }

void Thread::removeListener(Listener *listener) { listeners.remove(listener); }

bool Thread::isRealtime() const { return realtimeOptions.has_value(); }
...
void Thread::notify() const { defaultEvent.signal(); }

//==============================================================================
struct LambdaThread final : public Thread {
  LambdaThread(std::function<void()> &&f)
      : Thread("anonymous"), fn(std::move(f)) {}

  void run() override {
    fn();
    fn = nullptr; // free any objects that the lambda might contain while the
...
public:
  AtomicTests() : UnitTest("Atomics", UnitTestCategories::threads) {}
  ...
  void runTest() override {
    ...
    expect(ByteOrder::swap((uint64)0x1122334455667788ULL) ==
           (uint64)0x8877665544332211LL);
...
#if !JUCE_64BIT_ATOMICS_UNAVAILABLE // 64-bit intrinsics aren't available on
                                    // some old platforms
  ...
  template <typename Type> class AtomicTester {
  public:
...
//==============================================================================
class ThreadLocalValueUnitTest final : public UnitTest, private Thread {
public:
