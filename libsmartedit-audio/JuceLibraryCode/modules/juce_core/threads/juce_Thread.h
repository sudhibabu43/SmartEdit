...
public:
  ...
  struct RealtimeOptions {
    ...
    */
    [[nodiscard]] RealtimeOptions withPriority(int newPriority) const {
      ...
      return withMember(*this, &RealtimeOptions::priority,
                        juce::jlimit(0, 10, newPriority));
    }

    /** Specify the expected amount of processing time required each time the
       thread wakes up.

        Only used by macOS/iOS.

        @see getProcessingTimeMs, withMaximumProcessingTimeMs, withPeriodMs,
       withPeriodHz
    */
    [[nodiscard]] RealtimeOptions
    withProcessingTimeMs(double newProcessingTimeMs) const {
      jassert(newProcessingTimeMs > 0.0);
      return withMember(*this, &RealtimeOptions::processingTimeMs,
                        newProcessingTimeMs);
    }

    /** Specify the maximum amount of processing time required each time the
       thread wakes up.

        Only used by macOS/iOS.

        @see getMaximumProcessingTimeMs, withProcessingTimeMs, withPeriodMs,
       withPeriodHz
    */
    [[nodiscard]] RealtimeOptions
    withMaximumProcessingTimeMs(double newMaximumProcessingTimeMs) const {
      jassert(newMaximumProcessingTimeMs > 0.0);
      return withMember(*this, &RealtimeOptions::maximumProcessingTimeMs,
    ...
    /** Specify the maximum amount of processing time required each time the
       thread wakes up.
        ...
        This is identical to 'withMaximumProcessingTimeMs' except it calculates
       the processing time from a sample rate and block size. This is useful if
       you want to run this thread in parallel to an audio device thread.
    ...
    */
    [[nodiscard]] RealtimeOptions
    withApproximateAudioProcessingTime(int samplesPerFrame,
    ...
    /** Specify the approximate amount of time between each thread wake up.
        ...
        @see getPeriodMs, withPeriodHz, withProcessingTimeMs,
    ...
    */
    ...
    /** Specify the approximate frequency at which the thread will be woken up.
        ...
        @see getPeriodHz, withPeriodMs, withProcessingTimeMs,
       withMaximumProcessingTimeMs,
    */
    ...
    */
    ...
    /** Returns the expected amount of processing time required each time the
       thread wakes up.
    ...
    */
    ...
    /** Returns the maximum amount of processing time required each time the
       thread wakes up.
    ...
    */
    ...
    /** Returns the approximate amount of time between each thread wake up, or
        ...
        @see withPeriodMs, withPeriodHz, getProcessingTimeMs,
       getMaximumProcessingTimeMs
    */
    [[nodiscard]] std::optional<double> getPeriodMs() const { return periodMs; }
  ...
  */
  ...
  /** Destructor.
     ...
     - always call stopThread() and make sure your thread has stopped before
     deleting the object. Failing to do so will throw an assertion, and put you
     firmly into undefined behaviour territory.
  */
  virtual ~Thread();
  ...
  */
  ...
  /** Attempts to start a new thread with default ('Priority::normal') priority.
      ...
      If a thread cannot be created with the requested priority, this will
     return false and Thread::run() will not be called. An exception to this is
     the Android platform, which always starts a thread and attempts to upgrade
     the thread after creation.

      @returns    true if the thread started successfully. false if it was
     unsuccesful.
  ...
  */
  ...
  /** Attempts to start a new thread with a given priority.
     ...
     the thread after creation.
      ...
      @param newPriority    Priority the thread should be assigned. This
  ...
  */
  ...
  /** Attempts to stop the thread running.
      ...
      @param timeOutMilliseconds  The number of milliseconds to wait for the
                                  thread to finish before killing it by force. A
     negative value in here will wait forever.
      @returns    true if the thread was cleanly stopped before the timeout, or
     false if it had to be killed by force.
      @see signalThreadShouldExit, threadShouldExit, waitForThreadToExit,
     isThreadRunning
  */
  ...
  /** Invokes a lambda or function on its own thread with the default priority.
      ...
      Bear in mind that starting and stopping a thread can be a fairly
     heavyweight operation, so you might prefer to use a ThreadPool if you're
     kicking off a lot of short background tasks.

      Also note that using an anonymous thread makes it very difficult to
     interrupt the function when you need to stop it, e.g. when your app quits.
     So it's up to you to deal with situations where the function may fail to
     stop in time.

      @param functionToRun  The lambda to be called from the new Thread.

      @returns    true if the thread started successfully, or false if it
     failed.
  ...
  */
  ...
  /** Sets a flag to tell the thread it should stop.
      ...
      Calling this means that the threadShouldExit() method will then return
     true. The thread should be regularly checking this to see whether it should
     exit.

      If your thread makes use of wait(), you might want to call notify() after
     calling this method, to interrupt any waits that might be in progress, and
     allow it to reach a point where it can exit.
  ...
  */
  ...
  /** Waits for the thread to stop.
      This will wait until isThreadRunning() is false or until a timeout
     expires.

      @param timeOutMilliseconds  the time to wait, in milliseconds. If this
     value is less than zero, it will wait forever.
      @returns    true if the thread exits, or false if the timeout expires
     first.
  */
  ...
  */
  ...
  /** Sets the affinity mask for the thread.
      ...
      This will only have an effect next time the thread is started - i.e. if
     the thread is already running when called, it'll have no effect.
  ...
  */
  ...
  /** Changes the affinity mask for the caller thread.
      ...
      This will change the affinity mask for the thread that calls this static
     method.
  ...
  */
  ...
  //==============================================================================
  /** Suspends the execution of the current thread until the specified timeout
     period has elapsed (note that this may not be exact).

      The timeout period must not be negative and whilst sleeping the thread
     cannot be woken up so it should only be used for short periods of time and
     when other methods such as using a WaitableEvent or CriticalSection are not
     possible.
  */
  ...
  /** Yields the current thread's CPU time-slot and allows a new thread to run.
      ...
      If there are no other threads of equal or higher priority currently
     running then this will return immediately and the current thread will
     continue to run.
  */
  static void JUCE_CALLTYPE yield();

  //==============================================================================
  /** Suspends the execution of this thread until either the specified timeout
     period has elapsed, or another thread calls the notify() method to wake it
     up.

      A negative timeout value means that the method will wait indefinitely.

      @returns    true if the event has been signalled, false if the timeout
     expires.
  */
  ...
  */
  ...
  /** Finds the thread object that is currently running.
      ...
      Note that the main UI thread (or other non-JUCE threads) don't have a
     Thread object associated with them, so this will return nullptr.
  */
  static Thread *JUCE_CALLTYPE getCurrentThread();

  /** Returns the ID of this thread.

      That means the ID of this thread object - not of the thread that's calling
     the method. This can change when the thread is started and stopped, and
     will be invalid if the thread's not actually running.

      @see getCurrentThreadId
  */
  ThreadID getThreadId() const noexcept;

  /** Returns the name of the thread. This is the name that gets set in the
   * constructor. */
  ...
  */
  ...
  /** Initialises the JUCE subsystem for projects not created by the Projucer
      ...
      On Android, JUCE needs to be initialised once before it is used. The
     Projucer will automatically generate the necessary java code to do this.
     However, if you are using JUCE without the Projucer or are creating a
     library made with JUCE intended for use in non-JUCE apks, then you must
     call this method manually once on apk startup.
      ...
      Note that the above java method is only available in Android Studio
     projects created by the Projucer. If you need to call this from another
     type of project then you need to add the following java file to your
     project:
      ...
      @param jContext this is a jobject referring to your app/service/receiver/
                      provider's Context. JUCE needs this for many of it's
     internal functions.
  */
...
protected:
  ...
  /** Returns the current priority of this thread.
      ...
      This can only be called from the target thread. Doing so from another
     thread will cause an assert.

      @see setPriority
  */
  Priority getPriority() const;

  /** Attempts to set the priority for this thread. Returns true if the new
     priority was set successfully, false if not.

      This can only be called from the target thread. Doing so from another
     thread will cause an assert.

      @param newPriority The new priority to be applied to the thread. Note:
     This has no effect on Linux platforms, subsequent calls to 'getPriority'
