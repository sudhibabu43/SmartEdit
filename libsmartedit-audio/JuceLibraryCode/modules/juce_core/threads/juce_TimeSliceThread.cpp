...
TimeSliceThread::~TimeSliceThread() { stopThread(2000); }
...
void TimeSliceThread::addTimeSliceClient(TimeSliceClient *const client,
  ...
  if (client != nullptr) {
    ...
    client->nextCallTime =
        Time::getCurrentTime() +
        RelativeTime::milliseconds(millisecondsBeforeStarting);
...

    {
      ...
      if (numClients > 0) {
        ...
        if (nextClientTime > now) {
          timeToWait =
              (int)jmin((int64)500, (nextClientTime - now).inMilliseconds());
        } else {
          ...
          if (clientBeingCalled != nullptr) {
            ...
            if (msUntilNextCall >= 0)
              clientBeingCalled->nextCallTime =
                  now + RelativeTime::milliseconds(msUntilNextCall);
            else
