...
//==============================================================================
static String translateTimeField(int n, const char *singular,
                                 const char *plural) {
  return TRANS(n == 1 ? singular : plural)
...
String
RelativeTime::getDescription(const String &returnValueForZeroTime) const {
  ...
  if (fields.size() < 2) {
    ...
    if (fields.size() < 2) {
      ...
      if (fields.size() < 2) {
        ...
        if (fields.isEmpty())
          fields.add(String(((int)inMilliseconds()) % 1000) + " " +
                     TRANS("ms"));
      }
    }
  }
