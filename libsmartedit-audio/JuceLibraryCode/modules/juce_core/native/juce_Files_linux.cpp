...
bool File::isOnHardDisk() const {
  ...
  if (statfs(getFullPathName().toUTF8(), &buf) == 0) {
    ...
    case U_MSDOS_SUPER_MAGIC: // Probably floppy (but could be mounted FAT
                              // filesystem)
...
//==============================================================================
static File resolveXDGFolder(const char *const type,
...
bool File::moveToTrash() const {
  ...
  return moveFileTo(trashCan.getNonexistentChildFile(
...
static bool isFileExecutable(const String &filename) {
  ...
  return juce_stat(filename, info) && S_ISREG(info.st_mode) &&
