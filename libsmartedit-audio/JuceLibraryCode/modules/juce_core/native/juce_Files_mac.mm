...
static bool isHiddenFile(const String &path) {
  ...
  JUCE_AUTORELEASEPOOL {
    ...
    return [createNSURLFromFile(path) getResourceValue:&hidden
...
static String getIOSSystemLocation(NSSearchPathDirectory type) {
  return nsStringToJuce([NSSearchPathForDirectoriesInDomains(
      type, NSUserDomainMask, YES) objectAtIndex:0]);
}
#else
static bool launchExecutable(const String &pathAndArguments) {
  ...
  if (cpid == 0) {
    const char *const argv[4] = {"/bin/sh", "-c", pathAndArguments.toUTF8(),
                                 nullptr};
...
bool File::isOnCDRomDrive() const {
  static const char *const cdTypes[] = {"cd9660", "cdfs", "cddafs", "udf",
                                        nullptr};
...
bool File::isOnHardDisk() const {
  ...
  return !(isOnCDRomDrive() ||
           MacFileHelpers::isFileOnDriveType(*this, nonHDTypes));
}
...
File File::getSpecialLocation(const SpecialLocationType type) {
  JUCE_AUTORELEASEPOOL {
    ...
    case tempDirectory: {
      ...
      tmp = tmp.getChildFile(
          juce_getExecutableFile().getFileNameWithoutExtension());
      tmp.createDirectory();
    ...
    case invokedExecutableFile:
      if (juce_argv != nullptr && juce_argc > 0)
        return File::getCurrentWorkingDirectory().getChildFile(
            String(CharPointer_UTF8(juce_argv[0])));
...
String File::getVersion() const {
  JUCE_AUTORELEASEPOOL {
    if (NSBundle *bundle =
            [NSBundle bundleWithPath:juceStringToNS(getFullPathName())])
      if (NSDictionary *info = [bundle infoDictionary])
        if (NSString *name = [info
                valueForKey:nsStringLiteral("CFBundleShortVersionString")])
...
static NSString *getFileLink(const String &path) {
  return [[NSFileManager defaultManager]
      destinationOfSymbolicLinkAtPath:juceStringToNS(path)
                                error:nil];
}

bool File::isSymbolicLink() const { return getFileLink(fullPath) != nil; }
...
bool File::moveToTrash() const {
  ...
  JUCE_AUTORELEASEPOOL {
    if (@available(macOS 10.8, iOS 11.0, *)) {
      NSError *error = nil;
      return [[NSFileManager defaultManager]
            trashItemAtURL:createNSURLFromFile(*this)
...
#else
    [[NSWorkspace sharedWorkspace]
              recycleURLs:[NSArray arrayWithObject:createNSURLFromFile(*this)]
        completionHandler:nil];

    // recycleURLs is async, so we need to block until it has finished. We can't
    // use a run-loop here because it'd dispatch unexpected messages, so have to
    // do this very nasty bodge. But this is only needed for support of pre-10.8
    // versions.
  ...
  Pimpl(const File &directory, const String &wildcard)
    ...
    JUCE_AUTORELEASEPOOL {
      enumerator = [[[NSFileManager defaultManager]
          enumeratorAtPath:juceStringToNS(directory.getFullPathName())] retain];
    }
  }

  ~Pimpl() { [enumerator release]; }
  ...
  bool next(String &filenameFound, bool *const isDir, bool *const isHidden,
    ...
    JUCE_AUTORELEASEPOOL {
      ...
      for (;;) {
        ...
        updateStatInfoForFile(fullPath, isDir, fileSize, modTime, creationTime,
                              isReadOnly);
...
//==============================================================================
bool JUCE_CALLTYPE Process::openDocument(
    const String &fileName, [[maybe_unused]] const String &parameters) {
  JUCE_AUTORELEASEPOOL {
    NSString *fileNameAsNS(juceStringToNS(fileName));
    NSURL *filenameAsURL =
        File::createFileWithoutCheckingPath(fileName).exists()
            ? [NSURL fileURLWithPath:fileNameAsNS]
    ...
    if (file.exists())
      return MacFileHelpers::launchExecutable("\"" + fileName + "\" " +
                                              parameters);
...
#if !JUCE_IOS
  if (exists())
    [[NSWorkspace sharedWorkspace] selectFile:juceStringToNS(getFullPathName())
                     inFileViewerRootedAtPath:nsEmptyString()];
...
OSType File::getMacOSType() const {
  JUCE_AUTORELEASEPOOL {
    NSDictionary *fileDict = [[NSFileManager defaultManager]
        attributesOfItemAtPath:juceStringToNS(getFullPathName())
                         error:nil];
...
#if JUCE_IOS
  return false; // xxx can't find a sensible way to do this without trying to
                // open the bundle..
#else
  JUCE_AUTORELEASEPOOL {
    return [[NSWorkspace sharedWorkspace]
        isFilePackageAtPath:juceStringToNS(getFullPathName())];
  }
#endif
}
...
File File::getContainerForSecurityApplicationGroupIdentifier(
    const String &appGroup) {
  if (@available(macOS 10.8, *))
    if (auto *url = [[NSFileManager defaultManager]
            containerURLForSecurityApplicationGroupIdentifier:juceStringToNS(
                                                                  appGroup)])
