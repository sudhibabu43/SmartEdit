...
//==============================================================================
template <typename JavaType> class LocalRef {
public:
  LocalRef() noexcept = default;

  /*  This constructor must not be used to wrap local references that were not
     created through JNI, i.e. for native function callback parameters.
  */
  explicit LocalRef(JavaType o) noexcept : LocalRef(o, false) {}

  LocalRef(const LocalRef &other) noexcept : obj(retain(other.obj)) {}
  LocalRef(LocalRef &&other) noexcept : obj(nullptr) {
  ...
  auto release() { return std::exchange(obj, nullptr); }

  /** Creates a new internal local reference. */
  static auto addOwner(JavaType o) { return LocalRef{o, true}; }

  /** Takes ownership of the passed in local reference, and deletes it when the
     LocalRef goes out of scope.
  */
  static auto becomeOwner(JavaType o) { return LocalRef{o, false}; }
...
private:
  ...
  /*  We cannot delete local references that were not created by JNI, e.g.
     references that were created by the VM and passed into the native function.

      For these references we should use createNewLocalRef = true, which will
     create a new local reference that this wrapper is allowed to delete.

      Doing otherwise will result in an "Attempt to remove non-JNI local
     reference" warning in the VM, which could even cause crashes in future VM
     implementations.
  */
...
/*  Creates a new local reference that shares ownership with the passed in
   pointer.

    Can be used for wrapping function parameters that were created outside the
   JNI.
*/
template <class JavaType> auto addLocalRefOwner(JavaType t) {
  return LocalRef<JavaType>::addOwner(t);
}

/*   Wraps a local reference and destroys it when it goes out of scope. */
template <class JavaType> auto becomeLocalRefOwner(JavaType t) {
  return LocalRef<JavaType>::becomeOwner(t);
}

//==============================================================================
template <typename JavaType> class GlobalRefImpl {
public:
  ...
  GlobalRefImpl(GlobalRefImpl &&other) noexcept : obj(nullptr) {
...
#define DECLARE_CALL_TYPE_METHOD(returnType, typeName)                         \
  returnType call##typeName##Method(jmethodID methodID, ...) const {           \
    ...
    returnType result =                                                        \
        getEnv()->Call##typeName##MethodV(obj, methodID, args);                \
    va_end(args);                                                              \
    return result;                                                             \
  }
...
class JNIClassBase {
public:
  JNIClassBase(const char *classPath, int minSDK, const uint8 *byteCode,
               size_t byteCodeSize);
...
protected:
  ...
  jmethodID resolveStaticMethod(JNIEnv *, const char *methodName,
                                const char *params);
  jfieldID resolveField(JNIEnv *, const char *fieldName, const char *signature);
  jfieldID resolveStaticField(JNIEnv *, const char *fieldName,
...
//==============================================================================
template <typename T, size_t N> constexpr auto numBytes(const T (&)[N]) {
  return N;
}
...
#define DECLARE_JNI_CLASS_WITH_BYTECODE(CppClassName, javaPath, minSDK,        \
                                        byteCodeData)                          \
  class CppClassName##_Class : public JNIClassBase {                           \
  public:                                                                      \
    CppClassName##_Class()                                                     \
        : JNIClassBase(javaPath, minSDK, byteCodeData,                         \
                       numBytes(byteCodeData)) {}                              \
                                                                               \
    void initialiseFields(JNIEnv *env) {                                       \
      ...
      JNI_CLASS_MEMBERS(CREATE_JNI_METHOD, CREATE_JNI_STATICMETHOD,            \
                        CREATE_JNI_FIELD, CREATE_JNI_STATICFIELD,              \
                        CREATE_JNI_CALLBACK);                                  \
      resolveCallbacks(env, callbacks);                                        \
    }                                                                          \
                                                                               \
    JNI_CLASS_MEMBERS(DECLARE_JNI_METHOD, DECLARE_JNI_METHOD,                  \
                      DECLARE_JNI_FIELD, DECLARE_JNI_FIELD,                    \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(getSystemService, "getSystemService",                                 \
         "(Ljava/lang/String;)Ljava/lang/Object;")                             \
  METHOD(getPackageManager, "getPackageManager",                               \
  ...
  METHOD(bindService, "bindService",                                           \
         "(Landroid/content/Intent;Landroid/content/ServiceConnection;I)Z")    \
  METHOD(unbindService, "unbindService",                                       \
         "(Landroid/content/ServiceConnection;)V")                             \
  METHOD(startActivity, "startActivity", "(Landroid/content/Intent;)V")        \
  METHOD(getContentResolver, "getContentResolver",                             \
         "()Landroid/content/ContentResolver;")                                \
  METHOD(getApplicationContext, "getApplicationContext",                       \
         "()Landroid/content/Context;")                                        \
  METHOD(getApplicationInfo, "getApplicationInfo",                             \
         "()Landroid/content/pm/ApplicationInfo;")                             \
  METHOD(checkCallingOrSelfPermission, "checkCallingOrSelfPermission",         \
         "(Ljava/lang/String;)I")                                              \
  METHOD(checkCallingOrSelfUriPermission, "checkCallingOrSelfUriPermission",   \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(getWindowManager, "getWindowManager",                                 \
         "()Landroid/view/WindowManager;")                                     \
  METHOD(setRequestedOrientation, "setRequestedOrientation", "(I)V")           \
  METHOD(startIntentSenderForResult, "startIntentSenderForResult",             \
         "(Landroid/content/IntentSender;ILandroid/content/Intent;III)V")      \
  METHOD(moveTaskToBack, "moveTaskToBack", "(Z)Z")                             \
  METHOD(startActivityForResult, "startActivityForResult",                     \
         "(Landroid/content/Intent;I)V")                                       \
  METHOD(getFragmentManager, "getFragmentManager",                             \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(setContentType, "setContentType",                                     \
         "(I)Landroid/media/AudioAttributes$Builder;")                         \
  METHOD(setUsage, "setUsage", "(I)Landroid/media/AudioAttributes$Builder;")

DECLARE_JNI_CLASS_WITH_MIN_SDK(AndroidAudioAttributesBuilder,
                               "android/media/AudioAttributes$Builder", 21)
#undef JNI_CLASS_MEMBERS

#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  METHOD(abandonAudioFocus, "abandonAudioFocus",                               \
         "(Landroid/media/AudioManager$OnAudioFocusChangeListener;)I")         \
  METHOD(requestAudioFocus, "requestAudioFocus",                               \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  STATICMETHOD(createBitmapFrom, "createBitmap",                               \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  STATICMETHOD(decodeByteArray, "decodeByteArray",                             \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(getCharSequence, "getCharSequence",                                   \
  ...
  METHOD(getParcelable, "getParcelable",                                       \
         "(Ljava/lang/String;)Landroid/os/Parcelable;")                        \
  METHOD(getString, "getString", "(Ljava/lang/String;)Ljava/lang/String;")     \
  METHOD(getStringArrayList, "getStringArrayList",                             \
  ...
  METHOD(putStringArrayList, "putStringArrayList",                             \
...
#define JUCE_LOG_JNI_REFERENCES_TABLE                                          \
  getEnv()->CallStaticVoidMethod(AndroidDebug,                                 \
                                 AndroidDebug.dumpReferenceTables);
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  STATICMETHOD(createChooser, "createChooser",                                 \
               "(Landroid/content/Intent;Ljava/lang/CharSequence;)Landroid/"   \
               "content/Intent;")                                              \
  METHOD(addCategory, "addCategory",                                           \
         "(Ljava/lang/String;)Landroid/content/Intent;")                       \
  METHOD(constructor, "<init>", "()V")                                         \
  METHOD(constructorWithContextAndClass, "<init>",                             \
  ...
  METHOD(getStringExtra, "getStringExtra",                                     \
         "(Ljava/lang/String;)Ljava/lang/String;")                             \
  METHOD(                                                                      \
      putExtra, "putExtra",                                                    \
      "(Ljava/lang/String;Ljava/lang/CharSequence;)Landroid/content/Intent;")  \
  METHOD(putExtras, "putExtras",                                               \
         "(Landroid/os/Bundle;)Landroid/content/Intent;")                      \
  METHOD(putExtraString, "putExtra",                                           \
         "(Ljava/lang/String;Ljava/lang/String;)Landroid/content/Intent;")     \
  METHOD(putExtraStrings, "putExtra",                                          \
         "(Ljava/lang/String;[Ljava/lang/String;)Landroid/content/Intent;")    \
  METHOD(                                                                      \
      putExtraParcelable, "putExtra",                                          \
      "(Ljava/lang/String;Landroid/os/Parcelable;)Landroid/content/Intent;")   \
  METHOD(putExtraBool, "putExtra",                                             \
         "(Ljava/lang/String;Z)Landroid/content/Intent;")                      \
  METHOD(putExtraInt, "putExtra",                                              \
         "(Ljava/lang/String;I)Landroid/content/Intent;")                      \
  METHOD(putParcelableArrayListExtra, "putParcelableArrayListExtra",           \
         "(Ljava/lang/String;Ljava/util/ArrayList;)Landroid/content/Intent;")  \
  METHOD(setAction, "setAction",                                               \
         "(Ljava/lang/String;)Landroid/content/Intent;")                       \
  METHOD(setFlags, "setFlags", "(I)Landroid/content/Intent;")                  \
  METHOD(setPackage, "setPackage",                                             \
         "(Ljava/lang/String;)Landroid/content/Intent;")                       \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  STATICMETHOD(createChooser, "createChooser",                                 \
               "(Landroid/content/Intent;Ljava/lang/CharSequence;Landroid/"    \
               "content/IntentSender;)Landroid/content/Intent;")
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  METHOD(getPackageInfo, "getPackageInfo",                                     \
         "(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;")              \
  METHOD(resolveActivity, "resolveActivity",                                   \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(setTypeface, "setTypeface",                                           \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  METHOD(getIdentifier, "getIdentifier",                                       \
         "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I")          \
  METHOD(openRawResourceFd, "openRawResourceFd",                               \
         "(I)Landroid/content/res/AssetFileDescriptor;")                       \
  METHOD(getConfiguration, "getConfiguration",                                 \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(setLayoutParams, "setLayoutParams",                                   \
  ...
  METHOD(addOnLayoutChangeListener, "addOnLayoutChangeListener",               \
         "(Landroid/view/View$OnLayoutChangeListener;)V")                      \
  METHOD(announceForAccessibility, "announceForAccessibility",                 \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  METHOD(setOnApplyWindowInsetsListener, "setOnApplyWindowInsetsListener",     \
         "(Landroid/view/View$OnApplyWindowInsetsListener;)V")                 \
  METHOD(getRootWindowInsets, "getRootWindowInsets",                           \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(                                                                      \
      requestSendAccessibilityEvent, "requestSendAccessibilityEvent",          \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  METHOD(registerActivityLifecycleCallbacks,                                   \
         "registerActivityLifecycleCallbacks",                                 \
         "(Landroid/app/Application$ActivityLifecycleCallbacks;)V")            \
  METHOD(unregisterActivityLifecycleCallbacks,                                 \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  ...
  METHOD(addCallback, "addCallback",                                           \
         "(Landroid/view/SurfaceHolder$Callback;)V")                           \
  METHOD(removeCallback, "removeCallback",                                     \
         "(Landroid/view/SurfaceHolder$Callback;)V")
...
inline String juceString(jstring s) { return juceString(getEnv(), s); }
...
inline LocalRef<jobjectArray>
juceStringArrayToJava(const StringArray &juceArray) {
  auto *env = getEnv();

  LocalRef<jobjectArray> result(env->NewObjectArray(
      (jsize)juceArray.size(), JavaString, javaString("").get()));
...
inline StringArray
javaStringArrayToJuce(const LocalRef<jobjectArray> &javaArray) {
  ...
  for (int i = 0; i < env->GetArrayLength(javaArray.get()); ++i) {
    LocalRef<jstring> javaString(
        (jstring)env->GetObjectArrayElement(javaArray.get(), i));
...
// goes out of scope (and no other Java routine has a reference on the
// return-value) then the implementer will be deleted as well.
...
//==============================================================================
jobject juce_invokeImplementer(JNIEnv *, jobject, jlong, jobject, jobject,
                               jobjectArray);
...
protected:
  ...
  //==============================================================================
  friend LocalRef<jobject> CreateJavaInterface(AndroidInterfaceImplementer *,
                                               const StringArray &,
                                               LocalRef<jobject>);
  friend jobject juce_invokeImplementer(JNIEnv *, jobject, jlong, jobject,
...
public:
  ...
  virtual void onActivityPreSaveInstanceState(jobject /*activity*/,
                                              jobject /*bundle*/) {}
  ...
  virtual void onActivitySaveInstanceState(jobject /*activity*/,
                                           jobject /*bundle*/) {}
  ...
  virtual void onActivityPostSaveInstanceState(jobject /*activity*/,
                                               jobject /*bundle*/) {}
...
struct SurfaceHolderCallback : AndroidInterfaceImplementer {
  ...
  virtual void surfaceChanged(LocalRef<jobject> holder, int format, int width,
                              int height) = 0;
...
private:
  jobject invoke(jobject proxy, jobject method, jobjectArray args) override {
    ...
    LocalRef<jobject> holder(env->GetArrayLength(args) > 0
...
public:
  ...
  /** @internal */
  static void onRequestPermissionsResultCallback(JNIEnv *, FragmentOverlay &,
                                                 jint requestCode,
                                                 jobjectArray jPermissions,
                                                 jintArray jGrantResults);
  /** @internal */
  static void onActivityResultCallback(JNIEnv *, FragmentOverlay &,
...
public:
  ...
  void onStart() override {
    if (!std::exchange(activityHasStarted, true))
      getEnv()->CallVoidMethod(getNativeHandle(),
                               AndroidFragment.startActivityForResult,
                               intent.get(), requestCode);
  }

  void onActivityResult(int activityRequestCode, int resultCode,
                        LocalRef<jobject> data) override {
    NullCheckedInvocation::invoke(callback, activityRequestCode, resultCode,
                                  std::move(data));
  }
...
inline constexpr auto generatedCallbackImpl = juce::toFnPtr(
    JNICALL[](JNIEnv * env, jobject, jlong host, Args... args)->Result {
...
template <auto Fn, typename Result, typename Class, typename... Args>
constexpr auto generateCallbackImpl(Result (*)(JNIEnv *, const Class &,
                                               Args...)) {
  return generatedCallbackImpl<Fn, Result, Class, Args...>;
}
...
// Evaluates to a static function that forwards to the provided Fn, assuming
// that the 'host' argument points to an object on which it is valid to call Fn
template <auto Fn>
