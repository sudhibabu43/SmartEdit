...
//==============================================================================
static StringArray jucePermissionToAndroidPermissions(
...
static RuntimePermissions::PermissionID
androidPermissionToJucePermission(const String &permission) {
  static const std::map<String, RuntimePermissions::PermissionID> map{
      ...
      {"android.permission.READ_MEDIA_AUDIO",
       RuntimePermissions::readMediaAudio},
      {"android.permission.READ_MEDIA_IMAGES",
       RuntimePermissions::readMediaImages},
      {"android.permission.READ_MEDIA_VIDEO",
       RuntimePermissions::readMediaVideo},
  ...
  void onRequestPermissionsResult(int /*requestCode*/,
...

    {
      ...
      if (requests.size() > 0) {
        ...
        auto permissionsArray =
            jucePermissionToAndroidPermissions(request.permission);
        auto jPermissionsArray = juceStringArrayToJava(permissionsArray);

        auto requestPermissionsMethodID = env->GetMethodID(
            AndroidFragment, "requestPermissions", "([Ljava/lang/String;I)V");

        // this code should only be reached for SDKs >= 23, so this method
        // should be be available
        jassert(requestPermissionsMethodID != nullptr);

        env->CallVoidMethod(getNativeHandle(), requestPermissionsMethodID,
                            jPermissionsArray.get(), 0);
      } else {
...
void RuntimePermissions::request(PermissionID permission, Callback callback) {
  const auto requestedPermissions =
      jucePermissionToAndroidPermissions(permission);

  const auto allPermissionsInManifest = std::all_of(
      requestedPermissions.begin(), requestedPermissions.end(),
      [](const auto &p) { return isPermissionDeclaredInManifest(p); });
  ...
  std::unique_ptr<PermissionsOverlay> &overlay =
      PermissionsOverlay::getSingleton();
...
bool RuntimePermissions::isGranted(PermissionID permission) {
  ...
  return std::all_of(requestedPermissions.begin(), requestedPermissions.end(),
                     [env](const auto &p) {
                       return 0 ==
                              env->CallIntMethod(
                                  getAppContext().get(),
