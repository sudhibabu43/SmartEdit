...
static int findNumberOfPhysicalCores() noexcept {
  ...
  const auto numBuffers =
      (size_t)(bufferSize / sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION));
...
String SystemStats::getCpuVendor() {
  static const auto cpuVendor = [] {
    static constexpr auto *path = "HKEY_LOCAL_"
                                  "MACHINE\\HARDWARE\\DESCRIPTION\\System\\Cent"
                                  "ralProcessor\\0\\VendorIdentifier";
...
String SystemStats::getCpuModel() {
  static const auto cpuModel = [] {
    static constexpr auto *path = "HKEY_LOCAL_"
                                  "MACHINE\\HARDWARE\\DESCRIPTION\\System\\Cent"
                                  "ralProcessor\\0\\ProcessorNameString";
    ...
    if (GetFileVersionInfo(filename, handle, size, data)) {
      ...
      if (VerQueryValue(data, (LPCTSTR) _T ("\\"), (void **)&info, &verSize))
        if (size > 0 && info != nullptr && info->dwSignature == 0xfeef04bd)
          return ((uint64)info->dwFileVersionMS << 32) |
                 (uint64)info->dwFileVersionLS;
    }
  }
...
RTL_OSVERSIONINFOW getWindowsVersionInfo() {
  ...
  if (auto *moduleHandle = ::GetModuleHandleW(L"ntdll.dll")) {
    ...
    if (auto *rtlGetVersion =
            (RtlGetVersion)::GetProcAddress(moduleHandle, "RtlGetVersion")) {
...
bool SystemStats::isOperatingSystem64Bit() {
  ...
  return fnIsWow64Process != nullptr &&
         fnIsWow64Process(GetCurrentProcess(), &isWow64) && isWow64 != FALSE;
#endif
}
...
static String getLocaleValue(LCID locale, LCTYPE key,
...
String SystemStats::getDisplayLanguage() {
  ...
  JUCE_LOAD_WINAPI_FUNCTION(dll, GetUserPreferredUILanguages,
                            getUserPreferredUILanguages, BOOL,
  ...
  const auto success = getUserPreferredUILanguages(
      MUI_LANGUAGE_NAME, &numLanguages, languagesBuffer.data(),
      &numCharsInLanguagesBuffer);
...
static constexpr DWORD generateProviderID(const char *string) {
  return (DWORD)string[0] << 0x18 | (DWORD)string[1] << 0x10 |
         (DWORD)string[2] << 0x08 | (DWORD)string[3] << 0x00;
}
...
static std::optional<std::vector<std::byte>> readSMBIOSData() {
  ...
  if (const auto bufLen = GetSystemFirmwareTable(sig, id, nullptr, 0);
      bufLen > 0) {
...
#pragma pack(pop)
    ...
    String uuid;
    const auto *asRawSMBIOSData =
        unalignedPointerCast<const RawSMBIOSData *>(smbiosBuffer->data());

    if (smbiosBuffer->size() <
        sizeof(RawSMBIOSData) + static_cast<size_t>(asRawSMBIOSData->length)) {
      // Malformed buffer; declared length is longer than the buffer we were
      // given
      jassertfalse;
      return {};
    }

    Span<const std::byte> content(smbiosBuffer->data() + sizeof(RawSMBIOSData),
                                  asRawSMBIOSData->length);
    ...
    while (!content.empty()) {
      ...
      const auto *header =
          unalignedPointerCast<const SMBIOSHeader *>(content.data());
      ...
      const auto endOfStringTable = [&header, &strings, &content] {
        const auto *dataTable =
            unalignedPointerCast<const char *>(content.data());
        ...
        const auto lengthAfterHeader =
            jmax((size_t)header->length + 2, stringOffset + 1);
        return jmin(lengthAfterHeader, content.size());
      }();

      const auto stringFromOffset = [&content,
                                     &strings](size_t byteOffset) -> String {
      ...
      enum {
        ...
        systemUUID = 0x08, // 16byte UUID. Can be all 0xFF or all 0x00. Might be
                           // user changeable.
        systemSKU = 0x19,
        systemFamily = 0x1a,
      ...
      case 1: // System
      {
        ...
        if (systemUUID + 16 < content.size()) {
          ...
          for (auto i = 0; i != 16; ++i)
            snprintf(hexBuf + 2 * i, 3, "%02hhX",
                     std::to_integer<uint8_t>(src[i]));
        }
      ...
      content = Span(content.data() + endOfStringTable,
                     content.size() - endOfStringTable);
    }
