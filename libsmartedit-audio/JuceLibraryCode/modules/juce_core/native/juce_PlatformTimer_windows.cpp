...
public:
  explicit PlatformTimer(PlatformTimerListener &ptl) : listener{ptl} {}

  void startTimer(int newIntervalMs) {
    jassert(newIntervalMs > 0);

    // Use the static callback
    timerId = timeSetEvent((UINT)newIntervalMs, 1,
                           &PlatformTimer::timerCallback, (DWORD_PTR)&listener,
...
private:
  ...
  static void __stdcall timerCallback(UINT, UINT, DWORD_PTR context, DWORD_PTR,
                                      DWORD_PTR) {
