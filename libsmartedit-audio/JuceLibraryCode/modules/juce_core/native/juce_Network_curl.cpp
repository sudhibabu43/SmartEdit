...
#define JUCE_INIT_CURL_SYMBOL(name) symbols->name = ::name;
  ...
  // so we need to get a lock during calls to curl_multi_init and
  // curl_multi_cleanup
...
private:
  ...
  template <typename FuncPtr> bool loadSymbol(FuncPtr &dst, const char *name) {
...
public:
  ...
  bool setOptions() {
    ...
    if (hasBodyDataToSend)
      WebInputStream::createHeadersAndPostData(
          url, requestHeaders, headersAndPostData, addParametersToRequestBody);
    ...
    if (symbols->curl_easy_setopt(curl, CURLOPT_URL, address.toRawUTF8()) ==
            CURLE_OK &&
        symbols->curl_easy_setopt(curl, CURLOPT_WRITEDATA, this) == CURLE_OK &&
        symbols->curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
                                  StaticCurlWrite) == CURLE_OK &&
        symbols->curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1) == CURLE_OK &&
        symbols->curl_easy_setopt(curl, CURLOPT_MAXREDIRS,
                                  static_cast<long>(maxRedirects)) ==
            CURLE_OK &&
        symbols->curl_easy_setopt(curl, CURLOPT_USERAGENT,
                                  userAgent.toRawUTF8()) == CURLE_OK &&
        symbols->curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION,
                                  (maxRedirects > 0 ? 1 : 0)) == CURLE_OK) {
      if (hasBodyDataToSend) {
        if (symbols->curl_easy_setopt(curl, CURLOPT_READDATA, this) !=
                CURLE_OK ||
            symbols->curl_easy_setopt(curl, CURLOPT_READFUNCTION,
                                      StaticCurlRead) != CURLE_OK)
          return false;

        if (symbols->curl_easy_setopt(curl, CURLOPT_POST, 1) != CURLE_OK ||
            symbols->curl_easy_setopt(
                curl, CURLOPT_POSTFIELDSIZE_LARGE,
                static_cast<curl_off_t>(headersAndPostData.getSize())) !=
                CURLE_OK)
          return false;
      }

      // handle special http request commands
      const auto hasSpecialRequestCmd =
          hasBodyDataToSend ? (httpRequest != "POST") : (httpRequest != "GET");

      if (hasSpecialRequestCmd)
        if (symbols->curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST,
                                      httpRequest.toRawUTF8()) != CURLE_OK)
          return false;

      if (symbols->curl_easy_setopt(curl, CURLOPT_HEADERDATA, this) !=
              CURLE_OK ||
          symbols->curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION,
                                    StaticCurlHeader) != CURLE_OK)
        return false;

      if (timeOutMs > 0) {
        auto timeOutSecs = ((long)timeOutMs + 999) / 1000;

        if (symbols->curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT,
                                      timeOutSecs) != CURLE_OK ||
            symbols->curl_easy_setopt(curl, CURLOPT_LOW_SPEED_LIMIT, 100) !=
                CURLE_OK ||
            symbols->curl_easy_setopt(curl, CURLOPT_LOW_SPEED_TIME,
                                      timeOutSecs) != CURLE_OK)
          return false;
      }
  ...
  bool connect(WebInputStream::Listener *webInputListener) {
    {
      ...
      if (requestHeaders.isNotEmpty()) {
        ...
        // fromLines will always return at least one line if the string is not
        // empty
        jassert(headerLines.size() > 0);
        headerList =
            symbols->curl_slist_append(headerList, headerLines[0].toRawUTF8());

        for (int i = 1; (i < headerLines.size() && headerList != nullptr); ++i)
          headerList = symbols->curl_slist_append(headerList,
                                                  headerLines[i].toRawUTF8());
        ...
        if (symbols->curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headerList) !=
            CURLE_OK) {
          cleanup();
          return false;
        }
    ...
    while ((!finished) && curlBuffer.isEmpty()) {
      ...
      if (hasBodyDataToSend && listener != nullptr && lastPos != postPosition) {
        ...
        if (!listener->postDataSendProgress(
                owner, static_cast<int>(lastPos),
                static_cast<int>(headersAndPostData.getSize()))) {
...

    {
      ...
      long responseCode;
      if (symbols->curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE,
                                     &responseCode) == CURLE_OK)
        statusCode = static_cast<int>(responseCode);

#if LIBCURL_VERSION_MAJOR < 7 ||                                               \
    (LIBCURL_VERSION_MAJOR == 7 && LIBCURL_VERSION_MINOR < 55)
      double curlLength;
      if (symbols->curl_easy_getinfo(curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD,
                                     &curlLength) == CURLE_OK) {
#else
      curl_off_t curlLength;
      if (symbols->curl_easy_getinfo(curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD_T,
                                     &curlLength) == CURLE_OK) {
#endif
    ...
    for (;;) {
      ...
      if (CURLMsg *msg = symbols->curl_multi_info_read(multi, &cnt)) {
        if (msg->msg == CURLMSG_DONE && msg->easy_handle == curl) {
          lastError = msg->data.result; // this is the error that stopped our
  ...
  void singleStep() {
...

    {
      ...
      if ((lastError = (int)symbols->curl_multi_timeout(multi, &curl_timeo)) !=
          CURLM_OK)
        return;
    }
...

    {
      ...
      if ((lastError = (int)symbols->curl_multi_fdset(
               multi, &fdread, &fdwrite, &fdexcep, &maxfd)) != CURLM_OK)
        return;
    }
    ...
    if (maxfd != -1) {
      ...
      // if curl does not return any sockets for to wait on, then the doc says
      // to wait 100 ms
      Thread::sleep(100);
    }
...

    {
      ...
      while ((curlRet = (int)symbols->curl_multi_perform(
                  multi, &still_running)) == CURLM_CALL_MULTI_PERFORM) {
      }
    }
  ...
  static size_t StaticCurlWrite(char *ptr, size_t size, size_t nmemb,
                                void *userdata) {
    WebInputStream::Pimpl *wi =
        reinterpret_cast<WebInputStream::Pimpl *>(userdata);
    return wi->curlWriteCallback(ptr, size, nmemb);
  }

  static size_t StaticCurlRead(char *ptr, size_t size, size_t nmemb,
                               void *userdata) {
    WebInputStream::Pimpl *wi =
        reinterpret_cast<WebInputStream::Pimpl *>(userdata);
  ...
  static size_t StaticCurlHeader(char *ptr, size_t size, size_t nmemb,
                                 void *userdata) {
    WebInputStream::Pimpl *wi =
        reinterpret_cast<WebInputStream::Pimpl *>(userdata);
