...
static std::optional<Type> firstOptionalWithValue(
    const std::initializer_list<std::optional<Type>> &optionals) {
...
static bool
tryToUpgradeCurrentThreadToRealtime(const Thread::RealtimeOptions &options) {
  const auto periodMs = options.getPeriodMs().value_or(0.0);

  const auto processingTimeMs =
      firstOptionalWithValue({options.getProcessingTimeMs(),
                              options.getMaximumProcessingTimeMs(),
                              options.getPeriodMs()})
          .value_or(10.0);

  const auto maxProcessingTimeMs =
      options.getMaximumProcessingTimeMs().value_or(processingTimeMs);
  ...
  policy.period =
      (uint32_t)Time::secondsToHighResolutionTicks(periodMs / 1'000.0);
  policy.computation =
      (uint32_t)Time::secondsToHighResolutionTicks(processingTimeMs / 1'000.0);
  policy.constraint = (uint32_t)Time::secondsToHighResolutionTicks(
      maxProcessingTimeMs / 1'000.0);
  ...
  if (result == KERN_INVALID_ARGUMENT && options.getProcessingTimeMs() > 50.0)
    return tryToUpgradeCurrentThreadToRealtime(
        options.withProcessingTimeMs(50.0));
...
bool Thread::createNativeThread(Priority priority) {
  ...
  if (@available(macos 10.10, *))
    ...
    PosixSchedulerPriority::getNativeSchedulerAndPriority(realtimeOptions,
                                                          priority)
        .apply(attribute);
  ...
  threadId = threadHandle =
      makeThreadHandle(attribute, &threadData, [](void *userData) -> void * {
        ...
        JUCE_AUTORELEASEPOOL { juce_threadEntryPoint(&thread); }
...
Thread::Priority Thread::getPriority() const {
  ...
  if (!isRealtime()) {
    ...
    if (min != 0 && max != 0) {
      const auto native =
          PosixSchedulerPriority::findCurrentSchedulerAndPriority()
              .getPriority();
...
bool Thread::setPriority(Priority priority) {
  ...
  const auto psp =
      PosixSchedulerPriority::getNativeSchedulerAndPriority({}, priority);
