...
HWND juce_messageWindowHandle =
    nullptr; // (this is used by other parts of the codebase)
...
CriticalSection::CriticalSection() noexcept {
  ...
  static_assert(sizeof(CRITICAL_SECTION) <= sizeof(lock),
                "win32 lock array too small to hold CRITICAL_SECTION: please "
                "report this JUCE bug!");
...
static unsigned int STDMETHODCALLTYPE threadEntryProc(void *userData) {
  if (juce_messageWindowHandle != nullptr)
    AttachThreadInput(
        GetWindowThreadProcessId(juce_messageWindowHandle, nullptr),
...
static bool setPriorityInternal(bool isRealtime, HANDLE handle,
                                Thread::Priority priority) {
  auto nativeThreadFlag = isRealtime
...
bool Thread::createNativeThread(Priority priority) {
  ...
  threadHandle = (void *)_beginthreadex(nullptr, (unsigned int)threadStackSize,
                                        &threadEntryProc, this,
                                        CREATE_SUSPENDED, &newThreadId);
...
void JUCE_CALLTYPE
Thread::setCurrentThreadName([[maybe_unused]] const String &name) {
  ...
  } __except (GetExceptionCode() == EXCEPTION_NONCONTINUABLE_EXCEPTION
                  ? EXCEPTION_EXECUTE_HANDLER
                  : EXCEPTION_CONTINUE_EXECUTION) {
    OutputDebugStringA(
        "** Warning - Encountered noncontinuable exception **\n");
  }
#endif
}
...
void JUCE_CALLTYPE
Thread::setCurrentThreadAffinityMask(const uint32 affinityMask) {
...
void JUCE_CALLTYPE Thread::sleep(const int millisecs) {
  ...
  if (millisecs >= 10 || sleepEvent.handle == nullptr)
    ...
    // likely to need to be accurate
    WaitForSingleObject(sleepEvent.handle, (DWORD)millisecs);
}

void Thread::yield() { Sleep(0); }

//==============================================================================
static int lastProcessPriority = -1;

// called when the app gains focus because Windows does weird things to process
// priority when you swap apps, and this forces an update when the app is
// brought to the front.
void juce_repeatLastProcessPriority();
void juce_repeatLastProcessPriority() {
  if (lastProcessPriority >=
      0) // (avoid changing this if it's not been explicitly set by the app..)
  {
    DWORD p;
...
void *JUCE_CALLTYPE Process::getCurrentModuleInstanceHandle() noexcept {
  if (currentModuleHandle == nullptr) {
    auto status = GetModuleHandleEx(
        ...
        (LPCTSTR)&currentModuleHandle, (HMODULE *)&currentModuleHandle);
...
void JUCE_CALLTYPE
Process::setCurrentModuleInstanceHandle(void *const newHandle) noexcept {
...
public:
  Pimpl(String nameIn, const int timeOutMillisecs)
    ...
    handle =
        CreateMutexW(nullptr, TRUE, ("Global\\" + nameIn).toWideCharPointer());

    // Not 100% sure why a global mutex sometimes can't be allocated, but if it
    // fails, fall back to a local one. (A local one also sometimes fails on
    // other machines so neither type appears to be universally reliable)
    if (handle == nullptr)
      handle =
          CreateMutexW(nullptr, TRUE, ("Local\\" + nameIn).toWideCharPointer());
    ...
    if (handle != nullptr && GetLastError() == ERROR_ALREADY_EXISTS) {
      ...
      switch (WaitForSingleObject(
          handle, timeOutMillisecs < 0 ? INFINITE : (DWORD)timeOutMillisecs)) {
      case WAIT_OBJECT_0:
      case WAIT_ABANDONED:
        break;
...
InterProcessLock::InterProcessLock(const String &name_) : name(name_) {}
...
public:
  ActiveProcess(const String &command, int streamFlags)
    ...
    if (CreatePipe(&readPipe, &writePipe, &securityAtts, 0) &&
      ...
      startupInfo.hStdOutput =
          (streamFlags & wantStdOut) != 0 ? writePipe : nullptr;
      startupInfo.hStdError =
          (streamFlags & wantStdErr) != 0 ? writePipe : nullptr;
      ...
      ok = CreateProcess(nullptr,
                         ...
                         nullptr, &startupInfo, &processInfo) != FALSE;
      JUCE_END_IGNORE_WARNINGS_MSVC
    }
  }
  ...
  int read(void *dest, int numNeeded) const noexcept {
    ...
    while (ok && numNeeded > 0) {
      ...
      if (!PeekNamedPipe((HANDLE)readPipe, nullptr, 0, nullptr, &available,
                         nullptr))
        break;
      ...
      if (available == 0) {
        ...
        DWORD numRead = 0;
        if (!ReadFile((HANDLE)readPipe, dest, (DWORD)numToDo, &numRead,
                      nullptr))
          break;
