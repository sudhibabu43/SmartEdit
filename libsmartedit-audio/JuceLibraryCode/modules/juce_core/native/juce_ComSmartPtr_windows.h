...
template <> struct UUIDGetter<::IUnknown> {
  static CLSID get() {
...
#define JUCE_DECLARE_UUID_GETTER(name, uuid)                                   \
  template <> struct UUIDGetter<name> {                                        \
...
inline GUID uuidFromString(const char *s) noexcept {
  ...
  return {ints[0],
          (uint16)(ints[1] >> 16),
          (uint16)ints[1],
          {(uint8)(ints[2] >> 24), (uint8)(ints[2] >> 16),
           (uint8)(ints[2] >> 8), (uint8)ints[2], (uint8)(ints[3] >> 24),
           (uint8)(ints[3] >> 16), (uint8)(ints[3] >> 8), (uint8)ints[3]}};
}
...
public:
  ...
  ComSmartPtr(ComClass *obj) : p(obj) {
    if (p)
      p->AddRef();
  }
  ComSmartPtr(const ComSmartPtr &other) : p(other.p) {
    if (p)
      p->AddRef();
  }
  ~ComSmartPtr() { release(); }
  ...
  HRESULT CoCreateInstance(REFCLSID classUUID,
    ...
    auto hr =
        ::CoCreateInstance(classUUID, nullptr, dwClsContext, __uuidof(ComClass),
                           (void **)resetAndGetPointerAddress());
    jassert(hr != CO_E_NOTINITIALIZED); // You haven't called CoInitialize for
                                        // the current thread!
    return hr;
  }
  ...
  HRESULT QueryInterface(REFCLSID classUUID,
    ...
    return p->QueryInterface(classUUID,
                             (void **)destObject.resetAndGetPointerAddress());
  }
...
private:
  ...
  ComClass **
  operator&() noexcept; // private to avoid it being used accidentally
};
...
protected:
  ...
  template <class Type> JUCE_COMRESULT castToType(void **result) {
    this->AddRef();
...
/** Handy base class for writing COM objects, providing ref-counting and a basic
...
public:
  ...
  JUCE_COMRESULT QueryInterface(REFIID refId, void **result) override {
    const std::tuple<IID, void *> bases[]{std::make_tuple(
        __uuidof(ComClasses),
