  ...
  StringPairArray getResponseHeaders() const {
    ...
    if (!isError()) {
      for (int i = 0; i < headerLines.size(); ++i) {
        ...
        responseHeaders.set(key, previousValue.isEmpty()
                                     ? value
                                     : (previousValue + "," + value));
      }
    }
  ...
  int read(void *buffer, int bytesToRead) {
    ...
    if (isChunked && !readingChunk) {
      if (position >= chunkEnd) {
        ...
        if (chunkEnd > 0) {
          if (read(&c, 1) != 1 || c != '\r' || read(&c, 1) != 1 || c != '\n') {
            finished = true;
            return 0;
          }
        }

        while (chunkLengthBuffer.getDataSize() < 512 &&
               !(finished || isError())) {
          if (read(&c, 1) != 1) {
            finished = true;
            return 0;
        ...
        auto chunkSize =
            chunkLengthBuffer.toString().trimStart().getHexValue64();
    ...
    auto bytesRead = jmax(
        0, (int)recv(socketHandle, buffer, (size_t)bytesToRead, MSG_WAITALL));
...
private:
  ...
  int createConnection(WebInputStream::Listener *listener, int numRedirects) {
    ...
    if (getaddrinfo(serverName.toUTF8(), String(port).toUTF8(), &hints,
                    &result) != 0 ||
        result == nullptr)
      return 0;
    ...
    setsockopt(socketHandle, SOL_SOCKET, SO_RCVBUF, (char *)&receiveBufferSize,
    ...
    {
      const MemoryBlock requestHeader(createRequestHeader(
          hostName, hostPort, proxyName, proxyPort, hostPath, address, headers,
          postData, httpRequestCmd));

      if (!sendHeader(socketHandle, requestHeader, timeOutTime, owner,
                      listener)) {
        closeSocket();
        return 0;
      }
    ...
    if (responseHeader.isNotEmpty()) {
      ...
      if (++levelsOfRedirection <= numRedirects && status >= 300 &&
          status < 400 && location.isNotEmpty() && location != address) {
        if (!(location.startsWithIgnoreCase("http://") ||
              location.startsWithIgnoreCase("https://") ||
              location.startsWithIgnoreCase("ftp://"))) {
          // The following is a bit dodgy. Ideally, we should do a proper
          // transform of the relative URI to a target URI
      ...
      isChunked =
          (findHeaderItem(headerLines, "Transfer-Encoding:") == "chunked");
  ...
  String readResponse(uint32 timeOutTime) {
    ...
    while (numConsecutiveLFs < 2 && buffer.getDataSize() < 32768 &&
  ...
  static void writeValueIfNotPresent(MemoryOutputStream &dest,
                                     const String &headers, const String &key,
                                     const String &value) {
  ...
  static void writeHost(MemoryOutputStream &dest, const String &httpRequestCmd,
    ...
    /* HTTP spec 14.23 says that the port number must be included in the header
     * if it is not 80 */
    if (port != 80)
      dest << ':' << port;
  }
  ...
  static MemoryBlock createRequestHeader(const String &hostName, int hostPort,
    ...
    writeValueIfNotPresent(
        header, userHeaders, "User-Agent:",
        "JUCE/" JUCE_STRINGIFY(JUCE_MAJOR_VERSION) "." JUCE_STRINGIFY(
            JUCE_MINOR_VERSION) "." JUCE_STRINGIFY(JUCE_BUILDNUMBER));
    ...
    if (hasPostData)
      writeValueIfNotPresent(header, userHeaders,
                             "Content-Length:", String((int)postDataSize));
  ...
  static bool sendHeader(int socketHandle, const MemoryBlock &requestHeader,
                         uint32 timeOutTime, WebInputStream &pimplOwner,
    ...
    while (totalHeaderSent < requestHeader.getSize()) {
      ...
      auto numToSend =
          jmin(1024, (int)(requestHeader.getSize() - totalHeaderSent));

      if (send(socketHandle,
               static_cast<const char *>(requestHeader.getData()) +
                   totalHeaderSent,
      ...
      if (listener != nullptr &&
          !listener->postDataSendProgress(pimplOwner, (int)totalHeaderSent,
                                          (int)requestHeader.getSize()))
        return false;
    }
  ...
  static bool decomposeURL(const String &url, String &host, String &path,
                           int &port) {
  ...
  static String findHeaderItem(const StringArray &lines,
