...
void CPUInformation::initialise() noexcept {
  ...
  SystemStatsHelpers::getCPUInfo(
      hasMMX, hasSSE, hasSSE2, has3DNow, hasSSE3, hasSSSE3, hasFMA3, hasSSE41,
      hasSSE42, hasAVX, hasFMA4, hasAVX2, hasAVX512F, hasAVX512DQ,
      hasAVX512IFMA, hasAVX512PF, hasAVX512ER, hasAVX512CD, hasAVX512BW,
      hasAVX512VL, hasAVX512VBMI, hasAVX512VPOPCNTDQ);
...
static String getOSXVersion() {
  JUCE_AUTORELEASEPOOL {
    const auto *dict = [] {
      const String systemVersionPlist(
          "/System/Library/CoreServices/SystemVersion.plist");

      if (@available(macOS 10.13, *)) {
        NSError *error = nullptr;
        return [NSDictionary
            dictionaryWithContentsOfURL:createNSURLFromFile(systemVersionPlist)
                                  error:&error];
      }

      return [NSDictionary
          dictionaryWithContentsOfFile:juceStringToNS(systemVersionPlist)];
    }();

    if (dict != nullptr)
      return nsStringToJuce(
          [dict objectForKey:nsStringLiteral("ProductVersion")]);
...
String SystemStats::getDeviceDescription() {
  if (auto *userInfo = [[NSProcessInfo processInfo] environment])
    if (auto *simDeviceName =
            [userInfo objectForKey:@"SIMULATOR_MODEL_IDENTIFIER"])
...
String SystemStats::getDeviceManufacturer() { return "Apple"; }
...
//==============================================================================
String SystemStats::getLogonName() { return nsStringToJuce(NSUserName()); }
...
static String getLocaleValue(CFStringRef key) {
  ...
  const String result(
      String::fromCFString((CFStringRef)CFLocaleGetValue(cfLocale.get(), key)));
  return result;
}
...
String SystemStats::getDisplayLanguage() {
  ...
  const String result(String::fromCFString(
      (CFStringRef)CFArrayGetValueAtIndex(cfPrefLangs.get(), 0)));
  return result;
}

//==============================================================================
/*  NB: these are kept outside the HiResCounterInfo struct and initialised to 1
   to avoid division-by-zero errors if some other static constructor calls us
   before this file's static constructors have had a chance to fill them in
   correctly..
*/
...
public:
  HiResCounterInfo() {
    ...
    highResTimerFrequency =
        (timebase.denom * (uint64)1000000000) / timebase.numer;
    highResTimerToMillisecRatio =
        (double)hiResCounterNumerator / (double)hiResCounterDenominator;
  }

  uint32 millisecondsSinceStartup() const noexcept {
    return (uint32)((mach_absolute_time() * hiResCounterNumerator) /
                    hiResCounterDenominator);
  }
...
//==============================================================================
int SystemStats::getPageSize() { return (int)NSPageSize(); }
...
bool SystemStats::isAppSandboxEnabled() {
  static const auto result = [&] {
    ...
    if (const auto err = SecCodeCopySelf(kSecCSDefaultFlags, &ref);
        err != noErr)
      return false;
    ...
    if (const auto err = SecCodeCopySigningInformation(
            managedRef.get(), kSecCSDynamicInformation, &infoDict);
        err != noErr)
      return false;
    ...
    if (!CFDictionaryGetValueIfPresent(managedInfoDict.get(),
    ...
    if (!CFDictionaryGetValueIfPresent(
            static_cast<CFDictionaryRef>(entitlementsDict),
            @"com.apple.security.app-sandbox", &flag))
      return false;

    return static_cast<bool>(
        CFBooleanGetValue(static_cast<CFBooleanRef>(flag)));
  }();
