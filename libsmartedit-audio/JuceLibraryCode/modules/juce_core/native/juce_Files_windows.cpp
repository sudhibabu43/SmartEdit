...
static bool changeAtts(const String &path, DWORD bitsToSet,
                       DWORD bitsToClear) noexcept {
...
static int64 fileTimeToTime(const FILETIME *const ft) noexcept {
  ...
  return (int64)((reinterpret_cast<const ULARGE_INTEGER *>(ft)->QuadPart -
...
static FILETIME *timeToFileTime(const int64 time, FILETIME *const ft) noexcept {
  ...
  reinterpret_cast<ULARGE_INTEGER *>(ft)->QuadPart =
      (ULONGLONG)(time * 10000 + 116444736000000000LL);
  return ft;
}
...
static String getDriveFromPath(String path) {
  ...
  const size_t numBytes =
      CharPointer_UTF16::getBytesRequiredFor(path.getCharPointer()) + 4;
...
static int64 getDiskSpaceInfo(const String &path, const bool total) {
  ...
  if (GetDiskFreeSpaceEx(getDriveFromPath(path).toWideCharPointer(), &spc, &tot,
                         &totFree))
    return total ? (int64)tot.QuadPart : (int64)spc.QuadPart;
...
static Result getResultForLastError() {
  ...
  FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                ...
                MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), messageBuffer,
                (DWORD)numElementsInArray(messageBuffer) - 1, nullptr);

  return Result::fail(String(messageBuffer));
}

// The docs for the Windows security API aren't very clear. Some parts of the
// following function (the flags passed to GetNamedSecurityInfo, duplicating the
// primary access token) were guided by the example at
...
static bool hasFileAccess(const File &file, DWORD accessType) {
  ...
  if (GetNamedSecurityInfo(
          path.toWideCharPointer(), SE_FILE_OBJECT,
  ...
  if (!DuplicateToken(primaryTokenGuard.handle, SecurityImpersonation,
  ...
  if (!AccessCheck(descriptorGuard.psecurityDescriptor,
                   duplicatedTokenGuard.handle, accessType, &mapping, &set,
                   &setSize, &allowed, &granted)) {
    return false;
  }
...
bool File::existsAsFile() const {
  return fullPath.isNotEmpty() && (WindowsFileHelpers::getAtts(fullPath) &
...
bool File::setFileReadOnlyInternal(bool shouldBeReadOnly) const {
  return WindowsFileHelpers::changeAtts(
      fullPath, shouldBeReadOnly ? FILE_ATTRIBUTE_READONLY : 0,
...
bool File::moveToTrash() const {
  ...
  const size_t numBytes =
      CharPointer_UTF16::getBytesRequiredFor(fullPath.getCharPointer()) + 8;
...
bool File::replaceInternal(const File &dest) const {
  return ReplaceFile(dest.getFullPathName().toWideCharPointer(),
                     fullPath.toWideCharPointer(), nullptr,
...
Result File::createDirectoryInternal(const String &fileName) const {
  return CreateDirectory(fileName.toWideCharPointer(), nullptr)
             ? Result::ok()
...
int64 juce_fileSetPosition(void *handle, int64 pos) {
  LARGE_INTEGER li;
  li.QuadPart = pos;
  li.LowPart = SetFilePointer((HANDLE)handle, (LONG)li.LowPart, &li.HighPart,
...
void FileInputStream::openHandle() {
  auto h =
      CreateFile(file.getFullPathName().toWideCharPointer(), GENERIC_READ,
...
FileInputStream::~FileInputStream() { CloseHandle((HANDLE)fileHandle); }

size_t FileInputStream::readInternal(void *buffer, size_t numBytes) {
  if (fileHandle != nullptr) {
    DWORD actualNum = 0;

    if (!ReadFile((HANDLE)fileHandle, buffer, (DWORD)numBytes, &actualNum,
                  nullptr))
      status = WindowsFileHelpers::getResultForLastError();
...
void FileOutputStream::openHandle() {
  auto h = CreateFile(file.getFullPathName().toWideCharPointer(), GENERIC_WRITE,
...
void FileOutputStream::closeHandle() { CloseHandle((HANDLE)fileHandle); }

ssize_t FileOutputStream::writeInternal(const void *bufferToWrite,
                                        size_t numBytes) {
  DWORD actualNum = 0;

  if (fileHandle != nullptr)
    if (!WriteFile((HANDLE)fileHandle, bufferToWrite, (DWORD)numBytes,
                   &actualNum, nullptr))
      status = WindowsFileHelpers::getResultForLastError();
...
Result FileOutputStream::truncate() {
  ...
  flush();
  return SetEndOfFile((HANDLE)fileHandle)
             ? Result::ok()
             : WindowsFileHelpers::getResultForLastError();
}
...
void MemoryMappedFile::openInternal(const File &file, AccessMode mode,
  ...
  if (range.getStart() > 0) {
    ...
    range.setStart(range.getStart() -
                   (range.getStart() % systemInfo.dwAllocationGranularity));
  }
  ...
  auto h =
      CreateFile(file.getFullPathName().toWideCharPointer(), accessMode,
                 exclusive ? 0
                           : (FILE_SHARE_READ | FILE_SHARE_DELETE |
                              (mode == readWrite ? FILE_SHARE_WRITE : 0)),
                 nullptr, createType,
                 FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
  ...
  if (h != INVALID_HANDLE_VALUE) {
    ...
    auto mappingHandle =
        CreateFileMapping(h, nullptr, protect, (DWORD)(range.getEnd() >> 32),
                          (DWORD)range.getEnd(), nullptr);

    if (mappingHandle != nullptr) {
      address =
          MapViewOfFile(mappingHandle, access, (DWORD)(range.getStart() >> 32),
...
int64 File::getSize() const {
  ...
  if (GetFileAttributesEx(fullPath.toWideCharPointer(), GetFileExInfoStandard,
                          &attributes))
...
void File::getFileTimesInternal(int64 &modificationTime, int64 &accessTime,
                                int64 &creationTime) const {
...
bool File::setFileTimesInternal(int64 modificationTime, int64 accessTime,
                                int64 creationTime) const {
  using namespace WindowsFileHelpers;

  bool ok = false;
  auto h =
      CreateFile(fullPath.toWideCharPointer(), GENERIC_WRITE, FILE_SHARE_READ,
                 nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);

  if (h != INVALID_HANDLE_VALUE) {
    FILETIME m, a, c;

    ok = SetFileTime(h, timeToFileTime(creationTime, &c),
...
String File::getVolumeLabel() const {
  ...
  if (!GetVolumeInformation(
          WindowsFileHelpers::getDriveFromPath(getFullPathName())
              .toWideCharPointer(),
          dest, (DWORD)numElementsInArray(dest), nullptr, nullptr, nullptr,
          nullptr, 0))
    dest[0] = 0;
...
int File::getVolumeSerialNumber() const {
  ...
  if (!GetVolumeInformation(
          WindowsFileHelpers::getDriveFromPath(getFullPathName())
              .toWideCharPointer(),
          dest, (DWORD)numElementsInArray(dest), &serialNum, nullptr, nullptr,
          nullptr, 0))
    return 0;
...
uint64 File::getFileIdentifier() const {
  ...
  auto h =
      CreateFile(path.toWideCharPointer(), GENERIC_READ, FILE_SHARE_READ,
...
bool File::isOnCDRomDrive() const {
  return WindowsFileHelpers::getWindowsDriveType(getFullPathName()) ==
         DRIVE_CDROM;
}
...
bool File::isOnHardDisk() const {
  ...
  return n != DRIVE_REMOVABLE && n != DRIVE_CDROM && n != DRIVE_REMOTE &&
         n != DRIVE_NO_ROOT_DIR;
}
...
bool File::isOnRemovableDrive() const {
  ...
  return n == DRIVE_CDROM || n == DRIVE_REMOTE || n == DRIVE_REMOVABLE ||
         n == DRIVE_RAMDISK;
}
...
String File::getVersion() const {
  ...
  if (GetFileVersionInfo(getFullPathName().toWideCharPointer(), 0, bufferSize,
                         buffer)) {
...
bool File::isShortcut() const { return hasFileExtension(".lnk"); }
...
static String readWindowsLnkFile(File lnkFile, bool wantsAbsolutePath) {
  ...
  if (lnkFile.exists()) {
    ...
    if (SUCCEEDED(shellLink.CoCreateInstance(CLSID_ShellLink)) &&
        ...
        SUCCEEDED(persistFile->Load(
            lnkFile.getFullPathName().toWideCharPointer(), STGM_READ)) &&
        (!wantsAbsolutePath ||
         SUCCEEDED(shellLink->Resolve(nullptr, SLR_ANY_MATCH | SLR_NO_UI)))) {
      ...
      if (SUCCEEDED(
              shellLink->GetPath(resolvedPath, MAX_PATH, &winFindData, flags)))
        return resolvedPath;
    }
  }
...
static String readWindowsShortcutOrLink(const File &shortcut,
                                        bool wantsAbsolutePath) {
#if JUCE_WINDOWS
  if (!wantsAbsolutePath) {
    HANDLE h = CreateFile(
        shortcut.getFullPathName().toWideCharPointer(), GENERIC_READ,
      ...
      if (success) {
        if (IsReparseTagMicrosoft(reparseData->ReparseTag)) {
          ...
          case IO_REPARSE_TAG_SYMLINK: {
            ...
            targetPath = {
                symlinkData.PathBuffer +
                    (symlinkData.SubstituteNameOffset / sizeof(WCHAR)),
                symlinkData.SubstituteNameLength / sizeof(WCHAR)};
          } break;

          case IO_REPARSE_TAG_MOUNT_POINT: {
            auto &mountData = reparseData->MountPointReparseBuffer;
            targetPath = {mountData.PathBuffer +
                              (mountData.SubstituteNameOffset / sizeof(WCHAR)),
  ...
  typedef DWORD(WINAPI * GetFinalPathNameByHandleFunc)(HANDLE, LPTSTR, DWORD,
                                                       DWORD);

  static GetFinalPathNameByHandleFunc getFinalPathNameByHandle =
      (GetFinalPathNameByHandleFunc)getUser32Function(
  ...
  if (getFinalPathNameByHandle != nullptr) {
    ...
    if (h != INVALID_HANDLE_VALUE) {
      if (DWORD requiredSize = getFinalPathNameByHandle(
              h, nullptr, 0, 0 /* FILE_NAME_NORMALIZED */)) {
        HeapBlock<WCHAR> buffer(requiredSize + 2, true);

        if (getFinalPathNameByHandle(h, buffer, requiredSize,
          ...
          // It turns out that GetFinalPathNameByHandleW prepends \\?\ to the
          // path. This is not a bug, it's feature. See MSDN for more
          // information.
...
bool File::createShortcut(const String &description,
  ...
  return SUCCEEDED(shellLink.CoCreateInstance(CLSID_ShellLink)) &&
         ...
         SUCCEEDED(shellLink.QueryInterface(persistFile)) &&
         SUCCEEDED(persistFile->Save(
             linkFileToCreate.getFullPathName().toWideCharPointer(), TRUE));
}
...
public:
  Pimpl(const File &directory, const String &wildCardIn)
      : directoryWithWildCard(
            directory.getFullPathName().isNotEmpty()
  ...
  bool next(String &filenameFound, bool *const isDir, bool *const isHidden,
    ...
    if (handle == INVALID_HANDLE_VALUE) {
      handle =
          FindFirstFile(directoryWithWildCard.toWideCharPointer(), &findData);
    ...
    if (isReadOnly != nullptr)
      *isReadOnly =
          ((findData.dwFileAttributes & FILE_ATTRIBUTE_READONLY) != 0);
    if (fileSize != nullptr)
      *fileSize =
          findData.nFileSizeLow + (((int64)findData.nFileSizeHigh) << 32);
    if (modTime != nullptr)
...
DirectoryIterator::NativeIterator::NativeIterator(const File &directory,
                                                  const String &wildCardIn)
    : pimpl(new DirectoryIterator::NativeIterator::Pimpl(directory,
                                                         wildCardIn)) {}

DirectoryIterator::NativeIterator::~NativeIterator() {}

bool DirectoryIterator::NativeIterator::next(
    String &filenameFound, bool *const isDir, bool *const isHidden,
...
//==============================================================================
bool JUCE_CALLTYPE Process::openDocument(const String &fileName,
                                         const String &parameters) {
  HINSTANCE hInstance =
      ShellExecute(nullptr, nullptr, fileName.toWideCharPointer(),
...
void File::revealToUser() const {
  ...
  JUCE_LOAD_WINAPI_FUNCTION(dll, ILCreateFromPathW, ilCreateFromPathW,
                            ITEMIDLIST *, (LPCWSTR))
  JUCE_LOAD_WINAPI_FUNCTION(dll, ILFree, ilFree, void, (ITEMIDLIST *))
  JUCE_LOAD_WINAPI_FUNCTION(dll, SHOpenFolderAndSelectItems,
                            shOpenFolderAndSelectItems, HRESULT,
                            (ITEMIDLIST *, UINT, void *, DWORD))

  if (ilCreateFromPathW != nullptr && shOpenFolderAndSelectItems != nullptr &&
      ilFree != nullptr) {
    if (ITEMIDLIST *const itemIDList =
            ilCreateFromPathW(fullPath.toWideCharPointer())) {
...
public:
  ...
  bool connect(const int timeOutMs) {
    if (!ownsPipe) {
      ...
      const Time timeOutEnd(Time::getCurrentTime() +
                            RelativeTime::milliseconds(timeOutMs));
      ...
      for (;;) {
        ...
        if (shouldStop ||
            (timeOutMs >= 0 && Time::getCurrentTime() > timeOutEnd))
          return false;
  ...
  int read(void *destBuffer, const int maxBytesToRead,
    ...
    while (connect(timeOutMilliseconds)) {
      ...
      if (ReadFile(pipeH, destBuffer, (DWORD)maxBytesToRead, &numRead,
                   &over.over))
        return (int)numRead;
      ...
      if (ownsPipe && (lastError == ERROR_BROKEN_PIPE ||
  ...
  int write(const void *sourceBuffer, int numBytesToWrite,
    ...
    if (connect(timeOutMilliseconds)) {
      ...
      if (WriteFile(pipeH, sourceBuffer, (DWORD)numBytesToWrite, &numWritten,
                    &over.over))
...
private:
  struct OverlappedEvent {
    ...
    ~OverlappedEvent() { CloseHandle(over.hEvent); }
...
int NamedPipe::read(void *destBuffer, int maxBytesToRead,
                    int timeOutMilliseconds) {
  ScopedReadLock sl(lock);
  return pimpl != nullptr
             ? pimpl->read(destBuffer, maxBytesToRead, timeOutMilliseconds)
             : -1;
}

int NamedPipe::write(const void *sourceBuffer, int numBytesToWrite,
                     int timeOutMilliseconds) {
  ScopedReadLock sl(lock);
  return pimpl != nullptr
             ? pimpl->write(sourceBuffer, numBytesToWrite, timeOutMilliseconds)
             : -1;
}
