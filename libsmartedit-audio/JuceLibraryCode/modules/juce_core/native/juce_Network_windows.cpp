...
public:
  Pimpl(WebInputStream &pimplOwner, const URL &urlToCopy,
      ...
      if (!isError()) {
        ...
        for (;;) {
          ...
          if (HttpQueryInfo(request, HTTP_QUERY_RAW_HEADERS_CRLF,
                            buffer.getData(), &bufferSizeBytes, nullptr)) {
            StringArray headersArray;
            headersArray.addLines(
                String(reinterpret_cast<const WCHAR *>(buffer.getData())));

            for (int i = 0; i < headersArray.size(); ++i) {
              const String &header = headersArray[i];
              const String key(
                  header.upToFirstOccurrenceOf(": ", false, false));
              const String value(
                  header.fromFirstOccurrenceOf(": ", false, false));
              const String previousValue(dataHeaders[key]);
              dataHeaders.set(key, previousValue.isEmpty()
                                       ? value
                                       : (previousValue + "," + value));
            }
        ...
        if (HttpQueryInfo(request,
          ...
          if (numRedirectsToFollow >= 0 &&
            ...
            // Check whether location is a relative URI - this is an incomplete
            // test for relative path, but we'll use it for now (valid protocols
            // for this implementation are http, https & ftp)
            if (!(newLocation.startsWithIgnoreCase("http://") ||
              ...
              if (newLocation.startsWithChar('/'))
                newLocation =
                    URL(address).withNewSubPath(newLocation).toString(true);
              else
  ...
  bool setPosition(int64 wantedPos) {
    ...
    if (wantedPos != position) {
      finished = false;
      position = (int64)InternetSetFilePointer(request, (LONG)wantedPos,
                                               nullptr, FILE_BEGIN, 0);
...
private:
  ...
  void createConnection(const String &address,
                        WebInputStream::Listener *listener) {
    static HINTERNET sessionHandle = InternetOpen(
        _T ("juce"), INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
  ...
  void openConnection(URL_COMPONENTS &uc, HINTERNET sessionHandle,
                      ...
                      WebInputStream::Listener *listener) {
    int disable = 1;
    InternetSetOption(sessionHandle, INTERNET_OPTION_DISABLE_AUTODIAL, &disable,
                      sizeof(disable));
...

    {
      ...
      connection =
          hasBeenCancelled
              ? nullptr
              : InternetConnect(sessionHandle, uc.lpszHostName, uc.nPort,
    ...
    if (connection != nullptr) {
      if (isFtp)
        request =
            FtpOpenFile(connection, uc.lpszUrlPath, GENERIC_READ,
  ...
  void sendHTTPRequest(INTERNET_BUFFERS &buffers,
                       WebInputStream::Listener *listener) {
    ...
    while (totalBytesSent < (int)postData.getSize()) {
      ...
      if (bytesToSend == 0 ||
          !InternetWriteFile(request,
                             static_cast<const char *>(postData.getData()) +
                                 totalBytesSent,
      ...
      if (listener != nullptr &&
          !listener->postDataSendProgress(owner, totalBytesSent,
  ...
  void openHTTPConnection(URL_COMPONENTS &uc, const String &address,
                          WebInputStream::Listener *listener) {
    ...
    if (address.startsWithIgnoreCase("https:"))
      flags |= INTERNET_FLAG_SECURE; // (this flag only seems necessary if the
                                     // OS is running IE6 -
                                     //  IE7 seems to automatically work out
                                     //  when it's https)
      ...
      request =
          hasBeenCancelled
              ...
              : HttpOpenRequest(connection, httpRequestCmd.toWideCharPointer(),
                                uc.lpszUrlPath, nullptr, nullptr, mimeTypes,
                                flags, 0);
    }
...
struct GetAdaptersAddressesHelper {
  bool callGetAdaptersAddresses() {
    ...
    JUCE_LOAD_WINAPI_FUNCTION(
        dll, GetAdaptersAddresses, getAdaptersAddresses, DWORD,
    ...
    if (getAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, nullptr,
                             adaptersAddresses, &len) == ERROR_BUFFER_OVERFLOW)
      adaptersAddresses.malloc(len, 1);

    return getAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, nullptr,
                                adaptersAddresses, &len) == NO_ERROR;
  }
...
static void getViaGetAdaptersAddresses(Array<MACAddress> &result) {
  ...
  if (addressesHelper.callGetAdaptersAddresses()) {
    for (PIP_ADAPTER_ADDRESSES adapter = addressesHelper.adaptersAddresses;
...
static void findAddresses(Array<IPAddress> &result, bool includeIPv6,
                          Type start) {
  for (auto addr = start; addr != nullptr; addr = addr->Next) {
    if (addr->Address.lpSockaddr->sa_family == AF_INET)
      result.addIfNotAlreadyThere(createAddress(
          unalignedPointerCast<sockaddr_in *>(addr->Address.lpSockaddr)));
    else if (addr->Address.lpSockaddr->sa_family == AF_INET6 && includeIPv6)
      result.addIfNotAlreadyThere(createAddress(
          unalignedPointerCast<sockaddr_in6 *>(addr->Address.lpSockaddr)));
  }
}
} // namespace MACAddressHelpers
...
void IPAddress::findAllAddresses(Array<IPAddress> &result, bool includeIPv6) {
  ...
  if (addressesHelper.callGetAdaptersAddresses()) {
    for (PIP_ADAPTER_ADDRESSES adapter = addressesHelper.adaptersAddresses;
         adapter != nullptr; adapter = adapter->Next) {
      MACAddressHelpers::findAddresses(result, includeIPv6,
                                       adapter->FirstUnicastAddress);
      MACAddressHelpers::findAddresses(result, includeIPv6,
                                       adapter->FirstAnycastAddress);
      MACAddressHelpers::findAddresses(result, includeIPv6,
...
bool JUCE_CALLTYPE Process::openEmailWithAttachments(
  ...
  JUCE_LOAD_WINAPI_FUNCTION(dll, MAPISendMail, mapiSendMail, ULONG,
                            (LHANDLE, ULONG, lpMapiMessage, ::FLAGS, ULONG))
  ...
  return mapiSendMail(0, 0, &message, MAPI_DIALOG | MAPI_LOGON_UI, 0) ==
         SUCCESS_SUCCESS;
}
