...
String SystemStats::getDeviceDescription() {
  ...
  auto result = sysctl(mib, numElementsInArray(mib), nullptr,
...
String SystemStats::getCpuModel() {
  ...
  auto result =
      sysctl(mib, numElementsInArray(mib), nullptr, &modelLength, nullptr, 0);

  if (result != 0 || modelLength == 0)
    return {};

  MemoryBlock model{modelLength};
  result = sysctl(mib, numElementsInArray(mib), model.getData(), &modelLength,
                  nullptr, 0);
...
int SystemStats::getCpuSpeedInMegahertz() {
  ...
  auto result =
      sysctlbyname("hw.clockrate", &clockRate, &clockRateSize, nullptr, 0);
...
int SystemStats::getMemorySizeInMegabytes() {
  ...
  auto result =
      sysctl(mib, numElementsInArray(mib), &memory, &memorySize, nullptr, 0);
...
String SystemStats::getUserLanguage() {
  ...
  if (auto langEnv = getenv("LANG"))
    return String::fromUTF8(langEnv).upToLastOccurrenceOf(".UTF-8", false,
                                                          true);
...
void CPUInformation::initialise() noexcept {
  ...
  numLogicalCPUs = numPhysicalCPUs = [] {
    ...
    auto result = sysctl(mib, numElementsInArray(mib), &numCPUs, &numCPUsSize,
                         nullptr, 0);
...
String SystemStats::getUniqueDeviceID() {
  static const auto deviceId = []() {
    ...
    // 'board_serial' is enough on its own, fallback to bios stuff if we can't
    // find it.
    if (data.isEmpty()) {
...
JUCE_API bool JUCE_CALLTYPE juce_isRunningUnderDebugger() noexcept {
  ...
  auto result =
      sysctl(mib, numElementsInArray(mib), &info, &infoSize, nullptr, 0);
