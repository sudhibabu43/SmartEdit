...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)  \
  STATICMETHOD(getProperty, "getProperty",                                     \
...
static String getSystemProperty(const String &name) {
  return juceString(LocalRef<jstring>((jstring)getEnv()->CallStaticObjectMethod(
...
static String getAndroidID() {
  ...
  if (auto settings =
    ...
    if (auto fId = env->GetStaticFieldID(settings, "ANDROID_ID",
                                         "Ljava/lang/String;")) {
...
static String getLocaleValue(bool isRegion) {
  auto *env = getEnv();
  LocalRef<jobject> locale(
      env->CallStaticObjectMethod(JavaLocale, JavaLocale.getDefault));

  auto stringResult =
      isRegion ? env->CallObjectMethod(locale.get(), JavaLocale.getCountry)
...
static String getAndroidOsBuildValue(const char *fieldName) {
  return juceString(LocalRef<jstring>((jstring)getEnv()->GetStaticObjectField(
      AndroidBuild, getEnv()->GetStaticFieldID(AndroidBuild, fieldName,
                                               "Ljava/lang/String;"))));
}
} // namespace AndroidStatsHelpers
...
int SystemStats::getCpuSpeedInMegahertz() {
  ...
  for (int i = 0; i < getNumCpus(); ++i) {
    int freqKHz = File("/sys/devices/system/cpu/cpu" + String(i) +
...
void CPUInformation::initialise() noexcept {
  ...
  if (cpuFamily == ANDROID_CPU_FAMILY_X86 ||
      cpuFamily == ANDROID_CPU_FAMILY_X86_64) {
...
uint32 juce_millisecondsSinceStartup() noexcept {
  ...
  return static_cast<uint32>(t.tv_sec) * 1000U +
         static_cast<uint32>(t.tv_nsec) / 1000000U;
}
