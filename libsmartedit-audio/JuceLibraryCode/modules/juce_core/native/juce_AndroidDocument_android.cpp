...
/*  This is mainly used to pass implementation information from AndroidDocument
   to AndroidDocumentIterator. This needs to be defined in a .cpp because it
   uses the internal GlobalRef type.

    To preserve encapsulation, this struct should only contain information that
   would normally be public, were internal types not in use.
*/
...
#if JUCE_ANDROID
  /*
      A very basic type that acts a bit like an iterator, in that it can be
     incremented, and read-from.

      Instances of this type can be passed to the constructor of
     AndroidDirectoryIterator to provide stdlib-like iterator facilities.
  */
  template <typename Columns> class AndroidIteratorEngine {
  public:
    AndroidIteratorEngine(Columns columnsIn, jobject uri)
        : columns(std::move(columnsIn)),
          cursor{LocalRef<jobject>{getEnv()->CallObjectMethod(
              AndroidContentUriResolver::getContentResolver().get(),
              ContentResolver.query, uri, columns.getColumnNames().get(),
    ...
    bool increment() {
      ...
      return getEnv()->CallBooleanMethod(cursor.get(),
                                         AndroidCursor.moveToNext);
    }
  ...
  static LocalRef<jobjectArray> makeStringArray(std::index_sequence<Ix...>,
                                                Args &&...args) {
    auto *env = getEnv();
    LocalRef<jobjectArray> array{
        env->NewObjectArray(sizeof...(args), JavaString, nullptr)};
  ...
  static LocalRef<jobjectArray> makeStringArray(Args &&...args) {
    return makeStringArray(std::make_index_sequence<sizeof...(args)>(),
                           std::forward<Args>(args)...);
  }

  static URL uriToUrl(jobject uri) {
    return URL(juceString(
        (jstring)getEnv()->CallObjectMethod(uri, AndroidUri.toString)));
  }
  ...
  struct Columns {
    ...
    auto getColumnNames() const { return makeStringArray(idColumn); }

    auto readFromCursor(jobject cursor) const {
      auto *env = getEnv();
      const auto idColumnIndex = env->CallIntMethod(
          cursor, AndroidCursor.getColumnIndex, idColumn.get());

      const auto documentUri = [&] {
        if (idColumnIndex < 0)
          return LocalRef<jobject>{};

        LocalRef<jstring> documentId{(jstring)env->CallObjectMethod(
            cursor, AndroidCursor.getString, idColumnIndex)};
        return LocalRef<jobject>{getEnv()->CallStaticObjectMethod(
            DocumentsContract21, DocumentsContract21.buildDocumentUriUsingTree,
            treeUri.get(), documentId.get())};
      }();
  ...
  static DocumentsContractIteratorEngine
  makeDocumentsContractIteratorEngine(const GlobalRef &uri) {
    const LocalRef<jobject> documentId{getEnv()->CallStaticObjectMethod(
        DocumentsContract19, DocumentsContract19.getDocumentId, uri.get())};
    const LocalRef<jobject> childrenUri{getEnv()->CallStaticObjectMethod(
        DocumentsContract21,
        DocumentsContract21.buildChildDocumentsUriUsingTree, uri.get(),
        documentId.get())};

    return DocumentsContractIteratorEngine{
        Columns{GlobalRef{uri},
  ...
  public:
    ...
    bool increment() {
      if (directory && subIterator == nullptr)
        subIterator = std::make_unique<RecursiveEngine>(
            engine.read().getNativeInfo().uri);
...
public:
  ...
  Args withModified(Detail::Opt x) const {
    return with(&Args::lastModified, x);
  }
  Args withReadPermission(bool x) const {
    return with(&Args::readPermission, x);
  }
  Args withWritePermission(bool x) const {
    return with(&Args::writePermission, x);
  }
  ...
  AndroidDocumentInfo build() const { return AndroidDocumentInfo(*this); }

private:
  template <typename Value> Args with(Value Args::*member, Value v) const {
    auto copy = *this;
...
AndroidDocumentInfo::AndroidDocumentInfo(Args args)
    : name(args.name), type(args.type), lastModified(args.lastModified.value),
      sizeInBytes(args.sizeInBytes.value), nativeFlags(args.flags),
      juceFlags(flagExists | (args.lastModified.valid ? flagValidModified : 0) |
...
bool AndroidDocumentInfo::isDirectory() const {
  return type == AndroidDocumentDetail::dirMime;
}
...
public:
  ...
  virtual std::unique_ptr<Pimpl>
  copyDocumentToParentDocument(const Pimpl &) const {
  ...
  virtual std::unique_ptr<Pimpl>
  moveDocumentFromParentToParent(const Pimpl &, const Pimpl &) const {
    // This function is not supported on the current platform.
    jassertfalse;
    return {};
  }
  ...
  virtual std::unique_ptr<Pimpl>
  createChildDocumentWithTypeAndName(const String &, const String &) const {
    // This function is not supported on the current platform.
    jassertfalse;
    return {};
  }
  ...
  static const Pimpl &getPimpl(const AndroidDocument &doc) {
    return *doc.pimpl;
  }
};
...
struct AndroidDocument::Utils {
...
#if JUCE_ANDROID
  template <typename> struct VersionTag {
    int version;
  };
  ...
  public:
    String getMimeTypeFromExtension(const String &str) const {
      ...
      return juceString((jstring)getEnv()->CallObjectMethod(
          map.get(), AndroidMimeTypeMap.getMimeTypeFromExtension,
          javaStr.get()));
    }
    ...
    String getExtensionFromMimeType(const String &str) const {
      const auto javaStr = javaString(str);
      return juceString((jstring)getEnv()->CallObjectMethod(
          map.get(), AndroidMimeTypeMap.getExtensionFromMimeType,
          javaStr.get()));
    }

  private:
    GlobalRef map{LocalRef<jobject>{getEnv()->CallStaticObjectMethod(
        AndroidMimeTypeMap, AndroidMimeTypeMap.getSingleton)}};
  };
  ...
  public:
    ...
    std::unique_ptr<Pimpl> clone() const override {
      return std::make_unique<AndroidDocumentPimplApi19>(*this);
    }

    bool deleteDocument() const override {
      if (const auto resolver =
              AndroidContentUriResolver::getContentResolver()) {
        return getEnv()->CallStaticBooleanMethod(
            DocumentsContract19, DocumentsContract19.deleteDocument,
            resolver.get(), uri.get());
      }
    ...
    std::unique_ptr<OutputStream> createOutputStream() const override {
      auto stream = AndroidStreamHelpers::createStream(
          uri, AndroidStreamHelpers::StreamKind::output);

      return stream.get() != nullptr
                 ? std::make_unique<AndroidContentUriOutputStream>(
                       std::move(stream))
                 : nullptr;
    }
    ...
    AndroidDocumentInfo getInfo() const override {
      struct Columns {
        auto getColumnNames() const {
          return Detail::makeStringArray(flagsColumn, nameColumn, mimeColumn,
                                         idColumn, modifiedColumn, sizeColumn);
        }

        auto readFromCursor(jobject cursor) const {
          auto *env = getEnv();

          const auto flagsColumnIndex = env->CallIntMethod(
              cursor, AndroidCursor.getColumnIndex, flagsColumn.get());
          const auto nameColumnIndex = env->CallIntMethod(
              cursor, AndroidCursor.getColumnIndex, nameColumn.get());
          const auto mimeColumnIndex = env->CallIntMethod(
              cursor, AndroidCursor.getColumnIndex, mimeColumn.get());
          const auto idColumnIndex = env->CallIntMethod(
              cursor, AndroidCursor.getColumnIndex, idColumn.get());
          const auto modColumnIndex = env->CallIntMethod(
              cursor, AndroidCursor.getColumnIndex, modifiedColumn.get());
          const auto sizeColumnIndex = env->CallIntMethod(
              cursor, AndroidCursor.getColumnIndex, sizeColumn.get());

          const auto indices = {flagsColumnIndex, nameColumnIndex,
                                mimeColumnIndex,  idColumnIndex,
                                modColumnIndex,   sizeColumnIndex};

          if (std::any_of(indices.begin(), indices.end(),
                          [](auto index) { return index < 0; }))
            return AndroidDocumentInfo::Args{};

          const LocalRef<jstring> nameString{(jstring)env->CallObjectMethod(
              cursor, AndroidCursor.getString, nameColumnIndex)};
          const LocalRef<jstring> mimeString{(jstring)env->CallObjectMethod(
              cursor, AndroidCursor.getString, mimeColumnIndex)};

          const auto readOpt = [&](int column) -> Detail::Opt {
            const auto missing =
                env->CallBooleanMethod(cursor, AndroidCursor.isNull, column);

            if (missing)
              return {};

            return Detail::Opt{
                env->CallLongMethod(cursor, AndroidCursor.getLong, column)};
          };

          return AndroidDocumentInfo::Args{}
              .withName(juceString(nameString.get()))
              .withType(juceString(mimeString.get()))
              .withFlags(env->CallIntMethod(cursor, AndroidCursor.getInt,
                                            flagsColumnIndex))
      ...
      const auto hasPermission = [&](auto permission) {
        return env->CallIntMethod(
                   ...
                   uri.get(), permission) == 0;
      };

      return iterator.read()
          .withReadPermission(
              hasPermission(Detail::FLAG_GRANT_READ_URI_PERMISSION))
          .withWritePermission(
              hasPermission(Detail::FLAG_GRANT_WRITE_URI_PERMISSION))
          .build();
    }

    URL getUrl() const override { return Detail::uriToUrl(uri); }
  ...
  public:
    ...
    std::unique_ptr<Pimpl> clone() const override {
      return std::make_unique<AndroidDocumentPimplApi21>(*this);
    }

    std::unique_ptr<Pimpl>
    createChildDocumentWithTypeAndName(const String &type,
                                       const String &name) const override {
      return Utils::createPimplForSdk(
          LocalRef<jobject>{getEnv()->CallStaticObjectMethod(
              DocumentsContract21, DocumentsContract21.createDocument,
              AndroidContentUriResolver::getContentResolver().get(),
              getNativeInfo().uri.get(), javaString(type).get(),
              javaString(name).get())});
    }
    ...
    std::unique_ptr<Pimpl> renameTo(const String &name) const override {
      if (const auto resolver =
        ...
        return Utils::createPimplForSdk(
            LocalRef<jobject>{getEnv()->CallStaticObjectMethod(
                DocumentsContract21, DocumentsContract21.renameDocument,
                resolver.get(), getNativeInfo().uri.get(),
                javaString(name).get())});
      }
  ...
  public:
    ...
    std::unique_ptr<Pimpl> clone() const override {
      return std::make_unique<AndroidDocumentPimplApi24>(*this);
    }

    std::unique_ptr<Pimpl>
    copyDocumentToParentDocument(const Pimpl &target) const override {
      ...
      return Utils::createPimplForSdk(
          LocalRef<jobject>{getEnv()->CallStaticObjectMethod(
              DocumentsContract24, DocumentsContract24.copyDocument,
              AndroidContentUriResolver::getContentResolver().get(),
              getNativeInfo().uri.get(), target.getNativeInfo().uri.get())});
    }

    std::unique_ptr<Pimpl>
    moveDocumentFromParentToParent(const Pimpl &currentParent,
                                   const Pimpl &newParent) const override {
      if (currentParent.getNativeInfo().uri == nullptr ||
          newParent.getNativeInfo().uri == nullptr) {
      ...
      return Utils::createPimplForSdk(
          LocalRef<jobject>{getEnv()->CallStaticObjectMethod(
              DocumentsContract24, DocumentsContract24.moveDocument,
              AndroidContentUriResolver::getContentResolver().get(),
  ...
  createPimplForSdk(const LocalRef<jobject> &uri) {
    if (jniCheckHasExceptionOccurredAndClear())
      return nullptr;

    return createPimplForSdkImpl(uri, VersionTag<AndroidDocumentPimplApi24>{24},
                                 VersionTag<AndroidDocumentPimplApi21>{21},
                                 VersionTag<AndroidDocumentPimplApi19>{19});
  }

  static std::unique_ptr<Pimpl>
  createPimplForSdkImpl(const LocalRef<jobject> &) {
  ...
  static std::unique_ptr<Pimpl>
  createPimplForSdkImpl(const LocalRef<jobject> &uri, VersionTag<T> head,
                        VersionTag<Ts>... tail) {
  ...
  public:
    explicit AndroidDocumentPimplFile(const File &f) : file(f) {}

    std::unique_ptr<Pimpl> clone() const override {
      return std::make_unique<AndroidDocumentPimplFile>(*this);
    }
    ...
    std::unique_ptr<Pimpl> renameTo(const String &name) const override {
      ...
      return file.moveFileTo(target)
                 ? std::make_unique<AndroidDocumentPimplFile>(target)
                 : nullptr;
    }
    ...
    createChildDocumentWithTypeAndName(const String &type,
      ...
      const auto target = file.getChildFile(
          extension.isNotEmpty() ? name + "." + extension : name);

      if (!target.exists() &&
          (type == Detail::dirMime ? target.createDirectory()
                                   : target.create()))
    ...
    std::unique_ptr<Pimpl>
    moveDocumentFromParentToParent(const Pimpl &currentParentPimpl,
    ...
    AndroidDocumentInfo getInfo() const override {
      ...
      const auto extension =
          file.getFileExtension().removeCharacters(".").toLowerCase();
      const auto type = file.isDirectory()
                            ? Detail::dirMime
...
//==============================================================================
void AndroidDocumentPermission::takePersistentReadWriteAccess(
    [[maybe_unused]] const URL &url) {
#if JUCE_ANDROID
  AndroidDocumentDetail::setPermissions(
      url, ContentResolver19.takePersistableUriPermission);
#endif
}

void AndroidDocumentPermission::releasePersistentReadWriteAccess(
    [[maybe_unused]] const URL &url) {
#if JUCE_ANDROID
  AndroidDocumentDetail::setPermissions(
      url, ContentResolver19.releasePersistableUriPermission);
#endif
}

std::vector<AndroidDocumentPermission>
AndroidDocumentPermission::getPersistedPermissions() {
#if !JUCE_ANDROID
  return {};
#else
  if (getAndroidSDKVersion() < 19)
  ...
  auto *env = getEnv();
  const LocalRef<jobject> permissions{env->CallObjectMethod(
      AndroidContentUriResolver::getContentResolver().get(),
  ...
  for (auto i = (decltype(size))0; i < size; ++i) {
    const LocalRef<jobject> uriPermission{
        env->CallObjectMethod(permissions, JavaList.get, i)};

    AndroidDocumentPermission permission;
    permission.time = env->CallLongMethod(
        uriPermission, AndroidUriPermission.getPersistedTime);
    permission.read = env->CallBooleanMethod(
        uriPermission, AndroidUriPermission.isReadPermission);
    permission.write = env->CallBooleanMethod(
        uriPermission, AndroidUriPermission.isWritePermission);
    permission.url = AndroidDocumentDetail::uriToUrl(
        env->CallObjectMethod(uriPermission, AndroidUriPermission.getUri));
...
#if JUCE_ANDROID
  const LocalRef<jobject> info{getEnv()->CallObjectMethod(
      getAppContext(), AndroidContext.getApplicationInfo)};
  const auto targetSdkVersion = getEnv()->GetIntField(
      info.get(), AndroidApplicationInfo.targetSdkVersion);

  // At the current API level, plain file paths may not work for accessing files
  // in shared locations. It's recommended to use fromDocument() or fromTree()
  // instead when targeting this API level.
  jassert(__ANDROID_API_Q__ <= targetSdkVersion);
#endif

  return AndroidDocument{
      filePath != File()
          ? std::make_unique<Utils::AndroidDocumentPimplFile>(filePath)
          : nullptr};
}

AndroidDocument
AndroidDocument::fromDocument([[maybe_unused]] const URL &documentUrl) {
#if JUCE_ANDROID
  if (getAndroidSDKVersion() < 19) {
  ...
  if (!getEnv()->CallStaticBooleanMethod(
          DocumentsContract19, DocumentsContract19.isDocumentUri,
          getAppContext().get(), javaUri.get())) {
    return AndroidDocument{};
  }
...
AndroidDocument AndroidDocument::fromTree([[maybe_unused]] const URL &treeUrl) {
  ...
  LocalRef<jobject> treeDocumentId{getEnv()->CallStaticObjectMethod(
      DocumentsContract21, DocumentsContract21.getTreeDocumentId,
      javaUri.get())};
  ...
  LocalRef<jobject> documentUri{getEnv()->CallStaticObjectMethod(
      DocumentsContract21, DocumentsContract21.buildDocumentUriUsingTree,
      javaUri.get(), treeDocumentId.get())};
...
AndroidDocument &
AndroidDocument::operator=(AndroidDocument &&) noexcept = default;
...
AndroidDocument AndroidDocument::copyDocumentToParentDocument(
    const AndroidDocument &target) const {
  jassert(hasValue() && target.hasValue());
  return AndroidDocument{pimpl->copyDocumentToParentDocument(*target.pimpl)};
}

AndroidDocument
AndroidDocument::createChildDocumentWithTypeAndName(const String &type,
...
AndroidDocument::createChildDirectory(const String &name) const {
  return createChildDocumentWithTypeAndName(AndroidDocumentDetail::dirMime,
                                            name);
}

bool AndroidDocument::moveDocumentFromParentToParent(
    const AndroidDocument &currentParent, const AndroidDocument &newParent) {
  jassert(hasValue() && currentParent.hasValue() && newParent.hasValue());
  auto moved = pimpl->moveDocumentFromParentToParent(*currentParent.pimpl,
                                                     *newParent.pimpl);
...
struct AndroidDocumentIterator::Utils {
  ...
  static AndroidDocumentIterator makeWithEngineInplace(Args &&...args) {
    return AndroidDocumentIterator{
        std::make_unique<TemplatePimpl<Engine>>(std::forward<Args>(args)...)};
  }

  template <typename Engine>
  static AndroidDocumentIterator makeWithEngine(Engine engine) {
    return AndroidDocumentIterator{
        std::make_unique<TemplatePimpl<Engine>>(std::move(engine))};
  }
...
AndroidDocumentIterator::makeNonRecursive(const AndroidDocument &dir) {
  if (!dir.hasValue())
    return {};
  ...
  if (21 <= getAndroidSDKVersion()) {
    if (auto uri = dir.getNativeInfo().uri)
      return Utils::makeWithEngine(
          Detail::makeDocumentsContractIteratorEngine(uri));
  }
#endif

  return Utils::makeWithEngineInplace<Detail::DirectoryIteratorEngine>(
      dir.getUrl().getLocalFile(), false);
}

AndroidDocumentIterator
AndroidDocumentIterator::makeRecursive(const AndroidDocument &dir) {
  if (!dir.hasValue())
    return {};
