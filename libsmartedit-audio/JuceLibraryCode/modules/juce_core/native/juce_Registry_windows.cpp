...
struct RegistryKeyWrapper {
  RegistryKeyWrapper(String name, bool createForWriting, DWORD wow64Flags) {
    if (HKEY rootKey = getRootKey(name)) {
      ...
      if (createForWriting)
        RegCreateKeyEx(
            rootKey, wideCharName, 0, nullptr, REG_OPTION_NON_VOLATILE,
  ...
  static HKEY getRootKey(const String &name) noexcept {
    ...
    jassertfalse; // The name starts with an unknown root key (or maybe an old
                  // Win9x type)
    return nullptr;
  }
  ...
  static uint32 getBinaryValue(const String &regValuePath, MemoryBlock &result,
                               DWORD wow64Flags) {
    ...
    if (key.key != nullptr) {
      for (unsigned long bufferSize = 1024;; bufferSize *= 2) {
        ...
        auto err =
            RegQueryValueEx(key.key, key.wideCharValueName, nullptr, &type,
  ...
  static String getValue(const String &regValuePath, const String &defaultValue,
                         DWORD wow64Flags) {
    MemoryBlock buffer;
  ...
  static bool keyExists(const String &regKeyPath, const DWORD wow64Flags) {
    return RegistryKeyWrapper(regKeyPath + "\\", false, wow64Flags).key !=
           nullptr;
  }
  ...
  static bool valueExists(const String &regValuePath, const DWORD wow64Flags) {
    ...
    auto result = RegQueryValueEx(key.key, key.wideCharValueName, nullptr,
...
uint32 JUCE_CALLTYPE WindowsRegistry::getBinaryValue(const String &regValuePath,
                                                     MemoryBlock &result,
                                                     WoW64Mode mode) {
  return RegistryKeyWrapper::getBinaryValue(regValuePath, result, (DWORD)mode);
}

String JUCE_CALLTYPE WindowsRegistry::getValue(const String &regValuePath,
                                               const String &defaultValue,
                                               WoW64Mode mode) {
  return RegistryKeyWrapper::getValue(regValuePath, defaultValue, (DWORD)mode);
}

bool JUCE_CALLTYPE WindowsRegistry::setValue(const String &regValuePath,
                                             const String &value,
                                             WoW64Mode mode) {
  return RegistryKeyWrapper::setValue(
      regValuePath, REG_SZ, value.toWideCharPointer(),
...
bool JUCE_CALLTYPE WindowsRegistry::setValue(const String &regValuePath,
                                             const uint32 value,
                                             WoW64Mode mode) {
  return RegistryKeyWrapper::setValue(regValuePath, REG_DWORD, &value,
                                      sizeof(value), (DWORD)mode);
}

bool JUCE_CALLTYPE WindowsRegistry::setValue(const String &regValuePath,
                                             const uint64 value,
                                             WoW64Mode mode) {
  return RegistryKeyWrapper::setValue(regValuePath, REG_QWORD, &value,
                                      sizeof(value), (DWORD)mode);
}
...
bool JUCE_CALLTYPE WindowsRegistry::setValue(const String &regValuePath,
                                             ...
                                             WoW64Mode mode) {
  return RegistryKeyWrapper::setValue(regValuePath, REG_BINARY, value.getData(),
                                      value.getSize(), (DWORD)mode);
}

bool JUCE_CALLTYPE WindowsRegistry::valueExists(const String &regValuePath,
                                                WoW64Mode mode) {
  return RegistryKeyWrapper::valueExists(regValuePath, (DWORD)mode);
}

bool JUCE_CALLTYPE WindowsRegistry::keyExists(const String &regKeyPath,
                                              WoW64Mode mode) {
  return RegistryKeyWrapper::keyExists(regKeyPath, (DWORD)mode);
}

bool JUCE_CALLTYPE WindowsRegistry::deleteValue(const String &regValuePath,
                                                WoW64Mode mode) {
  const RegistryKeyWrapper key(regValuePath, true, (DWORD)mode);

  return key.key != nullptr &&
         RegDeleteValue(key.key, key.wideCharValueName) == ERROR_SUCCESS;
}

static bool deleteKeyNonRecursive(const String &regKeyPath,
                                  WindowsRegistry::WoW64Mode mode) {
  const RegistryKeyWrapper key(regKeyPath, true, (DWORD)mode);

  return key.key != nullptr &&
         RegDeleteKey(key.key, key.wideCharValueName) == ERROR_SUCCESS;
}

bool JUCE_CALLTYPE WindowsRegistry::deleteKey(const String &regKeyPath,
                                              WoW64Mode mode) {
  ...
  for (const RegistryKeyWrapper key(regKeyPath + "\\", false, (DWORD)mode);;) {
    ...
    if (RegEnumKeyEx(key.key, 0, subKey, &subKeySize, nullptr, nullptr, nullptr,
...
bool JUCE_CALLTYPE WindowsRegistry::registerFileAssociation(
    const String &fileExtension, const String &symbolicDescription,
    const String &fullDescription, const File &targetExecutable,
    const int iconResourceNumber, const bool registerForCurrentUserOnly,
    WoW64Mode mode) {
  auto root = registerForCurrentUserOnly
  ...
  return setValue(root + fileExtension + "\\", symbolicDescription, mode) &&
         ...
         setValue(key + "\\shell\\open\\command\\",
                  targetExecutable.getFullPathName() + " \"%1\"", mode) &&
         (iconResourceNumber == 0 ||
          setValue(key + "\\DefaultIcon\\", targetExecutable.getFullPathName() +
                                                "," +
                                                String(iconResourceNumber)));
}
