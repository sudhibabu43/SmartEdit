...
JNIEnv *getEnv() noexcept {
  ...
  // You did not call Thread::initialiseJUCE which must be called at least once
  // in your apk before using any JUCE APIs. The Projucer will automatically
  // generate java code which will invoke Thread::initialiseJUCE for you.
  jassertfalse;
  return nullptr;
}

static void JNICALL juce_JavainitialiseJUCE(JNIEnv *env, jobject /*jclass*/,
                                            jobject context) {
  ...
  void checkActivityIsMain(jobject context) {
    ...
    if (mainActivity == nullptr) {
      ...
      if (mainActivityPath.isNotEmpty()) {
        ...
        auto activityPath = juceString(LocalRef<jstring>(
            (jstring)env->CallObjectMethod(clasz, JavaClass.getName)));

        // This may be problematic for apps which use several activities with
        // the same type. We just assume that the very first activity of this
  ...
  static String getMainActivityClassPath() {
    ...
    if (mainActivityClassPath.isEmpty()) {
      ...
      if (appContext != nullptr) {
        ...
        LocalRef<jstring> pkgName((jstring)env->CallObjectMethod(
            appContext.get(), AndroidContext.getPackageName));

        LocalRef<jobject> intent(
            env->NewObject(AndroidIntent, AndroidIntent.constructWithString,
                           javaString("android.intent.action.MAIN").get()));

        intent = LocalRef<jobject>(env->CallObjectMethod(
            intent.get(), AndroidIntent.setPackage, pkgName.get()));

        LocalRef<jobject> resolveInfo(env->CallObjectMethod(
            pkgManager.get(), AndroidPackageManager.resolveActivity,
            intent.get(), 0));

        if (resolveInfo != nullptr) {
          LocalRef<jobject> activityInfo(env->GetObjectField(
              resolveInfo.get(), AndroidResolveInfo.activityInfo));
          LocalRef<jstring> jName((jstring)env->GetObjectField(
              activityInfo.get(), AndroidPackageItemInfo.name));
          LocalRef<jstring> jPackage((jstring)env->GetObjectField(
              activityInfo.get(), AndroidPackageItemInfo.packageName));
...
void Thread::initialiseJUCE(void *jniEnv, void *context) {
  ...
  if (firstCall) {
    ...
    // if we ever support unloading then this should probably be a weak
    // reference
...
LocalRef<jobject> getAppContext() noexcept {
  ...
  LocalRef<jobject> applicationContext(
      env->CallObjectMethod(context, AndroidContext.getApplicationContext));
...
//==============================================================================
using RealtimeThreadFactory = pthread_t (*)(void *(*entry)(void *),
                                            void *userPtr);
// This is defined in the juce_audio_devices module, with different definitions
// depending on whether OpenSL/Oboe are enabled.
...
static auto setPriorityOfThisThread(Thread::Priority p) {
  return setpriority(PRIO_PROCESS, (id_t)gettid(),
