...
public:
  Pimpl(const String &pipePath, bool createPipe)
      : pipeInName(pipePath + "_in"), pipeOutName(pipePath + "_out"),
    ...
    while (bytesRead < maxBytesToRead) {
      ...
      if (numRead <= 0) {
        ...
        if (!(error == EWOULDBLOCK || error == EAGAIN) ||
            stopReadOperation.load() || hasExpired(timeoutEnd))
          return -1;

        const int maxWaitingTime = 30;
        waitForInput(pipe, timeoutEnd == 0
                               ? maxWaitingTime
                               : jmin(maxWaitingTime,
                                      (int)(timeoutEnd -
  ...
  int write(const char *sourceBuffer, int numBytesToWrite,
  ...
  static bool createFifo(const String &name, bool mustNotExist) {
    return mkfifo(name.toUTF8(), 0666) == 0 ||
           ((!mustNotExist) && errno == EEXIST);
  }
  ...
  public:
    template <typename Fn> int get(Fn &&fn) {
      {
...
private:
  ...
  static uint32 getTimeoutEnd(int timeOutMilliseconds) {
    return timeOutMilliseconds >= 0
               ? Time::getMillisecondCounter() + (uint32)timeOutMilliseconds
               : 0;
  }
  ...
  int openPipe(const String &name, int flags, uint32 timeoutEnd) {
    for (;;) {
      ...
      if (p != invalidPipe || hasExpired(timeoutEnd) ||
          stopReadOperation.load())
        return p;
...
void NamedPipe::close() {
  {
    ...
    if (pimpl != nullptr) {
      ...
      [[maybe_unused]] const auto done =
          ::write(pimpl->pipeIn.get(), buffer, numElementsInArray(buffer));
    }
  }
...
bool NamedPipe::openInternal(const String &pipeName, bool createPipe,
                             bool mustNotExist) {
#if JUCE_IOS
...
int NamedPipe::read(void *destBuffer, int maxBytesToRead,
  ...
  return pimpl != nullptr ? pimpl->read(static_cast<char *>(destBuffer),
                                        maxBytesToRead, timeOutMilliseconds)
                          : -1;
}
...
int NamedPipe::write(const void *sourceBuffer, int numBytesToWrite,
  ...
  return pimpl != nullptr
             ? pimpl->write(static_cast<const char *>(sourceBuffer),
                            numBytesToWrite, timeOutMilliseconds)
             : -1;
}
