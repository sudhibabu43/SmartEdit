...
void MACAddress::findAllAddresses(Array<MACAddress> &result) {
  ...
  if (getifaddrs(&addrs) == 0) {
    for (const ifaddrs *cursor = addrs; cursor != nullptr;
...
bool JUCE_CALLTYPE Process::openEmailWithAttachments(
    [[maybe_unused]] const String &targetEmailAddress,
  ...
  JUCE_AUTORELEASEPOOL {
    ...
    NSAppleScript *s =
        [[NSAppleScript alloc] initWithSource:juceStringToNS(script)];
...
public:
  explicit URLConnectionStateBase(NSURLRequest *req, int maxRedirects)
      : Thread("http connection"), request([req retain]),
...
// so we'll turn off deprecation warnings. This code will be removed at some
// point in the future.
...
public:
  ...
  bool start(WebInputStream &inputStream,
             WebInputStream::Listener *listener) override {
    startThread();

    while (isThreadRunning() && !initialised) {
      if (listener != nullptr)
        if (!listener->postDataSendProgress(inputStream, (int)latestTotalBytes,
  ...
  int read(char *dest, int numBytes) override {
    ...
    while (numBytes > 0) {
      ...
      if (available > 0) {
        ...
        [data replaceBytesInRange:NSMakeRange(0, (NSUInteger)available)
                        withBytes:nil
                           length:0];
  ...
  NSURLRequest *willSendRequest(NSURLRequest *newRequest,
  ...
  void didSendBodyData(NSInteger totalBytesWritten,
...
private:
  ...
  struct DelegateClass final : public ObjCClass<NSObject> {
    DelegateClass() : ObjCClass<NSObject>("JUCENetworkDelegate_") {
      ...
      addMethod(@selector(connection:willSendRequest:redirectResponse:),
                willSendRequest);
    ...
    static URLConnectionStatePreYosemite *getState(id self) {
      return getIvar<URLConnectionStatePreYosemite *>(self, "state");
    }

  private:
    static void didReceiveResponse(id self, SEL, NSURLConnection *,
                                   NSURLResponse *response) {
      getState(self)->didReceiveResponse(response);
    }

    static void didFailWithError(id self, SEL, NSURLConnection *,
                                 NSError *error) {
      getState(self)->didFailWithError(error);
    }

    static void didReceiveData(id self, SEL, NSURLConnection *,
                               NSData *newData) {
      getState(self)->didReceiveData(newData);
    }

    static NSURLRequest *willSendRequest(id self, SEL, NSURLConnection *,
                                         NSURLRequest *request,
                                         NSURLResponse *response) {
      return getState(self)->willSendRequest(request, response);
    }

    static void connectionDidSendBodyData(id self, SEL, NSURLConnection *,
                                          NSInteger,
...
//==============================================================================
class API_AVAILABLE(macos(10.9)) URLConnectionState final
    : public URLConnectionStateBase {
public:
  ...
  void didReceiveResponse(NSURLResponse *response, id completionHandler) {
    ...
    if (completionHandler != nil) {
      ...
      void (^callbackBlock)(NSURLSessionResponseDisposition) =
          completionHandler;
  ...
  void willPerformHTTPRedirection(NSURLRequest *urlRequest,
...
private:
  ...
  private:
    static void didReceiveResponse(id self, SEL, NSURLSession *,
    ...
    static void didBecomeInvalidWithError(id self, SEL, NSURLSession *,
                                          NSError *error) {
      if (auto state = getState(self))
        state->didComplete(error);
    }

    static void didReceiveData(id self, SEL, NSURLSession *,
                               NSURLSessionDataTask *, NSData *newData) {
      if (auto state = getState(self))
        state->didReceiveData(newData);
    }

    static void didSendBodyData(id self, SEL, NSURLSession *,
                                NSURLSessionTask *, int64_t,
                                int64_t totalBytesWritten, int64_t) {
      if (auto state = getState(self))
        state->didSendBodyData(totalBytesWritten);
    }

    static void
    willPerformHTTPRedirection(id self, SEL, NSURLSession *, NSURLSessionTask *,
                               NSHTTPURLResponse *, NSURLRequest *request,
                               void (^completionHandler)(NSURLRequest *)) {
      if (auto state = getState(self))
        state->willPerformHTTPRedirection(request, completionHandler);
    }

    static void didCompleteWithError(id self, SEL, NSURLConnection *,
                                     NSURLSessionTask *, NSError *error) {
      if (auto state = getState(self))
        state->didComplete(error);
    }
  };
...
struct BackgroundDownloadTask final : public URL::DownloadTask {
  BackgroundDownloadTask(const URL &urlToUse, const File &targetLocationToUse,
                         const URL::DownloadTaskOptions &options)
      : listener(options.listener),
        uniqueIdentifier(String(urlToUse.toString(true).hashCode64()) +
                         String(Random().nextInt64())) {
    ...
    for (int i = 0; i < headerLines.size(); ++i) {
      String key =
          headerLines[i].upToFirstOccurrenceOf(":", false, false).trim();
      String value =
          headerLines[i].fromFirstOccurrenceOf(":", false, false).trim();

      if (key.isNotEmpty() && value.isNotEmpty())
        [request addValue:juceStringToNS(value)
            forHTTPHeaderField:juceStringToNS(key)];
    }

    auto *configuration = [NSURLSessionConfiguration
        backgroundSessionConfigurationWithIdentifier:juceStringToNS(
                                                         uniqueIdentifier)];

    if (options.sharedContainer.isNotEmpty())
      [configuration
          setSharedContainerIdentifier:juceStringToNS(options.sharedContainer)];
    ...
    // Workaround for an Apple bug. See
  ...
  bool initOK() { return (downloadTask != nullptr); }
  ...
  static HashMap<String, BackgroundDownloadTask *, DefaultHashFunctions,
                 CriticalSection>
      activeSessions;
  ...
  void didWriteData(int64 totalBytesWritten, int64 totalBytesExpectedToWrite) {
    ...
    if (connectFinished && error == false && finished == false &&
        listener != nullptr)
  ...
  static int getHTTPErrorCode(NSError *nsError) {
    // see
    // https://developer.apple.com/reference/foundation/nsurlsessiondownloadtask?language=objc
    switch ([nsError code]) {
  ...
  //==============================================================================
  struct DelegateClass final
      : public ObjCClass<NSObject<NSURLSessionDelegate>> {
    DelegateClass()
    ...
    static void setState(id self, BackgroundDownloadTask *state) {
      object_setInstanceVariable(self, "state", state);
    }
    static BackgroundDownloadTask *getState(id self) {
      return getIvar<BackgroundDownloadTask *>(self, "state");
    }

  private:
    static void didWriteData(id self, SEL, NSURLSession *,
                             NSURLSessionDownloadTask *, int64_t,
    ...
    static void didFinishDownloadingToURL(id self, SEL, NSURLSession *,
    ...
    static void didCompleteWithError(id self, SEL, NSURLSession *,
                                     NSURLSessionTask *, NSError *nsError) {
      if (auto state = getState(self))
        state->didCompleteWithError(nsError);
    }

    static void didBecomeInvalidWithError(id self, SEL, NSURLSession *,
                                          NSURLSessionTask *, NSError *) {
      if (auto state = getState(self))
...
HashMap<String, BackgroundDownloadTask *, DefaultHashFunctions, CriticalSection>
    BackgroundDownloadTask::activeSessions;

std::unique_ptr<URL::DownloadTask>
URL::downloadToFile(const File &targetLocation,
                    const DownloadTaskOptions &options) {
  auto downloadTask =
      std::make_unique<BackgroundDownloadTask>(*this, targetLocation, options);
...
URL::downloadToFile(const File &targetLocation,
                    const DownloadTaskOptions &options) {
  return URL::DownloadTask::createFallbackDownloader(*this, targetLocation,
                                                     options);
}
#endif
...
public:
  ...
  bool connect(WebInputStream::Listener *webInputListener,
    ...
    if (!connection->start(owner, webInputListener)) {
      ...
      // Workaround for macOS versions below 10.10 where HTTPS POST requests
      // with keep-alive fail with the NSURLErrorNetworkConnectionLost error
      // code.
  ...
  //==============================================================================
  bool isError() const {
    return (connection == nullptr || connection->getHeaders() == nullptr);
  }
  int64 getTotalLength() {
    return connection == nullptr ? -1 : connection->getContentLength();
  }
  ...
  int read(void *buffer, int bytesToRead) {
    ...
    JUCE_AUTORELEASEPOOL {
      const int bytesRead =
          connection->read(static_cast<char *>(buffer), bytesToRead);
      position += bytesRead;
  ...
  bool setPosition(int64 wantedPos) {
    if (wantedPos != position) {
      ...
      while (numBytesToSkip > 0 && !isExhausted())
        numBytesToSkip -=
            read(temp, (int)jmin(numBytesToSkip, (int64)skipBufferSize));
    }
...
private:
  ...
  void createConnection() {
    ...
    if (NSURL *nsURL = [NSURL
            URLWithString:juceStringToNS(
                              url.toString(!addParametersToRequestBody))]) {
      ...
      if (NSMutableURLRequest *req = [NSMutableURLRequest
               requestWithURL:nsURL
                  cachePolicy:NSURLRequestReloadIgnoringLocalCacheData
              timeoutInterval:timeOutSeconds]) {
        if (NSString *httpMethod =
                [NSString stringWithUTF8String:httpRequestCmd.toRawUTF8()]) {
          [req setHTTPMethod:httpMethod];

          if (hasBodyDataToSend) {
            WebInputStream::createHeadersAndPostData(
                url, headers, postData, addParametersToRequestBody);
          ...
          for (int i = 0; i < headerLines.size(); ++i) {
            ...
            if (key.isNotEmpty() && value.isNotEmpty())
              [req addValue:juceStringToNS(value)
                  forHTTPHeaderField:juceStringToNS(key)];
          }
          ...
          if (@available(macOS 10.10, *))
            connection =
                std::make_unique<URLConnectionState>(req, numRedirectsToFollow);
#if JUCE_MAC
          else
            connection = std::make_unique<URLConnectionStatePreYosemite>(
                req, numRedirectsToFollow);
#endif
        }
      }
    }
