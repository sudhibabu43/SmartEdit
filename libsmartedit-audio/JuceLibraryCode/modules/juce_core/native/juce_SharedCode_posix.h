...
bool Process::setMaxNumberOfFileHandles(int newMaxNumber) noexcept {
  ...
  if (getrlimit(RLIMIT_NOFILE, &lim) == 0) {
    if (newMaxNumber <= 0 && lim.rlim_cur == RLIM_INFINITY &&
  ...
  lim.rlim_cur = lim.rlim_max =
      newMaxNumber <= 0 ? RLIM_INFINITY : (rlim_t)newMaxNumber;
...
//==============================================================================
namespace {
#if JUCE_LINUX || (JUCE_IOS && (!TARGET_OS_MACCATALYST) &&                     \
                   (!__DARWIN_ONLY_64_BIT_INO_T)) // (this iOS stuff is to avoid
                                                  // a simulator bug)
...
bool juce_stat(const String &fileName, juce_statStruct &info) {
  return fileName.isNotEmpty() && JUCE_STAT(fileName.toUTF8(), &info) == 0;
}
...
void updateStatInfoForFile(const String &path, bool *isDir, int64 *fileSize,
                           ...
                           bool *isReadOnly) {
  if (isDir != nullptr || fileSize != nullptr || modTime != nullptr ||
...
Result getResultForErrno() { return Result::fail(String(strerror(errno))); }
...
bool File::exists() const {
  return fullPath.isNotEmpty() && access(fullPath.toUTF8(), F_OK) == 0;
}

bool File::existsAsFile() const { return exists() && !isDirectory(); }
...
bool File::hasReadAccess() const {
  return fullPath.isNotEmpty() && access(fullPath.toUTF8(), R_OK) == 0;
}

static bool setFileModeFlags(const String &fullPath, mode_t flags,
                             bool shouldSet) noexcept {
  juce_statStruct info;
...
bool File::setFileReadOnlyInternal(bool shouldBeReadOnly) const {
  ...
  return setFileModeFlags(fullPath, S_IWUSR | S_IWGRP | S_IWOTH,
                          !shouldBeReadOnly);
}

bool File::setFileExecutableInternal(bool shouldBeExecutable) const {
  return setFileModeFlags(fullPath, S_IXUSR | S_IXGRP | S_IXOTH,
                          shouldBeExecutable);
}
...
void File::getFileTimesInternal(int64 &modificationTime, int64 &accessTime,
  ...
  if (juce_stat(fullPath, info)) {
    ...
    modificationTime = (int64)info.st_mtimespec.tv_sec * 1000 +
                       info.st_mtimespec.tv_nsec / 1000000;
    accessTime = (int64)info.st_atimespec.tv_sec * 1000 +
                 info.st_atimespec.tv_nsec / 1000000;
    creationTime = (int64)info.st_birthtimespec.tv_sec * 1000 +
...
bool File::setFileTimesInternal(int64 modificationTime, int64 accessTime,
  ...
  if ((modificationTime != 0 || accessTime != 0) && juce_stat(fullPath, info)) {
    ...
    times[0].tv_sec = setAccessTime
                          ? static_cast<__darwin_time_t>(accessTime / 1000)
                          : info.st_atimespec.tv_sec;

    times[0].tv_usec =
        setAccessTime
            ? static_cast<__darwin_suseconds_t>((accessTime % 1000) * 1000)
            : static_cast<__darwin_suseconds_t>(info.st_atimespec.tv_nsec /
                                                1000);

    times[1].tv_sec =
        setModificationTime
            ? static_cast<__darwin_time_t>(modificationTime / 1000)
            : info.st_mtimespec.tv_sec;

    times[1].tv_usec = setModificationTime
    ...
    times.actime = accessTime != 0 ? static_cast<time_t>(accessTime / 1000)
                                   : static_cast<time_t>(info.st_atime);
    times.modtime = modificationTime != 0
                        ? static_cast<time_t>(modificationTime / 1000)
                        : static_cast<time_t>(info.st_mtime);
...
Result FileOutputStream::truncate() {
  ...
  flush();
  return getResultForReturnValue(
      ftruncate(getFD(fileHandle), (off_t)currentPosition));
}
...
int64 File::getBytesFreeOnVolume() const {
  ...
  if (juce_doStatFS(*this, buf))
    return (int64)buf.f_bsize *
           (int64)buf.f_bavail; // Note: this returns space available to
                                // non-super user
...
String File::getVolumeLabel() const {
  ...
  zerostruct(
      attrList); // (can't use "= {}" on this object because it's a C struct)
  ...
  for (;;) {
    if (getattrlist(f.getFullPathName().toUTF8(), &attrList, &attrBuf,
                    sizeof(attrBuf), 0) == 0)
      return String::fromUTF8(((const char *)&attrBuf.mountPointRef) +
...
String juce_getOutputFromCommand(const String &command) {
  // slight bodge here, as we just pipe the output into a temp file and read
  // it...
  auto tempFile =
      File::getSpecialLocation(File::tempDirectory)
          .getNonexistentChildFile(
              String::toHexString(Random::getSystemRandom().nextInt()), ".tmp",
              false);
...
public:
...
#if JUCE_MAC
    if (!createLockFile(
            File("~/Library/Caches/com.juce.locks").getChildFile(lockName),
            timeOutMillisecs))
      // Fallback if the user's home folder is on a network drive with no
      // ability to lock..
      createLockFile(File("/tmp/com.juce.locks").getChildFile(lockName),
                     timeOutMillisecs);
  ...
  bool createLockFile(const File &file, int timeOutMillisecs) {
    ...
    closeFile();
    return true; // only false if there's a file system error. Failure to lock
                 // still returns true.
  }
...
public:
  ...
  static PosixSchedulerPriority
  getNativeSchedulerAndPriority(const Optional<Thread::RealtimeOptions> &rt,
...
#elif JUCE_LINUX
    const auto backgroundSched =
        prio == Thread::Priority::background ? SCHED_IDLE : SCHED_OTHER;
...
static void *makeThreadHandle(PosixThreadAttribute &attr, void *userData,
...
void JUCE_CALLTYPE Thread::yield() { sched_yield(); }
...
void JUCE_CALLTYPE
Thread::setCurrentThreadAffinityMask([[maybe_unused]] uint32 affinityMask) {
  ...
  for (int i = 0; i < 32; ++i) {
    if ((affinityMask & (uint32)(1 << i)) != 0) {
      ...
      JUCE_END_IGNORE_WARNINGS_GCC_LIKE
                        ...
                        ((__GLIBC__ * 1000 + __GLIBC_MINOR__) >= 2004))
  pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &affinity);
#elif JUCE_ANDROID
  sched_setaffinity(gettid(), sizeof(cpu_set_t), &affinity);
#else
  // NB: this call isn't really correct because it sets the affinity of the
  // process, (getpid) not the thread (not gettid). But it's included here as a
  // fallback for people who are using ridiculously old versions of glibc
...
#else
  // affinities aren't supported because either the appropriate header files
  // weren't found, or the SUPPORT_AFFINITIES macro was turned off
  jassertfalse;
#endif
}
...
bool DynamicLibrary::open(const String &name) {
  close();
  handle = dlopen(name.isEmpty() ? nullptr : name.toUTF8().getAddress(),
                  RTLD_LOCAL | RTLD_NOW);
...
static String readPosixConfigFileValue(const char *file, const char *key) {
  ...
  for (int i = lines.size();
       --i >= 0;) // (NB - it's important that this runs in reverse order)
    if (lines[i]
...
public:
  ActiveProcess(const StringArray &arguments, int streamFlags) {
    ...
    // Looks like you're trying to launch a non-existent exe or a folder
    // (perhaps on OSX you're trying to launch the .app folder rather than the
    // actual binary inside it?)
    jassert(
        File::getCurrentWorkingDirectory().getChildFile(exe).existsAsFile() ||
...
#error // some crazy 3rd party headers (e.g. zlib) define this function as NULL!
  ...
  bool killProcess() const noexcept { return ::kill(childPID, SIGKILL) == 0; }
