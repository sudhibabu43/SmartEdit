...
static auto getValueWithDefault(const StringMap &m, const String &key,
...
StringPairArray WavAudioFormat::createBWAVMetadata(
    const String &description, const String &originator,
    const String &originatorRef, Time date, int64 timeReferenceSamples,
...
const char *const WavAudioFormat::aswgIsd = "isd";
...
constexpr inline int chunkName(const char *name) noexcept {
  return (int)ByteOrder::littleEndianInt(name);
}
constexpr inline size_t roundUpSize(size_t sz) noexcept {
  return (sz + 3) & ~3u;
}
...
struct BWAVChunk {
  ...
  void copyTo(StringMap &values, const int totalSize) const {
    values[WavAudioFormat::bwavDescription] =
        String::fromUTF8(description, sizeof(description));
    values[WavAudioFormat::bwavOriginator] =
        String::fromUTF8(originator, sizeof(originator));
    values[WavAudioFormat::bwavOriginatorRef] =
        String::fromUTF8(originatorRef, sizeof(originatorRef));
    values[WavAudioFormat::bwavOriginationDate] =
        String::fromUTF8(originationDate, sizeof(originationDate));
    values[WavAudioFormat::bwavOriginationTime] =
        String::fromUTF8(originationTime, sizeof(originationTime));
    ...
    values[WavAudioFormat::bwavCodingHistory] = String::fromUTF8(
        codingHistory, totalSize - (int)offsetof(BWAVChunk, codingHistory));
  }

  static MemoryBlock createFrom(const StringMap &values) {
    MemoryBlock data(roundUpSize(
        sizeof(BWAVChunk) +
        getValueWithDefault(values, WavAudioFormat::bwavCodingHistory)
            .getNumBytesAsUTF8()));
    data.fillWith(0);

    auto *b = (BWAVChunk *)data.getData();

    // Allow these calls to overwrite an extra byte at the end, which is fine as
    // long as they get called in the right order.
    getValueWithDefault(values, WavAudioFormat::bwavDescription)
        .copyToUTF8(b->description, 257);
    getValueWithDefault(values, WavAudioFormat::bwavOriginator)
        .copyToUTF8(b->originator, 33);
    getValueWithDefault(values, WavAudioFormat::bwavOriginatorRef)
        .copyToUTF8(b->originatorRef, 33);
    getValueWithDefault(values, WavAudioFormat::bwavOriginationDate)
        .copyToUTF8(b->originationDate, 11);
    getValueWithDefault(values, WavAudioFormat::bwavOriginationTime)
        .copyToUTF8(b->originationTime, 9);

    auto time = getValueWithDefault(values, WavAudioFormat::bwavTimeReference)
                    .getLargeIntValue();
    b->timeRefLow = ByteOrder::swapIfBigEndian((uint32)(time & 0xffffffff));
    b->timeRefHigh = ByteOrder::swapIfBigEndian((uint32)(time >> 32));

    getValueWithDefault(values, WavAudioFormat::bwavCodingHistory)
        .copyToUTF8(b->codingHistory, 0x7fffffff);

    if (b->description[0] != 0 || b->originator[0] != 0 ||
        b->originationDate[0] != 0 || b->originationTime[0] != 0 ||
        b->codingHistory[0] != 0 || time != 0) {
      return data;
    }
...
struct SMPLChunk {
  ...
  static void setValue(StringMap &values, int prefix, const char *name,
                       uint32 val) {
  ...
  template <typename NameType>
  static uint32 getValue(const StringMap &values, NameType name,
                         const char *def) {
    return ByteOrder::swapIfBigEndian(
        (uint32)getValueWithDefault(values, name, def).getIntValue());
  }

  static uint32 getValue(const StringMap &values, int prefix, const char *name,
                         const char *def) {
  ...
  static MemoryBlock createFrom(const StringMap &values) {
    MemoryBlock data;
    auto numLoops = jmin(
        64, getValueWithDefault(values, "NumSampleLoops", "0").getIntValue());

    data.setSize(
        roundUpSize(sizeof(SMPLChunk) +
                    (size_t)(jmax(0, numLoops - 1)) * sizeof(SampleLoop)),
        true);
...
struct InstChunk {
  ...
  static int8 getValue(const StringMap &values, const char *name,
                       const char *def) {
...
struct CueChunk {
  ...
  static void setValue(StringMap &values, int prefix, const char *name,
                       uint32 val) {
    values["Cue" + String(prefix) + name] =
        String(ByteOrder::swapIfBigEndian(val));
  }
  ...
  static MemoryBlock createFrom(const StringMap &values) {
    MemoryBlock data;
    const int numCues =
        getValueWithDefault(values, "NumCuePoints", "0").getIntValue();

    if (numCues > 0) {
      data.setSize(
          roundUpSize(sizeof(CueChunk) + (size_t)(numCues - 1) * sizeof(Cue)),
          true);
      ...
      for (int i = 0; i < numCues; ++i) {
        ...
        auto identifier =
            (uint32)getValueWithDefault(values, prefix + "Identifier", "0")
                .getIntValue();
        ...
        auto order =
            getValueWithDefault(values, prefix + "Order", String(nextOrder))
                .getIntValue();
        ...
        cue.chunkID = ByteOrder::swapIfBigEndian(
            (uint32)getValueWithDefault(values, prefix + "ChunkID", dataChunkID)
                .getIntValue());
        cue.chunkStart = ByteOrder::swapIfBigEndian(
            (uint32)getValueWithDefault(values, prefix + "ChunkStart", "0")
                .getIntValue());
        cue.blockStart = ByteOrder::swapIfBigEndian(
            (uint32)getValueWithDefault(values, prefix + "BlockStart", "0")
                .getIntValue());
        cue.offset = ByteOrder::swapIfBigEndian(
            (uint32)getValueWithDefault(values, prefix + "Offset", "0")
                .getIntValue());
      }
    }
...
static int getValue(const StringMap &values, const String &prefix,
                    const char *name) {
  return getValue(values, prefix + name);
}

static void appendLabelOrNoteChunk(const StringMap &values,
                                   const String &prefix, const int chunkType,
...
static void appendExtraChunk(const StringMap &values, const String &prefix,
                             MemoryOutputStream &out) {
  auto text = getValueWithDefault(values, prefix + "Text", prefix);

  auto textLength =
      (int)text.getNumBytesAsUTF8() + 1; // include null terminator
...
static MemoryBlock createFrom(const StringMap &values) {
  ...
  if (numCueLabels + numCueNotes + numCueRegions > 0) {
    ...
    for (int i = 0; i < numCueLabels; ++i)
      appendLabelOrNoteChunk(values, "CueLabel" + String(i), chunkName("labl"),
                             out);

    for (int i = 0; i < numCueNotes; ++i)
      appendLabelOrNoteChunk(values, "CueNote" + String(i), chunkName("note"),
                             out);
...
//==============================================================================
/** Reads a RIFF List Info chunk from a stream positioned just after the size
 * byte. */
namespace ListInfoChunk {
static const char *const types[] = {WavAudioFormat::riffInfoArchivalLocation,
...
static bool isMatchingTypeIgnoringCase(const int value,
                                       const char *const name) noexcept {
  for (int i = 0; i < 4; ++i)
    if ((juce_wchar)name[i] != CharacterFunctions::toUpperCase(
                                   (juce_wchar)((value >> (i * 8)) & 0xff)))
      return false;

  return true;
}

static void addToMetadata(StringMap &values, InputStream &input,
                          int64 chunkEnd) {
    ...
    if (infoLength > 0) {
      ...
      for (auto &type : types) {
        if (isMatchingTypeIgnoringCase(infoType, type)) {
          ...
          values[type] = String::createStringFromData(
              (const char *)mb.getData(), (int)mb.getSize());
          break;
        }
      }
    }
...
static bool writeValue(const StringMap &values, MemoryOutputStream &out,
...
//==============================================================================
struct AcidChunk {
  /** Reads an acid RIFF chunk from a stream positioned just after the size
   * byte. */
  ...
  AcidChunk(const StringMap &values) {
    ...
    if (getValueWithDefault(values, WavAudioFormat::acidRootSet)
            .getIntValue() != 0)
      rootNote = ByteOrder::swapIfBigEndian(
          (uint16)getValueWithDefault(values, WavAudioFormat::acidRootNote)
              .getIntValue());

    numBeats = ByteOrder::swapIfBigEndian(
        (uint32)getValueWithDefault(values, WavAudioFormat::acidBeats)
            .getIntValue());
    meterDenominator = ByteOrder::swapIfBigEndian(
        (uint16)getValueWithDefault(values, WavAudioFormat::acidDenominator)
            .getIntValue());
    meterNumerator = ByteOrder::swapIfBigEndian(
        (uint16)getValueWithDefault(values, WavAudioFormat::acidNumerator)
            .getIntValue());
  ...
  MemoryBlock toMemoryBlock() const {
    return (flags != 0 || rootNote != 0 || numBeats != 0 ||
            meterDenominator != 0 || meterNumerator != 0)
               ? MemoryBlock(this, sizeof(*this))
               : MemoryBlock();
  }
  ...
  void addToMetadata(StringMap &values) const {
    ...
    if (flags & 0x02) // root note set
      values[WavAudioFormat::acidRootNote] =
          String(ByteOrder::swapIfBigEndian(rootNote));

    values[WavAudioFormat::acidBeats] =
        String(ByteOrder::swapIfBigEndian(numBeats));
    values[WavAudioFormat::acidDenominator] =
        String(ByteOrder::swapIfBigEndian(meterDenominator));
    values[WavAudioFormat::acidNumerator] =
        String(ByteOrder::swapIfBigEndian(meterNumerator));
  ...
  static uint32 getFlagIfPresent(const StringMap &values, const char *name,
                                 uint32 flag) {
    return getValueWithDefault(values, name).getIntValue() != 0
               ? ByteOrder::swapIfBigEndian(flag)
               : 0;
  }
...
static MemoryBlock createFrom(const StringMap &values) {
  ...
  if (aswgElement != nullptr) {
    ...
    auto aswgVersion =
        getValueWithDefault(values, WavAudioFormat::aswgVersion, "3.01");
    xml.addChildElement(
        createTextElement(WavAudioFormat::aswgVersion, aswgVersion));
      ...
      if (auto xml2 = xml->getChildByName("ebucore:coreMetadata")) {
        if (auto xml3 = xml2->getChildByName("ebucore:identifier")) {
          if (auto xml4 = xml3->getChildByName("dc:identifier")) {
            auto ISRCCode = xml4->getAllSubText().fromFirstOccurrenceOf(
                "ISRC:", false, true);
            ...
            if (ISRCCode.isNotEmpty()) {
              ...
              destValues[WavAudioFormat::riffInfoSource] = destValues
...
static MemoryBlock createFrom(const StringMap &values) {
  ...
  auto ISRC = getValueWithDefault(
      values, WavAudioFormat::internationalStandardRecordingCode,
  ...
  if (ISRC.isNotEmpty()) {
    ...
    xml << "<ebucore:ebuCoreMain xmlns:dc=\" "
           ...
           "formatLink=\"http://www.ebu.ch/metadata/cs/"
           "ebu_IdentifierTypeCodeCS.xml#3.7\">"
           "<dc:identifier>ISRC:"
        << ISRC
        << "</dc:identifier>"
           "</ebucore:identifier>"
    ...
    xml.writeRepeatedByte(
        0, xml.getDataSize()); // ensures even size, null termination and room
                               // for future growing
  }
...
struct DataSize64Chunk // chunk ID = 'ds64' if data size > 0xffffffff, 'JUNK'
                       // otherwise
{
...
public:
  WavAudioFormatReader(InputStream *in) : AudioFormatReader(in, wavFormatName) {
    ...
    if (input->readInt() == chunkName("WAVE")) {
      ...
      while ((uint64)input->getPosition() < end && !input->isExhausted()) {
        ...
        if (chunkType == chunkName("fmt ")) {
          ...
          } else if (format == 0xfffe) // WAVE_FORMAT_EXTENSIBLE
          {
            if (length < 40) // too short
            {
              ...
              channelLayout =
                  getChannelLayoutFromMask(channelMask, numChannels);
        ...
        } else if (chunkType == chunkName("inst") ||
                   chunkType == chunkName("INST")) // need to check which...
        {
          ...
          if (subChunkType == chunkName("info") ||
              subChunkType == chunkName("INFO")) {
    ...
    if (cueLabelIndex > 0)
      dict["NumCueLabels"] = String(cueLabelIndex);
    if (cueNoteIndex > 0)
      dict["NumCueNotes"] = String(cueNoteIndex);
    if (cueRegionIndex > 0)
      dict["NumCueRegions"] = String(cueRegionIndex);
    if (dict.size() > 0)
      dict["MetaDataSource"] = "WAV";
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    clearSamplesBeyondAvailableLength(
        destSamples, numDestChannels, startOffsetInDestBuffer,
    ...
    while (numSamples > 0) {
      ...
      if (bytesRead < numThisTime * bytesPerFrame) {
        jassert(bytesRead >= 0);
        zeromem(tempBuffer + bytesRead,
                (size_t)(numThisTime * bytesPerFrame - bytesRead));
      }

      copySampleData(bitsPerSample, usesFloatingPointData, destSamples,
                     startOffsetInDestBuffer, numDestChannels, tempBuffer,
  ...
  static void copySampleData(unsigned int numBitsPerSample,
                             const bool floatingPointData,
                             int *const *destSamples,
                             int startOffsetInDestBuffer, int numDestChannels,
                             const void *sourceData, int numberOfChannels,
                             int numSamples) noexcept {
    switch (numBitsPerSample) {
    case 8:
      ReadHelper<AudioData::Int32, AudioData::UInt8,
                 AudioData::LittleEndian>::read(destSamples,
                                                startOffsetInDestBuffer,
                                                numDestChannels, sourceData,
                                                numberOfChannels, numSamples);
      break;
    case 16:
      ReadHelper<AudioData::Int32, AudioData::Int16,
                 AudioData::LittleEndian>::read(destSamples,
                                                startOffsetInDestBuffer,
                                                numDestChannels, sourceData,
                                                numberOfChannels, numSamples);
      break;
    case 24:
      ReadHelper<AudioData::Int32, AudioData::Int24,
                 AudioData::LittleEndian>::read(destSamples,
                                                startOffsetInDestBuffer,
                                                numDestChannels, sourceData,
                                                numberOfChannels, numSamples);
      break;
    case 32:
      if (floatingPointData)
        ReadHelper<AudioData::Float32, AudioData::Float32,
                   AudioData::LittleEndian>::read(destSamples,
                                                  startOffsetInDestBuffer,
                                                  numDestChannels, sourceData,
                                                  numberOfChannels, numSamples);
      else
        ReadHelper<AudioData::Int32, AudioData::Int32,
                   AudioData::LittleEndian>::read(destSamples,
                                                  startOffsetInDestBuffer,
                                                  numDestChannels, sourceData,
                                                  numberOfChannels, numSamples);
  ...
  AudioChannelSet getChannelLayout() override {
    ...
    return WavFileHelpers::canonicalWavChannelSet(
        static_cast<int>(numChannels));
  }

  static AudioChannelSet getChannelLayoutFromMask(int dwChannelMask,
    ...
    for (auto bit = channelBits.findNextSetBit(0); bit >= 0;
         bit = channelBits.findNextSetBit(bit + 1))
      wavFileChannelLayout.addChannel(
          static_cast<AudioChannelSet::ChannelType>(bit + 1));
    ...
    if (wavFileChannelLayout.size() != static_cast<int>(totalNumChannels)) {
      ...
      if (totalNumChannels <= 2 && dwChannelMask == 0)
        wavFileChannelLayout = AudioChannelSet::canonicalChannelSet(
            static_cast<int>(totalNumChannels));
      else {
        auto discreteSpeaker =
            static_cast<int>(AudioChannelSet::discreteChannel0);

        while (wavFileChannelLayout.size() < static_cast<int>(totalNumChannels))
          wavFileChannelLayout.addChannel(
              static_cast<AudioChannelSet::ChannelType>(discreteSpeaker++));
      }
    }
...
public:
  ...
  ~WavAudioFormatWriter() override { writeHeader(); }

  //==============================================================================
  bool write(const int **data, int numSamples) override {
    jassert(numSamples >= 0);
    jassert(data != nullptr &&
            *data != nullptr); // the input must contain at least one channel!
    ...
    case 8:
      WriteHelper<AudioData::UInt8, AudioData::Int32,
                  AudioData::LittleEndian>::write(tempBlock.getData(),
                                                  (int)numChannels, data,
                                                  numSamples);
      break;
    case 16:
      WriteHelper<AudioData::Int16, AudioData::Int32,
                  AudioData::LittleEndian>::write(tempBlock.getData(),
                                                  (int)numChannels, data,
                                                  numSamples);
      break;
    case 24:
      WriteHelper<AudioData::Int24, AudioData::Int32,
                  AudioData::LittleEndian>::write(tempBlock.getData(),
                                                  (int)numChannels, data,
                                                  numSamples);
      break;
    case 32:
      WriteHelper<AudioData::Int32, AudioData::Int32,
                  AudioData::LittleEndian>::write(tempBlock.getData(),
                                                  (int)numChannels, data,
                                                  numSamples);
      break;
...
private:
  MemoryBlock tempBlock, bwavChunk, ixmlChunk, axmlChunk, smplChunk, instChunk,
      cueChunk, listChunk, listInfoChunk, acidChunk, trckChunk;
  ...
  void writeHeader() {
    ...
    if (headerPosition != output->getPosition() &&
        !output->setPosition(headerPosition)) {
      // if this fails, you've given it an output stream that can't seek! It
      // needs to be able to seek back to go back and write the header after the
    ...
    int64 riffChunkSize =
        (int64)(4 /* 'RIFF' */ + 8 + 40 /* WAVEFORMATEX */
                + 8 + audioDataSize + (audioDataSize & 1) +
                chunkSize(bwavChunk) + chunkSize(ixmlChunk) +
                chunkSize(axmlChunk) + chunkSize(smplChunk) +
                chunkSize(instChunk) + chunkSize(cueChunk) +
                chunkSize(listChunk) + chunkSize(listInfoChunk) +
                chunkSize(acidChunk) + chunkSize(trckChunk) +
                (8 + 28)); // (ds64 chunk)
    ...
    if (!isRF64) {
      ...
      /* NB: This junk chunk is added for padding, so that the header is a fixed
         size regardless of whether it's RF64 or not. That way, we can begin
         recording a file, and when it's finished, can go back and write either
         a RIFF or RF64 header, depending on whether more than 2^32 samples were
         written.

         The JUCE_WAV_DO_NOT_PAD_HEADER_SIZE macro allows you to disable this
         feature in case you need to create files for crappy WAV players with
         bugs that stop them skipping chunks which they don't recognise. But DO
         NOT USE THIS option unless you really have no choice, because it means
         that if you write more than 2^32 samples to the file, you'll corrupt
         it.
      */
...
#if JUCE_WAV_DO_NOT_PAD_HEADER_SIZE
      // If you disable padding, then you MUST NOT write more than 2^32 samples
      // to a file.
      jassertfalse;
#endif

      writeChunkHeader(chunkName("ds64"),
                       28); // chunk size for uncompressed data (no table)
    ...
    if (isWaveFmtEx) {
      ...
      output->writeShort(bitsPerSample < 32
                             ? (short)1 /*WAVE_FORMAT_PCM*/
    ...
    output->writeInt(
        (int)((double)bytesPerFrame * sampleRate)); // nAvgBytesPerSec
    ...
    if (isWaveFmtEx) {
      ...
      const ExtensibleWavSubFormat &subFormat =
    ...
    writeChunkHeader(chunkName("data"),
                     isRF64 ? -1 : (int)(lengthInSamples * bytesPerFrame));
  ...
  static int getChannelMaskFromChannelLayout(const AudioChannelSet &layout) {
    ...
    // Don't add an extended format chunk for mono and stereo. Basically, all
    // wav players interpret a wav file with only one or two channels to be mono
    // or stereo anyway.
    if (layout == AudioChannelSet::mono() ||
...
public:
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    if (map == nullptr ||
        !mappedSection.contains(
            Range<int64>(startSampleInFile, startSampleInFile + numSamples))) {
      jassertfalse; // you must make sure that the window contains all the
                    // samples you're going to attempt to read.
      return false;
    }

    WavAudioFormatReader::copySampleData(
        bitsPerSample, usesFloatingPointData, destSamples,
        startOffsetInDestBuffer, numDestChannels,
  ...
  void getSample(int64 sample, float *result) const noexcept override {
    ...
    switch (bitsPerSample) {
    case 8:
      ReadHelper<AudioData::Float32, AudioData::UInt8,
                 AudioData::LittleEndian>::read(dest, 0, 1, source, 1, num);
      break;
    case 16:
      ReadHelper<AudioData::Float32, AudioData::Int16,
                 AudioData::LittleEndian>::read(dest, 0, 1, source, 1, num);
      break;
    case 24:
      ReadHelper<AudioData::Float32, AudioData::Int24,
                 AudioData::LittleEndian>::read(dest, 0, 1, source, 1, num);
      break;
    case 32:
      if (usesFloatingPointData)
        ...
        ReadHelper<AudioData::Float32, AudioData::Int32,
                   AudioData::LittleEndian>::read(dest, 0, 1, source, 1, num);
      break;
    default:
      jassertfalse;
  ...
  void readMaxLevels(int64 startSampleInFile, int64 numSamples,
    ...
    if (map == nullptr || numSamples <= 0 ||
        !mappedSection.contains(
            Range<int64>(startSampleInFile, startSampleInFile + numSamples))) {
      jassert(numSamples <=
              0); // you must make sure that the window contains all the samples
    ...
    switch (bitsPerSample) {
    case 8:
      scanMinAndMax<AudioData::UInt8>(startSampleInFile, numSamples, results,
                                      numChannelsToRead);
      break;
    case 16:
      scanMinAndMax<AudioData::Int16>(startSampleInFile, numSamples, results,
                                      numChannelsToRead);
      break;
    case 24:
      scanMinAndMax<AudioData::Int24>(startSampleInFile, numSamples, results,
                                      numChannelsToRead);
      break;
    case 32:
      if (usesFloatingPointData)
        scanMinAndMax<AudioData::Float32>(startSampleInFile, numSamples,
                                          results, numChannelsToRead);
      else
        scanMinAndMax<AudioData::Int32>(startSampleInFile, numSamples, results,
                                        numChannelsToRead);
      break;
    default:
      jassertfalse;
...
private:
  ...
  void scanMinAndMax(int64 startSampleInFile, int64 numSamples,
                     Range<float> *results,
                     int numChannelsToRead) const noexcept {
    for (int i = 0; i < numChannelsToRead; ++i)
      results[i] =
          scanMinAndMaxInterleaved<SampleType, AudioData::LittleEndian>(
              i, startSampleInFile, numSamples);
  }
...
Array<int> WavAudioFormat::getPossibleBitDepths() { return {8, 16, 24, 32}; }

bool WavAudioFormat::canDoStereo() { return true; }
bool WavAudioFormat::canDoMono() { return true; }

bool WavAudioFormat::isChannelLayoutSupported(
  ...
  for (auto channel : channelTypes)
    if (channel < AudioChannelSet::left ||
        channel > AudioChannelSet::topRearRight)
      return false;
...
AudioFormatReader *
WavAudioFormat::createReaderFor(InputStream *sourceStream,
                                bool deleteStreamIfOpeningFails) {
  std::unique_ptr<WavAudioFormatReader> r(
      new WavAudioFormatReader(sourceStream));

#if JUCE_USE_OGGVORBIS
  if (r->isSubformatOggVorbis) {
    r->input = nullptr;
    return OggVorbisAudioFormat().createReaderFor(sourceStream,
...
WavAudioFormat::createMemoryMappedReader(const File &file) {
  return createMemoryMappedReader(file.createInputStream().release());
}

MemoryMappedAudioFormatReader *
WavAudioFormat::createMemoryMappedReader(FileInputStream *fin) {
  if (fin != nullptr) {
...
AudioFormatWriter *
WavAudioFormat::createWriterFor(OutputStream *out, double sampleRate,
                                unsigned int numChannels, int bitsPerSample,
                                const StringPairArray &metadataValues,
                                int qualityOptionIndex) {
  return createWriterFor(
      out, sampleRate,
      WavFileHelpers::canonicalWavChannelSet(static_cast<int>(numChannels)),
      bitsPerSample, metadataValues, qualityOptionIndex);
}

AudioFormatWriter *WavAudioFormat::createWriterFor(
    OutputStream *out, double sampleRate, const AudioChannelSet &channelLayout,
    int bitsPerSample, const StringPairArray &metadataValues,
    int /*qualityOptionIndex*/) {
  if (out != nullptr && getPossibleBitDepths().contains(bitsPerSample) &&
      isChannelLayoutSupported(channelLayout))
...
namespace WavFileHelpers {
static bool slowCopyWavFileWithNewMetadata(const File &file,
  ...
  if (reader != nullptr) {
    ...
    if (outStream != nullptr) {
      std::unique_ptr<AudioFormatWriter> writer(wav.createWriterFor(
          outStream.get(), reader->sampleRate, reader->numChannels,
...
bool WavAudioFormat::replaceMetadataInFile(const File &wavFile,
  ...
  std::unique_ptr<WavAudioFormatReader> reader(
      static_cast<WavAudioFormatReader *>(
          createReaderFor(wavFile.createInputStream().release(), true)));
...
struct WaveAudioFormatTests final : public UnitTest {
  ...
  void runTest() override {
    ...
    auto metadataValues = toMap(WavAudioFormat::createBWAVMetadata(
        "description", "originator", "originatorRef", Time::getCurrentTime(),
        numTestAudioBufferSamples, "codingHistory"));

    for (int i = numElementsInArray(WavFileHelpers::ListInfoChunk::types);
         --i >= 0;)
      metadataValues[WavFileHelpers::ListInfoChunk::types[i]] =
...

    {
      ...
      const auto newMetadata =
          getMetadataAfterReading(format, writeToBlock(format, metadataArray));
      expect(newMetadata == metadataArray,
             "Somehow, the metadata is different!");
    }

    {
      beginTest("Files containing a riff info source and an empty ISRC "
                "associate the source with the riffInfoSource key");
    ...
    {
      beginTest("Files containing a riff info source and no ISRC associate the "
                "source with both keys "
    ...
    {
      beginTest("Files containing an ISRC associate the value with the "
                "internationalStandardRecordingCode key "
                "and the riffInfoSource key for backwards compatibility");
      StringPairArray meta;
      meta.addMap({{WavAudioFormat::internationalStandardRecordingCode,
                    "AABBBCCDDDDD"}});
      const auto mb = writeToBlock(format, meta);
    ...
    {
      beginTest("Files containing an ISRC and a riff info source associate the "
                "values with the appropriate keys");
      StringPairArray meta;
...

    {
      ...
      {
        auto writer = rawToUniquePtr(WavAudioFormat().createWriterFor(
            new MemoryOutputStream(block, false), 48000, 1, 32, meta, 0));
      ...
      {
        auto reader = rawToUniquePtr(WavAudioFormat().createReaderFor(
            new MemoryInputStream(block, false), true));
...
private:
  MemoryBlock writeToBlock(WavAudioFormat &format, StringPairArray meta) {
    ...
    {
      // The destructor of the writer will modify the block, so make sure that
      // we've destroyed the writer before returning the block!
      auto writer = rawToUniquePtr(
          format.createWriterFor(new MemoryOutputStream(mb, false), 44100.0,
                                 numTestAudioBufferChannels, 16, meta, 0));
      expect(writer != nullptr);
      AudioBuffer<float> buffer(numTestAudioBufferChannels,
                                numTestAudioBufferSamples);
      expect(writer->writeFromAudioSampleBuffer(buffer, 0,
                                                numTestAudioBufferSamples));
    }
  ...
  StringPairArray getMetadataAfterReading(WavAudioFormat &format,
                                          const MemoryBlock &mb) {
    auto reader = rawToUniquePtr(
        format.createReaderFor(new MemoryInputStream(mb, false), true));
    expect(reader != nullptr);
    return reader->metadataValues;
  }

  template <typename Fn>
  void checkPatterns(const MemoryBlock &mb,
  ...
  void checkPatternsPresent(const MemoryBlock &mb,
                            const std::vector<std::string> &patterns) {
    checkPatterns(mb, patterns, std::not_equal_to<>{});
  }

  void checkPatternsNotPresent(const MemoryBlock &mb,
                               const std::vector<std::string> &patterns) {
    checkPatterns(mb, patterns, std::equal_to<>{});
  }

  enum { numTestAudioBufferChannels = 2, numTestAudioBufferSamples = 256 };
