...
StringArray getStringInfo(AudioFilePropertyID property, UInt32 size,
                          void *data) {
  CFObjectHolder<CFArrayRef> extensions;
  UInt32 sizeOfArray = sizeof(extensions.object);

  const auto err = AudioFileGetGlobalInfo(property, size, data, &sizeOfArray,
                                          &extensions.object);
  ...
  for (CFIndex i = 0; i < numValues; ++i)
    extensionsArray.add(
        "." + String::fromCFString(
                  (CFStringRef)CFArrayGetValueAtIndex(extensions.object, i)));

  return extensionsArray;
}

StringArray findFileExtensionsForCoreAudioCodec(AudioFileTypeID type) {
  return getStringInfo(kAudioFileGlobalInfo_ExtensionsForType,
                       sizeof(AudioFileTypeID), &type);
}
...
struct CoreAudioFormatMetatdata {
  ...
  static StringPairArray parseUserDefinedChunk(InputStream &input, int64 size) {
    ...
    if (memcmp(
            uuid,
            "\x29\x81\x92\x73\xB5\xBF\x4A\xEF\xB7\x8D\x62\xD1\xEF\x90\xBB\x2C",
            16) == 0) {
      auto numEntries = (uint32)input.readIntBigEndian();

      for (uint32 i = 0;
           i < numEntries && input.getPosition() < originalPosition + size;
           ++i) {
  ...
  static StringPairArray parseMidiChunk(InputStream &input, int64 size) {
    ...
    if (midiFile.readFrom(midiInputStream)) {
      midiMetadata.set(CoreAudioFormat::midiDataBase64,
                       midiBlock.toBase64Encoding());
  ...
  static void findTempoEvents(MidiFile &midiFile,
                              StringPairArray &midiMetadata) {
  ...
  static double
  getTempoFromTempoMetaEvent(MidiMessageSequence::MidiEventHolder *holder) {
    if (holder != nullptr) {
      auto &midiMessage = holder->message;

      if (midiMessage.isTempoMetaEvent()) {
        auto tempoSecondsPerQuarterNote =
  ...
  static void findTimeSigEvents(MidiFile &midiFile,
                                StringPairArray &midiMetadata) {
    ...
    for (int i = 0; i < numTimeSigEvents; ++i) {
      ...
      timeSigEvents.getEventPointer(i)->message.getTimeSignatureInfo(
  ...
  static void findKeySigEvents(MidiFile &midiFile,
                               StringPairArray &midiMetadata) {
    ...
    for (int i = 0; i < numKeySigEvents; ++i) {
      ...
      auto key =
          jlimit(0, 14, message.getKeySignatureNumberOfSharpsOrFlats() + 7);
      bool isMajor = message.isKeySignatureMajorKey();

      static const char *majorKeys[] = {"Cb", "Gb", "Db", "Ab", "Eb",
                                        "Bb", "F",  "C",  "G",  "D",
                                        "A",  "E",  "B",  "F#", "C#"};
    ...
    if (isCafFile) {
      while (!input.isExhausted()) {
        ...
        if (chunkHeader.chunkType == chunkName("desc")) {
          ...
          metadataValues.addArray(
              parseUserDefinedChunk(input, chunkHeader.chunkSize));
...
public:
  ...
  CoreAudioReader(InputStream *inp, StreamKind streamKind)
    ...
    if (status == noErr) {
      ...
      if (status == noErr) {
        ...
        status = AudioFileGetPropertyInfo(audioFileID,
                                          kAudioFilePropertyChannelLayout,
                                          &sizeOfLayout, &isWritable);

        if (status == noErr &&
            sizeOfLayout >= (sizeof(AudioChannelLayout) -
                             sizeof(AudioChannelDescription))) {
          caLayout.malloc(1, static_cast<size_t>(sizeOfLayout));

          status =
              AudioFileGetProperty(audioFileID, kAudioFilePropertyChannelLayout,
        ...
        destinationAudioFormat.mFormatFlags =
        ...
        destinationAudioFormat.mBytesPerPacket =
            destinationAudioFormat.mFramesPerPacket *
            destinationAudioFormat.mBytesPerFrame;

        status = ExtAudioFileSetProperty(
            audioFileRef, kExtAudioFileProperty_ClientDataFormat,
            sizeof(AudioStreamBasicDescription), &destinationAudioFormat);
        if (status == noErr) {
          bufferList.malloc(1, sizeof(AudioBufferList) +
          ...
          if (hasLayout && caLayout != nullptr) {
            auto caOrder =
                CoreAudioLayouts::getCoreAudioLayoutChannels(*caLayout);

            for (int i = 0; i < static_cast<int>(numChannels); ++i) {
              auto idx =
                  channelSet.getChannelIndexForType(caOrder.getReference(i));
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    while (numSamples > 0) {
      ...
      auto status =
          ExtAudioFileRead(audioFileRef, &numFramesToRead, bufferList);
      ...
      for (int i = numDestChannels; --i >= 0;) {
        ...
        if (dest != nullptr) {
          if (i < (int)numChannels)
            memcpy(dest + startOffsetInDestBuffer,
                   bufferList->mBuffers[i].mData, numBytes);
          else
...
private:
  ...
  static SInt64 getSizeCallback(void *inClientData) {
    return static_cast<CoreAudioReader *>(inClientData)
        ->input->getTotalLength();
  }

  static OSStatus readCallback(void *inClientData, SInt64 inPosition,
...
CoreAudioFormat::CoreAudioFormat(StreamKind kind)
    : AudioFormat(coreAudioFormatName,
...
//==============================================================================
AudioFormatReader *
CoreAudioFormat::createReaderFor(InputStream *sourceStream,
                                 bool deleteStreamIfOpeningFails) {
  std::unique_ptr<CoreAudioReader> r(
      new CoreAudioReader(sourceStream, streamKind));
...
#define DEFINE_CHANNEL_LAYOUT_DFL_ENTRY(x)                                     \
  CoreAudioChannelLayoutTag { x, #x, AudioChannelSet() }
#define DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(x, y)                                  \
  CoreAudioChannelLayoutTag { x, #x, y }

class CoreAudioLayoutsUnitTest final : public UnitTest {
public:
  CoreAudioLayoutsUnitTest()
      : UnitTest("Core Audio Layout <-> JUCE channel layout conversion",
                 UnitTestCategories::audio) {}
  ...
  void runTest() override {
    ...
    {
      // Check that all known tags defined in CoreAudio SDK version 10.12.4 are
      // known to JUCE Include all defined tags even if there are duplicates as
      // Apple will sometimes change definitions
      beginTest("All CA tags handled");

      for (auto tagEntry : knownTags) {
        auto labels = CoreAudioLayouts::fromCoreAudio(tagEntry.tag);

        expect(!labels.isDiscreteLayout(),
               "Tag \"" + String(tagEntry.name) + "\" is not handled by JUCE");
      }
    }

    {
      ...
      for (auto tagEntry : knownTags) {
        auto labels =
            CoreAudioLayouts::getSpeakerLayoutForCoreAudioTag(tagEntry.tag);

        expect(labels.size() == (tagEntry.tag & 0xffff),
...

    {
      ...
      for (auto tagEntry : knownTags)
        expect(AudioChannelSet::channelSetWithChannels(
                   CoreAudioLayouts::getSpeakerLayoutForCoreAudioTag(
                       tagEntry.tag)) ==
...

    {
      ...
      for (auto tagEntry : knownTags) {
        ...
        expect(CoreAudioLayouts::fromCoreAudio(tagEntry.tag) ==
                   tagEntry.equivalentChannelSet,
               "Documentation for tag \"" + String(tagEntry.name) +
                   "\" is incorrect");
      }
    }

    {
      ...
      for (auto tagEntry : knownTags) {
        ...
        expect(CoreAudioLayouts::toCoreAudio(tagEntry.equivalentChannelSet) ==
                   tagEntry.tag,
               "Incorrect reverse conversion for tag \"" +
...
private:
  struct CoreAudioChannelLayoutTag {
    ...
    const char *name;
    AudioChannelSet equivalentChannelSet; /* referred to this in the
                                             AudioChannelSet documentation */
  };

  //==============================================================================
  const Array<CoreAudioChannelLayoutTag> &getAllKnownLayoutTags() const {
    static CoreAudioChannelLayoutTag tags[] = {
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_Mono,
                                        AudioChannelSet::mono()),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_Stereo,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_Quadraphonic,
                                        AudioChannelSet::quadraphonic()),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_Pentagonal,
                                        AudioChannelSet::pentagonal()),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_Hexagonal,
                                        AudioChannelSet::hexagonal()),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_Octagonal,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_3_0_A,
                                        AudioChannelSet::createLCR()),
        DEFINE_CHANNEL_LAYOUT_DFL_ENTRY(kAudioChannelLayoutTag_MPEG_3_0_B),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_4_0_A,
                                        AudioChannelSet::createLCRS()),
        DEFINE_CHANNEL_LAYOUT_DFL_ENTRY(kAudioChannelLayoutTag_MPEG_4_0_B),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_5_0_A,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_5_1_A,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_6_1_A,
                                        AudioChannelSet::create6point1()),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_7_1_A,
                                        AudioChannelSet::create7point1SDDS()),
        DEFINE_CHANNEL_LAYOUT_DFL_ENTRY(kAudioChannelLayoutTag_MPEG_7_1_B),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_MPEG_7_1_C,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_ITU_2_1,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_AudioUnit_6_0,
                                        AudioChannelSet::create6point0()),
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_AudioUnit_7_0,
        ...
        // DEFINE_CHANNEL_LAYOUT_DFL_ENTRY
        // (kAudioChannelLayoutTag_TMH_10_2_full), no indication on how to
        // handle this tag
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_DTS_6_0_A,
        ...
        DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_DTS_6_1_A,
