...
inline int chunkName(const char *name) noexcept {
...
struct InstChunk {
  ...
  void copyTo(std::map<String, String> &values) const {
    ...
    values.emplace("Gain",
                   String((int16)ByteOrder::swapIfLittleEndian((uint16)gain)));

    values.emplace("NumSampleLoops",
                   String(2)); // always 2 with AIFF, WAV can have more
    values.emplace("Loop0Type",
                   String(ByteOrder::swapIfLittleEndian(sustainLoop.type)));
    values.emplace("Loop0StartIdentifier", String(ByteOrder::swapIfLittleEndian(
                                               sustainLoop.startIdentifier)));
    values.emplace("Loop0EndIdentifier", String(ByteOrder::swapIfLittleEndian(
                                             sustainLoop.endIdentifier)));
    values.emplace("Loop1Type",
                   String(ByteOrder::swapIfLittleEndian(releaseLoop.type)));
    values.emplace("Loop1StartIdentifier", String(ByteOrder::swapIfLittleEndian(
                                               releaseLoop.startIdentifier)));
    values.emplace("Loop1EndIdentifier", String(ByteOrder::swapIfLittleEndian(
                                             releaseLoop.endIdentifier)));
  }

  static uint16 getValue16(const StringPairArray &values, const char *name,
                           const char *def) {
    return ByteOrder::swapIfLittleEndian(
        (uint16)values.getValue(name, def).getIntValue());
  }

  static int8 getValue8(const StringPairArray &values, const char *name,
                        const char *def) {
  ...
  static void create(MemoryBlock &block, const StringPairArray &values) {
    if (values.getAllKeys().contains("MidiUnityNote", true)) {
      ...
      inst.sustainLoop.startIdentifier =
          getValue16(values, "Loop0StartIdentifier", "0");
      inst.sustainLoop.endIdentifier =
          getValue16(values, "Loop0EndIdentifier", "0");
      inst.releaseLoop.type = getValue16(values, "Loop1Type", "0");
      inst.releaseLoop.startIdentifier =
          getValue16(values, "Loop1StartIdentifier", "0");
      inst.releaseLoop.endIdentifier =
          getValue16(values, "Loop1EndIdentifier", "0");
    }
  }
...
struct BASCChunk {
  ...
  void setBoolFlag(std::map<String, String> &values, const char *name,
...
static bool isValidTag(const char *d) noexcept {
  return CharacterFunctions::isLetterOrDigit(d[0]) &&
         CharacterFunctions::isUpperCase(static_cast<juce_wchar>(d[0])) &&
         CharacterFunctions::isLetterOrDigit(d[1]) &&
         CharacterFunctions::isLowerCase(static_cast<juce_wchar>(d[1])) &&
         CharacterFunctions::isLetterOrDigit(d[2]) &&
         CharacterFunctions::isLowerCase(static_cast<juce_wchar>(d[2]));
}

static bool isAppleGenre(const String &tag) noexcept {
  static const char *appleGenres[] = {
      "Rock/Blues",   "Electronic/Dance",  "Jazz",       "Urban",
      "World/Ethnic", "Cinematic/New Age", "Orchestral", "Country/Folk",
      "Experimental", "Other Genre"};
...
static String read(InputStream &input, const uint32 length) {
  ...
  while (data < dataEnd) {
    ...
    if (data < dataEnd && data[0] == 0) {
      if (data + 52 < dataEnd && isValidTag(data + 50))
        data += 50;
      else if (data + 120 < dataEnd && isValidTag(data + 118))
        data += 118;
      else if (data + 170 < dataEnd && isValidTag(data + 168))
        data += 168;
    }
  }
...
static void create(MemoryBlock &block, const StringPairArray &values) {
  ...
  if (numCues > 0) {
    ...
    auto idOffset = metaDataContainsZeroIdentifiers(values)
                        ? 1
                        : 0; // can't have zero IDs in AIFF
    ...
    for (int i = 0; i < numCues; ++i) {
      ...
      auto identifier =
          idOffset +
          values.getValue(prefixCue + "Identifier", "1").getIntValue();
      ...
      for (int labelIndex = 0; labelIndex < numCueLabels; ++labelIndex) {
        ...
        auto labelIdentifier =
            idOffset +
            values.getValue(prefixLabel + "Identifier", "1").getIntValue();
      ...
      auto labelLength =
          jmin((size_t)254,
               label.getNumBytesAsUTF8()); // seems to need null terminator even
                                           // though it's a pstring
...
static void create(MemoryBlock &block, const StringPairArray &values) {
  ...
  if (numNotes > 0) {
    ...
    for (int i = 0; i < numNotes; ++i) {
      ...
      out.writeIntBigEndian(
          values.getValue(prefix + "TimeStamp", "0").getIntValue());
      out.writeShortBigEndian(
          (short)values.getValue(prefix + "Identifier", "0").getIntValue());
...
public:
      ...
      if (nextType == chunkName("AIFF") || nextType == chunkName("AIFC")) {
        ...
        while (input->getPosition() < end) {
          ...
          if (type == chunkName("FVER")) {
            ...
            if ((byte0 & 0x80) != 0 || byte0 <= 0x3F || byte0 > 0x40 ||
            ...
            if (length <= 18) {
              // some types don't have a chunk large enough to include a
              // compression type, so assume it's just big-endian pcm
              littleEndian = false;
            } else {
              auto compType = input->readInt();

              if (compType == chunkName("NONE") ||
                  compType == chunkName("twos")) {
                littleEndian = false;
            ...
            lengthInSamples = (bytesPerFrame > 0)
                                  ? jmin(lengthInSamples,
                                         ((int64)length) / (int64)bytesPerFrame)
                                  : 0;
            ...
            for (uint16 i = 0; i < numCues; ++i) {
              ...
              auto prefixCue = "Cue" + String(i);
              metadataValuesMap.emplace(prefixCue + "Identifier",
                                        String(identifier));
              metadataValuesMap.emplace(prefixCue + "Offset", String(offset));

              auto prefixLabel = "CueLabel" + String(i);
              metadataValuesMap.emplace(prefixLabel + "Identifier",
                                        String(identifier));
              metadataValuesMap.emplace(prefixLabel + "Text",
                                        textBlock.toString());
            }
            ...
            for (uint16 i = 0; i < numNotes; ++i) {
              ...
              auto identifier =
                  (uint16)
                      input->readShortBigEndian(); // may be zero in this case
              auto stringLength = (uint16)input->readShortBigEndian();

              MemoryBlock textBlock;
              input->readIntoMemoryBlock(textBlock,
                                         stringLength + (stringLength & 1));

              auto prefix = "CueNote" + String(i);
              metadataValuesMap.emplace(prefix + "TimeStamp",
                                        String(timestamp));
              metadataValuesMap.emplace(prefix + "Identifier",
                                        String(identifier));
          ...
          input->setPosition(
              chunkEnd +
              (chunkEnd &
               1)); // (chunks should be aligned to an even byte address)
        }
      }
    }
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    while (numSamples > 0) {
      ...
      const int bytesRead =
          input->read(tempBuffer, numThisTime * bytesPerFrame);
      ...
      if (littleEndian)
        copySampleData<AudioData::LittleEndian>(
        ...
        copySampleData<AudioData::BigEndian>(
            bitsPerSample, usesFloatingPointData, destSamples,
            startOffsetInDestBuffer, numDestChannels, tempBuffer,
            (int)numChannels, numThisTime);
  ...
  static void copySampleData(unsigned int numBitsPerSample,
                             bool floatingPointData, int *const *destSamples,
                             int startOffsetInDestBuffer, int numDestChannels,
                             const void *sourceData, int numberOfChannels,
                             int numSamples) noexcept {
    switch (numBitsPerSample) {
    case 8:
      ReadHelper<AudioData::Int32, AudioData::Int8, Endianness>::read(
          destSamples, startOffsetInDestBuffer, numDestChannels, sourceData,
          numberOfChannels, numSamples);
      break;
    case 16:
      ReadHelper<AudioData::Int32, AudioData::Int16, Endianness>::read(
          destSamples, startOffsetInDestBuffer, numDestChannels, sourceData,
          numberOfChannels, numSamples);
      break;
    case 24:
      ReadHelper<AudioData::Int32, AudioData::Int24, Endianness>::read(
          destSamples, startOffsetInDestBuffer, numDestChannels, sourceData,
          numberOfChannels, numSamples);
      break;
    case 32:
      if (floatingPointData)
        ReadHelper<AudioData::Float32, AudioData::Float32, Endianness>::read(
            destSamples, startOffsetInDestBuffer, numDestChannels, sourceData,
            numberOfChannels, numSamples);
      else
        ReadHelper<AudioData::Int32, AudioData::Int32, Endianness>::read(
            destSamples, startOffsetInDestBuffer, numDestChannels, sourceData,
            numberOfChannels, numSamples);
      break;
    default:
...
public:
  AiffAudioFormatWriter(OutputStream *out, double rate, unsigned int numChans,
                        unsigned int bits,
  ...
  bool write(const int **data, int numSamples) override {
    ...
    case 8:
      WriteHelper<AudioData::Int8, AudioData::Int32,
                  AudioData::BigEndian>::write(tempBlock.getData(),
...
private:
  ...
  void writeHeader() {
    ...
    [[maybe_unused]] const bool couldSeekOk =
    ...
    auto headerLen =
        (int)(54 + (markChunk.isEmpty() ? 0 : markChunk.getSize() + 8) +
              (comtChunk.isEmpty() ? 0 : comtChunk.getSize() + 8) +
              (instChunk.isEmpty() ? 0 : instChunk.getSize() + 8));
    auto audioBytes =
        (int)(lengthInSamples * ((bitsPerSample * numChannels) / 8));
...
public:
  ...
  void getSample(int64 sample, float *result) const noexcept override {
    ...
    if (littleEndian) {
      ...
      case 8:
        ReadHelper<AudioData::Float32, AudioData::UInt8,
                   AudioData::BigEndian>::read(dest, 0, 1, source, 1, num);
        break;
      case 16:
        ReadHelper<AudioData::Float32, AudioData::Int16,
                   AudioData::BigEndian>::read(dest, 0, 1, source, 1, num);
...
private:
  ...
  void scanMinAndMax(int64 startSampleInFile, int64 numSamples,
    ...
    for (int i = 0; i < numChannelsToRead; ++i)
      results[i] =
          scanMinAndMaxForChannel<SampleType>(i, startSampleInFile, numSamples);
  }

  template <typename SampleType>
  Range<float> scanMinAndMaxForChannel(int channel, int64 startSampleInFile,
                                       int64 numSamples) const noexcept {
    return littleEndian
               ? scanMinAndMaxInterleaved<SampleType, AudioData::LittleEndian>(
                     channel, startSampleInFile, numSamples)
               : scanMinAndMaxInterleaved<SampleType, AudioData::BigEndian>(
                     channel, startSampleInFile, numSamples);
  }
...
AiffAudioFormat::AiffAudioFormat()
    : AudioFormat(aiffFormatName, ".aiff .aif") {}
AiffAudioFormat::~AiffAudioFormat() {}
...
Array<int> AiffAudioFormat::getPossibleBitDepths() { return {8, 16, 24}; }
...
AudioFormatReader *
AiffAudioFormat::createReaderFor(InputStream *sourceStream,
                                 bool deleteStreamIfOpeningFails) {
  std::unique_ptr<AiffAudioFormatReader> w(
...
AiffAudioFormat::createMemoryMappedReader(const File &file) {
  return createMemoryMappedReader(file.createInputStream().release());
}

MemoryMappedAudioFormatReader *
AiffAudioFormat::createMemoryMappedReader(FileInputStream *fin) {
  if (fin != nullptr) {
