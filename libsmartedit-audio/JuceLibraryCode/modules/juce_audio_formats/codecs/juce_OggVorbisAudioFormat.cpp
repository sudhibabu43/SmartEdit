...
JUCE_BEGIN_IGNORE_WARNINGS_MSVC(
    4267 4127 4244 4996 4100 4701 4702 4013 4133 4206 4305 4189 4706 4995 4365 4456 4457 4459 6297 6011 6001 6308 6255 6386 6385 6246 6387 6263 6262 28182)

JUCE_BEGIN_IGNORE_WARNINGS_GCC_LIKE(
    "-Wcast-align", "-Wconversion", "-Wdeprecated-declarations",
    "-Wdeprecated-register", "-Wfloat-conversion", "-Wfloat-equal",
    "-Wmaybe-uninitialized", "-Wmisleading-indentation", "-Wmissing-prototypes",
    "-Wredundant-decls", "-Wshadow", "-Wsign-conversion", "-Wswitch-default",
    "-Wswitch-enum", "-Wzero-as-null-pointer-constant")
...
public:
  OggReader(InputStream *inp) : AudioFormatReader(inp, oggFormatName) {
    ...
    if (err == 0) {
      ...
      addMetadataItem(comment, "TRACKNUMBER",
  ...
  ~OggReader() override { ov_clear(&ovFile); }

  void addMetadataItem(OggVorbisNamespace::vorbis_comment *comment,
                       const char *name, const char *metadataName) {
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    const auto readFromReservoir = [this, &destSamples, &numDestChannels,
    ...
    const auto fillReservoir = [this](int64 requestedStart) {
      ...
      bufferedRange =
          Range<int64>{newStart, newStart + reservoir.getNumSamples()};
      ...
      while (numToRead > 0) {
        ...
        for (int i = jmin((int)numChannels, reservoir.getNumChannels());
             --i >= 0;)
          memcpy(reservoir.getWritePointer(i, offset), dataIn[i],
                 (size_t)samps * sizeof(float));
  ...
  //==============================================================================
  static size_t oggReadCallback(void *ptr, size_t size, size_t nmemb,
                                void *datasource) {
    return (size_t)(static_cast<InputStream *>(datasource)
                        ->read(ptr, (int)(size * nmemb))) /
           size;
  }

  static int oggSeekCallback(void *datasource,
  ...
  static int oggCloseCallback(void *) { return 0; }
...
public:
  OggWriter(OutputStream *out, double rate, unsigned int numChans,
            unsigned int bitsPerSamp, int qualityIndex,
            const StringPairArray &metadata)
      : AudioFormatWriter(out, oggFormatName, rate, numChans, bitsPerSamp) {
    vorbis_info_init(&vi);

    if (vorbis_encode_init_vbr(
            &vi, (int)numChans, (int)rate,
      ...
      addMetadata(metadata, OggVorbisAudioFormat::id3trackNumber,
                  "TRACKNUMBER");
...
private:
  ...
  void addMetadata(const StringPairArray &metadata, const char *name,
                   const char *vorbisName) {
    auto s = metadata[name];

    if (s.isNotEmpty())
      vorbis_comment_add_tag(&vc, vorbisName,
                             const_cast<char *>(s.toRawUTF8()));
  }
...
Array<int> OggVorbisAudioFormat::getPossibleBitDepths() { return {32}; }
...
AudioFormatReader *
OggVorbisAudioFormat::createReaderFor(InputStream *in,
...
AudioFormatWriter *OggVorbisAudioFormat::createWriterFor(
    OutputStream *out, double sampleRate, unsigned int numChannels,
    int bitsPerSample, const StringPairArray &metadataValues,
    int qualityOptionIndex) {
  if (out == nullptr)
    return nullptr;

  std::unique_ptr<OggWriter> w(
      new OggWriter(out, sampleRate, numChannels, (unsigned int)bitsPerSample,
...
StringArray OggVorbisAudioFormat::getQualityOptions() {
  return {"64 kbps",  "80 kbps",  "96 kbps",  "112 kbps",
          "128 kbps", "160 kbps", "192 kbps", "224 kbps",
          "256 kbps", "320 kbps", "500 kbps"};
}

int OggVorbisAudioFormat::estimateOggFileQuality(const File &source) {
  if (auto in = source.createInputStream()) {
    if (auto r = std::unique_ptr<AudioFormatReader>(
            createReaderFor(in.release(), true))) {
      auto lengthSecs = (double)r->lengthInSamples / r->sampleRate;
      auto approxBitsPerSecond =
          (int)((double)source.getSize() * 8 / lengthSecs);
