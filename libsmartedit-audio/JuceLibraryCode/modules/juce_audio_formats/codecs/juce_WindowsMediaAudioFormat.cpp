...
public:
  JuceIStream(InputStream &in) noexcept : ComBaseClassHelper(0), source(in) {}

  JUCE_COMRESULT Commit(DWORD) override { return S_OK; }
  JUCE_COMRESULT Write(const void *, ULONG, ULONG *) override {
    return E_NOTIMPL;
  }
  ...
  JUCE_COMRESULT LockRegion(ULARGE_INTEGER, ULARGE_INTEGER, DWORD) override {
    return E_NOTIMPL;
  }
  JUCE_COMRESULT UnlockRegion(ULARGE_INTEGER, ULARGE_INTEGER, DWORD) override {
    return E_NOTIMPL;
  }
  ...
  JUCE_COMRESULT Seek(LARGE_INTEGER position, DWORD origin,
                      ULARGE_INTEGER *resultPosition) override {
  ...
  JUCE_COMRESULT CopyTo(IStream *destStream, ULARGE_INTEGER numBytesToDo,
    ...
    if (bytesRead != nullptr)
      bytesRead->QuadPart = totalCopied;
    if (bytesWritten != nullptr)
      bytesWritten->QuadPart = totalCopied;
...
static const char *const extensions[] = {".mp3", ".wmv", ".asf",
                                         ".wm",  ".wma", nullptr};
...
public:
  WMAudioReader(InputStream *const input_)
    ...
    JUCE_LOAD_WINAPI_FUNCTION(wmvCoreLib, WMCreateSyncReader,
                              wmCreateSyncReader, HRESULT,
                              (IUnknown *, DWORD, IWMSyncReader **))

    if (wmCreateSyncReader != nullptr) {
      checkCoInitialiseCalled();

      HRESULT hr = wmCreateSyncReader(nullptr, WMT_RIGHT_PLAYBACK,
                                      wmSyncReader.resetAndGetPointerAddress());
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    while (numSamples > 0) {
      if (!bufferedRange.contains(startSampleInFile)) {
        ...
        if (hasJumped)
          wmSyncReader->SetRange(
              (QWORD)(startSampleInFile * 10000000 / (int64)sampleRate), 0);
        ...
        HRESULT hr = wmSyncReader->GetNextSample(
            1, sampleBuffer.resetAndGetPointerAddress(), &sampleTime, &duration,
            &flags, &outputNum, &streamNum);
        ...
        if (sampleBuffer != nullptr) {
          ...
          if (hasJumped)
            bufferedRange.setStart(
                (int64)((sampleTime * (QWORD)sampleRate) / 10000000));
          else
            bufferedRange.setStart(
                bufferedRange.getEnd()); // (because the positions returned
                                         // often aren't contiguous)
      ...
      auto *rawData = static_cast<const int16 *>(
          addBytesToPointer(buffer.getData(), offsetInBuffer * stride));
      auto numToDo =
          jmin(numSamples, (int)(bufferedRange.getLength() - offsetInBuffer));
  ...
  void scanFileForDetails() {
    if (auto wmHeaderInfo = wmSyncReader.getInterface<IWMHeaderInfo>()) {
      ...
      wmHeaderInfo->GetAttributeByName(&streamNum, L"Duration", &wmAttrDataType,
                                       (BYTE *)&lengthInNanoseconds,
                                       &lengthOfLength);

      if (auto wmProfile = wmSyncReader.getInterface<IWMProfile>()) {
        ComSmartPtr<IWMStreamConfig> wmStreamConfig;
        auto hr =
            wmProfile->GetStream(0, wmStreamConfig.resetAndGetPointerAddress());

        if (SUCCEEDED(hr)) {
          if (auto wmMediaProperties =
                  wmStreamConfig.getInterface<IWMMediaProps>()) {
            DWORD sizeMediaType;
            ...
            if (mediaType->majortype == WMMEDIATYPE_Audio) {
              ...
              bitsPerSample = inputFormat->wBitsPerSample != 0
                                  ? inputFormat->wBitsPerSample
                                  : 16;
              lengthInSamples =
                  (lengthInNanoseconds * (QWORD)sampleRate) / 10000000;
            }
          }
        }
      }
...
//==============================================================================
AudioFormatReader *
WindowsMediaAudioFormat::createReaderFor(InputStream *sourceStream,
                                         bool deleteStreamIfOpeningFails) {
  std::unique_ptr<WindowsMediaCodec::WMAudioReader> r(
...
AudioFormatWriter *WindowsMediaAudioFormat::createWriterFor(
