...
static int mapping0_forward(vorbis_block *vb) {
  ...
  int ***floor_posts =
      (int ***)_vorbis_block_alloc(vb, vi->channels * sizeof(*floor_posts));
...

  {
    ...
    for (i = 0; i < vi->channels; i++) {
      ...
      floor_posts[i] =
          (int **)_vorbis_block_alloc(vb, PACKETBLOBS * sizeof(**floor_posts));
    ...
    for (k = (vorbis_bitrate_managed(vb) ? 0 : PACKETBLOBS / 2);
      ...
      for (i = 0; i < vi->channels; i++) {
        ...
        nonzero[i] = floor1_encode(
            opb, vb, (vorbis_look_floor1 *)(b->flr[info->floorsubmap[submap]]),
      ...
      for (i = 0; i < info->submaps; i++) {
        ...
        classifications = _residue_P[ci->residue_type[resnum]]->classx(
