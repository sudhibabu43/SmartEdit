...
static void vorbis_encode_residue_setup(vorbis_info *vi, int number, int block,
  ...
  vorbis_info_residue0 *r =
      (vorbis_info_residue0 *)(ci->residue_param[number] =
                                   (vorbis_info_residue0 *)_ogg_malloc(
                                       sizeof(*r)));
  ...
  /* lowpass setup/pointlimit */
  {
    ...
    vorbis_info_floor1 *f =
        (vorbis_info_floor1 *)ci->floor_param[block]; /* by convention */
...
int vorbis_encode_setup_init(vorbis_info *vi) {
  ...
  if (ci == NULL)
    return (OV_EINVAL);
  if (vi->channels < 1 || vi->channels > 255)
    return (OV_EINVAL);
