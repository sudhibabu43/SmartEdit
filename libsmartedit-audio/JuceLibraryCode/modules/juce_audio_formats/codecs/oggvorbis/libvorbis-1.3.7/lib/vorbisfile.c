...
static void _add_serialno(ogg_page *og, long **serialno_list, int *n) {
  ...
  if (*serialno_list) {
    *serialno_list =
        (long *)_ogg_realloc(*serialno_list, sizeof(**serialno_list) * (*n));
  } else {
...
static int _ov_open1(void *f, OggVorbis_File *vf, const char *initial,
  ...
  if ((ret = _fetch_headers(vf, vf->vi, vf->vc, &serialno_list,
    ...
    vf->serialnos =
        (long *)_ogg_calloc(serialno_list_size + 2, sizeof(*vf->serialnos));
