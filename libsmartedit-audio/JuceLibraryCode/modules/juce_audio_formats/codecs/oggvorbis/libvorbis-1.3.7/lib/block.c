...
int vorbis_block_init(vorbis_dsp_state *v, vorbis_block *vb) {
  ...
  if (v->analysisp) {
    vorbis_block_internal *vbi =
        (vorbis_block_internal *)(vb->internal =
                                      (vorbis_block_internal *)_ogg_calloc(
  ...
  if (bytes + vb->localtop > vb->localalloc) {
    ...
    if (vb->localstore) {
      struct alloc_chain *link =
          (struct alloc_chain *)_ogg_malloc(sizeof(*link));
...
static int _vds_shared_init(vorbis_dsp_state *v, vorbis_info *vi, int encp) {
  ...
  b = (private_state *)(v->backend_state =
                            (private_state *)_ogg_calloc(1, sizeof(*b)));
  ...
  b->residue =
      (vorbis_look_residue **)_ogg_calloc(ci->residues, sizeof(*b->residue));
...
float **vorbis_analysis_buffer(vorbis_dsp_state *v, int vals) {
  ...
  if (v->pcm_current + vals >= v->pcm_storage) {
    ...
    for (i = 0; i < vi->channels; i++) {
      v->pcm[i] =
          (float *)_ogg_realloc(v->pcm[i], v->pcm_storage * sizeof(*v->pcm[i]));
    }
  }
...
int vorbis_analysis_blockout(vorbis_dsp_state *v, vorbis_block *vb) {
  ...
  vbi->pcmdelay =
      (float **)_vorbis_block_alloc(vb, sizeof(*vbi->pcmdelay) * vi->channels);
  for (i = 0; i < vi->channels; i++) {
