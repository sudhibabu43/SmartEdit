...
JUCE_BEGIN_IGNORE_WARNINGS_GCC_LIKE("-Wconversion", "-Wdeprecated-register",
                                    "-Wfloat-equal", "-Wimplicit-fallthrough",
                                    "-Wlanguage-extension-token",
                                    "-Wredundant-decls", "-Wshadow",
                                    "-Wsign-conversion", "-Wswitch-default",
                                    "-Wswitch-enum",
...
public:
  ...
  bool readSamples(int *const *destSamples, int numDestChannels,
    ...
    const auto fillReservoir = [this](const int64 requestedStart) {
      ...
      if (requestedStart < bufferedRange.getStart() ||
          jmax(bufferedRange.getEnd(), bufferedRange.getStart() + (int64)511) <
              requestedStart) {
        ...
        FLAC__stream_decoder_seek_absolute(
            decoder, (FlacNamespace::FLAC__uint64)bufferedRange.getStart());
        return;
      }
      ...
      for (int i = numDestChannels; --i >= 0;)
        if (destSamples[i] != nullptr)
          zeromem(destSamples[i] + startOffsetInDestBuffer +
                      (remainingSamples.getStart() - startSampleInFile),
                  (size_t)remainingSamples.getLength() * sizeof(int));

    return true;
  }

  void useSamples(const FlacNamespace::FLAC__int32 *const buffer[],
                  int numSamples) {
  ...
  static void errorCallback_(const FlacNamespace::FLAC__StreamDecoder *,
...
public:
  ...
  bool write(const int **samplesToWrite, int numSamples) override {
    ...
    return FLAC__stream_encoder_process(
               encoder, (const FlacNamespace::FLAC__int32 **)samplesToWrite,
  ...
  static void packUint32(FlacNamespace::FLAC__uint32 val,
                         FlacNamespace::FLAC__byte *b, const int bytes) {
    b += bytes;
  ...
  void writeMetaData(const FlacNamespace::FLAC__StreamMetadata *metadata) {
    ...
    [[maybe_unused]] const bool seekOk =
        output->setPosition(streamStartPos + 4);
  ...
  //==============================================================================
  static FlacNamespace::FLAC__StreamEncoderWriteStatus
  encodeWriteCallback(const FlacNamespace::FLAC__StreamEncoder *,
                      const FlacNamespace::FLAC__byte buffer[], size_t bytes,
  ...
  static FlacNamespace::FLAC__StreamEncoderSeekStatus
  encodeSeekCallback(const FlacNamespace::FLAC__StreamEncoder *,
                     FlacNamespace::FLAC__uint64, void *) {
    return FlacNamespace::FLAC__STREAM_ENCODER_SEEK_STATUS_UNSUPPORTED;
  }

  static FlacNamespace::FLAC__StreamEncoderTellStatus
  encodeTellCallback(const FlacNamespace::FLAC__StreamEncoder *,
    ...
    *absolute_byte_offset =
        (FlacNamespace::FLAC__uint64) static_cast<FlacWriter *>(client_data)
            ->output->getPosition();
    return FlacNamespace::FLAC__STREAM_ENCODER_TELL_STATUS_OK;
  }

  static void
  encodeMetadataCallback(const FlacNamespace::FLAC__StreamEncoder *,
                         const FlacNamespace::FLAC__StreamMetadata *metadata,
                         void *client_data) {
...
Array<int> FlacAudioFormat::getPossibleBitDepths() { return {16, 24}; }
...
AudioFormatReader *
FlacAudioFormat::createReaderFor(InputStream *in,
...
AudioFormatWriter *FlacAudioFormat::createWriterFor(
  ...
  if (out != nullptr && getPossibleBitDepths().contains(bitsPerSample)) {
    std::unique_ptr<FlacWriter> w(
        new FlacWriter(out, sampleRate, numberOfChannels, (uint32)bitsPerSample,
                       qualityOptionIndex));
    if (w->ok)
      return w.release();
  }
