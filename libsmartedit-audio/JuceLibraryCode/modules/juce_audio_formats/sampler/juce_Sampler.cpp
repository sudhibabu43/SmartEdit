...
SamplerSound::SamplerSound(const String &soundName, AudioFormatReader &source,
  ...
  if (sourceSampleRate > 0 && source.lengthInSamples > 0) {
    ...
    data.reset(
        new AudioBuffer<float>(jmin(2, (int)source.numChannels), length + 4));
...
bool SamplerSound::appliesToChannel(int /*midiChannel*/) { return true; }
...
void SamplerVoice::startNote(int midiNoteNumber, float velocity,
                             SynthesiserSound *s,
...
void SamplerVoice::renderNextBlock(AudioBuffer<float> &outputBuffer,
  ...
  if (auto *playingSound =
          static_cast<SamplerSound *>(getCurrentlyPlayingSound().get())) {
