...
AudioFormatReader::~AudioFormatReader() { delete input; }

static void convertFixedToFloat(int *const *channels, int numChannels,
                                int numSamples) {
  constexpr auto scaleFactor = 1.0f / static_cast<float>(0x7fffffff);

  for (int i = 0; i < numChannels; ++i)
    if (auto d = channels[i])
      FloatVectorOperations::convertFixedToFloat(reinterpret_cast<float *>(d),
                                                 d, scaleFactor, numSamples);
}
...
bool AudioFormatReader::read(float *const *destChannels, int numDestChannels,
  ...
  if (!read(channelsAsInt, numDestChannels, startSampleInSource,
            numSamplesToRead, false))
    return false;
...
bool AudioFormatReader::read(int *const *destChannels, int numDestChannels,
  ...
  if (!readSamples(destChannels, jmin((int)numChannels, numDestChannels),
                   startOffsetInDestBuffer, startSampleInSource,
                   numSamplesToRead))
    return false;
...
static bool readChannels(AudioFormatReader &reader, int **chans,
                         AudioBuffer<float> *buffer, int startSample,
                         int numSamples, int64 readerStartSample,
                         int numTargetChannels, bool convertToFloat) {
  ...
  const bool success = reader.read(chans, numTargetChannels, readerStartSample,
                                   numSamples, true);
...
bool AudioFormatReader::read(AudioBuffer<float> *buffer, int startSample,
                             int numSamples, int64 readerStartSample,
                             bool useReaderLeftChan, bool useReaderRightChan) {
  ...
  if (numTargetChannels <= 2) {
    int *dests[2] = {
        reinterpret_cast<int *>(buffer->getWritePointer(0, startSample)),
        reinterpret_cast<int *>(numTargetChannels > 1
  ...
  if (numTargetChannels <= 64) {
    ...
    return readChannels(*this, chans, buffer, startSample, numSamples,
                        readerStartSample, numTargetChannels,
...
void AudioFormatReader::readMaxLevels(int64 startSampleInFile, int64 numSamples,
                                      Range<float> *const results,
                                      const int channelsToRead) {
  ...
  while (numSamples > 0) {
    ...
    for (int i = 0; i < channelsToRead; ++i) {
      ...
      if (usesFloatingPointData) {
        ...
        r = Range<float>(
            (float)intRange.getStart() / (float)std::numeric_limits<int>::max(),
...
int64 AudioFormatReader::searchForLevel(int64 startSample,
  ...
  int *tempBuffer[3] = {tempSpace.get(), tempSpace.get() + bufferSize, nullptr};
  ...
  auto doubleMin =
      jlimit(0.0, (double)std::numeric_limits<int>::max(),
             magnitudeRangeMinimum * std::numeric_limits<int>::max());
  auto doubleMax =
      jlimit(doubleMin, (double)std::numeric_limits<int>::max(),
  ...
  while (numSamplesToSearch != 0) {
    auto numThisTime =
        (int)jmin(std::abs(numSamplesToSearch), (int64)bufferSize);
...
//==============================================================================
MemoryMappedAudioFormatReader::MemoryMappedAudioFormatReader(
    const File &f, const AudioFormatReader &reader, int64 start, int64 length,
    int frameSize)
...
bool MemoryMappedAudioFormatReader::mapSectionOfFile(
  ...
  if (map == nullptr || samplesToMap != mappedSection) {
    ...
    map.reset(
        new MemoryMappedFile(file, fileRange, MemoryMappedFile::readOnly));

    if (map->getData() == nullptr)
      map.reset();
    else
      mappedSection = Range<int64>(
          jmax((int64)0, filePosToSample(map->getRange().getStart() +
                                         (bytesPerFrame - 1))),
...
static int memoryReadDummyVariable; // used to force the compiler not to
                                    // optimise-away the read operation
