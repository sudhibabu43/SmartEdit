...
//==============================================================================
/**
    A class for keeping a list of available audio formats, and for deciding
   which one to use to open a given file.

    After creating an AudioFormatManager object, you should call
   registerFormat() or registerBasicFormats() to give it a list of format types
   that it can use.
...
public:
  ...
  */
  ...
  /** Adds a format to the manager's list of available file types.
      ...
      The object passed-in will be deleted by this object, so don't keep a
     pointer to it!

      If makeThisTheDefaultFormat is true, then the getDefaultFormat() method
  ...
  */
  ...
  /** Handy method to make it easy to register the formats that come with JUCE.
      This will add WAV and AIFF to the list, along with any other formats
     enabled in either the Projucer or your application's preprocessor
     definitions.
  */
  ...
  /** Looks for which of the known formats is listed as being for a given file
      ...
      The extension may have a dot before it, so e.g. ".wav" or "wav" are both
     ok.
  */
  ...
  /** Returns the format which has been set as the default one.
      ...
      You can set a format as being the default when it is registered. It's
     useful when you want to write to a file, because the best format may change
     between platforms, e.g. AIFF is preferred on the Mac, WAV on Windows.

      If none has been set as the default, this method will just return the
     first one in the list.
  */
  AudioFormat *getDefaultFormat() const;

  /** Returns a set of wildcards for file-matching that contains the extensions
  ...
  */
  ...
  /** Searches through the known formats to try to create a suitable reader for
      ...
      The stream object that is passed-in will be deleted by this method or by
     the reader that is returned, so the caller should not keep any references
     to it.

      The stream that is passed-in must be capable of being repositioned so
      that all the formats can have a go at opening it.

      If none of the registered formats can open the stream, it'll return
     nullptr. If it returns a reader, it's the caller's responsibility to delete
     the reader.
  */
  AudioFormatReader *
  createReaderFor(std::unique_ptr<InputStream> audioFileStream);
