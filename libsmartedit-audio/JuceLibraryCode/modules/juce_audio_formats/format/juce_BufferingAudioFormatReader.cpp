...
bool BufferingAudioReader::readSamples(int *const *destSamples,
                                       int numDestChannels,
                                       int startOffsetInDestBuffer,
                                       int64 startSampleInFile,
                                       int numSamples) {
  auto startTime = Time::getMillisecondCounter();
  clearSamplesBeyondAvailableLength(destSamples, numDestChannels,
                                    startOffsetInDestBuffer, startSampleInFile,
  ...
  while (numSamples > 0) {
    if (auto block = getBlockContaining(startSampleInFile)) {
      ...
      auto numToDo =
          jmin(numSamples, (int)(block->range.getEnd() - startSampleInFile));
        ...
        if (auto *dest = (float *)destSamples[j]) {
          ...
          if (j < (int)numChannels)
            FloatVectorOperations::copy(
                dest, block->buffer.getReadPointer(j, offset), numToDo);
          else
      ...
      if (timeoutMs >= 0 &&
        ...
        for (int j = 0; j < numDestChannels; ++j)
          if (auto *dest = (float *)destSamples[j])
            FloatVectorOperations::clear(dest + startOffsetInDestBuffer,
                                         numSamples);
...
BufferingAudioReader::BufferedBlock::BufferedBlock(AudioFormatReader &reader,
                                                   int64 pos, int numSamples)
    : range(pos, pos + numSamples), buffer((int)reader.numChannels, numSamples),
      allSamplesRead(reader.read(&buffer, 0, numSamples, pos, true, true)) {}

BufferingAudioReader::BufferedBlock *
BufferingAudioReader::getBlockContaining(int64 pos) const noexcept {
  for (auto *b : blocks)
...
struct TestAudioFormatReader : public AudioFormatReader {
  explicit TestAudioFormatReader(const AudioBuffer<float> *b)
      : AudioFormatReader(nullptr, {}), buffer(b) {
  ...
  bool readSamples(int *const *destChannels, int numDestChannels,
    ...
    for (int j = 0; j < numDestChannels; ++j) {
      static_assert(sizeof(int) == sizeof(float),
                    "Int and float size must match in order for pointer "
      ...
      if (auto *dest = reinterpret_cast<float *>(destChannels[j])) {
        ...
        if (j < (int)numChannels)
          FloatVectorOperations::copy(
              dest, buffer->getReadPointer(j, (int)startSampleInFile),
              numSamples);
        else
...
public:
  ...
  void runTest() override {
    ...
    beginTest("Reading samples from a blocked reader should produce silence");
    {
      struct BlockingReader final : public TestAudioFormatReader {
        ...
        bool readSamples(int *const *destChannels, int numDestChannels,
                         ...
                         int numSamples) override {
          unblock.wait();
          return TestAudioFormatReader::readSamples(
              destChannels, numDestChannels, startOffsetInDestBuffer,
    ...
    beginTest("Reading samples from a reader should produce the same samples "
              "as its source");
    {
      Random random{getRandom()};
      ...
      for (auto i = 4; i < 18; ++i) {
        ...
        BufferingAudioReader reader(new TestAudioFormatReader(&source), thread,
                                    bufferSize);
...
private:
  void read(BufferingAudioReader &reader, AudioBuffer<float> &readBuffer) {
    ...
    for (;;) {
      reader.read(&readBuffer, readPos, jmin(blockSize, numSamples - readPos),
                  readPos, true, true);
