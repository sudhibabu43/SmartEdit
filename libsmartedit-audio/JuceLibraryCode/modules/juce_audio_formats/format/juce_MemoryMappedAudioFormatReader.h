...
/**
    ...
    Note that before reading samples from a MemoryMappedAudioFormatReader, you
   must first call mapEntireFile() or mapSectionOfFile() to ensure that the
   region you want to read has been mapped.
...
protected:
  ...
  /** Creates an MemoryMappedAudioFormatReader object.
      ...
      Note that before attempting to read any data, you must call
     mapEntireFile() or mapSectionOfFile() to ensure that the region you want to
     read has been mapped.
  */
  MemoryMappedAudioFormatReader(const File &file,
                                const AudioFormatReader &details,
                                int64 dataChunkStart, int64 dataChunkLength,
                                int bytesPerFrame);
...
public:
  ...
  /** Returns the sample range that's currently memory-mapped and available for
   * reading. */
  Range<int64> getMappedSection() const noexcept { return mappedSection; }

  /** Touches the memory for the given sample, to force it to be loaded into
   * active memory. */
...
protected:
  ...
  inline const void *sampleToPointer(int64 sample) const noexcept {
    return addBytesToPointer(map->getData(), sampleToFilePos(sample) -
                                                 map->getRange().getStart());
  }

  /** Used by AudioFormatReader subclasses to scan for min/max ranges in
   * interleaved data. */
  template <typename SampleType, typename Endianness>
  Range<float> scanMinAndMaxInterleaved(int channel, int64 startSampleInFile,
                                        int64 numSamples) const noexcept {
    using SourceType =
        AudioData::Pointer<SampleType, Endianness, AudioData::Interleaved,
                           AudioData::Const>;

    return SourceType(addBytesToPointer(sampleToPointer(startSampleInFile),
