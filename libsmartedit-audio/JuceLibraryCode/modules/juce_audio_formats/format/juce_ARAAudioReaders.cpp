...
ARAAudioSourceReader::~ARAAudioSourceReader() { invalidate(); }
...
void ARAAudioSourceReader::willUpdateAudioSourceProperties(
    ARAAudioSource *audioSource, ARAAudioSource::PropertiesPtr newProperties) {
...
void ARAAudioSourceReader::doUpdateAudioSourceContent(
...
void ARAAudioSourceReader::willEnableAudioSourceSamplesAccess(
    [[maybe_unused]] ARAAudioSource *audioSource, bool enable) {
  jassert(audioSourceBeingRead == audioSource);
...
void ARAAudioSourceReader::didEnableAudioSourceSamplesAccess(
    [[maybe_unused]] ARAAudioSource *audioSource, bool enable) {
  jassert(audioSourceBeingRead == audioSource);
...
void ARAAudioSourceReader::willDestroyAudioSource(
    [[maybe_unused]] ARAAudioSource *audioSource) {
  jassert(audioSourceBeingRead == audioSource);
...
bool ARAAudioSourceReader::readSamples(int *const *destSamples,
                                       int numDestChannels,
    ...
    if (readLock.isLocked() && hostReader != nullptr) {
      for (size_t i = 0; i < tmpPtrs.size(); ++i) {
        if ((i < (size_t)numDestChannels) && (destSamples[i] != nullptr)) {
          ...
          // We need to provide destination pointers for all channels in the ARA
          // read call, even if readSamples is not reading all of them. Hence we
          // use this dummy buffer to pad the read destination area.
      ...
      return hostReader->readAudioSamples(startSampleInFile, numSamples,
                                          tmpPtrs.data());
    }
  }
...
ARAPlaybackRegionReader::ARAPlaybackRegionReader(
    ...
    : ARAPlaybackRegionReader(playbackRegion->getAudioModification()
                                  ->getAudioSource()
                                  ->getSampleRate(),
...
ARAPlaybackRegionReader::ARAPlaybackRegionReader(
  ...
  // We're only providing the minimal set of meaningful values, since the ARA
  // renderer should only look at the time position and the playing state, and
  // read any related tempo or bar signature information from the ARA model
  // directly (MusicalContext).
  ...
  auto *documentController =
      (!playbackRegions.empty())
          ? playbackRegions.front()
                ->getDocumentController<ARADocumentController>()
          : nullptr;

  playbackRenderer.reset(
      documentController ? static_cast<ARAPlaybackRenderer *>(
  ...
  if (playbackRenderer != nullptr) {
    ...
    for (const auto &playbackRegion : playbackRegions) {
      ...
      auto playbackRegionTimeRange = playbackRegion->getTimeRange(
          ARAPlaybackRegion::IncludeHeadAndTail::yes);
      regionsStartTime =
          jmin(regionsStartTime, playbackRegionTimeRange.getStart());
    ...
    lengthInSamples =
        (int64)((regionsEndTime - regionsStartTime) * sampleRate + 0.5);

    playbackRenderer->prepareToPlay(
        rate, maximumBlockSize, numChans,
...
ARAPlaybackRegionReader::~ARAPlaybackRegionReader() { invalidate(); }
...
bool ARAPlaybackRegionReader::readSamples(int *const *destSamples,
                                          int numDestChannels,
                                          int startOffsetInDestBuffer,
                                          int64 startSampleInFile,
                                          int numSamples) {
  ...
  if (readLock.isLocked()) {
    if (isValid()) {
      ...
      while (numSamples > 0) {
        ...
        AudioBuffer<float> buffer((float **)destSamples, numDestChannels,
                                  startOffsetInDestBuffer, numSliceSamples);
        positionInfo.setTimeInSeconds(
            static_cast<double>(*positionInfo.getTimeInSamples()) / sampleRate);
        success &= playbackRenderer->processBlock(
            buffer, AudioProcessor::Realtime::no, positionInfo);
        numSamples -= numSliceSamples;
        startOffsetInDestBuffer += numSliceSamples;
        positionInfo.setTimeInSamples(*positionInfo.getTimeInSamples() +
                                      numSliceSamples);
      }
    }
  }
...
void ARAPlaybackRegionReader::willUpdatePlaybackRegionProperties(
    ARAPlaybackRegion *playbackRegion,
    ARAPlaybackRegion::PropertiesPtr newProperties) {
  jassert(
      ARA::contains(playbackRenderer->getPlaybackRegions(), playbackRegion));

  if ((!exactlyEqual(playbackRegion->getStartInAudioModificationTime(),
                     newProperties->startInModificationTime)) ||
      !exactlyEqual(playbackRegion->getDurationInAudioModificationTime(),
                    newProperties->durationInModificationTime) ||
      !exactlyEqual(playbackRegion->getStartInPlaybackTime(),
                    newProperties->startInPlaybackTime) ||
      !exactlyEqual(playbackRegion->getDurationInPlaybackTime(),
                    newProperties->durationInPlaybackTime) ||
      (playbackRegion->isTimestretchEnabled() !=
       ((newProperties->transformationFlags &
         ARA::kARAPlaybackTransformationTimestretch) != 0)) ||
      (playbackRegion->isTimeStretchReflectingTempo() !=
       ((newProperties->transformationFlags &
         ARA::kARAPlaybackTransformationTimestretchReflectingTempo) != 0)) ||
      (playbackRegion->hasContentBasedFadeAtHead() !=
       ((newProperties->transformationFlags &
         ARA::kARAPlaybackTransformationContentBasedFadeAtHead) != 0)) ||
      (playbackRegion->hasContentBasedFadeAtTail() !=
       ((newProperties->transformationFlags &
         ARA::kARAPlaybackTransformationContentBasedFadeAtTail) != 0))) {
    invalidate();
  }
}

void ARAPlaybackRegionReader::didUpdatePlaybackRegionContent(
...
void ARAPlaybackRegionReader::willDestroyPlaybackRegion(
    [[maybe_unused]] ARAPlaybackRegion *playbackRegion) {
  jassert(
      ARA::contains(playbackRenderer->getPlaybackRegions(), playbackRegion));
