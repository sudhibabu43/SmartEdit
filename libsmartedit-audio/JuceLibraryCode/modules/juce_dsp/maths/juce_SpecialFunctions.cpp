...
void SpecialFunctions::ellipticIntegralK(double k, double &K,
                                         double &Kp) noexcept {
  constexpr int M = 4;
...
Complex<double> SpecialFunctions::cde(Complex<double> u, double k) noexcept {
  ...
  *kei = k;
  ...
  for (int i = 0; i < M; ++i) {
    auto next =
        std::pow(*kei / (1.0 + std::sqrt(1.0 - std::pow(*kei, 2.0))), 2.0);
    *++kei = next;
  }

  // NB: the spurious cast to double here is a workaround for a very odd
  // link-time failure
  std::complex<double> last =
      std::cos(u * (double)MathConstants<double>::halfPi);
...
Complex<double> SpecialFunctions::sne(Complex<double> u, double k) noexcept {
  ...
  *kei = k;
  ...
  // link-time failure
  std::complex<double> last =
      std::sin(u * (double)MathConstants<double>::halfPi);
...
Complex<double> SpecialFunctions::asne(Complex<double> w, double k) noexcept {
  ...
  *kei = k;
  ...
  for (int i = 1; i <= M; ++i)
    last = 2.0 * last /
           ((1.0 + ke[i]) *
            (1.0 + std::sqrt(1.0 - std::pow(ke[i - 1] * last, 2.0))));
