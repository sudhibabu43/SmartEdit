...
/**
    General matrix and vectors class, meant for classic math manipulation such
   as additions, multiplications, and linear systems of equations solving.
...
public:
  ...
  */
  Matrix(size_t numRows, size_t numColumns, const ElementType *dataPointer)
      : rows(numRows), columns(numColumns) {
    resize();
    memcpy(data.getRawDataPointer(), dataPointer,
           rows * columns * sizeof(ElementType));
  }
  ...
  /** Creates a squared size x size Hankel Matrix from a vector with an optional
     offset.

      @param vector    The vector from which the Hankel matrix should be
     generated. Its number of rows should be at least 2 * (size - 1) + 1
  ...
  */
  ...
  //==============================================================================
  /** Swaps the contents of two rows in the matrix and returns a reference to
   * itself. */
  Matrix &swapRows(size_t rowOne, size_t rowTwo) noexcept;

  /** Swaps the contents of two columns in the matrix and returns a reference to
   * itself. */
  ...
  inline ElementType operator()(size_t row, size_t column) const noexcept {
    ...
    return data.getReference(
        static_cast<int>(dataAcceleration.getReference(static_cast<int>(row))) +
  ...
  /** Returns a pointer to the raw data of the matrix object, ordered in
     row-major order (for modifying).
  */
  ...
  /** Scalar multiplication */
  inline Matrix operator*(ElementType scalar) const {
    Matrix result(*this);
    result *= scalar;
    return result;
  }
  ...
  /** Does a hadarmard product with the receiver and other and stores the result
   * in the receiver */
  inline Matrix &hadarmard(const Matrix &other) noexcept {
    return apply(other, [](ElementType a, ElementType b) { return a * b; });
  }
  ...
  static bool compare(const Matrix &a, const Matrix &b,
                      ElementType tolerance = 0) noexcept;
  ...
  //==============================================================================
  /** Solves a linear system of equations represented by this object and the
     argument b, using various algorithms depending on the size of the
     arguments.

      The matrix must be a square matrix N times N, and b must be a vector N
     times 1, with the coefficients of b. After the execution of the algorithm,
