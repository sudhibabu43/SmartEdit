...
/**
    ...
    changes to the delay systematically using either a ramp or a low-pass
   filter.
...
public:
  ...
  /** Sets a new maximum delay in samples.
      ...
      This may allocate internally, so you should never call it from the audio
     thread.
  */
  ...
  */
  ...
  /** Pushes a single sample into one channel of the delay line.
      ...
      the delay in real time instead of using a fixed delay value, or if you
     want to code a delay effect with a feedback loop.
  ...
  */
  ...
  /** Pops a single sample from one channel of the delay line.
      ...
      @param delayInSamples       sets the wanted fractional delay in samples,
     or -1 to use the value being used before or set with setDelay function.
  ...
  */
  SampleType popSample(int channel, SampleType delayInSamples = -1,
...
private:
  ...
  SampleType interpolateSample(int channel) {
    if constexpr (std::is_same_v<InterpolationType,
    ...
    } else if constexpr (std::is_same_v<InterpolationType,
      ...
      return value1 * c1 +
             delayFrac * (value2 * c2 + value3 * c3 + value4 * c4);
    } else if constexpr (std::is_same_v<InterpolationType,
