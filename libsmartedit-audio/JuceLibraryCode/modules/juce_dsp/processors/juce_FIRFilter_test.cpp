...
class FIRFilterTest final : public UnitTest {
...
#if JUCE_USE_SIMD
  template <typename Type> struct Helpers<SIMDRegister<Type>> {
    static void fillRandom(Random &random, SIMDRegister<Type> *buffer,
                           size_t n) {
      Helpers<Type>::fillRandom(random, reinterpret_cast<Type *>(buffer),
                                n * SIMDRegister<Type>::size());
    }

    static bool checkArrayIsSimilar(SIMDRegister<Type> *a,
                                    SIMDRegister<Type> *b, size_t n) noexcept {
  ...
  static void fillRandom(Random &random, Type *buffer, size_t n) {
    Helpers<Type>::fillRandom(random, buffer, n);
  }
  ...
  static void reference(const NumericType *firCoefficients,
                        size_t numCoefficients, const SampleType *input,
                        SampleType *output, size_t n) noexcept {
    if (numCoefficients == 0) {
      zeromem(output, sizeof(SampleType) * n);
      return;
    }

    HeapBlock<SampleType> scratchBuffer(
        numCoefficients
#if JUCE_USE_SIMD
  ...
  struct LargeBlockTest {
    ...
    static void run(FIR::Filter<FloatType> &filter, FloatType *src,
                    FloatType *dst, size_t n) {
  ...
  struct SplitBlockTest {
    template <typename FloatType>
    static void run(FIR::Filter<FloatType> &filter, FloatType *input,
                    FloatType *output, size_t n) {
      size_t len = 0;
  ...
  void runTestForType() {
    ...
    for (auto size : {1, 2, 4, 8, 12, 13, 25}) {
      ...
      AudioBlock<SampleType> input(inputBuffer, 1, n),
          output(outputBuffer, 1, n), ref(refBuffer, 1, n);
      ...
      FIR::Filter<SampleType> filter(*new FIR::Coefficients<NumericType>(
          fir.getChannelPointer(0), static_cast<size_t>(size)));
      ProcessSpec spec{0.0, n, 1};
      filter.prepare(spec);

      reference<SampleType, NumericType>(
          fir.getChannelPointer(0), static_cast<size_t>(size),
          input.getChannelPointer(0), ref.getChannelPointer(0), n);

      TheTest::template run<SampleType>(filter, input.getChannelPointer(0),
                                        output.getChannelPointer(0), n);
      expect(checkArrayIsSimilar(output.getChannelPointer(0),
                                 ref.getChannelPointer(0), n));
    }
  }
...
public:
  FIRFilterTest() : UnitTest("FIR Filter", UnitTestCategories::dsp) {}
