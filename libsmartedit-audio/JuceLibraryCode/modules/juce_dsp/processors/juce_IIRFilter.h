...
/** A set of coefficients for use in an Filter object.
    ...
    @tags{DSP}
*/
template <typename NumericType> struct ArrayCoefficients {
  /** Returns the coefficients for a first order low-pass filter. */
  static std::array<NumericType, 4>
  ...
  static std::array<NumericType, 6> makeLowPass(double sampleRate,
                                                NumericType frequency);

  /** Returns the coefficients for a low-pass filter with variable Q. */
  static std::array<NumericType, 6>
  makeLowPass(double sampleRate, NumericType frequency, NumericType Q);

  /** Returns the coefficients for a high-pass filter. */
  static std::array<NumericType, 6> makeHighPass(double sampleRate,
                                                 NumericType frequency);
  ...
  static std::array<NumericType, 6> makeBandPass(double sampleRate,
                                                 NumericType frequency);
  ...
  static std::array<NumericType, 6> makeNotch(double sampleRate,
                                              NumericType frequency);
  ...
  static std::array<NumericType, 6> makeAllPass(double sampleRate,
                                                NumericType frequency);
  ...
  /** Returns the coefficients for a low-pass shelf filter with variable Q and
     gain.

      The gain is a scale factor that the low frequencies are multiplied by, so
     values greater than 1.0 will boost the low frequencies, values less
     than 1.0 will attenuate them.
  */
  ...
  /** Returns the coefficients for a high-pass shelf filter with variable Q and
     gain.

      The gain is a scale factor that the high frequencies are multiplied by, so
     values greater than 1.0 will boost the high frequencies, values less
     than 1.0 will attenuate them.
  */
  ...
  /** Returns the coefficients for a peak filter centred around a
      ...
      The gain is a scale factor that the centre frequencies are multiplied by,
     so values greater than 1.0 will boost the centre frequencies, values less
     than 1.0 will attenuate them.
  */
...
private:
  // Unfortunately, std::sqrt is not marked as constexpr just yet in all
  // compilers
...
/** A set of coefficients for use in an Filter object.
    ...
    @tags{DSP}
*/
template <typename NumericType> struct Coefficients : public ProcessorState {
  ...
  /** Directly constructs an object from the raw coefficients.
      ...
      constructor is public to allow tinkerers to create their own custom
     filters!
  */
  Coefficients(NumericType b0, NumericType b1, NumericType a0, NumericType a1);

  Coefficients(NumericType b0, NumericType b1, NumericType b2, NumericType a0,
  ...
  template <size_t Num>
  explicit Coefficients(const std::array<NumericType, Num> &values) {
  ...
  /** The Coefficients structure is ref-counted, so this is a handy type that
     can be used as a pointer to one.
  */
  ...
  */
  ...
  /** Returns the magnitude frequency response of the filter for a given
  ...
  */
  ...
  /** Returns the magnitude frequency response of the filter for a given
     frequency array and sample rate.
  */
  void getMagnitudeForFrequencyArray(const double *frequencies,
                                     double *magnitudes, size_t numSamples,
                                     double sampleRate) const noexcept;

  /** Returns the phase frequency response of the filter for a given frequency
     and sample rate
  */
  double getPhaseForFrequency(double frequency,
                              double sampleRate) const noexcept;
  ...
  */
  ...
  /** The raw coefficients.
      You should leave these numbers alone unless you really know what you're
     doing.
  */
...
private:
  ...
  template <size_t Num> Coefficients &assignImpl(const NumericType *values);

  template <size_t Num> Coefficients &assign(const NumericType (&values)[Num]) {
...
public:
  /** The NumericType is the underlying primitive type used by the SampleType
     (which could be either a primitive or vector)
  */
  ...
  /** Creates a filter.
      ...
      you process with it. You can modify the coefficients member to turn it
     into the type of filter needed.
  */
  Filter();
  ...
  */
  ...
  //==============================================================================
  /** Resets the filter's processing pipeline, ready to start a new stream of
     data.
  ...
  */
  ...
  /** Processes a single sample, without any locking.
      ...
      Moreover, you might need the function snapToZero after a few calls to
     avoid potential denormalisation issues.
  */
