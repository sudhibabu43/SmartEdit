...
struct Oversampling<SampleType>::OversamplingStage {
  OversamplingStage(size_t numChans, size_t newFactor)
      : numChannels(numChans), factor(newFactor) {}
  ...
  virtual void reset() { buffer.clear(); }
...
*/
template <typename SampleType>
struct OversamplingDummy final
    : public Oversampling<SampleType>::OversamplingStage {
  ...
  //==============================================================================
  SampleType getLatencyInSamples() const override { return 0; }

  void
  processSamplesUp(const AudioBlock<const SampleType> &inputBlock) override {
    jassert(inputBlock.getNumChannels() <=
            static_cast<size_t>(ParentType::buffer.getNumChannels()));
    jassert(inputBlock.getNumSamples() * ParentType::factor <=
  ...
  void processSamplesDown(AudioBlock<SampleType> &outputBlock) override {
    jassert(outputBlock.getNumChannels() <=
            static_cast<size_t>(ParentType::buffer.getNumChannels()));
    jassert(outputBlock.getNumSamples() * ParentType::factor <=
            static_cast<size_t>(ParentType::buffer.getNumSamples()));

    outputBlock.copyFrom(
        ParentType::getProcessedSamples(outputBlock.getNumSamples()));
  }
...
*/
template <typename SampleType>
struct Oversampling2TimesEquirippleFIR final
    : public Oversampling<SampleType>::OversamplingStage {
  using ParentType = typename Oversampling<SampleType>::OversamplingStage;
  ...
  Oversampling2TimesEquirippleFIR(size_t numChans,
    ...
    coefficientsUp =
        *FilterDesign<SampleType>::designFIRLowpassHalfBandEquirippleMethod(
    ...
    stateDown2.setSize(static_cast<int>(this->numChannels),
  ...
  SampleType getLatencyInSamples() const override {
    return static_cast<SampleType>(coefficientsUp.getFilterOrder() +
                                   coefficientsDown.getFilterOrder()) *
           0.5f;
  }
  ...
  processSamplesUp(const AudioBlock<const SampleType> &inputBlock) override {
    ...
    for (size_t channel = 0; channel < inputBlock.getNumChannels(); ++channel) {
      auto bufferSamples =
          ParentType::buffer.getWritePointer(static_cast<int>(channel));
  ...
  void processSamplesDown(AudioBlock<SampleType> &outputBlock) override {
    ...
    // Processing
    for (size_t channel = 0; channel < outputBlock.getNumChannels();
         ++channel) {
      auto bufferSamples =
          ParentType::buffer.getWritePointer(static_cast<int>(channel));
...
//==============================================================================
/** Oversampling stage class performing 2 times oversampling using the Filter
    Design IIR Polyphase Allpass Cascaded method. The resulting filter is
   minimum phase, and provided with a method to get the exact resulting latency.
*/
template <typename SampleType>
struct Oversampling2TimesPolyphaseIIR final
    : public Oversampling<SampleType>::OversamplingStage {
  using ParentType = typename Oversampling<SampleType>::OversamplingStage;
  ...
  Oversampling2TimesPolyphaseIIR(size_t numChans,
                                 ...
                                 SampleType stopbandAmplitudedBDown)
      : ParentType(numChans, 2) {
    auto structureUp = FilterDesign<SampleType>::
        designIIRLowpassHalfBandPolyphaseAllpassMethod(
            normalisedTransitionWidthUp, stopbandAmplitudedBUp);
    auto coeffsUp = getCoefficients(structureUp);
    latency =
        static_cast<SampleType>(-(coeffsUp.getPhaseForFrequency(0.0001, 1.0)) /
                                (0.0001 * MathConstants<double>::twoPi));

    auto structureDown = FilterDesign<SampleType>::
        designIIRLowpassHalfBandPolyphaseAllpassMethod(
            normalisedTransitionWidthDown, stopbandAmplitudedBDown);
    auto coeffsDown = getCoefficients(structureDown);
    latency += static_cast<SampleType>(
        -(coeffsDown.getPhaseForFrequency(0.0001, 1.0)) /
        (0.0001 * MathConstants<double>::twoPi));
    ...
    v1Down.setSize(static_cast<int>(this->numChannels),
                   coefficientsDown.size());
    delayDown.resize(static_cast<int>(this->numChannels));
  }

  //==============================================================================
  SampleType getLatencyInSamples() const override { return latency; }
  ...
  void snapToZero(bool snapUpProcessing) {
    if (snapUpProcessing) {
      for (auto channel = 0; channel < ParentType::buffer.getNumChannels();
           ++channel) {
...
private:
  ...
  */
  IIR::Coefficients<SampleType> getCoefficients(
      typename FilterDesign<SampleType>::IIRPolyphaseAllpassStructure
          &structure) const {
    ...
    for (auto *i : structure.directPath) {
      ...
      if (i->getFilterOrder() == 1) {
        numerator1 = numerator1.getProductWith(
            Polynomial<SampleType>({coeffs[0], coeffs[1]}));
        denominator1 = denominator1.getProductWith(
            Polynomial<SampleType>({one, coeffs[2]}));
      } else {
        numerator1 = numerator1.getProductWith(
            Polynomial<SampleType>({coeffs[0], coeffs[1], coeffs[2]}));
        denominator1 = denominator1.getProductWith(
            Polynomial<SampleType>({one, coeffs[3], coeffs[4]}));
      }
    }
    ...
    for (auto *i : structure.delayedPath) {
      ...
      if (i->getFilterOrder() == 1) {
        numerator2 = numerator2.getProductWith(
            Polynomial<SampleType>({coeffs[0], coeffs[1]}));
        denominator2 = denominator2.getProductWith(
            Polynomial<SampleType>({one, coeffs[2]}));
      } else {
        numerator2 = numerator2.getProductWith(
            Polynomial<SampleType>({coeffs[0], coeffs[1], coeffs[2]}));
...
Oversampling<SampleType>::Oversampling(size_t newNumChannels, size_t newFactor,
  ...
  if (newFactor == 0) {
    ...
    for (size_t n = 0; n < newFactor; ++n) {
      ...
      addOversamplingStage(FilterType::filterHalfBandPolyphaseIIR, twUp,
                           gaindBStartUp + gaindBFactorUp * (float)n, twDown,
    ...
    for (size_t n = 0; n < newFactor; ++n) {
      ...
      addOversamplingStage(FilterType::filterHalfBandFIREquiripple, twUp,
                           gaindBStartUp + gaindBFactorUp * (float)n, twDown,
                           gaindBStartDown + gaindBFactorDown * (float)n);
    }
  }
...
template <typename SampleType>
void Oversampling<SampleType>::addOversamplingStage(
    FilterType type, float normalisedTransitionWidthUp,
    float stopbandAmplitudedBUp, float normalisedTransitionWidthDown,
    float stopbandAmplitudedBDown) {
  if (type == FilterType::filterHalfBandPolyphaseIIR) {
    stages.add(new Oversampling2TimesPolyphaseIIR<SampleType>(
        numChannels, normalisedTransitionWidthUp, stopbandAmplitudedBUp,
        normalisedTransitionWidthDown, stopbandAmplitudedBDown));
  } else {
    stages.add(new Oversampling2TimesEquirippleFIR<SampleType>(
        numChannels, normalisedTransitionWidthUp, stopbandAmplitudedBUp,
        normalisedTransitionWidthDown, stopbandAmplitudedBDown));
  }
...
//==============================================================================
template <typename SampleType>
void Oversampling<SampleType>::setUsingIntegerLatency(
    bool useIntegerLatency) noexcept {
...
//==============================================================================
template <typename SampleType>
void Oversampling<SampleType>::initProcessing(
  ...
  ProcessSpec spec = {0.0, (uint32)maximumNumberOfSamplesBeforeOversampling,
                      (uint32)numChannels};
  delay.prepare(spec);
  updateDelayLine();
...
template <typename SampleType>
AudioBlock<SampleType> Oversampling<SampleType>::processSamplesUp(
  ...
  auto block = firstStage->getProcessedSamples(inputBlock.getNumSamples() *
                                               firstStage->factor);

  for (int i = 1; i < stages.size(); ++i) {
    stages[i]->processSamplesUp(block);
    block = stages[i]->getProcessedSamples(block.getNumSamples() *
                                           stages[i]->factor);
  }
...
template <typename SampleType>
void Oversampling<SampleType>::processSamplesDown(
  ...
  for (int n = stages.size() - 1; n > 0; --n) {
    ...
    auto audioBlock =
        stages.getUnchecked(n - 1)->getProcessedSamples(currentNumSamples);
...
void Oversampling<SampleType>::updateDelayLine() {
  auto latency = getUncompensatedLatency();
  fractionalDelay =
      static_cast<SampleType>(1.0) - (latency - std::floor(latency));
