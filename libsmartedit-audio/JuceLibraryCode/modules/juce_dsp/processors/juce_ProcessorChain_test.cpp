...
class ProcessorChainTest final : public UnitTest {
  template <int AddValue> struct MockProcessor {
    void prepare(const ProcessSpec &) noexcept { isPrepared = true; }
    void reset() noexcept { isReset = true; }

    template <typename Context> void process(const Context &context) noexcept {
      bufferWasClear =
          approximatelyEqual(context.getInputBlock().getSample(0, 0), 0.0f);
...
public:
  ProcessorChainTest() : UnitTest("ProcessorChain", UnitTestCategories::dsp) {}
  ...
  void runTest() override {
    ...
    beginTest("Chains with trailing items that only support replacing contexts "
              "can be built");
    {
      ...
      {
        juce::dsp::ProcessorChain<juce::dsp::Gain<float>, OnlyReplacing,
                                  OnlyReplacing>
            c;
