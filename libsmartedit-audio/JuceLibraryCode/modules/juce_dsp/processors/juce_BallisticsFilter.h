...
/**
    A processor to apply standard attack / release ballistics to an input
   signal. This is useful in dynamics processors, envelope followers, modulated
   audio effects and for smoothing animation in data visualisation.

    @tags{DSP}
*/
template <typename SampleType> class BallisticsFilter {
public:
  //==============================================================================
  ...
  /** Sets the attack time in ms.
      ...
      Attack times less than 0.001 ms will be snapped to zero and very long
     attack times will eventually saturate depending on the numerical precision
     used.
  */
  ...
  /** Sets the release time in ms.
      ...
      release times will eventually saturate depending on the numerical
     precision used.
  */
  ...
  /** Sets how the filter levels are calculated.
      ...
      an RMS (root mean squared) implementation of the ballistics filter
     instead. This is useful in some compressor and noise-gate designs, or in
     specific types of volume meters.
  */
  ...
  /** Resets the internal state variables of the filter to the given initial
   * value. */
