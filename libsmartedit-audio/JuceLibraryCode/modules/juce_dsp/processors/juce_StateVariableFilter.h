...
/**
    ...
    signal, with 12 dB of attenuation per octave, using a TPT structure,
   designed for fast modulation (see Vadim Zavalishin's documentation about TPT
    structures for more information). Its behaviour is based on the analog
    state variable filter circuit.

    Note: The bandpass here is not the one in the RBJ CookBook as its gain can
   be higher than 0 dB. For the classic 0 dB bandpass, we need to multiply the
    result by R2.

    Note 2: Using this class prevents some loud audio artefacts commonly
   encountered when changing the cutoff frequency using other filter simulation
   structures and IIR filter classes. However, this class may still require
   additional smoothing for cutoff frequency changes.
...
public:
  ...
  */
  ...
  [[deprecated("The classes in the StateVariableFilter namespace are "
               "deprecated. you should "
               "use the equivalent functionality in the StateVariableTPTFilter "
               "class.")]] Filter()
  ...
  [[deprecated("The classes in the StateVariableFilter namespace are "
               "deprecated. you should "
               "use the equivalent functionality in the StateVariableTPTFilter "
               "class.")]] Filter(ParametersPtr parametersToUse)
  ...
  */
  ...
  //==============================================================================
  /** The parameters of the state variable filter. It's up to the caller to
     ensure that these parameters are modified in a thread-safe way. */
  ...
  void process(const ProcessContext &context) noexcept {
    static_assert(
        std::is_same_v<typename ProcessContext::SampleType, SampleType>,
        "The sample-type of the filter must match the sample-type supplied to "
  ...
  SampleType JUCE_VECTOR_CALLTYPE processSample(SampleType sample) noexcept {
    ...
    case Parameters<NumericType>::Type::lowPass:
      return processLoop<false, Parameters<NumericType>::Type::lowPass>(
          sample, *parameters);
      break;
    case Parameters<NumericType>::Type::bandPass:
      return processLoop<false, Parameters<NumericType>::Type::bandPass>(
          sample, *parameters);
      break;
    case Parameters<NumericType>::Type::highPass:
      return processLoop<false, Parameters<NumericType>::Type::highPass>(
          sample, *parameters);
      break;
    default:
      jassertfalse;
...
private:
  ...
  processLoop(SampleType sample, Parameters<NumericType> &state) noexcept {
  ...
  template <bool isBypassed, typename Parameters<NumericType>::Type type>
  void processBlock(const SampleType *input, SampleType *output,
                    size_t n) noexcept {
  ...
  void processInternal(const ProcessContext &context) noexcept {
    ...
    // This class can only process mono signals. Use the ProcessorDuplicator
    // class to apply this filter on a multi-channel audio stream.
    ...
    switch (parameters->type) {
    case Parameters<NumericType>::Type::lowPass:
      processBlock<isBypassed, Parameters<NumericType>::Type::lowPass>(src, dst,
                                                                       n);
      break;
    case Parameters<NumericType>::Type::bandPass:
      processBlock<isBypassed, Parameters<NumericType>::Type::bandPass>(src,
                                                                        dst, n);
      break;
    case Parameters<NumericType>::Type::highPass:
      processBlock<isBypassed, Parameters<NumericType>::Type::highPass>(src,
                                                                        dst, n);
      break;
    default:
      jassertfalse;
...
template <typename NumericType> struct Parameters : public ProcessorState {
  ...
  */
  void setCutOffFrequency(double sampleRate, NumericType frequency,
                          NumericType resonance = static_cast<NumericType>(
                              1.0 / MathConstants<double>::sqrt2)) noexcept {
    jassert(sampleRate > 0);
    jassert(resonance > NumericType(0));
    jassert(frequency > NumericType(0) &&
            frequency <= NumericType(sampleRate * 0.5));

    g = static_cast<NumericType>(
        std::tan(MathConstants<double>::pi * frequency / sampleRate));
  ...
  */
  ...
  //==============================================================================
  NumericType g = static_cast<NumericType>(
      std::tan(MathConstants<double>::pi * 200.0 / 44100.0));
