...
template <typename SampleType>
void DryWetMixer<SampleType>::setWetMixProportion(
    SampleType newWetMixProportion) {
  jassert(isPositiveAndNotGreaterThan(newWetMixProportion, 1.0));

  mix = jlimit(static_cast<SampleType>(0.0), static_cast<SampleType>(1.0),
               newWetMixProportion);
  update();
}
...
void DryWetMixer<SampleType>::prepare(const ProcessSpec &spec) {
  ...
  bufferDry.setSize((int)spec.numChannels, (int)spec.maximumBlockSize, false,
                    false, true);
...
template <typename SampleType> void DryWetMixer<SampleType>::reset() {
  ...
  bufferDry.setSize(bufferDry.getNumChannels(), fifo.getSize(), false, false,
                    true);
}

//==============================================================================
template <typename SampleType>
void DryWetMixer<SampleType>::pushDrySamples(
    const AudioBlock<const SampleType> drySamples) {
  ...
  for (const auto &range : fifo.write((int)drySamples.getNumSamples())) {
    ...
    auto block =
        AudioBlock<SampleType>(bufferDry)
            .getSubsetChannelBlock(0, drySamples.getNumChannels())
            .getSubBlock((size_t)range.getStart(), (size_t)range.getLength());

    auto inputBlock =
        drySamples.getSubBlock((size_t)offset, (size_t)range.getLength());

    if (maximumWetLatencyInSamples == 0)
      block.copyFrom(inputBlock);
    else
      dryDelayLine.process(
          ProcessContextNonReplacing<SampleType>(inputBlock, block));
...
//==============================================================================
template <typename SampleType> void DryWetMixer<SampleType>::update() {
  ...
  case MixingRule::balanced:
    ...
    wetValue =
        static_cast<SampleType>(2.0) * jmin(static_cast<SampleType>(0.5), mix);
    break;
  ...
  case MixingRule::sin3dB:
    dryValue = static_cast<SampleType>(
        std::sin(0.5 * MathConstants<double>::pi * (1.0 - mix)));
    wetValue = static_cast<SampleType>(
        std::sin(0.5 * MathConstants<double>::pi * mix));
    break;

  case MixingRule::sin4p5dB:
    dryValue = static_cast<SampleType>(
        std::pow(std::sin(0.5 * MathConstants<double>::pi * (1.0 - mix)), 1.5));
    wetValue = static_cast<SampleType>(
        std::pow(std::sin(0.5 * MathConstants<double>::pi * mix), 1.5));
    break;

  case MixingRule::sin6dB:
    dryValue = static_cast<SampleType>(
        std::pow(std::sin(0.5 * MathConstants<double>::pi * (1.0 - mix)), 2.0));
    wetValue = static_cast<SampleType>(
        std::pow(std::sin(0.5 * MathConstants<double>::pi * mix), 2.0));
    break;
...
struct DryWetMixerTests final : public UnitTest {
  ...
  static auto getRampBuffer(ProcessSpec spec, Kind kind) {
    AudioBuffer<float> buffer((int)spec.numChannels,
    ...
    for (uint32_t sample = 0; sample < spec.maximumBlockSize; ++sample) {
      for (uint32_t channel = 0; channel < spec.numChannels; ++channel) {
        const auto ramp =
            kind == Kind::up ? sample : spec.maximumBlockSize - sample;
    ...
    for (auto maxLatency : {0, 100, 200, 512}) {
      beginTest("Mixer can push multiple small buffers");
      {
        ...
        for (auto block = 0; block < numBlocks; ++block) {
          ...
          for (uint32_t sample = 0; sample < spec.maximumBlockSize; ++sample)
            mixer.pushDrySamples(
                AudioBlock<const float>(dryBuffer).getSubBlock(sample, 1));
          ...
          for (uint32_t sample = 0; sample < spec.maximumBlockSize; ++sample) {
            for (uint32_t channel = 0; channel < spec.numChannels; ++channel) {
              const auto outputValue =
                  outputBlock.getSample((int)channel, (int)sample);
      ...
      beginTest("Mixer can pop multiple small buffers");
      {
        ...
        for (auto block = 0; block < numBlocks; ++block) {
          ...
          for (uint32_t sample = 0; sample < spec.maximumBlockSize; ++sample) {
            ...
            AudioBlock<const float>(wetBuffer).getSubBlock(sample, 1).copyTo(
                outputBlock);
            mixer.mixWetSamples({outputBlock});
      ...
      beginTest("Mixer can push and pop full-sized blocks after encountering a "
                "shorter block");
      {
        DryWetMixer<float> mixer(maxLatency);
        mixer.setWetMixProportion(0.5f);
        ...
        mixer.pushDrySamples(
            AudioBlock<const float>(dryBuffer).getSubBlock(shortBlockLength));
