...
template <typename NumericType> struct Coefficients;
...
/**
    ...
    Using FIRFilter is fast enough for FIRCoefficients with a size lower than
...
public:
  ...
  */
  ...
  /** Resets the filter's processing pipeline, ready to start a new stream of
     data.
      ...
      its coefficients aren't changed. To disable the filter, call setEnabled
     (false).
  */
  void reset() {
  ...
  */
  ...
  void process(const ProcessContext &context) noexcept {
    static_assert(
        ...
        "The sample-type of the FIR filter must match the sample-type supplied "
        "to this process callback");
    check();
  ...
  */
  SampleType JUCE_VECTOR_CALLTYPE processSample(SampleType sample) noexcept {
    check();
    return processSingleSample(sample, fifo, coefficients->getRawCoefficients(),
                               size, pos);
  }
...
private:
  ...
  static SampleType JUCE_VECTOR_CALLTYPE
  processSingleSample(SampleType sample, SampleType *buf,
...
template <typename NumericType> struct Coefficients : public ProcessorState {
  ...
  */
  ...
  //==============================================================================
  /** Scales the values of the FIR filter with the sum of the squared
   * coefficients. */
