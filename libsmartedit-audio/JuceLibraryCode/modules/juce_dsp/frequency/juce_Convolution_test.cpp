...
class ConvolutionTest final : public UnitTest {
  template <typename Callback> static void nTimes(int n, Callback &&callback) {
    for (auto i = 0; i < n; ++i)
      callback();
  }
  ...
  static AudioBuffer<float> makeStereoRamp(int length) {
    ...
    std::for_each(channels, channels + result.getNumChannels(),
                  [length](auto *channel) {
  ...
  void checkAllChannelsNonZero(const AudioBlock<float> &block) {
    for (size_t i = 0; i != block.getNumChannels(); ++i) {
      ...
      expect(std::any_of(
          channel, channel + block.getNumSamples(),
          [](float sample) { return !approximatelyEqual(sample, 0.0f); }));
    }
  }

  template <typename T>
  void nonAllocatingExpectWithinAbsoluteError(const T &a, const T &b,
                                              const T &error) {
  ...
  void checkLatency(const Convolution &convolution,
  ...
  void testConvolution(const ProcessSpec &spec, const ConvolutionConfig &config,
                       const AudioBuffer<float> &ir, double irSampleRate,
    ...
    const auto numBlocksPerSecond =
        (int)std::ceil(spec.sampleRate / spec.maximumBlockSize);
    const auto numBlocksForImpulse = (int)std::ceil(
    ...
    if (initSequence == InitSequence::loadThenPrepare)
      convolution.loadImpulseResponse(std::move(copiedIr), irSampleRate, stereo,
                                      trim, normalise);
    ...
    auto processBlocksWithDiracImpulse = [&] {
      for (auto i = 0; i != numBlocksForImpulse; ++i) {
        ...
        for (auto c = 0; c != static_cast<int>(spec.numChannels); ++c) {
          outBuffer.copyFrom(c, i * static_cast<int>(spec.maximumBlockSize),
    ...
    // If we load an IR while the convolution is already running, we'll need to
    // wait for it to be loaded on a background thread
    if (initSequence == InitSequence::prepareThenLoad) {
    ...
    // At this point, our convolution should be loaded and the current IR size
    // should match the expected result size
    expect(convolution.getCurrentIRSize() ==
    ...
    nTimes(5, [&] {
      ...
      for (size_t c = 0;
           c != static_cast<size_t>(expectedResult.getNumChannels()); ++c) {
        for (size_t i = 0;
             i != static_cast<size_t>(expectedResult.getNumSamples()); ++i) {
          const auto equivalentSample = i + actualLatency;

          if (static_cast<int>(equivalentSample) >= outBuffer.getNumSamples())
            continue;

          nonAllocatingExpectWithinAbsoluteError(
              outBuffer.getSample((int)c, (int)equivalentSample),
              expectedResult.getSample((int)c, (int)i), 0.01f);
        }
      }
    });
  }
  ...
  void testConvolution(const ProcessSpec &spec, const ConvolutionConfig &config,
    ...
    for (const auto sequence :
         {InitSequence::prepareThenLoad, InitSequence::loadThenPrepare})
      testConvolution(spec, config, ir, irSampleRate, stereo, trim, normalise,
                      expectedResult, sequence);
  }

public:
  ConvolutionTest() : UnitTest("Convolution", UnitTestCategories::dsp) {}
  ...
  void runTest() override {
    ...
    beginTest("Impulse responses can be loaded without allocating on the audio "
              "thread");
    {
      ...
      nTimes(100, [&] {
        convolution.loadImpulseResponse(
            std::move(copy), 1000, Convolution::Stereo::yes,
            Convolution::Trim::yes, Convolution::Normalise::no);
        addDiracImpulse(block);
        convolution.process(context);
        checkForNans(block);
      });
        ...
        "Convolution can be reset without allocating on the audio thread");
    {
      ...
      convolution.loadImpulseResponse(
          std::move(copy), 1000, Convolution::Stereo::yes,
          Convolution::Trim::yes, Convolution::Normalise::yes);
    ...
    beginTest("Completely empty IRs don't crash");
    {
      ...
      convolution.loadImpulseResponse(
          std::move(copy), 2000, Convolution::Stereo::yes,
          Convolution::Trim::yes, Convolution::Normalise::yes);
        ...
        "Convolutions can cope with a change in samplerate and blocksize");
    {
      ...
      convolution.loadImpulseResponse(
          std::move(copy), 2000, Convolution::Stereo::yes,
          Convolution::Trim::no, Convolution::Normalise::yes);

      const dsp::ProcessSpec specs[] = {
          {96'000.0, 1024, 2}, {48'000.0, 512, 2}, {44'100.0, 256, 2}};

      for (const auto &thisSpec : specs) {
        convolution.prepare(thisSpec);

        expectWithinAbsoluteError((double)convolution.getCurrentIRSize(),
                                  thisSpec.sampleRate * 0.5, 1.0);
    ...
    beginTest("Short uniform convolutions work");
    {
      ...
      testConvolution(spec, Convolution::Latency{0}, ramp, spec.sampleRate,
                      Convolution::Stereo::yes, Convolution::Trim::yes,
                      Convolution::Normalise::no, ramp);
    }
    ...
    beginTest("Normalisation works");
    {
      ...
      const auto factor =
          ...
          std::sqrt(std::accumulate(
              channels, channels + numChannels, 0.0f,
      ...
      std::for_each(channels, channels + numChannels,
                    [factor, numSamples](auto *channel) {
                      FloatVectorOperations::multiply(channel, factor,
                                                      numSamples);
                    });

      testConvolution(spec, Convolution::Latency{0}, ramp, spec.sampleRate,
                      Convolution::Stereo::yes, Convolution::Trim::yes,
                      Convolution::Normalise::yes, copy);
    }

    beginTest("Stereo convolutions work");
    {
      const auto ramp =
          makeStereoRamp(static_cast<int>(spec.maximumBlockSize) * 5);
      testConvolution(spec, Convolution::Latency{0}, ramp, spec.sampleRate,
                      Convolution::Stereo::yes, Convolution::Trim::yes,
                      Convolution::Normalise::no, ramp);
    }
    ...
    beginTest("Stereo IRs only use first channel if stereo is disabled");
    {
      ...
      testConvolution(spec, Convolution::Latency{0}, ramp, spec.sampleRate,
                      Convolution::Stereo::no, Convolution::Trim::yes,
                      Convolution::Normalise::no,
                      AudioBlock<const float>(channels,
                                              numElementsInArray(channels),
                                              (size_t)length));
    }
    ...
    beginTest("IRs with extra silence are trimmed appropriately");
    {
      ...
      AudioBuffer<float> paddedRamp(ramp.getNumChannels(),
                                    ramp.getNumSamples() * 2);
      paddedRamp.clear();

      const auto offset =
          (paddedRamp.getNumSamples() - ramp.getNumSamples()) / 2;

      for (auto channel = 0; channel != ramp.getNumChannels(); ++channel)
        paddedRamp.copyFrom(channel, offset, ramp.getReadPointer(channel),
                            length);

      testConvolution(spec, Convolution::Latency{0}, paddedRamp,
                      spec.sampleRate, Convolution::Stereo::no,
                      Convolution::Trim::yes, Convolution::Normalise::no, ramp);
    }

    beginTest("IRs are resampled if their sample rate is different to the "
              "playback rate");
    {
      for (const auto resampleRatio : {0.1, 0.5, 2.0, 10.0}) {
        ...
        const auto resampled = [&] {
          ...
          const auto finalSize =
              roundToInt(original.getNumSamples() / resampleRatio);
          resamplingSource.setResamplingRatio(resampleRatio);
          resamplingSource.prepareToPlay(finalSize,
        ...
        testConvolution(spec, Convolution::Latency{0}, ramp,
                        spec.sampleRate * resampleRatio,
                        Convolution::Stereo::yes, Convolution::Trim::yes,
                        Convolution::Normalise::no, resampled);
      }
    }
    ...
    beginTest("Non-uniform convolutions work");
    {
      ...
      for (auto headSize : {spec.maximumBlockSize / 2, spec.maximumBlockSize,
                            spec.maximumBlockSize * 9}) {
        testConvolution(
            spec, Convolution::NonUniform{static_cast<int>(headSize)}, ramp,
            spec.sampleRate, Convolution::Stereo::yes, Convolution::Trim::yes,
            Convolution::Normalise::no, ramp);
      }
    }
    ...
    beginTest("Convolutions with latency work");
    {
      ...
      for (auto latency :
           {static_cast<BlockSize>(0), spec.maximumBlockSize / 3,
            spec.maximumBlockSize, spec.maximumBlockSize * 2,
            static_cast<BlockSize>(spec.maximumBlockSize * 2.5)}) {
        testConvolution(spec, Convolution::Latency{static_cast<int>(latency)},
                        ramp, spec.sampleRate, Convolution::Stereo::yes,
                        Convolution::Trim::yes, Convolution::Normalise::no,
                        ramp);
      }
    }
