...
struct FFTUnitTest final : public UnitTest {
  FFTUnitTest() : UnitTest("FFT", UnitTestCategories::dsp) {}
  ...
  static Complex<float> freqConvolution(const Complex<float> *in, float freq,
                                        size_t n) {
  ...
  static void performReferenceFourier(const Complex<float> *in,
                                      Complex<float> *out, size_t n,
                                      bool reverse) {
    auto base_freq = static_cast<float>(
        ((reverse ? 1.0 : -1.0) * MathConstants<double>::twoPi) /
        static_cast<float>(n));
  ...
  static void performReferenceFourier(const float *in, Complex<float> *out,
    ...
    float base_freq = static_cast<float>(
        ((reverse ? 1.0 : -1.0) * MathConstants<double>::twoPi) /
        static_cast<float>(n));

    for (size_t i = 0; i < n; ++i)
      out[i] = freqConvolution(buffer.getData(),
  ...
  struct RealTest {
    static void run(FFTUnitTest &u) {
      ...
      for (size_t order = 0; order <= 8; ++order) {
        ...
        memcpy(reinterpret_cast<float *>(output.getData()), input.getData(),
               n * sizeof(float));
        ...
        std::fill(reference.getData() + ((n >> 1) + 1), reference.getData() + n,
                  std::complex<float>(0.0f));
        u.expect(checkArrayIsSimilar(reference.getData(), output.getData(),
                                     (n >> 1) + 1));

        memcpy(output.getData(), reference.getData(),
               n * sizeof(Complex<float>));
        fft.performRealOnlyInverseTransform((float *)output.getData());
        u.expect(
            checkArrayIsSimilar((float *)output.getData(), input.getData(), n));
      }
    }
  };
    ...
    static void run(FFTUnitTest &u) {
      ...
      for (size_t order = 0; order <= 8; ++order) {
        ...
        for (auto ignoreNegative : {false, true}) {
          ...
          fft.performFrequencyOnlyForwardTransform(inoutCopy.data(),
                                                   ignoreNegative);
          auto numMatching = ignoreNegative ? (n / 2) + 1 : n;
          u.expect(checkArrayIsSimilar(inoutCopy.data(), reference.data(),
                                       numMatching));
        }
      }
    }
  ...
  struct ComplexTest {
    static void run(FFTUnitTest &u) {
      ...
      for (size_t order = 0; order <= 7; ++order) {
        ...
        memcpy(buffer.getData(), reference.getData(),
  ...
  template <class TheTest> void runTestForAllTypes(const char *unitTestName) {
