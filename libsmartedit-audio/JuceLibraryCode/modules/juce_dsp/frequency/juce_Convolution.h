...
public:
  ...
  */
  ...
  /** Must be called before first calling process.
      ...
      asynchronously. The IR will become active once it has been completely
     loaded and processed, which may take some time.

      Calling prepare() will ensure that the IR supplied to the most recent call
     to loadImpulseResponse() is fully initialised. This IR will then be active
     during the next call to process(). It is recommended to call
     loadImpulseResponse() *before* prepare() if a specific IR must be active
     during the first process() call.
  */
  ...
  */
  template <
      ...
      std::enable_if_t<
          std::is_same_v<typename ProcessContext::SampleType, float>, int> = 0>
  ...
  */
  void loadImpulseResponse(const File &fileImpulseResponse, Stereo isStereo,
                           Trim requiresTrimming, size_t size,
                           Normalise requiresNormalisation = Normalise::yes);
  ...
  */
  void loadImpulseResponse(AudioBuffer<float> &&buffer, double bufferSampleRate,
                           Stereo isStereo, Trim requiresTrimming,
...
private:
  //==============================================================================
  Convolution(const Latency &, const NonUniform &,
  ...
  public:
    ...
    void processSamples(const AudioBlock<const float> &, AudioBlock<float> &,
                        bool isBypassed, ProcessWet &&) noexcept;
