...
struct FFT::Instance {
  ...
  virtual void perform(const Complex<float> *input, Complex<float> *output,
                       bool inverse) const noexcept = 0;
  virtual void performRealOnlyForwardTransform(float *,
...
struct FFT::Engine {
  Engine(int priorityToUse) : enginePriority(priorityToUse) {
    ...
    list.add(this);
    std::sort(list.begin(), list.end(), [](Engine *a, Engine *b) {
      return b->enginePriority < a->enginePriority;
    });
  }
  ...
  static FFT::Instance *createBestEngineForPlatform(int order) {
    ...
    jassertfalse; // This should never happen as the fallback engine should
                  // always work!
    return nullptr;
  }
...
private:
  ...
  int enginePriority; // used so that faster engines have priority over slower
                      // ones
};

template <typename InstanceToUse> struct FFT::EngineImpl : public FFT::Engine {
...
struct FFTFallback final : public FFT::Instance {
  ...
  static FFTFallback *create(int order) { return new FFTFallback(order); }
  ...
  void perform(const Complex<float> *input, Complex<float> *output,
               bool inverse) const noexcept override {
    if (size == 1) {
      *output = *input;
      return;
    }
  ...
  void performRealOnlyForwardTransform(float *d, bool) const noexcept override {
    ...
    if (scratchSize < maxFFTScratchSpaceToAlloca) {
      ...
      performRealOnlyForwardTransform(
          static_cast<Complex<float> *>(alloca(scratchSize)), d);
      JUCE_END_IGNORE_WARNINGS_MSVC
    } else {
      HeapBlock<char> heapSpace(scratchSize);
      performRealOnlyForwardTransform(
          unalignedPointerCast<Complex<float> *>(heapSpace.getData()), d);
    }
  }
  ...
  void performRealOnlyForwardTransform(Complex<float> *scratch,
  ...
  void performRealOnlyInverseTransform(Complex<float> *scratch,
                                       float *d) const noexcept {
  ...
  struct FFTConfig {
    FFTConfig(int sizeOfFFT, bool isInverse)
        : fftSize(sizeOfFFT), inverse(isInverse),
          twiddleTable((size_t)sizeOfFFT) {
      auto inverseFactor =
          (inverse ? 2.0 : -2.0) * MathConstants<double>::pi / (double)fftSize;

      if (fftSize <= 4) {
        for (int i = 0; i < fftSize; ++i) {
          auto phase = i * inverseFactor;

          twiddleTable[i] = {(float)std::cos(phase), (float)std::sin(phase)};
        }
      } else {
    ...
    void perform(const Complex<float> *input,
                 Complex<float> *output) const noexcept {
    ...
    void perform(const Complex<float> *input, Complex<float> *output,
                 int stride, int strideIn, const Factor *facs) const noexcept {
      auto factor = *facs++;
      ...
      if (stride == 1 && factor.radix <= 5) {
        for (int i = 0; i < factor.radix; ++i)
          perform(input + stride * strideIn * i, output + i * factor.length,
                  stride * factor.radix, strideIn, facs);
    ...
    void butterfly(const Factor factor, Complex<float> *data,
                   int stride) const noexcept {
      switch (factor.radix) {
      case 1:
        break;
      case 2:
        butterfly2(data, stride, factor.length);
        return;
      case 4:
        butterfly4(data, stride, factor.length);
        return;
      default:
        jassertfalse;
        break;
      ...
      JUCE_BEGIN_IGNORE_WARNINGS_MSVC(6255)
      auto *scratch = static_cast<Complex<float> *>(
          alloca((size_t)factor.radix * sizeof(Complex<float>)));
      JUCE_END_IGNORE_WARNINGS_MSVC
    ...
    void butterfly2(Complex<float> *data, const int stride,
    ...
    void butterfly4(Complex<float> *data, const int stride,
                    const int length) const noexcept {
      ...
      for (int i = length; --i >= 0;) {
        ...
        *data += s3;
        ...
        if (inverse) {
          ...
          data[lengthX3] = {s5.real() + s4.imag(), s5.imag() - s4.real()};
        } else {
          data[length] = {s5.real() + s4.imag(), s5.imag() - s4.real()};
...
struct AppleFFT final : public FFT::Instance {
  ...
  static AppleFFT *create(int order) { return new AppleFFT(order); }

  AppleFFT(int orderToUse)
      : order(static_cast<vDSP_Length>(orderToUse)),
        fftSetup(vDSP_create_fftsetup(order, 2)), forwardNormalisation(0.5f),
  ...
  void perform(const Complex<float> *input, Complex<float> *output,
    ...
    DSPSplitComplex splitInput(
        toSplitComplex(const_cast<Complex<float> *>(input)));
    DSPSplitComplex splitOutput(toSplitComplex(output));

    vDSP_fft_zop(fftSetup, &splitInput, 2, &splitOutput, 2, order,
                 inverse ? kFFTDirection_Inverse : kFFTDirection_Forward);

    float factor =
        (inverse ? inverseNormalisation : forwardNormalisation * 2.0f);
    vDSP_vsmul((float *)output, 1, &factor, (float *)output, 1,
               static_cast<size_t>(size << 1));
  }

  void performRealOnlyForwardTransform(
      float *inoutData, bool ignoreNegativeFreqs) const noexcept override {
    auto size = (1 << order);
    ...
    vDSP_vsmul(inoutData, 1, &forwardNormalisation, inoutData, 1,
               static_cast<size_t>(size << 1));
  ...
  void
  performRealOnlyInverseTransform(float *inoutData) const noexcept override {
    auto *inout = reinterpret_cast<Complex<float> *>(inoutData);
    auto size = (1 << order);
    auto splitInOut(toSplitComplex(inout));
    ...
    vDSP_vsmul(inoutData, 1, &inverseNormalisation, inoutData, 1,
               static_cast<size_t>(size << 1));
...
private:
  //==============================================================================
  void mirrorResult(Complex<float> *out,
                    bool ignoreNegativeFreqs) const noexcept {
    auto size = (1 << order);
    auto i = size >> 1;
...
struct FFTWImpl : public FFT::Instance {
  ...
  enum { measure = 0, unaligned = (1 << 1), estimate = (1 << 6) };

  struct Symbols {
    FFTWPlanRef (*plan_dft_fftw)(unsigned, Complex<float> *, Complex<float> *,
                                 int, unsigned);
    ...
    void (*execute_dft_fftw)(FFTWPlanRef, const Complex<float> *,
                             Complex<float> *);
...
#if JUCE_DSP_USE_STATIC_FFTW
      if (!Symbols::symbol(symbols.plan_dft_fftw, fftwf_plan_dft_1d))
        return nullptr;
      if (!Symbols::symbol(symbols.plan_r2c_fftw, fftwf_plan_dft_r2c_1d))
        return nullptr;
      if (!Symbols::symbol(symbols.plan_c2r_fftw, fftwf_plan_dft_c2r_1d))
        return nullptr;
      if (!Symbols::symbol(symbols.destroy_fftw, fftwf_destroy_plan))
        return nullptr;

      if (!Symbols::symbol(symbols.execute_dft_fftw, fftwf_execute_dft))
        return nullptr;
      if (!Symbols::symbol(symbols.execute_r2c_fftw, fftwf_execute_dft_r2c))
        return nullptr;
      if (!Symbols::symbol(symbols.execute_c2r_fftw, fftwf_execute_dft_c2r))
        return nullptr;
#else
      if (!Symbols::symbol(lib, symbols.plan_dft_fftw, "fftwf_plan_dft_1d"))
        return nullptr;
      if (!Symbols::symbol(lib, symbols.plan_r2c_fftw, "fftwf_plan_dft_r2c_1d"))
        return nullptr;
      if (!Symbols::symbol(lib, symbols.plan_c2r_fftw, "fftwf_plan_dft_c2r_1d"))
        return nullptr;
      if (!Symbols::symbol(lib, symbols.destroy_fftw, "fftwf_destroy_plan"))
        return nullptr;

      if (!Symbols::symbol(lib, symbols.execute_dft_fftw, "fftwf_execute_dft"))
        return nullptr;
      if (!Symbols::symbol(lib, symbols.execute_r2c_fftw,
                           "fftwf_execute_dft_r2c"))
        return nullptr;
      if (!Symbols::symbol(lib, symbols.execute_c2r_fftw,
  ...
  FFTWImpl(size_t orderToUse, DynamicLibrary &&libraryToUse,
           const Symbols &symbols)
      : fftwLibrary(std::move(libraryToUse)), fftw(symbols),
    ...
    c2cForward = fftw.plan_dft_fftw(n, in.getData(), out.getData(), -1,
                                    unaligned | estimate);
    c2cInverse = fftw.plan_dft_fftw(n, in.getData(), out.getData(), +1,
                                    unaligned | estimate);

    r2c = fftw.plan_r2c_fftw(n, (float *)in.getData(), in.getData(),
                             unaligned | estimate);
    c2r = fftw.plan_c2r_fftw(n, in.getData(), (float *)in.getData(),
                             unaligned | estimate);
  }
  ...
  void perform(const Complex<float> *input, Complex<float> *output,
    ...
    if (inverse) {
      ...
      FloatVectorOperations::multiply(
          (float *)output, 1.0f / static_cast<float>(n), (int)n << 1);
    } else {
  ...
  void performRealOnlyForwardTransform(
      ...
      bool ignoreNegativeFreqs) const noexcept override {
    if (order == 0)
      return;
  ...
  void performRealOnlyInverseTransform(
      float *inputOutputData) const noexcept override {
    auto n = (1u << order);

    fftw.execute_c2r_fftw(c2r, (Complex<float> *)inputOutputData,
                          inputOutputData);
    FloatVectorOperations::multiply((float *)inputOutputData,
                                    1.0f / static_cast<float>(n), (int)n);
  }

  //==============================================================================
  // fftw's plan_* and destroy_* methods are NOT thread safe. So we need to
  // share a lock between all instances of FFTWImpl
...
struct IntelFFT final : public FFT::Instance {
  ...
  static IntelFFT *create(int orderToUse) {
    ...
    if (DftiCreateDescriptor(&mklc2c, DFTI_SINGLE, DFTI_COMPLEX, 1,
                             1 << orderToUse) == 0) {
      if (succeeded(DftiSetValue(mklc2c, DFTI_PLACEMENT, DFTI_NOT_INPLACE)) &&
          succeeded(DftiSetValue(mklc2c, DFTI_BACKWARD_SCALE,
                                 1.0f / static_cast<float>(1 << orderToUse))) &&
          succeeded(DftiCommitDescriptor(mklc2c))) {
        if (succeeded(DftiCreateDescriptor(&mklc2r, DFTI_SINGLE, DFTI_REAL, 1,
                                           1 << orderToUse))) {
          if (succeeded(DftiSetValue(mklc2r, DFTI_PLACEMENT, DFTI_INPLACE)) &&
              succeeded(
                  DftiSetValue(mklc2r, DFTI_BACKWARD_SCALE,
                               1.0f / static_cast<float>(1 << orderToUse))) &&
              succeeded(DftiCommitDescriptor(mklc2r))) {
            return new IntelFFT(static_cast<size_t>(orderToUse), mklc2c,
                                mklc2r);
          }
  ...
  IntelFFT(size_t orderToUse, DFTI_DESCRIPTOR_HANDLE c2cToUse,
           DFTI_DESCRIPTOR_HANDLE cr2ToUse)
...
#if _IPP_SEQUENTIAL_STATIC || _IPP_SEQUENTIAL_DYNAMIC ||                       \
    _IPP_PARALLEL_STATIC || _IPP_PARALLEL_DYNAMIC
...
public:
  ...
  static IntelPerformancePrimitivesFFT *create(const int order) {
    ...
    if (complexContext.isValid() && realContext.isValid())
      return new IntelPerformancePrimitivesFFT(std::move(complexContext),
                                               std::move(realContext), order);
  ...
  void perform(const Complex<float> *input, Complex<float> *output,
    ...
    if (inverse) {
      ippsFFTInv_CToC_32fc(reinterpret_cast<const Ipp32fc *>(input),
                           reinterpret_cast<Ipp32fc *>(output), cplx.specPtr,
                           cplx.workBuf.get());
    } else {
...
private:
  ...
  struct IppFree {
    template <typename Ptr> void operator()(Ptr *ptr) const noexcept {
      ippsFree(ptr);
    }
  };
  ...
  template <typename Traits> struct Context {
    ...
    static Context create(const int order) {
      ...
      if (Traits::init(&specPtr, order, flag, hint, specBuf.get(),
                       initBuf.get()) != ippStsNoErr)
        return {};
  ...
  IntelPerformancePrimitivesFFT(Context<ComplexTraits> &&complexToUse,
      ...
      : cplx(std::move(complexToUse)), real(std::move(realToUse)),
        order(orderToUse) {}
...
void FFT::perform(const Complex<float> *input, Complex<float> *output,
                  bool inverse) const noexcept {
  if (engine != nullptr)
    engine->perform(input, output, inverse);
}

void FFT::performRealOnlyForwardTransform(
    float *inputOutputData, bool ignoreNegativeFreqs) const noexcept {
  if (engine != nullptr)
    engine->performRealOnlyForwardTransform(inputOutputData,
                                            ignoreNegativeFreqs);
}

void FFT::performRealOnlyInverseTransform(
    float *inputOutputData) const noexcept {
  if (engine != nullptr)
    engine->performRealOnlyInverseTransform(inputOutputData);
}

void FFT::performFrequencyOnlyForwardTransform(
    float *inputOutputData, bool ignoreNegativeFreqs) const noexcept {
  if (size == 1)
    return;
  ...
  zeromem(inputOutputData + limit,
          static_cast<size_t>(size * 2 - limit) * sizeof(float));
}
