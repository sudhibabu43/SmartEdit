...
public:
  explicit Queue(int size) : fifo(size), storage(static_cast<size_t>(size)) {}
  ...
  template <typename Fn> void pop(Fn &&fn) { popN(1, std::forward<Fn>(fn)); }
...
private:
  template <typename Fn> void popN(int n, Fn &&fn) {
    fifo.read(n).forEach(
        [&](int index) { fn(storage[static_cast<size_t>(index)]); });
  }
...
public:
  ...
  void popAll() {
    ...
    queue.popAll([](IncomingCommand &command) {
      command();
      command = nullptr;
    });
  }
  ...
  void run() override {
    while (!threadShouldExit()) {
      const auto tryPop = [&] {
        ...
        queue.pop([](IncomingCommand &command) {
          command();
          command = nullptr;
        });
        return true;
...
ConvolutionMessageQueue::ConvolutionMessageQueue(
    ConvolutionMessageQueue &&) noexcept = default;
ConvolutionMessageQueue &ConvolutionMessageQueue::operator=(
    ConvolutionMessageQueue &&) noexcept = default;
...
//==============================================================================
template <typename Element> class TryLockedPtr {
public:
...
// That way, we can avoid dangling references in the background thread in the
// case that a Convolution instance is deleted before the background message
// queue.
class ConvolutionEngineQueue final
...
public:
  ...
  void loadImpulseResponse(AudioBuffer<float> &&buffer, double sr,
                           Convolution::Stereo stereo, Convolution::Trim trim,
                           Convolution::Normalise normalise) {
    callLater([b = std::move(buffer), sr, stereo, trim,
               normalise](ConvolutionEngineFactory &f) mutable {
  ...
  void loadImpulseResponse(const void *sourceData, size_t sourceDataSize,
    ...
    callLater([sourceData, sourceDataSize, stereo, trim, size,
               normalise](ConvolutionEngineFactory &f) mutable {
      setImpulseResponse(f, sourceData, sourceDataSize, stereo, trim, size,
                         normalise);
    });
  }
  ...
  void loadImpulseResponse(const File &fileImpulseResponse,
                           Convolution::Stereo stereo, Convolution::Trim trim,
                           size_t size, Convolution::Normalise normalise) {
    callLater([fileImpulseResponse, stereo, trim, size,
               normalise](ConvolutionEngineFactory &f) mutable {
      setImpulseResponse(f, fileImpulseResponse, stereo, trim, size, normalise);
    });
  }

  void prepare(const ProcessSpec &spec) { factory.setProcessSpec(spec); }
...
private:
  template <typename Fn> void callLater(Fn &&fn) {
    // If there was already a pending command (because the queue was full) we'll
    // end up deleting it here. Not much we can do about that!
    pendingCommand = [weak = weakFromThis(),
...
public:
  void reset() { smoother.setCurrentAndTargetValue(1.0f); }

  void prepare(const ProcessSpec &spec) {
    smoother.reset(spec.sampleRate, 0.05);
    smootherBuffer.setSize(1, static_cast<int>(spec.maximumBlockSize));
    mixBuffer.setSize(static_cast<int>(spec.numChannels),
                      static_cast<int>(spec.maximumBlockSize));
    reset();
  }

  template <typename ProcessCurrent, typename ProcessPrevious,
            typename NotifyDone>
  void processSamples(const AudioBlock<const float> &input,
                      AudioBlock<float> &output, ProcessCurrent &&current,
                      ProcessPrevious &&previous, NotifyDone &&notifyDone) {
    if (smoother.isSmoothing()) {
      ...
      FloatVectorOperations::multiply(smootherBuffer.getWritePointer(0), -1.0f,
                                      numSamples);
      FloatVectorOperations::add(smootherBuffer.getWritePointer(0), 1.0f,
                                 numSamples);
...
public:
  Impl(Latency requiredLatency, NonUniform requiredHeadSize,
      ...
      : messageQueue(std::move(queue)),
        engineQueue(std::make_shared<ConvolutionEngineQueue>(
            *messageQueue->pimpl, requiredLatency, requiredHeadSize)) {}
  ...
  void loadImpulseResponse(AudioBuffer<float> &&buffer,
                           double originalSampleRate, Stereo stereo, Trim trim,
                           Normalise normalise) {
    engineQueue->loadImpulseResponse(std::move(buffer), originalSampleRate,
                                     stereo, trim, normalise);
  }

  void loadImpulseResponse(const void *sourceData, size_t sourceDataSize,
                           Stereo stereo, Trim trim, size_t size,
                           Normalise normalise) {
    engineQueue->loadImpulseResponse(sourceData, sourceDataSize, stereo, trim,
                                     size, normalise);
  }

  void loadImpulseResponse(const File &fileImpulseResponse, Stereo stereo,
                           Trim trim, size_t size, Normalise normalise) {
    engineQueue->loadImpulseResponse(fileImpulseResponse, stereo, trim, size,
                                     normalise);
  }
...
private:
  void destroyPreviousEngine() {
    ...
    BackgroundMessageQueue::IncomingCommand command =
        [p = std::move(previousEngine)]() mutable { p = nullptr; };
...
void Convolution::Mixer::prepare(const ProcessSpec &spec) {
  ...
  dryBlock = AudioBlock<float>(dryBlockStorage, jmin(spec.numChannels, 2u),
...
void Convolution::Mixer::processSamples(const AudioBlock<const float> &input,
  ...
  if (volumeDry[0].isSmoothing()) {
    ...
    for (size_t channel = 0; channel < numChannels; ++channel)
      volumeDry[channel].applyGain(dry.getChannelPointer(channel),
                                   (int)numSamples);

    processWet(input, output);

    for (size_t channel = 0; channel < numChannels; ++channel)
      volumeWet[channel].applyGain(output.getChannelPointer(channel),
                                   (int)numSamples);
...
//==============================================================================
Convolution::Convolution() : Convolution(Latency{0}) {}
...
Convolution::Convolution(const Latency &latency, const NonUniform &nonUniform,
                         OptionalQueue &&queue)
...
void Convolution::loadImpulseResponse(const void *sourceData,
                                      size_t sourceDataSize, Stereo stereo,
                                      Trim trim, size_t size,
                                      Normalise normalise) {
...
void Convolution::loadImpulseResponse(AudioBuffer<float> &&buffer,
                                      double originalSampleRate, Stereo stereo,
                                      Trim trim, Normalise normalise) {
...
void Convolution::processSamples(const AudioBlock<const float> &input,
  ...
  mixer.processSamples(
      input, output, isBypassed,
      [this](const auto &in, auto &out) { pimpl->processSamples(in, out); });
}
