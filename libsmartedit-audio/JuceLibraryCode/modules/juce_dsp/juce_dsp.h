...
/*******************************************************************************
 ...
 BEGIN_JUCE_MODULE_DECLARATION
  ...
  description:        Classes for audio buffer manipulation, digital audio
processing, filtering, oversampling, fast math functions etc. website:
http://www.juce.com/juce :            GPL/Commercial minimumCppStandard: 17
...
#if defined(_M_X64) || defined(__amd64__) || defined(__SSE2__) ||              \
...
// it's ok to check for _M_ARM below as this is only defined on Windows for Arm
// 32-bit which has a minimum requirement of armv7, which supports neon.
#elif defined(__ARM_NEON__) || defined(__ARM_NEON) || defined(__arm64__) ||    \
...
/** Config: JUCE_DSP_USE_SHARED_FFTW
    ...
    You must respect the FFTW  when enabling this option.
*/
...
/** Config: JUCE_DSP_USE_STATIC_FFTW
    ...
    You must add the fftw header/library folder to the extra header/library
   search paths of your JUCE project. You also need to add the fftw library
   itself to the extra libraries supplied to your JUCE project during linking.
...
/** Config: JUCE_DSP_ENABLE_SNAP_TO_ZERO
    ...
    filters and algorithms. If your audio app already disables denormals
   altogether (for example, by using the ScopedNoDenormals class or the
    FloatVectorOperations::disableDenormalisedNumberSupport method), then you
    can safely disable this flag to shave off a few cpu cycles from the DSP
   module's filters and algorithms.
*/
...
template <typename Type> using Complex = std::complex<Type>;
...
#if defined(__i386__) || defined(__amd64__) || defined(_M_X64) ||              \
