...
//==============================================================================
template <typename Type> struct SIMDRegister<Type>::ElementAccess {
...
template <typename Scalar> struct CmplxSIMDOps {
  ...
  static void JUCE_VECTOR_CALLTYPE store(vSIMDType value,
                                         Scalar *dest) noexcept {
  ...
  static vSIMDType JUCE_VECTOR_CALLTYPE set(vSIMDType v, std::size_t i,
                                            Scalar s) noexcept {
  ...
  static vSIMDType JUCE_VECTOR_CALLTYPE muladd(vSIMDType a, vSIMDType b,
...
template <typename Scalar> struct CmplxSIMDOps<std::complex<Scalar>> {
  using vSIMDType = typename SIMDNativeOps<Scalar>::vSIMDType;

  static vSIMDType JUCE_VECTOR_CALLTYPE
  load(const std::complex<Scalar> *a) noexcept {
    return SIMDNativeOps<Scalar>::load(reinterpret_cast<const Scalar *>(a));
  }

  static void JUCE_VECTOR_CALLTYPE store(vSIMDType value,
                                         std::complex<Scalar> *dest) noexcept {
  ...
  static std::complex<Scalar> JUCE_VECTOR_CALLTYPE get(vSIMDType v,
                                                       std::size_t i) noexcept {
    auto j = i << 1;
    return std::complex<Scalar>(SIMDNativeOps<Scalar>::get(v, j),
                                SIMDNativeOps<Scalar>::get(v, j + 1));
  }

  static vSIMDType JUCE_VECTOR_CALLTYPE set(vSIMDType v, std::size_t i,
                                            std::complex<Scalar> s) noexcept {
    auto j = i << 1;
    return SIMDNativeOps<Scalar>::set(
        SIMDNativeOps<Scalar>::set(v, j, s.real()), j + 1, s.imag());
  }
...
template <typename Type>
inline dsp::SIMDRegister<Type> JUCE_VECTOR_CALLTYPE
