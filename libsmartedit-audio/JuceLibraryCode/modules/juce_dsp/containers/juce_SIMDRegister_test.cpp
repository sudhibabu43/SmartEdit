...
template <typename type> struct RandomPrimitive {
  static type next(Random &random) {
    if constexpr (std::is_floating_point_v<type>) {
      return static_cast<type>(std::is_signed_v<type>
...
template <typename type> struct RandomValue {
  static type next(Random &random) {
    return RandomPrimitive<type>::next(random);
  }
};

template <typename type> struct RandomValue<std::complex<type>> {
  static std::complex<type> next(Random &random) {
    return {RandomPrimitive<type>::next(random),
            RandomPrimitive<type>::next(random)};
  }
};

template <typename type> static void fillVec(type *dst, Random &random) {
  std::generate_n(dst, SIMDRegister<type>::SIMDNumElements,
                  [&] { return RandomValue<type>::next(random); });
}

// Avoid visual studio warning
template <typename type> static type safeAbs(type a) {
  return static_cast<type>(std::abs(static_cast<double>(a)));
}

template <typename type> static type safeAbs(std::complex<type> a) {
  return std::abs(a);
}

template <typename type> static double difference(type a) {
  return static_cast<double>(safeAbs(a));
}

template <typename type> static double difference(type a, type b) {
...
public:
  ...
  static bool allValuesEqualTo(const SIMDRegister<type> &vec,
                               const type scalar) {
    alignas(sizeof(SIMDRegister<type>))
        type elements[SIMDRegister<type>::SIMDNumElements];

    vec.copyToRawArray(elements);

    // as we do not want to rely on the access operator we cast this to a
    // primitive pointer
    return std::all_of(
        std::begin(elements), std::end(elements),
        [scalar](const auto x) { return exactlyEqual(x, scalar); });
  }

  template <typename type>
  static bool vecEqualToArray(const SIMDRegister<type> &vec,
                              const type *array) {
    HeapBlock<type> vecElementsStorage(SIMDRegister<type>::SIMDNumElements * 2);
    auto *ptr =
        SIMDRegister<type>::getNextSIMDAlignedPtr(vecElementsStorage.getData());
    ...
    for (size_t i = 0; i < SIMDRegister<type>::SIMDNumElements; ++i) {
      ...
      if (delta > 1e-4) {
        DBG("a: " << SIMDRegister_test_internal::difference(ptr[i])
  ...
  struct InitializationTest {
    ...
    static void run(UnitTest &u, Random &random, Tag<type>) {
      u.expect(allValuesEqualTo<type>(
          SIMDRegister<type>::expand(static_cast<type>(23)), 23));

      {
#ifdef _MSC_VER
        __declspec(align(sizeof(SIMDRegister<type>)))
            type elements[SIMDRegister<type>::SIMDNumElements];
#else
        type elements[SIMDRegister<type>::SIMDNumElements]
            __attribute__((aligned(sizeof(SIMDRegister<type>))));
#endif
  ...
  template <class Operation> struct OperatorTests {
    template <typename type>
    static void run(UnitTest &u, Random &random, Tag<type>) {
      for (int n = 0; n < 100; ++n) {
        // set-up
        ...
        Operation::template inplace<SIMDRegister<type>, SIMDRegister<type>>(a,
                                                                            b);
        ...
        for (size_t i = 0; i < SIMDRegister<type>::SIMDNumElements; ++i)
          Operation::template inplace<type, type>(array_b[i],
                                                  static_cast<type>(2));
        ...
        for (size_t i = 0; i < SIMDRegister<type>::SIMDNumElements; ++i)
          array_c[i] = Operation::template outofplace<type, type>(array_a[i],
                                                                  array_b[i]);

        c = Operation::template outofplace<SIMDRegister<type>,
        ...
        for (size_t i = 0; i < SIMDRegister<type>::SIMDNumElements; ++i)
          array_c[i] = Operation::template outofplace<type, type>(
              array_b[i], static_cast<type>(2));
  ...
  template <class Operation> struct BitOperatorTests {
    template <typename type>
    static void run(UnitTest &u, Random &random, Tag<type>) {
      ...
      for (int n = 0; n < 100; ++n) {
        // Check flip sign bit and using as a union
        {
          ...
          vMaskType bitmask = vMaskType::expand(static_cast<MaskType>(1)
                                                << (sizeof(MaskType) - 1));
          SIMDRegister_test_internal::fillVec(array_a, random);
          copy(a.floatVersion, array_a);
          copy(b.floatVersion, array_a);

          Operation::template inplace<SIMDRegister<type>, vMaskType>(
              a.floatVersion, bitmask);
          Operation::template inplace<vMaskType, vMaskType>(b.intVersion,
                                                            bitmask);
        ...
        for (size_t i = 0; i < SIMDRegister<MaskType>::SIMDNumElements; ++i)
          Operation::template inplace<MaskType, MaskType>(array_a[i],
                                                          array_b[i]);
        memcpy(float_a, array_a,
               sizeof(type) * SIMDRegister<type>::SIMDNumElements);
        ...
        // test in-place with one param being scalar
        for (size_t i = 0; i < SIMDRegister<MaskType>::SIMDNumElements; ++i)
          Operation::template inplace<MaskType, MaskType>(
              array_a[i], static_cast<MaskType>(9));
        memcpy(float_a, array_a,
               sizeof(type) * SIMDRegister<type>::SIMDNumElements);

        Operation::template inplace<SIMDRegister<type>, MaskType>(
            a, static_cast<MaskType>(9));
        ...
        // test out-of-place with both params being vectors
        for (size_t i = 0; i < SIMDRegister<MaskType>::SIMDNumElements; ++i) {
          array_c[i] = Operation::template outofplace<MaskType, MaskType>(
              array_a[i], array_b[i]);
        }
        memcpy(float_a, array_a,
               sizeof(type) * SIMDRegister<type>::SIMDNumElements);
        memcpy(float_c, array_c,
               sizeof(type) * SIMDRegister<type>::SIMDNumElements);
        ...
        c = Operation::template outofplace<SIMDRegister<type>, MaskType>(
            a, static_cast<MaskType>(9));
    ...
    static void run(UnitTest &u, Random &random, Tag<type>) {
      ...
      for (int i = 0; i < 100; ++i) {
        ...
        for (size_t j = 0; j < SIMDRegister<type>::SIMDNumElements; ++j) {
          array_eq[j] = (exactlyEqual(array_a[j], array_b[j]))
                            ? static_cast<MaskType>(-1)
                            : 0;
          array_neq[j] = (!exactlyEqual(array_a[j], array_b[j]))
                             ? static_cast<MaskType>(-1)
                             : 0;
          array_lt[j] =
              (array_a[j] < array_b[j]) ? static_cast<MaskType>(-1) : 0;
          array_le[j] =
              (array_a[j] <= array_b[j]) ? static_cast<MaskType>(-1) : 0;
          array_gt[j] =
              (array_a[j] > array_b[j]) ? static_cast<MaskType>(-1) : 0;
        ...
        do {
          ...
          SIMDRegister_test_internal::fillVec(array_b, random);
            ...
            array_a, array_a + SIMDRegister<type>::SIMDNumElements, array_b));
  ...
  struct CheckSum {
    ...
    static void run(UnitTest &u, Random &random, Tag<type>) {
      ...
      const auto sumCheck = (type)std::accumulate(std::begin(array),
                                                  std::end(array), AddedType{});

      SIMDRegister<type> a;
      copy(a, array);

      u.expect(SIMDRegister_test_internal::difference(sumCheck, a.sum()) <
               1e-4);
    }
  };
  ...
  struct CheckBoolEquals {
    ...
    static void run(UnitTest &u, Random &random, Tag<type>) {
      ...
      auto value = std::is_signed_v<type>
                       ? static_cast<type>((random.nextFloat() * 16.0) - 8.0)
  ...
  void runTest() override {
    ...
    runTestForAllTypes("MultiplicationOperators",
                       OperatorTests<Multiplication>{});
