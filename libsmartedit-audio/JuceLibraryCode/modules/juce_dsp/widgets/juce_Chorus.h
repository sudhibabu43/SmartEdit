...
/**
    A simple chorus DSP widget that modulates the delay of a delay line in order
   to create sweeping notches in the magnitude frequency response.

    This audio effect can be controlled via the speed and depth of the LFO
   controlling the frequency response, a mix control, a feedback control, and
   the centre delay of the modulation.

    Note: To get classic chorus sounds try to use a centre delay time around 7-8
   ms with a low feedback volume and a low depth. This effect can also be used
   as a flanger with a lower centre delay time and a lot of feedback, and as a
   vibrato effect if the mix value is 1.

    @tags{DSP}
*/
template <typename SampleType> class Chorus {
public:
  //==============================================================================
  /** Constructor. */
  Chorus();
  ...
  //==============================================================================
  /** Sets the rate (in Hz) of the LFO modulating the chorus delay line. This
     rate must be lower than 100 Hz.
  */
  void setRate(SampleType newRateHz);

  /** Sets the volume of the LFO modulating the chorus delay line (between 0 and
   * 1).
   */
  ...
  */
  ...
  /** Sets the amount of dry and wet signal in the output of the chorus (between
  ...
  */
  ...
  void process(const ProcessContext &context) noexcept {
    ...
    auto delayValuesBlock =
        AudioBlock<SampleType>(bufferDelayTimes).getSubBlock(0, numSamples);
    ...
    for (size_t i = 0; i < numSamples; ++i) {
      auto lfo = jmax(static_cast<SampleType>(1.0),
                      maximumDelayModulation * delaySamples[i] + centreDelay);
...
private:
  ...
  std::vector<SmoothedValue<SampleType, ValueSmoothingTypes::Linear>>
      feedbackVolume{2};
  ...
  static constexpr SampleType maxDepth = 1.0, maxCentreDelayMs = 100.0,
