...
//==============================================================================
template <typename SampleType> Compressor<SampleType>::Compressor() {
  update();
}
...
template <typename SampleType> void Compressor<SampleType>::reset() {
...
SampleType Compressor<SampleType>::processSample(int channel,
  ...
  auto gain = (env < threshold)
                  ...
                  : std::pow(env * thresholdInverse,
                             ratioInverse - static_cast<SampleType>(1.0));

  // Output
  return gain * inputValue;
}

template <typename SampleType> void Compressor<SampleType>::update() {
  threshold =
