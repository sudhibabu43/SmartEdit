...
//==============================================================================
template <typename SampleType> Chorus<SampleType>::Chorus() {
...
void Chorus<SampleType>::setCentreDelay(SampleType newDelayMs) {
  ...
  centreDelay =
      jlimit(static_cast<SampleType>(1.0), maxCentreDelayMs, newDelayMs);
}
...
void Chorus<SampleType>::prepare(const ProcessSpec &spec) {
  ...
  const auto maxPossibleDelay =
      std::ceil((maximumDelayModulation * maxDepth * oscVolumeMultiplier +
                 maxCentreDelayMs) *
                sampleRate / 1000.0);
  delay = DelayLine<SampleType, DelayLineInterpolationTypes::Linear>{
...
template <typename SampleType> void Chorus<SampleType>::reset() {
...
template <typename SampleType> void Chorus<SampleType>::update() {
