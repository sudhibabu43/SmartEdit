...
/**
    Processor wrapper around juce::Reverb for easy integration into
   ProcessorChain.
...
public:
  //==============================================================================
  /** Creates an uninitialised Reverb processor. Call prepare() before first
   * use. */
  Reverb() = default;
  ...
  void process(const ProcessContext &context) noexcept {
    ...
    if (numInChannels == 1 && numOutChannels == 1) {
      ...
      reverb.processStereo(outputBlock.getChannelPointer(0),
                           outputBlock.getChannelPointer(1), (int)numSamples);
    } else {
