...
class Timer::TimerThread final : private Thread, private AsyncUpdater {
public:
  using LockType = CriticalSection; // (mysteriously, using a SpinLock here
  ...
  void run() override {
    ...
    ReferenceCountedObjectPtr<CallTimersMessage> messageToSend(
      ...
      if (timeUntilFirstTimer <= 0) {
        if (callbackArrived.wait(0)) {
          ...
          if (!callbackArrived.wait(300)) {
            // Sometimes our message can get discarded by the OS (e.g. when
            // running as an RTAS when the app has a modal loop), so this is how
            // long to wait before assuming the message has been lost and trying
            // again.
            messageToSend->post();
          }
  ...
  void callTimers() {
    ...
    while (!timers.empty()) {
      ...
      JUCE_TRY { timer->timerCallback(); }
      JUCE_CATCH_EXCEPTION

      // avoid getting stuck in a loop if a timer callback repeatedly takes too
      // long
...
private:
  ...
  struct CallTimersMessage final : public MessageManager::MessageBase {
    ...
    void messageCallback() override {
      if (auto instance = SharedResourcePointer<
              TimerThread>::getSharedObjectWithoutCreating())
        (*instance)->callTimers();
    }
  };
  ...
  void handleAsyncUpdate() override { startThread(Priority::high); }
...
Timer::~Timer() {
  // If you're destroying a timer on a background thread, make sure the timer
  // has been stopped before execution reaches this point. A simple way to
  // achieve this is to add a call to `stopTimer()` to the destructor of your
  // class which inherits from Timer.
...
void JUCE_CALLTYPE Timer::callPendingTimersSynchronously() {
  if (auto instance =
          SharedResourcePointer<TimerThread>::getSharedObjectWithoutCreating())
...
void JUCE_CALLTYPE Timer::callAfterDelay(int milliseconds,
                                         std::function<void()> f) {
