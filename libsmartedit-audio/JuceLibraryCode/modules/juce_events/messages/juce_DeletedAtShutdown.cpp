...
static SpinLock deletedAtShutdownLock; // use a spin lock because it can be
...
// that you have no classes of DeletedAtShutdown that could throw an exception
// in their destructor.
...
void DeletedAtShutdown::deleteAll() {
  ...
  for (int i = localCopy.size(); --i >= 0;) {
    JUCE_TRY {
      ...
      // double-check that it's not already been deleted during another object's
      // destructor.
      {
        const SpinLock::ScopedLockType sl(deletedAtShutdownLock);
  ...
  // if this fails, then it's likely that some new DeletedAtShutdown objects
  // were created while executing the destructors of the other ones.
  jassert(getDeletedAtShutdownObjects().isEmpty());

  getDeletedAtShutdownObjects().clear(); // just to make sure the array doesn't
                                         // have any memory still allocated
}
