...
void JUCEApplicationBase::setApplicationReturnValue(
    const int newReturnValue) noexcept {
  appReturnValue = newReturnValue;
}

// This is called on the Mac and iOS where the OS doesn't allow the stack to
// unwind on shutdown..
...
struct JUCEApplicationBase::MultipleInstanceHandler final
...
bool JUCEApplicationBase::sendCommandLineToPreexistingInstance() {
  ...
  multipleInstanceHandler.reset(
      new MultipleInstanceHandler(getApplicationName()));
...
String JUCE_CALLTYPE JUCEApplicationBase::getCommandLineParameters() {
  return CharacterFunctions::findEndOfToken(
             CharPointer_UTF16(GetCommandLineW()), CharPointer_UTF16(L" "),
             CharPointer_UTF16(L"\""))
      .findEndOfWhitespace();
}
...
#if (JUCE_LINUX || JUCE_BSD) && JUCE_MODULE_AVAILABLE_juce_gui_extra &&        \
    (!defined(JUCE_WEB_BROWSER) || JUCE_WEB_BROWSER)
...
String JUCEApplicationBase::getCommandLineParameters() {
  ...
  for (const auto &arg : getCommandLineParameterArray()) {
    const auto withQuotes =
        arg.containsChar(' ') && !arg.isQuotedString() ? arg.quoted('"') : arg;
...
bool JUCEApplicationBase::initialiseApp() {
  ...
  if (isStandaloneApp() && AttachConsole(ATTACH_PARENT_PROCESS) != 0) {
    // if we've launched a GUI app from cmd.exe or PowerShell, we need this to
    // enable printf etc. However, only reassign stdout, stderr, stdin if they
    // have not been already opened by a redirect or similar.
    FILE *ignore;
