...
MessageManager::~MessageManager() noexcept {
  ...
  instance = nullptr; // do this last in case this instance is still needed by
                      // doPlatformSpecificShutdown()
}
...
void MessageManager::deleteInstance() { deleteAndZero(instance); }
...
bool MessageManager::MessageBase::post() {
  ...
  if (mm == nullptr || mm->quitMessagePosted.get() != 0 ||
      !postMessageToSystemQueue(this)) {
    Ptr deleter(this); // (this will delete messages that were just created with
                       // a 0 ref count)
    return false;
  }
...
// implemented in platform-specific code (juce_Messaging_linux.cpp and
...
void *MessageManager::callFunctionOnMessageThread(MessageCallbackFunction *func,
  ...
  const ReferenceCountedObjectPtr<AsyncFunctionCallback> message(
      new AsyncFunctionCallback(func, parameter));
...
void MessageManager::deregisterBroadcastListener(
...
#if JUCE_WINDOWS
    // This is needed on windows to make sure the message window is created by
    // this thread
    doPlatformSpecificShutdown();
    doPlatformSpecificInitialisation();
#endif
...
/*  The only safe way to lock the message thread while another thread does
    some work is by posting a special message, whose purpose is to tie up the
   event loop until the other thread has finished its business.
   ...
   OS lock gets indirectly accessed from another thread inside a MM lock, you're
   screwed. (this is exactly what happens in Cocoa).
*/
struct MessageManager::Lock::BlockingMessage final
...
bool MessageManager::Lock::tryEnter() const noexcept {
  return exclusiveTryAcquire(false);
}

bool MessageManager::Lock::exclusiveTryAcquire(
    bool lockIsMandatory) const noexcept {
  if (lockIsMandatory)
    entryMutex.enter();
...
void MessageManager::Lock::abort() const noexcept { setAcquired(false); }
...
bool MessageManagerLock::attemptLock(Thread *threadToCheck,
                                     ThreadPoolJob *jobToCheck) {
  ...
  // tryEnter may have a spurious abort (return false) so keep checking the
  // condition
...
void MessageManagerLock::exitSignalSent() { mmLock.abort(); }

//==============================================================================
JUCE_API void JUCE_CALLTYPE initialiseJuce_GUI() {
  JUCE_AUTORELEASEPOOL { MessageManager::getInstance(); }
}
