...
struct AppDelegateClass final : public ObjCClass<NSObject> {
  AppDelegateClass() : ObjCClass("JUCEAppDelegate_") {
    addMethod(@selector(applicationWillFinishLaunching:),
              [](id self, SEL, NSNotification *) {
    ...
    addMethod(@selector(applicationShouldTerminate:),
              [](id /*self*/, SEL, NSApplication *) {
    ...
    addMethod(@selector(applicationWillTerminate:),
              [](id /*self*/, SEL, NSNotification *) {
                JUCEApplicationBase::appWillTerminateByForce();
              });

    addMethod(@selector(application:openFile:),
              [](id /*self*/, SEL, NSApplication *, NSString *filename) {
                if (auto *app = JUCEApplicationBase::getInstance()) {
    ...
    addMethod(@selector(application:openFiles:),
              [](id /*self*/, SEL, NSApplication *, NSArray *filenames) {
                if (auto *app = JUCEApplicationBase::getInstance()) {
                  StringArray files;
    ...
    addMethod(@selector(applicationDidBecomeActive:),
              [](id /*self*/, SEL, NSNotification *) { focusChanged(); });
    addMethod(@selector(applicationDidResignActive:),
              [](id /*self*/, SEL, NSNotification *) { focusChanged(); });
    addMethod(@selector(applicationWillUnhide:),
              [](id /*self*/, SEL, NSNotification *) { focusChanged(); });

    JUCE_BEGIN_IGNORE_WARNINGS_GCC_LIKE("-Wundeclared-selector")
    addMethod(@selector(getUrl:withReplyEvent:),
              [](id /*self*/, SEL, NSAppleEventDescriptor *event,
                 NSAppleEventDescriptor *) {
                if (auto *app = JUCEApplicationBase::getInstance())
                  app->anotherInstanceStarted(quotedIfContainsSpaces([[event
                      paramDescriptorForKeyword:keyDirectObject] stringValue]));
              });

    addMethod(@selector(broadcastMessageCallback:), [](id /*self*/, SEL,
                                                       NSNotification *n) {
      NSDictionary *dict = (NSDictionary *)[n userInfo];
      auto messageString = nsStringToJuce(
          (NSString *)[dict valueForKey:nsStringLiteral("message")]);
      MessageManager::getInstance()->deliverBroadcastMessage(messageString);
    });

    addMethod(@selector(mainMenuTrackingBegan:), [](id /*self*/, SEL,
                                                    NSNotification *) {
      NullCheckedInvocation::invoke(menuTrackingChangedCallback, true);
    });

    addMethod(@selector(mainMenuTrackingEnded:), [](id /*self*/, SEL,
                                                    NSNotification *) {
    ...
    addMethod(
        ...
        [](id self, SEL, NSNotification *notification) {
          if (notification.userInfo != nil) {
            ...
            // NSUserNotification is deprecated from macOS 11, but there doesn't
            // seem to be a replacement for
            // NSApplicationLaunchUserNotificationKey returning a non-deprecated
            // type
            ...
            if (userNotification != nil && userNotification.userInfo != nil)
              [self application:[NSApplication sharedApplication]
                  didReceiveRemoteNotification:userNotification.userInfo];
            JUCE_END_IGNORE_WARNINGS_GCC_LIKE
          }
        });

    JUCE_BEGIN_IGNORE_WARNINGS_GCC_LIKE("-Wundeclared-selector")
    addMethod(@selector(setPushNotificationsDelegate:),
              [](id self, SEL,
                 NSObject<NSApplicationDelegate,
                          NSUserNotificationCenterDelegate> *delegate) {
                object_setInstanceVariable(self, "pushNotificationsDelegate",
                                           delegate);
              });
    JUCE_END_IGNORE_WARNINGS_GCC_LIKE
    ...
    addMethod(
        ...
        [](id self, SEL, NSApplication *application, NSData *deviceToken) {
    ...
    addMethod(@selector(application:
                  didFailToRegisterForRemoteNotificationsWithError:),
              [](id self, SEL, NSApplication *application, NSError *error) {
                auto *delegate = getPushNotificationsDelegate(self);
    ...
    addMethod(
        ...
        [](id self, SEL, NSApplication *application, NSDictionary *userInfo) {
          auto *delegate = getPushNotificationsDelegate(self);
...
private:
  ...
  //==============================================================================
#if JUCE_PUSH_NOTIFICATIONS
  static NSObject<NSApplicationDelegate, NSUserNotificationCenterDelegate> *
  getPushNotificationsDelegate(id self) {
    return getIvar<
        NSObject<NSApplicationDelegate, NSUserNotificationCenterDelegate> *>(
        self, "pushNotificationsDelegate");
  }
#endif
};
...
public:
  ...
  static NSString *getBroadcastEventName() {
    return juceStringToNS(
        "juce_" + String::toHexString(
...
void MessageManager::runDispatchLoop() {
  if (quitMessagePosted.get() ==
      0) // check that the quit message wasn't already posted..
  {
    JUCE_AUTORELEASEPOOL {
      ...
      @try {
        ...
        // An AppKit exception will kill the app, but at least this provides a
        // chance to log it.,
        std::runtime_error ex(std::string("NSException: ") +
                              [[e name] UTF8String] +
...
bool MessageManager::runDispatchLoopUntil(int millisecondsToRunFor) {
  ...
  jassert(
      isThisTheMessageThread()); // must only be called by the message thread
  ...
  while (quitMessagePosted.get() == 0) {
    JUCE_AUTORELEASEPOOL {
      ...
      CFRunLoopRunInMode(kCFRunLoopDefaultMode, jmin(1.0, msRemaining * 0.001),
                         true);
      ...
      if (NSEvent *e = [NSApp
              nextEventMatchingMask:NSEventMaskAny
                          untilDate:[NSDate dateWithTimeIntervalSinceNow:0.001]
                            ...
                            dequeue:YES])
        if (isEventBlockedByModalComps == nullptr ||
            !(*isEventBlockedByModalComps)(e))
          [NSApp sendEvent:e];
    }
  }
...
void MessageManager::doPlatformSpecificShutdown() { appDelegate = nullptr; }
...
void MessageManager::broadcastMessage(const String &message) {
  NSDictionary *info =
      [NSDictionary dictionaryWithObject:juceStringToNS(message)
                                  forKey:nsStringLiteral("message")];

  [[NSDistributedNotificationCenter defaultCenter]
      postNotificationName:AppDelegate::getBroadcastEventName()
                    object:nil
                  userInfo:info];
}
