...
private:
  jobject invoke(jobject proxy, jobject method, jobjectArray args) override {
    auto *env = getEnv();
    auto methodName = juce::juceString(
        (jstring)env->CallObjectMethod(method, JavaMethod.getName));
...
struct Handler {
  Handler()
      : nativeHandler(LocalRef<jobject>(
            getEnv()->NewObject(AndroidHandler, AndroidHandler.constructor))) {}
  ~Handler() { clearSingletonInstance(); }

  JUCE_DECLARE_SINGLETON(Handler, false)

  bool post(jobject runnable) {
    return (getEnv()->CallBooleanMethod(nativeHandler.get(),
                                        AndroidHandler.post, runnable) != 0);
  }
...
void MessageManager::stopDispatchLoop() {
  struct QuitCallback final : public CallbackMessage {
    ...
    void messageCallback() override {
      ...
      if (activity != nullptr) {
        jmethodID quitMethod =
            env->GetMethodID(AndroidActivity, "finishAndRemoveTask", "()V");
...
public:
  JuceAppLifecycle(juce::JUCEApplicationBase *(*initSymbolAddr)())
    ...
    if (appContext != nullptr) {
      ...
      myself = GlobalRef(CreateJavaInterface(
          this, "android/app/Application$ActivityLifecycleCallbacks"));
  ...
  void onActivityCreated(jobject, jobject) override { checkCreated(); }
  ...
  void onActivityDestroyed(jobject activity) override {
    ...
    if (env->IsSameObject(getMainActivity().get(), activity) != 0) {
      ...
      jmethodID exitMethod =
          env->GetStaticMethodID(systemClass, "exit", "(I)V");
      env->CallStaticVoidMethod(systemClass, exitMethod, 0);
    }
  }

  void onActivityStarted(jobject) override { checkCreated(); }
  ...
  static JuceAppLifecycle &
  getInstance(juce::JUCEApplicationBase *(*initSymbolAddr)()) {
  ...
  void checkCreated() {
    if (JUCEApplicationBase::getInstance() == nullptr) {
      ...
      if (!JUCEApplicationBase::createInstance())
        jassertfalse; // you must supply an application object for an android
                      // app!
...
void juce_juceEventsAndroidStartApp() {
  ...
  auto addr = reinterpret_cast<juce::JUCEApplicationBase *(*)()>(
      DynamicLibrary(dllPath).getFunction("juce_CreateApplication"));
