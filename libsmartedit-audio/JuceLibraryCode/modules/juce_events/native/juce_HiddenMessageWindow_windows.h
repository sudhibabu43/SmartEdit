...
public:
  HiddenMessageWindow(const TCHAR *const messageWindowName, WNDPROC wndProc) {
    ...
    hwnd = CreateWindow(getClassNameFromAtom(), messageWindowName, 0, 0, 0, 0,
                        0, nullptr, nullptr, moduleHandle, nullptr);
...
private:
  ...
  LPCTSTR getClassNameFromAtom() noexcept {
    return (LPCTSTR)(pointer_sized_uint)atom;
  }
};
...
public:
  ...
  static void setAsJUCEWindow(HWND hwnd, bool isJuceWindow) noexcept {
    SetWindowLongPtr(hwnd, GWLP_USERDATA,
                     isJuceWindow ? getImprobableWindowNumber() : 0);
  }
...
public:
  ...
  void triggerAsyncDeviceChangeCallback() {
    // We'll pause before sending a message, because on device removal, the OS
    // hasn't always updated its device lists correctly at this point. This also
    // helps avoid repeated callbacks.
    startTimer(500);
  }
