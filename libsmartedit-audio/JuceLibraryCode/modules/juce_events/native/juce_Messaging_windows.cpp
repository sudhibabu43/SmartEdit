...
public:
  InternalMessageQueue() {
    messageWindow = std::make_unique<HiddenMessageWindow>(
        messageWindowName, (WNDPROC)messageWndProc);
  ...
  void broadcastMessage(const String &message) {
    ...
    for (int i = windows.size(); --i >= 0;) {
      ...
      data.cbData = (DWORD)(((size_t)localCopy.length() + 1) *
      ...
      SendMessageTimeout(windows.getUnchecked(i), WM_COPYDATA,
                         (WPARAM)juce_messageWindowHandle, (LPARAM)&data,
  ...
  bool dispatchNextMessage(bool returnIfNoPendingMessages) {
    ...
    if (GetMessage(&m, nullptr, 0, 0) >= 0) {
      ...
      if (m.message == customMessageID && m.hwnd == juce_messageWindowHandle) {
      ...
      } else if (isEventBlockedByModalComps == nullptr ||
                 !isEventBlockedByModalComps(m)) {
        if ((m.message == WM_LBUTTONDOWN || m.message == WM_RBUTTONDOWN) &&
          ...
          if (currentFocus == nullptr ||
              JuceWindowIdentifier::isJUCEWindow(currentFocus))
            SetFocus(m.hwnd);
        }
...
private:
  //==============================================================================
  static LRESULT CALLBACK messageWndProc(HWND h, UINT message, WPARAM wParam,
  ...
  static void dispatchMessage(MessageManager::MessageBase *message) {
    JUCE_TRY { message->messageCallback(); }
    ...
    if (data != nullptr && data->dwData == broadcastMessageMagicNumber) {
      struct BroadcastMessage final : public CallbackMessage {
        ...
        void messageCallback() override {
          MessageManager::getInstance()->deliverBroadcastMessage(message);
        }
      ...
      (new BroadcastMessage(
           CharPointer_UTF32((const CharPointer_UTF32::CharType *)data->lpData),
...
struct MountedVolumeListChangeDetector::Pimpl {
  explicit Pimpl(MountedVolumeListChangeDetector &d) : owner(d) {
  ...
  DeviceChangeDetector detector{L"MountedVolumeList",
