...
AsyncUpdater::~AsyncUpdater() {
  // You're deleting this object with a background thread while there's an
  // update pending on the main event thread - that's pretty dodgy threading, as
  // the callback could happen after this destructor has finished. You should
  // either use a MessageManagerLock while deleting this object, or find some
  // other way to avoid such a race condition.
