...
/**
   ...
   and creates InterprocessConnection objects for each one.

    To use this, create a class derived from it which implements the
   createConnectionObject() method, so that it creates suitable connection
   objects for each client that tries to connect.
...
public:
  ...
  */
  ...
  /** Returns the local port number to which this server is currently bound.
     ...
     your socket when calling beginWaitingForSocket with a port number of zero.
