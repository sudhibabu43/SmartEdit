...
public:
  explicit SafeActionImpl(InterprocessConnection &p) : ref(p) {}

  template <typename Fn> void ifSafe(Fn &&fn) {
...
//==============================================================================
InterprocessConnection::InterprocessConnection(bool callbacksOnMessageThread,
...
InterprocessConnection::~InterprocessConnection() {
  // You *must* call `disconnect` in the destructor of your derived class to
  // ensure that any pending messages are not delivered. If the messages were
  // delivered after destroying the derived class, we'd end up calling the pure
  // virtual implementations of `messageReceived`, `connectionMade` and
  // `connectionLost` which is definitely not a good idea!
...
bool InterprocessConnection::createPipe(const String &pipeName, int timeoutMs,
                                        bool mustNotExist) {
  disconnect();
...
bool InterprocessConnection::sendMessage(const MemoryBlock &message) {
  uint32 messageHeader[2] = {
      ByteOrder::swapIfBigEndian(magicMessageHeader),
  ...
  return writeData(messageData.getData(), (int)messageData.getSize()) ==
...
void InterprocessConnection::initialiseWithSocket(
    std::unique_ptr<StreamingSocket> newSocket) {
  jassert(socket == nullptr && pipe == nullptr);
  socket = std::move(newSocket);
  initialise();
}

void InterprocessConnection::initialiseWithPipe(
    std::unique_ptr<NamedPipe> newPipe) {
  jassert(socket == nullptr && pipe == nullptr);
...
struct ConnectionStateMessage final : public MessageManager::MessageBase {
  ConnectionStateMessage(std::shared_ptr<SafeActionImpl> ipc,
...
struct DataDeliveryMessage final : public Message {
  ...
  void messageCallback() override {
    safeAction->ifSafe(
        [this](InterprocessConnection &owner) { owner.messageReceived(data); });
  }
  ...
  if (bytes == (int)sizeof(messageHeader) &&
    ...
    if (bytesInMessage > 0) {
      ...
      while (bytesInMessage > 0) {
        ...
        auto bytesIn = readData(
            addBytesToPointer(messageData.getData(), bytesRead), numThisTime);
