...
static bool isMessageType(const MemoryBlock &mb,
...
struct ChildProcessPingThread : public Thread, private AsyncUpdater {
...
private:
  ...
  void run() override {
    while (!threadShouldExit()) {
      if (--countdown <= 0 ||
          !sendPingMessage({pingMessage, specialMessageSize})) {
...
//==============================================================================
struct ChildProcessCoordinator::Connection final
  ...
  Connection(ChildProcessCoordinator &m, const String &pipeName, int timeout)
      : InterprocessConnection(false, magicCoordWorkerConnectionHeader),
        ChildProcessPingThread(timeout), owner(m) {
...
ChildProcessCoordinator::~ChildProcessCoordinator() { killWorkerProcess(); }
...
bool ChildProcessCoordinator::launchWorkerProcess(
    const File &executable, const String &commandLineUniqueID, int timeoutMs,
    int streamFlags) {
  killWorkerProcess();
  ...
  childProcess = [&]() -> std::shared_ptr<ChildProcess> {
    if ((SystemStats::getOperatingSystemType() & SystemStats::Linux) != 0)
      return ChildProcessManager::getInstance()
          ->createAndStartManagedChildProcess(args, streamFlags);
...
struct ChildProcessWorker::Connection final : public InterprocessConnection,
  ...
  Connection(ChildProcessWorker &p, const String &pipeName, int timeout)
      : InterprocessConnection(false, magicCoordWorkerConnectionHeader),
        ChildProcessPingThread(timeout), owner(p) {
...
bool ChildProcessWorker::initialiseFromCommandLine(
    const String &commandLine, const String &commandLineUniqueID,
    int timeoutMs) {
