...
NetworkServiceDiscovery::Advertiser::Advertiser(
    ...
    : Thread("Discovery_broadcast"), message(serviceTypeUID),
      broadcastPort(broadcastPortToUse), minInterval(minTimeBetweenBroadcasts) {
...
void NetworkServiceDiscovery::Advertiser::sendBroadcast() {
  ...
  for (auto &address : IPAddress::getAllAddresses()) {
    ...
    auto data =
        message.toString(XmlElement::TextFormat().singleLine().withoutHeader());

    socket.write(broadcastAddress.toString(), broadcastPort, data.toRawUTF8(),
...
NetworkServiceDiscovery::AvailableServiceList::AvailableServiceList(
    const String &serviceType, int broadcastPort)
...
std::vector<NetworkServiceDiscovery::Service>
NetworkServiceDiscovery::AvailableServiceList::getServices() const {
...
void NetworkServiceDiscovery::AvailableServiceList::handleMessage(
...
static void
sortServiceList(std::vector<NetworkServiceDiscovery::Service> &services) {
  ...
  for (auto &s : services) {
    if (s.instanceID == service.instanceID) {
      if (s.description != service.description ||
          s.address != service.address || s.port != service.port) {
        s = service;
        triggerAsyncUpdate();
      }
...
void NetworkServiceDiscovery::AvailableServiceList::removeTimedOutServices() {
  ...
  auto oldestAllowedTime =
      Time::getCurrentTime() - RelativeTime::seconds(timeoutSeconds);

  const ScopedLock sl(listLock);

  auto oldEnd = std::end(services);
  auto newEnd =
      std::remove_if(std::begin(services), oldEnd, [=](const Service &s) {
