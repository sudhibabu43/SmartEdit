...
template <> struct OboeAudioIODeviceBufferHelpers<int16> {
  static oboe::AudioFormat oboeAudioFormat() { return oboe::AudioFormat::I16; }

  static constexpr int bitDepth() { return 16; }

  static bool
  referAudioBufferDirectlyToOboeIfPossible(int16 *, AudioBuffer<float> &, int) {
    return false;
  }
  ...
  using NativeFloat32 =
      AudioData::Format<AudioData::Float32, AudioData::NativeEndian>;

  static void convertFromOboe(const int16 *srcInterleaved,
                              AudioBuffer<float> &audioBuffer, int numSamples) {
    const auto numChannels = audioBuffer.getNumChannels();

    AudioData::deinterleaveSamples(
        AudioData::InterleavedSource<NativeInt16>{
            reinterpret_cast<const uint16 *>(srcInterleaved), numChannels},
        AudioData::NonInterleavedDest<NativeFloat32>{
            audioBuffer.getArrayOfWritePointers(), numChannels},
        numSamples);
  }

  static void convertToOboe(const AudioBuffer<float> &audioBuffer,
                            int16 *dstInterleaved, int numSamples) {
    const auto numChannels = audioBuffer.getNumChannels();

    AudioData::interleaveSamples(
        AudioData::NonInterleavedSource<NativeFloat32>{
            audioBuffer.getArrayOfReadPointers(), numChannels},
        AudioData::InterleavedDest<NativeInt16>{
            reinterpret_cast<uint16 *>(dstInterleaved), numChannels},
        numSamples);
  }
};

template <> struct OboeAudioIODeviceBufferHelpers<float> {
  static oboe::AudioFormat oboeAudioFormat() {
  ...
  static bool referAudioBufferDirectlyToOboeIfPossible(
      float *nativeBuffer, AudioBuffer<float> &audioBuffer, int numSamples) {
  ...
  static void convertFromOboe(const float *srcInterleaved,
                              AudioBuffer<float> &audioBuffer, int numSamples) {
    auto numChannels = audioBuffer.getNumChannels();

    if (numChannels > 0) {
      // No need to convert, we instructed the buffer to point to the src data
      // directly already
      jassert(audioBuffer.getWritePointer(0) != srcInterleaved);

      AudioData::deinterleaveSamples(
          AudioData::InterleavedSource<Format>{srcInterleaved, numChannels},
  ...
  static void convertToOboe(const AudioBuffer<float> &audioBuffer,
                            float *dstInterleaved, int numSamples) {
    auto numChannels = audioBuffer.getNumChannels();
...
template <typename Type> static String getOboeString(const Type &value) {
...
public:
  //==============================================================================
  OboeAudioIODevice(const String &deviceName, int inputDeviceIdToUse,
                    const Array<int> &supportedInputSampleRatesToUse,
                    int maxNumInputChannelsToUse, int outputDeviceIdToUse,
  ...
  ~OboeAudioIODevice() override { close(); }
  ...
  Array<double> getAvailableSampleRates() override {
    ...
    if (inputDeviceId == -1) {
      ...
      // For best performance, the same sample rate should be used for input and
      // output,
    ...
    // either invalid device was requested or its input&output don't have
  ...
  Array<int> getAvailableBufferSizes() override {
    return AndroidHighPerformanceAudioHelpers::getAvailableBufferSizes(
        getNativeBufferSize(), getAvailableSampleRates());
  }
  ...
  String open(const BigInteger &inputChannels, const BigInteger &outputChannels,
    ...
    sampleRate =
        (int)(requestedSampleRate > 0
                  ? requestedSampleRate
                  : AndroidHighPerformanceAudioHelpers::getNativeSampleRate());
    actualBufferSize = (bufferSize <= 0) ? getDefaultBufferSize() : bufferSize;

    // The device may report no max, claiming "no limits". Pick sensible
    // defaults.
    ...
    activeOutputChans.setRange(
        maxOutChans, activeOutputChans.getHighestBit() + 1 - maxOutChans,
        false);

    activeInputChans = inputChannels;
    activeInputChans.setRange(
        maxInChans, activeInputChans.getHighestBit() + 1 - maxInChans, false);
    ...
    if (numInputChans > 0 &&
      ...
      // If you hit this assert, you probably forgot to get
      // RuntimePermissions::recordAudio before trying to open an audio input
      // device. This is not going to work!
      jassertfalse;
      lastError = "Error opening Oboe input device: the app was not granted "
    ...
    session.reset(OboeSessionBase::create(*this, inputDeviceId, outputDeviceId,
  ...
  int getDefaultBufferSize() override {
    return AndroidHighPerformanceAudioHelpers::getDefaultBufferSize(
        getNativeBufferSize(), getCurrentSampleRate());
  }
...
private:
  ...
  Array<int> getAvailableSampleRates(bool forInput) {
    auto &supportedSampleRates =
        forInput ? supportedInputSampleRates : supportedOutputSampleRates;
  ...
  static Array<int> getDefaultSampleRates() {
    static const int standardRates[] = {8000,  11025, 12000, 16000, 22050,
  ...
  static int getNativeBufferSize() {
    ...
    // providing a callback is required on some devices to get a FAST track, so
    // we pass an empty one to the temp stream to get the best available buffer
    // size
    struct DummyCallback final : public oboe::AudioStreamCallback {
      oboe::DataCallbackResult onAudioReady(oboe::AudioStream *, void *,
                                            int32_t) override {
    ...
    // NB: Exclusive mode could be rejected if a device is already opened in
    // that mode, so to get
    ...
    //     android.media.property.OUTPUT_FRAMES_PER_BUFFER property, but then we
    //     verify the actual size after the stream is open.
    OboeAudioIODevice::OboeStream tempStream(
        oboe::kUnspecified, oboe::Direction::Output,
        oboe::SharingMode::Exclusive, 2,
        getAndroidSDKVersion() >= 21 ? oboe::AudioFormat::Float
                                     : oboe::AudioFormat::I16,
        (int)AndroidHighPerformanceAudioHelpers::getNativeSampleRate(),
        bufferSizeHint, &callback);
  ...
  void process(const float *const *inputChannelData, int numInputChannels,
    ...
    if (auto *cb = callback.exchange(nullptr)) {
      cb->audioDeviceIOCallbackWithContext(inputChannelData, numInputChannels,
  ...
  public:
    OboeStream(int deviceId, oboe::Direction direction,
               oboe::SharingMode sharingMode, int channelCount,
               oboe::AudioFormat format, int32 sampleRateIn, int32 bufferSize,
               oboe::AudioStreamCallback *callbackIn = nullptr) {
      open(deviceId, direction, sharingMode, channelCount, format, sampleRateIn,
           bufferSize, callbackIn);
    }
    ...
    void start() {
      ...
      if (openedOk() && stream != nullptr) {
        ...
        JUCE_OBOE_LOG("Requested Oboe stream start with result: " +
                      getOboeString(startResult));

        startResult =
            stream->waitForStateChange(expectedState, &nextState, timeoutNanos);
        ...
        JUCE_OBOE_LOG(
            ...
            "\nBufferSizeInFrames = " +
            String(stream->getBufferSizeInFrames()) +
            "\nBufferCapacityInFrames = " +
            String(stream->getBufferCapacityInFrames()) +
            ...
            "\nPerformanceMode = " +
            getOboeString(stream->getPerformanceMode()) +
    ...
    int getXRunCount() const {
      if (stream != nullptr) {
        ...
        JUCE_OBOE_LOG("Failed to get Xrun count: " +
                      getOboeString(count.error()));
      }
  ...
  private:
    void open(int deviceId, oboe::Direction direction,
              oboe::SharingMode sharingMode, int channelCount,
              oboe::AudioFormat format, int32 newSampleRate,
              int32 newBufferSize,
      ...
      if (newCallback != nullptr) {
        stabilizedCallback =
            std::make_unique<oboe::StabilizedCallback>(newCallback);
      ...
      JUCE_OBOE_LOG(
          String("Preparing Oboe stream with params:") +
          ...
          getOboeString(oboe::PerformanceMode::LowLatency));

      openResult = builder.openStream(&stream);
      JUCE_OBOE_LOG("Building Oboe stream with result: " +
                    getOboeString(openResult) + "\nStream state = " +
                    (stream != nullptr ? getOboeString(stream->getState())
                                       : String("?")));

      if (stream != nullptr && newBufferSize != 0) {
        JUCE_OBOE_LOG("Setting the bufferSizeInFrames to " +
                      String(newBufferSize));
        stream->setBufferSizeInFrames(newBufferSize);
      }

      JUCE_OBOE_LOG(
          String("Stream details:") + "\nUses AAudio = " +
          (stream != nullptr ? String((int)stream->usesAAudio())
                             : String("?")) +
          "\nDeviceId = " +
          (stream != nullptr ? String(stream->getDeviceId()) : String("?")) +
          "\nDirection = " +
          ...
          (stream != nullptr ? getOboeString(stream->getSharingMode())
                             : String("?")) +
          "\nChannelCount = " +
          (stream != nullptr ? String(stream->getChannelCount())
                             : String("?")) +
          "\nFormat = " +
          (stream != nullptr ? getOboeString(stream->getFormat())
                             : String("?")) +
          "\nSampleRate = " +
          (stream != nullptr ? String(stream->getSampleRate()) : String("?")) +
          "\nBufferSizeInFrames = " +
          (stream != nullptr ? String(stream->getBufferSizeInFrames())
                             : String("?")) +
          "\nBufferCapacityInFrames = " +
          (stream != nullptr ? String(stream->getBufferCapacityInFrames())
                             : String("?")) +
          "\nFramesPerBurst = " +
          (stream != nullptr ? String(stream->getFramesPerBurst())
                             : String("?")) +
          "\nFramesPerCallback = " +
          (stream != nullptr ? String(stream->getFramesPerCallback())
                             : String("?")) +
          "\nBytesPerFrame = " +
          (stream != nullptr ? String(stream->getBytesPerFrame())
                             : String("?")) +
          "\nBytesPerSample = " +
          (stream != nullptr ? String(stream->getBytesPerSample())
                             : String("?")) +
          "\nPerformanceMode = " +
    ...
    void close() {
      if (stream != nullptr) {
        ...
        JUCE_OBOE_LOG("Requested Oboe stream close with result: " +
                      getOboeString(result));
      }
    }
  ...
  public:
    static OboeSessionBase *create(OboeAudioIODevice &owner, int inputDeviceId,
                                   int outputDeviceId, int numInputChannels,
                                   int numOutputChannels, int sampleRate,
                                   int bufferSize);
    ...
    int getXRunCount() const {
      int inputXRunCount =
          jmax(0, inputStream != nullptr ? inputStream->getXRunCount() : 0);
      int outputXRunCount =
          jmax(0, outputStream != nullptr ? outputStream->getXRunCount() : 0);

      return inputXRunCount + outputXRunCount;
    }

  protected:
    OboeSessionBase(OboeAudioIODevice &ownerToUse, int inputDeviceIdToUse,
                    int outputDeviceIdToUse, int numInputChannelsToUse,
                    int numOutputChannelsToUse, int sampleRateToUse,
                    int bufferSizeToUse, oboe::AudioFormat streamFormatToUse,
                    int bitDepthToUse)
        : owner(ownerToUse), inputDeviceId(inputDeviceIdToUse),
          outputDeviceId(outputDeviceIdToUse),
          numInputChannels(numInputChannelsToUse),
          numOutputChannels(numOutputChannelsToUse),
          sampleRate(sampleRateToUse), bufferSize(bufferSizeToUse),
          streamFormat(streamFormatToUse), bitDepth(bitDepthToUse),
          outputStream(new OboeStream(outputDeviceId, oboe::Direction::Output,
                                      oboe::SharingMode::Exclusive,
                                      numOutputChannels, streamFormatToUse,
                                      sampleRateToUse, bufferSizeToUse, this)) {
      if (numInputChannels > 0) {
        inputStream.reset(new OboeStream(
            inputDeviceId, oboe::Direction::Input, oboe::SharingMode::Exclusive,
            numInputChannels, streamFormatToUse, sampleRateToUse,
            bufferSizeToUse, nullptr));
    ...
    // Not strictly required as these should not change, but recommended by
    // Google anyway
    void checkStreamSetup(OboeStream *stream, [[maybe_unused]] int deviceId,
                          [[maybe_unused]] int numChannels,
                          [[maybe_unused]] int expectedSampleRate,
                          [[maybe_unused]] int expectedBufferSize,
                          oboe::AudioFormat format) {
      if ([[maybe_unused]] auto *nativeStream =
              stream != nullptr ? stream->getNativeStream() : nullptr) {
        jassert(numChannels == 0 ||
                numChannels == nativeStream->getChannelCount());
        jassert(expectedSampleRate == 0 ||
  ...
  public:
    OboeSessionImpl(OboeAudioIODevice &ownerToUse, int inputDeviceIdIn,
                    int outputDeviceIdIn, int numInputChannelsToUse,
                    int numOutputChannelsToUse, int sampleRateToUse,
                    int bufferSizeToUse)
        : OboeSessionBase(
              ownerToUse, inputDeviceIdIn, outputDeviceIdIn,
              numInputChannelsToUse, numOutputChannelsToUse, sampleRateToUse,
              bufferSizeToUse,
              OboeAudioIODeviceBufferHelpers<SampleType>::oboeAudioFormat(),
              OboeAudioIODeviceBufferHelpers<SampleType>::bitDepth()),
          inputStreamNativeBuffer(static_cast<size_t>(
              numInputChannelsToUse * getBufferCapacityInFrames(true))),
          inputStreamSampleBuffer(numInputChannels,
                                  getBufferCapacityInFrames(true)),
          outputStreamSampleBuffer(numOutputChannels,
                                   getBufferCapacityInFrames(false)) {}
    ...
    void start() override {
      ...
      isInputLatencyDetectionSupported =
          isLatencyDetectionSupported(inputStream.get());
      isOutputLatencyDetectionSupported =
          isLatencyDetectionSupported(outputStream.get());
    }
  ...
  private:
    bool isLatencyDetectionSupported(OboeStream *stream) {
      ...
      auto result = stream->getNativeStream()->getTimestamp(CLOCK_MONOTONIC,
                                                            nullptr, nullptr);
    ...
    oboe::DataCallbackResult onAudioReady(oboe::AudioStream *stream,
      ...
      if (lock.isLocked()) {
        ...
        jassert(stream->getDirection() == oboe::Direction::Output &&
                stream == outputStream->getNativeStream());

        // Read input from Oboe
        const auto expandedBufferSize =
            jmax(inputStreamNativeBuffer.size(),
                 static_cast<size_t>(numInputChannels *
                                     jmax(bufferSize, numFrames)));
        inputStreamNativeBuffer.resize(expandedBufferSize);

        if (inputStream != nullptr) {
          auto *nativeInputStream = inputStream->getNativeStream();

          if (nativeInputStream->getFormat() != oboe::AudioFormat::I16 &&
          ...
          auto result = inputStream->getNativeStream()->read(
              inputStreamNativeBuffer.data(), numFrames, 0);

          if (result) {
            auto referringDirectlyToOboeData =
                OboeAudioIODeviceBufferHelpers<SampleType>::
                    referAudioBufferDirectlyToOboeIfPossible(
                        inputStreamNativeBuffer.data(), inputStreamSampleBuffer,
                        result.value());
            ...
            JUCE_OBOE_LOG("Failed to read from input stream: " +
                          getOboeString(result.error()));
          }
        ...
        auto referringDirectlyToOboeData =
            OboeAudioIODeviceBufferHelpers<SampleType>::
                referAudioBufferDirectlyToOboeIfPossible(
                    ...
                    outputStreamSampleBuffer, numFrames);

        if (!referringDirectlyToOboeData)
          outputStreamSampleBuffer.clear();

        // Process
        // NB: the number of samples read from the input can potentially differ
        // from numFrames.
        owner.process(inputStreamSampleBuffer.getArrayOfReadPointers(),
                      numInputChannels,
                      outputStreamSampleBuffer.getArrayOfWritePointers(),
                      numOutputChannels, numFrames);

        // Write output to Oboe
        if (!referringDirectlyToOboeData)
          OboeAudioIODeviceBufferHelpers<SampleType>::convertToOboe(
              outputStreamSampleBuffer, static_cast<SampleType *>(audioData),
              numFrames);
    ...
    int getLatencyFor(OboeStream &stream) {
      ...
      auto result = nativeStream.getTimestamp(
          CLOCK_MONOTONIC, &hardwareFrameIndex, &hardwareFrameHardwareTime);
      ...
      const bool isOutput =
          nativeStream.getDirection() == oboe::Direction::Output;
      const int64_t appFrameIndex = isOutput ? nativeStream.getFramesWritten()
                                             : nativeStream.getFramesRead();
      ...
      int64_t frameTimeDelta =
          (frameIndexDelta * oboe::kNanosPerSecond) / sampleRate;
      int64_t appFrameHardwareTime = hardwareFrameHardwareTime + frameTimeDelta;

      // Calculate latency as a difference in time between when the current
      // frame is at the app and when it is at the hardware.
      auto latencyNanos = isOutput ? (appFrameHardwareTime - appFrameAppTime)
                                   : (appFrameAppTime - appFrameHardwareTime);
      return static_cast<int>((latencyNanos * sampleRate) /
    ...
    void onErrorBeforeClose(oboe::AudioStream *stream,
                            [[maybe_unused]] oboe::Result error) override {
      // only output stream should be the master stream receiving callbacks
      jassert(stream->getDirection() == oboe::Direction::Output);

      JUCE_OBOE_LOG("Oboe stream onErrorBeforeClose(): " +
                    getOboeString(error));
    ...
    void onErrorAfterClose(oboe::AudioStream *stream,
      ...
      if (error == oboe::Result::ErrorDisconnected) {
        ...
        if (streamRestartLock.isLocked()) {
          ...
          // Use default device id, to let the OS pick the best ID (since our
          // was disconnected).
          ...
          outputStream = nullptr;
          outputStream.reset(
              new OboeStream(oboe::kUnspecified, oboe::Direction::Output,
                             oboe::SharingMode::Exclusive, numOutputChannels,
                             streamFormat, sampleRate, bufferSize, this));
    ...
    AudioBuffer<float> inputStreamSampleBuffer, outputStreamSampleBuffer;
...
//==============================================================================
OboeAudioIODevice::OboeSessionBase *OboeAudioIODevice::OboeSessionBase::create(
    OboeAudioIODevice &owner, int inputDeviceId, int outputDeviceId,
  ...
  if (sdkVersion >= 21) {
    session.reset(new OboeSessionImpl<float>(
        owner, inputDeviceId, outputDeviceId, numInputChannels,
  ...
  if (session == nullptr) {
    session.reset(new OboeSessionImpl<int16>(
        owner, inputDeviceId, outputDeviceId, numInputChannels,
        numOutputChannels, sampleRate, bufferSize));
...
public:
  OboeAudioIODeviceType() : AudioIODeviceType(OboeAudioIODevice::oboeTypeName) {
    // Not using scanForDevices() to maintain behaviour backwards compatible
    // with older APIs
  ...
  int getIndexOfDevice(AudioIODevice *device, bool asInput) const override {
    if (auto oboeDevice = static_cast<OboeAudioIODevice *>(device)) {
      auto oboeDeviceId =
          asInput ? oboeDevice->inputDeviceId : oboeDevice->outputDeviceId;
  ...
  AudioIODevice *createDevice(const String &outputDeviceName,
    ...
    return new OboeAudioIODevice(
        name, inputDeviceInfo.id, inputDeviceInfo.sampleRates,
        inputDeviceInfo.numChannels, outputDeviceInfo.id,
        outputDeviceInfo.sampleRates, outputDeviceInfo.numChannels);
  }
...
private:
  void checkAvailableDevices() {
    ...
    inputDevices.add(
        {"System Default (Input)", oboe::kUnspecified, sampleRates, 1});
    outputDevices.add(
        {"System Default (Output)", oboe::kUnspecified, sampleRates, 2});
    ...
    auto audioManager = LocalRef<jobject>(env->CallObjectMethod(
    ...
    auto devices = LocalRef<jobjectArray>((jobjectArray)env->CallObjectMethod(
        audioManager, getDevicesMethod, allDevices));

    const int numDevices = env->GetArrayLength(devices.get());

    for (int i = 0; i < numDevices; ++i) {
      auto device = LocalRef<jobject>(
          (jobject)env->GetObjectArrayElement(devices.get(), i));
    ...
    for ([[maybe_unused]] auto &device : inputDevices) {
      ...
      JUCE_OBOE_LOG(
          "sample rates size = " << String(device.sampleRates.size()));
  ...
  void addDevice(const LocalRef<jobject> &device, JNIEnv *env) {
    auto deviceClass = LocalRef<jclass>(
        (jclass)env->FindClass("android/media/AudioDeviceInfo"));

    jmethodID getProductNameMethod = env->GetMethodID(
        deviceClass, "getProductName", "()Ljava/lang/CharSequence;");

    jmethodID getTypeMethod = env->GetMethodID(deviceClass, "getType", "()I");
    jmethodID getIdMethod = env->GetMethodID(deviceClass, "getId", "()I");
    jmethodID getSampleRatesMethod =
        env->GetMethodID(deviceClass, "getSampleRates", "()[I");
    jmethodID getChannelCountsMethod =
        env->GetMethodID(deviceClass, "getChannelCounts", "()[I");
    jmethodID isSourceMethod = env->GetMethodID(deviceClass, "isSource", "()Z");

    auto deviceTypeString =
        deviceTypeToString(env->CallIntMethod(device, getTypeMethod));

    if (deviceTypeString.isEmpty()) // unknown device
      return;

    auto name = juceString((jstring)env->CallObjectMethod(
                    device, getProductNameMethod)) +
                " " + deviceTypeString;
    auto id = env->CallIntMethod(device, getIdMethod);

    auto jSampleRates = LocalRef<jintArray>(
        (jintArray)env->CallObjectMethod(device, getSampleRatesMethod));
    auto sampleRates = jintArrayToJuceArray(jSampleRates);

    auto jChannelCounts = LocalRef<jintArray>(
        (jintArray)env->CallObjectMethod(device, getChannelCountsMethod));
...
public:
  OboeRealtimeThread()
      : testStream(new OboeStream(
            oboe::kUnspecified, oboe::Direction::Output,
            oboe::SharingMode::Exclusive, 1, oboe::AudioFormat::Float,
            (int)AndroidHighPerformanceAudioHelpers::getNativeSampleRate(),
            OboeAudioIODevice::getNativeBufferSize(), this)),
        formatUsed(oboe::AudioFormat::Float) {
    // Fallback to I16 stream format if Float has not worked
    if (!testStream->openedOk()) {
      testStream.reset(new OboeStream(
          oboe::kUnspecified, oboe::Direction::Output,
          oboe::SharingMode::Exclusive, 1, oboe::AudioFormat::I16,
          (int)AndroidHighPerformanceAudioHelpers::getNativeSampleRate(),
          OboeAudioIODevice::getNativeBufferSize(), this));
  ...
  bool isOk() const { return testStream != nullptr && testStream->openedOk(); }
  ...
  oboe::DataCallbackResult onAudioReady(oboe::AudioStream *, void *,
                                        int32_t) override {
    // When running with OpenSL, the first callback will come on the parent
    // thread.
    if (threadEntryProc != nullptr &&
        !pthread_equal(parentThreadID, pthread_self())) {
      pthread_mutex_lock(&threadReadyMutex);
  ...
  void onErrorBeforeClose(oboe::AudioStream *,
                          [[maybe_unused]] oboe::Result error) override {
    JUCE_OBOE_LOG("OboeRealtimeThread: Oboe stream onErrorBeforeClose(): " +
                  getOboeString(error));
    jassertfalse; // Should never get here!
  }

  void onErrorAfterClose(oboe::AudioStream *,
                         [[maybe_unused]] oboe::Result error) override {
    JUCE_OBOE_LOG("OboeRealtimeThread: Oboe stream onErrorAfterClose(): " +
                  getOboeString(error));
    jassertfalse; // Should never get here!
  }
