...
static void logFailure([[maybe_unused]] HRESULT hr) {
  jassert(hr != (HRESULT)0x800401f0); // If you hit this, it means you're trying
                                      // to call from a thread which hasn't been
                                      // initialised with CoInitialize().
...
#define JUCE_WASAPI_ERR(desc, n)                                               \
  case MAKE_HRESULT(1, 0x889, n):                                              \
    ...
    Logger::writeToLog("WASAPI error: " + (m != nullptr
                                               ? String(m)
...
JUCE_IUNKNOWNCLASS(IMMDeviceEnumerator,
  ...
  JUCE_COMCALL RegisterEndpointNotificationCallback(IMMNotificationClient *) =
      0;
  JUCE_COMCALL UnregisterEndpointNotificationCallback(IMMNotificationClient *) =
      0;
};
...
JUCE_IUNKNOWNCLASS(IAudioClient, "1CB9AD4C-DBFA-4c32-B178-C2F568A703B2") {
  JUCE_COMCALL Initialize(AUDCLNT_SHAREMODE, DWORD, REFERENCE_TIME,
  ...
  JUCE_COMCALL IsFormatSupported(AUDCLNT_SHAREMODE, const WAVEFORMATEX *,
                                 WAVEFORMATEX **) = 0;
...
JUCE_COMCLASS(IAudioClient2, "726778CD-F60A-4eda-82DE-E47610CD78AA")
  ...
  JUCE_COMCALL GetBufferSizeLimits(const WAVEFORMATEX *, BOOL, REFERENCE_TIME *,
                                   REFERENCE_TIME *) = 0;
};

JUCE_COMCLASS(IAudioClient3, "1CB9AD4C-DBFA-4c32-B178-C2F568A703B2")
    : public IAudioClient2 {
  JUCE_COMCALL GetSharedModeEnginePeriod(const WAVEFORMATEX *, UINT32 *,
                                         UINT32 *, UINT32 *, UINT32 *) = 0;
  JUCE_COMCALL GetCurrentSharedModeEnginePeriod(WAVEFORMATEX **, UINT32 *) = 0;
  JUCE_COMCALL InitializeSharedAudioStream(DWORD, UINT32, const WAVEFORMATEX *,
                                           LPCGUID) = 0;
};
...
__CRT_UUID_DECL(juce::IPropertyStore, 0x886d8eeb, 0x8cf2, 0x4446, 0x8d, 0x02,
                0xcd, 0xba, 0x1d, 0xbd, 0xcf, 0x99)
__CRT_UUID_DECL(juce::IMMDevice, 0xD666063F, 0x1587, 0x4E43, 0x81, 0xF1, 0xB9,
                0x48, 0xE8, 0x07, 0x36, 0x3F)
__CRT_UUID_DECL(juce::IMMEndpoint, 0x1BE09788, 0x6894, 0x4089, 0x85, 0x86, 0x9A,
                0x2A, 0x6C, 0x26, 0x5A, 0xC5)
__CRT_UUID_DECL(juce::IMMNotificationClient, 0x7991EEC9, 0x7E89, 0x4D85, 0x83,
                0x90, 0x6C, 0x70, 0x3C, 0xEC, 0x60, 0xC0)
__CRT_UUID_DECL(juce::IMMDeviceEnumerator, 0xA95664D2, 0x9614, 0x4F35, 0xA7,
                0x46, 0xDE, 0x8D, 0xB6, 0x36, 0x17, 0xE6)
__CRT_UUID_DECL(juce::MMDeviceEnumerator, 0xBCDE0395, 0xE52F, 0x467C, 0x8E,
                0x3D, 0xC4, 0x57, 0x92, 0x91, 0x69, 0x2E)
__CRT_UUID_DECL(juce::IAudioClient, 0x1CB9AD4C, 0xDBFA, 0x4c32, 0xB1, 0x78,
                0xC2, 0xF5, 0x68, 0xA7, 0x03, 0xB2)
__CRT_UUID_DECL(juce::IAudioClient2, 0x726778CD, 0xF60A, 0x4eda, 0x82, 0xDE,
                0xE4, 0x76, 0x10, 0xCD, 0x78, 0xAA)
__CRT_UUID_DECL(juce::IAudioClient3, 0x1CB9AD4C, 0xDBFA, 0x4c32, 0xB1, 0x78,
                0xC2, 0xF5, 0x68, 0xA7, 0x03, 0xB2)
__CRT_UUID_DECL(juce::IAudioCaptureClient, 0xC8ADBD64, 0xE71E, 0x48a0, 0xA4,
                0xDE, 0x18, 0x5C, 0x39, 0x5C, 0xD3, 0x17)
__CRT_UUID_DECL(juce::IAudioRenderClient, 0xF294ACFC, 0x3146, 0x4483, 0xA7,
                0xBF, 0xAD, 0xDC, 0xA7, 0xC2, 0x60, 0xE2)
__CRT_UUID_DECL(juce::IAudioEndpointVolume, 0x5CDF2C82, 0x841E, 0x4546, 0x97,
                0x22, 0x0C, 0xF7, 0x40, 0x78, 0x22, 0x9A)
__CRT_UUID_DECL(juce::IAudioSessionEvents, 0x24918ACC, 0x64B3, 0x37C1, 0x8C,
                0xA9, 0x74, 0xA6, 0x6E, 0x99, 0x57, 0xA8)
__CRT_UUID_DECL(juce::IAudioSessionControl, 0xF4B1A599, 0x7266, 0x4319, 0xA8,
                0xCA, 0xE7, 0x0A, 0xCB, 0x11, 0xE8, 0xCD)
#endif
...
static int refTimeToSamples(const REFERENCE_TIME &t,
                            double sampleRate) noexcept {
  return roundToInt(sampleRate * ((double)t) * 0.0000001);
}

static REFERENCE_TIME samplesToRefTime(int numSamples,
                                       double sampleRate) noexcept {
  return (REFERENCE_TIME)((numSamples * 10000.0 * 1000.0 / sampleRate) + 0.5);
}

static void copyWavFormat(WAVEFORMATEXTENSIBLE &dest,
                          const WAVEFORMATEX *src) noexcept {
  memcpy(&dest, src,
         src->wFormatTag == WAVE_FORMAT_EXTENSIBLE
...
public:
  WASAPIDeviceBase(const ComSmartPtr<IMMDevice> &d, WASAPIDeviceMode mode)
    ...
    if (isExclusiveMode(deviceMode))
      if (auto optFormat = findSupportedFormat(tempClient, defaultNumChannels,
                                               defaultSampleRate))
        format = optFormat;
  ...
  bool openClient(const double newSampleRate, const BigInteger &newChannels,
    ...
    channels.setRange(maxNumChannels,
                      channels.getHighestBit() + 1 - maxNumChannels, false);
  ...
  void closeClient() {
    ...
    // N.B. this is needed to prevent a double-deletion of the
    // IAudioSessionEvents object on older versions of Windows
    Thread::sleep(5);
  ...
  void deviceSampleRateChanged() { sampleRateHasChanged = true; }

  void deviceSessionBecameInactive() { isActive = false; }

  void deviceSessionExpired() { shouldShutdown = true; }

  void deviceSessionBecameActive() { isActive = true; }

  std::optional<BigInteger> getDefaultLayout() const {
    if (countNumberOfBits((uint64)defaultFormatChannelMask) ==
        defaultNumChannels)
  ...
  int numChannels = 0, actualNumChannels = 0, maxNumChannels = 0,
  ...
  std::atomic<bool> sampleRateHasChanged{false}, shouldShutdown{false},
      isActive{true};
...
private:
  //==============================================================================
  struct SessionEventCallback final
      : public ComBaseClassHelper<IAudioSessionEvents> {
    SessionEventCallback(WASAPIDeviceBase &d) : owner(d) {}

    JUCE_COMRESULT OnDisplayNameChanged(LPCWSTR, LPCGUID) override {
      return S_OK;
    }
    JUCE_COMRESULT OnIconPathChanged(LPCWSTR, LPCGUID) override { return S_OK; }
    JUCE_COMRESULT OnSimpleVolumeChanged(float, BOOL, LPCGUID) override {
      return S_OK;
    }
    JUCE_COMRESULT OnChannelVolumeChanged(DWORD, float *, DWORD,
                                          LPCGUID) override {
      return S_OK;
    }
    JUCE_COMRESULT OnGroupingParamChanged(LPCGUID, LPCGUID) override {
      return S_OK;
    }
  ...
  void createSessionEventCallback() {
    ...
    if (audioSessionControl != nullptr) {
      ...
      sessionEventCallback
          ->Release(); // (required because ComBaseClassHelper objects are
                       // constructed with a ref count of 1)
    }
  }
  ...
  ComSmartPtr<IAudioClient> createClient() {
    ...
    if (device != nullptr)
      logFailure(device->Activate(
          __uuidof(IAudioClient), CLSCTX_INPROC_SERVER, nullptr,
          (void **)newClient.resetAndGetPointerAddress()));

    return newClient;
  }

  static std::optional<WAVEFORMATEXTENSIBLE>
  getClientMixFormat(ComSmartPtr<IAudioClient> &client) {
  ...
  //==============================================================================
  void querySupportedBufferSizes(WAVEFORMATEXTENSIBLE format,
                                 ComSmartPtr<IAudioClient> &audioClient) {
    if (isLowLatencyMode(deviceMode)) {
      if (auto audioClient3 = audioClient.getInterface<IAudioClient3>()) {
        UINT32 defaultPeriod = 0, fundamentalPeriod = 0, minPeriod = 0,
               maxPeriod = 0;

        if (check(audioClient3->GetSharedModeEnginePeriod(
                (WAVEFORMATEX *)&format, &defaultPeriod, &fundamentalPeriod,
                &minPeriod, &maxPeriod))) {
  ...
  void querySupportedSampleRates(WAVEFORMATEXTENSIBLE format,
                                 ComSmartPtr<IAudioClient> &audioClient) {
    for (auto rate : SampleRateHelpers::getAllSampleRates()) {
      ...
      if (SUCCEEDED(audioClient->IsFormatSupported(
              isExclusiveMode(deviceMode) ? AUDCLNT_SHAREMODE_EXCLUSIVE
                                          : AUDCLNT_SHAREMODE_SHARED,
              (WAVEFORMATEX *)&format,
              isExclusiveMode(deviceMode) ? nullptr : &nearestFormat))) {
  ...
  static constexpr AudioSampleFormat formatsToTry[] = {
      {true, 32, 4},  {false, 32, 4}, {false, 24, 4}, {false, 24, 3},
      {false, 20, 4}, {false, 20, 3}, {false, 16, 2}};

  static std::optional<WAVEFORMATEXTENSIBLE>
  tryFormat(const AudioSampleFormat sampleFormat, IAudioClient *clientToUse,
    ...
    format.Format.wBitsPerSample =
        (WORD)(8 * sampleFormat.bytesPerSampleContainer);
    format.Samples.wValidBitsPerSample =
        (WORD)(sampleFormat.bitsPerSampleToTry);
    ...
    format.SubFormat = sampleFormat.useFloat ? KSDATAFORMAT_SUBTYPE_IEEE_FLOAT
    ...
    HRESULT hr = clientToUse->IsFormatSupported(
        isExclusiveMode(mode) ? AUDCLNT_SHAREMODE_EXCLUSIVE
                              : AUDCLNT_SHAREMODE_SHARED,
        (WAVEFORMATEX *)&format,
        isExclusiveMode(mode) ? nullptr : &nearestFormat);
    logFailure(hr);

    auto supportsSRC = supportsSampleRateConversion(mode);

    if (hr == S_FALSE && nearestFormat != nullptr &&
  ...
  findSupportedFormat(IAudioClient *clientToUse, int newNumChannels,
                      double newSampleRate) const {
    for (auto ch = newNumChannels; ch <= maxNumChannels; ++ch) {
      auto maskWithLowestNBitsSet = static_cast<DWORD>((1 << ch) - 1);
      auto mixFormatChannelMask =
          (ch == defaultNumChannels ? defaultFormatChannelMask
                                    : maskWithLowestNBitsSet);

      for (auto const &sampleFormat : formatsToTry)
        if (auto format = tryFormat(sampleFormat, clientToUse, deviceMode, ch,
                                    newSampleRate, mixFormatChannelMask))
          return format;
    }
  ...
  int queryMaxNumChannels(IAudioClient *clientToUse) const {
    static constexpr auto maxNumChannelsToQuery =
        static_cast<int>(AudioChannelSet::maxChannelsOfNamedLayout);
  ...
  bool initialiseLowLatencyClient(int bufferSizeSamples,
                                  WAVEFORMATEXTENSIBLE format) {
    if (auto audioClient3 = client.getInterface<IAudioClient3>())
      return check(audioClient3->InitializeSharedAudioStream(
          getStreamFlags(), (UINT32)bufferSizeSamples, (WAVEFORMATEX *)&format,
          nullptr));

    return false;
  }

  bool initialiseStandardClient(int bufferSizeSamples,
                                WAVEFORMATEXTENSIBLE format) {
    ...
    if (isExclusiveMode(deviceMode) && bufferSizeSamples > 0)
      defaultPeriod =
          jmax(minPeriod, samplesToRefTime(bufferSizeSamples,
                                           format.Format.nSamplesPerSec));

    for (;;) {
      GUID session;
      auto hr = client->Initialize(
          isExclusiveMode(deviceMode) ? AUDCLNT_SHAREMODE_EXCLUSIVE
                                      : AUDCLNT_SHAREMODE_SHARED,
          getStreamFlags(), defaultPeriod,
          isExclusiveMode(deviceMode) ? defaultPeriod : 0,
          (WAVEFORMATEX *)&format, &session);
      ...
      // (see Remarks)
      if (hr !=
          MAKE_HRESULT(1, 0x889, 0x19)) // AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED
        break;
      ...
      defaultPeriod =
          samplesToRefTime((int)numFrames, format.Format.nSamplesPerSec);
    }
  ...
  bool tryInitialisingWithBufferSize(int bufferSizeSamples) {
    if (auto format = findSupportedFormat(client, numChannels, sampleRate)) {
      auto isInitialised =
          isLowLatencyMode(deviceMode)
              ? initialiseLowLatencyClient(bufferSizeSamples, *format)
...
public:
  ...
  ~WASAPIInputDevice() override { close(); }

  bool open(double newSampleRate, const BigInteger &newChannels,
            int bufferSizeSamples) {
    return openClient(newSampleRate, newChannels, bufferSizeSamples) &&
           (numChannels == 0 ||
            check(client->GetService(
  ...
  template <class SourceType> void updateFormatWithType(SourceType *) noexcept {
    using NativeType =
    ...
    converter.reset(
        new AudioData::ConverterInstance<
            AudioData::Pointer<SourceType, AudioData::LittleEndian,
                               AudioData::Interleaved, AudioData::Const>,
            NativeType>(actualNumChannels, 1));
  }
  ...
  bool start(int userBufferSizeIn) {
    const auto reservoirSize =
        nextPowerOfTwo((int)(actualBufferSize + (UINT32)userBufferSizeIn));
  ...
  void purgeInputBuffers() {
    ...
    while (captureClient->GetBuffer(&inputData, &numSamplesAvailable, &flags,
                                    nullptr, nullptr) !=
  ...
  void handleDeviceBuffer() {
    ...
    while (check(captureClient->GetBuffer(&inputData, &numSamplesAvailable,
      ...
      for (const auto &block : queue.write((int)numSamplesAvailable)) {
        ...
        auto *reservoirPtr = addBytesToPointer(
            reservoir.getData(), block.getStart() * bytesPerFrame);

        if ((flags & AUDCLNT_BUFFERFLAGS_SILENT) != 0)
          zeromem(reservoirPtr, (size_t)samplesToDoBytes);
        else
          memcpy(reservoirPtr, inputData + offset * bytesPerFrame,
                 (size_t)samplesToDoBytes);
  ...
  void copyBuffersFromReservoir(float *const *destBuffers,
                                const int numDestBuffers,
    ...
    for (const auto &block :
         queue.read(jmin(queue.getNumReadable(), bufferSize))) {
      for (auto i = 0; i < numDestBuffers; ++i)
        converter->convertSamples(
            destBuffers[i] + offset, 0,
            addBytesToPointer(reservoir.getData(),
                              block.getStart() * bytesPerFrame),
            channelMaps.getUnchecked(i), block.getLength());
...
public:
  ...
  ~WASAPIOutputDevice() override { close(); }
  ...
  template <class DestType> void updateFormatWithType(DestType *) {
    using NativeType =
        AudioData::Pointer<AudioData::Float32, AudioData::NativeEndian,
                           AudioData::NonInterleaved, AudioData::Const>;
    converter.reset(
        new AudioData::ConverterInstance<
            NativeType,
            AudioData::Pointer<DestType, AudioData::LittleEndian,
                               AudioData::Interleaved, AudioData::NonConst>>(
            1, actualNumChannels));
  }
  ...
  bool start() {
    ...
    if (check(renderClient->GetBuffer((UINT32)samplesToDo, &outputData)))
      renderClient->ReleaseBuffer((UINT32)samplesToDo,
  ...
  void copyBuffers(const float *const *srcBuffers, int numSrcBuffers,
                   int bufferSize, WASAPIInputDevice *inputDevice,
                   Thread &thread) {
    if (numChannels <= 0)
      return;
    ...
    while (bufferSize > 0) {
      // This is needed in order not to drop any input data if the output device
      // endpoint buffer was full
      ...
      if (samplesToDo == 0) {
        ...
        if (!thread.threadShouldExit() &&
            WaitForSingleObject(clientEvent, 1000) == WAIT_OBJECT_0)
          continue;
      ...
      if (check(renderClient->GetBuffer((UINT32)samplesToDo, &outputData))) {
        for (int i = 0; i < numChannelsToCopy; ++i)
          converter->convertSamples(outputData, channelMaps.getUnchecked(i),
                                    srcBuffers[i] + offset, 0, samplesToDo);
...
public:
  WASAPIAudioIODevice(const String &deviceName, const String &typeNameIn,
                      const String &outputDeviceID, const String &inputDeviceID,
                      WASAPIDeviceMode mode)
      : AudioIODevice(deviceName, typeNameIn), Thread("JUCE WASAPI"),
  ...
  bool initialise() {
    ...
    if (createDevices()) {
      ...
      if (inputDevice != nullptr && outputDevice != nullptr) {
        defaultSampleRate = jmin(inputDevice->defaultSampleRate,
                                 outputDevice->defaultSampleRate);
        minBufferSize =
            jmax(inputDevice->minBufferSize, outputDevice->minBufferSize);
        defaultBufferSize = jmax(inputDevice->defaultBufferSize,
                                 outputDevice->defaultBufferSize);

        if (isLowLatencyMode(deviceMode)) {
          lowLatencyMaxBufferSize = jmin(inputDevice->lowLatencyMaxBufferSize,
                                         outputDevice->lowLatencyMaxBufferSize);
          lowLatencyBufferSizeMultiple =
              jmax(inputDevice->lowLatencyBufferSizeMultiple,
                   outputDevice->lowLatencyBufferSizeMultiple);
        }
        ...
        auto *d = inputDevice != nullptr
                      ? static_cast<WASAPIDeviceBase *>(inputDevice.get())
  ...
  int getXRunCount() const noexcept override {
    return inputDevice != nullptr ? inputDevice->xruns : -1;
  }

  std::optional<BigInteger> getDefaultOutputChannels() const override {
    return outputDevice != nullptr ? outputDevice->getDefaultLayout()
                                   : std::nullopt;
  }

  std::optional<BigInteger> getDefaultInputChannels() const override {
    return inputDevice != nullptr ? inputDevice->getDefaultLayout()
                                  : std::nullopt;
  }
  ...
  String open(const BigInteger &inputChannels, const BigInteger &outputChannels,
    ...
    if (sampleRates.size() == 0 && inputDevice != nullptr &&
    ...
    currentBufferSizeSamples = bufferSizeSamples <= 0
                                   ? defaultBufferSize
                                   : jmax(bufferSizeSamples, minBufferSize);
    ...
    if (inputDevice != nullptr &&
        !inputDevice->open(currentSampleRate, inputChannels,
                           bufferSizeSamples)) {
    ...
    if (outputDevice != nullptr &&
        !outputDevice->open(currentSampleRate, outputChannels,
                            bufferSizeSamples)) {
      close();
    ...
    if (isExclusiveMode(deviceMode)) {
      // This is to make sure that the callback uses actualBufferSize in case of
      // exclusive mode
      if (inputDevice != nullptr && outputDevice != nullptr &&
          inputDevice->actualBufferSize != outputDevice->actualBufferSize) {
        close();
        lastError =
            TRANS("Couldn't open the output device (buffer size mismatch)");
        return lastError;
      }

      currentBufferSizeSamples =
          (int)(outputDevice != nullptr ? outputDevice->actualBufferSize
    ...
    if (outputDevice != nullptr && outputDevice->client != nullptr) {
      latencyOut =
          (int)(outputDevice->latencySamples + currentBufferSizeSamples);
  ...
  void setMMThreadPriority() {
    ...
    JUCE_LOAD_WINAPI_FUNCTION(dll, AvSetMmThreadCharacteristicsW,
                              avSetMmThreadCharacteristics, HANDLE,
                              (LPCWSTR, LPDWORD))
    JUCE_LOAD_WINAPI_FUNCTION(dll, AvSetMmThreadPriority, avSetMmThreadPriority,
                              HANDLE, (HANDLE, AVRT_PRIORITY))

    if (avSetMmThreadCharacteristics != nullptr &&
        avSetMmThreadPriority != nullptr) {
      DWORD dummy = 0;
  ...
  void run() override {
    ...
    while (!threadShouldExit()) {
      ...
      auto inputDeviceActive =
          (inputDevice != nullptr && inputDevice->isActive);
      auto outputDeviceActive =
          (outputDevice != nullptr && outputDevice->isActive);
      ...
      if (inputDeviceActive) {
        if (outputDevice == nullptr) {
          if (WaitForSingleObject(inputDevice->clientEvent, 1000) ==
              WAIT_TIMEOUT)
            break;
        ...
        inputDevice->copyBuffersFromReservoir(inputBuffers, numInputBuffers,
                                              bufferSize);
...

      {
        ...
        if (sl.isLocked() && isStarted)
          callback->audioDeviceIOCallbackWithContext(
              inputBuffers, numInputBuffers, outputBuffers, numOutputBuffers,
              bufferSize, {});
        else
          outs.clear();
      }
      ...
      if (outputDeviceActive) {
        // Note that this function is handed the input device so it can check
        // for the event and make sure the input reservoir is filled up
        // correctly even when bufferSize > device actualBufferSize
        outputDevice->copyBuffers(outputBuffers, numOutputBuffers, bufferSize,
                                  inputDevice.get(), *this);
...
private:
  ...
  bool createDevices() {
    ...
    if (!check(enumerator->EnumAudioEndpoints(
            eAll, DEVICE_STATE_ACTIVE,
            deviceCollection.resetAndGetPointerAddress())))
      return false;
    ...
    return (outputDeviceId.isEmpty() ||
            (outputDevice != nullptr && outputDevice->isOk())) &&
           (inputDeviceId.isEmpty() ||
            (inputDevice != nullptr && inputDevice->isOk()));
  }
  ...
  void handleAsyncUpdate() override {
    ...
    if (shouldShutdown) {
      ...
      auto sampleRateChangedByInput =
          (inputDevice != nullptr && inputDevice->sampleRateHasChanged);
      ...
      open(lastKnownInputChannels, lastKnownOutputChannels, changedSampleRate,
           currentBufferSizeSamples);
...
public:
  explicit WASAPIAudioIODeviceType(WASAPIDeviceMode mode)
      : AudioIODeviceType(getDeviceTypename(mode)), deviceMode(mode) {}
  ...
  AudioIODevice *createDevice(const String &outputDeviceName,
    ...
    if (outputIndex >= 0 || inputIndex >= 0) {
      device.reset(new WASAPIAudioIODevice(
          outputDeviceName.isNotEmpty() ? outputDeviceName : inputDeviceName,
          getTypeName(), devices.outputDeviceIds[outputIndex],
  ...
  struct Devices {
    ...
    auto tie() const {
      return std::tie(outputDeviceNames, outputDeviceIds, inputDeviceNames,
                      inputDeviceIds);
    }
...
private:
  DeviceChangeDetector deviceChangeDetector{L"Windows Audio",
                                            [this] { systemDeviceChanged(); }};
  WASAPIDeviceMode deviceMode;
  bool hasScanned = false;
  ComSmartPtr<IMMDeviceEnumerator> enumerator;

  //==============================================================================
  class ChangeNotificationClient final
      : public ComBaseClassHelper<IMMNotificationClient> {
  public:
    ...
    JUCE_COMRESULT OnDeviceStateChanged(LPCWSTR, DWORD) override {
      return notify();
    }
    JUCE_COMRESULT OnDefaultDeviceChanged(EDataFlow, ERole, LPCWSTR) override {
      return notify();
    }
    JUCE_COMRESULT OnPropertyValueChanged(LPCWSTR, const PROPERTYKEY) override {
      return notify();
    }
  ...
  //==============================================================================
  static String getDefaultEndpoint(IMMDeviceEnumerator *enumerator,
                                   bool forCapture) {
    String s;
    IMMDevice *dev = nullptr;

    if (check(enumerator->GetDefaultAudioEndpoint(
            forCapture ? eCapture : eRender, eMultimedia, &dev))) {
      WCHAR *deviceId = nullptr;
    ...
    for (UINT32 i = 0; i < numDevices; ++i) {
...

      {
        ...
        if (!check(device->OpenPropertyStore(
                STGM_READ, properties.resetAndGetPointerAddress())))
          continue;
...
struct MMDeviceMasterVolume {
  MMDeviceMasterVolume() {
    ...
    if (check(enumerator.CoCreateInstance(__uuidof(MMDeviceEnumerator)))) {
      ...
      if (check(enumerator->GetDefaultAudioEndpoint(
              eRender, eConsole, device.resetAndGetPointerAddress())))
  ...
  bool setGain(float newGain) const {
    return endpointVolume != nullptr &&
           check(endpointVolume->SetMasterVolumeLevelScalar(
               jlimit(0.0f, 1.0f, newGain), nullptr));
  }

  bool isMuted() const {
    BOOL mute = 0;
    return endpointVolume != nullptr && check(endpointVolume->GetMute(&mute)) &&
           mute != 0;
  }
