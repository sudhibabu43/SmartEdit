...
static int logErrorMessage(int err, int lineNum) {
  if (err < 0)
    JUCE_ALSA_LOG("Error: line " << lineNum << ": code " << err << " ("
                                 << snd_strerror(err) << ")");
...
static void getDeviceSampleRates(snd_pcm_t *handle, Array<double> &rates) {
  ...
  for (const auto rateToTry : SampleRateHelpers::getAllSampleRates()) {
    if (snd_pcm_hw_params_any(handle, hwParams) >= 0 &&
        snd_pcm_hw_params_test_rate(handle, hwParams, (unsigned int)rateToTry,
                                    0) == 0) {
...
static void getDeviceNumChannels(snd_pcm_t *handle, unsigned int *minChans,
  ...
  if (snd_pcm_hw_params_any(handle, params) >= 0) {
    ...
    JUCE_ALSA_LOG("getDeviceNumChannels: " << (int)*minChans << " "
                                           << (int)*maxChans);
...
static void getDeviceProperties(const String &deviceID,
                                ...
                                unsigned int &maxChansIn, Array<double> &rates,
                                bool testOutput, bool testInput) {
  ...
  if (testOutput) {
    ...
    if (JUCE_CHECKED_RESULT(
            snd_pcm_open(&pcmHandle, deviceID.toUTF8().getAddress(),
  ...
  if (testInput) {
    ...
    if (JUCE_CHECKED_RESULT(snd_pcm_open(&pcmHandle, deviceID.toUTF8(),
...
static void silentErrorHandler(const char *, int, const char *, int,
                               const char *, ...) {}
...
public:
  ALSADevice(const String &devID, bool forInput)
      : handle(nullptr), bitDepth(16), numChannelsRunning(0), latency(0),
        deviceID(devID), isInput(forInput), isInterleaved(true) {
    JUCE_ALSA_LOG("snd_pcm_open (" << deviceID.toUTF8().getAddress()
                                   << ", forInput=" << (int)forInput << ")");

    int err = snd_pcm_open(&handle, deviceID.toUTF8(),
                           forInput ? SND_PCM_STREAM_CAPTURE
                                    : SND_PCM_STREAM_PLAYBACK,
                           SND_PCM_ASYNC);
    if (err < 0) {
      if (-err == EBUSY)
        error << "The device \"" << deviceID
              << "\" is busy (another application is using it).";
      else if (-err == ENOENT)
        error << "The device \"" << deviceID << "\" is not available.";
      else
        error << "Could not open " << (forInput ? "input" : "output")
              << " device \"" << deviceID << "\": " << snd_strerror(err) << " ("
              << err << ")";
  ...
  bool setParameters(unsigned int sampleRate, int numChannels, int bufferSize) {
    ...
    JUCE_ALSA_LOG("ALSADevice::setParameters("
                  << deviceID << ", " << (int)sampleRate << ", " << numChannels
                  << ", " << bufferSize << ")");
    ...
    if (snd_pcm_hw_params_any(handle, hwParams) < 0) {
      // this is the error message that aplay returns when an error happens
      // here, it is a bit more explicit that "Invalid parameter"
    ...
    if (snd_pcm_hw_params_set_access(handle, hwParams,
      ...
      isInterleaved = true;
                 ...
                 handle, hwParams, SND_PCM_ACCESS_RW_NONINTERLEAVED) >= 0)
      isInterleaved = false;
    else {
      jassertfalse;
      return false;
    ...
    for (int i = 0; i < numElementsInArray(formatsToTry); i += 2) {
      if (snd_pcm_hw_params_set_format(handle, hwParams,
        ...
        converter.reset(createConverter(
            isInput, bitDepth, (type & isFloatBit) != 0,
            (type & isLittleEndianBit) != 0, (type & onlyUseLower24Bits) != 0,
            numChannels, isInterleaved));
        break;
      }
    }
    ...
    if (JUCE_ALSA_FAILED(snd_pcm_hw_params_set_rate_near(
            handle, hwParams, &sampleRate, nullptr)) ||
        JUCE_ALSA_FAILED(snd_pcm_hw_params_set_channels(
            handle, hwParams, (unsigned int)numChannels)) ||
        JUCE_ALSA_FAILED(snd_pcm_hw_params_set_periods_near(handle, hwParams,
                                                            &periods, &dir)) ||
        JUCE_ALSA_FAILED(snd_pcm_hw_params_set_period_size_near(
            handle, hwParams, &samplesPerPeriod, &dir)) ||
    ...
    if (JUCE_ALSA_FAILED(snd_pcm_sw_params_current(handle, swParams)) ||
        ...
        JUCE_ALSA_FAILED(
            snd_pcm_sw_params_set_silence_threshold(handle, swParams, 0)) ||
        JUCE_ALSA_FAILED(
            snd_pcm_sw_params_set_silence_size(handle, swParams, boundary)) ||
        JUCE_ALSA_FAILED(snd_pcm_sw_params_set_start_threshold(
            handle, swParams, samplesPerPeriod)) ||
        JUCE_ALSA_FAILED(
  ...
  //==============================================================================
  bool writeToOutputDevice(AudioBuffer<float> &outputChannelBuffer,
    ...
    if (isInterleaved) {
      scratch.ensureSize(
          (size_t)((int)sizeof(float) * numSamples * numChannelsRunning),
          false);

      for (int i = 0; i < numChannelsRunning; ++i)
        converter->convertSamples(scratch.getData(), i, data[i], 0, numSamples);

      numDone = snd_pcm_writei(handle, scratch.getData(),
      ...
      numDone =
          snd_pcm_writen(handle, (void **)data, (snd_pcm_uframes_t)numSamples);
    }
    ...
    if (numDone < 0) {
      ...
      if (JUCE_ALSA_FAILED(
              snd_pcm_recover(handle, (int)numDone, 1 /* silent */)))
        return false;
    }

    if (numDone < numSamples)
      JUCE_ALSA_LOG("Did not write all samples: numDone: "
                    << numDone << ", numSamples: " << numSamples);

    return true;
  }

  bool readFromInputDevice(AudioBuffer<float> &inputChannelBuffer,
                           const int numSamples) {
    ...
    if (isInterleaved) {
      scratch.ensureSize(
          ...
          false);
      scratch.fillWith(
          0); // (not clearing this data causes warnings in valgrind)

      auto num = snd_pcm_readi(handle, scratch.getData(),
                               (snd_pcm_uframes_t)numSamples);
      ...
      if (num < numSamples)
        JUCE_ALSA_LOG("Did not read all samples: num: "
                      << num << ", numSamples: " << numSamples);

      for (int i = 0; i < numChannelsRunning; ++i)
        converter->convertSamples(data[i], 0, scratch.getData(), i, numSamples);
    } else {
      auto num =
          snd_pcm_readn(handle, (void **)data, (snd_pcm_uframes_t)numSamples);
...
private:
  ...
  //==============================================================================
  template <class SampleType> struct ConverterHelper {
    static AudioData::Converter *
    createConverter(const bool forInput, const bool isLittleEndian,
                    const int numInterleavedChannels, bool interleaved) {
      if (interleaved)
        return create<AudioData::Interleaved>(forInput, isLittleEndian,
                                              numInterleavedChannels);

      return create<AudioData::NonInterleaved>(forInput, isLittleEndian,
                                               numInterleavedChannels);
    }

  private:
    template <class InterleavedType>
    static AudioData::Converter *create(const bool forInput,
      ...
      if (forInput) {
        ...
        if (isLittleEndian)
          return new AudioData::ConverterInstance<
              AudioData::Pointer<SampleType, AudioData::LittleEndian,
                                 InterleavedType, AudioData::Const>,
              DestType>(numInterleavedChannels, 1);

        return new AudioData::ConverterInstance<
            AudioData::Pointer<SampleType, AudioData::BigEndian,
                               InterleavedType, AudioData::Const>,
            DestType>(numInterleavedChannels, 1);
      }
      ...
      if (isLittleEndian)
        return new AudioData::ConverterInstance<
            SourceType,
            AudioData::Pointer<SampleType, AudioData::LittleEndian,
                               InterleavedType, AudioData::NonConst>>(
            1, numInterleavedChannels);

      return new AudioData::ConverterInstance<
          SourceType, AudioData::Pointer<SampleType, AudioData::BigEndian,
                                         InterleavedType, AudioData::NonConst>>(
          1, numInterleavedChannels);
    }
  };
  ...
  static AudioData::Converter *
  createConverter(bool forInput, int bitDepth, bool isFloat,
                  bool isLittleEndian, bool useOnlyLower24Bits,
                  int numInterleavedChannels, bool interleaved) {
    JUCE_ALSA_LOG("format: bitDepth="
                  << bitDepth << ", isFloat=" << (int)isFloat
                  << ", isLittleEndian=" << (int)isLittleEndian
                  << ", numChannels=" << numInterleavedChannels);

    if (isFloat)
      return ConverterHelper<AudioData::Float32>::createConverter(
          forInput, isLittleEndian, numInterleavedChannels, interleaved);
    if (bitDepth == 16)
      return ConverterHelper<AudioData::Int16>::createConverter(
          forInput, isLittleEndian, numInterleavedChannels, interleaved);
    if (bitDepth == 24)
      return ConverterHelper<AudioData::Int24>::createConverter(
          forInput, isLittleEndian, numInterleavedChannels, interleaved);

    jassert(bitDepth == 32);

    if (useOnlyLower24Bits)
      return ConverterHelper<AudioData::Int24in32>::createConverter(
          forInput, isLittleEndian, numInterleavedChannels, interleaved);

    return ConverterHelper<AudioData::Int32>::createConverter(
        forInput, isLittleEndian, numInterleavedChannels, interleaved);
  }
...
public:
  ALSAThread(const String &inputDeviceID, const String &outputDeviceID)
      : Thread("JUCE ALSA"), inputId(inputDeviceID), outputId(outputDeviceID) {
    initialiseRatesAndChannels();
  }

  ~ALSAThread() override { close(); }

  void open(BigInteger inputChannels, BigInteger outputChannels,
    ...
    maxInputsRequested =
        jmax((int)minChansIn, jmin((int)maxChansIn, maxInputsRequested));
    ...
    maxOutputsRequested =
        jmax((int)minChansOut, jmin((int)maxChansOut, maxOutputsRequested));
    ...
    // Note that the input device is opened before an output, because we've
    // heard of drivers where doing it in the reverse order mysteriously fails..
    // If this order also causes problems, let us know and we'll see if we can
    // find a compromise!
    ...
    if (inputChannelDataForCallback.size() > 0 && inputId.isNotEmpty()) {
      ...
      if (!inputDevice->setParameters(
              (unsigned int)sampleRate,
              jlimit((int)minChansIn, (int)maxChansIn,
    ...
    if (outputChannels.getHighestBit() >= 0) {
      for (int i = 0; i < maxOutputsRequested; ++i) {
        if (outputChannels[i]) {
          outputChannelDataForCallback.add(
              outputChannelBuffer.getWritePointer(i));
    ...
    if (inputDevice != nullptr &&
        JUCE_ALSA_FAILED(snd_pcm_prepare(inputDevice->handle)))
      return;

    if (outputDevice != nullptr &&
        JUCE_ALSA_FAILED(snd_pcm_prepare(outputDevice->handle)))
      return;
  ...
  void close() {
    if (isThreadRunning()) {
      // problem: when pulseaudio is suspended (with pasuspend) , the
      // ALSAThread::run is just stuck in snd_pcm_writei -- no error, no nothing
      // it just stays stuck. So the only way I found to exit "nicely" (that is
      // without the "killing thread by force" of stopThread) , is to just call
      // snd_pcm_close from here which will cause the thread to resume, and exit
      signalThreadShouldExit();

      const int callbacksToStop = numCallbacks;

      if ((!waitForThreadToExit(400)) && audioIoInProgress &&
          numCallbacks == callbacksToStop) {
    ...
    while (!threadShouldExit()) {
      if (inputDevice != nullptr && inputDevice->handle != nullptr) {
        if (outputDevice == nullptr || outputDevice->handle == nullptr) {
          ...
          if (avail < 0)
            JUCE_ALSA_FAILED(
                snd_pcm_recover(inputDevice->handle, (int)avail, 0));
        }
...

      {
        ...
        if (callback != nullptr) {
          callback->audioDeviceIOCallbackWithContext(
              inputChannelDataForCallback.getRawDataPointer(),
              inputChannelDataForCallback.size(),
              outputChannelDataForCallback.getRawDataPointer(),
              outputChannelDataForCallback.size(), bufferSize, {});
        } else {
          for (int i = 0; i < outputChannelDataForCallback.size(); ++i)
            zeromem(outputChannelDataForCallback[i],
      ...
      if (outputDevice != nullptr && outputDevice->handle != nullptr) {
        ...
        if (avail < 0)
          JUCE_ALSA_FAILED(
              snd_pcm_recover(outputDevice->handle, (int)avail, 0));

        audioIoInProgress = true;

        if (!outputDevice->writeToOutputDevice(outputChannelBuffer,
                                               bufferSize)) {
...
private:
  ...
  void initialiseRatesAndChannels() {
    ...
    getDeviceProperties(inputId, dummy, dummy, minChansIn, maxChansIn,
                        sampleRates, false, true);
    getDeviceProperties(outputId, minChansOut, maxChansOut, dummy, dummy,
                        sampleRates, true, false);
...
public:
  ALSAAudioIODevice(const String &deviceName, const String &deviceTypeName,
                    const String &inputDeviceID, const String &outputDeviceID)
      : AudioIODevice(deviceName, deviceTypeName), inputId(inputDeviceID),
        outputId(outputDeviceID), internal(inputDeviceID, outputDeviceID) {}

  ~ALSAAudioIODevice() override { close(); }
  ...
  Array<int> getAvailableBufferSizes() override {
    ...
    for (int i = 0; i < 50; ++i) {
      r.add(n);
      n += n < 64 ? 16
                  : (n < 512 ? 32 : (n < 1024 ? 64 : (n < 2048 ? 128 : 256)));
    }
...
public:
  ALSAAudioIODeviceType(bool onlySoundcards, const String &deviceTypeName)
      : AudioIODeviceType(deviceTypeName), listOnlySoundcards(onlySoundcards) {
#if !JUCE_ALSA_LOGGING
  ...
  ~ALSAAudioIODeviceType() override {
    ...
    snd_config_update_free_global(); // prevent valgrind from screaming about
                                     // alsa leaks
  }
  ...
  AudioIODevice *createDevice(const String &outputDeviceName,
    ...
    String deviceName(outputIndex >= 0 ? outputDeviceName : inputDeviceName);
...
private:
  ...
  bool testDevice(const String &id, const String &outputName,
    ...
    getDeviceProperties(id, minChansOut, maxChansOut, minChansIn, maxChansIn,
                        rates, isOutput, isInput);

    isInput = maxChansIn > 0;
    isOutput = maxChansOut > 0;

    if ((isInput || isOutput) && rates.size() > 0) {
      JUCE_ALSA_LOG("testDevice: '" << id.toUTF8().getAddress()
                                    << "' -> isInput: " << (int)isInput
  ...
  void enumerateAlsaSoundcards() {
    ...
    while (outputIds.size() + inputIds.size() <= 64) {
      ...
      if (JUCE_CHECKED_RESULT(
              snd_ctl_open(&handle, ("hw:" + String(cardNum)).toRawUTF8(),
                           SND_CTL_NONBLOCK)) >= 0) {
        if (JUCE_CHECKED_RESULT(snd_ctl_card_info(handle, info)) >= 0) {
          ...
          for (;;) {
            ...
            for (unsigned int subDevice = 0, nbSubDevice = 1;
                 subDevice < nbSubDevice; ++subDevice) {
  ...
  void enumerateAlsaPCMDevices() {
    ...
    if (JUCE_CHECKED_RESULT(snd_device_name_hint(-1, "pcm", &hints)) == 0) {
      for (char **h = (char **)hints; *h; ++h) {
        ...
        JUCE_ALSA_LOG("ID: " << id << "; desc: " << description
                             << "; ioid: " << ioid);

        String ss = id.fromFirstOccurrenceOf("=", false, false)
                        .upToFirstOccurrenceOf(",", false, false);

        if (id.isEmpty() || id.startsWith("default:") ||
            id.startsWith("sysdefault:") || id.startsWith("plughw:") ||
            id == "null")
          continue;
        ...
        // alsa is stupid here, it advertises dmix and dsnoop as input/output
        // devices, but opening dmix as input, or dsnoop as output will trigger
        // errors..
    ...
    // sometimes the "default" device is not listed, but it is nice to see it
    ...
    // make sure the default device is listed first, and followed by the pulse
    // device (if present)
