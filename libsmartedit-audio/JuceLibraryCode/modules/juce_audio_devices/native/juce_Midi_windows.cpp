...
template <typename T> class CheckedReference {
public:
  template <typename Ptr> friend auto createCheckedReference(Ptr *);
  ...
  template <typename Callback> void access(Callback &&callback) {
    std::lock_guard lock{mutex};
    callback(ptr);
  }
...
template <typename Ptr> auto createCheckedReference(Ptr *ptrIn) {
  return std::shared_ptr<CheckedReference<Ptr>>{
      new CheckedReference<Ptr>(ptrIn)};
}
...
struct MidiServiceType {
  ...
  virtual MidiInput::Pimpl *createInputWrapper(MidiInput &, const String &,
                                               MidiInputCallback &) = 0;
...
//==============================================================================
struct Win32MidiService final : public MidiServiceType, private Timer {
  ...
  MidiInput::Pimpl *createInputWrapper(MidiInput &input,
                                       const String &deviceIdentifier,
                                       MidiInputCallback &callback) override {
    return new Win32InputWrapper(*this, input, deviceIdentifier, callback);
  }

  MidiOutput::Pimpl *
  createOutputWrapper(const String &deviceIdentifier) override {
...
private:
  ...
  struct MidiInCollector final : public ReferenceCountedObject {
    ...
    static void CALLBACK midiInCallback(HMIDIIN, UINT uMsg,
      ...
      if (activeMidiCollectors.contains(collector)) {
        if (uMsg == MIM_DATA)
          collector->handleMessage((const uint8 *)&midiMessage,
                                   (uint32)timeStamp);
  ...
  private:
    ...
    // This static array is used to prevent occasional callbacks to objects that
    ...
    struct MidiHeader {
      ...
      void unprepare(HMIDIIN device) {
        if ((hdr.dwFlags & WHDR_DONE) != 0) {
          int c = 10;
          while (--c >= 0 && midiInUnprepareHeader(device, &hdr, sizeof(hdr)) ==
  ...
  //==============================================================================
  template <class WrapperType> struct Win32MidiDeviceQuery {
    ...
    static Array<MidiDeviceInfo> getAvailableDevices() {
      ...
      deviceNames.appendNumbersToDuplicates(false, false, CharPointer_UTF8("-"),
                                            CharPointer_UTF8(""));
      deviceIDs.appendNumbersToDuplicates(false, false, CharPointer_UTF8("-"),
                                          CharPointer_UTF8(""));
  ...
  private:
    static String getInterfaceIDForDevice(UINT id) {
      ...
      if (WrapperType::sendMidiMessage(
              (UINT_PTR)id, DRV_QUERYDEVICEINTERFACESIZE, (DWORD_PTR)&size,
              0) == MMSYSERR_NOERROR) {
  ...
  struct Win32InputWrapper final
      : public MidiInput::Pimpl,
        public Win32MidiDeviceQuery<Win32InputWrapper> {
    Win32InputWrapper(Win32MidiService &parentService, MidiInput &midiInput,
                      const String &deviceIdentifier, MidiInputCallback &c)
        : input(midiInput), callback(c) {
      collector = getOrCreateCollector(parentService, deviceIdentifier);
      collector->addClient(this);
    }

    ~Win32InputWrapper() override { collector->removeClient(this); }

    static MidiInCollector::Ptr
    getOrCreateCollector(Win32MidiService &parentService,
                         const String &deviceIdentifier) {
      ...
      MidiInCollector::Ptr c(
          new MidiInCollector(parentService, {deviceName, deviceIdentifier}));

      HMIDIIN h;
      auto err =
          midiInOpen(&h, deviceID, (DWORD_PTR)&MidiInCollector::midiInCallback,
                     (DWORD_PTR)(MidiInCollector *)c.get(), CALLBACK_FUNCTION);
    ...
    static DWORD sendMidiMessage(UINT_PTR deviceID, UINT msg, DWORD_PTR arg1,
                                 DWORD_PTR arg2) {
    ...
    String getDeviceIdentifier() override {
      return collector->deviceInfo.identifier;
    }
  ...
  //==============================================================================
  struct Win32OutputWrapper final
    ...
    void sendMessageNow(const MidiMessage &message) override {
      if (message.getRawDataSize() > 3 || message.isSysEx()) {
        ...
        if (midiOutPrepareHeader(han->handle, &h, sizeof(MIDIHDR)) ==
            MMSYSERR_NOERROR) {
        ...
        for (int i = 0; i < 50; ++i) {
          if (midiOutShortMsg(han->handle,
                              *unalignedPointerCast<const unsigned int *>(
                                  message.getRawData())) != MIDIERR_NOTREADY)
            break;
  ...
  //==============================================================================
  void asyncCheckForUnusedCollectors() { startTimer(10); }
...
Array<Win32MidiService::MidiInCollector *, CriticalSection>
    Win32MidiService::MidiInCollector::activeMidiCollectors;
...
public:
  ...
  WinRTMidiService() {
    ...
    midiInFactory = wrtWrapper->getWRLFactory<IMidiInPortStatics>(
        &RuntimeClass_Windows_Devices_Midi_MidiInPort[0]);

    if (midiInFactory == nullptr)
      throw std::runtime_error("Failed to create midi in factory");

    midiOutFactory = wrtWrapper->getWRLFactory<IMidiOutPortStatics>(
        &RuntimeClass_Windows_Devices_Midi_MidiOutPort[0]);

    if (midiOutFactory == nullptr)
      throw std::runtime_error("Failed to create midi out factory");

    // The WinRT BLE MIDI API doesn't provide callbacks when devices become
    // disconnected, but it does require a disconnection via the API before a
    // device will reconnect again. We can monitor the BLE connection state of
    // paired devices to get callbacks when connections are broken.
    ...
    inputDeviceWatcher.reset(
        new MidiIODeviceWatcher<IMidiInPortStatics>(midiInFactory));

    if (!inputDeviceWatcher->start())
      throw std::runtime_error("Failed to start the midi input device watcher");

    outputDeviceWatcher.reset(
        new MidiIODeviceWatcher<IMidiOutPortStatics>(midiOutFactory));

    if (!outputDeviceWatcher->start())
      throw std::runtime_error(
          "Failed to start the midi output device watcher");
  }
...
private:
  ...
  public:
    ...
    bool attach(HSTRING deviceSelector, DeviceInformationKind infoKind) {
      ...
      auto deviceInfoFactory =
          wrtWrapper->getWRLFactory<IDeviceInformationStatics2>(
              &RuntimeClass_Windows_Devices_Enumeration_DeviceInformation[0]);
      ...
      auto requestedProperties = [wrtWrapper] {
        auto devicePicker = wrtWrapper->activateInstance<IDevicePicker>(
            &RuntimeClass_Windows_Devices_Enumeration_DevicePicker[0],
      ...
      auto hr = requestedProperties->QueryInterface(
          __uuidof(IIterable<HSTRING>),
          (void **)iter.resetAndGetPointerAddress());
    ...
    String getGUIDFromInspectable(IInspectable &inspectable) {
      ...
      auto hr = inspectable.QueryInterface(
          __uuidof(IReference<GUID>),
  ...
  private:
    ...
    struct DeviceEnumerationThread final : public Thread {
      DeviceEnumerationThread(DeviceCallbackHandler &h,
          ...
          : Thread("WinRT Device Enumeration Thread"), handler(h), watcher(w),
            deviceAddedToken(added), deviceRemovedToken(removed),
      ...
      EventRegistrationToken &deviceAddedToken, deviceRemovedToken,
          deviceUpdatedToken;
    };
    ...
    DeviceEnumerationThread enumerationThread{*this, watcher, deviceAddedToken,
                                              deviceRemovedToken,
                                              deviceUpdatedToken};
  };
  ...
  struct BLEDeviceWatcher final : private DeviceCallbackHandler {
    ...
    HRESULT addDevice(IDeviceInformation *addedDeviceInfo) override {
      ...
      if (auto *containerIDValue = getValueFromDeviceInfo(
              "System.Devices.Aep.ContainerId", addedDeviceInfo)) {
        auto containerID = getGUIDFromInspectable(*containerIDValue);

        if (containerID.isNotEmpty()) {
          DeviceInfo info = {containerID};

          if (auto *connectedValue = getValueFromDeviceInfo(
                  "System.Devices.Aep.IsConnected", addedDeviceInfo))
            info.isConnected = getBoolFromInspectable(*connectedValue);

          JUCE_WINRT_MIDI_LOG(
              "Adding BLE device: "
              << deviceID << " " << info.containerID << " "
      ...
      JUCE_WINRT_MIDI_LOG(
          "Failed to get a container ID for BLE device: " << deviceID);
      return S_OK;
    }
    ...
    HRESULT updateDevice(IDeviceInformationUpdate *updatedDeviceInfo) override {
      ...
      if (auto *connectedValue = getValueFromDeviceInfo(
              "System.Devices.Aep.IsConnected", updatedDeviceInfo)) {
...

        {
          ...
          if (info.isConnected && !isConnected) {
            JUCE_WINRT_MIDI_LOG(
                "BLE device connection status change: "
                << updatedDeviceId << " " << info.containerID << " "
    ...
    //==============================================================================
    bool start() {
      WinRTWrapper::ScopedHString deviceSelector(
          "System.Devices.Aep.ProtocolId:=\"{bb7bb05e-5972-42b5-94fc-"
          "76eaa7084d49}\""
          " AND "
          "System.Devices.Aep.IsPaired:=System.StructuredQueryType.Boolean#"
          "True");
      return attach(
          deviceSelector.get(),
          DeviceInformationKind::DeviceInformationKind_AssociationEndpoint);
    }
  ...
  struct MidiIODeviceWatcher final : private DeviceCallbackHandler {
    ...
    HRESULT addDevice(IDeviceInformation *addedDeviceInfo) override {
      ...
      // We use the container ID to match a MIDI device with a generic BLE
      // device, if possible
      if (auto *containerIDValue = getValueFromDeviceInfo(
              "System.Devices.ContainerId", addedDeviceInfo))
      ...
      if (FAILED(hr)) {
        JUCE_WINRT_MIDI_LOG("Failed to query detected MIDI device name for "
                            << info.deviceID);
        return S_OK;
      }
      ...
      if (FAILED(hr)) {
        JUCE_WINRT_MIDI_LOG(
            "Failed to query detected MIDI device defaultness for "
      ...
      JUCE_WINRT_MIDI_LOG("Adding MIDI device: " << info.deviceID << " "
                                                 << info.containerID << " "
                                                 << info.name);
    ...
    bool start() {
      ...
      return attach(
          deviceSelector,
          DeviceInformationKind::DeviceInformationKind_DeviceInterface);
    }
  ...
  //==============================================================================
  template <typename COMType, typename COMFactoryType,
            typename COMInterfaceType>
  static void openMidiPortThread(String threadName, String midiDeviceID,
    ...
    std::thread{[&] {
      ...
      auto callback =
          [p = std::move(promise)](IAsyncOperation<COMType *> *asyncOpPtr,
                                   AsyncStatus) mutable {
            ...
            const auto hr =
                asyncOpPtr->GetResults(result.resetAndGetPointerAddress());
      ...
      const auto ir = asyncOp->put_Completed(
          Callback<IAsyncOperationCompletedHandler<COMType *>>(
              std::move(callback))
              .Get());
  ...
  public:
    WinRTIOWrapper(BLEDeviceWatcher &bleWatcher,
        ...
        : bleDeviceWatcher(bleWatcher) {
      {
        ...
        deviceInfo =
            midiDeviceWatcher.getWinRTDeviceInfoForDevice(deviceIdentifier);
      }
      ...
      if (deviceInfo.containerID.isNotEmpty()) {
        ...
        HashMap<String, BLEDeviceWatcher::DeviceInfo>::Iterator iter(
            bleDeviceWatcher.devices);
  ...
  private:
    ...
    void bleDeviceDisconnected(const String &containerID) override {
      if (containerID == deviceInfo.containerID) {
        JUCE_WINRT_MIDI_LOG("Disconnecting MIDI port from BLE disconnection: "
                            << deviceInfo.deviceID << " "
  ...
  //==============================================================================
  struct WinRTInputWrapper final
      : public MidiInput::Pimpl,
        private WinRTIOWrapper<IMidiInPortStatics, IMidiInPort>

  {
    WinRTInputWrapper(WinRTMidiService &service, MidiInput &input,
                      const String &deviceIdentifier, MidiInputCallback &cb)
        : WinRTIOWrapper<IMidiInPortStatics, IMidiInPort>(
              *service.bleDeviceWatcher, *service.inputDeviceWatcher,
              deviceIdentifier),
          inputDevice(input), callback(cb) {
      openMidiPortThread<MidiInPort>("Open WinRT MIDI input port",
                                     deviceInfo.deviceID, service.midiInFactory,
                                     midiPort);
      ...
      auto hr = midiPort->add_MessageReceived(
          Callback<
              ...
              [self = checkedReference](IMidiInPort *,
    ...
    HRESULT midiInMessageReceived(IMidiMessageReceivedEventArgs *args) {
      ...
      ComSmartPtr<Windows::Storage::Streams::IBufferByteAccess>
          bufferByteAccess;
    ...
    std::shared_ptr<CheckedReference<WinRTInputWrapper>> checkedReference =
        createCheckedReference(this);

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(WinRTInputWrapper);
  };

  //==============================================================================
  struct WinRTOutputWrapper final
      : public MidiOutput::Pimpl,
        private WinRTIOWrapper<IMidiOutPortStatics, IMidiOutPort> {
    WinRTOutputWrapper(WinRTMidiService &service,
                       const String &deviceIdentifier)
        : WinRTIOWrapper<IMidiOutPortStatics, IMidiOutPort>(
              *service.bleDeviceWatcher, *service.outputDeviceWatcher,
              deviceIdentifier) {
      openMidiPortThread<IMidiOutPort>("Open WinRT MIDI output port",
                                       deviceInfo.deviceID,
                                       service.midiOutFactory, midiPort);
      ...
      auto bufferFactory = wrtWrapper->getWRLFactory<IBufferFactory>(
          &RuntimeClass_Windows_Storage_Streams_Buffer[0]);

      if (bufferFactory == nullptr)
        throw std::runtime_error("Failed to create output buffer factory");

      auto hr = bufferFactory->Create(static_cast<UINT32>(65536),
...
struct MidiService final : public DeletedAtShutdown {
  MidiService() {
    ...
    if (windowsVersionInfo.dwMajorVersion >= 10 &&
        windowsVersionInfo.dwBuildNumber >= 17763)
#endif
    {
      try {
  ...
  ~MidiService() { clearSingletonInstance(); }
...
//==============================================================================
static int findDefaultDeviceIndex(const Array<MidiDeviceInfo> &available,
...
std::unique_ptr<MidiInput> MidiInput::openDevice(const String &deviceIdentifier,
  ...
  try {
    wrapper.reset(MidiService::getService().createInputWrapper(
        *in, deviceIdentifier, *callback));
  } catch (std::runtime_error &) {
    return {};
  }
...
MidiOutput::openDevice(const String &deviceIdentifier) {
  ...
  try {
    wrapper.reset(
        MidiService::getService().createOutputWrapper(deviceIdentifier));
  } catch (std::runtime_error &) {
    return {};
  }
