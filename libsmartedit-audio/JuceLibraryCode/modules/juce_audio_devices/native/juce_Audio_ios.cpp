...
constexpr std::initializer_list<double> iOSExplicitSampleRates{
    JUCE_IOS_AUDIO_EXPLICIT_SAMPLERATES};
...
static const char *
getRoutingChangeReason(AVAudioSessionRouteChangeReason reason) noexcept {
  switch (reason) {
...
bool getNotificationValueForKey(NSNotification *notification, NSString *key,
                                NSUInteger &value) noexcept {
  if (notification != nil) {
...
- (void)audioSessionChangedInterruptionType:(NSNotification *)notification {
  ...
  if (juce::getNotificationValueForKey(
          notification, AVAudioSessionInterruptionTypeKey, value)) {
    switch ((AVAudioSessionInterruptionType)value) {
    case AVAudioSessionInterruptionTypeBegan:
      audioSessionHolder->handleStatusChange(
          false, "AVAudioSessionInterruptionTypeBegan");
      break;

    case AVAudioSessionInterruptionTypeEnded:
      audioSessionHolder->handleStatusChange(
          true, "AVAudioSessionInterruptionTypeEnded");
      break;
...
- (void)handleMediaServicesReset {
  audioSessionHolder->handleStatusChange(
      true, "AVAudioSessionMediaServicesWereResetNotification");
}

- (void)handleMediaServicesLost {
  audioSessionHolder->handleStatusChange(
      false, "AVAudioSessionMediaServicesWereLostNotification");
}
...
- (void)handleRouteChange:(NSNotification *)notification {
  ...
  if (juce::getNotificationValueForKey(
          notification, AVAudioSessionRouteChangeReasonKey, value))
    audioSessionHolder->handleRouteChange(
        (AVAudioSessionRouteChangeReason)value);
}
...
static void logNSError(NSError *e) {
  if (e != nil) {
    JUCE_IOS_AUDIO_LOG("iOS Audio error: " <<
...
struct iOSAudioIODevice::Pimpl final : public AsyncUpdater {
  Pimpl(iOSAudioIODeviceType *ioDeviceType, iOSAudioIODevice &ioDevice)
      : deviceType(ioDeviceType), owner(ioDevice) {
    JUCE_IOS_AUDIO_LOG("Creating iOS audio device");

    // We need to activate the audio session here to obtain the available sample
    // rates and buffer sizes, but if we don't set a category first then
    // background audio will always be stopped. This category may be changed
    // later.
  ...
  static void setAudioSessionCategory(NSString *category) {
    ...
    options |=
        AVAudioSessionCategoryOptionMixWithOthers; // Alternatively
  ...
  int getBufferSize(const double currentSampleRate) {
    return roundToInt(currentSampleRate *
  ...
  int tryBufferSize(const double currentSampleRate, const int newBufferSize) {
    NSTimeInterval bufferDuration =
        currentSampleRate > 0
            ? (NSTimeInterval)((newBufferSize + 1) / currentSampleRate)
            : 0.0;
  ...
  void updateAvailableBufferSizes() {
    ...
    JUCE_IOS_AUDIO_LOG(
        "Buffer size after detecting available buffer sizes: " << bufferSize);
  }
  ...
  void updateAvailableSampleRates() {
    ...
    AudioUnitRemovePropertyListenerWithUserData(
        ...
        dispatchAudioUnitPropertyChange, this);
    ...
    for (double rate = lowestRate + 1000; rate < highestRate; rate += 1000) {
      ...
      JUCE_IOS_AUDIO_LOG("Trying a sample rate of " << rate << ", got "
                                                    << supportedRate);
    ...
    AudioUnitAddPropertyListener(audioUnit, kAudioUnitProperty_StreamFormat,
                                 dispatchAudioUnitPropertyChange, this);
    ...
    JUCE_IOS_AUDIO_LOG(
        "Sample rate after detecting available sample rates: " << sampleRate);
  }

  void updateHardwareInfo(bool forceUpdate = false) {
    if (!forceUpdate &&
        !hardwareInfoNeedsUpdating.compareAndSetBool(false, true))
      return;
  ...
  String open(const BigInteger &inputChannelsWanted,
              const BigInteger &outputChannelsWanted, double sampleRateWanted,
    ...
    targetBufferSize =
        bufferSizeWanted > 0 ? bufferSizeWanted : defaultBufferSize;
    ...
    setAudioSessionActive(true);
    setAudioSessionCategory(requestedInputChannels > 0
  ...
  bool setAudioPreprocessingEnabled(bool enable) {
    ...
    NSString *mode =
        (enable ? AVAudioSessionModeDefault : AVAudioSessionModeMeasurement);

    JUCE_NSERROR_CHECK([session setMode:mode error:&error]);
  ...
  public:
    ...
    void transportPlay(bool shouldSartPlaying) override {
      ...
      [[maybe_unused]] OSStatus err = callbackInfo.transportStateProc2(
          callbackInfo.hostUserData, &hostIsPlaying, nullptr, nullptr, nullptr,
          nullptr, nullptr, nullptr);

      jassert(err == noErr);

      if (hostIsPlaying != shouldSartPlaying)
        impl.handleAudioTransportEvent(
    ...
    void transportRecord(bool shouldStartRecording) override {
      ...
      [[maybe_unused]] OSStatus err = callbackInfo.transportStateProc2(
          callbackInfo.hostUserData, nullptr, &hostIsRecording, nullptr,
    ...
    Optional<PositionInfo> getPosition() const override {
      ...
      auto transportErr = callbackInfo.transportStateProc2(
          callbackInfo.hostUserData, &hostIsPlaying, &hostIsRecording, nullptr,
          &hostCurrentSampleInTimeLine, &hostIsCycling, &hostCycleStartBeat,
          &hostCycleEndBeat);
      ...
      result.setTimeInSeconds((double)*result.getTimeInSamples() /
                              impl.sampleRate);

      Float64 hostBeat = 0;
      Float64 hostTempo = 0;
      [[maybe_unused]] auto batErr = callbackInfo.beatAndTempoProc(
          callbackInfo.hostUserData, &hostBeat, &hostTempo);
      ...
      [[maybe_unused]] auto timeErr = callbackInfo.musicalTimeLocationProc(
          callbackInfo.hostUserData, nullptr, &hostTimeSigNumerator,
          &hostTimeSigDenominator, &hostCurrentMeasureDownBeat);
      jassert(timeErr == noErr);

      result.setPpqPositionOfLastBarStart(hostCurrentMeasureDownBeat);
      result.setTimeSignature(TimeSignature{(int)hostTimeSigNumerator,
                                            (int)hostTimeSigDenominator});
  ...
  Image getIcon(int size) {
...
#endif
    {
      if (interAppAudioConnected) {
        if (UIImage *hostUIImage =
                AudioOutputUnitGetHostIcon(audioUnit, (float)size))
  ...
  void switchApplication() {
    ...
    OSStatus err =
        AudioUnitGetProperty(audioUnit, kAudioUnitProperty_PeerURL,
  ...
  void handleStatusChange(bool enabled, const char *reason) {
    ...
    JUCE_IOS_AUDIO_LOG("handleStatusChange: enabled: "
                       << (int)enabled << ", reason: " << reason);
  ...
  void
  handleAudioUnitPropertyChange(AudioUnit, AudioUnitPropertyID propertyID,
                                [[maybe_unused]] AudioUnitScope scope,
                                [[maybe_unused]] AudioUnitElement element) {
    JUCE_IOS_AUDIO_LOG("handleAudioUnitPropertyChange: propertyID: "
                       << String(propertyID) << " scope: " << String(scope)
  ...
  void handleInterAppAudioConnectionChange() {
    ...
    [[maybe_unused]] OSStatus err =
        AudioUnitGetProperty(audioUnit, kAudioUnitProperty_IsInterAppConnected,
                             kAudioUnitScope_Global, 0, &connected, &dataSize);
    jassert(err == noErr);

    JUCE_IOS_AUDIO_LOG("handleInterAppAudioConnectionChange: "
  ...
  //==============================================================================
  OSStatus process(AudioUnitRenderActionFlags *flags,
                   const AudioTimeStamp *time, const UInt32 numFrames,
    ...
    if (stl.isLocked() && callback != nullptr) {
      ...
      float *const *const inputData =
          channelData.audioData.getArrayOfWritePointers();
      float *const *const outputData =
          inputData + channelData.inputs->numActiveChannels;

      if (useInput) {
        for (int c = 0; c < channelData.inputs->numActiveChannels; ++c) {
          auto channelIndex = channelData.inputs->activeChannelIndices[c];
          memcpy(inputData[c], (float *)data->mBuffers[channelIndex].mData,
                 channelDataSize);
        }
      } else {
        for (int c = 0; c < channelData.inputs->numActiveChannels; ++c)
      ...
      const auto nanos = time != nullptr
                             ? timeConversions.hostTimeToNanos(time->mHostTime)
                             : 0;

      callback->audioDeviceIOCallbackWithContext(
          (const float **)inputData, channelData.inputs->numActiveChannels,
          outputData, channelData.outputs->numActiveChannels, (int)numFrames,
          {(time != nullptr &&
            (time->mFlags & kAudioTimeStampHostTimeValid) != 0)
               ? &nanos
               : nullptr});

      for (int c = 0; c < channelData.outputs->numActiveChannels; ++c) {
        auto channelIndex = channelData.outputs->activeChannelIndices[c];
        memcpy(data->mBuffers[channelIndex].mData, outputData[c],
               channelDataSize);
      }
  ...
  void recordXruns(const AudioTimeStamp *time, UInt32 numFrames) {
    if (time != nullptr &&
        (time->mFlags & kAudioTimeStampSampleTimeValid) != 0) {
      if (!firstHostTime) {
        if (!approximatelyEqual((time->mSampleTime - lastSampleTime),
                                (double)lastNumFrames))
          xrun++;
      } else
        firstHostTime = false;
  ...
  //==============================================================================
  static OSStatus processStatic(void *client, AudioUnitRenderActionFlags *flags,
  ...
  bool createAudioUnit() {
    ...
    OSStatus err = AudioOutputUnitPublish(&appDesc, CFSTR(JucePlugin_IAAName),
                                          JucePlugin_VersionCode, audioUnit);
    ...
    err = AudioUnitSetProperty(
        audioUnit, kAudioOutputUnitProperty_MIDICallbacks,
        kAudioUnitScope_Global, 0, &midiCallbacks, sizeof(midiCallbacks));
    jassert(err == noErr);
#endif
...

    {
      ...
      format.mChannelsPerFrame =
          (UInt32)jmax(channelData.inputs->numHardwareChannels,
    ...
    {
      // Querying the kAudioUnitProperty_MaximumFramesPerSlice property after
      // calling AudioUnitInitialize seems to be more reliable than calling it
      // before.
  ...
  void fillHostCallbackInfo(HostCallbackInfo &callbackInfo) {
    ...
    [[maybe_unused]] OSStatus err = AudioUnitGetProperty(
        audioUnit, kAudioUnitProperty_HostCallbacks, kAudioUnitScope_Global, 0,
        &callbackInfo, &dataSize);
    jassert(err == noErr);
  }

  void handleAudioTransportEvent(AudioUnitRemoteControlEvent event) {
    [[maybe_unused]] OSStatus err = AudioUnitSetProperty(
        audioUnit, kAudioOutputUnitProperty_RemoteControlToHost,
        kAudioUnitScope_Global, 0, &event, sizeof(event));
    jassert(err == noErr);
  }

  // If the routing is set to go through the receiver (i.e. the speaker, but
  // quiet), this re-routes it to make it loud. Needed because by default when
  // using an input + output, the output is kept quiet.
  ...
  void handleAsyncUpdate() override { restart(); }
  ...
  void handleStreamFormatChange() {
    ...
    AudioUnitGetProperty(audioUnit, kAudioUnitProperty_StreamFormat,
                         kAudioUnitScope_Output, 0, &desc, &dataSize);

    if (!approximatelyEqual(desc.mSampleRate, 0.0) &&
        !approximatelyEqual(desc.mSampleRate, sampleRate)) {
      JUCE_IOS_AUDIO_LOG("Stream format has changed: Sample rate "
                         << desc.mSampleRate);
      triggerAsyncUpdate();
    }
  }

  static void dispatchAudioUnitPropertyChange(void *data, AudioUnit unit,
                                              AudioUnitPropertyID propertyID,
                                              AudioUnitScope scope,
                                              AudioUnitElement element) {
    static_cast<Pimpl *>(data)->handleAudioUnitPropertyChange(unit, propertyID,
                                                              scope, element);
  }
  ...
  static void midiEventCallback(void *client, UInt32 status, UInt32 data1,
                                UInt32 data2, UInt32) {
    return static_cast<Pimpl *>(client)->handleMidiMessage(MidiMessage(
        (int)status, (int)data1, (int)data2, getTimestampForMIDI()));
  }

  static void midiSysExCallback(void *client, const UInt8 *data,
                                UInt32 length) {
    return static_cast<Pimpl *>(client)->handleMidiMessage(
        MidiMessage(data, (int)length, getTimestampForMIDI()));
  }
  ...
  struct IOChannelData {
    ...
    public:
      IOChannelConfig(const bool isInput, const BigInteger requiredChannels)
          : hardwareChannelNames(getHardwareChannelNames(isInput)),
            ...
            areChannelsAccessible(
                (!isInput) || [AVAudioSession sharedInstance].isInputAvailable),
            activeChannels(limitRequiredChannelsToHardware(numHardwareChannels,
                                                           requiredChannels)),
            numActiveChannels(activeChannels.countNumberOfSetBits()),
            activeChannelIndices(getActiveChannelIndices(activeChannels)),
            inactiveChannelIndices(getInactiveChannelIndices(
                activeChannelIndices, numHardwareChannels)) {
#if JUCE_IOS_AUDIO_LOGGING
        {
          String info;
          ...
          info << ", Are channels available: "
               << (areChannelsAccessible ? "yes" : "no")
          ...
          JUCE_IOS_AUDIO_LOG((isInput ? "Input" : "Output")
    ...
    private:
      static StringArray getHardwareChannelNames(const bool isInput) {
        ...
        for (AVAudioSessionPortDescription *port in (isInput ? route.inputs
                                                             : route.outputs)) {
      ...
      static BigInteger
      limitRequiredChannelsToHardware(const int numHardwareChannelsAvailable,
                                      BigInteger requiredChannels) {
        requiredChannels.setRange(numHardwareChannelsAvailable,
                                  requiredChannels.getHighestBit() + 1, false);

        return requiredChannels;
      }

      static Array<int>
      getActiveChannelIndices(const BigInteger activeChannelsToIndex) {
        Array<int> result;
      ...
      static Array<int>
      getInactiveChannelIndices(const Array<int> &activeIndices,
                                int numChannels) {
        Array<int> result;
        ...
        for (int i = 0; i < numChannels; ++i)
          if (nextActiveChannel != activeIndices.end() &&
              i == *nextActiveChannel)
            ++nextActiveChannel;
          else
            result.add(i);
    ...
    int getFloatBufferSize() const { return audioData.getNumSamples(); }
...
//==============================================================================
iOSAudioIODevice::iOSAudioIODevice(iOSAudioIODeviceType *ioDeviceType,
                                   const String &, const String &)
    : AudioIODevice(iOSAudioDeviceName, iOSAudioDeviceName),
      pimpl(new Pimpl(ioDeviceType, *this)) {}

//==============================================================================
String iOSAudioIODevice::open(const BigInteger &inChans,
                              const BigInteger &outChans,
                              double requestedSampleRate,
                              int requestedBufferSize) {
  return pimpl->open(inChans, outChans, requestedSampleRate,
                     requestedBufferSize);
}

void iOSAudioIODevice::close() { pimpl->close(); }

void iOSAudioIODevice::start(AudioIODeviceCallback *callbackToUse) {
...
bool iOSAudioIODevice::setAudioPreprocessingEnabled(bool enabled) {
...
StringArray iOSAudioIODevice::getInputChannelNames() {
  return pimpl->channelData.inputs->areChannelsAccessible
...
int iOSAudioIODevice::getInputLatencyInSamples() {
  return roundToInt(pimpl->sampleRate *
...
AudioWorkgroup iOSAudioIODevice::getWorkgroup() const {
  return pimpl->workgroup;
}

void iOSAudioIODevice::setMidiMessageCollector(
    MidiMessageCollector *collector) {
  pimpl->messageCollector = collector;
}
AudioPlayHead *iOSAudioIODevice::getAudioPlayHead() const {
  return &pimpl->playhead;
}

bool iOSAudioIODevice::isInterAppAudioConnected() const {
...
StringArray iOSAudioIODeviceType::getDeviceNames(bool) const {
  return {iOSAudioDeviceName};
}
int iOSAudioIODeviceType::getDefaultDeviceIndex(bool) const { return 0; }
int iOSAudioIODeviceType::getIndexOfDevice(AudioIODevice *, bool) const {
  return 0;
}
bool iOSAudioIODeviceType::hasSeparateInputsAndOutputs() const { return false; }

AudioIODevice *
iOSAudioIODeviceType::createDevice(const String &outputDeviceName,
...
void iOSAudioIODeviceType::handleAsyncUpdate() { callDeviceChangeListeners(); }
...
void AudioSessionHolder::handleStatusChange(bool enabled,
                                            const char *reason) const {
  for (auto device : activeDevices)
    device->handleStatusChange(enabled, reason);
}

void AudioSessionHolder::handleRouteChange(
