...
static bool checkError(OSStatus err, [[maybe_unused]] int lineNum) {
  ...
  Logger::writeToLog("CoreMIDI error: " + String(lineNum) + " - " +
                     String::toHexString((int)err));
#endif
...
enum class ImplementationStrategy { onlyNew, both, onlyOld };
...
struct SenderBase {
  ...
  virtual void send(MIDIPortRef port, MIDIEndpointRef endpoint,
                    const ump::BytestreamMidiView &m) = 0;
  virtual void send(MIDIPortRef port, MIDIEndpointRef endpoint, ump::Iterator b,
                    ump::Iterator e) = 0;
};

template <ImplementationStrategy> struct Sender;

#if JUCE_HAS_NEW_COREMIDI_API
template <>
struct API_AVAILABLE(macos(11.0),
                     ios(14.0)) Sender<ImplementationStrategy::onlyNew> final
    : public SenderBase {
  void send(MIDIPortRef port, MIDIEndpointRef endpoint,
            const ump::BytestreamMidiView &m) override {
    newSendImpl(port, endpoint, m);
  }

  void send(MIDIPortRef port, MIDIEndpointRef endpoint, ump::Iterator b,
...
private:
  ...
  static ump::PacketProtocol
  getProtocolForEndpoint(MIDIEndpointRef ep) noexcept {
    SInt32 protocol = 0;
    CHECK_ERROR(
        MIDIObjectGetIntegerProperty(ep, kMIDIPropertyProtocolID, &protocol));
  ...
  void newSendImpl(MIDIPortRef port, MIDIEndpointRef endpoint,
                   Params &&...params) {
#if JUCE_IOS
    ...
    const auto init = [&] {
      // At the moment, we can only send MIDI 1.0 protocol. If the device is
      // using MIDI 2.0 protocol (as may be the case for the IAC driver), we
      // trust in the system to translate it.
    ...
    const auto add = [&](const ump::View &view) {
      ...
      end = MIDIEventListAdd(&stackList, sizeof(MIDIEventList::packet), end,
                             timeStamp, view.size(),
    ...
    ump::GenericUMPConverter::convertImpl(
        umpConverter, params..., [&](const auto &v) {
...
struct Sender<ImplementationStrategy::onlyOld> final : public SenderBase {
  ...
  void send(MIDIPortRef port, MIDIEndpointRef endpoint, ump::Iterator b,
    ...
    std::for_each(b, e, [&](const ump::View &v) {
      bytestreamConverter.convert(
          v, 0.0,
          [&](const ump::BytestreamMidiView &m) { send(port, endpoint, m); });
    });
  }
...
private:
  ...
  void oldSendImpl(MIDIPortRef port, MIDIEndpointRef endpoint,
    ...
    } else if (dataSize < 65536) // max packet size
    {
      ...
      if (dataSize > stackCapacity) {
        allocatedPackets.malloc(
            (sizeof(MIDIPacketList) - stackCapacity) + dataSize, 1);
        packetToSend = allocatedPackets;
      }
...
template <> struct Sender<ImplementationStrategy::both> {
  Sender() : sender(makeImpl()) {}
...
template <typename Resource, typename Destructor> class ScopedMidiResource {
public:
  ...
  void swap(ScopedMidiResource &other) noexcept {
...
using ScopedEndpointRef =
    ScopedMidiResource<MIDIEndpointRef, EndpointRefDestructor>;
...
public:
  ...
  ~MidiPortAndEndpoint() noexcept {
    // if port != 0, it means we didn't create the endpoint, so it's not safe to
    // delete it
    if (*port != 0)
      endpoint.release();
  }
...
static MidiDeviceInfo getMidiObjectInfo(MIDIObjectRef entity) {
...

  {
    ...
    if (CHECK_ERROR(MIDIObjectGetStringProperty(entity, kMIDIPropertyName,
                                                &str.object)))
  ...
  if (CHECK_ERROR(MIDIObjectGetIntegerProperty(entity, kMIDIPropertyUniqueID,
                                               &objectID))) {
    info.identifier = String(objectID);
  } else {
    CFObjectHolder<CFStringRef> str;

    if (CHECK_ERROR(MIDIObjectGetStringProperty(entity, kMIDIPropertyUniqueID,
                                                &str.object)))
...
static MidiDeviceInfo getEndpointInfo(MIDIEndpointRef endpoint,
                                      bool isExternal) {
  // NB: don't attempt to use nullptr for refs - it fails in some types of
  // build.
...
static MidiDeviceInfo getConnectedEndpointInfo(MIDIEndpointRef endpoint) {
  ...
  MIDIObjectGetDataProperty(endpoint, kMIDIPropertyConnectionUniqueID,
                            &connections.object);

  if (connections.object != nullptr) {
    numConnections =
        ((int)CFDataGetLength(connections.object)) / (int)sizeof(MIDIUniqueID);

    if (numConnections > 0) {
      auto *pid = reinterpret_cast<const SInt32 *>(
          CFDataGetBytePtr(connections.object));
...
static int createUniqueIDForMidiPort(String deviceName, bool isInput) {
  ...
  CFUniquePtr<CFStringRef> appBundlePath(
      appBundle.getFullPathName().toCFString());

  if (auto bundleURL = CFUniquePtr<CFURLRef>(CFURLCreateWithFileSystemPath(
          kCFAllocatorDefault, appBundlePath.get(), kCFURLPOSIXPathStyle,
          true)))
    if (auto bundleRef = CFUniquePtr<CFBundleRef>(
...
static void globalSystemChangeCallback(const MIDINotification *notification,
                                       void *) {
  if (notification != nullptr &&
      notification->messageID == kMIDIMsgSetupChanged)
...
static MIDIClientRef getGlobalMidiClient() {
  static const auto globalMidiClient = [&] {
    ...
    CHECK_ERROR(MIDIClientCreate(name.get(), globalSystemChangeCallback,
                                 nullptr, &result));
    return result;
  }();
...
static Array<MidiDeviceInfo> findDevices(bool forInput) {
  ...
  auto numDevices =
      (forInput ? MIDIGetNumberOfSources() : MIDIGetNumberOfDestinations());
...
//==============================================================================
template <ImplementationStrategy> struct Receiver;

#if JUCE_HAS_NEW_COREMIDI_API
template <> struct Receiver<ImplementationStrategy::onlyNew> {
  Receiver(ump::PacketProtocol protocol, ump::Receiver &receiver)
      : u32InputHandler(
            std::make_unique<ump::U32ToUMPHandler>(protocol, receiver)) {}

  Receiver(MidiInput &input, MidiInputCallback &callback)
      : u32InputHandler(
            std::make_unique<ump::U32ToBytestreamHandler>(input, callback)) {}
  ...
  void dispatch(const MIDIEventList *list, double time) const {
    ...
    for (uint32_t i = 0; i < list->numPackets; ++i) {
      static_assert(sizeof(uint32_t) == sizeof(UInt32) &&
                        alignof(uint32_t) == alignof(UInt32),
                    "If this fails, the cast below will be broken too!");
      u32InputHandler->pushMidiData(
          reinterpret_cast<const uint32_t *>(packet->words),
...
#if JUCE_HAS_OLD_COREMIDI_API
template <> struct Receiver<ImplementationStrategy::onlyOld> {
  Receiver(ump::PacketProtocol protocol, ump::Receiver &receiver)
      : bytestreamInputHandler(
            std::make_unique<ump::BytestreamToUMPHandler>(protocol, receiver)) {
  }

  Receiver(MidiInput &input, MidiInputCallback &callback)
      : bytestreamInputHandler(
            std::make_unique<ump::BytestreamToBytestreamHandler>(input,
                                                                 callback)) {}
...
#if JUCE_HAS_NEW_COREMIDI_API && JUCE_HAS_OLD_COREMIDI_API
template <> struct Receiver<ImplementationStrategy::both> {
  Receiver(ump::PacketProtocol protocol, ump::Receiver &receiver)
...
public:
  ...
  template <typename EventList> void handlePackets(const EventList *list) {
...
static Array<MIDIEndpointRef> getEndpoints(bool isInput) {
  ...
  auto numDevices =
      (isInput ? MIDIGetNumberOfSources() : MIDIGetNumberOfDestinations());
...
struct CreatorFunctionPointers {
  OSStatus (*createInputPort)(ump::PacketProtocol protocol,
                              MIDIClientRef client, CFStringRef portName,
                              void *refCon, MIDIPortRef *outPort);

  OSStatus (*createDestination)(ump::PacketProtocol protocol,
                                MIDIClientRef client, CFStringRef name,
                                void *refCon, MIDIEndpointRef *outDest);

  OSStatus (*createSource)(ump::PacketProtocol protocol, MIDIClientRef client,
                           CFStringRef name, MIDIEndpointRef *outSrc);
};

template <ImplementationStrategy> struct CreatorFunctions;

#if JUCE_HAS_NEW_COREMIDI_API
template <>
struct API_AVAILABLE(macos(11.0), ios(14.0))
    CreatorFunctions<ImplementationStrategy::onlyNew> {
  static OSStatus createInputPort(ump::PacketProtocol protocol,
                                  MIDIClientRef client, CFStringRef portName,
                                  void *refCon, MIDIPortRef *outPort) {
    return MIDIInputPortCreateWithProtocol(
        client, portName, convertToPacketProtocol(protocol), outPort,
  ...
  static OSStatus createDestination(ump::PacketProtocol protocol,
                                    MIDIClientRef client, CFStringRef name,
                                    void *refCon, MIDIEndpointRef *outDest) {
    return MIDIDestinationCreateWithProtocol(
        client, name, convertToPacketProtocol(protocol), outDest,
        ^void(const MIDIEventList *l, void *src) {
          newMidiInputProc(l, refCon, src);
        });
  }
  ...
  static OSStatus createSource(ump::PacketProtocol protocol,
    ...
    return MIDISourceCreateWithProtocol(
        client, name, convertToPacketProtocol(protocol), outSrc);
  }
...
private:
  static constexpr MIDIProtocolID
  convertToPacketProtocol(ump::PacketProtocol p) {
...
#if JUCE_HAS_OLD_COREMIDI_API
template <> struct CreatorFunctions<ImplementationStrategy::onlyOld> {
  static OSStatus createInputPort(ump::PacketProtocol, MIDIClientRef client,
                                  CFStringRef portName, void *refCon,
                                  MIDIPortRef *outPort) {
    return MIDIInputPortCreate(client, portName, oldMidiInputProc, refCon,
                               outPort);
  }

  static OSStatus createDestination(ump::PacketProtocol, MIDIClientRef client,
                                    CFStringRef name, void *refCon,
                                    MIDIEndpointRef *outDest) {
    return MIDIDestinationCreate(client, name, oldMidiInputProc, refCon,
                                 outDest);
  }

  static OSStatus createSource(ump::PacketProtocol, MIDIClientRef client,
                               CFStringRef name, MIDIEndpointRef *outSrc) {
...
private:
  static void oldMidiInputProc(const MIDIPacketList *list, void *readProcRefCon,
                               void *) {
    static_cast<MidiPortAndCallback *>(readProcRefCon)->handlePackets(list);
  }
};
...
#if JUCE_HAS_NEW_COREMIDI_API && JUCE_HAS_OLD_COREMIDI_API
template <> struct CreatorFunctions<ImplementationStrategy::both> {
  static OSStatus createInputPort(ump::PacketProtocol protocol,
                                  MIDIClientRef client, CFStringRef portName,
                                  void *refCon, MIDIPortRef *outPort) {
    return getCreatorFunctionPointers().createInputPort(
        protocol, client, portName, refCon, outPort);
  }

  static OSStatus createDestination(ump::PacketProtocol protocol,
                                    MIDIClientRef client, CFStringRef name,
                                    void *refCon, MIDIEndpointRef *outDest) {
    return getCreatorFunctionPointers().createDestination(
        protocol, client, name, refCon, outDest);
  }

  static OSStatus createSource(ump::PacketProtocol protocol,
                               MIDIClientRef client, CFStringRef name,
                               MIDIEndpointRef *outSrc) {
    return getCreatorFunctionPointers().createSource(protocol, client, name,
                                                     outSrc);
  }
  ...
  static CreatorFunctionPointers getCreatorFunctionPointers() {
    if (@available(macOS 11, iOS 14, *))
      return CreatorFunctions<
          ImplementationStrategy::onlyNew>::getCreatorFunctionPointers();

    return CreatorFunctions<
        ImplementationStrategy::onlyOld>::getCreatorFunctionPointers();
  }
};
#endif
...
public:
  ...
  static std::unique_ptr<Pimpl> makePimpl(MidiInput &midiInput,
    ...
    return std::make_unique<Pimpl>(
        midiInput, CoreMidiHelpers::ReceiverToUse(packetProtocol, umpReceiver));
  }

  static std::unique_ptr<Pimpl>
  makePimpl(MidiInput &midiInput, MidiInputCallback *midiInputCallback) {
    if (midiInputCallback == nullptr)
      return {};

    return std::make_unique<Pimpl>(
        midiInput,
        CoreMidiHelpers::ReceiverToUse(midiInput, *midiInputCallback));
  }

  template <typename... Args>
  static std::unique_ptr<MidiInput>
  makeInput(const String &name, const String &identifier, Args &&...args) {
    using namespace CoreMidiHelpers;

    if (auto midiInput = rawToUniquePtr(new MidiInput(name, identifier))) {
      if ((midiInput->internal =
               makePimpl(*midiInput, std::forward<Args>(args)...))) {
        const ScopedLock sl(callbackLock);
  ...
  static std::unique_ptr<MidiInput> openDevice(ump::PacketProtocol protocol,
    ...
    if (auto client = getGlobalMidiClient()) {
      for (auto &endpoint : getEndpoints(true)) {
        ...
        if (!CHECK_ERROR(MIDIObjectGetStringProperty(
                endpoint, kMIDIPropertyName, &cfName.object)))
          continue;

        if (auto input = makeInput(endpointInfo.name, endpointInfo.identifier,
                                   std::forward<Args>(args)...)) {
          MIDIPortRef port;

          if (!CHECK_ERROR(CreatorFunctionsToUse::createInputPort(
                  protocol, client, cfName.object, input->internal.get(),
                  &port)))
            continue;
          ...
          if (!CHECK_ERROR(
                  MIDIPortConnectSource(*scopedPort, endpoint, nullptr)))
            continue;

          input->internal->portAndEndpoint =
              std::make_unique<MidiPortAndEndpoint>(
                  std::move(scopedPort), ScopedEndpointRef{endpoint});
          return input;
        }
      }
    }
  ...
  static std::unique_ptr<MidiInput> createDevice(ump::PacketProtocol protocol,
    ...
    if (auto client = getGlobalMidiClient()) {
      ...
      if (auto input = makeInput(deviceName, String(deviceIdentifier),
                                 std::forward<Args>(args)...)) {
        MIDIEndpointRef endpoint;
        CFUniquePtr<CFStringRef> name(deviceName.toCFString());

        auto err = CreatorFunctionsToUse::createDestination(
            protocol, client, name.get(), input->internal.get(), &endpoint);
        ScopedEndpointRef scopedEndpoint{endpoint};

#if JUCE_IOS
        if (err == kMIDINotPermitted) {
          // If you've hit this assertion then you probably haven't enabled the
          // "Audio Background Capability" setting in the iOS exporter for your
          // app - this is required if you want to create a MIDI device!
          jassertfalse;
          return {};
        }
#endif
        ...
        if (!CHECK_ERROR(MIDIObjectSetIntegerProperty(
                endpoint, kMIDIPropertyUniqueID, (SInt32)deviceIdentifier)))
          return {};

        input->internal->portAndEndpoint =
            std::make_unique<MidiPortAndEndpoint>(ScopedPortRef{},
...
std::unique_ptr<MidiInput> MidiInput::openDevice(const String &deviceIdentifier,
  ...
  return Pimpl::openDevice(ump::PacketProtocol::MIDI_1_0, deviceIdentifier,
                           callback);
}

std::unique_ptr<MidiInput>
MidiInput::createNewDevice(const String &deviceName,
                           MidiInputCallback *callback) {
  return Pimpl::createDevice(ump::PacketProtocol::MIDI_1_0, deviceName,
                             callback);
}
...
std::unique_ptr<MidiInput> MidiInput::openDevice(int index,
                                                 MidiInputCallback *callback) {
...
MidiOutput::openDevice(const String &deviceIdentifier) {
  if (deviceIdentifier.isEmpty())
    return {};
  ...
  if (auto client = getGlobalMidiClient()) {
    for (auto &endpoint : getEndpoints(false)) {
      ...
      if (!CHECK_ERROR(MIDIObjectGetStringProperty(endpoint, kMIDIPropertyName,
                                                   &cfName.object)))
        continue;
      ...
      auto midiOutput = rawToUniquePtr(
          new MidiOutput(endpointInfo.name, endpointInfo.identifier));
...
std::unique_ptr<MidiOutput>
MidiOutput::createNewDevice(const String &deviceName) {
  using namespace CoreMidiHelpers;
  ...
  if (auto client = getGlobalMidiClient()) {
    ...
    auto err = CreatorFunctionsToUse::createSource(
        ump::PacketProtocol::MIDI_1_0, client, name.get(), &endpoint);
    ScopedEndpointRef scopedEndpoint{endpoint};

#if JUCE_IOS
    if (err == kMIDINotPermitted) {
      // If you've hit this assertion then you probably haven't enabled the
      // "Audio Background Capability" setting in the iOS exporter for your app
      // - this is required if you want to create a MIDI device!
      jassertfalse;
      return {};
    }
#endif
    ...
    if (!CHECK_ERROR(MIDIObjectSetIntegerProperty(
            *scopedEndpoint, kMIDIPropertyUniqueID, (SInt32)deviceIdentifier)))
      return {};

    auto midiOutput =
        rawToUniquePtr(new MidiOutput(deviceName, String(deviceIdentifier)));
    midiOutput->internal =
        std::make_unique<Pimpl>(ScopedPortRef{}, std::move(scopedEndpoint));
...
MidiOutput::~MidiOutput() { stopBackgroundThread(); }
...
MidiDeviceListConnection::make(std::function<void()> cb) {
