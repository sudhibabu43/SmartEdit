...
#define JUCE_DECL_JACK_FUNCTION(return_type, fn_name, argument_types,          \
                                arguments)                                     \
...
//==============================================================================
JUCE_DECL_JACK_FUNCTION(jack_client_t *, jack_client_open,
                        (const char *client_name, jack_options_t options,
                         jack_status_t *status, ...),
                        (client_name, options, status))
JUCE_DECL_JACK_FUNCTION(int, jack_client_close, (jack_client_t * client),
                        (client))
JUCE_DECL_JACK_FUNCTION(int, jack_activate, (jack_client_t * client), (client))
JUCE_DECL_JACK_FUNCTION(int, jack_deactivate, (jack_client_t * client),
                        (client))
JUCE_DECL_JACK_FUNCTION(jack_nframes_t, jack_get_buffer_size,
                        (jack_client_t * client), (client))
JUCE_DECL_JACK_FUNCTION(jack_nframes_t, jack_get_sample_rate,
                        (jack_client_t * client), (client))
...
JUCE_DECL_VOID_JACK_FUNCTION(jack_on_info_shutdown,
                             ...
                             (client, function, arg))
JUCE_DECL_JACK_FUNCTION(void *, jack_port_get_buffer,
...
JUCE_DECL_JACK_FUNCTION(jack_nframes_t, jack_port_get_total_latency,
                        ...
                        (client, port))
JUCE_DECL_JACK_FUNCTION(jack_port_t *, jack_port_register,
...
JUCE_DECL_JACK_FUNCTION(int, jack_set_process_callback,
                        (jack_client_t * client,
                         JackProcessCallback process_callback, void *arg),
                        (client, process_callback, arg))
JUCE_DECL_JACK_FUNCTION(const char **, jack_get_ports,
                        (jack_client_t * client, const char *port_name_pattern,
                         const char *type_name_pattern, unsigned long flags),
                        (client, port_name_pattern, type_name_pattern, flags))
JUCE_DECL_JACK_FUNCTION(int, jack_connect,
                        (jack_client_t * client, const char *source_port,
                         const char *destination_port),
                        (client, source_port, destination_port))
JUCE_DECL_JACK_FUNCTION(const char *, jack_port_name, (const jack_port_t *port),
                        (port))
JUCE_DECL_JACK_FUNCTION(void *, jack_set_port_connect_callback,
                        (jack_client_t * client,
                         JackPortConnectCallback connect_callback, void *arg),
                        (client, connect_callback, arg))
JUCE_DECL_JACK_FUNCTION(jack_port_t *, jack_port_by_id,
                        (jack_client_t * client, jack_port_id_t port_id),
                        (client, port_id))
JUCE_DECL_JACK_FUNCTION(int, jack_port_connected, (const jack_port_t *port),
                        (port))
JUCE_DECL_JACK_FUNCTION(int, jack_port_connected_to,
                        (const jack_port_t *port, const char *port_name),
                        (port, port_name))
JUCE_DECL_JACK_FUNCTION(int, jack_set_xrun_callback,
                        (jack_client_t * client, JackXRunCallback xrun_callback,
                         void *arg),
                        (client, xrun_callback, arg))
JUCE_DECL_JACK_FUNCTION(int, jack_port_flags, (const jack_port_t *port), (port))
JUCE_DECL_JACK_FUNCTION(jack_port_t *, jack_port_by_name,
...
namespace {
void jack_Log(const String &s) { std::cerr << s << std::endl; }
...
struct JackPortIterator {
  JackPortIterator(jack_client_t *const client, const bool forInput) {
    if (client != nullptr)
      ports.reset(
          juce::jack_get_ports(client, nullptr, nullptr,
...
public:
  JackAudioIODevice(const String &inName, const String &outName,
    ...
    jack_status_t status = {};
    client = juce::jack_client_open(JUCE_JACK_CLIENT_NAME, JackNoStartServer,
                                    &status);
    ...
    if (client == nullptr) {
      ...
      for (int i = 0; i < inputChannels.size(); ++i) {
        ...
        inputPorts.add(juce::jack_port_register(
            client, inputChannelName.toUTF8(), JACK_DEFAULT_AUDIO_TYPE,
            JackPortIsInput, 0));
      }
      ...
      for (int i = 0; i < outputChannels.size(); ++i) {
        ...
        outputPorts.add(juce::jack_port_register(
            client, outputChannelName.toUTF8(), JACK_DEFAULT_AUDIO_TYPE,
            JackPortIsOutput, 0));
      }
  ...
  int getCurrentBufferSizeSamples() override {
    return client != nullptr
               ? static_cast<int>(juce::jack_get_buffer_size(client))
               : 0;
  }
  double getCurrentSampleRate() override {
    return client != nullptr
               ? static_cast<int>(juce::jack_get_sample_rate(client))
               : 0;
  }
  ...
  String open(const BigInteger &inputChannels, const BigInteger &outputChannels,
    ...
    if (!inputChannels.isZero()) {
      forEachClientChannel(
          inputName, false, [&](const char *portName, int index) {
            ...
            jassert(
                juce::jack_port_flags(juce::jack_port_by_name(client, source)) &
                JackPortIsOutput);
            jassert(juce::jack_port_flags(inputPort) & JackPortIsInput);

            auto error = juce::jack_connect(client, source,
                                            juce::jack_port_name(inputPort));
            if (error != 0)
              JUCE_JACK_LOG("Cannot connect input port " + String(index) +
                            " (" + portName + "), error " + String(error));
          });
    }

    if (!outputChannels.isZero()) {
      forEachClientChannel(
          outputName, true, [&](const char *portName, int index) {
            ...
            jassert(juce::jack_port_flags(
                        juce::jack_port_by_name(client, destination)) &
                    JackPortIsInput);

            auto error = juce::jack_connect(
                client, juce::jack_port_name(outputPort), destination);
            if (error != 0)
              JUCE_JACK_LOG("Cannot connect output port " + String(index) +
                            " (" + portName + "), error " + String(error));
          });
    }
  ...
  void close() override {
    ...
    if (client != nullptr) {
      ...
      juce::jack_set_port_connect_callback(client, portConnectCallback,
                                           nullptr);
  ...
  int getOutputLatencyInSamples() override {
    ...
    for (int i = 0; i < outputPorts.size(); i++)
      latency =
          jmax(latency,
               (int)juce::jack_port_get_total_latency(client, outputPorts[i]));
  ...
  int getInputLatencyInSamples() override {
    ...
    for (int i = 0; i < inputPorts.size(); i++)
      latency =
          jmax(latency,
               (int)juce::jack_port_get_total_latency(client, inputPorts[i]));
...
private:
  ...
  void process(const int numSamples) {
    ...
    for (int i = 0; i < totalNumberOfInputChannels; ++i) {
      if (activeInputChannels[i])
        if (auto *in =
                (jack_default_audio_sample_t *)juce::jack_port_get_buffer(
    ...
    if (callback != nullptr) {
      if ((numActiveInChans + numActiveOutChans) > 0)
        callback->audioDeviceIOCallbackWithContext(
            inChans.getData(), numActiveInChans, outChans, numActiveOutChans,
            numSamples, {});
    } else {
  ...
  static int processCallback(jack_nframes_t nframes, void *callbackArgument) {
    if (callbackArgument != nullptr)
      ((JackAudioIODevice *)callbackArgument)
          ->process(static_cast<int>(nframes));
  ...
  static void portConnectCallback(jack_port_id_t, jack_port_id_t, int,
                                  void *arg) {
  ...
  static void infoShutdownCallback([[maybe_unused]] jack_status_t code,
...
public:
  ...
  void scanForDevices() {
    ...
    // open a dummy client
    if (auto *const client = juce::jack_client_open(
            "JuceJackDummy", JackNoStartServer, &status)) {
      // scan for output devices
      for (JackPortIterator i(client, false); i.next();)
        if (i.getClientName() != (JUCE_JACK_CLIENT_NAME) &&
