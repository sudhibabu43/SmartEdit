...
/* The ASIO SDK *should* declare its callback functions as being __cdecl, but
   different versions seem to be pretty random about whether or not they do
   this. If you hit an error using these functions it'll be because you're
   trying to build using __stdcall, in which case you'd need to either get hold
   of an ASIO SDK which correctly specifies __cdecl, or add the __cdecl keyword
   to its functions yourself.
*/
...
#define JUCE_ASIO_LOG_ERROR(msg, errNum)                                       \
  ASIODebugging::logError((msg), (errNum))
...
struct ASIOSampleFormat {
  ...
  void convertToFloat(const void *src, float *dst, int samps) const noexcept {
    if (formatIsFloat) {
      ...
      switch (bitDepth) {
      case 16:
        convertInt16ToFloat(static_cast<const char *>(src), dst, byteStride,
                            samps, littleEndian);
        break;
      case 24:
        convertInt24ToFloat(static_cast<const char *>(src), dst, byteStride,
                            samps, littleEndian);
        break;
      case 32:
        convertInt32ToFloat(static_cast<const char *>(src), dst, byteStride,
                            samps, littleEndian);
        break;
      default:
        jassertfalse;
  ...
  void convertFromFloat(const float *src, void *dst, int samps) const noexcept {
    if (formatIsFloat) {
      ...
      switch (bitDepth) {
      case 16:
        convertFloatToInt16(src, static_cast<char *>(dst), byteStride, samps,
                            littleEndian);
        break;
      case 24:
        convertFloatToInt24(src, static_cast<char *>(dst), byteStride, samps,
                            littleEndian);
        break;
      case 32:
        convertFloatToInt32(src, static_cast<char *>(dst), byteStride, samps,
                            littleEndian);
        break;
      default:
        jassertfalse;
...
private:
  static void convertInt16ToFloat(const char *src, float *dest,
                                  int srcStrideBytes, int numSamples,
  ...
  static void convertFloatToInt16(const float *src, char *dest,
                                  int dstStrideBytes, int numSamples,
                                  bool littleEndian) noexcept {
    const double maxVal = (double)0x7fff;

    if (littleEndian) {
      while (--numSamples >= 0) {
        *(uint16 *)dest = ByteOrder::swapIfBigEndian((uint16)(short)roundToInt(
            jlimit(-maxVal, maxVal, maxVal * *src++)));
        dest += dstStrideBytes;
      }
    } else {
      while (--numSamples >= 0) {
        *(uint16 *)dest = ByteOrder::swapIfLittleEndian((
            uint16)(short)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)));
        dest += dstStrideBytes;
      }
    }
  }

  static void convertInt24ToFloat(const char *src, float *dest,
                                  int srcStrideBytes, int numSamples,
                                  bool littleEndian) noexcept {
  ...
  static void convertFloatToInt24(const float *src, char *dest,
                                  int dstStrideBytes, int numSamples,
                                  bool littleEndian) noexcept {
    ...
    if (littleEndian) {
      while (--numSamples >= 0) {
        ByteOrder::littleEndian24BitToChars(
            (uint32)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)), dest);
        dest += dstStrideBytes;
      }
    } else {
      while (--numSamples >= 0) {
  ...
  static void convertInt32ToFloat(const char *src, float *dest,
                                  int srcStrideBytes, int numSamples,
                                  bool littleEndian) noexcept {
  ...
  static void convertFloatToInt32(const float *src, char *dest,
                                  int dstStrideBytes, int numSamples,
                                  bool littleEndian) noexcept {
    ...
    if (littleEndian) {
      while (--numSamples >= 0) {
        *(uint32 *)dest = ByteOrder::swapIfBigEndian(
            (uint32)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)));
        dest += dstStrideBytes;
      }
    } else {
      while (--numSamples >= 0) {
        *(uint32 *)dest = ByteOrder::swapIfLittleEndian(
            (uint32)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)));
        dest += dstStrideBytes;
      }
    }
...
//==============================================================================
class ASIOAudioIODevice final : public AudioIODevice, private Timer {
public:
  ASIOAudioIODevice(ASIOAudioIODeviceType *ownerType, const String &devName,
                    CLSID clsID, int slotNumber)
      : AudioIODevice(devName, "ASIO"), owner(ownerType), classId(clsID) {
  ...
  void updateSampleRates() {
    ...
    if (newRates.isEmpty()) {
      ...
      JUCE_ASIO_LOG("No sample rates supported - current rate: " +
                    String((int)cr));
  ...
  String open(const BigInteger &inputChannels, const BigInteger &outputChannels,
    ...
    auto err =
        asioObject->getChannels(&totalNumInputChans, &totalNumOutputChans);
    jassert(err == ASE_OK);
    ...
    if (sampleRate == 0 ||
        (sampleRates.size() > 0 && !sampleRates.contains(sampleRate)))
    ...
    currentBlockSizeSamples = bufferSizeSamples =
        readBufferSizes(bufferSizeSamples);

    // (need to get this again in case a sample rate change affected the channel
    // count)
    ...
    JUCE_ASIO_LOG("creating buffers: " + String(totalBuffers) + ", " +
                  String(currentBlockSizeSamples));
    err = asioObject->createBuffers(bufferInfos, totalBuffers,
                                    currentBlockSizeSamples, &callbacks);
    ...
    if (err == ASE_OK) {
      ...
      for (int i = 0; i < (int)totalNumOutputChans; ++i) {
        if (outputChannels[i]) {
          outBuffers[n] = ioBufferSpace +
                          (currentBlockSizeSamples * (numActiveInputChans + n));
      ...
      for (int i = 0; i < numActiveOutputChans; ++i) {
        outputFormat[i].clear(bufferInfos[numActiveInputChans + i].buffers[0],
                              currentBlockSizeSamples);
        outputFormat[i].clear(bufferInfos[numActiveInputChans + i].buffers[1],
                              currentBlockSizeSamples);
      }
  ...
  void resetRequest() noexcept { startTimer(500); }
...
private:
  ...
  long minBufferSize = 0, maxBufferSize = 0, preferredBufferSize = 0,
       bufferGranularity = 0;
  ...
  static String convertASIOString(char *text, int length) {
    ...
    MultiByteToWideChar(CP_ACP, 0, text, length, wideVersion,
                        numElementsInArray(wideVersion));
    return wideVersion;
  }
  ...
  void reloadChannelNames() {
    ...
    if (asioObject != nullptr &&
        asioObject->getChannels(&totalInChannels, &totalOutChannels) ==
            ASE_OK) {
  ...
  long refreshBufferSizes() {
    const auto err =
        asioObject->getBufferSize(&minBufferSize, &maxBufferSize,
                                  &preferredBufferSize, &bufferGranularity);

    if (err == ASE_OK) {
      bufferSizes.clear();
      addBufferSizes(minBufferSize, maxBufferSize, preferredBufferSize,
                     bufferGranularity);
    }
  ...
  int readBufferSizes(int bufferSizeSamples) {
    ...
    if (asioObject->getBufferSize(&minBufferSize, &maxBufferSize,
                                  &newPreferredSize,
                                  &bufferGranularity) == ASE_OK) {
      if (preferredBufferSize != 0 && newPreferredSize != 0 &&
          newPreferredSize != preferredBufferSize)
        shouldUsePreferredSize = true;

      if (bufferSizeSamples < minBufferSize ||
          bufferSizeSamples > maxBufferSize)
        shouldUsePreferredSize = true;
    ...
    shouldUsePreferredSize =
        shouldUsePreferredSize || getName().containsIgnoreCase("Digidesign");
  ...
  void addBufferSizes(long minSize, long maxSize, long preferredSize,
                      long granularity) {
    ...
    if (granularity >= 0) {
      ...
      for (int i = jmax((int)(minSize + 15) & ~15, (int)granularity);
           i <= jmin(6400, (int)maxSize); i += granularity)
  ...
  void setSampleRate(double newRate) {
    if (currentSampleRate != newRate) {
      JUCE_ASIO_LOG("rate change: " + String(currentSampleRate) + " to " +
                    String(newRate));
      ...
      // on fail, ignore the attempt to change rate, and run with the current
      // one..
    }
  }
  ...
  void readLatencies() {
    ...
    if (asioObject->getLatencies(&inputLatency, &outputLatency) != 0)
      ...
      JUCE_ASIO_LOG("Latencies: in = " + String((int)inputLatency) +
                    ", out = " + String((int)outputLatency));
  }
  ...
  void createDummyBuffers(long preferredSize) {
    ...
    JUCE_ASIO_LOG("creating buffers (dummy): " + String(numChans) + ", " +
                  String((int)preferredSize));

    if (preferredSize > 0) {
      auto err = asioObject->createBuffers(bufferInfos, numChans, preferredSize,
                                           &callbacks);
    ...
    if (totalNumInputChans != newInps || totalNumOutputChans != newOuts) {
      ...
      JUCE_ASIO_LOG(String((int)totalNumInputChans) + " in; " +
                    String((int)totalNumOutputChans) + " out");
    }
...
#endif
    {
      return CoCreateInstance(classId, 0, CLSCTX_INPROC_SERVER, classId,
                              (void **)&asioObject) == S_OK;
    }
#if !JUCE_MINGW
    __except (EXCEPTION_EXECUTE_HANDLER) {
      crashed = true;
  ...
  String initDriver() {
    ...
    if (driverError.isEmpty()) {
      char buffer[512] = {};
      asioObject->getDriverName(
          buffer); // just in case any flimsy drivers expect this to be called..
    }
  ...
  String openDevice() {
    ...
    if (loadDriver()) {
      if ((error = initDriver()).isEmpty()) {
        ...
        if (asioObject != nullptr &&
            (err = asioObject->getChannels(&totalNumInputChans,
                                           &totalNumOutputChans)) == 0) {
          JUCE_ASIO_LOG(String((int)totalNumInputChans) + " in, " +
                        String((int)totalNumOutputChans) + " out");

          const int chansToAllocate =
              totalNumInputChans + totalNumOutputChans + 4;
          ...
          if ((err = refreshBufferSizes()) == 0) {
            ...
            updateSampleRates();
            readLatencies(); // ..doing these steps because cubase does so at
                             // this stage
            createDummyBuffers(
                preferredBufferSize); // in initialisation, and some devices
                                      // fail if we don't.
            readLatencies();

            // start and stop because cubase does it..
            err = asioObject->start();
            // ignore an error here, as it might start later after setting other
            // stuff up
    ...
    if (bufferIndex >= 0) {
      ...
      if (currentCallback != nullptr) {
        ...
        currentCallback->audioDeviceIOCallbackWithContext(
            inBuffers.getData(), numActiveInputChans, outBuffers,
            numActiveOutputChans, samps, {});

        for (int i = 0; i < numActiveOutputChans; ++i) {
          jassert(outBuffers[i] != nullptr);
          outputFormat[i].convertFromFloat(
              outBuffers[i],
              infos[numActiveInputChans + i].buffers[bufferIndex], samps);
        }
      } else {
        for (int i = 0; i < numActiveOutputChans; ++i)
          outputFormat[i].clear(
              infos[numActiveInputChans + i].buffers[bufferIndex], samps);
      }
    }
  ...
  long asioMessagesCallback(long selector, long value) {
    ...
    case kAsioSelectorSupported:
      if (value == kAsioResetRequest || value == kAsioEngineVersion ||
          ...
          value == kAsioSupportsInputMonitor || value == kAsioOverload)
        return 1;
      break;
  ...
  template <int deviceIndex> struct ASIOCallbackFunctions {
    static ASIOTime *JUCE_ASIOCALLBACK bufferSwitchTimeInfoCallback(ASIOTime *,
                                                                    long index,
                                                                    long) {
    ...
    static long JUCE_ASIOCALLBACK asioMessagesCallback(long selector,
                                                       long value, void *,
                                                       double *) {
      if (auto *d = currentASIODev[deviceIndex])
    ...
    static void setCallbacksForDevice(ASIOCallbacks &callbacks,
                                      ASIOAudioIODevice *device) noexcept {
...
template <> struct ASIOAudioIODevice::ASIOCallbackFunctions<maxNumASIODevices> {
  static void setCallbacksForDevice(ASIOCallbacks &,
                                    ASIOAudioIODevice *) noexcept {}
};
...
public:
  ...
  void scanForDevices() override {
    ...
    if (RegOpenKey(HKEY_LOCAL_MACHINE, _T ("software\\asio"), &hk) ==
        ERROR_SUCCESS) {
      TCHAR name[256] = {};

      while (RegEnumKey(hk, index++, name, numElementsInArray(name)) ==
             ERROR_SUCCESS) {
  ...
  int getDefaultDeviceIndex(bool) const override {
    ...
    if (deviceNames.size() > 1 &&
        deviceNames[0].containsIgnoreCase("digidesign"))
  ...
  AudioIODevice *createDevice(const String &outputDeviceName,
    ...
    // ASIO can't open two different devices for input and output - they must be
    // the same one.
    jassert(inputDeviceName == outputDeviceName || outputDeviceName.isEmpty() ||
            inputDeviceName.isEmpty());
    jassert(hasScanned); // need to call scanForDevices() before doing this
  ...
  void sendDeviceChangeToListeners() { callDeviceChangeListeners(); }
...
private:
      ...
      while (RegEnumKey(hk, index++, name, numElementsInArray(name)) ==
        ...
        if (classId.equalsIgnoreCase(name)) {
          ...
          if (RegOpenKeyEx(hk, name, 0, KEY_READ, &subKey) == ERROR_SUCCESS) {
            if (RegOpenKeyEx(subKey, _T ("InprocServer32"), 0, KEY_READ,
                             &pathKey) == ERROR_SUCCESS) {
              TCHAR pathName[1024] = {};
              DWORD dtype = REG_SZ;
              DWORD dsize = sizeof(pathName);

              if (RegQueryValueEx(pathKey, 0, 0, &dtype, (LPBYTE)pathName,
                                  &dsize) == ERROR_SUCCESS)
                // In older code, this used to check for the existence of the
                // file, but there are situations where our process doesn't have
                // access to it, but where the driver still loads ok..
                ok = (pathName[0] != 0);
  ...
  void addDriverInfo(const String &keyName, HKEY hk) {
    ...
    if (RegOpenKeyEx(hk, keyName.toWideCharPointer(), 0, KEY_READ, &subKey) ==
        ERROR_SUCCESS) {
      TCHAR buf[256] = {};
      DWORD dtype = REG_SZ;
      DWORD dsize = sizeof(buf);

      if (RegQueryValueEx(subKey, _T ("clsid"), 0, &dtype, (LPBYTE)buf,
                          &dsize) == ERROR_SUCCESS) {
        if (dsize > 0 && checkClassIsOk(buf)) {
          ...
          if (CLSIDFromString((LPOLESTR)buf, &classId) == S_OK) {
            ...
            if (RegQueryValueEx(subKey, _T ("description"), 0, &dtype,
                                (LPBYTE)buf, &dsize) == ERROR_SUCCESS)
              deviceName = buf;
            else
              deviceName = keyName;
