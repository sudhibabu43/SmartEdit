...
/**
 * The default number of nanoseconds to wait for when performing state change
 * operations on the stream, such as `start` and `stop`.
 *
...
class AudioStream : public AudioStreamBase {
  friend class AudioStreamBuilder; // allow access to setWeakThis() and
                                   // lockWeakThis()
public:
  AudioStream() {}
   ...
   *
   * AAudioStream_close() should be called at some point after calling this
   * function.
   *
   ...
   * thread.
   *
   * NOTE: This function is only fully implemented for MMAP streams, which are
   * low latency streams supported by some devices. On other "Legacy" streams
   * some audio resources will still be in use and some callbacks may still be
   * in process after this call.
   *
   * Available in AAudio since API level 30. Returns Result::ErrorUnimplemented
   * otherwise.
   *
   * * @return either Result::OK or an error.
   */
  virtual Result release() { return Result::ErrorUnimplemented; }
  ...
  /**
   * Start the stream. This will block until the stream has been started, an
   * error occurs or `timeoutNanoseconds` has been reached.
   */
  virtual Result start(int64_t timeoutNanoseconds = kDefaultTimeoutNanos);

  /**
   * Pause the stream. This will block until the stream has been paused, an
   * error occurs or `timeoutNanoseconds` has been reached.
   */
  virtual Result pause(int64_t timeoutNanoseconds = kDefaultTimeoutNanos);

  /**
   * Flush the stream. This will block until the stream has been flushed, an
   * error occurs or `timeoutNanoseconds` has been reached.
   */
  virtual Result flush(int64_t timeoutNanoseconds = kDefaultTimeoutNanos);

  /**
   * Stop the stream. This will block until the stream has been stopped, an
   * error occurs or `timeoutNanoseconds` has been reached.
   */
  ...
  /**
   * Start the stream asynchronously. Returns immediately (does not block).
   * Equivalent to calling `start(0)`.
   */
  virtual Result requestStart() = 0;

  /**
   * Pause the stream asynchronously. Returns immediately (does not block).
   * Equivalent to calling `pause(0)`.
   */
  virtual Result requestPause() = 0;

  /**
   * Flush the stream asynchronously. Returns immediately (does not block).
   * Equivalent to calling `flush(0)`.
   */
  virtual Result requestFlush() = 0;

  /**
   * Stop the stream asynchronously. Returns immediately (does not block).
   * Equivalent to calling `stop(0)`.
   */
   ...
   *
   * @param requestedFrames requested number of frames that can be filled
   * without blocking
   * @return the resulting buffer size in frames (obtained using value()) or an
   * error (obtained using error())
   */
  virtual ResultWithValue<int32_t>
  setBufferSizeInFrames(int32_t /* requestedFrames  */) {
   ...
   *
   * @return a result which is either Result::OK with the xRun count as the
   * value, or a Result::Error* code
   */
  ...
  /**
   * Query the number of frames that are read or written by the endpoint at one
   * time.
   *
   * @return burst size
   */
  ...
  /**
   * Get the number of bytes in each audio frame. This is calculated using the
   * channel count and the sample format. For example, a 2 channel floating
   * point stream will have 2 * 4 = 8 bytes per frame.
   *
  ...
  /**
   * Get the number of bytes per sample. This is calculated using the sample
   * format. For example, a stream using 16-bit integer samples will have 2
   * bytes per sample.
   *
   ...
   * app to some type of digital-to-analog converter. If the DAC is external,
   * for example in a USB interface or a TV connected by HDMI, then there may be
   * additional latency that the Android device is unaware of.
   *
   * Input latency is the time it takes to a given frame to travel from an
   * analog-to-digital converter (ADC) to the app.
   *
   * Note that the latency of an OUTPUT stream will increase abruptly when you
   * write data to it and then decrease slowly over time as the data is
   * consumed.
   *
   * The latency of an INPUT stream will decrease abruptly when you read data
   * from it and then increase slowly over time as more data arrives.
   *
   * The latency of an OUTPUT stream is generally higher than the INPUT latency
   * because an app generally tries to keep the OUTPUT buffer full and the INPUT
   * buffer empty.
   *
   ...
   *
   * @return a ResultWithValue which has a result of Result::OK and a value
   * containing the latency in milliseconds, or a result of Result::Error*.
   */
  ...
  /**
   * Get the estimated time that the frame at `framePosition` entered or left
   * the audio processing pipeline.
   *
   * This can be used to coordinate events and interactions with the external
   * environment, and to estimate the latency of an audio stream. An example of
   * usage can be found in the hello-oboe sample (search for
   * "calculateCurrentOutputLatencyMillis").
   *
   * The time is based on the implementation's best effort, using whatever
   * knowledge is available to the system, but cannot account for any delay
   ...
   * https://github.com/google/oboe/wiki/TechNote_ReleaseBuffer
   *
   * @deprecated since 1.0, use AudioStream::getTimestamp(clockid_t clockId)
   * instead, which returns ResultWithValue
   * @param clockId the type of clock to use e.g. CLOCK_MONOTONIC
   * @param framePosition the frame number to query
   * @param timeNanoseconds an output parameter which will contain the
   ...
   * See
   * @param clockId the type of clock to use e.g. CLOCK_MONOTONIC
   * @return a FrameTimestamp containing the position and time at which a
   * particular audio frame entered or left the audio processing pipeline, or an
  ...
  // ============== I/O ===========================
  /**
   * Write data from the supplied buffer into the stream. This method will block
   * until the write is complete or it runs out of time.
   *
   ...
   * completion.
   * @return a ResultWithValue which has a result of Result::OK and a value
   * containing the number of frames actually written, or result of
   * Result::Error*.
   */
  ...
  /**
   * Read data into the supplied buffer from the stream. This method will block
   * until the read is complete or it runs out of time.
   *
   * If `timeoutNanoseconds` is zero then this call will not wait.
   *
   * @param buffer The address of the first sample.
   ...
   * completion.
   * @return a ResultWithValue which has a result of Result::OK and a value
   * containing the number of frames actually read, or result of Result::Error*.
   */
   ...
   */
  bool usesAAudio() const { return getAudioApi() == AudioApi::AAudio; }
   ...
   */
  virtual void *getUnderlyingStream() const { return nullptr; }
  ...
  swapDataCallback(AudioStreamDataCallback *dataCallback) {
  ...
  swapErrorCallback(AudioStreamErrorCallback *errorCallback) {
   ...
   * This can be used with an EXCLUSIVE MMAP input stream to avoid reading data
   * too close to the DSP write position, which may cause glitches.
   *
   * Starting with Oboe 1.7.1, the numFrames will be clipped internally against
   * the BufferCapacity minus BurstSize. This is to prevent trying to wait for
   * more frames than could possibly be available. In this case, the return
   * value may be less than numFrames. Note that there may still be glitching if
   * numFrames is too high.
   *
  ...
  int32_t getDelayBeforeCloseMillis() const { return mDelayBeforeCloseMillis; }
   ...
   *
   * Note that this only has an effect when OboeGlobals::areWorkaroundsEnabled()
   * is true.
   *
   ...
   *
   * The flag will be checked in the Oboe data callback. If it transitions from
   * false to true then the PerformanceHint feature will be started. This only
   * needs to be called once.
   *
   * You may want to enable this if you have a dynamically changing workload
   * and you notice that you are getting underruns and glitches when your
   * workload increases. This might happen, for example, if you suddenly go from
   * playing one note to ten notes on a synthesizer.
   *
   * Try the CPU Load test in OboeTester if you would like to experiment with
   * this interactively.
   *
   ...
   * It does not tell you if the PerformanceHint feature is implemented or
   * active on the device.
   *
   * @return true if set using setPerformanceHintEnabled().
   */
  bool isPerformanceHintEnabled() { return mPerformanceHintEnabled; }

protected:
  /**
   * This is used to detect more than one error callback from a stream.
   * These were bugs in some versions of Android that caused multiple error
   * callbacks. Internal bug b/63087953
   *
   * Calling this sets an atomic<bool> true and returns the previous value.
   *
   * @return false on first call, true on subsequent calls
   */
  bool wasErrorCallbackCalled() { return mErrorCallbackCalled.exchange(true); }
   ...
   * @return result
   */
  virtual DataCallbackResult onDefaultCallback(void * /* audioData  */,
                                               int /* numFrames */) {
   ...
   *
   * @param audioData container array which audio frames will be written into or
   * read from
   ...
   */
  bool isDataCallbackEnabled() { return mDataCallbackEnabled; }

  /**
   * This can be set false internally to prevent callbacks
   * after DataCallbackResult::Stop has been returned.
   */
  void setDataCallbackEnabled(bool enabled) { mDataCallbackEnabled = enabled; }
  ...
  /**
   * This will be called when the stream is closed just in case performance
   * hints were enabled.
   */
   ...
   */
  std::shared_ptr<oboe::AudioStream> lockWeakThis() { return mWeakThis.lock(); }
  ...
  /**
   * Number of frames which will be copied to/from the audio device in a single
   * read/write operation
   */
  int32_t mFramesPerBurst = kUnspecified;

  // Time to sleep in order to prevent a race condition with a callback after a
  // close(). Two milliseconds may be enough but 10 msec is even safer.
...
/**
 * This struct is a stateless functor which closes an AudioStream prior to its
 * deletion. This means it can be used to safely delete a smart pointer
 * referring to an open stream.
 */
