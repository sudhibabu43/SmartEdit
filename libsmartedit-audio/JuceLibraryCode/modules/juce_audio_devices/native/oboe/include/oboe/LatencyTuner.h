 ...
 *
 * This only affects the latency associated with the first level of buffering
 * that is closest to the application. It does not affect low latency in the
 * HAL, or touch latency in the UI.
 *
 * Call tune() right before returning from your data callback function if using
 * callbacks. Call tune() right before calling write() if using blocking writes.
 *
...
public:
  /**
   * Construct a new LatencyTuner object which will act on the given audio
   * stream
   *
   ...
   *
   * @param stream the stream who's latency will be tuned
   * @param the maximum buffer size which the tune() operation will set the
   * buffer size to
   */
   ...
   * which will lower the latency to the minimum and then allow it to rise back
   * up if there are glitches.
   *
   * This is typically called in response to a user decision to minimize
   * latency. In other words, call this from a button handler.
   */
  void requestReset();

  /**
   * @return true if the audio stream's buffer size is at the maximum value. If
   * no maximum value was specified when constructing the LatencyTuner then the
   * value of stream->getBufferCapacityInFrames is used
   */
  ...
  int32_t getMinimumBufferSize() const { return mMinimumBufferSize; }

  /**
   * Set the amount the bufferSize will be incremented while tuning.
   * By default, this will be one burst.
   *
   * Note that AAudio will quantize the buffer size to a multiple of the
   * burstSize. So the final buffer sizes may not be a multiple of this
   * increment.
   *
   * @param sizeIncrement
   */
  ...
  int32_t getBufferSizeIncrement() const { return mBufferSizeIncrement; }

private:
  /**
   * Drop the latency down to the minimum and then let it rise back up.
   * This is useful if a glitch caused the latency to increase and it hasn't
   * gone back down.
   *
  ...
  enum class State { Idle, Active, AtMax, Unsupported };
  ...
  std::atomic<int32_t> mLatencyTriggerRequests{
      0}; // TODO user atomic requester from AAudio
