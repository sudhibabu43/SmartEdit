...
enum class AudioFormat : int32_t { // aaudio_format_t
   ...
   * fail when opening the stream. Apps are able to get the correct
   * configuration for the playback by calling AudioManager#getDevices(int).
   *
...
/**
 * The result of an operation. All except the `OK` result indicates that an
 * error occurred. The `Result` can be converted into a human readable string
 * using `convertToText`.
 */
...
enum class SharingMode : int32_t { // aaudio_sharing_mode_t
   ...
   *
   * If you do not need the lowest possible latency then we recommend using
 ...
 * Higher quality conversion will probably be implemented using a sinc based
 * resampler.
 */
 ...
 *
 * Note that these match the equivalent values in AudioAttributes in the Android
 * Java API.
 *
...
enum class Usage : int32_t { // aaudio_usage_t
  ...
  VoiceCommunicationSignalling =
      3, // AAUDIO_USAGE_VOICE_COMMUNICATION_SIGNALLING
  ...
  AssistanceNavigationGuidance =
      12, // AAUDIO_USAGE_ASSISTANCE_NAVIGATION_GUIDANCE
 ...
 * optional. But in case it is known (for instance {@link Movie} for a movie
 * streaming service or {@link Speech} for an audio book application) this
 * information might be used by the audio framework to enforce audio focus.
 *
 * Note that these match the equivalent values in AudioAttributes in the Android
 * Java API.
 *
...
enum class ChannelMask : uint32_t { // aaudio_channel_mask_t
   ...
   */
  CM2Point1 = FrontLeft | FrontRight | LowFrequency,

  /**
   * Supported for only Output
   */
  Tri = FrontLeft | FrontRight | FrontCenter,

  /**
   * Supported for only Output
   */
  TriBack = FrontLeft | FrontRight | BackCenter,

  /**
   * Supported for only Output
   */
  CM3Point1 = FrontLeft | FrontRight | FrontCenter | LowFrequency,

  /**
   * Supported for Input and Output
   */
  CM2Point0Point2 = FrontLeft | FrontRight | TopSideLeft | TopSideRight,

  /**
   * Supported for Input and Output
   */
  CM2Point1Point2 = CM2Point0Point2 | LowFrequency,

  /**
   * Supported for Input and Output
   */
  CM3Point0Point2 =
      FrontLeft | FrontRight | FrontCenter | TopSideLeft | TopSideRight,
   ...
   * Supported for only Output
   */
  QuadSide = FrontLeft | FrontRight | SideLeft | SideRight,
   ...
   * Supported for only Output
   */
  Penta = Quad | FrontCenter,

  /**
   * Supported for Input and Output. aka 5Point1Back
   */
  CM5Point1 = FrontLeft | FrontRight | FrontCenter | LowFrequency | BackLeft |
              BackRight,

  /**
   * Supported for only Output
   */
  CM5Point1Side = FrontLeft | FrontRight | FrontCenter | LowFrequency |
                  SideLeft | SideRight,

  /**
   * Supported for only Output
   */
  CM6Point1 = FrontLeft | FrontRight | FrontCenter | LowFrequency | BackLeft |
              BackRight | BackCenter,

  /**
   * Supported for only Output
   */
  CM7Point1 = CM5Point1 | SideLeft | SideRight,

  /**
   * Supported for only Output
   */
  CM5Point1Point2 = CM5Point1 | TopSideLeft | TopSideRight,

  /**
   * Supported for only Output
   */
  CM5Point1Point4 =
      CM5Point1 | TopFrontLeft | TopFrontRight | TopBackLeft | TopBackRight,

  /**
   * Supported for only Output
   */
  CM7Point1Point2 = CM7Point1 | TopSideLeft | TopSideRight,

  /**
   * Supported for only Output
   */
  CM7Point1Point4 =
      CM7Point1 | TopFrontLeft | TopFrontRight | TopBackLeft | TopBackRight,

  /**
   * Supported for only Output
   */
  CM9Point1Point4 = CM7Point1Point4 | FrontWideLeft | FrontWideRight,

  /**
   * Supported for only Output
   */
  CM9Point1Point6 = CM9Point1Point4 | TopSideLeft | TopSideRight,

  /**
   * Supported for only Input
   */
  FrontBack = FrontCenter | BackCenter,
};
...
enum class SpatializationBehavior : int32_t {
  ...
  /**
   * Constant indicating the audio content associated with these attributes will
   * follow the default platform behavior with regards to which content will be
   * spatialized or not.
   */
  Auto = 1,

  /**
   * Constant indicating the audio content associated with these attributes
   * should never be spatialized.
   */
  Never = 2,
};

/**
 * The PrivacySensitiveMode attribute determines whether an input stream can be
 * shared with another privileged app, for example the Assistant.
 *
 * This allows to override the default behavior tied to the audio source (e.g
 * InputPreset::VoiceCommunication is private by default but
 * InputPreset::Unprocessed is not).
 */
enum class PrivacySensitiveMode : int32_t {

  /**
   * When not explicitly requested, set privacy sensitive mode according to
   * input preset: communication and camcorder captures are considered privacy
...
/**
 * Specifies whether audio may or may not be captured by other apps or the
 * system for an output stream.
 *
 * Note that these match the equivalent values in AudioAttributes in the Android
 * Java API.
 *
...
enum class AllowedCapturePolicy : int32_t {
  /**
   * When not explicitly requested, set privacy sensitive mode according to the
   * Usage. This should behave similarly to setting AllowedCapturePolicy::All.
   */
  Unspecified = kUnspecified,
  /**
   ...
   * guidance... but have strong restriction. See ALLOW_CAPTURE_BY_SYSTEM in the
   * AudioAttributes Java API for what the system apps can do with the capture
   * audio.
   */
  System = 2,
  /**
   ...
   * system app.
   *
   * It is encouraged to use AllowedCapturePolicy::System instead of this value
   * as system apps provide significant and useful features for the user (eg.
   * accessibility). See ALLOW_CAPTURE_BY_NONE in the AudioAttributes Java API
   */
  None = 3,
};

/**
 * On API 16 to 26 OpenSL ES will be used. When using OpenSL ES the optimal
 * values for sampleRate and framesPerBurst are not known by the native code. On
 * API 17+ these values should be obtained from the AudioManager using this
 * code:
 *
 * <pre><code>
 * // Note that this technique only works for built-in speakers and headphones.
 * AudioManager myAudioMgr = (AudioManager)
 * getSystemService(Context.AUDIO_SERVICE); String sampleRateStr =
 * myAudioMgr.getProperty(AudioManager.PROPERTY_OUTPUT_SAMPLE_RATE); int
 * defaultSampleRate = Integer.parseInt(sampleRateStr); String framesPerBurstStr
 * = myAudioMgr.getProperty(AudioManager.PROPERTY_OUTPUT_FRAMES_PER_BUFFER); int
 * defaultFramesPerBurst = Integer.parseInt(framesPerBurstStr);
 * </code></pre>
 *
 * It can then be passed down to Oboe through JNI.
 *
 * AAudio will get the optimal framesPerBurst from the HAL and will ignore this
 * value.
 */
...
class OboeGlobals {
public:
  static bool areWorkaroundsEnabled() { return mWorkaroundsEnabled; }
