...
public:
  ...
  // This class only contains primitives so we can use default constructor and
  // copy methods.
   ...
   */
  AudioStreamDataCallback *getDataCallback() const { return mDataCallback; }

  /**
   * For internal use only.
   * @return the error callback object for this stream, if set.
   */
  AudioStreamErrorCallback *getErrorCallback() const { return mErrorCallback; }

  /**
   * @return true if a data callback was set for this stream
   */
  bool isDataCallbackSpecified() const { return mDataCallback != nullptr; }
   ...
   */
  bool isErrorCallbackSpecified() const { return mErrorCallback != nullptr; }
  ...
  PrivacySensitiveMode getPrivacySensitiveMode() const {
    return mPrivacySensitiveMode;
  }

  /**
   * @return true if Oboe can convert channel counts to achieve optimal results.
   */
  bool isChannelConversionAllowed() const { return mChannelConversionAllowed; }

  /**
   * @return true if  Oboe can convert data formats to achieve optimal results.
   */
  bool isFormatConversionAllowed() const { return mFormatConversionAllowed; }

  /**
   * @return whether and how Oboe can convert sample rates to achieve optimal
   * results.
   */
   ...
   */
  ChannelMask getChannelMask() const { return mChannelMask; }

  /**
   * @return number of channels for the hardware, for example 2 for stereo, or
   * kUnspecified.
   */
...
protected:
  /** The callback which will be fired when new data is ready to be
   * read/written. **/
   ...
   * TODO InputPreset::Unspecified should be considered as a possible default
   * alternative.
   */
  ...
  AllowedCapturePolicy mAllowedCapturePolicy =
      AllowedCapturePolicy::Unspecified;

  /** Privacy Sensitive Mode. Only active on Android 30+ */
  PrivacySensitiveMode mPrivacySensitiveMode =
      PrivacySensitiveMode::Unspecified;

  /** Control the name of the package creating the stream. Only active on
   * Android 31+ */
  std::string mPackageName;
  /** Control the attribution tag of the context creating the stream. Only
   * active on Android 31+ */
  ...
  SpatializationBehavior mSpatializationBehavior =
      SpatializationBehavior::Unspecified;
  ...
  // Control whether and how Oboe can convert sample rates to achieve optimal
  // results.
  SampleRateConversionQuality mSampleRateConversionQuality =
