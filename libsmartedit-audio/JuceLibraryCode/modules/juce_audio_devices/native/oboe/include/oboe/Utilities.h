...
/**
 * @return the size of a sample of the given format in bytes or 0 if format is
 * invalid
 */
 ...
 *
 * @param input object to convert from. @see common/Utilities.cpp for concrete
 * implementations
 * @return text representation of an Oboe enum value. There is no need to call
 * free on this.
 */
template <typename FromType> const char *convertToText(FromType input);
...
/**
 * Returns whether a device is on a pre-release SDK that is at least the
 * specified codename version.
 *
 * @param codename the code name to verify.
 * @return boolean of whether the device is on a pre-release SDK and is at least
 * the specified codename
 */
