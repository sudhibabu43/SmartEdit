 ...
 *
 * For the builder of the output stream, call setDataCallback() with this
 * object.
 *
 * When both streams are ready, onAudioReady() of the output stream will call
 * onBothStreamsReady(). Callers must override onBothStreamsReady().
 *
 * To ensure best results, open an output stream before the input stream.
 * Call
 * inputBuilder.setBufferCapacityInFrames(mOutputStream->getBufferCapacityInFrames()
 * * 2). Also, call inputBuilder.setSampleRate(mOutputStream->getSampleRate()).
 *
 ...
 *
 * Callers should handle error callbacks with setErrorCallback() for the output
 * stream. When an error callback occurs for the output stream, Oboe will stop
 * and close the output stream. The caller is responsible for stopping and
 * closing the input stream. The caller should also reopen and restart both
 * streams when the error callback is ErrorDisconnected. See the LiveEffect
 * sample as an example of this.
 *
 */
...
public:
   ...
   */
  void setInputStream(AudioStream *stream) { mInputStream = stream; }
   ...
   */
  AudioStream *getInputStream() { return mInputStream; }
   ...
   * @param stream the output stream
   */
  void setOutputStream(AudioStream *stream) { mOutputStream = stream; }
   ...
   */
  AudioStream *getOutputStream() { return mOutputStream; }

  /**
   * Attempts to start both streams. Please call setInputStream() and
   * setOutputStream() before calling this function.
   *
  ...
  /**
   * Stops both streams. Returns Result::OK if neither stream had an error
   * during close.
   *
   * @return result of the operation
   */
  ...
  /**
   * Reads input from the input stream. Callers should not call this directly as
   * this is called in onAudioReady().
   *
   * @param numFrames
   * @return result of the operation
   */
  virtual ResultWithValue<int32_t> readInput(int32_t numFrames) {
    return getInputStream()->read(mInputBuffer.get(), numFrames,
                                  0 /* timeout */);
  }
   ...
   */
  virtual DataCallbackResult onBothStreamsReady(const void *inputData,
                                                int numInputFrames,
                                                void *outputData,
   ...
   * This in return calls onBothStreamsReady() when data is available on both
   * streams. Callers should call this function when the output stream is ready.
  ...
  DataCallbackResult onAudioReady(AudioStream *audioStream, void *audioData,
    ...
    if (mCountCallbacksToDrain > 0) {
      ...
      // Let the input fill up a bit so we are not so close to the write
      // pointer.
      ...
      ResultWithValue<int32_t> resultAvailable =
          getInputStream()->getAvailableFrames();
      if (!resultAvailable) {
  ...
  /**
   *
   * This is a cushion between the DSP and the application processor cursors to
   * prevent collisions. Typically 0 for latency measurements or 1 for glitch
   * tests.
   *
   ...
   */
  int32_t getNumInputBurstsCushion() const { return mNumInputBurstsCushion; }

  /**
   * Minimum number of frames in the input stream buffer before calling
   * readInput().
   *
  ...
  /**
   * Gets the minimum number of frames in the input stream buffer before calling
   * readInput().
   *
