...
public:
  ...
  /**
   * Write framesToWrite or, if too enough, then write as many as the fifo are
   * not empty.
   *
   * @param destination
  ...
  /**
   * Calls read(). If all of the frames cannot be read then the remainder of the
   ...
   */
  uint32_t getFullFramesAvailable() { return mFifo->getFullFramesAvailable(); }
   ...
   */
  uint32_t getBytesPerFrame() const { return mBytesPerFrame; }
   ...
   */
  uint64_t getReadCounter() const { return mFifo->getReadCounter(); }
   ...
   */
  void setReadCounter(uint64_t n) { mFifo->setReadCounter(n); }
   ...
   */
  void setWriteCounter(uint64_t n) { mFifo->setWriteCounter(n); }
