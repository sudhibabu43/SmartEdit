...
public:
   ...
   *
   * For an output stream, this function should render and write numFrames of
   * data in the stream's current data format to the audioData buffer.
   *
   * For an input stream, this function should read and process numFrames of
   * data from the audioData buffer.
   *
   ...
   * AudioStreamBuilder::setFramesPerCallback() is called.
   *
   ...
   */
  virtual DataCallbackResult onAudioReady(AudioStream *audioStream,
                                          void *audioData,
                                          int32_t numFrames) = 0;
};
...
public:
  ...
  /**
   * This will be called before other `onError` methods when an error occurs on
   * a stream, such as when the stream is disconnected.
   *
   ...
   * Or it might be used when an app want to signal a management thread that
   * handles all of the stream state.
   *
   * If this method returns false it indicates that the stream has *not been
   * stopped and closed by the application. In this case it will be stopped by
   * Oboe in the following way: onErrorBeforeClose() will be called, then the
   * stream will be closed and onErrorAfterClose() will be closed.
   *
   * If this method returns true it indicates that the stream *has* been stopped
   * and closed by the application and Oboe will not do this. In that case, the
   * app MUST stop() and close() the stream.
   *
   ...
   * and if onError() returns false (indicating that the error has not already
   * been handled).
   *
   ...
   * @param error
   */
  virtual void onErrorBeforeClose(AudioStream * /* audioStream */,
                                  Result /* error */) {}
   ...
   * been handled).
   *
   * The underlying AAudio or OpenSL ES stream will already be stopped AND
   * closed by Oboe. So the underlying stream cannot be referenced. But you can
   * still query most parameters.
   *
   ...
   * @param error
   */
  virtual void onErrorAfterClose(AudioStream * /* audioStream */,
                                 Result /* error */) {}
};
 ...
 *
 * It combines the interfaces defined by AudioStreamDataCallback and
 * AudioStreamErrorCallback. This was the original callback object. We now
 * recommend using the individual interfaces and using setDataCallback() and
 * setErrorCallback().
 *
 * @deprecated Use `AudioStreamDataCallback` and `AudioStreamErrorCallback`
 * instead
 */
