...
// This depends on AudioStream, so we use forward declaration, it will close and
// delete the stream
...
public:
   ...
   * the application should query for the actual value after the stream is
   * opened.
   *
   * As the channel count here may be different from the corresponding channel
   * count of provided channel mask used in setChannelMask(). The last called
   * will be respected if this function and setChannelMask() are called.
   */
   ...
   *
   * As the corresponding channel count of provided channel mask here may be
   * different from the channel count used in setChannelCount(). The last called
   * will be respected if this function and setChannelCount() are called.
   *
   * As the setChannelMask API is available on Android 32+, this call will only
   * take effects on Android 32+.
   */
   ...
   * the callbacks. But if your application is, for example, doing FFTs or other
   * block oriented operations, then call this function to get the sizes you
   * need.
   *
   * Calling setFramesPerDataCallback() does not guarantee anything about
   * timing. This just collects the data into a the number of frames that your
   * app requires. We encourage leaving this unspecified in most cases.
   *
   * If this number is larger than the burst size, some bursts will not receive
   * a callback. If this number is smaller than the burst size, there may be
   * multiple callbacks in a single burst.
   *
   ...
   *
   * The final stream capacity may differ. For AAudio it should be at least this
   * big. For OpenSL ES, it could be smaller.
   *
   * Default is kUnspecified.
   *
   * @param bufferCapacityInFrames the desired buffer capacity in frames or
   * kUnspecified
   * @return pointer to the builder so calls can be chained
   */
  AudioStreamBuilder *
  setBufferCapacityInFrames(int32_t bufferCapacityInFrames) {
    mBufferCapacityInFrames = bufferCapacityInFrames;
    return this;
  }

  /**
   * Get the audio API which will be requested when opening the stream. No
   * guarantees that this is the API which will actually be used. Query the
   * stream itself to find out the API which is being used.
   *
   * If you do not specify the API, then AAudio will be used if
   * isAAudioRecommended() returns true. Otherwise OpenSL ES will be used.
   *
   ...
   *
   * This should almost always be left unspecified, except for debugging
   * purposes. Specifying AAudio will force Oboe to use AAudio on 8.0, which is
   * extremely risky. Specifying OpenSLES should mainly be used to test legacy
   * performance/functionality.
   *
   * If the caller requests AAudio and it is supported then AAudio will be used.
   *
   * @param audioApi Must be AudioApi::Unspecified, AudioApi::OpenSLES or
   * AudioApi::AAudio.
   * @return pointer to the builder so calls can be chained
   */
   ...
   *
   * AAudio may be supported but not recommended because of version specific
   * issues. AAudio is not recommended for Android 8.0 or earlier versions.
   *
   ...
   * So the application should query for the actual mode after the stream is
   * opened.
   *
   ...
   *
   * The system will use this information to optimize the behavior of the
   * stream. This could, for example, affect how volume and focus is handled for
   * the stream. The usage is ignored for input streams.
   *
   ...
   *
   * The system will use this information to optimize the behavior of the
   * stream. This could, for example, affect whether a stream is paused when a
   * notification occurs. The contentType is ignored for input streams.
   *
   ...
   *
   * The system will use this information to optimize the behavior of the
   * stream. This could, for example, affect which microphones are used and how
   * the recorded data is processed.
   *
   * The default, if you do not call this function, is
   * InputPreset::VoiceRecognition. That is because VoiceRecognition is the
   * preset with the lowest latency on many platforms.
   *
   * Added in API level 28.
   *
   ...
   * AudioStream::getSessionId() and then used with this function when opening
  ...
  /**
   * Request a stream to a specific audio input/output device given an audio
   * device ID.
   *
   * In most cases, the primary device will be the appropriate device to use,
   * and the deviceId can be left kUnspecified.
   *
  ...
  /**
   * Specify whether this stream audio may or may not be captured by other apps
   * or the system.
   *
   * The default is AllowedCapturePolicy::Unspecified which maps to
   * AAUDIO_ALLOW_CAPTURE_BY_ALL.
   *
   * Note that an application can also set its global policy, in which case the
   * most restrictive policy is always applied. See
   * android.media.AudioAttributes.setAllowedCapturePolicy.
   *
   ...
   */
  AudioStreamBuilder *
  setAllowedCapturePolicy(AllowedCapturePolicy allowedCapturePolicy) {
   ...
   * not.
   *
   * When PrivacySensitiveMode::Enabled, this input stream is privacy sensitive
   * and any concurrent capture is not permitted.
   *
   * This is off (PrivacySensitiveMode::Disabled) by default except when the
   * input preset is InputPreset::VoiceRecognition or InputPreset::Camcorder
   *
   * Always takes precedence over default from input preset when set explicitly.
   *
   * Only relevant if the stream direction is Direction::Input and AAudio is
   * used.
   *
   * Added in API level 30 to AAudio.
   *
   * @param privacySensitive PrivacySensitiveMode::Enabled if capture from this
   * stream must be marked as privacy sensitive, PrivacySensitiveMode::Disabled
   * if stream should be marked as not sensitive.
   * @return pointer to the builder so calls can be chained
   */
  AudioStreamBuilder *
  setPrivacySensitiveMode(PrivacySensitiveMode privacySensitiveMode) {
    mPrivacySensitiveMode = privacySensitiveMode;
    return this;
  }

  /**
   * Specifies whether the audio data of this output stream has already been
   * processed for spatialization.
   *
   * If the stream has been processed for spatialization, setting this to true
   * will prevent issues such as double-processing on platforms that will
   * spatialize audio data.
   *
   ...
   *
   * The AAudio system will use this information to select whether the stream
   * will go through a spatializer effect or not when the effect is supported
   * and enabled.
   *
  ...
  /**
   * Specifies an object to handle data related callbacks from the underlying
   * API.
   *
   * <strong>Important: See AudioStreamCallback for restrictions on what may be
   * called from the callback methods.</strong>
   *
   * We pass a shared_ptr so that the sharedDataCallback object cannot be
   * deleted before the stream is deleted.
   *
  ...
  setDataCallback(std::shared_ptr<AudioStreamDataCallback> sharedDataCallback) {
    // Use this raw pointer in the rest of the code to retain backwards
    // compatibility.
    mDataCallback = sharedDataCallback.get();
    // Hold a shared_ptr to protect the raw pointer for the lifetime of the
    // stream.
  ...
  /**
   * Pass a raw pointer to a data callback. This is not recommended because the
   * dataCallback object might get deleted by the app while it is being used.
   *
   * @deprecated Call setDataCallback(std::shared_ptr<AudioStreamDataCallback>)
   * instead.
   * @param dataCallback
   * @return pointer to the builder so calls can be chained
   */
  ...
  /**
   * Specifies an object to handle error related callbacks from the underlying
   * API. This can occur when a stream is disconnected because a headset is
   * plugged in or unplugged. It can also occur if the audio service fails or if
   * an exclusive stream is stolen by another stream.
   *
   * <strong>Important: See AudioStreamCallback for restrictions on what may be
   * called from the callback methods.</strong>
   *
   * <strong>When an error callback occurs, the associated stream must be
   * stopped and closed in a separate thread.</strong>
   *
   * We pass a shared_ptr so that the errorCallback object cannot be deleted
   * before the stream is deleted. If the stream was created using a shared_ptr
   * then the stream cannot be deleted before the error callback has finished
   * running.
   *
   * @param sharedErrorCallback
   * @return pointer to the builder so calls can be chained
   */
  AudioStreamBuilder *setErrorCallback(
      std::shared_ptr<AudioStreamErrorCallback> sharedErrorCallback) {
    // Use this raw pointer in the rest of the code to retain backwards
    // compatibility.
  ...
  /**
   * Pass a raw pointer to an error callback. This is not recommended because
   * the errorCallback object might get deleted by the app while it is being
   * used.
   *
   * @deprecated Call
   ...
   */
  AudioStreamBuilder *
  setErrorCallback(AudioStreamErrorCallback *errorCallback) {
  ...
  /**
   * Specifies an object to handle data or error related callbacks from the
   * underlying API.
   *
   * This is the equivalent of calling both setDataCallback() and
   * setErrorCallback().
   *
   * <strong>Important: See AudioStreamCallback for restrictions on what may be
   * called from the callback methods.</strong>
   ...
   * On some versions of Android for example, stereo streams could not use a
   * FAST track. So a mono stream might be used instead and duplicated to two
   * channels. On some devices, mono streams might be broken, so a stereo stream
   * might be opened and converted to mono.
   *
   * Default is false.
   */
   ...
   * low latency data path. So an I16 stream might be opened and converted to
   * float.
   *
   * Default is false.
   */
   ...
   * underlying APIs might still do sample rate conversion if you specify a
   * sample rate. That can prevent you from getting a low latency stream.
   *
   * If you do the conversion in Oboe then you might still get a low latency
   * stream.
   *
   * Default is SampleRateConversionQuality::None
   */
  AudioStreamBuilder *
  setSampleRateConversionQuality(SampleRateConversionQuality quality) {
   ...
   * calling uid. The vast majority of apps have only one package per calling
   * UID. If an invalid package name is set, input streams may not be given
   * permission to record when started.
   *
   * The package name is usually the applicationId in your app's build.gradle
   * file.
   *
   ...
   *
   * @deprecated Use openStream(std::shared_ptr<oboe::AudioStream> &stream)
   * instead.
   ...
   * @deprecated Use openStream(std::shared_ptr<oboe::AudioStream> &stream)
   * instead.
   * @param stream Reference to the ManagedStream (uniqueptr) used to keep track
   * of stream
   * @return OBOE_OK if successful or a negative error code.
   */
