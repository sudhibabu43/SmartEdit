...
MultiChannelResampler *MultiChannelResampler::Builder::build() {
  ...
  ratio.reduce();
  bool usePolyphase =
      (getNumTaps() * ratio.getDenominator()) <= kMaxCoefficients;
  if (usePolyphase) {
...
void MultiChannelResampler::writeFrame(const float *frame) {
  ...
  float *dest = &mX[static_cast<size_t>(mCursor) *
...
// This is more complicated but readFrame() is called repeatedly and should be
// optimized.
void MultiChannelResampler::generateCoefficients(int32_t inputRate,
  ...
  mCoefficients.resize(static_cast<size_t>(getNumTaps()) *
...
#else
      float window =
          mCoshWindow(static_cast<double>(tapPhase) * numTapsHalfInverse);
#endif
