...
public:
  ...
  const char *getName() override { return "Limiter"; }

private:
  // These numbers are based on a polynomial spline for a quadratic solution
  // Ax^2 + Bx + C The range is up to 3 dB, (10^(3/20)), to match AudioTrack for
  // float data.
   ...
   * If between 1 and kXWhenYis3Decibels, use a quadratic spline (Ax^2 + Bx +
   * C). If between -kXWhenYis3Decibels and -1, use the absolute value for the
   * spline and flip it. The derivative of the spline is 1 at 1 and 0 at
   * kXWhenYis3Decibels. This way, the graph is both continuous and
   * differentiable.
   */
