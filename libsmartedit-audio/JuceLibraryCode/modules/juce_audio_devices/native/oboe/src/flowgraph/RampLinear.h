...
public:
  ...
  int32_t getLengthInFrames() const { return mLengthInFrames; }
   ...
   *
   * WARNING: this can cause a discontinuity if called while the ramp is being
   * used. Only call this when setting the initial ramp.
   *
   * @param level
   */
  ...
  const char *getName() override { return "RampLinear"; }
