...
// TODO Review use of raw pointers for connect(). Maybe use smart pointers but
// need to avoid
//      run-time deallocation in audio thread.

// Set flags FLOWGRAPH_ANDROID_INTERNAL and FLOWGRAPH_OUTER_NAMESPACE based on
// whether compiler flag __ANDROID_NDK__ is defined. __ANDROID_NDK__ should be
// defined in oboe and not aaudio.
...
// Default block size that can be overridden when the FlowGraphPortFloat is
// created. If it is too small then we will have too much overhead from
// switching between nodes. If it is too high then we will thrash the caches.
...
public:
   ...
   * generate multiple frames of data then write the results to the output
   * ports.
   *
  ...
  void addInputPort(FlowGraphPort &port) { mInputPorts.emplace_back(port); }

  bool isDataPulledAutomatically() const { return mDataPulledAutomatically; }

  /**
   * Set true if you want the data pulled through the graph automatically.
   * This is the default.
   *
   * Set false if you want to pull the data from the input ports in the
   * onProcess() method. You might do this, for example, in a sample rate
   * converting node.
   *
   * @param automatic
   */
  ...
  virtual const char *getName() { return "FlowGraph"; }

  int64_t getLastCallCount() { return mLastCallCount; }
...
public:
  ...
  int32_t getSamplesPerFrame() const { return mSamplesPerFrame; }
...
/***************************************************************************/
/**
 * This port contains a 32-bit float buffer that can contain several frames of
 * data. Processing the data in a block improves performance.
 *
 * The size is framesPerBuffer * samplesPerFrame).
 */
class FlowGraphPortFloat : public FlowGraphPort {
public:
  FlowGraphPortFloat(FlowGraphNode &parent, int32_t samplesPerFrame,
                     int32_t framesPerBuffer = kDefaultBufferSize);

  virtual ~FlowGraphPortFloat() = default;

  int32_t getFramesPerBuffer() const { return mFramesPerBuffer; }

protected:
  /**
   * @return buffer internal to the port or from a connected port
   */
  virtual float *getBuffer() { return mBuffer.get(); }
...
/***************************************************************************/
/**
 * The results of a node's processing are stored in the buffers of the output
 * ports.
 */
...
public:
   ...
   *
   * This not thread safe. Do not modify the graph topology from another thread
   * while running. Also do not delete a module while it is connected to another
   * port if the graph is running.
   */
 ...
 * If you connect an output port to this port then its value will be used
 * instead.
 */
...
public:
  ...
  void disconnect() { mConnected = nullptr; }
...
public:
  explicit FlowGraphSink(int32_t channelCount) : input(*this, channelCount) {}
   ...
   * @return number of frames actually processed
   */
  int32_t onProcess(int32_t numFrames) override { return numFrames; }
...
/***************************************************************************/
/**
 * Base class for a node that has an input and an output with the same number of
 * channels. This may include traditional filters, eg. FIR, but also include any
 * processing node that converts input to output.
 */
