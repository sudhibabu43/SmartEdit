...
// It appears from the spectrogram that the HyperbolicCosine window leads to
// fewer artifacts. And it is faster to calculate.
...
public:
  ...
  public:
     ...
     *
     * For polyphase filters, numTaps must be a multiple of four for loop
     * unrolling.
    ...
    /**
     * Set cutoff frequency relative to the Nyquist rate of the output sample
     * rate. Set to 1.0 to match the Nyquist frequency. Set lower to reduce
     * aliasing. Default is 0.70.
     *
    ...
    int32_t getNumTaps() const { return mNumTaps; }

    int32_t getChannelCount() const { return mChannelCount; }

    int32_t getInputRate() const { return mInputRate; }

    int32_t getOutputRate() const { return mOutputRate; }

    float getNormalizedCutoff() const { return mNormalizedCutoff; }
   ...
   */
  static MultiChannelResampler *make(int32_t channelCount, int32_t inputRate,
                                     int32_t outputRate, Quality quality);

  bool isWriteNeeded() const { return mIntegerPhase >= mDenominator; }
  ...
  int getNumTaps() const { return mNumTaps; }
...
protected:
  ...
  void advanceWrite() { mIntegerPhase -= mDenominator; }

  void advanceRead() { mIntegerPhase += mNumerator; }
   ...
   * @param inputRate sample rate of the input stream
   * @param outputRate  sample rate of the output stream
   * @param numRows number of rows in the array that contain a set of tap
   * coefficients
   * @param phaseIncrement how much to increment the phase between rows
   * @param normalizedCutoff filter cutoff frequency normalized to Nyquist rate
   * of output
   */
  void generateCoefficients(int32_t inputRate, int32_t outputRate,
                            int32_t numRows, double phaseIncrement,
                            float normalizedCutoff);

  int32_t getIntegerPhase() { return mIntegerPhase; }
