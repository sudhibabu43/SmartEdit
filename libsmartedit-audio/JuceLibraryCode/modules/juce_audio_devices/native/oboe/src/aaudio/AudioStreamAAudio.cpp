...
oboe_aaudio_data_callback_proc(AAudioStream *stream, void *userData,
                               void *audioData, int32_t numFrames) {

  AudioStreamAAudio *oboeStream =
      reinterpret_cast<AudioStreamAAudio *>(userData);
...
// It calls app error callbacks from a static function in case the stream gets
// deleted.
...
// This runs in its own thread.
// Only one of these threads will be launched from internalErrorCallback().
// Prevents deletion of the stream if the app is using
// AudioStreamBuilder::openSharedStream()
static void
oboe_aaudio_error_thread_proc_shared(std::shared_ptr<AudioStream> sharedStream,
                                     Result error) {
  AudioStreamAAudio *oboeStream =
      reinterpret_cast<AudioStreamAAudio *>(sharedStream.get());
...
void AudioStreamAAudio::internalErrorCallback(AAudioStream *stream,
                                              void *userData,
  ...
  // Coerce the error code if needed to workaround a regression in RQ1A that
  // caused the wrong code to be passed when headsets plugged in. See
  // b/173928197.
  if (OboeGlobals::areWorkaroundsEnabled() &&
      getSdkVersion() == __ANDROID_API_R__ &&
      oboeResult == oboe::Result::ErrorTimeout) {
    oboeResult = oboe::Result::ErrorDisconnected;
    LOGD("%s() ErrorTimeout changed to ErrorDisconnected to fix b/173928197",
         __func__);
  }
  ...
  // Prevents deletion of the stream if the app is using
  // AudioStreamBuilder::openStream(shared_ptr)
...
void AudioStreamAAudio::endPerformanceHintInCallback(int32_t numFrames) {
  if (mAdpfWrapper.isOpen()) {
    // Scale the measured duration based on numFrames so it is normalized to a
    // full burst.
    double durationScaler = static_cast<double>(mFramesPerBurst) / numFrames;
    // Skip this callback if numFrames is very small.
    // This can happen when buffers wrap around, particularly when doing sample
    // rate conversion.
  ...
  if (sdkVersion < __ANDROID_API_P__) {
    if (mUsage != Usage::Media) {
      LOGW("Usage [AudioStreamBuilder::setUsage()] "
           "is not supported on AAudio streams running on pre-Android P "
           "versions.");
    }
...
Result AudioStreamAAudio::open() {
  ...
  // Do not set INPUT capacity below 4096 because that prevents us from getting
  // a FAST track when using the Legacy data path. If the app requests > 4096
  // then we allow it but we are less likely to get LowLatency. See internal bug
  // b/80308183 for more details. Fixed in Q but let's still clip the capacity
  // because high input capacity does not increase latency.
  int32_t capacity = mBufferCapacityInFrames;
  constexpr int kCapacityRequiredForFastLegacyTrack =
      4096; // matches value in AudioFinger
  if (OboeGlobals::areWorkaroundsEnabled() &&
      mDirection == oboe::Direction::Input && capacity != oboe::Unspecified &&
      capacity < kCapacityRequiredForFastLegacyTrack &&
      mPerformanceMode == oboe::PerformanceMode::LowLatency) {
    capacity = kCapacityRequiredForFastLegacyTrack;
    LOGD("AudioStreamAAudio.open() capacity changed from %d to %d for lower "
         "latency",
  ...
  if (mLibLoader->builder_setSessionId != nullptr) {
    mLibLoader->builder_setSessionId(
        aaudioBuilder, static_cast<aaudio_session_id_t>(mSessionId));
    // Output effects do not support PerformanceMode::LowLatency.
    if (OboeGlobals::areWorkaroundsEnabled() && mSessionId != SessionId::None &&
        mDirection == oboe::Direction::Output &&
        mPerformanceMode == PerformanceMode::LowLatency) {
      mPerformanceMode = PerformanceMode::None;
      LOGD("AudioStreamAAudio.open() performance mode changed to None when "
           "session "
           "id is requested");
    }
  }

  // Channel mask was added in SC_V2. Given the corresponding channel count of
  // selected channel mask may be different from selected channel count, the
  // last set value will be respected. If channel count is set after channel
  // mask, the previously set channel mask will be cleared. If channel mask is
  // set after channel count, the channel count will be automatically calculated
  // from selected channel mask. In that case, only set channel mask when the
  // API is available and the channel mask is specified.
  if (mLibLoader->builder_setChannelMask != nullptr &&
      mChannelMask != ChannelMask::Unspecified) {
    mLibLoader->builder_setChannelMask(
        aaudioBuilder, static_cast<aaudio_channel_mask_t>(mChannelMask));
  } else {
  ...
  mLibLoader->builder_setFormat(aaudioBuilder,
                                static_cast<aaudio_format_t>(mFormat));
  mLibLoader->builder_setSampleRate(aaudioBuilder, mSampleRate);
  mLibLoader->builder_setSharingMode(
      aaudioBuilder, static_cast<aaudio_sharing_mode_t>(mSharingMode));
  mLibLoader->builder_setPerformanceMode(
      aaudioBuilder, static_cast<aaudio_performance_mode_t>(mPerformanceMode));
  ...
  if (mLibLoader->builder_setContentType != nullptr) {
    mLibLoader->builder_setContentType(
        aaudioBuilder, static_cast<aaudio_content_type_t>(mContentType));
  }
  ...
  if (mLibLoader->builder_setInputPreset != nullptr) {
    ...
    if (getSdkVersion() <= __ANDROID_API_P__ &&
      ...
      LOGD("InputPreset::VoicePerformance not supported before Q. Using "
           "VoiceRecognition.");
      inputPreset = InputPreset::VoiceRecognition; // most similar preset
    }
    mLibLoader->builder_setInputPreset(
        aaudioBuilder, static_cast<aaudio_input_preset_t>(inputPreset));
  }
  ...
  if (mLibLoader->builder_setPackageName != nullptr && !mPackageName.empty()) {
    mLibLoader->builder_setPackageName(aaudioBuilder, mPackageName.c_str());
  }

  if (mLibLoader->builder_setAttributionTag != nullptr &&
      !mAttributionTag.empty()) {
  ...
  if (mLibLoader->builder_setAllowedCapturePolicy != nullptr &&
      mDirection == oboe::Direction::Output) {
    mLibLoader->builder_setAllowedCapturePolicy(
        aaudioBuilder,
  ...
  if (mLibLoader->builder_setPrivacySensitive != nullptr &&
    ...
    mLibLoader->builder_setPrivacySensitive(
        aaudioBuilder, mPrivacySensitiveMode == PrivacySensitiveMode::Enabled);
  }

  if (mLibLoader->builder_setIsContentSpatialized != nullptr) {
    mLibLoader->builder_setIsContentSpatialized(aaudioBuilder,
  ...
  if (mLibLoader->builder_setSpatializationBehavior != nullptr) {
    ...
    mLibLoader->builder_setSpatializationBehavior(
        aaudioBuilder,
  ...
  if (isDataCallbackSpecified()) {
    mLibLoader->builder_setDataCallback(aaudioBuilder,
                                        oboe_aaudio_data_callback_proc, this);
    mLibLoader->builder_setFramesPerDataCallback(aaudioBuilder,
    ...
    mLibLoader->builder_setErrorCallback(aaudioBuilder, internalErrorCallback,
                                         this);
  }
  // Else if the data callback is not being used then the write method will
  // return an error and the app can stop and close the stream.

  // ============= OPEN THE STREAM ================
  {
    AAudioStream *stream = nullptr;
    result = static_cast<Result>(
        mLibLoader->builder_openStream(aaudioBuilder, &stream));
  ...
  mFormat =
      static_cast<AudioFormat>(mLibLoader->stream_getFormat(mAAudioStream));
  mSharingMode = static_cast<SharingMode>(
      mLibLoader->stream_getSharingMode(mAAudioStream));
  ...
  if (mLibLoader->stream_getContentType != nullptr) {
    mContentType = static_cast<ContentType>(
  ...
  if (mLibLoader->stream_getInputPreset != nullptr) {
    mInputPreset = static_cast<InputPreset>(
        mLibLoader->stream_getInputPreset(mAAudioStream));
  }
  if (mLibLoader->stream_getSessionId != nullptr) {
    mSessionId =
        static_cast<SessionId>(mLibLoader->stream_getSessionId(mAAudioStream));
  } else {
  ...
  if (mLibLoader->stream_getAllowedCapturePolicy != nullptr &&
      mDirection == oboe::Direction::Output) {
    mAllowedCapturePolicy = static_cast<AllowedCapturePolicy>(
        mLibLoader->stream_getAllowedCapturePolicy(mAAudioStream));
  } else {
  ...
  if (mLibLoader->stream_isPrivacySensitive != nullptr &&
    ...
    bool isPrivacySensitive =
        mLibLoader->stream_isPrivacySensitive(mAAudioStream);
  ...
  if (mLibLoader->stream_getChannelMask != nullptr) {
    mChannelMask = static_cast<ChannelMask>(
        mLibLoader->stream_getChannelMask(mAAudioStream));
  }

  if (mLibLoader->stream_isContentSpatialized != nullptr) {
    mIsContentSpatialized =
        mLibLoader->stream_isContentSpatialized(mAAudioStream);
  }
  ...
  if (mLibLoader->stream_getHardwareChannelCount != nullptr) {
    mHardwareChannelCount =
        mLibLoader->stream_getHardwareChannelCount(mAAudioStream);
  }
  if (mLibLoader->stream_getHardwareSampleRate != nullptr) {
    mHardwareSampleRate =
        mLibLoader->stream_getHardwareSampleRate(mAAudioStream);
  }
  if (mLibLoader->stream_getHardwareFormat != nullptr) {
    mHardwareFormat = static_cast<AudioFormat>(
        mLibLoader->stream_getHardwareFormat(mAAudioStream));
  }
...
error2:
  ...
  if (static_cast<int>(result) > 0) {
    ...
    LOGW("AudioStreamAAudio.open: AAudioStream_Open() returned positive error "
         "= %d",
...
Result AudioStreamAAudio::close() {
  ...
  AAudioStream *stream = nullptr;
  {
    ...
    // Closing will delete *mAAudioStream so we need to null out the pointer
    // atomically.
...
DataCallbackResult
AudioStreamAAudio::callOnAudioReady(AAudioStream * /*stream*/, void *audioData,
                                    int32_t numFrames) {
...
Result AudioStreamAAudio::requestStart() {
  ...
  if (stream != nullptr) {
    ...
    if (getSdkVersion() <= __ANDROID_API_O_MR1__) {
      StreamState state =
          static_cast<StreamState>(mLibLoader->stream_getState(stream));
      if (state == StreamState::Starting || state == StreamState::Started) {
        // WARNING: On P, AAudio is returning ErrorInvalidState for Output and
        // OK for Input.
        return Result::OK;
      }
    }
...
ResultWithValue<int32_t> AudioStreamAAudio::write(const void *buffer,
  ...
  AAudioStream *stream = mAAudioStream.load();
  if (stream != nullptr) {
    int32_t result = mLibLoader->stream_write(mAAudioStream, buffer, numFrames,
                                              timeoutNanoseconds);
...
ResultWithValue<int32_t> AudioStreamAAudio::read(void *buffer,
  ...
  AAudioStream *stream = mAAudioStream.load();
  if (stream != nullptr) {
    int32_t result = mLibLoader->stream_read(mAAudioStream, buffer, numFrames,
                                             timeoutNanoseconds);
    return ResultWithValue<int32_t>::createBasedOnSign(result);
  } else {
    return ResultWithValue<int32_t>(Result::ErrorClosed);
...
// AAudioStream_waitForStateChange() can crash if it is waiting on a stream and
// that stream is closed from another thread.  We do not want to lock the stream
// for the duration of the call. So we call AAudioStream_waitForStateChange()
// with a timeout of zero so that it will not block. Then we can do our own
// sleep with the lock unlocked.
Result AudioStreamAAudio::waitForStateChange(StreamState currentState,
  ...
  aaudio_stream_state_t currentAAudioState =
  ...
  mLock.lock();
  while (true) {
    // Do we still have an AAudio stream? If not then stream must have been
    // closed.
    AAudioStream *stream = mAAudioStream.load();
    if (stream == nullptr) {
    ...
    result = mLibLoader->stream_waitForStateChange(
        mAAudioStream, currentAAudioState, &aaudioNextState,
        0); // timeout=0 for non-blocking
    // AAudio will return AAUDIO_ERROR_TIMEOUT if timeout=0 and the state does
    // not change.
    ...
    if (OboeGlobals::areWorkaroundsEnabled() &&
        aaudioNextState ==
            static_cast<aaudio_stream_state_t>(StreamState::Starting)) {
      aaudioNextState =
          static_cast<aaudio_stream_state_t>(StreamState::Started);
    }
#endif // OBOE_FIX_FORCE_STARTING_TO_STARTED
    if (nextState != nullptr) {
...
AudioStreamAAudio::setBufferSizeInFrames(int32_t requestedFrames) {
  ...
  adjustedFrames =
      QuirksManager::getInstance().clipBufferSize(*this, adjustedFrames);

  std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
  AAudioStream *stream = mAAudioStream.load();
  if (stream != nullptr) {
    int32_t newBufferSize =
        mLibLoader->stream_setBufferSize(mAAudioStream, adjustedFrames);
    // Cache the result if it's valid
    if (newBufferSize > 0)
      mBufferSizeInFrames = newBufferSize;
...
// This was left in the code so that we could test the fix again easily in the
// future. We could not trigger the race condition without adding these get
// calls and the sleeps.
...
Result AudioStreamAAudio::getTimestamp(clockid_t clockId,
  ...
  AAudioStream *stream = mAAudioStream.load();
  if (stream != nullptr) {
    return static_cast<Result>(mLibLoader->stream_getTimestamp(
        stream, clockId, framePosition, timeNanoseconds));
  } else {
...
ResultWithValue<double> AudioStreamAAudio::calculateLatencyMillis() {
  ...
  auto result = getTimestamp(CLOCK_MONOTONIC, &hardwareFrameIndex,
  ...
  int64_t frameTimeDelta =
      (frameIndexDelta * oboe::kNanosPerSecond) / getSampleRate();
  int64_t appFrameHardwareTime = hardwareFrameHardwareTime + frameTimeDelta;

  // The current latency is the difference in time between when the current
  // frame is at the app and when it is at the hardware.
  double latencyNanos = static_cast<double>(
      isOutput
