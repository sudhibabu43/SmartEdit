...
#define AAUDIO_ERROR_UNAVAILABLE                                               \
  static_cast<aaudio_result_t>(Result::ErrorUnavailable)
...
public:
  ...
  bool isMMapUsed(oboe::AudioStream *oboeStream) {
    AAudioStream *aaudioStream =
        (AAudioStream *)oboeStream->getUnderlyingStream();
  ...
  int32_t setMMapEnabled(bool enabled) {
    ...
    if (mAAudio_setMMapPolicy == nullptr)
      return false;
    return mAAudio_setMMapPolicy(enabled ? AAUDIO_POLICY_AUTO
                                         : AAUDIO_POLICY_NEVER);
  }
  ...
  bool isMMapSupported() { return mMMapSupported; }

  bool isMMapExclusiveSupported() { return mMMapExclusiveSupported; }

private:
  enum { AAUDIO_POLICY_NEVER = 1, AAUDIO_POLICY_AUTO, AAUDIO_POLICY_ALWAYS };
  ...
  aaudio_result_t loadSymbols() {
    ...
    mAAudioStream_isMMap =
        (bool (*)(AAudioStream *stream))dlsym(libHandle, FUNCTION_IS_MMAP);
    if (mAAudioStream_isMMap == nullptr) {
      LOGI("%s() could not find " FUNCTION_IS_MMAP, __func__);
      return AAUDIO_ERROR_UNAVAILABLE;
    }

    mAAudio_setMMapPolicy = (int32_t(*)(aaudio_policy_t policy))dlsym(
        libHandle, FUNCTION_SET_MMAP_POLICY);
    if (mAAudio_setMMapPolicy == nullptr) {
    ...
    mAAudio_getMMapPolicy =
        (aaudio_policy_t(*)())dlsym(libHandle, FUNCTION_GET_MMAP_POLICY);
    if (mAAudio_getMMapPolicy == nullptr) {
