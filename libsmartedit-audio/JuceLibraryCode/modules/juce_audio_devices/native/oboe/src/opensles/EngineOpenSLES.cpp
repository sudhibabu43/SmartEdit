...
typedef SLresult (*prototype_slCreateEngine)(
    SLObjectItf *pEngine, SLuint32 numOptions,
...
static bool linkOpenSLES() {
  if (gLibOpenSlesLibraryHandle == nullptr &&
      gFunction_slCreateEngine == nullptr) {
    // Use RTLD_NOW to avoid the unpredictable behavior that RTLD_LAZY can
    // cause. Also resolving all the links now will prevent a run-time penalty
    // later.
    ...
    if (gLibOpenSlesLibraryHandle == nullptr) {
      ...
      gFunction_slCreateEngine = (prototype_slCreateEngine)dlsym(
          gLibOpenSlesLibraryHandle, "slCreateEngine");
...
SLresult EngineOpenSLES::open() {
  ...
  if (mOpenCount++ == 0) {
    ...
    // create engine
    result =
        (*gFunction_slCreateEngine)(&mEngineObject, 0, NULL, 0, NULL, NULL);
    ...
    if (SL_RESULT_SUCCESS != result) {
      LOGE("EngineOpenSLES - GetInterface() engine result:%s",
           getSLErrStr(result));
      goto error;
    }
  }
...
SLresult EngineOpenSLES::createOutputMix(SLObjectItf *objectItf) {
  return (*mEngineInterface)
      ->CreateOutputMix(mEngineInterface, objectItf, 0, 0, 0);
}
...
SLresult EngineOpenSLES::createAudioPlayer(SLObjectItf *objectItf,
  ...
  return (*mEngineInterface)
      ->CreateAudioPlayer(mEngineInterface, objectItf, audioSource, audioSink,
...
SLresult EngineOpenSLES::createAudioRecorder(SLObjectItf *objectItf,
  ...
  return (*mEngineInterface)
      ->CreateAudioRecorder(mEngineInterface, objectItf, audioSource, audioSink,
                            sizeof(ids) / sizeof(ids[0]), ids, reqs);
}
