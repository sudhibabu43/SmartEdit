...
constexpr int SL_ANDROID_SPEAKER_STEREO =
    (SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT);

constexpr int SL_ANDROID_SPEAKER_QUAD =
    (SL_ANDROID_SPEAKER_STEREO | SL_SPEAKER_BACK_LEFT | SL_SPEAKER_BACK_RIGHT);

constexpr int SL_ANDROID_SPEAKER_5DOT1 =
    (SL_ANDROID_SPEAKER_QUAD | SL_SPEAKER_FRONT_CENTER |
     SL_SPEAKER_LOW_FREQUENCY);

constexpr int SL_ANDROID_SPEAKER_7DOT1 =
    (SL_ANDROID_SPEAKER_5DOT1 | SL_SPEAKER_SIDE_LEFT | SL_SPEAKER_SIDE_RIGHT);

SLuint32
AudioOutputStreamOpenSLES::channelCountToChannelMask(int channelCount) const {
...
Result AudioOutputStreamOpenSLES::open() {
  ...
  if (mFormat == AudioFormat::Unspecified) {
    mFormat = (getSdkVersion() < __ANDROID_API_L__) ? AudioFormat::I16
                                                    : AudioFormat::Float;
  }
  ...
  SLuint32 bitsPerSample =
      static_cast<SLuint32>(getBytesPerSample() * kBitsPerByte);
  ...
  SLDataFormat_PCM format_pcm = {
      ...
      static_cast<SLuint32>(mSampleRate *
                            kMillisPerSecond),  // milliSamplesPerSec
  ...
  /**
   * API 21 (Lollipop) introduced support for floating-point data representation
   * and an extended data format type: SLAndroidDataFormat_PCM_EX. If running on
   * API 21+ use this newer format type, creating it from our original format.
   */
  SLAndroidDataFormat_PCM_EX format_pcm_ex;
  if (getSdkVersion() >= __ANDROID_API_L__) {
    SLuint32 representation =
        OpenSLES_ConvertFormatToRepresentation(getFormat());
  ...
  result = (*mObjectInterface)
               ->GetInterface(mObjectInterface, SL_IID_ANDROIDCONFIGURATION,
                              (void *)&configItf);
  if (SL_RESULT_SUCCESS != result) {
    ...
    result = (*configItf)
                 ->SetConfiguration(configItf, SL_ANDROID_KEY_STREAM_TYPE,
                                    &presetValue, sizeof(presetValue));
    if (SL_RESULT_SUCCESS != result) {
      goto error;
    }
  }
  ...
  result = (*mObjectInterface)
               ->GetInterface(mObjectInterface, SL_IID_PLAY, &mPlayInterface);
  if (SL_RESULT_SUCCESS != result) {
...
Result AudioOutputStreamOpenSLES::requestPause_l() {
  ...
  if (result == Result::OK) {
    // Note that OpenSL ES does NOT reset its millisecond position when OUTPUT
    // is paused.
...
Result AudioOutputStreamOpenSLES::requestFlush_l() {
  ...
  if (mPlayInterface == nullptr || mSimpleBufferQueueInterface == nullptr) {
    ...
    SLresult slResult =
        (*mSimpleBufferQueueInterface)->Clear(mSimpleBufferQueueInterface);
...
Result AudioOutputStreamOpenSLES::requestStop_l() {
  ...
  if (result == Result::OK) {
    ...
    // Also clear the buffer queue so the old data won't be played if the stream
    // is restarted. Call the _l function that expects to already be under a
    // lock.
    ...
    mPositionMillis
        .reset32(); // OpenSL ES resets its millisecond position when stopped.
    int64_t framesWritten = getFramesWritten();
    if (framesWritten >= 0) {
      setFramesRead(framesWritten);
    }
