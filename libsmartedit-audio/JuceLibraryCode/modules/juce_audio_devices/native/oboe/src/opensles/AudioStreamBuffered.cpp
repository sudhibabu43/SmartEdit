...
constexpr int kDefaultBurstsPerBuffer =
    16; // arbitrary, allows dynamic latency tuning
constexpr int kMinBurstsPerBuffer =
    4; // arbitrary, allows dynamic latency tuning
...
void AudioStreamBuffered::allocateFifo() {
  ...
  if (usingFIFO()) {
    ...
    if (capacityFrames == oboe::kUnspecified) {
      ...
      int32_t minFramesPerBufferByBursts =
          getFramesPerBurst() * kMinBurstsPerBuffer;
      if (capacityFrames <= minFramesPerBufferByBursts) {
        ...
        int32_t numBursts =
            (capacityFrames + getFramesPerBurst() - 1) / getFramesPerBurst();
        capacityFrames = numBursts * getFramesPerBurst();
      }
    }

    mFifoBuffer =
        std::make_unique<FifoBuffer>(getBytesPerFrame(), capacityFrames);
...
// This is called by the OpenSL ES callback to read or write the back end of the
// FIFO.
DataCallbackResult AudioStreamBuffered::onDefaultCallback(void *audioData,
                                                          int numFrames) {
  int32_t framesTransferred = 0;

  if (getDirection() == oboe::Direction::Output) {
    // Read from the FIFO and write to audioData, clear part of buffer if not
    // enough data.
    framesTransferred = mFifoBuffer->readNow(audioData, numFrames);
  } else {
    // Read from audioData and write to the FIFO
    framesTransferred =
        mFifoBuffer->write(audioData, numFrames); // There is no writeNow()
  }

  if (framesTransferred < numFrames) {
    LOGD("AudioStreamBuffered::%s(): xrun! framesTransferred = %d, numFrames = "
         "%d",
         __func__, framesTransferred, numFrames);
    // TODO If we do not allow FIFO to wrap then our timestamps will drift when
    // there is an XRun!
    incrementXRunCount();
  }
  markCallbackTime(
      static_cast<int32_t>(numFrames)); // so foreground knows how long to wait.
...
int64_t AudioStreamBuffered::predictNextCallbackTime() {
  ...
  int64_t nanosPerBuffer =
      (kNanosPerSecond * mLastBackgroundSize) / getSampleRate();
...
AudioStreamBuffered::transfer(void *readBuffer, const void *writeBuffer,
  ...
  do {
    ...
    if (getDirection() == Direction::Input) {
      ...
      int32_t emptyFrames =
          getBufferSizeInFrames() - static_cast<int32_t>(fullFrames);
    ...
    if (framesLeft > 0 && result >= 0 && timeoutNanoseconds > 0) {
      ...
      if (timeNow >= timeToQuit) {
        ...
        if (wakeTimeNanos <= 0) {
          ...
          sleepForNanos =
              (getFramesPerBurst() * kNanosPerSecond) / getSampleRate();
        } else {
...
ResultWithValue<int32_t>
AudioStreamBuffered::write(const void *buffer, int32_t numFrames,
...
ResultWithValue<int32_t>
AudioStreamBuffered::setBufferSizeInFrames(int32_t requestedFrames) {
  if (getState() == StreamState::Closed) {
    return ResultWithValue<int32_t>(Result::ErrorClosed);
  }
