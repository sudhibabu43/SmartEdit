...
SLuint32
AudioInputStreamOpenSLES::channelCountToChannelMask(int channelCount) const {
...
Result AudioInputStreamOpenSLES::open() {
  ...
  if (mFormat == AudioFormat::Unspecified) {
    mFormat = (getSdkVersion() < __ANDROID_API_M__) ? AudioFormat::I16
                                                    : AudioFormat::Float;
  }
  ...
  /**
   * API 23 (Marshmallow) introduced support for floating-point data
   * representation and an extended data format type: SLAndroidDataFormat_PCM_EX
   * for recording streams (playback streams got this in API 21). If running on
   * API 23+ use this newer format type, creating it from our original format.
   */
  ...
  SLresult result = EngineOpenSLES::getInstance().createAudioRecorder(
      &mObjectInterface, &audioSrc, &audioSink);
  ...
  if (SL_RESULT_SUCCESS != result) {
    ...
    if (getInputPreset() == InputPreset::VoicePerformance) {
      LOGD("OpenSL ES does not support InputPreset::VoicePerformance. Use "
           "VoiceRecognition.");
      mInputPreset = InputPreset::VoiceRecognition;
    }
    SLuint32 presetValue = OpenSLES_convertInputPreset(getInputPreset());
    result = (*configItf)
                 ->SetConfiguration(configItf, SL_ANDROID_KEY_RECORDING_PRESET,
  ...
  result =
      (*mObjectInterface)
          ->GetInterface(mObjectInterface, SL_IID_RECORD, &mRecordInterface);
  if (SL_RESULT_SUCCESS != result) {
...
Result AudioInputStreamOpenSLES::setRecordState_l(SLuint32 newState) {
  ...
  // LOGD("AudioInputStreamOpenSLES::%s(%u) returned %u", __func__, newState,
  // slResult);
  if (SL_RESULT_SUCCESS != slResult) {
    LOGE("AudioInputStreamOpenSLES::%s(%u) returned error %s", __func__,
         newState, getSLErrStr(slResult));
...
Result AudioInputStreamOpenSLES::requestPause() {
  LOGW("AudioInputStreamOpenSLES::%s() is intentionally not implemented for "
       "input "
       "streams",
       __func__);
...
Result AudioInputStreamOpenSLES::updateServiceFrameCounter() {
  ...
  if (mLock.try_lock()) {
    ...
    SLresult slResult =
        (*mRecordInterface)->GetPosition(mRecordInterface, &msec);
    if (SL_RESULT_SUCCESS != slResult) {
