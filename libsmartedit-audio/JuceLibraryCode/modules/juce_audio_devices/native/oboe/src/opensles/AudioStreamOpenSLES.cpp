...
static constexpr int32_t kHighLatencyBufferSizeMillis =
    20;                                               // typical Android period
static constexpr SLuint32 kAudioChannelCountMax = 30; // TODO Why 30?
static constexpr SLuint32 SL_ANDROID_UNKNOWN_CHANNELMASK =
    0; // Matches name used internally.

SLuint32
AudioStreamOpenSLES::channelCountToChannelMaskDefault(int channelCount) const {
...
static bool s_isLittleEndian() {
  ...
  return (*reinterpret_cast<uint8_t *>(&value) ==
          1); // Does address point to LSB?
}

SLuint32 AudioStreamOpenSLES::getDefaultByteOrder() {
  return s_isLittleEndian() ? SL_BYTEORDER_LITTLEENDIAN
                            : SL_BYTEORDER_BIGENDIAN;
}

Result AudioStreamOpenSLES::open() {

  LOGI("AudioStreamOpenSLES::open() chans=%d, rate=%d", mChannelCount,
       mSampleRate);

  // OpenSL ES only supports I16 and Float
  if (mFormat != AudioFormat::I16 && mFormat != AudioFormat::Float) {
    LOGW("%s() Android's OpenSL ES implementation only supports I16 and Float. "
         "Format: %d",
         __func__, mFormat);
...
SLresult
AudioStreamOpenSLES::finishCommonOpen(SLAndroidConfigurationItf configItf) {
  // Setting privacy sensitive mode and allowed capture policy are not supported
  // for OpenSL ES.
...
int32_t AudioStreamOpenSLES::calculateOptimalBufferQueueLength() {
  ...
  // The buffer capacity needs to be at least twice the size of the requested
  // callbackSize so that we can have double buffering.
  minCapacity = std::max(minCapacity, kDoubleBufferCount * mFramesPerCallback);
  if (minCapacity > 0) {
    int32_t queueLengthFromCapacity =
        roundUpDivideByN(minCapacity, likelyFramesPerBurst);
    queueLength = std::max(queueLength, queueLengthFromCapacity);
  }
  queueLength = std::min(queueLength, kBufferQueueLengthMax); // clip to max
  // TODO Investigate the effect of queueLength on latency for normal streams.
  // (not low latency)
  return queueLength;
}
...
int32_t AudioStreamOpenSLES::estimateNativeFramesPerBurst() {
  ...
  // Calculate the size of a fixed duration high latency buffer based on sample
  // rate. Estimate sample based on default options in order of priority.
  ...
  if (getSdkVersion() >= __ANDROID_API_N_MR1__ &&
    ...
    int32_t numBursts =
        roundUpDivideByN(framesPerHighLatencyBuffer, framesPerBurst);
    framesPerBurst *= numBursts;
    LOGD("AudioStreamOpenSLES:%s() NOT low latency, numBursts = %d, "
         "mSampleRate = %d, set framesPerBurst = %d",
...
Result AudioStreamOpenSLES::configureBufferSizes(int32_t sampleRate) {
  LOGD("AudioStreamOpenSLES:%s(%d) initial mFramesPerBurst = %d, "
       "mFramesPerCallback = %d",
  ...
  LOGD("AudioStreamOpenSLES:%s(%d) final mFramesPerBurst = %d, "
       "mFramesPerCallback = %d",
       __func__, mSampleRate, mFramesPerBurst, mFramesPerCallback);
  ...
  if (!usingFIFO()) {
    ...
    if (mBufferCapacityInFrames <= 0) {
      ...
      LOGE("AudioStreamOpenSLES::open() numeric overflow because "
           "mFramesPerBurst = %d",
           mFramesPerBurst);
...
SLuint32
AudioStreamOpenSLES::convertPerformanceMode(PerformanceMode oboeMode) const {
  ...
  case PerformanceMode::LowLatency:
    openslMode = (getSessionId() == SessionId::None)
...
PerformanceMode
AudioStreamOpenSLES::convertPerformanceMode(SLuint32 openslMode) const {
...
void AudioStreamOpenSLES::logUnsupportedAttributes() {
  ...
  if (mPerformanceMode != PerformanceMode::None &&
      sdkVersion < __ANDROID_API_N_MR1__) {
    LOGW("PerformanceMode [AudioStreamBuilder::setPerformanceMode()] "
         "is not supported on OpenSLES streams running on pre-Android N-MR1 "
         "versions.");
  }
  // Content Type
...
SLresult AudioStreamOpenSLES::configurePerformanceMode(
  ...
  result = (*configItf)
               ->SetConfiguration(configItf, SL_ANDROID_KEY_PERFORMANCE_MODE,
...
SLresult AudioStreamOpenSLES::updateStreamParameters(
    SLAndroidConfigurationItf configItf) {
  SLresult result = SL_RESULT_SUCCESS;
  if (getSdkVersion() >= __ANDROID_API_N_MR1__ && configItf != nullptr) {
    ...
    result = (*configItf)
                 ->GetConfiguration(configItf, SL_ANDROID_KEY_PERFORMANCE_MODE,
                                    &performanceModeSize, &performanceMode);
    // A bug in GetConfiguration() before P caused a wrong result code to be
    // returned.
    ...
    if (SL_RESULT_SUCCESS != result) {
      LOGW("GetConfiguration(SL_ANDROID_KEY_PERFORMANCE_MODE) returned %d",
           result);
      mPerformanceMode =
          PerformanceMode::None; // If we can't query it then assume None.
    } else {
      mPerformanceMode =
          convertPerformanceMode(performanceMode); // convert SL to Oboe mode
    }
  } else {
    mPerformanceMode =
        PerformanceMode::None; // If we can't query it then assume None.
...
SLresult
AudioStreamOpenSLES::enqueueCallbackBuffer(SLAndroidSimpleBufferQueueItf bq) {
...
bool AudioStreamOpenSLES::processBufferCallback(
    SLAndroidSimpleBufferQueueItf bq) {
  bool shouldStopStream = false;
  // Ask the app callback to process the buffer.
  DataCallbackResult result = fireDataCallback(
      mCallbackBuffer[mCallbackBufferIndex].get(), mFramesPerCallback);
...
// This callback handler is called every time a buffer has been processed by
// OpenSL ES.
...
SLresult AudioStreamOpenSLES::registerBufferQueueCallback() {
  ...
  SLresult result =
      (*mObjectInterface)
          ->GetInterface(mObjectInterface, SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                         &mSimpleBufferQueueInterface);
  if (SL_RESULT_SUCCESS != result) {
    LOGE("get buffer queue interface:%p result:%s", mSimpleBufferQueueInterface,
         getSLErrStr(result));
  } else {
    // Register the BufferQueue callback
    result = (*mSimpleBufferQueueInterface)
                 ->RegisterCallback(mSimpleBufferQueueInterface, bqCallbackGlue,
                                    this);
    if (SL_RESULT_SUCCESS != result) {
