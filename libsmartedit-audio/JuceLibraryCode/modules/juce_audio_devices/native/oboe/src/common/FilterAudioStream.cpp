...
// writes of any size. It uses DataConversionFlowGraph::read()
...
//                <= FilterAudioStream::onAudioReady()
//
// Input blocking uses FixedBlockReader::read() // TODO may not need block
// adapter
//                <= SourceFloatCaller::onProcess()
//                <=== SinkFloat::read()
//                <= DataConversionFlowGraph::read()
...
ResultWithValue<int32_t> FilterAudioStream::write(const void *buffer,
  ...
  while (true) {
    int32_t numRead = mFlowGraph->read(mBlockingBuffer.get(),
                                       getFramesPerBurst(), timeoutNanoseconds);
    if (numRead < 0) {
    ...
    auto writeResult =
        mChildStream->write(mBlockingBuffer.get(), numRead, timeoutNanoseconds);
    if (!writeResult) {
      return writeResult;
    }
...
// The necessary data will be read from the child stream using a flowgraph
// callback.
...
DataCallbackResult FilterAudioStream::onAudioReady(AudioStream *oboeStream,
  ...
  return (framesProcessed < numFrames) ? DataCallbackResult::Stop
