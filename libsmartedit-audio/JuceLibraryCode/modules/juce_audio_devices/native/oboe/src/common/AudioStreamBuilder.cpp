...
/**
 * The following default values are used when oboe does not have any better way
 * of determining the optimal values for an audio stream. This can happen when:
 *
 * - Client is creating a stream on API < 26 (OpenSLES) but has not supplied the
 * optimal sample rate and/or frames per burst
 * - Client is creating a stream on API 16 (OpenSLES) where
 * AudioManager.PROPERTY_OUTPUT_* values are not available
 */
int32_t DefaultStreamValues::SampleRate =
    48000; // Common rate for mobile audio and video
...
// This might be useful if you want to force all the unit tests to use OpenSL
// ES.
...
AudioStream *AudioStreamBuilder::build() {
  ...
  if (isAAudioRecommended() && mAudioApi != AudioApi::OpenSLES) {
    ...
    stream = new AudioStreamAAudio(*this);
    LOGE("Creating AAudio stream on 8.0 because it was specified. This is "
         "error prone.");
  } else {
...
bool AudioStreamBuilder::isCompatible(AudioStreamBase &other) {
  return (getSampleRate() == oboe::Unspecified ||
          getSampleRate() == other.getSampleRate()) &&
         (getFormat() == (AudioFormat)oboe::Unspecified ||
...
Result AudioStreamBuilder::openStream(AudioStream **streamPP) {
  ...
  LOGI("%s() %s -------- %s --------", __func__,
       getDirection() == Direction::Input ? "INPUT" : "OUTPUT",
       getVersionText());
  ...
  *streamPP = nullptr;
  ...
  bool conversionNeeded =
      QuirksManager::getInstance().isConversionNeeded(*this, childBuilder);
  ...
  if (conversionNeeded) {
    ...
    if (isCompatible(*tempStream)) {
      // The child stream would work as the requested stream so we can just use
      // it directly.
      *streamPP = tempStream;
      return result;
    } else {
      ...
      if (getFramesPerDataCallback() == oboe::Unspecified) {
        parentBuilder.setFramesPerCallback(
            tempStream->getFramesPerDataCallback());
      }

      // Use childStream in a FilterAudioStream.
      LOGI("%s() create a FilterAudioStream for data conversion.", __func__);
      FilterAudioStream *filterStream =
          new FilterAudioStream(parentBuilder, tempStream);
  ...
  bool wasMMapOriginallyEnabled =
      AAudioExtensions::getInstance().isMMapEnabled();
  ...
  if (result == Result::OK) {
    ...
    if (streamP->getDirection() == Direction::Input) {
      // For input, small size does not improve latency because the stream is
      // usually run close to empty. And a low size can result in XRuns so
    ...
    if (optimalBufferSize >= 0) {
      ...
      if (!setBufferResult) {
        LOGW("Failed to setBufferSizeInFrames(%d). Error was %s",
             optimalBufferSize, convertToText(setBufferResult.error()));
      }
    }
...
Result
AudioStreamBuilder::openStream(std::shared_ptr<AudioStream> &sharedStream) {
  sharedStream.reset();
  AudioStream *streamptr;
  auto result = openStream(&streamptr);
  if (result == Result::OK) {
