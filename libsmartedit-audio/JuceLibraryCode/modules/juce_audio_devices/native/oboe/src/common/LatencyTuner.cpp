...
Result LatencyTuner::tune() {
  ...
  // When state is Active attempt to change the buffer size if the number of
  // xRuns has increased.
  if (mState == State::Active) {
    ...
    if (xRunCountResult == Result::OK) {
      if ((xRunCountResult.value() - mPreviousXRuns) > 0) {
        ...
        if (requestedBufferSize > mMaxBufferSize)
          requestedBufferSize = mMaxBufferSize;

        // Note that this will not allocate more memory. It simply determines
        // how much of the existing buffer capacity will be used. The size will
        // be clipped to the bufferCapacity by AAudio.
        auto setBufferResult =
...
bool LatencyTuner::isAtMaximumBufferSize() { return mState == State::AtMax; }
