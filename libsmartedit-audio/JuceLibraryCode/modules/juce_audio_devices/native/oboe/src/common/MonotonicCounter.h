...
public:
   ...
   */
  int64_t get() const { return mCounter64; }

  /**
   * set the current value of the counter
   */
  void set(int64_t counter) { mCounter64 = counter; }
   ...
   *
   * This counter32 should NOT be allowed to advance by more than 0x7FFFFFFF
   * between calls. Think of the wrapping counter like a sine wave. If the
   * frequency of the signal is more than half the sampling rate (Nyquist rate)
   * then you cannot measure it properly. If the counter wraps around every 24
   * hours then we should measure it with a period of less than 12 hours.
   *
