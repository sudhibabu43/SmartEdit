...
/**
 * An AudioStream that wraps another AudioStream and provides audio data
 * conversion. Operations may include channel conversion, data format conversion
 * and/or sample rate conversion.
 */
class FilterAudioStream : public AudioStream, AudioStreamCallback {
public:
  /**
   * Construct an `AudioStream` using the given `AudioStreamBuilder` and a child
   * AudioStream.
   *
  ...
  FilterAudioStream(const AudioStreamBuilder &builder, AudioStream *childStream)
    ...
    if (builder.isDataCallbackSpecified()) {
      ...
      const int size =
          childStream->getFramesPerBurst() * childStream->getBytesPerFrame();
  ...
  AudioStream *getChildStream() const { return mChildStream.get(); }
  ...
  StreamState getState() override { return mChildStream->getState(); }

  Result waitForStateChange(StreamState inputState, StreamState *nextState,
                            int64_t timeoutNanoseconds) override {
    return mChildStream->waitForStateChange(inputState, nextState,
                                            timeoutNanoseconds);
  }
  ...
  AudioApi getAudioApi() const override { return mChildStream->getAudioApi(); }

  void updateFramesWritten() override {
    // TODO for output, just count local writes?
    mFramesWritten =
        static_cast<int64_t>(mChildStream->getFramesWritten() * mRateScaler);
  }

  void updateFramesRead() override {
    // TODO for input, just count local reads?
    mFramesRead =
        static_cast<int64_t>(mChildStream->getFramesRead() * mRateScaler);
  }
  ...
  Result getTimestamp(clockid_t clockId, int64_t *framePosition,
    ...
    Result result =
        mChildStream->getTimestamp(clockId, &childPosition, timeNanoseconds);
...
private:
  std::unique_ptr<AudioStream>
      mChildStream; // this stream wraps the child stream
