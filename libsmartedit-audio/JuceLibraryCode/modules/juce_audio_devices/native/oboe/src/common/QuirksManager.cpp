...
bool QuirksManager::DeviceQuirks::isAAudioMMapPossible(
    const AudioStreamBuilder &builder) const {
  bool isSampleRateCompatible = builder.getSampleRate() == oboe::Unspecified ||
                                builder.getSampleRate() == kCommonNativeRate ||
                                builder.getSampleRateConversionQuality() !=
...
bool QuirksManager::DeviceQuirks::shouldConvertFloatToI16ForOutputStreams() {
  std::string productManufacturer =
      getPropertyString("ro.product.manufacturer");
  if (getSdkVersion() < __ANDROID_API_L__) {
    return true;
  } else if ((productManufacturer == "vivo") &&
             (getSdkVersion() < __ANDROID_API_M__)) {
    return true;
  }
  return false;
}
...
public:
  ...
  int32_t getExclusiveTopMarginInBursts() const override { return kTopMargin; }

  // See Oboe issues #824 and #1247 for more information.
  bool isMonoMMapActuallyStereo() const override {
    return isExynos9810 || isExynos850; // TODO We can make this version
                                        // specific if it gets fixed.
  }
  ...
  bool isMMapSafe(const AudioStreamBuilder &builder) override {
    ...
    // This detects b/159066712 , S20 LSI has corrupt low latency audio
    // recording and turns off MMAP. See also
    // https://github.com/google/oboe/issues/892
    bool isRecordingCorrupted =
        isInput && isExynos990 && mBuildChangelist < 19350896;

    // Certain S9+ builds record silence when using MMAP and not using the
    // VoiceCommunication preset. See https://github.com/google/oboe/issues/1110
    bool wouldRecordSilence =
        isInput && isExynos9810 && mBuildChangelist <= 18847185 &&
        (builder.getInputPreset() != InputPreset::VoiceCommunication);

    if (wouldRecordSilence) {
      LOGI("QuirksManager::%s() Requested stream configuration would result in "
           "silence on "
           "this device. Switching off MMAP.",
           __func__);
    }
...
bool QuirksManager::isConversionNeeded(const AudioStreamBuilder &builder,
  ...
  // There should be no conversion for IEC61937. Sample rates and channel counts
  ...
  // There are multiple bugs involving using callback with a specified callback
  // size. Issue #778: O to Q had a problem with Legacy INPUT streams for FLOAT
  // streams and a specified callback size. It would assert because of a bad
  // buffer size.
  //
  // Issue #973: O to R had a problem with Legacy output streams using callback
  // and a specified callback size. An AudioTrack stream could still be running
  // when the AAudio FixedBlockReader was closed. Internally
  // b/161914201#comment25
  //
  // Issue #983: O to R would glitch if the framesPerCallback was too small.
  //
  // Most of these problems were related to Legacy stream. MMAP was OK. But we
  // don't know if we will get an MMAP stream. So, to be safe, just do the
  // conversion in Oboe.
  if (OboeGlobals::areWorkaroundsEnabled() && builder.willUseAAudio() &&
  ...
  // If a SAMPLE RATE is specified for low latency, let the native code choose
  // an optimal rate. This isn't really a workaround. It is an Oboe feature that
  // is convenient to place here.
  ...
  if (builder.getSampleRate() != oboe::Unspecified &&
      builder.getSampleRateConversionQuality() !=
          SampleRateConversionQuality::None &&
      isLowLatency) {
    childBuilder.setSampleRate(
        oboe::Unspecified); // native API decides the best sample rate
    conversionNeeded = true;
  }

  // Data Format
  // OpenSL ES and AAudio before P do not support FAST path for FLOAT capture.
  if (OboeGlobals::areWorkaroundsEnabled() && isFloat && isInput &&
      builder.isFormatConversionAllowed() && isLowLatency &&
      (!builder.willUseAAudio() || (getSdkVersion() < __ANDROID_API_P__))) {
    childBuilder.setFormat(AudioFormat::I16); // needed for FAST track
    conversionNeeded = true;
    LOGI("QuirksManager::%s() forcing internal format to I16 for low latency",
         __func__);
  }
  ...
  if (OboeGlobals::areWorkaroundsEnabled() && isFloat && !isInput &&
    ...
    conversionNeeded = true;
    LOGI("QuirksManager::%s() float was requested but not supported on pre-L "
         "devices "
         "and some devices like Vivo devices may have issues on L devices, "
         "creating an underlying I16 stream and using format conversion to "
         "provide a float "
         "stream",
         __func__);
  }
  ...
  if (OboeGlobals::areWorkaroundsEnabled() &&
      ...
      builder.getChannelCount() == kChannelCountStereo && isInput &&
      isLowLatency &&
    ...
    conversionNeeded = true;
    LOGI("QuirksManager::%s() using mono internally for low latency on O",
         __func__);
  } else if (OboeGlobals::areWorkaroundsEnabled() &&
             builder.getChannelCount() == kChannelCountMono && isInput &&
             mDeviceQuirks->isMonoMMapActuallyStereo() &&
             builder.willUseAAudio()
             // Note: we might use this workaround on a device that supports
             // MMAP but will use Legacy for this stream.  But this will only
             // happen on devices that have the broken mono.
             && mDeviceQuirks->isAAudioMMapPossible(builder)) {
    // Workaround for mono actually running in stereo mode.
    childBuilder.setChannelCount(
        kChannelCountStereo); // Use stereo and extract first channel.
    conversionNeeded = true;
    LOGI("QuirksManager::%s() using stereo internally to avoid broken mono",
         __func__);
  }
  // Note that MMAP does not support mono in 8.1. But that would only matter on
  // Pixel 1 phones and they have almost all been updated to 9.0.
