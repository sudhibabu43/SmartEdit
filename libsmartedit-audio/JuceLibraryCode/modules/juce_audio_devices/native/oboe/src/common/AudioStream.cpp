...
void AudioStream::checkScheduler() {
  int scheduler =
      sched_getscheduler(0) & ~SCHED_RESET_ON_FORK; // for current thread
...
DataCallbackResult AudioStream::fireDataCallback(void *audioData,
                                                 int32_t numFrames) {
...
Result AudioStream::start(int64_t timeoutNanoseconds) {
  ...
  if (timeoutNanoseconds <= 0)
    return result;
  return waitForStateTransition(StreamState::Starting, StreamState::Started,
                                timeoutNanoseconds);
}
...
Result AudioStream::pause(int64_t timeoutNanoseconds) {
  ...
  if (timeoutNanoseconds <= 0)
    return result;
  return waitForStateTransition(StreamState::Pausing, StreamState::Paused,
                                timeoutNanoseconds);
}
...
Result AudioStream::flush(int64_t timeoutNanoseconds) {
  ...
  if (timeoutNanoseconds <= 0)
    return result;
  return waitForStateTransition(StreamState::Flushing, StreamState::Flushed,
                                timeoutNanoseconds);
}
...
Result AudioStream::stop(int64_t timeoutNanoseconds) {
  ...
  if (timeoutNanoseconds <= 0)
    return result;
  return waitForStateTransition(StreamState::Stopping, StreamState::Stopped,
                                timeoutNanoseconds);
}
...
ResultWithValue<int32_t> AudioStream::getAvailableFrames() {
  ...
  if (readCounter < 0)
    return ResultWithValue<int32_t>::createBasedOnSign(readCounter);
  int64_t writeCounter = getFramesWritten();
  if (writeCounter < 0)
    return ResultWithValue<int32_t>::createBasedOnSign(writeCounter);
  int32_t framesAvailable = writeCounter - readCounter;
  return ResultWithValue<int32_t>(framesAvailable);
}

ResultWithValue<int32_t>
AudioStream::waitForAvailableFrames(int32_t numFrames,
                                    int64_t timeoutNanoseconds) {
  if (numFrames == 0)
    return Result::OK;
  if (numFrames < 0)
  ...
  return (!ready) ? ResultWithValue<int32_t>(Result::ErrorTimeout)
