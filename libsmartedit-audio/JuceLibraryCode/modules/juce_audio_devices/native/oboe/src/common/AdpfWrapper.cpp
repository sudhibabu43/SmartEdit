...
typedef APerformanceHintSession *(*APH_createSession)(APerformanceHintManager *,
                                                      const int32_t *, size_t,
                                                      int64_t);
typedef void (*APH_reportActualWorkDuration)(APerformanceHintSession *,
                                             int64_t);
...
static int loadAphFunctions() {
  if (gAPerformanceHintBindingInitialized)
    return true;
  ...
  gAPH_getManagerFn =
      (APH_getManager)dlsym(handle_, "APerformanceHint_getManager");
  ...
  gAPH_closeSessionFn =
      (APH_closeSession)dlsym(handle_, "APerformanceHint_closeSession");
  if (gAPH_getManagerFn == nullptr) {
    return -1004;
  }
...
int AdpfWrapper::open(pid_t threadId, int64_t targetDurationNanos) {
  ...
  if (sUseAlternativeHack) {
    // TODO Remove this hack when we finish experimenting with alternative
    // algorithms. The A5 is an arbitrary signal to a hacked version of ADPF to
    // try an alternative algorithm that is not based on PID.
    targetDurationNanos = (targetDurationNanos & ~0xFF) | 0xA5;
  }
  mHintSession = gAPH_createSessionFn(manager, &thread32, 1 /* size */,
                                      targetDurationNanos);
...
void AdpfWrapper::onEndCallback(double durationScaler) {
  if (isOpen()) {
    ...
    int64_t scaledDurationNanos =
        static_cast<int64_t>(actualDurationNanos * durationScaler);
