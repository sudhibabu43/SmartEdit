...
void convertFloatToPcm16(const float *source, int16_t *destination,
                         int32_t numSamples) {
...
void convertPcm16ToFloat(const int16_t *source, float *destination,
                         int32_t numSamples) {
  for (int i = 0; i < numSamples; i++) {
...
template <> const char *convertToText<Result>(Result returnCode) {
  switch (returnCode) {
  case Result::OK:
    return "OK";
...
template <> const char *convertToText<AudioFormat>(AudioFormat format) {
  switch (format) {
...
template <> const char *convertToText<PerformanceMode>(PerformanceMode mode) {
  switch (mode) {
...
template <> const char *convertToText<SharingMode>(SharingMode mode) {
  switch (mode) {
...
template <> const char *convertToText<Direction>(Direction direction) {
  switch (direction) {
...
template <> const char *convertToText<StreamState>(StreamState state) {
  switch (state) {
...
template <> const char *convertToText<AudioApi>(AudioApi audioApi) {
...
template <> const char *convertToText<AudioStream *>(AudioStream *stream) {
  ...
  s << "StreamID: " << static_cast<void *>(stream) << std::endl
    ...
    << "FramesPerDataCallback: " << stream->getFramesPerDataCallback()
    << std::endl
...
template <> const char *convertToText<Usage>(Usage usage) {
...
template <> const char *convertToText<ContentType>(ContentType contentType) {
...
template <> const char *convertToText<InputPreset>(InputPreset inputPreset) {
...
template <> const char *convertToText<SessionId>(SessionId sessionId) {
...
template <> const char *convertToText<ChannelCount>(ChannelCount channelCount) {
