...
class AlsaClient {
  ...
  auto findPortIterator(int portId) const {
    ...
    return (iter == ports.end() || (*iter)->getPortId() != portId) ? ports.end()
                                                                   : iter;
  }
...
public:
  ...
  struct Port {
    explicit Port(bool forInput) noexcept : isInput(forInput) {}
    ...
    void enableCallback(bool enable) { callbackEnabled = enable; }
    ...
    bool sendMessageNow(const MidiMessage &message) {
      ...
      while (numBytes > 0) {
        auto numSent =
            snd_midi_event_encode(midiParser, data, numBytes, &event);
    ...
    void createPort(const String &name, bool enableSubscription) {
      if (auto seqHandle = client->get()) {
        const unsigned int caps =
            isInput ? (SND_SEQ_PORT_CAP_WRITE |
    ...
    void handlePartialSysexMessage(const uint8 *messageData, int numBytesSoFar,
                                   double timeStamp) {
      if (callbackEnabled)
        callback->handlePartialSysexMessage(midiInput, messageData,
                                            numBytesSoFar, timeStamp);
    }
  ...
  void handleIncomingMidiMessage(snd_seq_event *event,
  ...
  void handlePartialSysexMessage(snd_seq_event *event, const uint8 *messageData,
  ...
  void deletePort(Port *port) {
    ...
    if (const auto iter = findPortIterator(port->getPortId());
        iter != ports.end())
      ports.erase(iter);
  }
...
private:
  AlsaClient() {
    ...
    if (handle != nullptr) {
      ...
      announcementsIn = snd_seq_create_simple_port(
          handle, TRANS("announcements").toRawUTF8(), SND_SEQ_PORT_CAP_WRITE,
          SND_SEQ_PORT_TYPE_MIDI_GENERIC | SND_SEQ_PORT_TYPE_APPLICATION);
      snd_seq_connect_from(handle, announcementsIn, SND_SEQ_CLIENT_SYSTEM,
  ...
  public:
    explicit SequencerThread(AlsaClient &c) : client(c) {}
  ...
  private:
    // If we directly call MidiDeviceListConnectionBroadcaster::get() from the
    // background thread, there's a possibility that we'll deadlock in the
    // following scenario:
    // - The main thread calls MidiDeviceListConnectionBroadcaster::get() for
    // the first time
    //   (e.g. to register a listener). The static
    //   MidiDeviceListConnectionBroadcaster singleton begins construction.
    //   During the constructor, an AlsaClient is created to iterate midi
    //   ins/outs.
    // - The AlsaClient starts a new SequencerThread. If connections are
    // updated, the
    //   SequencerThread may call
    //   MidiDeviceListConnectionBroadcaster::get().notify() while the
    //   MidiDeviceListConnectionBroadcaster singleton is still being created.
    // - The SequencerThread blocks until the
    // MidiDeviceListConnectionBroadcaster has been
    //   created on the main thread, but the
    //   MidiDeviceListConnectionBroadcaster's constructor can't complete until
    //   the AlsaClient's destructor has run, which in turn requires the
    ...
    std::thread thread{[this] {
      ...
      if (snd_midi_event_new(maxEventSize, &midiParser) >= 0) {
        ...
        snd_seq_poll_descriptors(seqHandle, pfd.data(), (unsigned int)numPfds,
                                 POLLIN);

        std::vector<uint8> buffer(maxEventSize);

        while (!shouldStop) {
          // This timeout shouldn't be too long, so that the program can exit in
          // a timely manner
          if (poll(pfd.data(), (nfds_t)numPfds, 100) > 0) {
            ...
            do {
              ...
              if (snd_seq_event_input(seqHandle, &inputEvent) >= 0) {
                ...
                const auto foundEvent =
                    std::find(std::begin(systemEvents), std::end(systemEvents),
                              inputEvent->type);
                ...
                const auto numBytes = snd_midi_event_decode(
                    midiParser, buffer.data(), maxEventSize, inputEvent);
...
static AlsaClient::Port *
iterateMidiClient(AlsaClient &client, snd_seq_client_info_t *clientInfo,
                  bool forInput, Array<MidiDeviceInfo> &devices,
  ...
  while (--numPorts >= 0) {
    if (snd_seq_query_next_port(seqHandle, portInfo) == 0 &&
        (snd_seq_port_info_get_capability(portInfo) &
         (forInput ? SND_SEQ_PORT_CAP_SUBS_READ
                   : SND_SEQ_PORT_CAP_SUBS_WRITE)) != 0) {
      String portName(snd_seq_port_info_get_name(portInfo));
      auto portID = snd_seq_port_info_get_port(portInfo);

      MidiDeviceInfo device(portName,
                            getFormattedPortIdentifier(sourceClient, portID));
      devices.add(device);

      if (deviceIdentifierToOpen.isNotEmpty() &&
          deviceIdentifierToOpen == device.identifier) {
        if (portID != -1) {
...
static AlsaClient::Port *
iterateMidiDevices(bool forInput, Array<MidiDeviceInfo> &devices,
                   const String &deviceIdentifierToOpen) {
  AlsaClient::Port *port = nullptr;
    ...
    if (snd_seq_system_info(seqHandle, systemInfo) == 0) {
      ...
      while (--numClients >= 0) {
        if (snd_seq_query_next_client(seqHandle, clientInfo) == 0) {
          port = iterateMidiClient(*client, clientInfo, forInput, devices,
...
struct AlsaPortPtr {
  explicit AlsaPortPtr(AlsaClient::Port *p) : ptr(p) {}

  virtual ~AlsaPortPtr() noexcept {
...
std::unique_ptr<MidiInput> MidiInput::openDevice(const String &deviceIdentifier,
  ...
  std::unique_ptr<MidiInput> midiInput(
      new MidiInput(port->getPortName(), deviceIdentifier));
...
MidiInput::createNewDevice(const String &deviceName,
  ...
  std::unique_ptr<MidiInput> midiInput(
      new MidiInput(deviceName, getFormattedPortIdentifier(client->getId(),
                                                           port->getPortId())));
...
void MidiInput::start() { internal->ptr->enableCallback(true); }

void MidiInput::stop() { internal->ptr->enableCallback(false); }
...
MidiOutput::openDevice(const String &deviceIdentifier) {
  ...
  std::unique_ptr<MidiOutput> midiOutput(
      new MidiOutput(port->getPortName(), deviceIdentifier));
...
MidiOutput::createNewDevice(const String &deviceName) {
  ...
  std::unique_ptr<MidiOutput> midiOutput(new MidiOutput(
      deviceName,
...
MidiDeviceListConnection::make(std::function<void()> cb) {
  ...
  // We capture the AlsaClient instance here to ensure that it remains alive for
  // at least as long as the MidiDeviceListConnection. This is necessary because
  // system change messages will only be processed when the AlsaClient's
  // SequencerThread is running.
  return {&broadcaster, broadcaster.add([fn = std::move(cb),
                                         client = AlsaClient::getInstance()] {
...
std::unique_ptr<MidiOutput> MidiOutput::openDevice(const String &) {
  return {};
}
