...
public:
  ...
  static Ref create(std::size_t numBuffers) {
    ...
    if (std::unique_ptr<std::byte[]> storage{
            new std::byte[storageSizeForNumBuffers(numBuffers)]})
...
private:
  // Do not call the base constructor here as this will zero-initialize the
  // first buffer, for which no storage may be available though (when numBuffers
  // == 0).
  explicit ManagedAudioBufferList(std::size_t numBuffers) {
    mNumberBuffers = static_cast<UInt32>(numBuffers);
  }

  static constexpr auto audioBufferListHeaderSize =
...
template <typename T> static auto getDataPtrAndSize(T &t) {
...
audioObjectHasProperty(AudioObjectID objectID,
                       const AudioObjectPropertyAddress address) {
  return objectID != kAudioObjectUnknown &&
         AudioObjectHasProperty(objectID, &address);
}

template <typename T, typename OnError = IgnoreUnused>
[[nodiscard]] static auto
audioObjectGetProperty(AudioObjectID objectID,
                       const AudioObjectPropertyAddress address,
                       OnError &&onError = {}) {
  using Result =
      std::conditional_t<std::is_same_v<T, AudioBufferList>,
                         ManagedAudioBufferList::Ref, std::optional<T>>;
  ...
  auto result = [&] {
    if constexpr (std::is_same_v<T, AudioBufferList>) {
      ...
      if (auto status = AudioObjectGetPropertyDataSize(objectID, &address, 0,
                                                       nullptr, &size);
          status != noErr) {
        onError(status);
        return Result{};
      ...
      return ManagedAudioBufferList::create(
          ManagedAudioBufferList::numBuffersForStorageSize(size));
    } else {
      return T{};
    }
  }();
  ...
  if (auto status = AudioObjectGetPropertyData(objectID, &address, 0, nullptr,
                                               &size, ptr);
      status != noErr) {
    onError(status);
    return Result{};
...
static bool audioObjectSetProperty(AudioObjectID objectID,
  ...
  if (auto status =
          AudioObjectSetPropertyData(objectID, &address, 0, nullptr,
                                     static_cast<UInt32>(sizeof(T)), &value);
      status != noErr) {
    onError(status);
    return false;
  }
...
template <typename T, typename OnError = IgnoreUnused>
[[nodiscard]] static std::vector<T>
audioObjectGetProperties(AudioObjectID objectID,
                         const AudioObjectPropertyAddress address,
                         OnError &&onError = {}) {
  if (!audioObjectHasProperty(objectID, address))
    return {};
  ...
  if (auto status =
          AudioObjectGetPropertyDataSize(objectID, &address, 0, nullptr, &size);
      status != noErr) {
    onError(status);
    return {};
  }
  ...
  // AudioObjectGetPropertyData will probably write past the end of the result
  // buffer.
...
struct SystemVol {
  explicit SystemVol(AudioObjectPropertySelector selector) noexcept
      : outputDeviceID(audioObjectGetProperty<AudioDeviceID>(
                           kAudioObjectSystemObject,
                           {kAudioHardwarePropertyDefaultOutputDevice,
                            kAudioObjectPropertyScopeGlobal,
                            juceAudioObjectPropertyElementMain})
                           .value_or(kAudioObjectUnknown)),
        addr{selector, kAudioDevicePropertyScopeOutput,
             juceAudioObjectPropertyElementMain} {}
  ...
  bool setGain(float gain) const noexcept {
    return audioObjectSetProperty(outputDeviceID, addr,
                                  static_cast<Float32>(gain));
  }

  bool isMuted() const noexcept {
    return audioObjectGetProperty<UInt32>(outputDeviceID, addr).value_or(0) !=
           0;
  }

  bool setMuted(bool mute) const noexcept {
    return audioObjectSetProperty(outputDeviceID, addr,
                                  static_cast<UInt32>(mute ? 1 : 0));
  }
...
float JUCE_CALLTYPE SystemAudioVolume::getGain() {
  return SystemVol(juceAudioHardwareServiceDeviceProperty_VirtualMainVolume)
      .getGain();
}
...
struct CoreAudioClasses {
  ...
  //==============================================================================
  class CoreAudioInternal final : private Timer, private AsyncUpdater {
  private:
  ...
  public:
    CoreAudioInternal(CoreAudioIODevice &d, AudioDeviceID id, bool hasInput,
                      bool hasOutput)
        : owner(d), deviceID(id),
    ...
    ~CoreAudioInternal() override {
      ...
      AudioObjectRemovePropertyListener(deviceID, &pa, deviceListenerProc,
                                        this);

      stop(false);
    }

    auto getStreams() const {
      return std::array<Stream *, 2>{{inStream.get(), outStream.get()}};
    }
    ...
    void allocateTempBuffers() {
      ...
      const auto total = std::accumulate(
          streams.begin(), streams.end(), 0, [](int n, const auto &s) {
      ...
      for (auto *stream : streams)
        channels += stream != nullptr ? stream->allocateTempBuffers(
                                            tempBufSize, channels, audioBuffer)
                                      : 0;
    }
    ...
    Array<double> getSampleRatesFromDevice() const {
      ...
      if (auto ranges = audioObjectGetProperties<AudioValueRange>(
              deviceID,
    ...
    bool isDeviceAlive() const {
      return deviceID != 0 && audioObjectGetProperty<UInt32>(
                                  deviceID,
    ...
    bool updateDetailsFromDevice(const BigInteger &activeIns,
                                 const BigInteger &activeOuts) {
      stopTimer();

      if (!isDeviceAlive())
        return false;

      // this collects all the new details from the device without any locking,
      // then locks + swaps them afterwards.
      ...
      auto newInput = rawToUniquePtr(
          inStream != nullptr ? new Stream(true, *this, activeIns) : nullptr);
      auto newOutput = rawToUniquePtr(outStream != nullptr
                                          ? new Stream(false, *this, activeOuts)
                                          : nullptr);
      ...
      audioWorkgroup = [this]() -> AudioWorkgroup {
        ...
        if (auto *workgroup =
                audioObjectGetProperty<os_workgroup_t>(deviceID, pa)
                    .value_or(nullptr)) {
    ...
    bool updateDetailsFromDevice() {
      return updateDetailsFromDevice(getActiveChannels(inStream),
                                     getActiveChannels(outStream));
    }
    ...
    static auto getScope(bool input) {
      return input ? kAudioDevicePropertyScopeInput
    ...
    StringArray getSources(bool input) {
      StringArray s;
      auto types = audioObjectGetProperties<OSType>(
          deviceID,
          {kAudioDevicePropertyDataSources, kAudioObjectPropertyScopeWildcard,
           juceAudioObjectPropertyElementMain});
      ...
      for (auto type : types) {
        ...
        if (OK(AudioObjectGetPropertyData(deviceID, &pa, 0, nullptr, &transSize,
                                          &avt)))
          s.add(buffer);
      }
    ...
    int getCurrentSourceIndex(bool input) const {
      if (deviceID != 0) {
        if (auto currentSourceID = audioObjectGetProperty<OSType>(
                deviceID,
                {kAudioDevicePropertyDataSource, getScope(input),
                 juceAudioObjectPropertyElementMain},
                err2log())) {
          auto types = audioObjectGetProperties<OSType>(
              deviceID, {kAudioDevicePropertyDataSources,
                         kAudioObjectPropertyScopeWildcard,
                         juceAudioObjectPropertyElementMain});

          if (auto it = std::find(types.begin(), types.end(), *currentSourceID);
              it != types.end())
    ...
    double getNominalSampleRate() const {
      return static_cast<double>(audioObjectGetProperty<Float64>(
                                     deviceID,
    ...
    bool setNominalSampleRate(double newSampleRate) const {
      ...
      return audioObjectSetProperty(
          deviceID,
          {kAudioDevicePropertyNominalSampleRate,
           kAudioObjectPropertyScopeGlobal, juceAudioObjectPropertyElementMain},
    ...
    String reopen(const BigInteger &ins, const BigInteger &outs,
      ...
      // Annoyingly, after changing the rate and buffer size, some devices fail
      // to correctly report their new settings until some random time in the
      // future, so after calling updateDetailsFromDevice, we need to manually
      // bodge these values to make sure we're using the correct numbers..
      updateDetailsFromDevice(ins, outs);
      sampleRate = newSampleRate;
    ...
    bool start(AudioIODeviceCallback *callbackToNotify) {
      ...
      if (scopedProcID.get() == nullptr && deviceID != 0) {
        scopedProcID = [&self = *this, &lock = callbackLock,
                        nextProcID = ScopedAudioDeviceIOProcID{*this, deviceID,
                                                               audioIOProc},
                        dID = deviceID]() mutable -> ScopedAudioDeviceIOProcID {
          // It *looks* like AudioDeviceStart may start the audio callback
          // running, and then immediately lock an internal mutex. The same
          // mutex is locked before calling the audioIOProc. If we get very
          // unlucky, then we can end up with thread A taking the callbackLock
          // and calling AudioDeviceStart, followed by thread B taking the
          // CoreAudio lock and calling into audioIOProc, which waits on the
          // callbackLock. When thread A continues it attempts to take the
          // CoreAudio lock, and the program deadlocks.
    ...
    void audioCallback(const AudioTimeStamp *inputTimestamp,
      ...
      if (callback != nullptr) {
        for (int i = numInputChans; --i >= 0;) {
          ...
          auto src =
              ((const float *)inInputData->mBuffers[info.streamNum].mData) +
        ...
        const auto *timeStamp =
            numOutputChans > 0 ? outputTimestamp : inputTimestamp;
        const auto nanos =
            timeStamp != nullptr
                ? timeConversions.hostTimeToNanos(timeStamp->mHostTime)
                : 0;
        ...
        callback->audioDeviceIOCallbackWithContext(
            getTempBuffers(inStream), numInputChans, getTempBuffers(outStream),
            numOutputChans, bufferSize, context);

        for (int i = numOutputChans; --i >= 0;) {
          auto &info = outStream->channelInfo.getReference(i);
          auto src = outStream->tempBuffers[i];
          auto dest = ((float *)outOutputData->mBuffers[info.streamNum].mData) +
                      info.dataOffsetSamples;
          auto stride = info.dataStrideSamples;
    ...
    //==============================================================================
    struct Stream {
      Stream(bool isInput, CoreAudioInternal &parent,
             const BigInteger &activeRequested)
          : input(isInput), latency(getLatencyFromDevice(isInput, parent)),
            ...
            activeChans([&activeRequested, clearFrom = chanNames.size()] {
              ...
              result.setRange(clearFrom, result.getHighestBit() + 1 - clearFrom,
                              false);
              return result;
            }()),
      ...
      int allocateTempBuffers(int tempBufSize, int channelCount,
      ...
      static auto visitChannels(bool isInput, CoreAudioInternal &parent,
                                Visitor &&visitor) {
        struct Args {
          int stream, channelIdx, chanNum, streamChannels;
        };
        using VisitorResultType =
            typename std::invoke_result_t<Visitor, const Args &>::value_type;
        Array<VisitorResultType> result;
        int chanNum = 0;

        if (auto bufList = audioObjectGetProperty<AudioBufferList>(
                parent.deviceID,
                {kAudioDevicePropertyStreamConfiguration, getScope(isInput),
                 juceAudioObjectPropertyElementMain},
                parent.err2log())) {
          ...
          for (int i = 0; i < numStreams; ++i) {
            ...
            for (unsigned int j = 0; j < b.mNumberChannels; ++j) {
              // Passing an anonymous struct ensures that callback can't confuse
              // the argument order
              if (auto opt = visitor(Args{i, static_cast<int>(j), chanNum++,
                                          static_cast<int>(b.mNumberChannels)}))
      ...
      getChannelInfos(bool isInput, CoreAudioInternal &parent,
                      const BigInteger &active) {
        return visitChannels(
            ...
            [&](const auto &args) -> std::optional<CallbackDetailsForChannel> {
              ...
              return CallbackDetailsForChannel{args.stream, args.channelIdx,
                                               args.streamChannels};
            });
      }

      static StringArray getChannelNames(bool isInput,
                                         CoreAudioInternal &parent) {
        auto names = visitChannels(
            isInput, parent, [&](const auto &args) -> std::optional<String> {
              String name;
              const auto element =
                  static_cast<AudioObjectPropertyElement>(args.chanNum + 1);

              if (auto nameNSString =
                      audioObjectGetProperty<NSString *>(
                          parent.deviceID, {kAudioObjectPropertyElementName,
                                            getScope(isInput), element})
                          .value_or(nullptr)) {
      ...
      static int getBitDepthFromDevice(bool isInput,
                                       CoreAudioInternal &parent) {
        return static_cast<int>(
            audioObjectGetProperty<AudioStreamBasicDescription>(
                parent.deviceID,
                {kAudioStreamPropertyPhysicalFormat, getScope(isInput),
                 juceAudioObjectPropertyElementMain},
                parent.err2log())
      ...
      static int getLatencyFromDevice(bool isInput, CoreAudioInternal &parent) {
        ...
        const auto deviceLatency =
            audioObjectGetProperty<UInt32>(parent.deviceID,
                                           {kAudioDevicePropertyLatency, scope,
                                            juceAudioObjectPropertyElementMain})
                .value_or(0);

        const auto safetyOffset =
            audioObjectGetProperty<UInt32>(
                parent.deviceID, {kAudioDevicePropertySafetyOffset, scope,
                                  juceAudioObjectPropertyElementMain})
                .value_or(0);
        ...
        if (auto streams = audioObjectGetProperties<AudioStreamID>(
                parent.deviceID, {kAudioDevicePropertyStreams, scope,
                                  juceAudioObjectPropertyElementMain});
            !streams.empty())
          streamLatency =
              audioObjectGetProperty<UInt32>(
                  streams.front(), {kAudioStreamPropertyLatency, scope,
                                    juceAudioObjectPropertyElementMain})
                  .value_or(0);

        return static_cast<int>(deviceLatency + safetyOffset + framesInBuffer +
                                streamLatency);
      }

      bool checkTimestampsForDiscontinuity(
        ...
        if (channels > 0) {
          ...
          if (exactlyEqual(previousSampleTime, invalidSampleTime))
            previousSampleTime =
                timestamp != nullptr ? timestamp->mSampleTime : 0.0;
    ...
    static auto getWithDefault(const std::unique_ptr<Stream> &ptr,
                               Callback &&callback) {
    ...
    //==============================================================================
    static constexpr Float64 invalidSampleTime =
  ...
  private:
    ...
    public:
      ...
      ScopedAudioDeviceIOProcID(CoreAudioInternal &coreAudio, AudioDeviceID d,
                                AudioDeviceIOProc audioIOProc)
          : deviceID(d) {
        if (!coreAudio.OK(AudioDeviceCreateIOProcID(deviceID, audioIOProc,
                                                    &coreAudio, &proc)))
          proc = {};
      }
      ...
      operator=(ScopedAudioDeviceIOProcID &&other) noexcept {
    ...
    void timerCallback() override {
      ...
      if (!updateDetailsFromDevice())
        owner.stopInternal();
      else if ((oldBufferSize != bufferSize ||
                !approximatelyEqual(oldSampleRate, sampleRate)) &&
    ...
    static OSStatus deviceListenerProc(AudioDeviceID /*inDevice*/,
      ...
      const auto xruns = std::count_if(
          pa, pa + numAddresses, [](const AudioObjectPropertyAddress &x) {
            return x.mSelector == kAudioDeviceProcessorOverload;
          });

      intern.xruns += xruns;

      const auto detailsChanged = std::any_of(
          pa, pa + numAddresses, [](const AudioObjectPropertyAddress &x) {
            ...
            return std::find(std::begin(selectors), std::end(selectors),
                             x.mSelector) != std::end(selectors);
          });

      const auto requestedRestart = std::any_of(
          pa, pa + numAddresses, [](const AudioObjectPropertyAddress &x) {
            constexpr UInt32 selectors[]{
    ...
    bool OK(const OSStatus errorCode) const {
      ...
      const String errorMessage("CoreAudio error: " +
                                String::toHexString((int)errorCode));
  ...
  //==============================================================================
  class CoreAudioIODevice final : public AudioIODevice, private Timer {
  public:
    CoreAudioIODevice(CoreAudioIODeviceType *dt, const String &deviceName,
                      AudioDeviceID inputDeviceId, AudioDeviceID outputDeviceId)
        : AudioIODevice(deviceName, "CoreAudio"), deviceType(dt) {
      internal = [this, &inputDeviceId, &outputDeviceId] {
        if (outputDeviceId == 0 || outputDeviceId == inputDeviceId) {
          jassert(inputDeviceId != 0);
          return std::make_unique<CoreAudioInternal>(*this, inputDeviceId, true,
                                                     outputDeviceId != 0);
        }

        return std::make_unique<CoreAudioInternal>(*this, outputDeviceId, false,
                                                   true);
      }();
      ...
      AudioObjectAddPropertyListener(kAudioObjectSystemObject, &pa,
                                     hardwareListenerProc, internal.get());
    }
    ...
    ~CoreAudioIODevice() override {
      ...
      AudioObjectRemovePropertyListener(kAudioObjectSystemObject, &pa,
                                        hardwareListenerProc, internal.get());
    }

    StringArray getOutputChannelNames() override {
      return internal->outStream != nullptr ? internal->outStream->chanNames
                                            : StringArray();
    }
    StringArray getInputChannelNames() override {
      return internal->inStream != nullptr ? internal->inStream->chanNames
                                           : StringArray();
    }
    ...
    String open(const BigInteger &inputChannels,
                const BigInteger &outputChannels, double sampleRate,
      ...
      lastError = internal->reopen(inputChannels, outputChannels, sampleRate,
                                   bufferSizeSamples);
    ...
    int getOutputLatencyInSamples() override {
      return CoreAudioInternal::getLatency(internal->outStream);
    }
    int getInputLatencyInSamples() override {
      return CoreAudioInternal::getLatency(internal->inStream);
    }
  ...
  public:
    AudioIODeviceCombiner(const String &deviceName,
        ...
        : AudioIODevice(deviceName, "CoreAudio"), owner(deviceType),
    ...
    auto getDeviceWrappers() {
      return std::array<DeviceWrapper *, 2>{{&inputWrapper, &outputWrapper}};
    }
    auto getDeviceWrappers() const {
      return std::array<const DeviceWrapper *, 2>{
    ...
    StringArray getOutputChannelNames() override {
      return outputWrapper.getChannelNames();
    }
    StringArray getInputChannelNames() override {
      return inputWrapper.getChannelNames();
    }
    BigInteger getActiveOutputChannels() const override {
      return outputWrapper.getActiveChannels();
    }
    BigInteger getActiveInputChannels() const override {
      return inputWrapper.getActiveChannels();
    }
    ...
    int getCurrentBitDepth() override {
      return jmin(32, inputWrapper.getCurrentBitDepth(),
                  outputWrapper.getCurrentBitDepth());
    }

    int getDefaultBufferSize() override {
      return jmax(0, inputWrapper.getDefaultBufferSize(),
                  outputWrapper.getDefaultBufferSize());
    }
    ...
    void restart(AudioIODeviceCallback *cb) {
      ...
      open(inputChannelsRequested, outputChannelsRequested, newSampleRate,
           newBufferSize);
  ...
  private:
    static constexpr auto invalidSampleTime =
    ...
    // 1) We use absolute timestamps as the fifo's read/write positions. This
    // not only makes the code
    //    more readable (especially when checking for underruns/overflows) but
    //    also simplifies the initial setup when actual latency is not known yet
    //    until both callbacks have fired.
    // 2) AbstractFifo doesn't have the necessary mechanics to recover from
    // underrun/overflow conditions
    //    in a lock-free and data-race free way. It's great if you don't care
    //    (i.e. overwrite and/or read stale data) or can abort the operation
    //    entirely, but this is not the case here. We need bespoke
    //    underrun/overflow handling here which fits this use-case.
    template <typename Callback>
    void accessFifo(const uint64_t startPos, const int numChannels,
                    const int numItems, Callback &&operateOnRange) {
      const auto fifoSize = fifo.getNumSamples();
      auto fifoPos =
          static_cast<int>(startPos % static_cast<std::uint64_t>(fifoSize));
    ...
    void
    inputAudioCallback(const float *const *channels, int numChannels, int n,
...

      {
        ...
        if (callback != nullptr) {
          ...
          callback->audioDeviceIOCallbackWithContext(
              channels, numChannels, scratchBuffer.getArrayOfWritePointers(),
              numActiveOutputChannels, n, context);
        } else {
          scratchBuffer.clear();
        }
      }
      ...
      accessFifo(
          currentWritePos, scratchBuffer.getNumChannels(), n,
          [&](const auto &args) {
            FloatVectorOperations::copy(
                fifo.getWritePointer(args.channel, args.fifoPos),
    ...
    void outputAudioCallback(float *const *channels, int numChannels,
                             int n) noexcept {
      ...
      // If there was an xrun, we want to output zeros until we're sure that
      // there's some valid input for us to read.
      ...
      const auto sanitisedValidReadPos =
          validReadPos != invalidSampleTime ? validReadPos : nextReadPos;
      const auto numZerosToWrite =
          sanitisedValidReadPos <= currentReadPos
              ? 0
      ...
      accessFifo(
          currentReadPos + numZerosToWrite, numChannels,
          static_cast<int>(longN - numZerosToWrite), [&](const auto &args) {
            FloatVectorOperations::copy(
                channels[args.channel] + args.inputPos + numZerosToWrite,
                fifo.getReadPointer(args.channel, args.fifoPos), args.nItems);
          });
    ...
    void handleAudioDeviceAboutToStart(AudioIODevice *device) {
      ...
      if (!commonRates.contains(newSampleRate)) {
        ...
        if (newSampleRate < commonRates.getFirst() ||
            newSampleRate > commonRates.getLast()) {
          newSampleRate = jlimit(commonRates.getFirst(), commonRates.getLast(),
                                 newSampleRate);
        } else {
          for (auto it = commonRates.begin(); it < commonRates.end() - 1;
               ++it) {
            if (it[0] < newSampleRate && it[1] > newSampleRate) {
              newSampleRate =
                  newSampleRate - it[0] < it[1] - newSampleRate ? it[0] : it[1];
              break;
            }
          }
        }
    ...
    struct DeviceWrapper final : public AudioIODeviceCallback {
      DeviceWrapper(AudioIODeviceCombiner &cd,
                    std::unique_ptr<CoreAudioIODevice> d, bool shouldBeInput)
          : owner(cd), device(std::move(d)), input(shouldBeInput) {
      ...
      void audioDeviceIOCallbackWithContext(
        ...
        if (input)
          owner.inputAudioCallback(inputChannelData, numInputChannels,
                                   numSamples, context);
        else
          owner.outputAudioCallback(outputChannelData, numOutputChannels,
                                    numSamples);
      }
      ...
      StringArray getChannelNames() const {
        return input ? device->getInputChannelNames()
                     : device->getOutputChannelNames();
      }
      BigInteger getActiveChannels() const {
        return input ? device->getActiveInputChannels()
                     : device->getActiveOutputChannels();
      }
      int getLatencyInSamples() const {
        return input ? device->getInputLatencyInSamples()
                     : device->getOutputLatencyInSamples();
      }
      int getIndexOfDevice(bool asInput) const {
        return device->getIndexOfDevice(asInput);
      }
      ...
      Array<int> getAvailableBufferSizes() const {
      ...
      AudioIODeviceCallback *stopInternal() const {
        return device->stopInternal();
      }
      ...
      String open(const BigInteger &inputChannels,
        ...
        return device->open(inputChannels, outputChannels, sampleRate,
                            bufferSizeSamples);
      }

      std::uint64_t nsToSampleTime(std::uint64_t ns) const noexcept {
        return static_cast<std::uint64_t>(std::round(
            static_cast<double>(ns) * device->getCurrentSampleRate() * 1e-9));
      }

      void updateSampleTimeFromContext(
          const AudioIODeviceCallbackContext &context) noexcept {
        auto callbackSampleTime = context.hostTimeNs != nullptr
                                      ? nsToSampleTime(*context.hostTimeNs)
                                      : 0;
        ...
        if (sampleTime.compare_exchange_strong(copy, callbackSampleTime) &&
            (!input))
    ...
    /* If the current AudioIODeviceCombiner::callback is nullptr, it sets itself
       as the callback and forwards error related callbacks to the provided
       callback
    */
    class ScopedErrorForwarder final : public AudioIODeviceCallback {
    public:
      ScopedErrorForwarder(AudioIODeviceCombiner &ownerIn,
                           AudioIODeviceCallback *cb)
          : owner(ownerIn), target(cb) {
      ...
      // We only want to be notified about error conditions when the owner's
      // callback is nullptr. This class shouldn't be relied on for forwarding
      // this call.
  ...
  public:
    ...
    void scanForDevices() override {
      ...
      auto audioDevices = audioObjectGetProperties<AudioDeviceID>(
          kAudioObjectSystemObject,
          {kAudioHardwarePropertyDevices, kAudioObjectPropertyScopeWildcard,
           juceAudioObjectPropertyElementMain});

      for (const auto audioDevice : audioDevices) {
        if (const auto optionalName = audioObjectGetProperty<CFStringRef>(
                audioDevice, {kAudioDevicePropertyDeviceNameCFString,
                              kAudioObjectPropertyScopeWildcard,
                              juceAudioObjectPropertyElementMain})) {
          if (const CFUniquePtr<CFStringRef> name{*optionalName}) {
            const auto nameString = String::fromCFString(name.get());

            if (const auto numIns = getNumChannels(audioDevice, true);
                numIns > 0) {
              inputDeviceNames.add(nameString);
              inputIds.add(audioDevice);
            }

            if (const auto numOuts = getNumChannels(audioDevice, false);
                numOuts > 0) {
    ...
    int getDefaultDeviceIndex(bool forInput) const override {
      ...
      // if they're asking for any input channels at all, use the default input,
      // so we get the built-in mic rather than the built-in output with no
      // inputs..
      ...
      if (auto deviceID = audioObjectGetProperty<AudioDeviceID>(
              kAudioObjectSystemObject,
              {selector, kAudioObjectPropertyScopeWildcard,
               juceAudioObjectPropertyElementMain})) {
        auto &ids = forInput ? inputIds : outputIds;

        if (auto it = std::find(ids.begin(), ids.end(), deviceID);
            it != ids.end())
    ...
    AudioIODevice *createDevice(const String &outputDeviceName,
      ...
      auto combinedName =
          outputDeviceName.isEmpty() ? inputDeviceName : outputDeviceName;

      if (inputDeviceID == outputDeviceID)
        return std::make_unique<CoreAudioIODevice>(
                   this, combinedName, inputDeviceID, outputDeviceID)
            .release();

      auto in = inputDeviceID != 0
                    ? std::make_unique<CoreAudioIODevice>(this, inputDeviceName,
                                                          inputDeviceID, 0)
                    : nullptr;

      auto out = outputDeviceID != 0
                     ? std::make_unique<CoreAudioIODevice>(
                           this, outputDeviceName, 0, outputDeviceID)
                     : nullptr;
      ...
      auto combo = std::make_unique<AudioIODeviceCombiner>(
          combinedName, this, std::move(in), std::move(out));
  ...
  private:
    ...
    static OSStatus hardwareListenerProc(AudioDeviceID, UInt32,
