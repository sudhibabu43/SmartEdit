...
extern "C" {
// Declare just the minimum number of interfaces for the DSound objects that we
// need..
struct DSBUFFERDESC {
  DWORD dwSize;
  DWORD dwFlags;
...
DSOUND_FUNCTION(DirectSoundCaptureCreate,
                (const GUID *, IDirectSoundCapture **, LPUNKNOWN))
...
public:
  ...
  ~DSoundInternalOutChannel() { close(); }
  ...
  String open() {
    ...
    if (SUCCEEDED(hr)) {
      ...
      hr = pDirectSound->SetCooperativeLevel(GetDesktopWindow(),
                                             2 /* DSSCL_PRIORITY */);
      JUCE_DS_LOG_ERROR(hr);
      ...
      if (SUCCEEDED(hr)) {
        ...
        hr = pDirectSound->CreateSoundBuffer(&primaryDesc, &pPrimaryBuffer,
                                             nullptr);
        JUCE_DS_LOG_ERROR(hr);
        ...
        if (SUCCEEDED(hr)) {
          ...
          wfFormat.nBlockAlign = (unsigned short)(wfFormat.nChannels *
                                                  wfFormat.wBitsPerSample / 8);
          wfFormat.nAvgBytesPerSec =
              wfFormat.nSamplesPerSec * wfFormat.nBlockAlign;
          wfFormat.cbSize = 0;
          ...
          if (SUCCEEDED(hr)) {
            ...
            hr = pDirectSound->CreateSoundBuffer(&secondaryDesc, &pOutputBuffer,
                                                 nullptr);
            JUCE_DS_LOG_ERROR(hr);
  ...
  bool service() {
    ...
    if (bytesEmpty >= bytesPerBuffer) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        writeOffset =
            (writeOffset + dwSize1 + dwSize2) % (DWORD)totalBytesPerBuffer;
...
public:
  ...
  String open() {
    JUCE_DS_LOG("opening input: " + name + "  rate=" + String(sampleRate) +
                " bits=" + String(bitDepth) +
                " buf=" + String(bufferSizeSamples));
    ...
    HRESULT hr =
        dsDirectSoundCaptureCreate != nullptr
    ...
    if (SUCCEEDED(hr)) {
      ...
      wfFormat.nSamplesPerSec = (DWORD)sampleRate;
      wfFormat.wBitsPerSample = (unsigned short)bitDepth;
      wfFormat.nBlockAlign =
          (unsigned short)(wfFormat.nChannels * (wfFormat.wBitsPerSample / 8));
      ...
      hr = pDirectSoundCapture->CreateCaptureBuffer(&captureDesc, &pInputBuffer,
                                                    nullptr);
  ...
  bool service() {
    ...
    if (bytesFilled >= bytesPerBuffer) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        readOffset =
            (readOffset + dwsize1 + dwsize2) % (DWORD)totalBytesPerBuffer;
...
//==============================================================================
class DSoundAudioIODevice final : public AudioIODevice, public Thread {
public:
  DSoundAudioIODevice(const String &deviceName, const int outputDeviceIndex_,
                      const int inputDeviceIndex_)
      : AudioIODevice(deviceName, "DirectSound"), Thread("JUCE DSound"),
  ...
  ~DSoundAudioIODevice() override { close(); }

  String open(const BigInteger &inputChannels, const BigInteger &outputChannels,
              double newSampleRate, int newBufferSize) override {
    lastError =
        openDevice(inputChannels, outputChannels, newSampleRate, newBufferSize);
  ...
  Array<int> getAvailableBufferSizes() override {
    ...
    for (int i = 0; i < 50; ++i) {
      r.add(n);
      n += (n < 512) ? 32 : ((n < 1024) ? 64 : ((n < 2048) ? 128 : 256));
    }
...
private:
  ...
  String openDevice(const BigInteger &inputChannels,
                    const BigInteger &outputChannels, double sampleRate_,
    ...
    while (!threadShouldExit()) {
      ...
      if (isStarted) {
        callback->audioDeviceIOCallbackWithContext(
            inputBuffers.getArrayOfReadPointers(),
            inputBuffers.getNumChannels(),
            outputBuffers.getArrayOfWritePointers(),
            outputBuffers.getNumChannels(), bufferSizeSamples, {});
      } else {
        outputBuffers.clear();
        sleep(1);
      }
...
class DSoundDeviceList {
  auto tie() const {
    return std::tie(outputDeviceNames, inputDeviceNames, outputGuids,
                    inputGuids);
  }
...
private:
  static BOOL enumProc(LPGUID lpGUID, String desc, StringArray &names,
                       Array<GUID> &guids) {
    desc = desc.trim();
  ...
  static BOOL CALLBACK outputEnumProcW(LPGUID lpGUID, LPCWSTR description,
                                       LPCWSTR, LPVOID object) {
    return static_cast<DSoundDeviceList *>(object)->outputEnumProc(lpGUID,
                                                                   description);
  }

  static BOOL CALLBACK inputEnumProcW(LPGUID lpGUID, LPCWSTR description,
                                      LPCWSTR, LPVOID object) {
    return static_cast<DSoundDeviceList *>(object)->inputEnumProc(lpGUID,
                                                                  description);
  }
};
...
String DSoundAudioIODevice::openDevice(const BigInteger &inputChannels,
  ...
  for (int i = 0; i <= enabledInputs.getHighestBit(); i += 2) {
    float *left =
        enabledInputs[i] ? inputBuffers.getWritePointer(numIns++) : nullptr;
    float *right =
        enabledInputs[i + 1] ? inputBuffers.getWritePointer(numIns++) : nullptr;

    if (left != nullptr || right != nullptr)
      inChans.add(new DSoundInternalInChannel(
          dlh.inputDeviceNames[inputDeviceIndex],
          dlh.inputGuids[inputDeviceIndex], (int)sampleRate, bufferSizeSamples,
          left, right));
  }

  enabledOutputs = outputChannels;
  enabledOutputs.setRange(
      outChannels.size(),
      enabledOutputs.getHighestBit() + 1 - outChannels.size(), false);

  outputBuffers.setSize(enabledOutputs.countNumberOfSetBits(),
                        bufferSizeSamples);
  outputBuffers.clear();
  int numOuts = 0;
  ...
  for (int i = 0; i <= enabledOutputs.getHighestBit(); i += 2) {
    float *left =
        enabledOutputs[i] ? outputBuffers.getWritePointer(numOuts++) : nullptr;
    float *right = enabledOutputs[i + 1]
                       ? outputBuffers.getWritePointer(numOuts++)
                       : nullptr;

    if (left != nullptr || right != nullptr)
      outChans.add(new DSoundInternalOutChannel(
          dlh.outputDeviceNames[outputDeviceIndex],
          dlh.outputGuids[outputDeviceIndex], (int)sampleRate,
          bufferSizeSamples, left, right));
  }

  String error;

  // boost our priority while opening the devices to try to get better sync
  // between them
  ...
  for (int i = 0; i < outChans.size(); ++i) {
    ...
    if (error.isNotEmpty()) {
      error =
          "Error opening " + dlh.outputDeviceNames[i] + ": \"" + error + "\"";
      break;
    }
  }
  ...
  if (error.isEmpty()) {
    for (int i = 0; i < inChans.size(); ++i) {
      ...
      if (error.isNotEmpty()) {
        error =
            "Error opening " + dlh.inputDeviceNames[i] + ": \"" + error + "\"";
        break;
      }
    }
  }
...
public:
  DSoundAudioIODeviceType() : AudioIODeviceType("DirectSound") {
  ...
  int getIndexOfDevice(AudioIODevice *device, bool asInput) const override {
    ...
    if (DSoundAudioIODevice *const d =
            dynamic_cast<DSoundAudioIODevice *>(device))
      return asInput ? d->inputDeviceIndex : d->outputDeviceIndex;
  ...
  AudioIODevice *createDevice(const String &outputDeviceName,
    ...
    const int outputIndex =
        deviceList.outputDeviceNames.indexOf(outputDeviceName);
    const int inputIndex = deviceList.inputDeviceNames.indexOf(inputDeviceName);

    if (outputIndex >= 0 || inputIndex >= 0)
      return new DSoundAudioIODevice(
...
private:
  DeviceChangeDetector detector{L"DirectSound",
