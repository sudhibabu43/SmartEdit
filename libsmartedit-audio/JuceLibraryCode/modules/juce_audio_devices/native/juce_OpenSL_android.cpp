...
template <> struct IntfIID<SLObjectItf_> {
  static SLInterfaceID_ iid;
};
template <> struct IntfIID<SLEngineItf_> {
  static SLInterfaceID_ iid;
};
template <> struct IntfIID<SLOutputMixItf_> {
  static SLInterfaceID_ iid;
};
template <> struct IntfIID<SLPlayItf_> {
  static SLInterfaceID_ iid;
};
template <> struct IntfIID<SLRecordItf_> {
  static SLInterfaceID_ iid;
};
template <> struct IntfIID<SLAndroidSimpleBufferQueueItf_> {
  static SLInterfaceID_ iid;
};
template <> struct IntfIID<SLAndroidConfigurationItf_> {
  static SLInterfaceID_ iid;
};

SLInterfaceID_ IntfIID<SLObjectItf_>::iid = {
    0x79216360, 0xddd7, 0x11db, 0xac16, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}};
SLInterfaceID_ IntfIID<SLEngineItf_>::iid = {
    0x8d97c260, 0xddd4, 0x11db, 0x958f, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}};
SLInterfaceID_ IntfIID<SLOutputMixItf_>::iid = {
    0x97750f60, 0xddd7, 0x11db, 0x92b1, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}};
SLInterfaceID_ IntfIID<SLPlayItf_>::iid = {
    0xef0bd9c0, 0xddd7, 0x11db, 0xbf49, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}};
SLInterfaceID_ IntfIID<SLRecordItf_>::iid = {
    0xc5657aa0, 0xdddb, 0x11db, 0x82f7, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}};
SLInterfaceID_ IntfIID<SLAndroidSimpleBufferQueueItf_>::iid = {
    0x198e4940, 0xc5d7, 0x11df, 0xa2a6, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}};
SLInterfaceID_ IntfIID<SLAndroidConfigurationItf_>::iid = {
...
struct SLObjectItfFree {
  void operator()(SLObjectItf obj) const noexcept { destroyObject(obj); }
};
...
public:
  ...
  SlObjectRef(SlObjectRef &&obj) noexcept : cb(std::move(obj.cb)) {
    obj.cb = nullptr;
  }
  ...
  SLObjectItf operator->() noexcept {
    return (cb == nullptr ? nullptr : cb->ptr.get());
  }
  operator SLObjectItf() noexcept {
    return (cb == nullptr ? nullptr : cb->ptr.get());
  }
...
template <typename T> class SlRef final : public SlObjectRef {
public:
  //==============================================================================
  SlRef() noexcept {}
  SlRef(const SlRef &r) noexcept : SlObjectRef(r), type(r.type) {}
  SlRef(SlRef &&r) noexcept : SlObjectRef(std::move(r)), type(r.type) {
    r.type = nullptr;
  }
...
template <> struct BufferHelpers<int16> {
  enum { isFloatingPoint = 0 };

  static void initPCMDataFormat(PCMDataFormatEx &dataFormat, int numChannels,
                                double sampleRate) {
  ...
  using LittleEndianInt16 =
      AudioData::Format<AudioData::Int16, AudioData::LittleEndian>;
  using NativeFloat32 =
      AudioData::Format<AudioData::Float32, AudioData::NativeEndian>;

  static void convertFromOpenSL(const int16 *srcInterleaved,
                                AudioBuffer<float> &audioBuffer) {
    const auto numChannels = audioBuffer.getNumChannels();

    AudioData::deinterleaveSamples(
        AudioData::InterleavedSource<LittleEndianInt16>{
...
template <> struct BufferHelpers<float> {
  ...
  static void prepareCallbackBuffer(AudioBuffer<float> &audioBuffer,
                                    float *native) {
    if (audioBuffer.getNumChannels() == 1)
      audioBuffer.setDataToReferTo(&native, 1, audioBuffer.getNumSamples());
  }

  using LittleEndianFloat32 =
      AudioData::Format<AudioData::Float32, AudioData::LittleEndian>;
  using NativeFloat32 =
      AudioData::Format<AudioData::Float32, AudioData::NativeEndian>;

  static void convertFromOpenSL(const float *srcInterleaved,
                                AudioBuffer<float> &audioBuffer) {
    const auto numChannels = audioBuffer.getNumChannels();
    ...
    AudioData::deinterleaveSamples(
        AudioData::InterleavedSource<LittleEndianFloat32>{srcInterleaved,
                                                          numChannels},
        AudioData::NonInterleavedDest<NativeFloat32>{
            audioBuffer.getArrayOfWritePointers(), numChannels},
        audioBuffer.getNumSamples());
  ...
  static void convertToOpenSL(const AudioBuffer<float> &audioBuffer,
    ...
    AudioData::interleaveSamples(
        AudioData::NonInterleavedSource<NativeFloat32>{
            audioBuffer.getArrayOfReadPointers(), numChannels},
        AudioData::InterleavedDest<LittleEndianFloat32>{dstInterleaved,
                                                        numChannels},
        audioBuffer.getNumSamples());
  }
};
...
//==============================================================================
using CreateEngineFunc = SLresult (*)(SLObjectItf *, SLuint32,
                                      const SLEngineOption *, SLuint32,
                                      const SLInterfaceID *, const SLboolean *);

struct OpenSLEngineHolder {
  OpenSLEngineHolder() {
    if (auto createEngine =
            (CreateEngineFunc)slLibrary.getFunction("slCreateEngine")) {
...
public:
  //==============================================================================
  template <typename T> class OpenSLSessionT;
  ...
  struct OpenSLQueueRunner {
    OpenSLQueueRunner(OpenSLSessionT<T> &sessionToUse, int numChannelsToUse)
        : owner(sessionToUse), numChannels(numChannelsToUse),
          nativeBuffer(static_cast<size_t>(numChannels * owner.bufferSize *
                                           owner.numBuffers)),
          scratchBuffer(numChannelsToUse, owner.bufferSize),
          sampleBuffer(scratchBuffer.getArrayOfWritePointers(),
                       numChannelsToUse, owner.bufferSize) {}

    ~OpenSLQueueRunner() {
      if (config != nullptr && javaProxy != nullptr) {
        javaProxy.clear();
        (*config)->ReleaseJavaProxy(config,
                                    /*SL_ANDROID_JAVA_PROXY_ROUTING*/ 1);
      }
    }
    ...
    bool init() {
      ...
      if (supportsJavaProxy) {
        ...
        if (config != nullptr) {
          ...
          auto status = (*config)->AcquireJavaProxy(
              config, /*SL_ANDROID_JAVA_PROXY_ROUTING*/ 1, &audioRoutingJni);

          if (status == SL_RESULT_SUCCESS && audioRoutingJni != nullptr)
            javaProxy = GlobalRef(
                LocalRef<jobject>(getEnv()->NewLocalRef(audioRoutingJni)));
        }
      }
      ...
      return ((*queue)->RegisterCallback(queue, staticFinished, this) ==
              SL_RESULT_SUCCESS);
    }
    ...
    void clear() {
      ...
      zeromem(nativeBuffer.get(),
              static_cast<size_t>(owner.bufferSize * numChannels *
                                  owner.numBuffers) *
                  sizeof(T));
      scratchBuffer.clear();
      (*queue)->Clear(queue);
    ...
    void enqueueBuffer() {
      (*queue)->Enqueue(
          queue, getCurrentBuffer(),
          static_cast<SLuint32>(getBufferSizeInSamples() * sizeof(T)));
      ++numBlocksOut;
    }
    ...
    T *getCurrentBuffer() {
      return nativeBuffer.get() +
             (static_cast<size_t>(nextBlock.get()) * getBufferSizeInSamples());
    }
    size_t getBufferSizeInSamples() const {
    ...
    static void staticFinished(SLAndroidSimpleBufferQueueItf caller,
                               void *pContext) {
  ...
  //==============================================================================
  template <typename T>
  struct OpenSLQueueRunnerPlayer final
      : OpenSLQueueRunner<T, OpenSLQueueRunnerPlayer<T>, SLPlayItf_> {
    using Base = OpenSLQueueRunner<T, OpenSLQueueRunnerPlayer<T>, SLPlayItf_>;

    OpenSLQueueRunnerPlayer(OpenSLSessionT<T> &sessionToUse,
    ...
    SlRef<SLPlayItf_> createPlayerOrRecorder() {
      SLDataLocator_AndroidSimpleBufferQueue queueLocator = {
          ...
          static_cast<SLuint32>(Base::owner.numBuffers)};
      SLDataLocator_OutputMix outputMix = {SL_DATALOCATOR_OUTPUTMIX,
                                           Base::owner.outputMix};

      PCMDataFormatEx dataFormat;
      BufferHelpers<T>::initPCMDataFormat(dataFormat, Base::numChannels,
                                          Base::owner.sampleRate);

      SLDataSource source = {&queueLocator, &dataFormat};
      SLDataSink sink = {&outputMix, nullptr};

      SLInterfaceID queueInterfaces[] = {
          &IntfIID<SLAndroidSimpleBufferQueueItf_>::iid,
          &IntfIID<SLAndroidConfigurationItf_>::iid};
      ...
      if (auto e = *holder.engine) {
        auto status =
            e->CreateAudioPlayer(holder.engine, &obj, &source, &sink, 2,
                                 queueInterfaces, interfaceRequired);

        if (status != SL_RESULT_SUCCESS || obj == nullptr ||
            (*obj)->Realize(obj, 0) != SL_RESULT_SUCCESS) {
          destroyObject(obj);
          return {};
        }
    ...
    void setState(bool running) {
      (*Base::runner)
          ->SetPlayState(Base::runner,
                         running ? SL_PLAYSTATE_PLAYING : SL_PLAYSTATE_STOPPED);
    }
  };

  template <typename T>
  struct OpenSLQueueRunnerRecorder final
      : public OpenSLQueueRunner<T, OpenSLQueueRunnerRecorder<T>,
                                 SLRecordItf_> {
    using Base =
        OpenSLQueueRunner<T, OpenSLQueueRunnerRecorder<T>, SLRecordItf_>;

    OpenSLQueueRunnerRecorder(OpenSLSessionT<T> &sessionToUse,
                              int numChannelsToUse)
        : Base(sessionToUse, numChannelsToUse) {}
    ...
    SlRef<SLRecordItf_> createPlayerOrRecorder() {
      ...
      if (auto e = *holder.engine) {
        auto status =
            e->CreateAudioRecorder(holder.engine, &obj, &source, &sink, 2,
                                   queueInterfaces, interfaceRequired);
    ...
    bool setAudioPreprocessingEnabled(bool shouldEnable) {
      if (Base::config != nullptr) {
        ...
        auto status = (*Base::config)
                          ->SetConfiguration(Base::config,
                                             SL_ANDROID_KEY_RECORDING_PRESET,
                                             &recordingPresetValue,
                                             sizeof(recordingPresetValue));
    ...
    void setState(bool running) {
      (*Base::runner)
          ->SetRecordState(Base::runner, running ? SL_RECORDSTATE_RECORDING
  ...
  public:
    OpenSLSession(int numInputChannels, int numOutputChannels,
        ...
        : inputChannels(numInputChannels), outputChannels(numOutputChannels),
          sampleRate(samleRateToUse), bufferSize(bufferSizeToUse),
      ...
      if (outputChannels > 0) {
        ...
        auto err =
            (*holder.engine)
                ->CreateOutputMix(holder.engine, &obj, 0, nullptr, nullptr);
    ...
    void process(const float *const *inputChannelData,
      ...
      if (auto *cb = callback.exchange(nullptr)) {
        cb->audioDeviceIOCallbackWithContext(inputChannelData, inputChannels,
                                             outputChannelData, outputChannels,
                                             bufferSize, {});
        callback.set(cb);
      } else {
        for (int i = 0; i < outputChannels; ++i)
          zeromem(outputChannelData[i],
                  sizeof(float) * static_cast<size_t>(bufferSize));
      }
    }
  ...
  template <typename T> class OpenSLSessionT final : public OpenSLSession {
  public:
    OpenSLSessionT(int numInputChannels, int numOutputChannels,
                   double samleRateToUse, int bufferSizeToUse,
                   int numBuffersToUse)
        : OpenSLSession(numInputChannels, numOutputChannels, samleRateToUse,
      ...
      if (OpenSLSession::openedOK()) {
        if (inputChannels > 0) {
          recorder.reset(
              new OpenSLQueueRunnerRecorder<T>(*this, inputChannels));
        ...
        if (outputChannels > 0) {
          ...
          getUnderrunCount =
              supportsUnderrunCount
                  ? getEnv()->GetMethodID(AudioTrack, "getUnderrunCount", "()I")
                  : nullptr;
        }
      }
    }
    ...
    bool openedOK() const override {
      return OpenSLSession::openedOK() &&
             (inputChannels == 0 || recorder != nullptr) &&
    ...
    int getXRunCount() const noexcept override {
      if (player != nullptr && player->javaProxy != nullptr &&
          getUnderrunCount != nullptr)
        return getEnv()->CallIntMethod(player->javaProxy, getUnderrunCount);

      return -1;
    }

    bool supportsFloatingPoint() const noexcept override {
    ...
    void doSomeWorkOnAudioThread() {
      ...
      if (guard.compareAndSetBool(1, 0)) {
        ...
        if ((inputChannels == 0 || recorder->isBufferAvailable()) &&
            (outputChannels == 0 || player->isBufferAvailable())) {
          T *recorderBuffer =
              (inputChannels > 0 ? recorder->getNextBuffer() : nullptr);
          T *playerBuffer =
              (outputChannels > 0 ? player->getNextBuffer() : nullptr);

          const float *const *inputChannelData = nullptr;
          float *const *outputChannelData = nullptr;

          if (recorderBuffer != nullptr) {
            BufferHelpers<T>::prepareCallbackBuffer(recorder->sampleBuffer,
                                                    recorderBuffer);
            BufferHelpers<T>::convertFromOpenSL(recorderBuffer,
                                                recorder->sampleBuffer);

            inputChannelData = recorder->sampleBuffer.getArrayOfReadPointers();
          }

          if (playerBuffer != nullptr) {
            BufferHelpers<T>::prepareCallbackBuffer(player->sampleBuffer,
                                                    playerBuffer);
          ...
          if (playerBuffer != nullptr) {
            BufferHelpers<T>::convertToOpenSL(player->sampleBuffer,
                                              playerBuffer);
  ...
  OpenSLAudioIODevice(const String &deviceName)
    ...
    // OpenSL has piss-poor support for determining latency, so the only way I
    // can find to get a number for this is by asking the AudioTrack/AudioRecord
    // classes..
    AndroidAudioIODevice javaDevice(deviceName);

    // this is a total guess about how to calculate the latency, but seems to
    // vaguely agree with the devices I've tested.. YMMV
    ...
    outputLatency =
        (int)((longestLatency * outputLatency) / totalLatency) & ~15;

    // You can only create this class if you are sure that your hardware
    // supports OpenSL
    jassert(getEngineHolder().slLibrary.getNativeHandle() != nullptr);
  }

  ~OpenSLAudioIODevice() override { close(); }
  ...
  Array<double> getAvailableSampleRates() override {
    ...
    static const double rates[] = {8000.0,  11025.0, 12000.0, 16000.0, 22050.0,
                                   24000.0, 32000.0, 44100.0, 48000.0};
  ...
  String open(const BigInteger &inputChannels, const BigInteger &outputChannels,
    ...
    auto preferredBufferSize =
        (bufferSize > 0) ? bufferSize : getDefaultBufferSize();
    ...
    audioBuffersToEnqueue = [this, preferredBufferSize] {
      ...
      if (canUseHighPerformanceAudioPath(nativeBufferSize, preferredBufferSize,
                                         sampleRate))
    ...
    if (numInputChannels > 0 &&
      ...
      jassertfalse;
      lastError = "Error opening OpenSL input device: the app was not granted "
                  "android.permission.RECORD_AUDIO";
    }

    session.reset(OpenSLSession::create(numInputChannels, numOutputChannels,
                                        sampleRate, actualBufferSize,
                                        audioBuffersToEnqueue));
    if (session != nullptr) {
      session->setAudioPreprocessingEnabled(audioProcessingEnabled);
    } else {
      if (numInputChannels > 0 && numOutputChannels > 0 &&
          RuntimePermissions::isGranted(RuntimePermissions::recordAudio)) {
        // New versions of the Android emulator do not seem to support audio
        // input anymore on OS X
    ...
    DBG("OpenSL: numInputChannels = "
        << numInputChannels << ", numOutputChannels = " << numOutputChannels
        << ", nativeBufferSize = "
        << AndroidHighPerformanceAudioHelpers::getNativeBufferSizeHint()
        << ", nativeSampleRate = "
        << AndroidHighPerformanceAudioHelpers::getNativeSampleRate()
        ...
        << ", sampleRate = " << sampleRate << ", supportsFloatingPoint = "
        << (session != nullptr && session->supportsFloatingPoint() ? "true"
                                                                   : "false"));
  ...
  bool
  setAudioPreprocessingEnabled(bool shouldAudioProcessingBeEnabled) override {
...
OpenSLAudioIODevice::OpenSLSession *OpenSLAudioIODevice::OpenSLSession::create(
    int numInputChannels, int numOutputChannels, double samleRateToUse,
    int bufferSizeToUse, int numBuffersToUse) {
  std::unique_ptr<OpenSLSession> retval;
  auto sdkVersion = getAndroidSDKVersion();

  // SDK versions 21 and higher should natively support floating point...
  if (sdkVersion >= 21) {
    retval.reset(new OpenSLSessionT<float>(numInputChannels, numOutputChannels,
                                           samleRateToUse, bufferSizeToUse,
                                           numBuffersToUse));
  ...
  if (retval == nullptr) {
    retval.reset(new OpenSLSessionT<int16>(numInputChannels, numOutputChannels,
                                           samleRateToUse, bufferSizeToUse,
                                           numBuffersToUse));
...
public:
  ...
  StringArray getDeviceNames(bool) const override {
    return StringArray(OpenSLAudioIODevice::openSLTypeName);
  }
  ...
  AudioIODevice *createDevice(const String &outputDeviceName,
    ...
    if (outputDeviceName.isNotEmpty() || inputDeviceName.isNotEmpty())
      dev.reset(new OpenSLAudioIODevice(
...
public:
  ...
  SLRealtimeThread() {
    if (auto createEngine =
      ...
      SLDataLocator_AndroidSimpleBufferQueue queueLocator = {
          SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE,
          static_cast<SLuint32>(numBuffers)};
      SLDataLocator_OutputMix outputMixLocator = {SL_DATALOCATOR_OUTPUTMIX,
                                                  outputMix};

      PCMDataFormatEx dataFormat;
      BufferHelpers<int16>::initPCMDataFormat(
          dataFormat, 1,
          AndroidHighPerformanceAudioHelpers::getNativeSampleRate());
      ...
      obj = nullptr;
      err = (*engine)->CreateAudioPlayer(engine, &obj, &source, &sink, 1,
                                         queueInterfaces, &trueFlag);
      ...
      if ((*queue)->RegisterCallback(queue, staticFinished, this) !=
          SL_RESULT_SUCCESS) {
        queue = nullptr;
        return;
      }
  ...
  pthread_t startThread(void *(*entry)(void *), void *userPtr) {
    memset(buffer.get(), 0,
           static_cast<size_t>(sizeof(int16) *
                               static_cast<size_t>(bufferSize * numBuffers)));

    for (int i = 0; i < numBuffers; ++i) {
      int16 *dst = buffer.get() + (bufferSize * i);
      (*queue)->Enqueue(queue, dst,
                        static_cast<SLuint32>(static_cast<size_t>(bufferSize) *
                                              sizeof(int16)));
    }
...
private:
  ...
  int bufferSize =
      AndroidHighPerformanceAudioHelpers::getNativeBufferSizeHint();
  HeapBlock<int16> buffer{
      HeapBlock<int16>(static_cast<size_t>(1 * bufferSize * numBuffers))};
