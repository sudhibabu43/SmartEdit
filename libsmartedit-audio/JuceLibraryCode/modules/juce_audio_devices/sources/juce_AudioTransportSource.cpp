...
void AudioTransportSource::setSource(PositionableAudioSource *const newSource,
  ...
  if (source == newSource) {
    ...
    setSource(
        nullptr, 0,
        nullptr); // deselect and reselect to avoid releasing resources wrongly
  }
  ...
  if (newSource != nullptr) {
    ...
    if (readAheadSize > 0) {
      // If you want to use a read-ahead buffer, you must also provide a
      // TimeSliceThread for it to use!
    ...
    if (sourceSampleRateToCorrectFor > 0)
      newMasterSource = newResamplerSource = new ResamplingAudioSource(
          newPositionableSource, false, maxNumChannels);
    else
      newMasterSource = newPositionableSource;

    if (isPrepared) {
      if (newResamplerSource != nullptr && sourceSampleRateToCorrectFor > 0 &&
          sampleRate > 0)
        newResamplerSource->setResamplingRatio(sourceSampleRateToCorrectFor /
                                               sampleRate);
...
bool AudioTransportSource::hasStreamFinished() const noexcept {
  return positionableSource->getNextReadPosition() >
             positionableSource->getTotalLength() + 1 &&
...
void AudioTransportSource::setNextReadPosition(int64 newPosition) {
  if (positionableSource != nullptr) {
    if (sampleRate > 0 && sourceSampleRate > 0)
      newPosition =
          (int64)((double)newPosition * sourceSampleRate / sampleRate);
...
int64 AudioTransportSource::getNextReadPosition() const {
  ...
  if (positionableSource != nullptr) {
    const double ratio = (sampleRate > 0 && sourceSampleRate > 0)
...
void AudioTransportSource::releaseResources() { releaseMasterResources(); }
...
void AudioTransportSource::getNextAudioBlock(
  ...
  if (masterSource != nullptr && !stopped) {
    ...
    if (!playing) {
      ...
      for (int i = info.buffer->getNumChannels(); --i >= 0;)
        info.buffer->applyGainRamp(i, info.startSample,
                                   jmin(256, info.numSamples), 1.0f, 0.0f);
    ...
    for (int i = info.buffer->getNumChannels(); --i >= 0;)
      info.buffer->applyGainRamp(i, info.startSample, info.numSamples, lastGain,
                                 gain);
  } else {
