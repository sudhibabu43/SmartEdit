...
struct U32InputHandler {
  ...
  virtual void pushMidiData(const uint32_t *begin, const uint32_t *end,
                            double time) = 0;
};

/**
    Parses a continuous stream of U32 words and emits complete MidiMessages
   whenever a full message is received.
...
struct U32ToBytestreamHandler : public U32InputHandler {
  ...
  */
  class Factory {
  public:
    explicit Factory(MidiInputCallback *c) : callback(c) {}
  ...
  void pushMidiData(const uint32_t *begin, const uint32_t *end,
...
/**
    Parses a continuous stream of U32 words and emits full messages in the
   requested UMP format.
...
struct U32ToUMPHandler : public U32InputHandler {
  ...
  */
  class Factory {
  public:
    Factory(PacketProtocol p, Receiver &c) : protocol(p), callback(c) {}
  ...
  void pushMidiData(const uint32_t *begin, const uint32_t *end,
                    double time) override {
    dispatcher.dispatch(begin, end, time,
                        [this](const View &view, double thisTime) {
