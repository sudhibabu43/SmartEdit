...
/* If you cannot find any of the header files below then you are probably
   attempting to use the Windows 10 Bluetooth Low Energy API. For this to work
   you need to install version 10.0.14393.0 of the Windows Standalone SDK and
   you may need to add the path to the WinRT headers to your build system. This
   path should have the form "C:\Program Files (x86)\Windows
   Kits\10\Include\10.0.14393.0\winrt".

   Also please note that Microsoft's Bluetooth MIDI stack has multiple issues,
   so this API is EXPERIMENTAL - use at your own risk!
*/
...
/* This is very frustrating - we only need to use a handful of definitions from
   a couple of the header files in Steinberg's ASIO SDK, and it'd be easy to
   copy about 30 lines of code into this cpp file to create a fully stand-alone
   ASIO implementation...

   ..unfortunately that would break Steinberg's  agreement for use of
   ...
   3) Make sure that your header search path contains the iasiodrv.h file that
      comes with the SDK. (Only about a handful of the SDK header files are
   actually needed - so to simplify things, you could just copy these into your
   JUCE directory).
*/
...
#if JUCE_ALSA
/* Got an include error here? If so, you've either not got ALSA installed, or
   ...
   If you don't have the ALSA library and don't want to build JUCE with audio
   support, just set the JUCE_ALSA flag to 0.
*/
...
#if JUCE_JACK
/* Got an include error here? If so, you've either not got
   jack-audio-connection-kit installed, or you've not got your paths set up
   correctly to find its header files.

   The package you need to install to get JACK support is "libjack-dev".

   If you don't have the jack-audio-connection-kit library and don't want to
   build JUCE with low latency audio support, just set the JUCE_JACK flag to 0.
*/
...
#if JUCE_USE_ANDROID_OPENSLES
#error                                                                         \
    "Oboe cannot be enabled at the same time as openSL! Please disable JUCE_USE_ANDROID_OPENSLES"
#endif

JUCE_BEGIN_IGNORE_WARNINGS_GCC_LIKE(
    "-Wunused-parameter", "-Wzero-as-null-pointer-constant",
    "-Winconsistent-missing-destructor-override",
    "-Wshadow-field-in-constructor", "-Wshadow-field", "-Wsign-conversion",
    "-Wswitch-enum")
#include <oboe/Oboe.h>
JUCE_END_IGNORE_WARNINGS_GCC_LIKE
...
namespace juce {
// None of these methods are available. (On Windows you might need to enable
// WASAPI for this)
