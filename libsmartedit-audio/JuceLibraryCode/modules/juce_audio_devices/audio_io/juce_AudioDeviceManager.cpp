...
private:
  ...
  void audioDeviceStopped() override { owner.audioDeviceStoppedInt(); }
  ...
  void audioDeviceListChanged() override { owner.audioDeviceListChanged(); }
...
const OwnedArray<AudioIODeviceType> &
...
//==============================================================================
static void addIfNotNull(OwnedArray<AudioIODeviceType> &list,
...
void AudioDeviceManager::createAudioDeviceTypes(
    OwnedArray<AudioIODeviceType> &list) {
  addIfNotNull(list, AudioIODeviceType::createAudioIODeviceType_WASAPI(
                         WASAPIDeviceMode::shared));
  addIfNotNull(list, AudioIODeviceType::createAudioIODeviceType_WASAPI(
                         WASAPIDeviceMode::exclusive));
  addIfNotNull(list, AudioIODeviceType::createAudioIODeviceType_WASAPI(
                         WASAPIDeviceMode::sharedLowLatency));
...
void AudioDeviceManager::addAudioDeviceType(
    std::unique_ptr<AudioIODeviceType> newDeviceType) {
...
static bool deviceListContains(AudioIODeviceType *type, bool isInput,
                               const String &name) {
...
//==============================================================================
String AudioDeviceManager::initialise(
    const int numInputChannelsNeeded, const int numOutputChannelsNeeded,
    const XmlElement *const xml, const bool selectDefaultDeviceOnFailure,
...
String AudioDeviceManager::initialiseDefault(
    const String &preferredDefaultDeviceName,
    const AudioDeviceSetup *preferredSetupOptions) {
  ...
  if (preferredSetupOptions != nullptr) {
    ...
    const auto getWildcardMatch = [&nameMatches](const StringArray &names) {
      ...
      return WildcardMatch{iter != names.end() ? *iter : String(),
                           iter != names.end()};
    };
    ...
    const auto getMatchesForType =
        [&getWildcardMatch](const AudioIODeviceType *type) {
    ...
    const auto result = [&] {
      // First, look for a device type with an input and output which matches
      // the preferred name
      for (auto *type : availableDeviceTypes) {
        const auto matches = getMatchesForType(type);

        if (matches.input.successful && matches.output.successful)
          return SearchResult{type->getTypeName(), matches.input.value,
                              matches.output.value};
      }

      // No device type has matching ins and outs, so fall back to a device
      // where either the input or output match
      for (auto *type : availableDeviceTypes) {
        const auto matches = getMatchesForType(type);
      ...
      // No devices match the query, so just use the default devices from the
      // current type
...
String AudioDeviceManager::initialiseFromXML(
    const XmlElement &xml, bool selectDefaultDeviceOnFailure,
    const String &preferredDefaultDeviceName,
    const AudioDeviceSetup *preferredSetupOptions) {
  ...
  const auto midiInputs = [&] {
    ...
    for (auto *c : xml.getChildWithTagNameIterator("MIDIINPUT"))
      result.add(
          {c->getStringAttribute("name"), c->getStringAttribute("identifier")});

    return result;
  }();

  const MidiDeviceInfo defaultOutputDeviceInfo(
...
void AudioDeviceManager::openLastRequestedMidiDevices(
  ...
  const auto openDeviceIfAvailable = [&](const Array<MidiDeviceInfo> &devices,
    ...
    const auto iterWithMatchingIdentifier =
        std::find_if(devices.begin(), devices.end(), [&](const auto &x) {
...
void AudioDeviceManager::insertDefaultDeviceNames(
    AudioDeviceSetup &setup) const {
  ...
  if (auto *type = getCurrentDeviceTypeObject()) {
    ...
    // AudioIODevice classes should generate an appropriate error message when
    // opening or starting these devices.
    const auto getDevicesToTestForMatchingSampleRate = [&setup, type,
                                                        this](Direction dir) {
      ...
      auto deviceNames =
          numChannelsNeeded > 0 ? type->getDeviceNames(isInput) : StringArray{};
    ...
    const auto getSupportedSampleRates =
        [&sampleRatesCache, type](Direction dir, const String &deviceName) {
          ...
          auto &entry = [&]() -> auto & {
            ...
            auto tempDevice = rawToUniquePtr(
    ...
    const auto validate = [&getSupportedSampleRates](
    ...
    // We check all possible in-out pairs until the first validation pass. If no
    // pair passes we leave the setup unchanged.
  ...
  for (int i = 0; i < availableDeviceTypes.size(); ++i) {
    if (availableDeviceTypes.getUnchecked(i)->getTypeName() == type &&
      ...
      if (currentAudioDevice != nullptr) {
        closeAudioDevice();
        Thread::sleep(
            1500); // allow a moment for OS devices to sort themselves out, to
                   // help avoid things like DirectSound/ASIO clashes
      }
...
static void updateSetupChannels(AudioDeviceManager::AudioDeviceSetup &setup,
  ...
  auto updateChannels = [](const String &deviceName, BigInteger &channels,
  ...
  if (needsNewDevice) {
    ...
    if (currentAudioDevice == nullptr)
      error = "Can't open the audio device!\n\n"
              "This may be because another application is currently using the "
              "same device - "
...
void AudioDeviceManager::updateXml() {
  ...
  if (defaultMidiOutputDeviceInfo != MidiDeviceInfo()) {
    ...
    lastExplicitSettings->setAttribute("defaultMidiOutputDevice",
                                       defaultMidiOutputDeviceInfo.identifier);
  }
}
...
void AudioDeviceManager::removeAudioCallback(
    AudioIODeviceCallback *callbackToRemove) {
...
void AudioDeviceManager::audioDeviceIOCallbackInt(
  ...
  if (callbacks.size() > 0) {
    ...
    tempBuffer.setSize(jmax(1, numOutputChannels), jmax(1, numSamples), false,
                       false, true);
...
//==============================================================================
void AudioDeviceManager::setMidiInputDeviceEnabled(const String &identifier,
                                                   bool enabled) {
  if (enabled != isMidiInputDeviceEnabled(identifier)) {
    if (enabled) {
      if (auto midiIn =
              MidiInput::openDevice(identifier, callbackHandler.get())) {
        enabledMidiInputs.push_back(std::move(midiIn));
        enabledMidiInputs.back()->start();
      }
    } else {
      auto removePredicate =
          [identifier](const std::unique_ptr<MidiInput> &in) {
            return in->getIdentifier() == identifier;
          };
      enabledMidiInputs.erase(std::remove_if(std::begin(enabledMidiInputs),
...
void AudioDeviceManager::addMidiInputDeviceCallback(
    const String &identifier, MidiInputCallback *callbackToAdd) {
...
void AudioDeviceManager::removeMidiInputDeviceCallback(
    const String &identifier, MidiInputCallback *callbackToRemove) {
...
void AudioDeviceManager::playTestSound() {
  ...
  if (currentAudioDevice != nullptr) {
    ...
    auto phasePerSample =
        MathConstants<double>::twoPi / (sampleRate / frequency);

    std::unique_ptr<AudioBuffer<float>> newSound(
        new AudioBuffer<float>(1, soundLength));

    for (int i = 0; i < soundLength; ++i)
      newSound->setSample(0, i,
                          amplitude * (float)std::sin(i * phasePerSample));

    newSound->applyGainRamp(0, 0, soundLength / 10, 0.0f, 1.0f);
    newSound->applyGainRamp(0, soundLength - soundLength / 4, soundLength / 4,
                            1.0f, 0.0f);
...
void AudioDeviceManager::addMidiInputCallback(
...
void AudioDeviceManager::removeMidiInputCallback(
...
public:
  ...
  void runTest() override {
    beginTest("When the AudioDeviceSetup has non-empty device names, "
              "initialise uses the requested devices");
    {
      ...
      expect(
          manager.initialise(2, 2, nullptr, true, String{}, &setup).isEmpty());
    ...
    beginTest("When the AudioDeviceSetup has empty device names, initialise "
              "picks suitable default devices");
    {
      AudioDeviceManager manager;
      initialiseManager(manager);
    ...
    beginTest("When the preferred device name matches an input and an output "
              "on the same type, that type is used");
    {
      AudioDeviceManager manager;
    ...
    beginTest("When the preferred device name matches either an input and an "
              "output, but not both, that type is used");
    {
      AudioDeviceManager manager;
      initialiseManagerWithDifferentDeviceNames(manager);
    ...
    beginTest("When the preferred device name does not match any inputs or "
              "outputs, defaults are used");
    {
      AudioDeviceManager manager;
      initialiseManagerWithDifferentDeviceNames(manager);
    ...
    beginTest("When first device type has no devices, a device type with "
              "devices is used instead");
    {
      AudioDeviceManager manager;
    ...
    beginTest("AudioDeviceManager updates its current settings before "
              "notifying callbacks when device restarts itself");
    {
      AudioDeviceManager manager;
      auto deviceType = std::make_unique<MockDeviceType>(
          "foo", StringArray{"foo in a", "foo in b"},
...
private:
  ...
  void enableInputChannels(AudioDeviceManager &manager) {
    auto setup = manager.getAudioDeviceSetup();
    setup.inputDeviceName =
        manager.getCurrentDeviceTypeObject()->getDeviceNames(true)[0];
  ...
  void switchDeviceType(AudioDeviceManager &manager) {
    ...
    // We had no channels enabled, which means we don't need to open a new input
    // device
  ...
  public:
    MockDevice(ListenerList<Restartable> &l, String typeNameIn,
               String outNameIn, String inNameIn)
        : AudioIODevice("mock", typeNameIn), listeners(l), outName(outNameIn),
          inName(inNameIn) {
      listeners.add(this);
    }

    ~MockDevice() override { listeners.remove(this); }
    ...
    String open(const BigInteger &inputs, const BigInteger &outputs, double sr,
                int bs) override {
      inChannels = inputs;
      outChannels = outputs;
      sampleRate = sr;
  ...
  public:
    ...
    MockDeviceType(String kind, StringArray inputNames, StringArray outputNames)
        : AudioIODeviceType(std::move(kind)), inNames(std::move(inputNames)),
  ...
  private:
    const StringArray &getNames(bool isInput) const {
  ...
  public:
    ...
    void audioDeviceAboutToStart(AudioIODevice *) override {
      NullCheckedInvocation::invoke(aboutToStart);
    }
    void audioDeviceStopped() override {
      NullCheckedInvocation::invoke(stopped);
    }
    void audioDeviceError(const String &) override {
      NullCheckedInvocation::invoke(error);
    }
  };
  ...
  void initialiseManagerWithEmptyDeviceType(AudioDeviceManager &manager) {
    manager.addAudioDeviceType(std::make_unique<MockDeviceType>(
        emptyName, StringArray{}, StringArray{}));
    initialiseManager(manager);
  }
  ...
  void initialiseManagerWithDifferentDeviceNames(AudioDeviceManager &manager) {
    ...
    manager.addAudioDeviceType(std::make_unique<MockDeviceType>(
        "bar", StringArray{"bar in a", "bar in b"},
