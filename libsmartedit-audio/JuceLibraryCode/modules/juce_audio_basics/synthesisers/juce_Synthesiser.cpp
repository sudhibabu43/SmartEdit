...
bool SynthesiserVoice::wasStartedBefore(
...
void SynthesiserVoice::renderNextBlock(AudioBuffer<double> &outputBuffer,
  ...
  AudioBuffer<double> subBuffer(outputBuffer.getArrayOfWritePointers(),
                                outputBuffer.getNumChannels(), startSample,
                                numSamples);
...
void Synthesiser::setMinimumRenderingSubdivisionSize(
    int numSamples, bool shouldBeStrict) noexcept {
  jassert(numSamples >
          0); // it wouldn't make much sense for this to be less than 1
...
void Synthesiser::processNextBlock(AudioBuffer<floatType> &outputAudio,
                                   const MidiBuffer &midiData, int startSample,
                                   int numSamples) {
...
template void Synthesiser::processNextBlock<float>(AudioBuffer<float> &,
                                                   const MidiBuffer &, int,
                                                   int);
template void Synthesiser::processNextBlock<double>(AudioBuffer<double> &,
                                                    const MidiBuffer &, int,
                                                    int);
...
void Synthesiser::renderVoices(AudioBuffer<float> &buffer, int startSample,
                               int numSamples) {
  for (auto *voice : voices)
    voice->renderNextBlock(buffer, startSample, numSamples);
}

void Synthesiser::renderVoices(AudioBuffer<double> &buffer, int startSample,
                               int numSamples) {
  for (auto *voice : voices)
    voice->renderNextBlock(buffer, startSample, numSamples);
}
...
//==============================================================================
void Synthesiser::noteOn(const int midiChannel, const int midiNoteNumber,
...
void Synthesiser::stopVoice(SynthesiserVoice *voice, float velocity,
                            const bool allowTailOff) {
  jassert(voice != nullptr);

  voice->stopNote(velocity, allowTailOff);

  // the subclass MUST call clearCurrentNote() if it's not tailing off! RTFM for
  // stopNote()!
  jassert(allowTailOff || (voice->getCurrentlyPlayingNote() < 0 &&
                           voice->getCurrentlyPlayingSound() == nullptr));
}

void Synthesiser::noteOff(const int midiChannel, const int midiNoteNumber,
                          const float velocity, const bool allowTailOff) {
  const ScopedLock sl(lock);
  ...
  for (auto *voice : voices) {
    if (voice->getCurrentlyPlayingNote() == midiNoteNumber &&
      ...
      if (auto sound = voice->getCurrentlyPlayingSound()) {
        if (sound->appliesToNote(midiNoteNumber) &&
            sound->appliesToChannel(midiChannel)) {
          jassert(!voice->keyIsDown || voice->isSustainPedalDown() ==
                                           sustainPedalsDown[midiChannel]);
...
void Synthesiser::handleAftertouch(int midiChannel, int midiNoteNumber,
                                   int aftertouchValue) {
  const ScopedLock sl(lock);
...
void Synthesiser::handleChannelPressure(int midiChannel,
...
void Synthesiser::handleSoftPedal([[maybe_unused]] int midiChannel,
                                  bool /*isDown*/) {
  jassert(midiChannel > 0 && midiChannel <= 16);
}
...
//==============================================================================
SynthesiserVoice *
Synthesiser::findFreeVoice(SynthesiserSound *soundToPlay, int midiChannel,
                           int midiNoteNumber,
...
SynthesiserVoice *Synthesiser::findVoiceToSteal(SynthesiserSound *soundToPlay,
  ...
  // - Protect the lowest & topmost notes, even if sustained, but not if they've
  // been released.
  ...
  SynthesiserVoice *low = nullptr; // Lowest sounding note, might be sustained,
                                   // but NOT in release phase
  SynthesiserVoice *top = nullptr; // Highest sounding note, might be sustained,
                                   // but NOT in release phase

  // All major OSes use double-locking so this will be lock- and wait-free as
  // long as the lock is not contended. This is always the case if you do not
  // call findVoiceToSteal on multiple threads at the same time.
  const ScopedLock sl(stealLock);

  // this is a list of voices we can steal, sorted by how long they've been
  // running
  ...
  for (auto *voice : voices) {
    if (voice->canPlaySound(soundToPlay)) {
      ...
      // NB: Using a functor rather than a lambda here due to scare-stories
      // about compilers generating code containing heap allocations..
      struct Sorter {
      ...
      std::sort(usableVoicesToStealArray.begin(),
                usableVoicesToStealArray.end(), Sorter());
  ...
  // Eliminate pathological cases (ie: only 1 note playing): we always give
  // precedence to the lowest note(s)
  if (top == low)
    top = nullptr;
  ...
  // Oldest voice that has been released (no finger on it and not held by
  // sustain pedal)
  for (auto *voice : usableVoicesToStealArray)
