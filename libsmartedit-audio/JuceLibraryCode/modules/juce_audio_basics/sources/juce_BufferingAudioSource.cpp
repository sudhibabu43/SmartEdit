  ...
  if (!approximatelyEqual(newSampleRate, sampleRate) ||
    ...
    do {
      ...
      Thread::sleep(5);
             ...
             (bufferValidEnd - bufferValidStart <
              jmin(((int)newSampleRate) / 4, buffer.getNumSamples() / 2)));
  }
}
...
void BufferingAudioSource::releaseResources() {
  ...
  // MSVC2017 seems to need this if statement to not generate a warning during
  // linking. As source is set in the constructor, there is no way that source
  // could ever equal this, but it seems to make MSVC2017 happy.
  if (source != this)
    source->releaseResources();
}

void BufferingAudioSource::getNextAudioBlock(
  ...
  if (validStart > 0)
    info.buffer->clear(info.startSample,
                       validStart); // partial cache miss at start
  ...
  if (validStart < validEnd) {
    for (int chan = jmin(numberOfChannels, info.buffer->getNumChannels());
         --chan >= 0;) {
      jassert(buffer.getNumSamples() > 0);

      const auto startBufferIndex =
          (int)((validStart + nextPlayPos) % buffer.getNumSamples());
      const auto endBufferIndex =
          (int)((validEnd + nextPlayPos) % buffer.getNumSamples());

      if (startBufferIndex < endBufferIndex) {
        info.buffer->copyFrom(chan, info.startSample + validStart, buffer, chan,
                              startBufferIndex, validEnd - validStart);
      } else {
        const auto initialSize = buffer.getNumSamples() - startBufferIndex;

        info.buffer->copyFrom(chan, info.startSample + validStart, buffer, chan,
                              startBufferIndex, initialSize);
...
bool BufferingAudioSource::waitForNextAudioBlockReady(
    const AudioSourceChannelInfo &info, uint32 timeout) {
  ...
  while (elapsed <= timeout) {
    ...
    if (validStart <= 0 && validStart < validEnd &&
...
bool BufferingAudioSource::readNextBufferChunk() {
  ...
  const auto bufferIndexStart =
      (int)(sectionToReadStart % buffer.getNumSamples());
  ...
  if (bufferIndexStart < bufferIndexEnd) {
    ...
    readBufferSection(
        sectionToReadStart + initialSize,
...
void BufferingAudioSource::readBufferSection(int64 start, int length,
                                             int bufferOffset) {
