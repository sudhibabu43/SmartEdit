...
MemoryAudioSource::MemoryAudioSource(AudioBuffer<float> &bufferToUse,
...
void MemoryAudioSource::getNextAudioBlock(
...
int64 MemoryAudioSource::getNextReadPosition() const { return position; }
...
//==============================================================================
bool MemoryAudioSource::isLooping() const { return isCurrentlyLooping; }
...
struct MemoryAudioSourceTests final : public UnitTest {
  ...
  void runTest() override {
    ...
    beginTest("A zero-length buffer produces silence, whether or not looping "
              "is enabled");
    {
    ...
    beginTest("A short buffer without looping is played once and followed by "
              "silence");
    {
    ...
    beginTest("A short buffer with looping is played multiple times");
    {
      ...
      for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
        expectEquals(bufferToFill.getSample(0, sample + buffer.getNumSamples()),
    ...
    beginTest("A long buffer with looping is played multiple times");
    {
      ...
      for (int i = 0; i < 100; ++i) {
        play(source, channelInfo);
        expectEquals(
            bufferToFill.getSample(0, 0),
            buffer.getSample(0, (i * blockSize) % buffer.getNumSamples()));
      }
    }
  }
    ...
    for (int channel = 0; channel < buffer.getNumChannels(); ++channel)
      for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
        buffer.setSample(channel, sample,
                         jmap((float)sample, 0.0f, (float)length, -1.0f, 1.0f));
