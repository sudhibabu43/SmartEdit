...
struct BasicOps32 {
  ...
  // Integer and parallel types are the same for SSE. On neon they have
  // different types
  ...
  static forcedinline ParallelType load1(Type v) noexcept {
    return _mm_load1_ps(&v);
  }
  static forcedinline ParallelType loadA(const Type *v) noexcept {
    return _mm_load_ps(v);
  }
  static forcedinline ParallelType loadU(const Type *v) noexcept {
    return _mm_loadu_ps(v);
  }
  static forcedinline void storeA(Type *dest, ParallelType a) noexcept {
    _mm_store_ps(dest, a);
  }
  static forcedinline void storeU(Type *dest, ParallelType a) noexcept {
    _mm_storeu_ps(dest, a);
  }

  static forcedinline ParallelType add(ParallelType a,
                                       ParallelType b) noexcept {
    return _mm_add_ps(a, b);
  }
  static forcedinline ParallelType sub(ParallelType a,
                                       ParallelType b) noexcept {
    return _mm_sub_ps(a, b);
  }
  static forcedinline ParallelType mul(ParallelType a,
                                       ParallelType b) noexcept {
    return _mm_mul_ps(a, b);
  }
  static forcedinline ParallelType max(ParallelType a,
                                       ParallelType b) noexcept {
    return _mm_max_ps(a, b);
  }
  static forcedinline ParallelType min(ParallelType a,
                                       ParallelType b) noexcept {
    return _mm_min_ps(a, b);
  }

  static forcedinline ParallelType bit_and(ParallelType a,
                                           ParallelType b) noexcept {
    return _mm_and_ps(a, b);
  }
  static forcedinline ParallelType bit_not(ParallelType a,
                                           ParallelType b) noexcept {
    return _mm_andnot_ps(a, b);
  }
  static forcedinline ParallelType bit_or(ParallelType a,
                                          ParallelType b) noexcept {
    return _mm_or_ps(a, b);
  }
  static forcedinline ParallelType bit_xor(ParallelType a,
                                           ParallelType b) noexcept {
...
#define JUCE_PERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp)             \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp if (FloatVectorHelpers::isAligned(dest)) JUCE_VEC_LOOP(              \
      vecOp, dummy, Mode::loadA, Mode::storeA, locals,                         \
      JUCE_INCREMENT_DEST) else JUCE_VEC_LOOP(vecOp, dummy, Mode::loadU,       \
                                              Mode::storeU, locals,            \
                                              JUCE_INCREMENT_DEST)             \
      JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC_DEST(normalOp, vecOp, locals, increment,       \
                                     setupOp)                                  \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp if (FloatVectorHelpers::isAligned(dest)) {                           \
    if (FloatVectorHelpers::isAligned(src))                                    \
      JUCE_VEC_LOOP(vecOp, Mode::loadA, Mode::loadA, Mode::storeA, locals,     \
                    increment)                                                 \
    else                                                                       \
      JUCE_VEC_LOOP(vecOp, Mode::loadU, Mode::loadA, Mode::storeA, locals,     \
                    increment)                                                 \
  }                                                                            \
  else {                                                                       \
    if (FloatVectorHelpers::isAligned(src))                                    \
      JUCE_VEC_LOOP(vecOp, Mode::loadA, Mode::loadU, Mode::storeU, locals,     \
                    increment)                                                 \
    else                                                                       \
      JUCE_VEC_LOOP(vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals,     \
                    increment)                                                 \
  }                                                                            \
  JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(normalOp, vecOp, locals, increment, \
                                           setupOp)                            \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp if (FloatVectorHelpers::isAligned(dest)) {                           \
    if (FloatVectorHelpers::isAligned(src1)) {                                 \
      if (FloatVectorHelpers::isAligned(src2))                                 \
        JUCE_VEC_LOOP_TWO_SOURCES(vecOp, Mode::loadA, Mode::loadA,             \
                                  Mode::storeA, locals, increment)             \
      else                                                                     \
        JUCE_VEC_LOOP_TWO_SOURCES(vecOp, Mode::loadA, Mode::loadU,             \
                                  Mode::storeA, locals, increment)             \
    } else {                                                                   \
      if (FloatVectorHelpers::isAligned(src2))                                 \
        JUCE_VEC_LOOP_TWO_SOURCES(vecOp, Mode::loadU, Mode::loadA,             \
                                  Mode::storeA, locals, increment)             \
      else                                                                     \
        JUCE_VEC_LOOP_TWO_SOURCES(vecOp, Mode::loadU, Mode::loadU,             \
                                  Mode::storeA, locals, increment)             \
    }                                                                          \
  }                                                                            \
  else {                                                                       \
...
#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals,       \
                                                increment, setupOp)            \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp if (FloatVectorHelpers::isAligned(dest)) {                           \
    if (FloatVectorHelpers::isAligned(src1)) {                                 \
      if (FloatVectorHelpers::isAligned(src2))                                 \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadA, Mode::loadA, Mode::loadA, Mode::storeA,        \
            locals, increment)                                                 \
      else                                                                     \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadA, Mode::loadU, Mode::loadA, Mode::storeA,        \
            locals, increment)                                                 \
    } else {                                                                   \
      if (FloatVectorHelpers::isAligned(src2))                                 \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadU, Mode::loadA, Mode::loadA, Mode::storeA,        \
            locals, increment)                                                 \
      else                                                                     \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadU, Mode::loadU, Mode::loadA, Mode::storeA,        \
            locals, increment)                                                 \
    }                                                                          \
  }                                                                            \
  else {                                                                       \
    if (FloatVectorHelpers::isAligned(src1)) {                                 \
      if (FloatVectorHelpers::isAligned(src2))                                 \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadA, Mode::loadA, Mode::loadU, Mode::storeU,        \
            locals, increment)                                                 \
      else                                                                     \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadA, Mode::loadU, Mode::loadU, Mode::storeU,        \
            locals, increment)                                                 \
    } else {                                                                   \
      if (FloatVectorHelpers::isAligned(src2))                                 \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadU, Mode::loadA, Mode::loadU, Mode::storeU,        \
            locals, increment)                                                 \
      else                                                                     \
        JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
            vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU,        \
            locals, increment)                                                 \
    }                                                                          \
  }                                                                            \
  JUCE_FINISH_VEC_OP(normalOp)
...
#define JUCE_PERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp)             \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp JUCE_VEC_LOOP(vecOp, dummy, Mode::loadU, Mode::storeU, locals,       \
                        JUCE_INCREMENT_DEST) JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC_DEST(normalOp, vecOp, locals, increment,       \
                                     setupOp)                                  \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp JUCE_VEC_LOOP(vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, \
                        increment) JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(normalOp, vecOp, locals, increment, \
                                           setupOp)                            \
  JUCE_BEGIN_VEC_OP                                                            \
  setupOp JUCE_VEC_LOOP_TWO_SOURCES(vecOp, Mode::loadU, Mode::loadU,           \
                                    Mode::storeU, locals, increment)           \
      JUCE_FINISH_VEC_OP(normalOp)
...
#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals,       \
  ...
  setupOp JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                            \
      vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals,      \
      increment) JUCE_FINISH_VEC_OP(normalOp)
...
#define JUCE_VEC_LOOP_TWO_SOURCES(vecOp, src1Load, src2Load, dstStore, locals, \
                                  increment)                                   \
  for (auto i = (decltype(numLongOps))0; i < numLongOps; ++i) {                \
...
#define JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                              \
    vecOp, src1Load, src2Load, dstLoad, dstStore, locals, increment)           \
  for (auto i = (decltype(numLongOps))0; i < numLongOps; ++i) {                \
...
#define JUCE_LOAD_SRC1_SRC2_DEST(src1Load, src2Load, dstLoad)                  \
  const Mode::ParallelType d = dstLoad(dest), s1 = src1Load(src1),             \
                           s2 = src2Load(src2);
...
template <typename Mode> struct MinMax {
  using Type = typename Mode::Type;
  using ParallelType = typename Mode::ParallelType;

  template <typename Size>
  static Type findMinOrMax(const Type *src, Size num,
                           const bool isMinimum) noexcept {
...
//==============================================================================
namespace {
template <typename Size> void clear(float *dest, Size num) noexcept {
...
template <typename Size> void clear(double *dest, Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
void fill(float *dest, float valueToFill, Size num) noexcept {
  ...
  JUCE_PERFORM_VEC_OP_DEST(dest[i] = valueToFill, val, JUCE_LOAD_NONE,
                           const Mode::ParallelType val =
                               Mode::load1(valueToFill);)
#endif
}
...
template <typename Size>
void copyWithMultiply(float *dest, const float *src, float multiplier,
                      Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
  vDSP_vsmul(src, 1, &multiplier, dest, 1, (vDSP_Length)num);
#else
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] = src[i] * multiplier, Mode::mul(mult, s), JUCE_LOAD_SRC,
...
template <typename Size>
void copyWithMultiply(double *dest, const double *src, double multiplier,
                      Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
void add(float *dest, float amount, Size num) noexcept {
  ...
  JUCE_PERFORM_VEC_OP_DEST(
      dest[i] += amount, Mode::add(d, amountToAdd), JUCE_LOAD_DEST,
...
#else
  JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] += src[i], Mode::add(d, s),
...
void add(float *dest, const float *src1, const float *src2, Size num) noexcept {
  ...
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = src1[i] + src2[i],
                                     Mode::add(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                     JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

template <typename Size>
void add(double *dest, const double *src1, const double *src2,
         Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
#else
  JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] -= src[i], Mode::sub(d, s),
                               JUCE_LOAD_SRC_DEST, JUCE_INCREMENT_SRC_DEST, )
#endif
}
...
template <typename Size>
void subtract(float *dest, const float *src1, const float *src2,
              Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
  vDSP_vsub(src2, 1, src1, 1, dest, 1, (vDSP_Length)num);
#else
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = src1[i] - src2[i],
                                     Mode::sub(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                     JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

template <typename Size>
void subtract(double *dest, const double *src1, const double *src2,
              Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
template <typename Size>
void addWithMultiply(float *dest, const float *src, float multiplier,
                     Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
  vDSP_vsma(src, 1, &multiplier, dest, 1, dest, 1, (vDSP_Length)num);
#else
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] += src[i] * multiplier, Mode::add(d, Mode::mul(mult, s)),
      JUCE_LOAD_SRC_DEST, JUCE_INCREMENT_SRC_DEST,
      const Mode::ParallelType mult = Mode::load1(multiplier);)
#endif
}
...
template <typename Size>
void addWithMultiply(double *dest, const double *src, double multiplier,
                     Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
template <typename Size>
void addWithMultiply(float *dest, const float *src1, const float *src2,
                     Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
  vDSP_vma((float *)src1, 1, (float *)src2, 1, dest, 1, dest, 1,
           (vDSP_Length)num);
#else
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(
      dest[i] += src1[i] * src2[i], Mode::add(d, Mode::mul(s1, s2)),
      JUCE_LOAD_SRC1_SRC2_DEST, JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

template <typename Size>
void addWithMultiply(double *dest, const double *src1, const double *src2,
                     Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
  vDSP_vmaD((double *)src1, 1, (double *)src2, 1, dest, 1, dest, 1,
            (vDSP_Length)num);
#else
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(
      dest[i] += src1[i] * src2[i], Mode::add(d, Mode::mul(s1, s2)),
...
template <typename Size>
void subtractWithMultiply(float *dest, const float *src, float multiplier,
                          Size num) noexcept {
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] -= src[i] * multiplier, Mode::sub(d, Mode::mul(mult, s)),
      JUCE_LOAD_SRC_DEST, JUCE_INCREMENT_SRC_DEST,
      const Mode::ParallelType mult = Mode::load1(multiplier);)
}

template <typename Size>
void subtractWithMultiply(double *dest, const double *src, double multiplier,
                          Size num) noexcept {
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] -= src[i] * multiplier, Mode::sub(d, Mode::mul(mult, s)),
      JUCE_LOAD_SRC_DEST, JUCE_INCREMENT_SRC_DEST,
...
template <typename Size>
void subtractWithMultiply(float *dest, const float *src1, const float *src2,
                          Size num) noexcept {
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(
      dest[i] -= src1[i] * src2[i], Mode::sub(d, Mode::mul(s1, s2)),
      JUCE_LOAD_SRC1_SRC2_DEST, JUCE_INCREMENT_SRC1_SRC2_DEST, )
}

template <typename Size>
void subtractWithMultiply(double *dest, const double *src1, const double *src2,
                          Size num) noexcept {
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(
      dest[i] -= src1[i] * src2[i], Mode::sub(d, Mode::mul(s1, s2)),
      JUCE_LOAD_SRC1_SRC2_DEST, JUCE_INCREMENT_SRC1_SRC2_DEST, )
...
#else
  JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] *= src[i], Mode::mul(d, s),
                               JUCE_LOAD_SRC_DEST, JUCE_INCREMENT_SRC_DEST, )
#endif
}
...
template <typename Size>
void multiply(float *dest, const float *src1, const float *src2,
              Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
  vDSP_vmul(src1, 1, src2, 1, dest, 1, (vDSP_Length)num);
#else
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = src1[i] * src2[i],
                                     Mode::mul(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                     JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

template <typename Size>
void multiply(double *dest, const double *src1, const double *src2,
              Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
#else
  JUCE_PERFORM_VEC_OP_DEST(
      dest[i] *= multiplier, Mode::mul(d, mult), JUCE_LOAD_DEST,
      const Mode::ParallelType mult = Mode::load1(multiplier);)
#endif
}
...
template <typename Size>
void multiply(float *dest, const float *src, float multiplier,
              Size num) noexcept {
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] = src[i] * multiplier, Mode::mul(mult, s), JUCE_LOAD_SRC,
      JUCE_INCREMENT_SRC_DEST,
...
template <typename Size>
void multiply(double *dest, const double *src, double multiplier,
              Size num) noexcept {
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] = src[i] * multiplier, Mode::mul(mult, s), JUCE_LOAD_SRC,
      JUCE_INCREMENT_SRC_DEST,
...
void abs(float *dest, const float *src, Size num) noexcept {
  ...
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] = std::abs(src[i]), Mode::bit_and(s, mask), JUCE_LOAD_SRC,
      JUCE_INCREMENT_SRC_DEST,
...
void min(float *dest, const float *src1, const float *src2, Size num) noexcept {
  ...
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = jmin(src1[i], src2[i]),
                                     Mode::min(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                     JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

template <typename Size>
void min(double *dest, const double *src1, const double *src2,
         Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
void max(float *dest, const float *src1, const float *src2, Size num) noexcept {
  ...
  JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = jmax(src1[i], src2[i]),
                                     Mode::max(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                     JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

template <typename Size>
void max(double *dest, const double *src1, const double *src2,
         Size num) noexcept {
#if JUCE_USE_VDSP_FRAMEWORK
...
template <typename Size>
void clip(float *dest, const float *src, float low, float high,
          Size num) noexcept {
  jassert(high >= low);
  ...
  JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = jmax(jmin(src[i], high), low),
                               Mode::max(Mode::min(s, hi), lo), JUCE_LOAD_SRC,
                               JUCE_INCREMENT_SRC_DEST,
...
template <typename Size>
void clip(double *dest, const double *src, double low, double high,
          Size num) noexcept {
  jassert(high >= low);
...
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
  return FloatVectorHelpers::MinMax<
      FloatVectorHelpers::BasicOps32>::findMinAndMax(src, num);
#else
...
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
  return FloatVectorHelpers::MinMax<
      FloatVectorHelpers::BasicOps64>::findMinAndMax(src, num);
#else
...
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
  return FloatVectorHelpers::MinMax<
      FloatVectorHelpers::BasicOps32>::findMinOrMax(src, num, true);
#else
...
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
  return FloatVectorHelpers::MinMax<
      FloatVectorHelpers::BasicOps64>::findMinOrMax(src, num, true);
#else
  return juce::findMinimum(src, num);
#endif
}
...
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
  return FloatVectorHelpers::MinMax<
      FloatVectorHelpers::BasicOps32>::findMinOrMax(src, num, false);
#else
...
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
  return FloatVectorHelpers::MinMax<
      FloatVectorHelpers::BasicOps64>::findMinOrMax(src, num, false);
#else
  return juce::findMaximum(src, num);
#endif
}
...
template <typename Size>
void convertFixedToFloat(float *dest, const int *src, float multiplier,
                         Size num) noexcept {
#if JUCE_USE_ARM_NEON
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] = (float)src[i] * multiplier,
      vmulq_n_f32(vcvtq_f32_s32(vld1q_s32(src)), multiplier), JUCE_LOAD_NONE,
      JUCE_INCREMENT_SRC_DEST, )
#else
  JUCE_PERFORM_VEC_OP_SRC_DEST(
      dest[i] = (float)src[i] * multiplier,
      Mode::mul(mult, _mm_cvtepi32_ps(_mm_loadu_si128(
...
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::clear(
    FloatType *dest, CountType numValues) noexcept {
  FloatVectorHelpers::clear(dest, numValues);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::fill(
    FloatType *dest, FloatType valueToFill, CountType numValues) noexcept {
  FloatVectorHelpers::fill(dest, valueToFill, numValues);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::copy(
    FloatType *dest, const FloatType *src, CountType numValues) noexcept {
  memcpy(dest, src, (size_t)numValues * sizeof(FloatType));
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE
FloatVectorOperationsBase<FloatType, CountType>::copyWithMultiply(
    FloatType *dest, const FloatType *src, FloatType multiplier,
    CountType numValues) noexcept {
  FloatVectorHelpers::copyWithMultiply(dest, src, multiplier, numValues);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::add(
    FloatType *dest, FloatType amountToAdd, CountType numValues) noexcept {
  FloatVectorHelpers::add(dest, amountToAdd, numValues);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::add(
    FloatType *dest, const FloatType *src, FloatType amount,
    CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::add(
    FloatType *dest, const FloatType *src1, const FloatType *src2,
    CountType num) noexcept {
  FloatVectorHelpers::add(dest, src1, src2, num);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::subtract(
    FloatType *dest, const FloatType *src, CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE
FloatVectorOperationsBase<FloatType, CountType>::addWithMultiply(
    FloatType *dest, const FloatType *src, FloatType multiplier,
    CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE
FloatVectorOperationsBase<FloatType, CountType>::subtractWithMultiply(
    FloatType *dest, const FloatType *src, FloatType multiplier,
    CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::multiply(
    FloatType *dest, const FloatType *src, CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::multiply(
    FloatType *dest, FloatType multiplier, CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::negate(
    FloatType *dest, const FloatType *src, CountType numValues) noexcept {
  FloatVectorHelpers::negate(dest, src, numValues);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::abs(
    FloatType *dest, const FloatType *src, CountType numValues) noexcept {
  FloatVectorHelpers::abs(dest, src, numValues);
}

template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::min(
    FloatType *dest, const FloatType *src, FloatType comp,
    CountType num) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::max(
    FloatType *dest, const FloatType *src, FloatType comp,
    CountType num) noexcept {
...
template <typename FloatType, typename CountType>
void JUCE_CALLTYPE FloatVectorOperationsBase<FloatType, CountType>::clip(
    FloatType *dest, const FloatType *src, FloatType low, FloatType high,
    CountType num) noexcept {
  FloatVectorHelpers::clip(dest, src, low, high, num);
}

template <typename FloatType, typename CountType>
Range<FloatType> JUCE_CALLTYPE
FloatVectorOperationsBase<FloatType, CountType>::findMinAndMax(
    const FloatType *src, CountType numValues) noexcept {
  return FloatVectorHelpers::findMinAndMax(src, numValues);
}

template <typename FloatType, typename CountType>
FloatType JUCE_CALLTYPE
FloatVectorOperationsBase<FloatType, CountType>::findMinimum(
    const FloatType *src, CountType numValues) noexcept {
...
template <typename FloatType, typename CountType>
FloatType JUCE_CALLTYPE
FloatVectorOperationsBase<FloatType, CountType>::findMaximum(
    const FloatType *src, CountType numValues) noexcept {
...
void JUCE_CALLTYPE FloatVectorOperations::convertFixedToFloat(
    float *dest, const int *src, float multiplier, size_t num) noexcept {
  FloatVectorHelpers::convertFixedToFloat(dest, src, multiplier, num);
}

void JUCE_CALLTYPE FloatVectorOperations::convertFixedToFloat(
    float *dest, const int *src, float multiplier, int num) noexcept {
  FloatVectorHelpers::convertFixedToFloat(dest, src, multiplier, num);
}
...
intptr_t JUCE_CALLTYPE FloatVectorOperations::getFpStatusRegister() noexcept {
  ...
  jassertfalse; // No support for getting the floating point status register for
                // your platform
#endif
#endif
...
void JUCE_CALLTYPE FloatVectorOperations::setFpStatusRegister(
    [[maybe_unused]] intptr_t fpsr) noexcept {
#if JUCE_INTEL && JUCE_USE_SSE_INTRINSICS
...
void JUCE_CALLTYPE FloatVectorOperations::enableFlushToZeroMode(
    [[maybe_unused]] bool shouldEnable) noexcept {
...
void JUCE_CALLTYPE FloatVectorOperations::disableDenormalisedNumberSupport(
    [[maybe_unused]] bool shouldDisable) noexcept {
#if JUCE_USE_SSE_INTRINSICS || (JUCE_USE_ARM_NEON || (JUCE_64BIT && JUCE_ARM))
#if JUCE_USE_SSE_INTRINSICS
...
public:
  ...
  template <typename ValueType> struct TestRunner {
    static void runTest(UnitTest &u, Random random) {
...
#else
      // These tests deliberately operate on misaligned memory and will be
      // flagged up by checks for undefined behavior!
      ValueType *const data1 =
          addBytesToPointer(buffer1.get(), random.nextInt(16));
      ValueType *const data2 =
          addBytesToPointer(buffer2.get(), random.nextInt(16));
      ...
      Range<ValueType> minMax1(
          FloatVectorOperations::findMinAndMax(data1, num));
      Range<ValueType> minMax2(Range<ValueType>::findMinAndMax(data1, num));
      u.expect(minMax1 == minMax2);

      u.expect(valuesMatch(FloatVectorOperations::findMinimum(data1, num),
                           juce::findMinimum(data1, num)));
      u.expect(valuesMatch(FloatVectorOperations::findMaximum(data1, num),
                           juce::findMaximum(data1, num)));

      u.expect(valuesMatch(FloatVectorOperations::findMinimum(data2, num),
                           juce::findMinimum(data2, num)));
      u.expect(valuesMatch(FloatVectorOperations::findMaximum(data2, num),
                           juce::findMaximum(data2, num)));
    ...
    static void doConversionTest(UnitTest &u, float *data1, float *data2,
                                 int *const int1, int num) {
    ...
    static void convertFixed(float *d, const int *s, ValueType multiplier,
                             int num) {
      while (--num >= 0)
        *d++ = (float)*s++ * multiplier;
    }

    static bool areAllValuesEqual(const ValueType *d, int num,
                                  ValueType target) {
      while (--num >= 0)
    ...
    static bool buffersMatch(const ValueType *d1, const ValueType *d2,
                             int num) {
      while (--num >= 0)
