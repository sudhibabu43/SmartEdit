...
void AudioProcessLoadMeasurer::reset() { reset(0, 0); }
...
void AudioProcessLoadMeasurer::registerRenderTime(double milliseconds,
                                                  int numSamples) {
  const SpinLock::ScopedTryLockType lock(mutex);
...
void AudioProcessLoadMeasurer::registerRenderTimeLocked(double milliseconds,
                                                        int numSamples) {
  ...
  cpuUsageProportion =
      proportion + filterAmount * (usedProportion - proportion);
...
double AudioProcessLoadMeasurer::getLoadAsProportion() const {
  return jlimit(0.0, 1.0, cpuUsageProportion.load());
}
...
AudioProcessLoadMeasurer::ScopedTimer::ScopedTimer(AudioProcessLoadMeasurer &p,
  ...
  // numSamplesInBlock should never be zero. Did you remember to call
  // AudioProcessLoadMeasurer::reset(), passing the expected samples per block?
  jassert(numSamplesInBlock);
}

AudioProcessLoadMeasurer::ScopedTimer::~ScopedTimer() {
  owner.registerRenderTime(Time::getMillisecondCounterHiRes() - startTime,
                           samplesInBlock);
}
