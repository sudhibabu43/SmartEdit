...
public:
  ...
  /** Resets the counter, in preparation for use with the given sample rate and
   * block size. */
  ...
  /** Returns the number of over- (or under-) runs recorded since the state was
   * reset. */
  ...
  */
  ...
  /** Can be called manually to add the time of a callback to the stats.
      Normally you probably would never call this - it's simpler and more robust
     to use a ScopedTimer to measure the time using an RAII pattern.
  */
