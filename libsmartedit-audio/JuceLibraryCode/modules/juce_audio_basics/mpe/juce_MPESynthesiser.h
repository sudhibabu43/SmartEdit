...
/**
   ...
   MPESynthesiserVoice which can play back one of these sounds at a time.

    Then you can use the addVoice() methods to give the synthesiser a set of
   voices it can use to play notes. If you only give it one voice it will be
   monophonic - the more voices it has, the more polyphony it'll have available.

    Then repeatedly call the renderNextBlock() method to produce the audio
   (inherited from MPESynthesiserBase). The voices will be started, stopped, and
   modulated automatically, based on the MPE/MIDI messages that the synthesiser
   receives.
...
public:
  ...
  */
  ...
  /** Reduces the number of voices to newNumVoices.
      ...
      the total number of voices equals newNumVoices. If newNumVoices is greater
     than or equal to the current number of voices, this method does nothing.
  */
  ...
  */
  ...
  /** Tells the synthesiser what the sample rate is for the audio it's being
     used to render.
      ...
      propagate the new value to the voices so that they can use it to render
     the correct pitches.
  */
  ...
  /** Handle incoming MIDI events.
      ...
      controller messages to handleProgramChange and handleController,
     respectively, and then simply calls through to
     MPESynthesiserBase::handleMidiEvent to deal with MPE-related MIDI messages
...
protected:
  ...
  /** Attempts to start playing a new note.
      ...
      If isNoteStealingEnabled returns true (set this by calling
     setNoteStealingEnabled), the synthesiser will use the voice stealing
     algorithm to find a free voice for the note (if no voices are free
     otherwise).

      This method will be called automatically according to the midi data passed
     into renderNextBlock(). Do not call it yourself, otherwise the internal MPE
     note state will become inconsistent.
  */
  void noteAdded(MPENote newNote) override;

  /** Stops playing a note.

      This will be called whenever an  MPE note is released (either by a
     note-off message, or by a sustain/sostenuto pedal release for a note that
     already received a note-off), and should therefore stop playing.
  ...
  */
  ...
  /** Will find any voice that is currently playing changedNote, update its
      ...
      This method will be called automatically according to the midi data passed
     into renderNextBlock(). Do not call it yourself.
  */
  ...
  */
  void renderNextSubBlock(AudioBuffer<float> &outputAudio, int startSample,
  ...
  /** This will simply call renderNextBlock for each currently active
      ...
      Override this method if you need to do more work to render your audio.
  */
  void renderNextSubBlock(AudioBuffer<double> &outputAudio, int startSample,
                          int numSamples) override;
  ...
  /** Searches through the voices to find one that's not currently playing, and
      ...
      (e.g. for deleting it), you can pass an invalid (default-constructed)
     MPENote.
  */
  ...
  /** Chooses a voice that is most suitable for being re-used to play a new
      ...
      If you pass a valid MPENote for the optional argument, then the note
     number of that note will be taken into account for finding the ideal voice
     to steal. If you pass an invalid (default-constructed) MPENote instead,
     this part of the algorithm will be ignored.
  */
  virtual MPESynthesiserVoice *
  findVoiceToSteal(MPENote noteToStealVoiceFor = MPENote()) const;
  ...
  */
  ...
  /** Stops a given voice and tells it to stop playing a particular MPENote
      ...
      but is protected in case it's useful for some custom subclasses.
  */
  void stopVoice(MPESynthesiserVoice *voice, MPENote noteToStop,
                 bool allowTailOff);
