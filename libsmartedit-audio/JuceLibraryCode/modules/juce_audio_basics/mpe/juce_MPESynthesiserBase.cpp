...
MPESynthesiserBase::MPESynthesiserBase() : instrument(defaultInstrument) {
  instrument.addListener(this);
}

MPESynthesiserBase::MPESynthesiserBase(MPEInstrument &inst) : instrument(inst) {
  instrument.addListener(this);
}
...
//==============================================================================
void MPESynthesiserBase::enableLegacyMode(int pitchbendRange,
...
void MPESynthesiserBase::renderNextBlock(AudioBuffer<floatType> &outputAudio,
  ...
  for (auto it = inputMidi.findNextSamplePosition(startSample);
       it != inputMidi.cend(); ++it) {
    const auto metadata = *it;

    if (metadata.samplePosition >= endSample)
      break;

    const auto smallBlockAllowed =
        (prevSample == startSample && !subBlockSubdivisionIsStrict);
    const auto thisBlockSize = smallBlockAllowed ? 1 : minimumSubBlockSize;

    if (metadata.samplePosition >= prevSample + thisBlockSize) {
      renderNextSubBlock(outputAudio, prevSample,
                         metadata.samplePosition - prevSample);
...
template void MPESynthesiserBase::renderNextBlock<float>(AudioBuffer<float> &,
                                                         const MidiBuffer &,
                                                         int, int);
template void MPESynthesiserBase::renderNextBlock<double>(AudioBuffer<double> &,
                                                          const MidiBuffer &,
                                                          int, int);
...
//==============================================================================
void MPESynthesiserBase::setMinimumRenderingSubdivisionSize(
...
class MpeSynthesiserBaseTests final : public UnitTest {
  ...
  struct StartAndLength {
    ...
    std::tuple<const int &, const int &> tie() const noexcept {
      return std::tie(start, length);
    }

    bool operator==(const StartAndLength &other) const noexcept {
  ...
  private:
    ...
    void renderNextSubBlock(AudioBuffer<float> &, int startSample,
...
public:
  ...
  void runTest() override {
    ...
    beginTest("Subblocks respect their minimum size");
    {
      ...
      const auto blockLengthsAreValid =
          [](const std::vector<StartAndLength> &info, int minLength,
             bool strict) {
            if (info.size() <= 1)
              return true;
      ...
      for (auto strict : {false, true}) {
        for (auto subblockSize : {1, 16, 32, 64, 1024}) {
          ...
          expectWithinAbsoluteError(
              float(e.blocks.size()),
              std::ceil((float)blockSize / (float)subblockSize), 1.0f);
          expect(e.messages.size() == blockSize);
          expect(std::is_sorted(e.blocks.begin(), e.blocks.end()));
          expect(sumBlockLengths(e.blocks) == blockSize);
...

      {
        ...
        expect(synth.events.order ==
               std::vector<CallbackKind>{CallbackKind::process});
