...
struct JUCE_API MPENote {
  ...
  enum KeyState {
    ...
    keyDownAndSustained = 3 /**< The note key is down and sustained (by a
  ...
  /** Constructor.
      ...
      @param midiChannel    The MIDI channel of the note, between 2 and 15.
                            (Channel 1 and channel 16 can never be note channels
     in MPE).
      ...
      @param keyState       The key state of the note (whether the key is down
                            ...
                            be MPENote::off, since you are triggering a new
     note. (If not specified, the default value will be MPENote::keyDown.)
  */
  MPENote(int midiChannel, int initialNote, MPEValue velocity,
          MPEValue pitchbend, MPEValue pressure, MPEValue timbre,
  ...
  */
  ...
  /** Current key state. Indicates whether the note key is currently down
     (pressed) and/or the note is sustained (by a sustain or sostenuto pedal).
  */
  ...
  */
  ...
  /** Returns true if two notes are different notes, determined by their unique
   * ID. */
  bool operator!=(const MPENote &other) const noexcept;
};
