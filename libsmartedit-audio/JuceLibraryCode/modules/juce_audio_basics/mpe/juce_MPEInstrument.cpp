...
MPEInstrument::MPEInstrument(MPEZoneLayout layout) : MPEInstrument() {
...
void MPEInstrument::resetLastReceivedValues() {
  ...
  for (const auto &pair :
       {Defaults{pressureDimension, MPEValue::minValue()},
        Defaults{pitchbendDimension, MPEValue::centreValue()},
        Defaults{timbreDimension, MPEValue::centreValue()}}) {
    mpeInstrumentFill(pair.dimension.lastValueReceivedOnChannel,
                      pair.defaultValue);
  }
}
...
//==============================================================================
void MPEInstrument::enableLegacyMode(int pitchbendRange,
...
void MPEInstrument::processNextMidiEvent(const MidiMessage &message) {
  ...
  else if (message.isResetAllControllers() || message.isAllNotesOff())
...
void MPEInstrument::processMidiNoteOnMessage(const MidiMessage &message) {
  ...
  if (message.getVelocity() == 0) {
    ...
    noteOn(message.getChannel(), message.getNoteNumber(),
...
//==============================================================================
void MPEInstrument::processMidiChannelPressureMessage(
...
//==============================================================================
void MPEInstrument::processMidiResetAllControllersMessage(
    const MidiMessage &message) {
  // in MPE mode, "reset all controllers" is per-zone and expected on the master
  // channel; in legacy mode, it is per MIDI channel (within the channel range
  // used).

  if (legacyMode.isEnabled &&
      legacyMode.channelRange.contains(message.getChannel())) {
    for (int i = notes.size(); --i >= 0;) {
      ...
      if (note.midiChannel == message.getChannel()) {
        ...
        note.noteOffVelocity =
            MPEValue::from7BitInt(64); // some reasonable number
...
void MPEInstrument::handlePressureMSB(int midiChannel, int value) noexcept {
  ...
  pressure(midiChannel, lsb == noLSBValueReceived
                            ? MPEValue::from7BitInt(value)
...
void MPEInstrument::handleTimbreMSB(int midiChannel, int value) noexcept {
  ...
  timbre(midiChannel, lsb == noLSBValueReceived
                          ? MPEValue::from7BitInt(value)
                          : MPEValue::from14BitInt(lsb + (value << 7)));
}
...
//==============================================================================
void MPEInstrument::noteOn(int midiChannel, int midiNoteNumber,
                           MPEValue midiNoteOnVelocity) {
  if (!isUsingChannel(midiChannel))
    return;

  MPENote newNote(midiChannel, midiNoteNumber, midiNoteOnVelocity,
                  ...
                  isMemberChannelSustained[midiChannel - 1]
  ...
  if (auto *alreadyPlayingNote = getNotePtr(midiChannel, midiNoteNumber)) {
    ...
    alreadyPlayingNote->noteOffVelocity =
        MPEValue::from7BitInt(64); // some reasonable number
...
//==============================================================================
void MPEInstrument::noteOff(int midiChannel, int midiNoteNumber,
  ...
  if (auto *note = getNotePtr(midiChannel, midiNoteNumber)) {
    note->keyState = (note->keyState == MPENote::keyDownAndSustained)
                         ? MPENote::sustained
                         : MPENote::off;
    note->noteOffVelocity = midiNoteOffVelocity;

    // If no more notes are playing on this channel in mpe mode, reset the
    // dimension values
    if (!legacyMode.isEnabled && getLastNotePlayedPtr(midiChannel) == nullptr) {
      pressureDimension.lastValueReceivedOnChannel[midiChannel - 1] =
          MPEValue::minValue();
      pitchbendDimension.lastValueReceivedOnChannel[midiChannel - 1] =
          MPEValue::centreValue();
      timbreDimension.lastValueReceivedOnChannel[midiChannel - 1] =
          MPEValue::centreValue();
    }
...
void MPEInstrument::polyAftertouch(int midiChannel, int midiNoteNumber,
                                   MPEValue value) {
  const ScopedLock sl(lock);
...
MPEValue
MPEInstrument::getInitialValueForNewNote(int midiChannel,
                                         MPEDimension &dimension) const {
  if (!legacyMode.isEnabled && getLastNotePlayedPtr(midiChannel) != nullptr)
    return &dimension == &pressureDimension ? MPEValue::minValue()
                                            : MPEValue::centreValue();
...
//==============================================================================
void MPEInstrument::updateDimension(int midiChannel, MPEDimension &dimension,
                                    MPEValue value) {
...
void MPEInstrument::updateDimensionMaster(bool isLowerZone,
                                          ...
                                          MPEValue value) {
  auto zone =
      (isLowerZone ? zoneLayout.getLowerZone() : zoneLayout.getUpperZone());
  ...
  for (int i = notes.size(); --i >= 0;) {
    ...
    if (&dimension == &pitchbendDimension) {
      // master pitchbend is a special case: we don't change the note's own
      // pitchbend, instead we have to update its total (master + note)
      // pitchbend.
...
//==============================================================================
void MPEInstrument::updateDimensionForNote(MPENote &note,
                                           MPEDimension &dimension,
                                           MPEValue value) {
...
//==============================================================================
void MPEInstrument::callListenersDimensionChanged(
    const MPENote &note, const MPEDimension &dimension) {
  if (&dimension == &pressureDimension) {
    listeners.call([&](Listener &l) { l.notePressureChanged(note); });
    return;
  }
  if (&dimension == &timbreDimension) {
    listeners.call([&](Listener &l) { l.noteTimbreChanged(note); });
    return;
  }
  if (&dimension == &pitchbendDimension) {
    listeners.call([&](Listener &l) { l.notePitchbendChanged(note); });
...
void MPEInstrument::updateNoteTotalPitchbend(MPENote &note) {
  if (legacyMode.isEnabled) {
    note.totalPitchbendInSemitones =
        note.pitchbend.asSignedFloat() * (float)legacyMode.pitchbendRange;
  } else {
    ...
    if (zone.isUsingChannelAsMemberChannel(note.midiChannel))
      notePitchbendInSemitones =
          note.pitchbend.asSignedFloat() * (float)zone.perNotePitchbendRange;

    auto masterPitchbendInSemitones =
        pitchbendDimension
            .lastValueReceivedOnChannel[zone.getMasterChannel() - 1]
            .asSignedFloat() *
        (float)zone.masterPitchbendRange;

    note.totalPitchbendInSemitones =
        notePitchbendInSemitones + masterPitchbendInSemitones;
  }
}
...
//==============================================================================
void MPEInstrument::handleSustainOrSostenuto(int midiChannel, bool isDown,
                                             bool isSostenuto) {
  // in MPE mode, sustain/sostenuto is per-zone and expected on the master
  // channel; in legacy mode, sustain/sostenuto is per MIDI channel (within the
  // channel range used).

  if (legacyMode.isEnabled ? (!legacyMode.channelRange.contains(midiChannel))
  ...
  for (int i = notes.size(); --i >= 0;) {
    ...
    if (legacyMode.isEnabled ? (note.midiChannel == midiChannel)
                             : zone.isUsing(note.midiChannel)) {
  ...
  if (!isSostenuto) {
    ...
    if (!legacyMode.isEnabled) {
      if (zone.isLowerZone()) {
        for (int i = zone.getFirstMemberChannel();
             i <= zone.getLastMemberChannel(); ++i)
...
bool MPEInstrument::isMasterChannel(int midiChannel) const noexcept {
  ...
  return (lowerZone.isActive() &&
          midiChannel == lowerZone.getMasterChannel()) ||
...
//==============================================================================
int MPEInstrument::getNumPlayingNotes() const noexcept { return notes.size(); }
...
MPENote MPEInstrument::getMostRecentNoteOtherThan(
...
//==============================================================================
const MPENote *MPEInstrument::getNotePtr(int midiChannel,
                                         int midiNoteNumber) const noexcept {
...
MPENote *MPEInstrument::getNotePtr(int midiChannel,
                                   int midiNoteNumber) noexcept {
  return const_cast<MPENote *>(
      static_cast<const MPEInstrument &>(*this).getNotePtr(midiChannel,
                                                           midiNoteNumber));
}

//==============================================================================
const MPENote *MPEInstrument::getNotePtr(int midiChannel,
                                         TrackingMode mode) const noexcept {
...
MPENote *MPEInstrument::getNotePtr(int midiChannel,
                                   TrackingMode mode) noexcept {
  return const_cast<MPENote *>(
      static_cast<const MPEInstrument &>(*this).getNotePtr(midiChannel, mode));
}

//==============================================================================
const MPENote *
MPEInstrument::getLastNotePlayedPtr(int midiChannel) const noexcept {
  const ScopedLock sl(lock);
...
MPENote *MPEInstrument::getLastNotePlayedPtr(int midiChannel) noexcept {
  return const_cast<MPENote *>(
      static_cast<const MPEInstrument &>(*this).getLastNotePlayedPtr(
          midiChannel));
}

//==============================================================================
const MPENote *
MPEInstrument::getHighestNotePtr(int midiChannel) const noexcept {
...
MPENote *MPEInstrument::getHighestNotePtr(int midiChannel) noexcept {
  return const_cast<MPENote *>(
      static_cast<const MPEInstrument &>(*this).getHighestNotePtr(midiChannel));
}
...
MPENote *MPEInstrument::getLowestNotePtr(int midiChannel) noexcept {
  return const_cast<MPENote *>(
      static_cast<const MPEInstrument &>(*this).getLowestNotePtr(midiChannel));
}
...
void MPEInstrument::Listener::notePressureChanged(
    [[maybe_unused]] MPENote changedNote) {}
void MPEInstrument::Listener::notePitchbendChanged(
    [[maybe_unused]] MPENote changedNote) {}
void MPEInstrument::Listener::noteTimbreChanged(
    [[maybe_unused]] MPENote changedNote) {}
void MPEInstrument::Listener::noteKeyStateChanged(
    [[maybe_unused]] MPENote changedNote) {}
void MPEInstrument::Listener::noteReleased(
    [[maybe_unused]] MPENote finishedNote) {}
...
public:
  ...
  void runTest() override {
    ...
    beginTest("sustainPedal");
    {
      ...
      expectNote(test.getNote(3, 60), 100, 0, 8192, 64,
                 MPENote::keyDownAndSustained);
      expectNote(test.getNote(10, 60), 100, 0, 8192, 64, MPENote::keyDown);
      ...
      expectNote(test.getNote(4, 51), 100, 0, 8192, 64,
                 MPENote::keyDownAndSustained);
      expectEquals(test.noteKeyStateChangedCallCounter, 3);
    ...
    beginTest("sostenutoPedal");
    {
      ...
      // should only sustain notes turned on *before* the press (difference to
      // sustain pedal)
      test.sostenutoPedal(1, true);
      expectEquals(test.noteKeyStateChangedCallCounter, 3);
      test.noteOn(4, 51, MPEValue::from7BitInt(100));
      ...
      // note-off should not turn off sustained notes inside the same zone,
      // but only if they were turned on *before* the sostenuto pedal
      // (difference to sustain pedal)
      test.noteOff(3, 60, MPEValue::from7BitInt(100));
      test.noteOff(4, 51, MPEValue::from7BitInt(100));
      test.noteOff(10, 60, MPEValue::from7BitInt(100)); // not affected!
      expectEquals(test.getNumPlayingNotes(), 1);
    ...
    {
      MPENote testNote(3, 60, MPEValue::centreValue(), MPEValue::centreValue(),
    ...
    {
      {
        ...
        // applying pressure on a master channel should modulate all notes in
        // this zone
      ...
      }
      {
        ...
        // edge case: two notes on the same channel simultaneously. the second
        // one should use pressure = 0 initially but then react to additional
        // pressure messages
        test.noteOn(3, 61, MPEValue::from7BitInt(100));
        test.pressure(3, MPEValue::from7BitInt(77));
        test.noteOn(3, 60, MPEValue::from7BitInt(100));
    ...
    beginTest("pitchbend");
    {
      ...
      }
      {
        ...
        expectEquals(test.getNumPlayingNotes(), 2);
        expectNote(test.getNote(3, 60), 100, 0, 8192, 64, MPENote::sustained);
        expectNote(test.getNote(3, 61), 100, 0, 6666, 64,
                   MPENote::keyDownAndSustained);
        expectEquals(test.notePitchbendChangedCallCounter, 1);
      }
      {
        ...
        // - release the note
        // - press same note again without sending a pitchbend or timbre message
        // before the note-on
        // - the note should be turned on with a default value for
        // pitchbend/timbre,
      ...
      {
        // applying per-note pitchbend should set the note's
        // totalPitchbendInSemitones correctly depending on the per-note
        // pitchbend range of the zone.
        UnitTestInstrument test;
        ...
        expectDoubleWithinRelativeError(
      ...
      {
        // applying master pitchbend should set the note's
        // totalPitchbendInSemitones correctly depending on the master pitchbend
        // range of the zone.
        UnitTestInstrument test;
        ...
        test.noteOn(3, 60, MPEValue::from7BitInt(100));
        test.pitchbend(
            1, MPEValue::from14BitInt(4096)); // halfway between -max and centre
        expectDoubleWithinRelativeError(
            test.getMostRecentNote(3).totalPitchbendInSemitones, -1.0, 0.01);
        ...
        layout.setLowerZone(5, 48,
                            0); // pitchbendrange = 0 --> no pitchbend at all
        test.setZoneLayout(layout);
        test.noteOn(3, 60, MPEValue::from7BitInt(100));
      ...
      }
      {
        ...
        test.pitchbend(1, MPEValue::from14BitInt(
                              4096)); // master pitchbend 0.5 semitones down
        test.pitchbend(3, MPEValue::from14BitInt(
                              0)); // per-note pitchbend 12 semitones down
    ...
    {
      {
        ...
        // modulating timbre on a master channel should modulate all notes in
        // this zone
    ...
    beginTest("processNextMidiEvent");
    {
      ...
      // b) sending LSB and MSB (only the MSB should trigger the call) - per
      // MIDI channel!
      ...
      // sostenuto pedal message (CC66) should trigger sostenutoPedal method
      // call
    ...
    beginTest("default initial values for pitchbend and timbre");
    {
      ...
      test.pitchbend(
          3, MPEValue::from14BitInt(3333)); // use for next note-on on ch. 3
      test.pitchbend(2, MPEValue::from14BitInt(4444)); // ignore
      test.pitchbend(2, MPEValue::from14BitInt(5555)); // ignore

      test.timbre(3,
                  MPEValue::from7BitInt(66)); // use for next note-on on ch. 3
    ...
    beginTest("Legacy mode");
    {
      ...
      {
        // legacy mode w/ custom channel range: note on should trigger notes
        // only within range
...
public:
  UnitTestInstrument()
      : noteOnCallCounter(0), noteOffCallCounter(0), pitchbendCallCounter(0),
        pressureCallCounter(0), timbreCallCounter(0),
        sustainPedalCallCounter(0), sostenutoPedalCallCounter(0),
        noteAddedCallCounter(0), notePressureChangedCallCounter(0),
  ...
  void noteOn(int midiChannel, int midiNoteNumber,
              MPEValue midiNoteOnVelocity) override {
  ...
  void noteOff(int midiChannel, int midiNoteNumber,
               MPEValue midiNoteOffVelocity) override {
  ...
  void aftertouch(int midiChannel, int midiNoteNumber, MPEValue value) {
    const auto message = juce::MidiMessage::aftertouchChange(
        midiChannel, midiNoteNumber, value.as7BitInt());
  ...
  int noteOnCallCounter, noteOffCallCounter, pitchbendCallCounter,
      ...
      noteReleasedCallCounter, lastMidiChannelReceived,
      lastMidiNoteNumberReceived;
...
private:
  ...
  void notePressureChanged(MPENote) override {
    notePressureChangedCallCounter++;
  }
  void notePitchbendChanged(MPENote) override {
    notePitchbendChangedCallCounter++;
  }
  void noteTimbreChanged(MPENote) override { noteTimbreChangedCallCounter++; }
  void noteKeyStateChanged(MPENote) override {
    noteKeyStateChangedCallCounter++;
  }
...
//==============================================================================
void expectNote(MPENote noteToTest, int noteOnVelocity7Bit, int pressure7Bit,
                int pitchbend14Bit, int timbre7Bit,
...
void expectHasFinishedNote(const UnitTestInstrument &test, int channel,
                           int noteNumber, int noteOffVelocity7Bit) {
  ...
  expectEquals(test.lastNoteFinished->noteOffVelocity.as7BitInt(),
               noteOffVelocity7Bit);
  expect(test.lastNoteFinished->keyState == MPENote::off);
}

void expectDoubleWithinRelativeError(double actual, double expected,
