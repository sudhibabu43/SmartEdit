...
void MPESynthesiser::stopVoice(MPESynthesiserVoice *voice, MPENote noteToStop,
                               bool allowTailOff) {
  jassert(voice != nullptr);
...
void MPESynthesiser::handleMidiEvent(const MidiMessage &m) {
  if (m.isController())
    handleController(m.getChannel(), m.getControllerNumber(),
...
MPESynthesiserVoice *
MPESynthesiser::findFreeVoice(MPENote noteToFindVoiceFor,
...
MPESynthesiserVoice *
MPESynthesiser::findVoiceToSteal(MPENote noteToStealVoiceFor) const {
  ...
  MPESynthesiserVoice *low = nullptr; // Lowest sounding note, might be
                                      // sustained, but NOT in release phase
  MPESynthesiserVoice *top = nullptr; // Highest sounding note, might be
                                      // sustained, but NOT in release phase

  // All major OSes use double-locking so this will be lock- and wait-free as
  // long as stealLock is not contended. This is always the case if you do not
  ...
  for (auto *voice : voices) {
    ...
    struct Sorter {
      bool operator()(const MPESynthesiserVoice *a,
                      const MPESynthesiserVoice *b) const noexcept {
        return a->noteOnTime < b->noteOnTime;
      }
    };

    std::sort(usableVoicesToStealArray.begin(), usableVoicesToStealArray.end(),
              Sorter());
    ...
    if (!voice->isPlayingButReleased()) // Don't protect released notes
    {
      ...
      if (low == nullptr ||
          noteNumber < low->getCurrentlyPlayingNote().initialNote)
        low = voice;

      if (top == nullptr ||
          noteNumber > top->getCurrentlyPlayingNote().initialNote)
        top = voice;
    }
  }
  ...
  if (noteToStealVoiceFor.isValid())
    for (auto *voice : usableVoicesToStealArray)
      if (voice->getCurrentlyPlayingNote().initialNote ==
          noteToStealVoiceFor.initialNote)
        return voice;
  ...
  for (auto *voice : usableVoicesToStealArray)
    if (voice != low && voice != top &&
        ...
        voice->getCurrentlyPlayingNote().keyState !=
...
void MPESynthesiser::turnOffAllVoices(bool allowTailOff) {
  {
    ...
    for (auto *voice : voices) {
      voice->currentlyPlayingNote.noteOffVelocity =
...
//==============================================================================
void MPESynthesiser::renderNextSubBlock(AudioBuffer<float> &buffer,
...
void MPESynthesiser::renderNextSubBlock(AudioBuffer<double> &buffer,
                                        int startSample, int numSamples) {
  const ScopedLock sl(voicesLock);
