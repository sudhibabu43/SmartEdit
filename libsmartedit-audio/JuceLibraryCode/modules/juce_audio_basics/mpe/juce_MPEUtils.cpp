...
MPEChannelAssigner::MPEChannelAssigner(Range<int> channelRange)
    : isLegacy(true), channelIncrement(1),
...
int MPEChannelAssigner::findMidiChannelForNewNote(int noteNumber) noexcept {
  ...
  for (int ch = firstChannel;
       (isLegacy || zone->isLowerZone() ? ch <= lastChannel
                                        : ch >= lastChannel);
       ch += channelIncrement) {
    if (midiChannels[(size_t)ch].isFree() &&
        midiChannels[(size_t)ch].lastNotePlayed == noteNumber) {
  ...
  for (int ch = midiChannelLastAssigned + channelIncrement;;
       ch += channelIncrement) {
  ...
  midiChannelLastAssigned =
      findMidiChannelPlayingClosestNonequalNote(noteNumber);
  midiChannels[(size_t)midiChannelLastAssigned].notes.add(noteNumber);

  return midiChannelLastAssigned;
}

int MPEChannelAssigner::findMidiChannelForExistingNote(
    int noteNumber) noexcept {
  const auto iter =
      std::find_if(midiChannels.cbegin(), midiChannels.cend(), [&](auto &ch) {
        return std::find(ch.notes.begin(), ch.notes.end(), noteNumber) !=
               ch.notes.end();
      });

  return iter != midiChannels.cend()
             ? (int)std::distance(midiChannels.cbegin(), iter)
             : -1;
}
...
int MPEChannelAssigner::findMidiChannelPlayingClosestNonequalNote(
    int noteNumber) noexcept {
...
MPEChannelRemapper::MPEChannelRemapper(MPEZoneLayout::Zone zoneToRemap)
    : zone(zoneToRemap), channelIncrement(zone.isLowerZone() ? 1 : -1),
      firstChannel(zone.getFirstMemberChannel()),
      lastChannel(zone.getLastMemberChannel()) {
  // must be an active MPE zone!
...
void MPEChannelRemapper::remapMidiChannelIfNeeded(MidiMessage &message,
  ...
  if (channel == zone.getMasterChannel() &&
      (message.isResetAllControllers() || message.isAllNotesOff())) {
  ...
  if (messageIsNoteData(message)) {
    ...
    // find existing remap
    for (int chan = firstChannel;
         (zone.isLowerZone() ? chan <= lastChannel : chan >= lastChannel);
         chan += channelIncrement)
...
bool MPEChannelRemapper::applyRemapIfExisting(int channel,
...
struct MPEUtilsUnitTests final : public UnitTest {
  MPEUtilsUnitTests() : UnitTest("MPE Utilities", UnitTestCategories::midi) {}
    ...
    beginTest("MPEChannelAssigner");
    {
      ...
      // lower
      {
        ...
        for (int ch = 2; ch <= 16; ++ch) {
          ...
          expectEquals(channelAssigner.findMidiChannelForExistingNote(noteNum),
                       ch);
