...
/**
    ...
    It can either be a lower or an upper zone, where:
      - A lower zone encompasses master channel 1 and an arbitrary number of
   ascending MIDI channels, increasing from channel 2.
      - An upper zone encompasses master channel 16 and an arbitrary number of
   descending MIDI channels, decreasing from channel 15.
...
struct MPEZone {
  ...
  MPEZone(Type type, int memberChannels = 0, int perNotePitchbend = 48,
          int masterPitchbend = 2)
      : zoneType(type), numMemberChannels(memberChannels),
  ...
  int getFirstMemberChannel() const noexcept {
    return isLowerZone() ? lowerZoneMasterChannel + 1
  ...
  bool isUsingChannelAsMemberChannel(int channel) const noexcept {
    return isLowerZone() ? (lowerZoneMasterChannel < channel &&
                            channel <= getLastMemberChannel())
                         : (channel < upperZoneMasterChannel &&
                            getLastMemberChannel() <= channel);
  }

  bool isUsing(int channel) const noexcept {
    return isUsingChannelAsMemberChannel(channel) ||
           channel == getMasterChannel();
  }

  static auto tie(const MPEZone &z) {
    return std::tie(z.zoneType, z.numMemberChannels, z.perNotePitchbendRange,
  ...
  //==============================================================================
  static constexpr int lowerZoneMasterChannel = 1, upperZoneMasterChannel = 16;
...
//==============================================================================
/**
    This class represents the current MPE zone layout of a device capable of
   handling MPE.

    An MPE device can have up to two zones: a lower zone with master channel 1
   and allocated MIDI channels increasing from channel 2, and an upper zone with
   master channel 16 and allocated MIDI channels decreasing from channel 15. MPE
   mode is enabled on a device when one of these zones is active and disabled
   when both are inactive.
...
public:
  ...
  /** Creates a layout with a single upper or lower zone, leaving the other zone
   * uninitialised. */
  ...
  void setLowerZone(int numMemberChannels = 0, int perNotePitchbendRange = 48,
                    int masterPitchbendRange = 2) noexcept;

  /** Sets the upper zone of this layout. */
  void setUpperZone(int numMemberChannels = 0, int perNotePitchbendRange = 48,
                    int masterPitchbendRange = 2) noexcept;
