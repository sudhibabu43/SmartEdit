...
template <typename Integral> struct ReadTrait;

template <> struct ReadTrait<uint32> {
  static constexpr auto read = ByteOrder::bigEndianInt;
};

template <> struct ReadTrait<uint16> {
...
MidiFile::MidiFile(MidiFile &&other)
    : tracks(std::move(other.tracks)), timeFormat(other.timeFormat) {}
...
void MidiFile::clear() { tracks.clear(); }

//==============================================================================
int MidiFile::getNumTracks() const noexcept { return tracks.size(); }
...
//==============================================================================
short MidiFile::getTimeFormat() const noexcept { return timeFormat; }
...
void MidiFile::readNextTrack(const uint8 *data, int size,
  ...
  // sort so that we put all the note-offs before note-ons that have the same
  // time
  ...
  if (timeFormat != 0) {
    for (auto *ms : tracks) {
      for (int j = ms->getNumEvents(); --j >= 0;) {
        ...
        m.setTimeStamp(MidiFileHelpers::convertTicksToSeconds(
            m.getTimeStamp(), tempoEvents, timeFormat));
      }
    }
  }
}
...
bool MidiFile::writeTrack(OutputStream &mainOut,
                          const MidiMessageSequence &ms) const {
  ...
  for (int i = 0; i < ms.getNumEvents(); ++i) {
    ...
    if (statusByte == lastStatusByte && (statusByte & 0xf0) != 0xf0 &&
        dataSize > 1 && i > 0) {
      ++data;
      --dataSize;
...
struct MidiFileTest final : public UnitTest {
  MidiFileTest() : UnitTest("MidiFile", UnitTestCategories::midi) {}
  ...
  void runTest() override {
    ...
    beginTest("ReadTrack returns available messages if input is truncated");
    {
...

      {
        ...
        expectEquals(sequence.getEventPointer(0)->message.getNoteNumber(),
                     0x40);
        expectEquals(sequence.getEventPointer(0)->message.getVelocity(),
                     (uint8)0x00);
      }
    }
    ...
    beginTest("Header parsing works");
    {
...

      {
        ...
        const auto header =
            parseHeader([](OutputStream &os) { writeBytes(os, {0xff}); });
    ...
    beginTest("Read from stream");
    {
...

      {
        ...
        expectEquals(track.getEventPointer(0)->message.getTimeStamp(),
                     (double)0x0f);
      }
    }
  }

  template <typename Fn> static MidiMessageSequence parseSequence(Fn &&fn) {
    MemoryOutputStream os;
    fn(os);

    return MidiFileHelpers::readTrack(
        reinterpret_cast<const uint8 *>(os.getData()), (int)os.getDataSize());
  }
  ...
  static Optional<MidiFileHelpers::HeaderDetails> parseHeader(Fn &&fn) {
    ...
    return MidiFileHelpers::parseMidiHeader(
        reinterpret_cast<const uint8 *>(os.getData()), os.getDataSize());
  }

  template <typename Fn> static Optional<MidiFile> parseFile(Fn &&fn) {
    MemoryOutputStream os;
    fn(os);
