...
public:
  ...
  void runTest() override {
    ...
    beginTest("Short bytestream midi messages can be round-tripped through the "
              "UMP converter");
    {
      ...
      forEachNonSysExTestMessage(random, [&](const MidiMessage &m) {
        ...
        translator.dispatch(View{packets.data()}, 0,
    ...
    beginTest("Bytestream SysEx converts to universal packets");
    {
...

      {
        ...
        expect(packets.data()[0] ==
               Utils::bytesToWord(std::byte{0x30}, std::byte{0x01},
                                  std::byte{sysEx[0]}, std::byte{0}));
        expect(packets.data()[1] == 0x00000000);
      }

      {
        ...
        const auto *sysEx = message.getSysExData();
        expect(packets.data()[0] ==
               Utils::bytesToWord(std::byte{0x30}, std::byte{0x06},
                                  std::byte{sysEx[0]}, std::byte{sysEx[1]}));
        expect(packets.data()[1] ==
               Utils::bytesToWord(std::byte{sysEx[2]}, std::byte{sysEx[3]},
                                  std::byte{sysEx[4]}, std::byte{sysEx[5]}));
      }

      {
        ...
        const auto *sysEx = message.getSysExData();
        expect(packets.data()[0] ==
               Utils::bytesToWord(std::byte{0x30}, std::byte{0x16},
                                  std::byte{sysEx[0]}, std::byte{sysEx[1]}));
        expect(packets.data()[1] ==
               Utils::bytesToWord(std::byte{sysEx[2]}, std::byte{sysEx[3]},
                                  std::byte{sysEx[4]}, std::byte{sysEx[5]}));
        expect(packets.data()[2] ==
               Utils::bytesToWord(std::byte{0x30}, std::byte{0x36},
                                  std::byte{sysEx[6]}, std::byte{sysEx[7]}));
        expect(packets.data()[3] ==
               Utils::bytesToWord(std::byte{sysEx[8]}, std::byte{sysEx[9]},
                                  std::byte{sysEx[10]}, std::byte{sysEx[11]}));
      }

      {
        ...
        expect(packets.data()[1] ==
               Utils::bytesToWord(std::byte{sysEx[2]}, std::byte{sysEx[3]},
                                  std::byte{sysEx[4]}, std::byte{sysEx[5]}));
        expect(packets.data()[2] ==
               Utils::bytesToWord(std::byte{0x30}, std::byte{0x26},
                                  std::byte{sysEx[6]}, std::byte{sysEx[7]}));
        expect(packets.data()[3] ==
               Utils::bytesToWord(std::byte{sysEx[8]}, std::byte{sysEx[9]},
                                  std::byte{sysEx[10]}, std::byte{sysEx[11]}));
        expect(packets.data()[4] ==
               Utils::bytesToWord(std::byte{0x30}, std::byte{0x31},
                                  std::byte{sysEx[12]}, std::byte{0}));
    ...
    const auto checkRoundTrip = [&](const MidiBuffer &expected) {
      for (const auto meta : expected)
        Conversion::toMidi1(ump::BytestreamMidiView(meta),
                            [&](const auto p) { packets.add(p); });

      MidiBuffer output;
      converter.dispatch(packets.data(), packets.data() + packets.size(), 0,
                         [&](const BytestreamMidiView &roundTripped) {
                           output.addEvent(roundTripped.getMessage(),
                                           int(roundTripped.timestamp));
                         });
      packets.clear();
    ...
    beginTest("Long SysEx bytestream midi messages can be round-tripped "
              "through the UMP converter");
    {
    ...
    beginTest("UMP SysEx7 messages interspersed with utility messages convert "
              "to bytestream");
    {
      ...
      converter.dispatch(modifiedPackets.data(),
                         modifiedPackets.data() + modifiedPackets.size(), 0,
                         [&](const BytestreamMidiView &roundTripped) {
                           output.addEvent(roundTripped.getMessage(),
                                           int(roundTripped.timestamp));
                         });
    ...
    beginTest("UMP SysEx7 messages interspersed with System Realtime messages "
              "convert to bytestream");
    {
      ...
      const auto addRandomRealtimeUMP = [&] {
        ...
        modifiedPackets.add(View(newPacket.data()));
        realtimeMessages.addEvent(
            Midi1ToBytestreamTranslator::fromUmp(newPacket), 0);
      };
    ...
    beginTest("UMP SysEx7 messages interspersed with System Realtime and "
              "Utility messages convert to bytestream");
    {
        ...
        "SysEx messages are terminated by non-Utility, non-Realtime messages");
    {
      ...
      const auto noteOnPackets = [&] {
        ...
        for (const auto meta : noteOn)
          Conversion::toMidi1(ump::BytestreamMidiView(meta),
                              [&](const auto packet) { p.add(packet); });
      ...
      const auto modifiedPackets = [&] {
        ...
        std::for_each(originalPackets.begin(), insertionPoint,
                      [&](const View &view) { p.add(view); });

        for (const auto &view : noteOnPackets)
          p.add(view);

        std::for_each(insertionPoint, originalPackets.end(),
                      [&](const View &view) { p.add(view); });

        return p;
      }();

      // modifiedPackets now contains some SysEx packets interrupted by a MIDI 1
      // noteOn

      MidiBuffer output;

      const auto pushToOutput = [&](const Packets &p) {
        converter.dispatch(p.data(), p.data() + p.size(), 0,
                           [&](const BytestreamMidiView &roundTripped) {
                             output.addEvent(roundTripped.getMessage(),
                                             int(roundTripped.timestamp));
                           });
    ...
    beginTest("Widening conversions work");
    {
      ...
      const auto baselineScale = [](uint32_t srcVal, uint32_t srcBits,
                                    uint32_t dstBits) {
      ...
      expectEquals((int64_t)Conversion::scaleTo16((uint8_t)0x00),
                   (int64_t)0x0000);
      expectEquals((int64_t)Conversion::scaleTo16((uint8_t)0x0a),
                   (int64_t)0x1400);
      expectEquals((int64_t)Conversion::scaleTo16((uint8_t)0x40),
                   (int64_t)0x8000);
      expectEquals((int64_t)Conversion::scaleTo16((uint8_t)0x57),
                   (int64_t)0xaeba);
      expectEquals((int64_t)Conversion::scaleTo16((uint8_t)0x7f),
                   (int64_t)0xffff);
      ...
      expectEquals((int64_t)Conversion::scaleTo32((uint16_t)0x0000),
                   (int64_t)0x00000000);
      expectEquals((int64_t)Conversion::scaleTo32((uint16_t)0x2000),
                   (int64_t)0x80000000);
      expectEquals((int64_t)Conversion::scaleTo32((uint16_t)0x3fff),
                   (int64_t)0xffffffff);
    ...
    beginTest("Round-trip widening/narrowing conversions work");
    {
      for (auto i = 0; i != 100; ++i) {
...

        {
          const auto rand = (uint16_t)random.nextInt(0x4000);
          expectEquals(
              (uint64_t)Conversion::scaleTo14(Conversion::scaleTo16(rand)),
              (uint64_t)rand);
        }

        {
          const auto rand = (uint16_t)random.nextInt(0x4000);
          expectEquals(
              (uint64_t)Conversion::scaleTo14(Conversion::scaleTo32(rand)),
              (uint64_t)rand);
        }
      }
    }
    ...
    beginTest("MIDI 2 -> 1 program change and bank select conversion");
    {
      ...
      {
        // If the bank valid bit is 1, emit bank select control changes and a
        // program change
        Packets midi2;
    ...
    {
      const std::byte opcodes[]{std::byte{0x0}, std::byte{0x1}, std::byte{0x4},
                                std::byte{0x5}, std::byte{0x6}, std::byte{0xf}};

      for (const auto opcode : opcodes) {
        Packets midi2;
        midi2.add(PacketX2{Utils::bytesToWord(std::byte{0x40},
                                              std::byte{opcode << 0x4},
    ...
    beginTest("MIDI 2 -> 1 messages which are passed through");
    {
      ...
      for (const auto typecode : typecodesX1) {
        Packets p;
        p.add(PacketX1{(uint32_t)((int64_t)typecode << 0x1c |
                                  (random.nextInt64() & 0xffffff))});
...

      {
        Packets p;
        p.add(
            PacketX2{(uint32_t)(0x3 << 0x1c | (random.nextInt64() & 0xffffff)),
...

      {
        Packets p;
        p.add(
            PacketX4{(uint32_t)(0x5 << 0x1c | (random.nextInt64() & 0xffffff)),
                     (uint32_t)(random.nextInt64() & 0xffffffff),
                     (uint32_t)(random.nextInt64() & 0xffffffff),
                     (uint32_t)(random.nextInt64() & 0xffffffff)});
    ...
    {
      {
        ...
        Packets midi2;
        midi2.add(PacketX2{0x40904000,
                           static_cast<uint32_t>(Conversion::scaleTo16(0x40_u8))
                               << 0x10});
    ...
    beginTest("MIDI 1 -> 2 note off conversions");
    {
      ...
      Packets midi2;
      midi2.add(PacketX2{0x41831000,
                         static_cast<uint32_t>(Conversion::scaleTo16(0x20_u8))
                             << 0x10});
    ...
    beginTest("individual MIDI 1 -> 2 control changes which should be ignored");
    {
      ...
      for (const auto cc : CCs) {
        Packets midi1;
        midi1.add(PacketX1{Utils::bytesToWord(std::byte{0x20}, std::byte{0xb0},
                                              std::byte{cc}, std::byte{0x00})});
    ...
    beginTest("MIDI 1 -> 2 control change conversions");
    {
      ...
      // nrpn
      {
        ...
        midi2.add(PacketX2{
            0x40300123,
            Conversion::scaleTo32(static_cast<uint16_t>((0x45 << 7) | 0x67))});
      ...
      // rpn
      {
        ...
        midi2.add(PacketX2{
            0x40204321,
            Conversion::scaleTo32(static_cast<uint16_t>((0x01 << 7) | 0x23))});
    ...
    beginTest("MIDI 1 -> MIDI 2 pitch bend conversions");
    {
      ...
      midi2.add(PacketX2{
          0x40e70000,
          Conversion::scaleTo32(static_cast<uint16_t>((0x67 << 7) | 0x45))});
...
private:
  static Packets toMidi1(const MidiMessage &msg) {
    Packets packets;
    Conversion::toMidi1(ump::BytestreamMidiView(&msg),
                        [&](const auto p) { packets.add(p); });
    return packets;
  }
  ...
  static Packets convertMidi2ToMidi1(const Packets &midi2) {
    ...
    for (const auto &packet : midi2)
      Conversion::midi2ToMidi1DefaultTranslation(
          packet, [&r](const View &v) { r.add(v); });
  ...
  void checkBytestreamConversion(const Packets &actual,
    ...
    std::for_each(expected.data(), expected.data() + expected.size(),
                  [&](const uint32_t word) {
                    expectEquals((uint64_t)*actualPtr++, (uint64_t)word);
                  });
  }

  void checkMidi2ToMidi1Conversion(const Packets &midi2,
                                   const Packets &expected) {
    checkBytestreamConversion(convertMidi2ToMidi1(midi2), expected);
  }

  void checkMidi1ToMidi2Conversion(const Packets &midi1,
                                   const Packets &expected) {
  ...
  PacketX1 createRandomUtilityUMP(Random &random) {
    ...
    return PacketX1{
        Utils::bytesToWord(std::byte{0}, std::byte(status << 0x4),
  ...
  PacketX1 createRandomRealtimeUMP(Random &random) {
    ...
    return PacketX1{Utils::bytesToWord(std::byte{0x10}, status, std::byte{0x00},
                                       std::byte{0x00})};
  }
