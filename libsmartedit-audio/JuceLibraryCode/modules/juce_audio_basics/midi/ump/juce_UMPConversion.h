...
struct BytestreamMidiView {
  constexpr BytestreamMidiView(Span<const std::byte> bytesIn,
                               double timestampIn)
      : bytes(bytesIn), timestamp(timestampIn) {}

  /** Creates a view over the provided message.

      Note that the argument is a pointer, not a reference, in order to avoid
...
struct Conversion {
  ...
  */
  ...
  static void toMidi1(const BytestreamMidiView &m,
    ...
    if (firstByte != std::byte{0xf0}) {
      ...
      const auto extraByte =
          ((((firstByte & std::byte{0xf0}) == std::byte{0xf0}) ? std::byte{0x1}
                                                               : std::byte{0x2})
           << 0x4);
      const PacketX1 packet{
          mask & Utils::bytesToWord(extraByte, data[0], data[1], data[2])};
      callback(View(packet.data()));
      return;
    }

    const auto numSysExBytes = (ssize_t)(size - 2);
    const auto numMessages =
        SysEx7::getNumPacketsRequiredForDataSize((uint32_t)numSysExBytes);
    auto *dataOffset = data + 1;

    if (numMessages <= 1) {
      const auto packet =
          Factory::makeSysExIn1Packet(0, (uint8_t)numSysExBytes, dataOffset);
      callback(View(packet.data()));
      return;
    }
    ...
    for (auto i = static_cast<ssize_t>(numSysExBytes); i > 0;
  ...
  /** Converts UMP messages which may include MIDI 2.0 channel voice messages
     into equivalent MIDI 1.0 messages (still in UMP format).
  ...
  */
  ...
  static void midi2ToMidi1DefaultTranslation(const View &v,
                                             Callback &&callback) {
    ...
    const auto typeAndGroup =
        ((std::byte{0x2} << 0x4) | std::byte{Utils::getGroup(firstWord)});
    ...
    case 0xb: // control change
    {
      ...
      const PacketX1 packet{Utils::bytesToWord(typeAndGroup, statusAndChannel,
                                               byte2, correctedByte)};
      callback(View(packet.data()));
      return;
    }
    ...
    case 0x3: // nrpn
    {
      ...
      const auto statusAndChannel =
          std::byte((0xb << 0x4) | Utils::getChannel(firstWord));
      const auto data = scaleTo14(v[1]);

      const PacketX1 packets[]{
          PacketX1{Utils::bytesToWord(typeAndGroup, statusAndChannel, ccX,
                                      std::byte((firstWord >> 0x8) & 0x7f))},
          PacketX1{Utils::bytesToWord(typeAndGroup, statusAndChannel, ccY,
                                      std::byte((firstWord >> 0x0) & 0x7f))},
          PacketX1{Utils::bytesToWord(typeAndGroup, statusAndChannel,
                                      std::byte{6},
