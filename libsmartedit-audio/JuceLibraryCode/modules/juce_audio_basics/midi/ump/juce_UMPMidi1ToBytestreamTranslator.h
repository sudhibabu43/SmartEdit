...
/**
    Parses a raw stream of uint32_t holding a series of Universal MIDI Packets
   using the MIDI 1.0 Protocol, converting to plain (non-UMP) MidiMessages.

    @tags{Audio}
*/
class Midi1ToBytestreamTranslator {
public:
  /** Ensures that there is room in the internal buffer for a sysex message of
     at least `initialBufferSize` bytes.
  */
  ...
  /** Converts a Universal MIDI Packet using the MIDI 1.0 Protocol to
      ...
      @param callback     a callback which will be called with each converted
     MidiMessage.
  */
  ...
  */
  static MidiMessage fromUmp(const PacketX1 &m, double time = 0) {
    ...
    const auto numBytes =
        MidiMessage::getMessageLengthFromFirstByte(bytes.front());
    return MidiMessage(bytes.data(), numBytes, time);
  }

private:
  template <typename MessageCallback>
  void processSysEx(const PacketX2 &packet, double time,
  ...
  void pushBytes(const PacketX2 &packet) {
    ...
    pendingSysExData.insert(pendingSysExData.end(), bytes.data.begin(),
  ...
  static bool shouldPacketTerminateSysExEarly(uint32_t firstWord) {
    return !(isSysExContinuation(firstWord) || isSystemRealTime(firstWord) ||
             isJROrNOP(firstWord));
  }
  ...
  static bool isSystemRealTime(uint32_t word) {
    return Utils::getMessageType(word) == 0x1 &&
           ((word >> 0x10) & 0xff) >= 0xf8;
  }
