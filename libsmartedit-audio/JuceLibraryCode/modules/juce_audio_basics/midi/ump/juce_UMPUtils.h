...
struct Utils {
  ...
  static constexpr uint32_t bytesToWord(std::byte a, std::byte b, std::byte c,
                                        std::byte d) {
    return uint32_t(a) << 0x18 | uint32_t(b) << 0x10 | uint32_t(c) << 0x08 |
           uint32_t(d) << 0x00;
  }

  /** Returns the expected number of 32-bit words in a Universal MIDI Packet,
     given the first word of the packet.
  ...
  */
  template <size_t Index> struct U8 {
  ...
  */
  template <size_t Index> struct U16 {
