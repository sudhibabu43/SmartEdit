...
/**
    Parses a raw stream of uint32_t, and calls a user-provided callback every
   time a full Universal MIDI Packet is encountered.
...
public:
  ...
  /** Calls `callback` with a View of each packet encountered in the range
     delimited by `begin` and `end`.

      If the range ends part-way through a packet, the next call to `dispatch`
     will continue from that point in the packet (unless `reset` is called
     first).
  */
  ...
  void dispatch(const uint32_t *begin, const uint32_t *end, double timeStamp,
    ...
    std::for_each(begin, end, [&](uint32_t word) {
      ...
      if (currentPacketLen ==
          Utils::getNumWordsForMessageType(nextPacket.front())) {
...
//==============================================================================
/**
    Parses a stream of bytes representing a sequence of bytestream-encoded
   MIDI 1.0 messages, converting the messages to UMP format and passing the
   packets to a user-provided callback as they become ready.
...
public:
  ...
  */
  explicit BytestreamToUMPDispatcher(PacketProtocol pp, int storageSize)
      : concatenator(storageSize), converter(pp) {}
  ...
  /** Calls `callback` with a View of each converted packet as it becomes ready.
      ...
      @param begin        the first byte in a range of bytes representing
     bytestream-encoded MIDI messages.
      @param end          one-past the last byte in a range of bytes
     representing bytestream-encoded MIDI messages.
      @param timestamp    a timestamp to apply to the created packets.
      @param callback     a callback which will be passed a View pointing to
     each new packet as it becomes ready.
  */
  template <typename PacketCallbackFunction>
  void dispatch(const uint8_t *begin, const uint8_t *end, double timestamp,
                PacketCallbackFunction &&callback) {
...
#define JUCE_MINGW_HIDDEN_VISIBILITY
    ...
    struct JUCE_MINGW_HIDDEN_VISIBILITY Callback {
      ...
      void handlePartialSysexMessage(void *, const uint8_t *, int,
                                     double) const {}
...
#undef JUCE_MINGW_HIDDEN_VISIBILITY
    ...
    concatenator.pushMidiData(begin, int(end - begin), timestamp,
                              (void *)nullptr, inputCallback);
  }
...
//==============================================================================
/**
    Parses a stream of 32-bit words representing a sequence of UMP-encoded MIDI
   messages, converting the messages to MIDI 1.0 bytestream format and passing
   them to a user-provided callback as they become ready.
...
public:
  /** Initialises the dispatcher.
      ...
      `storageSize` bytes will be allocated to store incomplete messages.
  */
  explicit ToBytestreamDispatcher(int storageSize) : converter(storageSize) {}
  ...
  /** Calls `callback` with converted bytestream-formatted MidiMessage whenever
      ...
      @param begin        the first word in a stream of words representing
     UMP-encoded MIDI packets.
      @param end          one-past the last word in a stream of words
     representing UMP-encoded MIDI packets.
      @param timestamp    a timestamp to apply to converted messages.
      @param callback     a callback which will be passed a MidiMessage each
     time a new message becomes ready.
  */
  template <typename BytestreamMessageCallback>
  void dispatch(const uint32_t *begin, const uint32_t *end, double timestamp,
                BytestreamMessageCallback &&callback) {
    dispatcher.dispatch(begin, end, timestamp,
                        [&](const View &view, double time) {
