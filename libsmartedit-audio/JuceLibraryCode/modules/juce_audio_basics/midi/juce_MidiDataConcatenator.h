  ...
  void pushMidiData(const void *inputData, int numBytes, double time,
    ...
    while (numBytes > 0) {
      ...
      if (isInitialByte(nextByte)) {
        ...
        // message is too long or invalid MIDI - abandon it and start again with
        // the next byte
        currentMessageLen = 0;
        continue;
      }
      ...
      auto expectedLength =
          MidiMessage::getMessageLengthFromFirstByte(currentMessage[0]);

      if (expectedLength == currentMessageLen) {
        callback.handleIncomingMidiMessage(
            input, MidiMessage(currentMessage, expectedLength, time));
        currentMessageLen = 1; // reset, but leave the first byte to use as the
                               // running status byte
      }
    }
  }
  ...
  void processSysex(const uint8 *&d, int &numBytes, double time,
    ...
    if (pendingSysexSize > 0) {
      if (totalMessage[pendingSysexSize - 1] == 0xf7) {
        ...
        callback.handlePartialSysexMessage(input, totalMessage,
                                           pendingSysexSize, pendingSysexTime);
      }
    }
  }
