...
void MidiMessageSequence::clear() { list.clear(); }

int MidiMessageSequence::getNumEvents() const noexcept { return list.size(); }

MidiMessageSequence::MidiEventHolder *
MidiMessageSequence::getEventPointer(int index) const noexcept {
  return list[index];
}

MidiMessageSequence::MidiEventHolder **MidiMessageSequence::begin() noexcept {
  return list.begin();
}
MidiMessageSequence::MidiEventHolder *const *
MidiMessageSequence::begin() const noexcept {
  return list.begin();
}
MidiMessageSequence::MidiEventHolder **MidiMessageSequence::end() noexcept {
  return list.end();
}
MidiMessageSequence::MidiEventHolder *const *
MidiMessageSequence::end() const noexcept {
  return list.end();
}
...
int MidiMessageSequence::getIndexOfMatchingKeyUp(int index) const noexcept {
  if (auto *meh = list[index]) {
    if (auto *noteOff = meh->noteOffObject) {
      ...
      jassertfalse; // we've somehow got a pointer to a note-off object that
...
int MidiMessageSequence::getIndexOf(
    const MidiEventHolder *event) const noexcept {
...
//==============================================================================
MidiMessageSequence::MidiEventHolder *
MidiMessageSequence::addEvent(MidiEventHolder *newEvent,
...
MidiMessageSequence::MidiEventHolder *
MidiMessageSequence::addEvent(const MidiMessage &newMessage,
                              double timeAdjustment) {
  return addEvent(new MidiEventHolder(newMessage), timeAdjustment);
}

MidiMessageSequence::MidiEventHolder *
MidiMessageSequence::addEvent(MidiMessage &&newMessage, double timeAdjustment) {
...
//==============================================================================
void MidiMessageSequence::extractMidiChannelMessages(
    const int channelNumberToExtract, MidiMessageSequence &destSequence,
...
void MidiMessageSequence::extractSysExMessages(
    MidiMessageSequence &destSequence) const {
  for (auto *meh : list)
    if (meh->message.isSysEx())
      destSequence.addEvent(meh->message);
}

void MidiMessageSequence::deleteMidiChannelMessages(
    ...
    for (auto it = std::begin(values); it != std::end(values); ++it)
      if (it->hasValue())
        out.add(MidiMessage::controllerEvent(
            channel, (int)std::distance(std::begin(values), it), **it));
  }

  void set(int controller, int value) { values[controller] = (char)value; }
};
...
public:
  void emit(int channel, double time, Array<MidiMessage> &out) const {
    ...
    if (bankLSB.hasValue() && bankMSB.hasValue()) {
      out.add(MidiMessage::controllerEvent(channel, 0x00, *bankMSB)
                  .withTimeStamp(time));
      out.add(MidiMessage::controllerEvent(channel, 0x20, *bankLSB)
                  .withTimeStamp(time));
    }
...
class ParameterNumberState {
  ...
  Optional<char> newestRpnLsb, newestRpnMsb, newestNrpnLsb, newestNrpnMsb,
      lastSentLsb, lastSentMsb;
  Kind lastSentKind = Kind::rpn, newestKind = Kind::rpn;

public:
  // If the effective parameter number has changed since the last time this
  // function was called, this will emit the current parameter in full (MSB and
  // LSB). This should be called before each data message (entry, increment,
  // decrement: 0x06, 0x26, 0x60, 0x61) to ensure that the data message operates
  // on the correct parameter number.
  void sendIfNecessary(int channel, double time, Array<MidiMessage> &out) {
    const auto newestMsb =
        newestKind == Kind::rpn ? newestRpnMsb : newestNrpnMsb;
    const auto newestLsb =
        newestKind == Kind::rpn ? newestRpnLsb : newestNrpnLsb;
    ...
    out.add(MidiMessage::controllerEvent(
                channel, newestKind == Kind::rpn ? 0x65 : 0x63, *newestMsb)
                .withTimeStamp(time));
    out.add(MidiMessage::controllerEvent(
                channel, newestKind == Kind::rpn ? 0x64 : 0x62, *newestLsb)
                .withTimeStamp(time));
...
void MidiMessageSequence::createControllerUpdatesForTime(
    int channel, double time, Array<MidiMessage> &dest) {
  ...
  for (const auto &item : list) {
    ...
    if (mm.isController()) {
      ...
      if (parameterNumberState.trySetProgramNumber(num,
      ...
      if (std::find(std::begin(passthroughs), std::end(passthroughs), num) !=
          std::end(passthroughs)) {
...
struct MidiMessageSequenceTest final : public UnitTest {
  ...
  void runTest() override {
    ...
    expectEquals(s.getIndexOfMatchingKeyUp(0),
                 -1); // Truncated note, should be no note off
    ...
    struct DataEntry {
      ...
      void addToSequence(MidiMessageSequence &s) const {
        for (const auto &pair : getControlValues())
          s.addEvent(
              MidiMessage::controllerEvent(channel, pair.control, pair.value),
              time);
      }

      bool matches(const MidiMessage *begin, const MidiMessage *end) const {
        const auto isEqual = [this](const ControlValue &cv,
                                    const MidiMessage &msg) {
          return exactlyEqual(msg.getTimeStamp(), time) && msg.isController() &&
    ...
    const auto addNrpn = [&](MidiMessageSequence &seq, int channel,
                             int parameter, int value, double time = 0.0) {
      DataEntry{0x62, channel, parameter, value, time}.addToSequence(seq);
    };

    const auto addRpn = [&](MidiMessageSequence &seq, int channel,
                            int parameter, int value, double time = 0.0) {
      DataEntry{0x64, channel, parameter, value, time}.addToSequence(seq);
    };

    const auto checkNrpn = [&](const MidiMessage *begin, const MidiMessage *end,
                               int channel, int parameter, int value,
                               double time = 0.0) {
      expect(
          DataEntry{0x62, channel, parameter, value, time}.matches(begin, end));
    };

    const auto checkRpn = [&](const MidiMessage *begin, const MidiMessage *end,
                              int channel, int parameter, int value,
                              double time = 0.0) {
      expect(
          DataEntry{0x64, channel, parameter, value, time}.matches(begin, end));
    };

    beginTest("createControllerUpdatesForTime should emit (N)RPN components in "
              "the correct order");
    {
    ...
    beginTest("createControllerUpdatesForTime ignores (N)RPNs after the final "
              "requested time");
    {
    ...
    beginTest("createControllerUpdatesForTime should emit separate (N)RPN "
              "messages when appropriate");
    {
      ...
      checkRpn(std::next(m.begin(), 0), std::next(m.begin(), 4), channel,
               numberA, valueA, time);
      checkRpn(std::next(m.begin(), 4), std::next(m.begin(), 8), channel,
               numberB, valueB, time);
      checkNrpn(std::next(m.begin(), 8), std::next(m.begin(), 12), channel,
                numberC, valueC, time);
      checkNrpn(std::next(m.begin(), 12), std::next(m.begin(), 16), channel,
                numberD, valueD, time);
    }

    beginTest("createControllerUpdatesForTime correctly emits (N)RPN messages "
              "on multiple channels");
    {
      ...
      const Info infos[]{
          {2, 1111, 9999}, {8, 8888, 2222}, {5, 7777, 3333}, {1, 6666, 4444}};
      ...
      for (const auto &info : infos) {
        ...
        checkRpn(std::next(m.begin(), 0), std::next(m.begin(), 4), info.channel,
                 info.number, info.value, time);
      }
    }

    const auto messagesAreEqual = [](const MidiMessage &a,
                                     const MidiMessage &b) {
      return std::equal(a.getRawData(), a.getRawData() + a.getRawDataSize(),
                        b.getRawData(), b.getRawData() + b.getRawDataSize());
    };

    beginTest("createControllerUpdatesForTime sends bank select messages when "
              "the next program is in a new bank");
    {
      MidiMessageSequence sequence;
      ...
      const Array<MidiMessage> finalEvents{
          MidiMessage::controllerEvent(channel, 0x00, 50),
      ...
      expect(std::equal(m.begin(), m.end(), finalEvents.begin(),
                        finalEvents.end(), messagesAreEqual));
    }

    beginTest("createControllerUpdatesForTime preserves all Data Increment and "
              "Data Decrement messages");
    {
      ...
      const Array<MidiMessage> messages{
          MidiMessage::controllerEvent(channel, 0x60, 0),
      ...
      expect(std::equal(m.begin(), m.end(), messages.begin(), messages.end(),
                        messagesAreEqual));
    }

    beginTest("createControllerUpdatesForTime does not emit redundant "
              "parameter number changes");
    {
      ...
      const Array<MidiMessage> expected{
          MidiMessage::controllerEvent(channel, 0x63, 50),
          MidiMessage::controllerEvent(channel, 0x62, 10),
          MidiMessage::controllerEvent(channel, 0x06, 10)};

      expect(std::equal(m.begin(), m.end(), expected.begin(), expected.end(),
                        messagesAreEqual));
    }

    beginTest("createControllerUpdatesForTime sets parameter number correctly "
              "at end of sequence");
    {
      MidiMessageSequence sequence;
    ...
    beginTest("createControllerUpdatesForTime does not emit duplicate "
              "parameter number change messages");
    {
      MidiMessageSequence sequence;
      ...
      const Array<MidiMessage> messages{
          MidiMessage::controllerEvent(channel, 0x65, 1),
    ...
    beginTest("createControllerUpdatesForTime emits bank change messages "
              "immediately before program change");
    {
      MidiMessageSequence sequence;
      ...
      const Array<MidiMessage> messages{
          MidiMessage::controllerEvent(channel, 0x00, 1),
