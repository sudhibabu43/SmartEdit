...
std::optional<MidiRPNMessage> MidiRPNDetector::tryParse(int midiChannel,
  ...
  return states[midiChannel - 1].handleController(midiChannel, controllerNumber,
                                                  controllerValue);
}
...
//==============================================================================
std::optional<MidiRPNMessage> MidiRPNDetector::ChannelState::handleController(
    int channel, int controllerNumber, int value) noexcept {
...
//==============================================================================
std::optional<MidiRPNMessage>
MidiRPNDetector::ChannelState::sendIfReady(int channel) noexcept {
...
MidiBuffer MidiRPNGenerator::generate(MidiRPNMessage message) {
  return generate(message.channel, message.parameterNumber, message.value,
                  message.isNRPN, message.is14BitValue);
}

MidiBuffer MidiRPNGenerator::generate(int midiChannel, int parameterNumber,
                                      int value, bool isNRPN,
                                      bool use14BitValue) {
  jassert(midiChannel > 0 && midiChannel <= 16);
  ...
  buffer.addEvent(MidiMessage(channelByte, isNRPN ? 0x62 : 0x64, parameterLSB),
                  0);
  buffer.addEvent(MidiMessage(channelByte, isNRPN ? 0x63 : 0x65, parameterMSB),
                  0);

  buffer.addEvent(MidiMessage(channelByte, 0x06, valueMSB), 0);

  // According to the MIDI spec, whenever a MSB is received, the corresponding
  // LSB will be reset. Therefore, the LSB should be sent after the MSB.
  if (use14BitValue)
...
public:
  ...
  void runTest() override {
    ...
    // If 128 steps of resolution is sufficient the second byte (LSB) of the
    // data value can be omitted. If both the MSB and LSB are sent initially, a
    // subsequent fine adjustment only requires the sending of the LSB. The MSB
    // does not have to be retransmitted. If a subsequent major adjustment is
    // necessary the MSB must be transmitted again. When an MSB is received, the
    // receiver should set its concept of the LSB to zero.
...
public:
  ...
  void runTest() override {
    beginTest("generating RPN/NRPN");
    {
      ...
      {
        MidiBuffer buffer =
            MidiRPNGenerator::generate(16, 101, 34, false, false);
...
private:
  //==============================================================================
  void expectContainsRPN(const MidiBuffer &midiBuffer, int channel,
                         int parameterNumber, int value, bool isNRPN,
                         bool is14BitValue) {
    MidiRPNMessage expected = {channel, parameterNumber, value, isNRPN,
                               is14BitValue};
