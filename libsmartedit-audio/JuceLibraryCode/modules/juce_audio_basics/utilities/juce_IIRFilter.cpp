...
IIRCoefficients &
IIRCoefficients::operator=(const IIRCoefficients &other) noexcept {
  memcpy(coefficients, other.coefficients, sizeof(coefficients));
  return *this;
}

IIRCoefficients::IIRCoefficients(double c1, double c2, double c3, double c4,
                                 double c5, double c6) noexcept {
  const auto a = 1.0 / c4;
...
IIRCoefficients IIRCoefficients::makeLowPass(double sampleRate,
  ...
  const auto n =
      1.0 / std::tan(MathConstants<double>::pi * frequency / sampleRate);
...
IIRCoefficients IIRCoefficients::makeHighPass(double sampleRate,
  ...
  return IIRCoefficients(c1, c1 * -2.0, c1, 1.0, c1 * 2.0 * (nSquared - 1.0),
                         c1 * (1.0 - 1.0 / Q * n + nSquared));
}

IIRCoefficients IIRCoefficients::makeBandPass(double sampleRate,
                                              double frequency) noexcept {
  return makeBandPass(sampleRate, frequency,
                      1.0 / MathConstants<double>::sqrt2);
}
...
IIRCoefficients IIRCoefficients::makeBandPass(double sampleRate,
  ...
  return IIRCoefficients(c1 * n / Q, 0.0, -c1 * n / Q, 1.0,
                         c1 * 2.0 * (1.0 - nSquared),
                         c1 * (1.0 - 1.0 / Q * n + nSquared));
}
...
IIRCoefficients IIRCoefficients::makeNotchFilter(double sampleRate,
                                                 double frequency) noexcept {
  return makeNotchFilter(sampleRate, frequency,
                         1.0 / MathConstants<double>::sqrt2);
}
...
IIRCoefficients IIRCoefficients::makeNotchFilter(double sampleRate,
  ...
  return IIRCoefficients(
      c1 * (1.0 + nSquared), 2.0 * c1 * (1.0 - nSquared), c1 * (1.0 + nSquared),
      1.0, c1 * 2.0 * (1.0 - nSquared), c1 * (1.0 - n / Q + nSquared));
}
...
IIRCoefficients IIRCoefficients::makeAllPass(double sampleRate,
  ...
  return IIRCoefficients(
      c1 * (1.0 - n / Q + nSquared), c1 * 2.0 * (1.0 - nSquared), 1.0, 1.0,
      c1 * 2.0 * (1.0 - nSquared), c1 * (1.0 - n / Q + nSquared));
}
...
IIRCoefficients IIRCoefficients::makeLowShelf(double sampleRate,
  ...
  const auto A =
      std::sqrt(Decibels::gainWithLowerBound(gainFactor, minimumDecibels));
  const auto aminus1 = A - 1.0;
  const auto aplus1 = A + 1.0;
  const auto omega =
      (MathConstants<double>::twoPi * jmax(cutOffFrequency, 2.0)) / sampleRate;
  ...
  return IIRCoefficients(
      ...
      A * (aplus1 - aminus1TimesCoso - beta), aplus1 + aminus1TimesCoso + beta,
      -2.0 * (aminus1 + aplus1 * coso), aplus1 + aminus1TimesCoso - beta);
}
...
IIRCoefficients IIRCoefficients::makePeakFilter(double sampleRate,
  ...
  const auto A =
      std::sqrt(Decibels::gainWithLowerBound(gainFactor, minimumDecibels));
  const auto omega =
      (MathConstants<double>::twoPi * jmax(frequency, 2.0)) / sampleRate;
  ...
  return IIRCoefficients(1.0 + alphaTimesA, c2, 1.0 - alphaTimesA,
                         1.0 + alphaOverA, c2, 1.0 - alphaOverA);
}
...
//==============================================================================
template <typename Mutex> void IIRFilterBase<Mutex>::makeInactive() noexcept {
  const typename Mutex::ScopedLockType sl(processLock);
  active = false;
}

template <typename Mutex>
void IIRFilterBase<Mutex>::setCoefficients(
...
//==============================================================================
template <typename Mutex> void IIRFilterBase<Mutex>::reset() noexcept {
  const typename Mutex::ScopedLockType sl(processLock);
  v1 = v2 = 0.0;
}
...
float IIRFilterBase<Mutex>::processSingleSampleRaw(float in) noexcept {
  ...
  v1 = coefficients.coefficients[1] * in - coefficients.coefficients[3] * out +
       v2;
...
template <typename Mutex>
void IIRFilterBase<Mutex>::processSamples(float *const samples,
