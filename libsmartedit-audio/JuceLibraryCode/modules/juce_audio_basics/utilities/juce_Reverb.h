...
/**
    ...
    This is a simple stereo reverb, based on the technique and tunings used in
   FreeVerb. Use setSampleRate() to prepare it, and then call processStereo() or
   processMono() to apply the reverb to your audio data.
...
public:
  ...
  struct Parameters {
    float roomSize =
        0.5f; /**< Room size, 0 to 1.0, where 1.0 is big, 0 is small. */
    float damping = 0.5f; /**< Damping, 0 to 1.0, where 0 is not damped, 1.0 is
                             fully damped. */
  ...
  /** Applies a new set of parameters to the reverb.
      Note that this doesn't attempt to lock the reverb, so if you call this in
     parallel with the process method, you may get artifacts.
  */
  ...
  /** Sets the sample rate that will be used for the reverb.
      You must call this before the process methods, in order to tell it the
     correct sample rate.
  */
  void setSampleRate(const double sampleRate) {
    ...
    for (int i = 0; i < numCombs; ++i) {
      ...
      comb[1][i].setSize((intSampleRate * (combTunings[i] + stereoSpread)) /
                         44100);
    }
    ...
    for (int i = 0; i < numAllPasses; ++i) {
      ...
      allPass[1][i].setSize(
          (intSampleRate * (allPassTunings[i] + stereoSpread)) / 44100);
    }
  ...
  void processStereo(float *const left, float *const right,
    ...
    for (int i = 0; i < numSamples; ++i) {
      ...
      for (int j = 0; j < numCombs;
           ++j) // accumulate the comb filters in parallel
      {
        outL += comb[0][j].process(input, damp, feedbck);
        outR += comb[1][j].process(input, damp, feedbck);
      }

      for (int j = 0; j < numAllPasses;
           ++j) // run the allpass filters in series
      {
...
private:
  ...
  void setDamping(const float dampingToUse,
  ...
  public:
    ...
    float process(const float input, const float damp,
  ...
  public:
    ...
    void clear() noexcept { buffer.clear((size_t)bufferSize); }
