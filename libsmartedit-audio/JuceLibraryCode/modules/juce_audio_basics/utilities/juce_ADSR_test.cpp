...
struct ADSRTests final : public UnitTest {
  ...
  void runTest() override {
    ...
    beginTest("Sustain");
    {
      ...
      adsr.noteOn();
      advanceADSR(adsr,
                  roundToInt((parameters.attack + parameters.decay + 0.01) *
                             sampleRate));
      ...
      for (int numTests = 0; numTests < 100; ++numTests) {
        ...
        adsr.setParameters({parameters.attack, parameters.decay, sustainLevel,
                            parameters.release});
    ...
    beginTest("Release");
    {
      ...
      adsr.noteOn();
      advanceADSR(adsr, roundToInt((parameters.attack + parameters.decay) *
                                   sampleRate));
      adsr.noteOff();
    ...
    beginTest("Zero-length attack jumps to decay");
    {
      adsr.reset();
      adsr.setParameters(
          {0.0f, parameters.decay, parameters.sustain, parameters.release});
    ...
    beginTest("Zero-length decay jumps to sustain");
    {
      adsr.reset();
      adsr.setParameters(
          {parameters.attack, 0.0f, parameters.sustain, parameters.release});
    ...
    beginTest("Zero-length release resets to idle");
    {
      adsr.reset();
      adsr.setParameters(
          {parameters.attack, parameters.decay, parameters.sustain, 0.0f});
  ...
  static AudioBuffer<float> getTestBuffer(double sampleRate,
  ...
  static bool isSustained(const AudioBuffer<float> &b, float sustainLevel) {
    ...
    for (int channel = 0; channel < b.getNumChannels(); ++channel)
      if (b.findMinMax(channel, 0, b.getNumSamples()) !=
          Range<float>{sustainLevel, sustainLevel})
        return false;
