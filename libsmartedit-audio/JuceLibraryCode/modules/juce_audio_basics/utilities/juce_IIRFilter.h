...
public:
  ...
  /** Directly constructs an object from the raw coefficients.
      ...
      filters!
  */
  IIRCoefficients(double c1, double c2, double c3, double c4, double c5,
                  double c6) noexcept;
  ...
  static IIRCoefficients makeLowPass(double sampleRate, double frequency,
                                     double Q) noexcept;
  ...
  static IIRCoefficients makeHighPass(double sampleRate, double frequency,
                                      double Q) noexcept;
  ...
  static IIRCoefficients makeBandPass(double sampleRate, double frequency,
                                      double Q) noexcept;
  ...
  static IIRCoefficients makeNotchFilter(double sampleRate, double frequency,
                                         double Q) noexcept;
  ...
  static IIRCoefficients makeAllPass(double sampleRate, double frequency,
                                     double Q) noexcept;
  ...
  */
  static IIRCoefficients makeLowShelf(double sampleRate, double cutOffFrequency,
                                      double Q, float gainFactor) noexcept;
  ...
  */
  static IIRCoefficients makePeakFilter(double sampleRate,
                                        double centreFrequency, double Q,
                                        float gainFactor) noexcept;
...
/**
    ...
    @tags{Audio}
*/
template <typename Mutex> class JUCE_API IIRFilterBase {
public:
  //==============================================================================
  /** Creates a filter.
  ...
  */
  ...
  /** Resets the filter's processing pipeline, ready to start a new stream of
     data.
      ...
      its coefficients aren't changed. To put a filter into an inactive state,
     use the makeInactive() method.
  */
  void reset() noexcept;
...
protected:
  ...
  // The exact meaning of an assignment operator would be ambiguous since the
  // filters are stateful. If you want to copy the coefficients, then just use
  // setCoefficients().
...
/**
    ...
    If you can guarantee that calls to the filter will be synchronised
   externally, you could consider switching to SingleThreadedIIRFilter instead.
