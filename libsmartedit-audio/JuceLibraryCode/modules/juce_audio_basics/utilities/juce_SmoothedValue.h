  ...
  */
  void applyGain(FloatType *samplesOut, const FloatType *samplesIn,
    ...
    if (isSmoothing()) {
      ...
      FloatVectorOperations::multiply(samplesOut, samplesIn, target,
                                      numSamples);
    }
  }
...
public:
  ...
  SmoothedValue() noexcept
      : SmoothedValue((FloatType)(std::is_same_v<SmoothingType,
  ...
  SmoothedValue(FloatType initialValue) noexcept {
    ...
    jassert(
        !(std::is_same_v<SmoothingType, ValueSmoothingTypes::Multiplicative> &&
  ...
  */
  ...
  /** Using the new methods:
      ...
      @param newValue     The new target value
      @param force        If true, the value will be set immediately, bypassing
     the ramp
  */
  [[deprecated(
      "Use setTargetValue and setCurrentAndTargetValue instead.")]] void
...
private:
  //==============================================================================
  template <typename T = SmoothingType> void setStepSize() noexcept {
  ...
  //==============================================================================
  template <typename T = SmoothingType> void setNextValue() noexcept {
    if constexpr (std::is_same_v<T, ValueSmoothingTypes::Linear>) {
...
template <typename FloatType>
using LinearSmoothedValue =
    SmoothedValue<FloatType, ValueSmoothingTypes::Linear>;
...
public:
  CommonSmoothedValueTests()
      : UnitTest("CommonSmoothedValueTests",
                 UnitTestCategories::smoothedValues) {}
  ...
  void runTest() override {
    ...
    beginTest("Block processing");
    {
      ...
      auto compareData = [this](const AudioBuffer<float> &test,
        ...
        for (int i = 0; i < test.getNumSamples(); ++i)
          expectWithinAbsoluteError(test.getSample(0, i),
                                    reference.getSample(0, i), 2.0e-7f);
      };
      ...
      sv.setCurrentAndTargetValue(1.0f);
      sv.setTargetValue(2.0f);
      sv.applyGain(destData.getWritePointer(0), testData.getReadPointer(0),
                   numSamples);
