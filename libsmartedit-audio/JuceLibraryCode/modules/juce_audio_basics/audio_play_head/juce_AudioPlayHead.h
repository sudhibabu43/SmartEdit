...
public:
  ...
  public:
    ...
    */
    ...
    /** Returns true if the effective framerate is actually equal to the base
     * rate divided by 1.001 */
    bool isPullDown() const { return pulldown; }

    /** Returns the actual rate described by this object, taking pulldown into
     * account. */
    double getEffectiveRate() const {
      return pulldown ? (double)base / 1.001 : (double)base;
    }

    /** Returns a copy of this object with the specified base rate. */
    [[nodiscard]] FrameRate withBaseRate(int x) const {
      return with(&FrameRate::base, x);
    }

    /** Returns a copy of this object with drop frames enabled or disabled, as
     * specified. */
    [[nodiscard]] FrameRate withDrop(bool x = true) const {
      return with(&FrameRate::drop, x);
    }

    /** Returns a copy of this object with pulldown enabled or disabled, as
     * specified. */
    [[nodiscard]] FrameRate withPullDown(bool x = true) const {
      return with(&FrameRate::pulldown, x);
    }
  ...
  */
  ...
  /** Holds the begin and end points of a looped region.
      ...
      @see PositionInfo::getIsLooping() PositionInfo::setIsLooping()
     PositionInfo::getLoopPoints() PositionInfo::setLoopPoints()
  */
  ...
  /** This type is deprecated; prefer PositionInfo instead.
      ...
      Some position info may be unavailable, depending on the host or plugin
     format. Unfortunately, CurrentPositionInfo doesn't have any way of
     differentiating between default values and values that have been set
     explicitly.
  */
  struct JUCE_API CurrentPositionInfo {
    ...
    /** For timecode, the position of the start of the timeline, in seconds from
     * 00:00:00:00. */
    ...
    /** The position of the start of the last bar, in units of quarter-notes.
        ...
        This is the time from the start of the timeline to the start of the
       current bar, in ppq units.
    ...
    */
    ...
    bool operator==(const CurrentPositionInfo &other) const noexcept {
      const auto tie = [](const CurrentPositionInfo &i) {
        return std::tie(i.timeInSamples, i.ppqPosition, i.editOriginTime,
                        i.ppqPositionOfLastBarStart, i.frameRate, i.isPlaying,
                        i.isRecording, i.bpm, i.timeSigNumerator,
                        i.timeSigDenominator, i.ppqLoopStart, i.ppqLoopEnd,
                        i.isLooping);
      };
  ...
  /**
      ...
      an Optional that will only be engaged if the host provides the
     corresponding information. As a plugin developer, you should code
     defensively so that the plugin behaves sensibly even when the host fails to
  ...
  */
  ...
  public:
    ...
    */
    Optional<int64_t> getBarCount() const {
      return getOptional(flagBarCount, barCount);
    }

    /** @see getBarCount() */
    void setBarCount(Optional<int64_t> barCountIn) {
      setOptional(flagBarCount, barCount, barCountIn);
    }
    ...
    */
    ...
    void setIsRecording(bool isRecordingIn) {
      setFlag(flagIsRecording, isRecordingIn);
    }
    ...
    bool operator==(const PositionInfo &other) const noexcept {
      const auto tie = [](const PositionInfo &i) {
        return std::make_tuple(
            i.getTimeInSamples(), i.getTimeInSeconds(), i.getPpqPosition(),
            i.getEditOriginTime(), i.getPpqPositionOfLastBarStart(),
            i.getFrameRate(), i.getBarCount(), i.getTimeSignature(), i.getBpm(),
            i.getLoopPoints(), i.getHostTimeNs(), i.getIsPlaying(),
            i.getIsRecording(), i.getIsLooping());
      };
  ...
  /** Deprecated, use getPosition() instead.
      ...
      position at the start of the current processing block. If this method
     returns false then the current play head position is not available and the
     given structure will be undefined.

      You can ONLY call this from your processBlock() method! Calling it at
     other times will produce undefined behaviour, as the host may not have any
     context in which a time would make sense, and some hosts will almost
     certainly have multithreading issues if it's not called on the audio
     thread.
  */
  [[deprecated("Use getPosition instead. Not all hosts are able to provide all "
               "time position information; getPosition differentiates clearly "
               "between set and unset fields.")]] bool
  ...
  /** Fetches details about the transport's position at the start of the current
      processing block. If this method returns nullopt then the current play
     head position is not available.
