...
constexpr static const int magicNumber =
    (int)ByteOrder::makeInt('P', 'R', 'O', 'P');
constexpr static const int magicNumberCompressed =
    (int)ByteOrder::makeInt('C', 'P', 'R', 'P');
...
PropertiesFile::Options::Options()
    : commonToAllUsers(false), ignoreCaseOfKeyNames(false), doNotSave(false),
      millisecondsBeforeSaving(3000), storageFormat(PropertiesFile::storeAsXML),
      processLock(nullptr) {}
...
File PropertiesFile::Options::getDefaultFile() const {
  ...
  File dir(commonToAllUsers ? "/Library/" : "~/Library/");
  ...
  if (osxLibrarySubFolder != "Preferences" &&
    ...
    /* The PropertiesFile class always used to put its settings files in
       ...
       stipulate that settings should go in "Library/Application Support", or
       Library/Containers/[app_bundle_id] for a sandboxed app.
  ...
  auto dir =
      File(commonToAllUsers ? "/var" : "~")
          .getChildFile(folderName.isNotEmpty() ? folderName
                                                : ("." + applicationName));

#elif JUCE_WINDOWS
  auto dir = File::getSpecialLocation(commonToAllUsers
  ...
  dir =
      dir.getChildFile(folderName.isNotEmpty() ? folderName : applicationName);
#endif

  return (
      filenameSuffix.startsWithChar(L'.')
...
PropertiesFile::PropertiesFile(const File &f, const Options &o)
    : PropertySet(o.ignoreCaseOfKeyNames), file(f), options(o) {
  reload();
}

PropertiesFile::PropertiesFile(const Options &o)
    : PropertySet(o.ignoreCaseOfKeyNames), file(o.getDefaultFile()),
      options(o) {
  reload();
}
...
PropertiesFile::~PropertiesFile() { saveIfNeeded(); }
      ...
      if (name.isNotEmpty())
        getAllProperties().set(
            ...
            e->getFirstChildElement() != nullptr
                ? e->getFirstChildElement()->toString(
                      XmlElement::TextFormat().singleLine().withoutHeader())
...
bool PropertiesFile::saveAsXml() {
  ...
  for (int i = 0; i < props.size(); ++i) {
    ...
    e->setAttribute(PropertyFileConstants::nameAttribute,
                    props.getAllKeys()[i]);
...
bool PropertiesFile::writeToStream(OutputStream &out) {
  ...
  for (int i = 0; i < numProperties; ++i) {
    if (!out.writeString(keys[i]))
      return false;
    if (!out.writeString(values[i]))
      return false;
  }
...
void PropertiesFile::timerCallback() { saveIfNeeded(); }
