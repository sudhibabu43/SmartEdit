...
bool UndoManager::perform(UndoableAction *newAction) {
  if (newAction != nullptr) {
    ...
    if (isPerformingUndoRedo()) {
      jassertfalse; // Don't call perform() recursively from the
                    // UndoableAction::perform() or undo() methods, or else
                    // these actions will be discarded!
      return false;
    }
    ...
    if (action->perform()) {
      ...
      if (actionSet != nullptr && !newTransaction) {
        if (auto *lastAction = actionSet->actions.getLast()) {
          if (auto coalescedAction =
                  lastAction->createCoalescedAction(action.get())) {
...
void UndoManager::beginNewTransaction() { beginNewTransaction({}); }
...
//==============================================================================
UndoManager::ActionSet *UndoManager::getCurrentSet() const {
  return transactions[nextIndex - 1];
}
UndoManager::ActionSet *UndoManager::getNextSet() const {
...
void UndoManager::getActionsInCurrentTransaction(
