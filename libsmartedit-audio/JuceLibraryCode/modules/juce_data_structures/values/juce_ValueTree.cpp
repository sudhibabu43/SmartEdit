...
public:
  ...
  SharedObject(const SharedObject &other)
      : ReferenceCountedObject(), type(other.type),
  ...
  ~SharedObject() override {
    jassert(parent == nullptr); // this should never happen unless something
                                // isn't obeying the ref-counting!
  ...
  void callListeners(ValueTree::Listener *listenerToExclude,
                     Function fn) const {
    auto numListeners = valueTreesWithListeners.size();

    if (numListeners == 1) {
      valueTreesWithListeners.getUnchecked(0)->listeners.callExcluding(
  ...
  template <typename Function>
  void callListenersForAllParents(ValueTree::Listener *listenerToExclude,
                                  Function fn) const {
  ...
  void
  sendPropertyChangeMessage(const Identifier &property,
  ...
  void sendChildRemovedMessage(ValueTree child, int index) {
    ValueTree tree(*this);
    callListenersForAllParents(nullptr, [=, &tree, &child](Listener &l) {
  ...
  void sendChildOrderChangedMessage(int oldIndex, int newIndex) {
    ValueTree tree(*this);
    callListenersForAllParents(nullptr, [=, &tree](Listener &l) {
      l.valueTreeChildOrderChanged(tree, oldIndex, newIndex);
    });
  }
  ...
  void sendParentChangeMessage() {
    ...
    callListeners(nullptr,
                  [&](Listener &l) { l.valueTreeParentChanged(tree); });
  }
  ...
  void setProperty(const Identifier &name, const var &newValue,
    ...
    if (undoManager == nullptr) {
      ...
      if (auto *existingValue = properties.getVarPointer(name)) {
        ...
        undoManager->perform(new SetPropertyAction(
            *this, name, newValue, {}, true, false, listenerToExclude));
      }
    }
  }
  ...
  void removeProperty(const Identifier &name, UndoManager *undoManager) {
    if (undoManager == nullptr) {
      ...
      if (properties.contains(name))
        undoManager->perform(new SetPropertyAction(
            *this, name, {}, properties[name], false, true));
    }
  }
  ...
  void copyPropertiesFrom(const SharedObject &source,
                          UndoManager *undoManager) {
    for (auto i = properties.size(); --i >= 0;)
    ...
    for (int i = 0; i < source.properties.size(); ++i)
      setProperty(source.properties.getName(i), source.properties.getValueAt(i),
                  undoManager);
  }
  ...
  ValueTree getOrCreateChildWithName(const Identifier &typeToMatch,
                                     UndoManager *undoManager) {
    for (auto *s : children)
      if (s->type == typeToMatch)
        return ValueTree(*s);
  ...
  void addChild(SharedObject *child, int index, UndoManager *undoManager) {
    if (child != nullptr && child->parent != this) {
      if (child != this && !isAChildOf(child)) {
        // You should always make sure that a child is removed from its previous
        // parent before adding it somewhere else - otherwise, it's ambiguous as
        // to whether a different undomanager should be used when removing it
        // from its current parent..
        jassert(child->parent == nullptr);

        if (child->parent != nullptr) {
          jassert(child->parent->children.indexOf(child) >= 0);
          child->parent->removeChild(child->parent->children.indexOf(child),
                                     undoManager);
        }
  ...
  void moveChild(int currentIndex, int newIndex, UndoManager *undoManager) {
    ...
    if (currentIndex != newIndex &&
      ...
      if (undoManager == nullptr) {
        ...
        undoManager->perform(
            new MoveChildAction(*this, currentIndex, newIndex));
      }
    }
  }

  void reorderChildren(const OwnedArray<ValueTree> &newOrder,
                       UndoManager *undoManager) {
  ...
  bool isEquivalentTo(const SharedObject &other) const noexcept {
    if (type != other.type || properties.size() != other.properties.size() ||
        children.size() != other.children.size() ||
        properties != other.properties)
      return false;

    for (int i = 0; i < children.size(); ++i)
      if (!children.getObjectPointerUnchecked(i)->isEquivalentTo(
  ...
  XmlElement *createXml() const {
    ...
    for (auto i = children.size(); --i >= 0;)
      xml->prependChildElement(
          children.getObjectPointerUnchecked(i)->createXml());
  ...
  static void writeObjectToStream(OutputStream &output,
  ...
  struct SetPropertyAction final : public UndoableAction {
    SetPropertyAction(Ptr targetObject, const Identifier &propertyName,
                      const var &newVal, const var &oldVal, bool isAdding,
                      bool isDeleting,
                      ValueTree::Listener *listenerToExclude = nullptr)
        : target(std::move(targetObject)), name(propertyName), newValue(newVal),
          oldValue(oldVal), isAddingNewProperty(isAdding),
          isDeletingProperty(isDeleting), excludeListener(listenerToExclude) {}
      ...
      if (!(isAddingNewProperty || isDeletingProperty)) {
        if (auto *next = dynamic_cast<SetPropertyAction *>(nextAction))
          if (next->target == target && next->name == name &&
            ...
            return new SetPropertyAction(*target, name, next->newValue,
  ...
  struct AddOrRemoveChildAction final : public UndoableAction {
    AddOrRemoveChildAction(Ptr parentObject, int index, SharedObject *newChild)
        : target(std::move(parentObject)),
          child(newChild != nullptr ? newChild
                                    : target->children.getObjectPointer(index)),
          childIndex(index), isDeleting(newChild == nullptr) {
    ...
    bool undo() override {
      if (isDeleting) {
        ...
        // If you hit this, it seems that your object's state is getting
        // confused - probably because you've interleaved some undoable and
  ...
  struct MoveChildAction final : public UndoableAction {
    MoveChildAction(Ptr parentObject, int fromIndex, int toIndex) noexcept
        : parent(std::move(parentObject)), startIndex(fromIndex),
          endIndex(toIndex) {}
...
ValueTree::ValueTree(const Identifier &type)
    : object(new ValueTree::SharedObject(type)) {
  jassert(type.toString()
              .isNotEmpty()); // All objects must be given a sensible type name!
}
...
void ValueTree::copyPropertiesFrom(const ValueTree &source,
  ...
  jassert(
      object != nullptr ||
      source.object ==
          nullptr); // Trying to add properties to a null ValueTree will fail!
...
void ValueTree::copyPropertiesAndChildrenFrom(const ValueTree &source,
  ...
  jassert(object != nullptr ||
          source.object ==
              nullptr); // Trying to copy to a null ValueTree will fail!
...
const var *
ValueTree::getPropertyPointer(const Identifier &name) const noexcept {
  return object == nullptr ? nullptr : object->properties.getVarPointer(name);
}
...
ValueTree &ValueTree::setPropertyExcludingListener(Listener *listenerToExclude,
  ...
  jassert(object !=
          nullptr); // Trying to add a property to a null ValueTree will fail!
...
Identifier ValueTree::getPropertyName(int index) const noexcept {
  return object == nullptr ? Identifier() : object->properties.getName(index);
}
...
struct ValueTreePropertyValueSource final : public Value::ValueSource,
  ...
  ValueTreePropertyValueSource(const ValueTree &vt, const Identifier &prop,
                               UndoManager *um, bool sync)
      : tree(vt), property(prop), undoManager(um), updateSynchronously(sync) {
    tree.addListener(this);
  }

  ~ValueTreePropertyValueSource() override { tree.removeListener(this); }
...
ValueTree::Iterator::Iterator(const ValueTree &v, bool isEnd)
    : internal(v.object != nullptr ? (isEnd ? v.object->children.end()
...
bool ValueTree::Iterator::operator==(const Iterator &other) const {
...
ValueTree::Iterator ValueTree::begin() const noexcept {
  return Iterator(*this, false);
}
ValueTree::Iterator ValueTree::end() const noexcept {
...
ValueTree ValueTree::getOrCreateChildWithName(const Identifier &type,
                                              UndoManager *undoManager) {
  return object != nullptr ? object->getOrCreateChildWithName(type, undoManager)
                           : ValueTree();
}
...
void ValueTree::moveChild(int currentIndex, int newIndex,
                          UndoManager *undoManager) {
  if (object != nullptr)
...
void ValueTree::reorderChildren(const OwnedArray<ValueTree> &newOrder,
                                UndoManager *undoManager) {
  if (object != nullptr)
...
std::unique_ptr<XmlElement> ValueTree::createXml() const {
  return std::unique_ptr<XmlElement>(object != nullptr ? object->createXml()
                                                       : nullptr);
}
...
void ValueTree::Listener::valueTreePropertyChanged(ValueTree &,
                                                   const Identifier &) {}
...
public:
  ValueTreeTests() : UnitTest("ValueTrees", UnitTestCategories::values) {}
  ...
  static ValueTree createRandomTree(UndoManager *undoManager, int depth,
    ...
    for (int i = r.nextInt(10); --i >= 0;) {
      switch (r.nextInt(5)) {
      case 0:
        v.setProperty(createRandomIdentifier(r), createRandomWideCharString(r),
                      undoManager);
        break;
      case 1:
      ...
      case 2:
        if (depth < 5)
          v.addChild(createRandomTree(undoManager, depth + 1, r),
                     r.nextInt(v.getNumChildren() + 1), undoManager);
        break;
      case 3:
  ...
  void runTest() override {
    {
      ...
      for (int i = 10; --i >= 0;) {
        ...
        expect(v1.isEquivalentTo(ValueTree::readFromGZIPData(
            zipped.getData(), zipped.getDataSize())));
