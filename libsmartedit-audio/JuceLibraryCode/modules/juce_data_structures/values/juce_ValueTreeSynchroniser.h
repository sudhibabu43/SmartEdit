...
/**
    ...
    and implement the stateChanged() method to transmit the encoded change
   (maybe via a network or other means) to a remote destination, where it can be
...
public:
  ...
  */
  ...
  /** This callback happens when the ValueTree changes and the given
     state-change message needs to be applied to any other trees that need to
     stay in sync with it. The data is an opaque blob of binary that you should
     transmit to wherever your target tree lives, and use applyChange() to apply
     this to the target tree.
  */
  virtual void stateChanged(const void *encodedChange,
  ...
  /** Forces the sending of a full state message, which may be large, as it
      ...
      This will internally invoke stateChanged() with the encoded version of the
     state.
  */
  ...
  /** Applies an encoded change to the given destination tree.
      ...
      When you implement a receiver for changes that were sent by the
     stateChanged() message, this is the function that you'll need to call to
     apply them to the target tree that you want to be synced.
  */
  static bool applyChange(ValueTree &target, const void *encodedChangeData,
