...
void Value::ValueSource::handleAsyncUpdate() { sendChangeMessage(true); }
...
public:
  ...
  SimpleValueSource(const var &initialValue) : value(initialValue) {}
...
Value::Value(ValueSource *const v) : value(v) { jassert(v != nullptr); }
...
Value::~Value() { removeFromListenerList(); }

void Value::removeFromListenerList() {
  if (listeners.size() > 0 &&
      value != nullptr) // may be nullptr after a move operation
...
void Value::setValue(const var &newValue) { value->setValue(newValue); }
