# The SmartEdit Audio Library

## Getting Started

The best way to get started with libsmartedit-audio is to
...
and supports the cross-platform compiling of libsmartedit and libsmartedit-audio.
...
The following libraries are required to build libsmartedit-audio.
...
*   Used to automate the generation of Makefiles, check for dependencies,
    and is the backbone of libsmartedit-audio’s cross-platform build process.
...
*   Used to auto-generate the documentation used by libsmartedit-audio.
...
The first step in installing libsmartedit-audio is to obtain the source code.
The source code is available on
[GitHub](https://github.com/SmartEdit/libsmartedit-audio).
...
```sh
./build/src/smartedit-audio-demo
```
...
If your next step is to build libsmartedit, installation is optional here.
libsmartedit-audio can be used from the build directory by adding
`-DSmartEditAudio_ROOT="/path/to/libsmartedit-audio/build"`
to the `cmake` command line, when configuring libsmartedit.
