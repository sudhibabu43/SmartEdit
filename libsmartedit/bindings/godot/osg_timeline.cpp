...
void ExampleClass::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_image", "frame_number"),
                       &ExampleClass::get_image);
}
...
ExampleClass::ExampleClass() {
  ...
  timeline = new smartedit::Timeline(1920, 1080, smartedit::Fraction(30, 1),
                                     44100, 2, smartedit::LAYOUT_STEREO);
...
void ExampleClass::load_file(const String path) {
  if (reader == nullptr) {
    ...
    reader = new smartedit::FFmpegReader(path.utf8().get_data(), true);
    reader->Open();
  }
}
...
void ExampleClass::print_json(const Variant &p_variant) {
  ...
  smartedit::Profile p(
      "/home/jonathan/apps/smartedit-qt/src/profiles/01920x1080p2997_16-09");
  std::string s = timeline->Json();
  String output = "SmartEdit Profile JSON: " + String(s.c_str());
...
Ref<Image> ExampleClass::get_image(const int64_t frame_number) {
  if (reader && reader->IsOpen()) {
    ...
    print_line(vformat("✅ Image created: %dx%d (premultiplied alpha)", width,
                       height));
    return image;
  }
