...
The best way to get started with libsmartedit is to learn about our build system,
...
The following libraries are required to build libsmartedit.
...
#### SmartEdit Audio Library (libsmartedit-audio)
*   <https://github.com/SmartEdit/libsmartedit-audio/> **(Library)**
...
*   This library is used to communicate between libsmartedit and other applications (publisher / subscriber).
    Primarily used to send debug data from libsmartedit.

#### OpenMP (`-fopenmp`)
*   <http://openmp.org/wp/> **(Compiler Flag)**

*   If your compiler supports this flag (GCC, Clang, and most other compilers),
    it provides libsmartedit with implementations of common parallel programming techniques
...
*   This executable is used to automate the generation of Makefiles,
    check for dependencies, and is the backbone of libsmartedit’s cross-platform build process.

#### SWIG (`swig`)
*   <http://www.swig.org/> **(Executable)**

*   This executable is used to generate the Python and Ruby bindings for libsmartedit.
    It is a powerful wrapper for C++ libraries, and supports many languages.

#### Python 3 (libpython)
*   <http://www.python.org/> **(Executable and Library)**

*   This library is used by swig to create the Python (version 3+) bindings for libsmartedit.
    This is also the official language used by SmartEdit Video Editor (a graphical interface to libsmartedit).

#### Doxygen (doxygen)
*   <http://www.stack.nl/~dimitri/doxygen/> **(Executable)**
*   This executable is used to auto-generate the documentation used by libsmartedit.

#### Catch2 (catch)
*   <https://github.com/catchorg/Catch2/> **(Library)**

*   This library is used to execute unit tests for libsmartedit.
    It contains many macros used to keep our unit testing code clean and easy-to-follow.

## Obtaining Source Code

The first step in installing libsmartedit is to obtain the most recent source code.
The source code is available on [GitHub](https://github.com/SmartEdit/libsmartedit).
Use the following command to obtain the latest libsmartedit source code.
...
#### `src/`
This folder contains all source code (`*.cpp`) and headers (`*.h`) for libsmartedit.

#### `bindings/`
This folder contains language bindings for the libsmartedit API.
...
#### `thirdparty/`
This folder contains code not written by the SmartEdit team.
...
We use CMake's Makefile generators to compile libsmartedit and libsmartedit-audio.
...
It may be possible to build libsmartedit using other compiler stacks,
but most likely not without modifications to the build system which you would have to make yourself.

### CMake Flags (Optional)
There are many different build flags that can be passed to cmake to adjust how libsmartedit is compiled.
...
## Linux Build Instructions (libsmartedit-audio)
To compile libsmartedit-audio, we need to build it from source code and install the results.
Launch a terminal and enter:

```sh
cd [libsmartedit-audio repo folder]
cmake -B build -S .
cmake --build build
./build/src/smartedit-audio-test-sound  (This should play a test sound)
cmake --install build
```

## Linux Build Instructions (libsmartedit)
Run the following commands to compile libsmartedit:

```sh
cd [libsmartedit repo directory]
cmake -B build -S .
cmake --build build
...
[INSTALL-LINUX]: https://github.com/SmartEdit/libsmartedit/blob/develop/doc/INSTALL-LINUX.md
[INSTALL-MAC]: https://github.com/SmartEdit/libsmartedit/blob/develop/doc/INSTALL-MAC.md
[INSTALL-WINDOWS]: https://github.com/SmartEdit/libsmartedit/blob/develop/doc/INSTALL-WINDOWS.md
